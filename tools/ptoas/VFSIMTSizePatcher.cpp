...
constexpr uint64_t kVFSIMTVariableMask = kVFSIMTSizeMask | (kScalarRegisterMask << 16) | (kScalarRegisterMask << 11) |
                                         kVFSIMTJoinOffsetMask | kVFSIMTExitModeMask;
constexpr uint64_t kVFSIMTFixedMask = ~kVFSIMTVariableMask;
constexpr uint64_t kVFSIMTFixedBits = UINT64_C(0x15e0001c15200400) & kVFSIMTFixedMask;
...
template <typename T>
static std::optional<T> takeExpected(llvm::Expected<T> value, llvm::raw_ostream& diagOS, const llvm::Twine& context)
{
    if (value)
        return std::move(*value);
    diagOS << "Error: VF_SIMT size patch: " << context << ": " << llvm::toString(value.takeError()) << "\n";
    return std::nullopt;
}
...
static FailureOr<llvm::SmallVector<SimtCallSite, 4>> collectManifest(llvm::Module& module, llvm::raw_ostream& diagOS)
{
    ...
    for (llvm::Function& caller : module) {
        if (caller.isDeclaration() || caller.getCallingConv() == llvm::CallingConv::SimtEntry)
            continue;
        for (llvm::BasicBlock& block : caller) {
            for (llvm::Instruction& instruction : block) {
                ...
                if (!callee || callee->isDeclaration()) {
                    emitError(
                        diagOS,
                        llvm::Twine("caller '") + caller.getName() + "' contains an indirect or undefined SIMT call");
                    return failure();
                }
                if (callee->getCallingConv() != llvm::CallingConv::SimtEntry) {
                    emitError(
                        diagOS, llvm::Twine("callee '") + callee->getName() +
                                    "' does not use the SIMT entry calling convention");
                    return failure();
                }
                manifest.push_back(
                    {caller.getName().str(), getSIMTEntrySymbolName(callee->getName()),
...
static FailureOr<llvm::StringMap<ELFFunction>> readFunctions(
    const llvm::object::ELFObjectFileBase& object, llvm::ArrayRef<SimtCallSite> manifest, llvm::raw_ostream& diagOS)
{
    ...
    if (symbolTables != 1) {
        emitError(diagOS, llvm::Twine("expected exactly one symbol table, found ") + llvm::Twine(symbolTables));
        return failure();
    }
    if (executableSections != 1) {
        emitError(
            diagOS, llvm::Twine("expected exactly one executable section, found ") + llvm::Twine(executableSections));
        return failure();
    }
    ...
    for (const llvm::object::ELFSymbolRef symbol : object.symbols()) {
        if (symbol.getELFType() != llvm::ELF::STT_FUNC)
            continue;
        auto name = takeExpected(symbol.getName(), diagOS, "failed to read symbol name");
        if (!name)
            return failure();
        if (!requiredFunctions.contains(*name))
            continue;
        auto address =
            takeExpected(symbol.getAddress(), diagOS, llvm::Twine("failed to read address for '") + *name + "'");
        auto sectionIt =
            takeExpected(symbol.getSection(), diagOS, llvm::Twine("failed to read section for '") + *name + "'");
        ...
        llvm::object::ELFSectionRef elfSection(section);
        if ((elfSection.getFlags() & llvm::ELF::SHF_EXECINSTR) == 0) {
            emitError(diagOS, llvm::Twine("function '") + *name + "' is not in an executable section");
            return failure();
        }
        ...
        if (size == 0 || *address < sectionAddress || *address - sectionAddress > sectionSize ||
            size > sectionSize - (*address - sectionAddress)) {
            emitError(diagOS, llvm::Twine("function '") + *name + "' has an invalid ELF symbol range");
            return failure();
        }
        ELFFunction function{name->str(),       *address, size, sectionAddress, sectionSize, elfSection.getOffset(),
                             section.getIndex()};
        if (!functions.try_emplace(function.name, std::move(function)).second) {
            emitError(diagOS, llvm::Twine("duplicate function symbol '") + *name + "'");
            return failure();
        }
    }
    return functions;
...
static bool isVFSIMT(uint64_t instruction) { return (instruction & kVFSIMTFixedMask) == kVFSIMTFixedBits; }

static std::optional<uint16_t> decodeMOVKChunk(uint32_t instruction, unsigned chunk, unsigned targetRegister)
{
    ...
    if ((instruction & kMOVKFixedMask) != expected ||
        ((instruction >> kScalarDestinationShift) & kScalarRegisterMask) != targetRegister)
        return std::nullopt;
    return static_cast<uint16_t>(instruction);
}

static std::optional<uint64_t> decodeTargetAddress(
    StringRef bytes, uint64_t callerAddress, uint64_t instructionOffset, uint64_t instruction)
{
    ...
    const unsigned targetRegister = (instruction >> kVFSIMTRegisterShift) & kScalarRegisterMask;
    const uint64_t movPc =
        llvm::support::endian::read32le(reinterpret_cast<const uint8_t*>(bytes.data() + instructionOffset - 24));
    const uint64_t movi =
        llvm::support::endian::read32le(reinterpret_cast<const uint8_t*>(bytes.data() + instructionOffset - 20));
    const uint32_t movk1 =
        llvm::support::endian::read32le(reinterpret_cast<const uint8_t*>(bytes.data() + instructionOffset - 16));
    const uint32_t movk2 =
        llvm::support::endian::read32le(reinterpret_cast<const uint8_t*>(bytes.data() + instructionOffset - 12));
    const uint64_t shli =
        llvm::support::endian::read32le(reinterpret_cast<const uint8_t*>(bytes.data() + instructionOffset - 8));
    const uint64_t add =
        llvm::support::endian::read32le(reinterpret_cast<const uint8_t*>(bytes.data() + instructionOffset - 4));

    if ((movPc & 0xffc1ffff) != 0x02000880 || (movi & 0xffc10000) != 0x07000000 || (shli & 0xffc1ffff) != 0x02c00202 ||
        (add & 0xffc0007f) != 0x00000001)
        return std::nullopt;
    if (((movi >> kScalarDestinationShift) & kScalarRegisterMask) != targetRegister ||
        ((shli >> kScalarDestinationShift) & kScalarRegisterMask) != targetRegister ||
        ((add >> kScalarDestinationShift) & kScalarRegisterMask) != targetRegister ||
        ((add >> 12) & kScalarRegisterMask) != targetRegister)
        return std::nullopt;

    const unsigned pcRegister = (movPc >> kScalarDestinationShift) & kScalarRegisterMask;
    ...
    const uint64_t encodedRelativeWords =
        (movi & 0xffff) | (static_cast<uint64_t>(*upper1) << 16) | (static_cast<uint64_t>(*upper2) << 32);
    ...
    if (static_cast<uint64_t>(byteOffset) > std::numeric_limits<uint64_t>::max() - pcAddress)
        return std::nullopt;
    return pcAddress + static_cast<uint64_t>(byteOffset);
}

static FailureOr<llvm::SmallVector<DecodedCallSite, 4>> decodeCallSites(
    const ELFFunction& caller, StringRef objectBytes, llvm::raw_ostream& diagOS)
{
    ...
    if (fileOffset > objectBytes.size() || caller.size > objectBytes.size() - fileOffset) {
        emitError(diagOS, llvm::Twine("caller '") + caller.name + "' lies outside the object file");
        return failure();
    }
    StringRef bytes = objectBytes.substr(fileOffset, caller.size);
    llvm::SmallVector<DecodedCallSite, 4> callSites;
    for (uint64_t offset = 0; offset + sizeof(uint64_t) <= bytes.size(); offset += kInstructionBytes) {
        uint64_t instruction = llvm::support::endian::read64le(reinterpret_cast<const uint8_t*>(bytes.data() + offset));
        if (!isVFSIMT(instruction))
            continue;
        std::optional<uint64_t> target = decodeTargetAddress(bytes, caller.address, offset, instruction);
        if (!target) {
            emitError(
                diagOS, llvm::Twine("caller '") + caller.name + "' uses an unsupported VF_SIMT target sequence at 0x" +
                            llvm::Twine::utohexstr(caller.address + offset));
            return failure();
        }
        callSites.push_back(
            {caller.address + offset, fileOffset + offset, *target,
             static_cast<uint16_t>((instruction & kVFSIMTSizeMask) >> kVFSIMTSizeShift), instruction});
        offset += sizeof(uint64_t) - kInstructionBytes;
    }
    return callSites;
}

static LogicalResult validateObjectHeader(const llvm::object::ELFObjectFileBase& object, llvm::raw_ostream& diagOS)
{
    ...
    if (object.getEMachine() != kHiIPUMachine) {
        emitError(diagOS, llvm::Twine("unexpected ELF machine 0x") + llvm::Twine::utohexstr(object.getEMachine()));
        return failure();
    }
...
static FailureOr<llvm::SmallVector<PatchRecord, 4>> buildPatchPlan(
    llvm::ArrayRef<SimtCallSite> manifest, const llvm::StringMap<ELFFunction>& functions, StringRef objectBytes,
    llvm::raw_ostream& diagOS)
{
    ...
    llvm::DenseMap<StringRef, llvm::SmallVector<const SimtCallSite*, 4>> callsByCaller;
    for (const SimtCallSite& call : manifest)
    ...
    for (auto& entry : callsByCaller) {
        ...
        if (callerIt == functions.end()) {
            emitError(diagOS, llvm::Twine("missing ELF caller symbol '") + entry.first + "'");
            return failure();
        }
        ...
        for (const SimtCallSite* call : calls) {
            ...
            if (calleeIt == functions.end()) {
                emitError(diagOS, llvm::Twine("missing ELF callee symbol '") + call->calleeName + "'");
                return failure();
            }
            const ELFFunction& callee = calleeIt->second;
            if (callee.size % kInstructionBytes != 0) {
                emitError(diagOS, llvm::Twine("callee '") + callee.name + "' size is not a multiple of 4 bytes");
                return failure();
            }
            const uint64_t codeSize = callee.size / kInstructionBytes;
            if (codeSize == 0 || codeSize >= kInvalidVFSIMTSize) {
                emitError(diagOS, llvm::Twine("callee '") + callee.name + "' code size cannot be encoded safely");
                return failure();
            }
            ...
            auto existing = llvm::find_if(
                resolvedCalls, [&](const ResolvedCall& item) { return item.callee->name == callee.name; });
        ...
        for (const DecodedCallSite& decodedCall : *decoded) {
            ...
            for (ResolvedCall& candidate : resolvedCalls) {
                ...
                if (matched && matched->callee->name != candidate.callee->name) {
                    emitError(
                        diagOS, llvm::Twine("caller '") + entry.first + "' has an ambiguous VF_SIMT target at 0x" +
            ...
            if (!matched) {
                emitError(
                    diagOS, llvm::Twine("caller '") + entry.first + "' VF_SIMT target 0x" +
                                llvm::Twine::utohexstr(decodedCall.targetAddress) +
            ...
            plan.push_back(
                {matched->manifest, decodedCall, static_cast<uint16_t>(matched->callee->size / kInstructionBytes)});
        }
        for (const ResolvedCall& call : resolvedCalls) {
            if (call.observed)
                continue;
            emitError(
                diagOS, llvm::Twine("caller '") + entry.first + "' has no decoded VF_SIMT callsite for callee '" +
                            call.callee->name + "'");
            return failure();
        }
    }
    ...
    for (unsigned index = 1; index < plan.size(); ++index) {
        if (plan[index - 1].decoded.fileOffset + sizeof(uint64_t) > plan[index].decoded.fileOffset) {
...
static LogicalResult validateNoRelocationOverlap(
    const llvm::object::ELFObjectFileBase& object, llvm::ArrayRef<PatchRecord> plan, llvm::raw_ostream& diagOS)
{
    ...
    for (const llvm::object::SectionRef& section : object.sections()) {
        if (section.relocation_begin() == section.relocation_end())
            continue;
        auto relocatedSection = takeExpected(
            section.getRelocatedSection(), diagOS, "failed to identify the section targeted by relocations");
        if (!relocatedSection)
            return failure();
        ...
        for (const llvm::object::RelocationRef& relocation : section.relocations()) {
            ...
            const uint64_t relocationFileOffset = relocatedELFSection.getOffset() + relocation.getOffset();
            for (const PatchRecord& record : plan) {
                if (relocationFileOffset >= record.decoded.fileOffset &&
                    relocationFileOffset < record.decoded.fileOffset + sizeof(uint64_t)) {
                    emitError(
                        diagOS, llvm::Twine("relocation overlaps VF_SIMT at file offset 0x") +
...
static FailureOr<ObjectPatchAnalysis> analyzeObject(
    llvm::ArrayRef<SimtCallSite> manifest, StringRef objectPath, llvm::raw_ostream& diagOS)
{
    ...
    if (!bufferOrError) {
        emitError(diagOS, llvm::Twine("failed to read '") + objectPath + "': " + bufferOrError.getError().message());
        return failure();
    }
    std::unique_ptr<llvm::MemoryBuffer> buffer = std::move(*bufferOrError);
    auto objectOrError = llvm::object::ObjectFile::createObjectFile(buffer->getMemBufferRef());
    if (!objectOrError) {
        emitError(
            diagOS, llvm::Twine("failed to parse '") + objectPath + "': " + llvm::toString(objectOrError.takeError()));
        return failure();
    }
    auto* object = llvm::dyn_cast<llvm::object::ELFObjectFileBase>(objectOrError->get());
    if (!object) {
    ...
    if (failed(plan) || failed(validateNoRelocationOverlap(*object, *plan, diagOS)))
        return failure();
    return ObjectPatchAnalysis{std::move(buffer), std::move(*plan)};
}

static LogicalResult validatePatchedBytes(
    StringRef rawBytes, StringRef patchedBytes, llvm::ArrayRef<PatchRecord> plan, llvm::raw_ostream& diagOS)
{
    ...
    for (const PatchRecord& record : plan) {
        const uint64_t rawInstruction = llvm::support::endian::read64le(
            reinterpret_cast<const uint8_t*>(rawBytes.data() + record.decoded.fileOffset));
        const uint64_t patchedInstruction = llvm::support::endian::read64le(
            reinterpret_cast<const uint8_t*>(patchedBytes.data() + record.decoded.fileOffset));
    ...
    for (size_t offset = 0; offset < rawBytes.size(); ++offset) {
        ...
        if (!llvm::any_of(plan, [offset](const PatchRecord& record) {
                return offset >= record.decoded.fileOffset && offset < record.decoded.fileOffset + sizeof(uint64_t);
            })) {
            emitError(
                diagOS, llvm::Twine("unexpected byte difference at file offset 0x") + llvm::Twine::utohexstr(offset));
            return failure();
        }
    }
    return success();
...
static LogicalResult writePatchedObject(StringRef path, StringRef bytes, llvm::raw_ostream& diagOS)
{
    // FileOutputBuffer may leave a partial file if allocation or commit fails.
    auto removeOutput = llvm::make_scope_exit([&]() { llvm::sys::fs::remove(path); });
    auto output = llvm::FileOutputBuffer::create(path, bytes.size());
    if (!output) {
        diagOS << "Error: VF_SIMT size patch: failed to create '" << path << "': " << llvm::toString(output.takeError())
               << "\n";
        return failure();
    }
    std::copy(bytes.begin(), bytes.end(), (*output)->getBufferStart());
    if (llvm::Error error = (*output)->commit()) {
        diagOS << "Error: VF_SIMT size patch: failed to write '" << path << "': " << llvm::toString(std::move(error))
               << "\n";
        return failure();
    }
    ...
    llvm::raw_ostream& diagOS)
{
    ...
    for (const PatchRecord& record : analysis->plan) {
        ...
        if (record.decoded.codeSize != kInvalidVFSIMTSize) {
            if (record.decoded.codeSize == 0 || record.decoded.codeSize > record.expectedCodeSize) {
                emitError(
                    diagOS, llvm::Twine("caller '") + record.manifest->callerName + "' has VF_SIMT code size " +
                                llvm::Twine(record.decoded.codeSize) + " outside callee '" +
                                record.manifest->calleeName + "' symbol size " + llvm::Twine(record.expectedCodeSize));
                return failure();
            }
            continue;
        }
        ...
        if (mode == VFSIMTSizeFixMode::Verify) {
            emitError(
                diagOS, llvm::Twine("caller '") + record.manifest->callerName +
                            "' has the known invalid VF_SIMT code size 0xffff for callee '" +
                            record.manifest->calleeName + "'");
            return failure();
        }
        uint64_t patchedInstruction = (record.decoded.instruction & ~kVFSIMTSizeMask) |
                                      (static_cast<uint64_t>(record.expectedCodeSize) << kVFSIMTSizeShift);
        llvm::support::endian::write64le(
            reinterpret_cast<uint8_t*>(patchedBytes.data() + record.decoded.fileOffset), patchedInstruction);
    ...
    if (failed(validatePatchedBytes(analysis->buffer->getBuffer(), patchedBytes, analysis->plan, diagOS)))
        return failure();
    ...
    if (!writtenBuffer || writtenBuffer.get()->getBuffer() != patchedBytes) {
        ...
        emitError(diagOS, llvm::Twine("failed to verify written object '") + patchedObjectPath + "'");
        return failure();
    }
    auto patchedAnalysis = analyzeObject(*manifest, patchedObjectPath, diagOS);
    if (failed(patchedAnalysis) || patchedAnalysis->plan.size() != analysis->plan.size()) {
        llvm::sys::fs::remove(patchedObjectPath);
    ...
    for (unsigned index = 0; index < analysis->plan.size(); ++index) {
        ...
        const uint16_t expectedWrittenSize =
            rawRecord.decoded.codeSize == kInvalidVFSIMTSize ? rawRecord.expectedCodeSize : rawRecord.decoded.codeSize;
    ...
    for (const PatchRecord& record : analysis->plan) {
        ...
        diagOS << "PTOAS: patched VF_SIMT size\n"
               ...
               << "  symbol size: " << static_cast<uint64_t>(record.expectedCodeSize) * kInstructionBytes << " bytes\n"
