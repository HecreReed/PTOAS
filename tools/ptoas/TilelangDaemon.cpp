...
{
    auto pythonPath = mlir::pto::resolvePythonExecutable(pythonExe.empty() ? "python3" : pythonExe);
    if (!pythonPath) {
        llvm::errs() << "Error: Cannot find Python executable '" << (pythonExe.empty() ? "python3" : pythonExe)
                     << "' for daemon\n";
        return false;
    }
    ...
    llvm::sys::ProcessInfo procInfo = llvm::sys::ExecuteNoWait(
        *pythonPath, args, !pkgPath.empty() ? std::optional<llvm::ArrayRef<llvm::StringRef>>(envp) : std::nullopt, {},
    ...
    llvm::errs() << "TileLib daemon '" << daemonModule << "' started (pid=" << procInfo.Pid << ", socket=" << socketPath
                 << ")\n";
    return true;
}
...
bool DaemonManager::isRunning() { return processInfo.has_value(); }

static void daemonCleanupHandler() { DaemonManager::stop(); }

void registerDaemonCleanup() { std::atexit(daemonCleanupHandler); }
