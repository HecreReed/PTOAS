...
struct ApplySIMTEntryNoInlinePass final : public PassWrapper<ApplySIMTEntryNoInlinePass, OperationPass<ModuleOp>> {
...
struct FormEmitCExpressionsCompatPass final
    : public PassWrapper<FormEmitCExpressionsCompatPass, OperationPass<ModuleOp>> {
    MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(FormEmitCExpressionsCompatPass)

    static bool foldExpression(emitc::ExpressionOp expression, IRRewriter& rewriter)
    {
        bool changed = false;
        for (Operation& op : llvm::make_early_inc_range(expression.getBody()->without_terminator())) {
            ...
            for (OpOperand& operand : llvm::make_early_inc_range(op.getOpOperands())) {
                ...
                if (!producer || !producer.getResult().hasOneUse() || producer.hasSideEffects())
                    continue;
                ...
                assert(
                    clonedRoot && clonedRoot->getNumResults() == 1 &&
    ...
    void runOnOperation() final
    {
        ...
        module.walk([&](Operation* op) {
            if (op->hasTrait<OpTrait::emitc::CExpression>() && !op->getParentOfType<emitc::ExpressionOp>() &&
                op->getNumResults() == 1)
        ...
        do {
            ...
            module.walk<WalkOrder::PostOrder>(
                [&](emitc::ExpressionOp expression) { changed |= foldExpression(expression, rewriter); });
        } while (changed);
    }
};
...
static std::string resolveEffectiveTargetArch(ModuleOp module, llvm::StringRef fallbackArch)
{
...
static bool pathExists(llvm::StringRef path) { return !path.empty() && llvm::sys::fs::exists(path); }
...
static std::string detectInstalledPythonPkgRoot(const char* argv0, llvm::StringRef packageName)
{
...
static LogicalResult applyConfiguredPassManagerCLOptions(
    PassManager& pm, llvm::StringRef pipelineName, llvm::raw_ostream& diagOS = llvm::errs())
{
    if (succeeded(mlir::applyPassManagerCLOptions(pm)))
        return success();
    diagOS << "Error: failed to apply MLIR pass manager command-line options for " << pipelineName << ".\n";
    return failure();
}
...
static llvm::cl::opt<bool> enableBufidSync(
    "enable-bufid_sync", llvm::cl::desc("Enable A5 buffer-id synchronization insertion pass"), llvm::cl::init(false));

static llvm::cl::opt<bool> enableBufidSyncDebug(
    "enable-bufid-sync-debug", llvm::cl::desc("Enable verbose debug printing for --enable-bufid_sync"),
    llvm::cl::init(false));
...
static llvm::cl::opt<bool> enableTileOpExpand(
    ...
    llvm::cl::desc("Deprecated compatibility flag. TileOp expansion is controlled by "
                   "--pto-backend=vpto."),
    llvm::cl::init(false));
...
static llvm::cl::opt<TileLibBackend> tileLibBackend(
    "tile-lib-backend", llvm::cl::desc("TileLib backend used by ExpandTileOp"),
    llvm::cl::values(clEnumValN(TileLibBackend::PTODSL, "ptodsl", "Use the PTODSL TileLib daemon")),
...
static pto::ExpandTileOpOptions resolveExpandTileOpOptions(int argc, char** argv)
{
    ...
    if (!hasCLIOption(argc, argv, "--ptodsl-pkg-path")) {
        ...
        if (envPtodslRoot && envPtodslRoot[0] != '\0')
            ...
            std::string installedPtodslPkgPath = detectInstalledPythonPkgRoot(argv[0], "ptodsl");
    ...
    if (!hasCLIOption(argc, argv, "--tileops-pkg-path")) {
        ...
        if (envTileOpsRoot && envTileOpsRoot[0] != '\0')
            ...
            std::string installedTileOpsPkgPath = detectInstalledPythonPkgRoot(argv[0], "TileOps");
    ...
    if (ptoas::DaemonManager::start(
            socket, "ptodsl.tilelib.serving.daemon", expandOpts.pythonExe, expandOpts.tileLibPkgPath, "")) {
        ...
        llvm::errs() << "Error: Failed to start the PTODSL TileLib daemon; no TileLang "
...
static pto::InsertTemplateAttributesOptions buildInsertTemplateAttributesOptions(
...
static llvm::cl::opt<bool> enableSoftPostUpdate(
    "enable-vpto-soft-postupdate", llvm::cl::desc("Enable VPTO soft post-update optimization"), llvm::cl::init(false));
...
llvm::cl::opt<std::string> mlir::pto::ptoTargetArch(
    "pto-arch", llvm::cl::desc("Target Ascend architecture for codegen: a2, a3, or a5 (default: a3)"),
...
llvm::cl::opt<std::string> mlir::pto::ptoBackend(
    "pto-backend", llvm::cl::desc("Final PTOAS backend: emitc or vpto (default: emitc)"),
    llvm::cl::value_desc("emitc|vpto"), llvm::cl::init("emitc"));

llvm::cl::opt<bool> mlir::pto::emitVPTO(
    "emit-vpto", llvm::cl::desc("Write final post-pass VPTO IR to -o"), llvm::cl::init(false));

llvm::cl::opt<bool> mlir::pto::emitVPTOLLVMDialect(
    "emit-vpto-llvm-ir", llvm::cl::desc("Write translated VPTO LLVM IR to -o"), llvm::cl::init(false));

static llvm::cl::opt<bool> vptoPrintIR(
    "vpto-print-ir", llvm::cl::desc("Print post-pass VPTO backend IR to stderr"), llvm::cl::init(false));

static llvm::cl::opt<std::string> vptoLoweringStrategy(
    "vpto-lowering-strategy", llvm::cl::desc("VPTO vector lowering strategy: post-update or no-post-update"),
    llvm::cl::value_desc("post-update|no-post-update"), llvm::cl::init("post-update"));

static llvm::cl::opt<bool> dumpVPTOIR(
    "dump-vpto-ir", llvm::cl::desc("Print post-pass VPTO backend IR to stderr"), llvm::cl::init(false));

llvm::cl::opt<bool> mlir::pto::ptoPrintSeamIR(
    "pto-print-seam-ir", llvm::cl::desc("Print shared pre-backend seam IR to stderr"), llvm::cl::init(false));

llvm::cl::opt<std::string> mlir::pto::ptoSeamIRFile(
    "pto-seam-ir-file", llvm::cl::desc("Write shared pre-backend seam IR to a file"), llvm::cl::value_desc("path"),
    llvm::cl::init(""));

llvm::cl::opt<std::string> mlir::pto::cannOutputVersion(
    "cann-output-version",
    llvm::cl::desc(
        "Override the CANN version used for lowering and public ABI output selection; examples: 9.0.0, 9.0.0-beta.1"),
    llvm::cl::value_desc("version"), llvm::cl::init(""));

llvm::cl::opt<mlir::pto::VFSIMTSizeFixMode> mlir::pto::vptoFixVFSIMTSize(
    "vpto-fix-vfsimt-size", llvm::cl::desc("Validate or repair VF_SIMT code sizes in VPTO vector objects"),
    llvm::cl::value_desc("auto|off|verify"),
    llvm::cl::values(
        clEnumValN(mlir::pto::VFSIMTSizeFixMode::Auto, "auto", "Repair the known invalid 0xffff size (default)"),
        clEnumValN(mlir::pto::VFSIMTSizeFixMode::Off, "off", "Skip VF_SIMT size validation and repair"),
        clEnumValN(mlir::pto::VFSIMTSizeFixMode::Verify, "verify", "Validate VF_SIMT sizes without repairing them")),
...
static ReserveBufferMemSpec getReserveBufferMemSpec(PTOArch arch, AddressSpace space)
{
    switch (space) {
...
static LogicalResult validateReserveBufferBase(pto::ReserveBufferOp op, PTOArch arch)
{
    ...
    ReserveBufferMemSpec spec = getReserveBufferMemSpec(arch, op.getLocation().getAddressSpace());
    ...
    if (base % spec.alignmentBytes != 0) {
        return op.emitError("expects 'base' to be aligned to ")
               << spec.alignmentBytes << " bytes for " << stringifyEnum(op.getLocation().getAddressSpace());
    }
    ...
    if (base > spec.capacityBytes || size > spec.capacityBytes - base) {
        return op.emitError("reserved range exceeds ")
               << stringifyEnum(op.getLocation().getAddressSpace()) << " capacity: base " << base << " + size " << size
...
static bool validateReserveBufferLevelRules(ModuleOp module, PTOBuildLevel level)
{
    bool failed = false;
...
static LogicalResult emitSharedPreBackendSeamIR(ModuleOp module, llvm::StringRef outputPath)
{
    if (outputPath.empty())
        return success();
    ...
    if (ec) {
        llvm::errs() << "Error: failed to open seam IR file '" << outputPath << "': " << ec.message() << "\n";
        return failure();
    }
...
static bool hasUnexpandedTileOps(ModuleOp module)
{
    ...
    module.walk([&](Operation* op) {
        ...
        if (auto func = dyn_cast<func::FuncOp>(op); func && func->hasAttr("pto.tileop.helper"))
            found = true;
    });
    return found;
}

using FunctionBlockArgHintMap = llvm::StringMap<llvm::SmallVector<llvm::SmallVector<std::string, 4>, 4>>;

static bool isGeneratedValueName(llvm::StringRef name);
static SmallVector<std::string, 4> getValueNameHints(Value value);

static bool isCppIdentifierStart(char c) { return std::isalpha(static_cast<unsigned char>(c)) || c == '_'; }

static bool isCppIdentifierChar(char c) { return std::isalnum(static_cast<unsigned char>(c)) || c == '_'; }
...
static SmallVector<std::string, 4> expandTextualResultGroupHints(
    const AsmParserState::OperationDefinition& opDef, unsigned groupIndex)
{
    SmallVector<std::string, 4> hints;
    if (groupIndex >= opDef.resultGroups.size())
        return hints;
    const auto& group = opDef.resultGroups[groupIndex];
    std::optional<std::string> baseName = getTextualNameFromSMRange(group.definition.loc);
    if (!baseName)
        return hints;

    unsigned resultStart = group.startIndex;
    unsigned resultEnd = groupIndex + 1 == opDef.resultGroups.size() ? opDef.op->getNumResults() :
...
static void appendLocationNameHints(Location loc, SmallVectorImpl<std::string>& hints)
{
...
static void appendRawLocationProvenance(Location loc, SmallVectorImpl<std::string>& hints)
{
    if (auto nameLoc = dyn_cast<NameLoc>(loc)) {
...
static SmallVector<std::string, 4> getRawResultProvenance(Operation* op)
{
    ...
    if (hints.empty())
        return hints;
    hints.erase(
        std::remove_if(hints.begin(), hints.end(), [](const std::string& name) { return name.empty(); }), hints.end());
    if (hints.empty())
        return hints;
...
static SmallVector<std::string, 4> getRawLocationProvenance(Location loc)
{
    ...
    hints.erase(
        std::remove_if(hints.begin(), hints.end(), [](const std::string& hint) { return hint.empty(); }), hints.end());
    return hints;
}
...
static Location attachLocationNameHints(Location baseLoc, llvm::ArrayRef<std::string> hints, MLIRContext* context)
{
    ...
    return FusedLoc::get(ArrayRef<Location>{baseLoc}, ArrayAttr::get(context, attrs), context);
}
...
static void applyOperationResultNameHints(Operation* op, llvm::ArrayRef<std::string> hints)
{
    if (!op || op->getNumResults() == 0 || hints.empty() || hasLocationNameHints(op->getLoc()))
        return;

    SmallVector<std::string, 4> limitedHints;
    limitedHints.reserve(std::min<size_t>(op->getNumResults(), hints.size()));
    for (size_t i = 0, e = std::min<size_t>(op->getNumResults(), hints.size()); i < e; ++i)
...
static void splitDerivedSingleResultProvenanceLocsInBlock(Block& block)
{
    ...
    for (size_t i = 0; i < ops.size();) {
        ...
        size_t runEnd = i + 1;
        while (runEnd < ops.size() && ops[runEnd]->getNumResults() == 1 && ops[runEnd]->getLoc() == op->getLoc()) {
            ++runEnd;
        }
...
static void narrowUnusedMultiResultProvenanceLocs(Operation* root)
{
    ...
    root->walk([&](Operation* op) {
        ...
        op->setLoc(attachLocationNameHints(op->getLoc(), liveHints, op->getContext()));
    });
}

namespace {
struct NarrowUnusedMultiResultProvenancePass
    : public PassWrapper<NarrowUnusedMultiResultProvenancePass, OperationPass<ModuleOp>> {
    MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(NarrowUnusedMultiResultProvenancePass)

    void runOnOperation() override { narrowUnusedMultiResultProvenanceLocs(getOperation()); }
};
} // namespace
...
struct SerialAutoSyncPass : public PassWrapper<SerialAutoSyncPass, OperationPass<ModuleOp>> {
    ...
    SerialAutoSyncPass(Mode mode, bool enableBufidDebug, int64_t graphEventIdMax)
        : mode(mode), enableBufidDebug(enableBufidDebug), graphEventIdMax(graphEventIdMax)
    {}
...
namespace {
struct SerialFrontendPipeLoweringPass : public PassWrapper<SerialFrontendPipeLoweringPass, OperationPass<ModuleOp>> {
...
static void collectNonEntryBlocksInSourceOrder(Operation* op, SmallVectorImpl<Block*>& blocks)
{
    for (Region& region : op->getRegions()) {
...
void mlir::pto::applyTextualNameHintsToModule(ModuleOp module, const AsmParserState& parserState)
{
    if (!module)
        return;
    ...
    for (const AsmParserState::BlockDefinition& blockDef : parserState.getBlockDefs()) {
        ...
        for (auto [argIndex, argDef] : llvm::enumerate(blockDef.arguments)) {
            ...
            if (!hint)
                continue;
            applyValueNameHints(blockDef.block->getArgument(argIndex), llvm::ArrayRef<std::string>{*hint});
        }
    }
    ...
    for (const AsmParserState::OperationDefinition& opDef : parserState.getOpDefs()) {
        ...
        for (unsigned groupIndex = 0, e = opDef.resultGroups.size(); groupIndex < e; ++groupIndex) {
            SmallVector<std::string, 4> groupHints = expandTextualResultGroupHints(opDef, groupIndex);
...
static void applyFunctionBlockArgNameHintsToEmitC(ModuleOp module, const FunctionBlockArgHintMap& blockArgHints)
{
...
static std::string buildHintMarker(llvm::StringRef prefix, llvm::ArrayRef<std::string> hints)
{
...
static SmallVector<std::string, 8> collectExpressionProvenance(emitc::ExpressionOp expr)
{
    ...
    auto appendUnique = [&](llvm::ArrayRef<std::string> names) {
        for (const std::string& name : names) {
            if (name.empty())
                continue;
            if (std::find(provenance.begin(), provenance.end(), name) != provenance.end())
                continue;
...
static void annotateEmitCProvenanceHints(ModuleOp module)
{
    ...
    module.walk<WalkOrder::PreOrder>([&](Operation* op) {
        ...
        if (auto expr = dyn_cast<emitc::ExpressionOp>(op)) {
            ...
            opsToAnnotate.push_back(ProvenanceMarker{op, SmallVector<std::string, 8>(provenance)});
            return WalkResult::skip();
        }
        ...
        opsToAnnotate.push_back(
            ProvenanceMarker{op, SmallVector<std::string, 8>(provenance.begin(), provenance.end())});
        return WalkResult::advance();
    });
    ...
    for (const ProvenanceMarker& marker : opsToAnnotate) {
        ...
        if (!marker.names.empty()) {
            ...
            builder.create<emitc::VerbatimOp>(
                marker.op->getLoc(), builder.getStringAttr(buildHintMarker("PTOAS_PROVENANCE", marker.names)));
        }
    }
}
...
static void rewriteMarkerCallsToMembers(std::string& cpp, llvm::ArrayRef<MarkerRewriteSpec> rewrites)
{
    ...
    while (changed) {
        ...
        for (const MarkerRewriteSpec& rewrite : rewrites) {
            changed |= rewriteMarkerCallToMember(cpp, rewrite.marker, rewrite.memberName, rewrite.expectedNumArgs);
        }
    }
}
...
static void appendEmitCIntegerAttrLiteral(std::string& storage, const APInt& value, bool isUnsigned)
{
...
{
    std::string literal;
    appendEmitCIntegerAttrLiteral(literal, attr.getValue(), shouldPrintEmitCIntegerAttrAsUnsigned(attr));
    return literal;
}

static std::optional<std::string> getEmitCDenseIntElementsAttrLiteral(DenseIntElementsAttr attr)
{
...
static Attribute normalizeEmitCPrintedAttrForCppEmission(MLIRContext* ctx, Attribute attr)
{
    ...
    if (auto denseAttr = dyn_cast<DenseIntElementsAttr>(attr)) {
        if (std::optional<std::string> literal = getEmitCDenseIntElementsAttrLiteral(denseAttr))
    ...
    if (auto arrayAttr = dyn_cast<ArrayAttr>(attr)) {
        ...
        for (Attribute element : arrayAttr) {
            Attribute normalizedElement = normalizeEmitCPrintedAttrForCppEmission(ctx, element);
...
static IntegerAttr normalizeEmitCIndexPlaceholderAttr(MLIRContext* ctx, IntegerAttr attr)
{
...
static ArrayAttr normalizeEmitCCallArgsForCppEmission(MLIRContext* ctx, ArrayAttr args)
{
    SmallVector<Attribute> normalized;
    ...
    for (Attribute attr : args) {
        if (auto intAttr = dyn_cast<IntegerAttr>(attr)) {
            if (isa<IndexType>(intAttr.getType())) {
                Attribute normalizedAttr = normalizeEmitCIndexPlaceholderAttr(ctx, intAttr);
                changed |= normalizedAttr != attr;
                normalized.push_back(normalizedAttr);
                continue;
            }

            Attribute normalizedAttr = normalizeEmitCPrintedAttrForCppEmission(ctx, attr);
            changed |= normalizedAttr != attr;
            normalized.push_back(normalizedAttr);
            continue;
        }
...
static ArrayAttr normalizeEmitCTemplateArgsForCppEmission(MLIRContext* ctx, ArrayAttr args)
{
    SmallVector<Attribute> normalized;
    normalized.reserve(args.size());
    bool changed = false;
...
static void normalizeEmitCIntegerAttrsForCppEmission(Operation* rootOp)
{
    ...
    rootOp->walk([&](Operation* op) {
        if (auto constant = dyn_cast<emitc::ConstantOp>(op)) {
            ...
            Attribute normalized = normalizeEmitCPrintedAttrForCppEmission(ctx, value);
            if (normalized != value)
        ...
        if (auto global = dyn_cast<emitc::GlobalOp>(op)) {
            ...
            if (!initialValue)
                return;
            Attribute normalized = normalizeEmitCPrintedAttrForCppEmission(ctx, *initialValue);
        ...
        if (auto call = dyn_cast<emitc::CallOpaqueOp>(op)) {
            ...
            if (std::optional<ArrayAttr> templateArgs = call.getTemplateArgs()) {
                ArrayAttr normalized = normalizeEmitCTemplateArgsForCppEmission(ctx, *templateArgs);
...
static void materializeControlFlowOperands(Operation* rootOp)
{
    ...
    for (Operation* op : branches) {
        ...
        for (OpOperand& operand : op->getOpOperands()) {
            ...
            Value tmp =
                builder.create<emitc::VariableOp>(op->getLoc(), getEmitCVariableStorageType(value.getType()), initAttr)
                    .getResult();
    ...
    std::string& cpp, llvm::StringRef marker, unsigned expectedNumArgs, bool isStore)
{
    return rewriteMarkerCalls(cpp, marker, [&](const ParsedMarkerCall& call) -> std::optional<std::string> {
        if (call.args.size() != expectedNumArgs)
            return std::nullopt;
        std::string replacement;
        replacement.reserve(call.args[0].size() + call.args[1].size() + 8 + (isStore ? call.args[2].size() : 0));
        replacement.push_back('(');
        replacement.append(call.args[0].str());
        replacement.push_back(')');
        replacement.push_back('[');
...
static void rewriteGlobalTensorMetadataMarkers(std::string& cpp)
{
    auto rewrite = [&](llvm::StringRef marker, llvm::StringRef method) {
        (void)rewriteMarkerCalls(cpp, marker, [&](const ParsedMarkerCall& call) -> std::optional<std::string> {
            if (call.args.size() != 2)
                return std::nullopt;
            return ("(" + call.args[0] + ")." + method + "(static_cast<int>(" + call.args[1] + "))").str();
        });
    };
...
static void rewriteMarkerCallsToSubscripts(std::string& cpp, llvm::ArrayRef<MarkerSubscriptRewriteSpec> rewrites)
{
    ...
    while (changed) {
        ...
        for (const MarkerSubscriptRewriteSpec& rewrite : rewrites) {
            changed |= rewriteMarkerCallToSubscript(cpp, rewrite.marker, rewrite.expectedNumArgs, rewrite.isStore);
        }
    }
}
...
static bool stripScalarGMFlushMarkersFromLine(std::string& line)
{
    ...
    while (true) {
        ...
        while (eraseBegin > 0 && (line[eraseBegin - 1] == ' ' || line[eraseBegin - 1] == '\t'))
            --eraseBegin;

        size_t eraseEnd = call->rparenPos + 1;
        while (eraseEnd < line.size() && (line[eraseEnd] == ' ' || line[eraseEnd] == '\t'))
            ++eraseEnd;
...
static bool previousSignificantLineIsTailFlushPoint(llvm::ArrayRef<std::string> lines, size_t index)
{
    for (size_t i = index; i > 0; --i) {
        llvm::StringRef prev = llvm::StringRef(lines[i - 1]).trim();
        if (prev.empty())
            continue;
        return prev.starts_with("#endif // __DAV_") || prev.starts_with("ptoas_auto_sync_tail(");
    }
    return false;
}

static bool previousSignificantLineIsExitOrTailFlushPoint(llvm::ArrayRef<std::string> lines, size_t index)
{
    for (size_t i = index; i > 0; --i) {
        llvm::StringRef prev = llvm::StringRef(lines[i - 1]).trim();
        if (prev.empty())
    ...
    llvm::ArrayRef<std::string> functionLines, bool hasTrailingNewline)
{
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        if (trimmed.starts_with("return")) {
            ...
            insertHere = trimmed.starts_with("#endif // __DAV_") || trimmed.starts_with("ptoas_auto_sync_tail(");
        }
        if (i == fallbackIndex && !previousSignificantLineIsExitOrTailFlushPoint(lines, i))
            insertHere = true;
        if (insertHere) {
...
static void rewriteScalarGMStoreFlushMarkers(std::string& cpp)
{
    ...
    auto flushFunction = [&](bool hasTrailingNewline) {
        out.append(rewriteScalarGMStoreFlushMarkersInFunction(functionLines, hasTrailingNewline));
        functionLines.clear();
        inFunction = false;
...
static bool isPreprocessorDirectiveLine(llvm::StringRef trimmedLine) { return trimmedLine.starts_with("#"); }
...
static void rewriteMalformedVerbatimSemicolons(std::string& cpp)
{
    ...
    while (offset < input.size()) {
        ...
        llvm::StringRef line = hasNewline ? input.slice(offset, newlinePos) : input.drop_front(offset);
        ...
        if (trimmed == ";" && prevWasPreprocessorDirective) {
            ...
            if (isPreprocessorDirectiveLine(trimmed) && trimmed.ends_with(";")) {
                ...
                !trimmed.empty() && !trimmed.starts_with("//") && !trimmed.starts_with("/*") &&
                trimmed.ends_with(";;")) {
                size_t semicolonPos = current.find_last_of(';');
                if (semicolonPos != std::string::npos)
                    current.erase(semicolonPos, 1);
            ...
            if (hasNewline)
                rewritten.push_back('\n');
            prevWasPreprocessorDirective = isPreprocessorDirectiveLine(llvm::StringRef(current).trim());
        }
...
static std::optional<llvm::SmallVector<std::string, 4>> parseNameHintMarker(llvm::StringRef markerBody)
{
    auto decodeHintMarkerToken = [](llvm::StringRef token) {
        ...
        for (size_t i = 0; i < token.size();) {
            if (token[i] == '%' && i + 2 < token.size()) {
                ...
                if (hi >= 0 && lo >= 0) {
                    decoded.push_back(static_cast<char>((static_cast<unsigned>(hi) << 4) | lo));
                    i += 3;
                    continue;
                }
            }
    ...
    while (start <= markerBody.size()) {
        ...
        llvm::StringRef token = markerBody.slice(start, comma == llvm::StringRef::npos ? markerBody.size() : comma);
        token = token.trim();
        if (!token.empty())
...
static void stripHintMarkersWithPrefix(std::string& cpp, llvm::StringRef markerPrefix)
{
    std::string out;
    ...
    while (searchPos < cpp.size()) {
        ...
        markerEnd += 2;
        while (markerEnd < cpp.size() && (cpp[markerEnd] == '\r' || cpp[markerEnd] == '\n'))
            ++markerEnd;
        searchPos = markerEnd;
    }
    cpp.swap(out);
...
static void stripAllHintMarkers(std::string& cpp) { stripHintMarkersWithPrefix(cpp, "/* PTOAS_PROVENANCE:"); }
...
{
    static constexpr llvm::StringLiteral kProvenancePrefix = "/* PTOAS_PROVENANCE:";
    std::string out;
    ...
    while (i < segment.size()) {
        ...
        auto names = parseNameHintMarker(llvm::StringRef(segment).slice(mp + kProvenancePrefix.size(), me));
        ...
        me += 2;
        while (me < segment.size() && (segment[me] == '\r' || segment[me] == '\n'))
            ++me;
        i = me;
    }
    segment.swap(out);
...
static bool parseConstantDeclarationLine(llvm::StringRef line, ConstantDeclCandidate& candidate, std::string& valueName)
{
    ...
    if (body.starts_with("return") || body.starts_with("goto ") || body.starts_with("if ") || body.starts_with("if(") ||
        body.starts_with("switch ") || body.starts_with("switch(") || body.starts_with("for ") ||
        body.starts_with("for(") || body.starts_with("while ") || body.starts_with("while(") ||
...
static void rewriteScalarConstantDecls(std::string& cpp)
{
    ...
    auto rewriteSegment = [&](size_t beginLine, size_t endLine) {
        ...
        for (auto& entry : candidates) {
            ...
            lines[info.declLine] =
                (info.indent + "const " + info.type + " " + valueName.str() + " = " + initializer + ";");
        }
    };
...
static void prepareVPTOForEmission(PassManager& pm)
{
    ...
    kernelModulePM.addNestedPass<func::FuncOp>(pto::createPTOUnrollSIMTForPass());
    kernelModulePM.addPass(createSCCPPass());
    kernelModulePM.addPass(createCanonicalizerPass());
    kernelModulePM.addPass(createCSEPass());
    kernelModulePM.addNestedPass<func::FuncOp>(pto::createPTOAnalyzeSIMTPersistentFragmentPass());
    kernelModulePM.addNestedPass<func::FuncOp>(pto::createPTOMaterializeSIMTPersistentFragmentPass());
    ...
    kernelModulePM.addNestedPass<func::FuncOp>(pto::createPTOInferVPTOVecScopePass());
...
static void lowerPTOToVPTOBackend(PassManager& pm, ModuleOp module, const pto::ExpandTileOpOptions& expandOpts)
{
    auto& kernelModulePM = pm.nest<ModuleOp>();
    auto moduleArchAttr = module->getAttrOfType<mlir::StringAttr>("pto.target_arch");
    ...
    kernelModulePM.addNestedPass<mlir::func::FuncOp>(pto::createLowerPTOToUBufOpsPass());
    if (isA2A3) {
        kernelModulePM.addNestedPass<mlir::func::FuncOp>(memref::createExpandStridedMetadataPass());
    ...
    kernelModulePM.addNestedPass<mlir::func::FuncOp>(pto::createFoldTileBufIntrinsicsPass("shape-only"));
    if (enableA5VPTOPostLoweringFusionLifecycle) {
        ...
        kernelModulePM.addNestedPass<mlir::func::FuncOp>(pto::createPTOFusionPredicateElisionPass());
        kernelModulePM.addNestedPass<mlir::func::FuncOp>(pto::createPTOFusionLoadStoreElisionPass());
        if (enableUnrollAfterLoopFusion) {
            kernelModulePM.addNestedPass<mlir::func::FuncOp>(pto::createPTOUnrollAfterLoopFusionPass());
            kernelModulePM.addPass(mlir::createCanonicalizerPass());
            kernelModulePM.addPass(mlir::createCSEPass());
            kernelModulePM.addNestedPass<mlir::func::FuncOp>(pto::createPTOFusionLoadStoreElisionPass());
        }
        kernelModulePM.addNestedPass<mlir::func::FuncOp>(pto::createPTOFlattenFusionRegionPass());
        kernelModulePM.addPass(mlir::createCSEPass());
    }
    kernelModulePM.addNestedPass<mlir::func::FuncOp>(pto::createFoldTileBufIntrinsicsPass("addr-only"));
    kernelModulePM.addPass(mlir::createSCCPPass());
    kernelModulePM.addPass(mlir::createCanonicalizerPass());
}

static pto::VPTOEmissionOptions buildVPTOEmissionOptions(
    const pto::CANNVersion& cannVersion, llvm::StringRef targetArch)
{
...
static int emitVPTOBackendResult(
    ModuleOp module, PTOASCompileResult& result, bool emitHostStub, const pto::CANNVersion& cannVersion)
{
    if (emitVPTO) {
    ...
    if (emitVPTOLLVMDialect) {
        result.kind = PTOASCompileResultKind::Text;
        pto::VPTOEmissionOptions options =
            buildVPTOEmissionOptions(cannVersion, resolveEffectiveTargetArch(module, ptoTargetArch));
        if (failed(pto::lowerVPTOModuleToLLVMIRText(module, options, result.textOutput, llvm::errs()))) {
    ...
    if (failed(pto::lowerVPTOModuleToLLVMModules(
            module, options, result.vptoCubeModule, result.vptoVectorModule, llvm::errs()))) {
...
static LogicalResult runVPTOBackendPipeline(
    OwningOpRef<ModuleOp>& module, bool hasTileOpsToExpand, const pto::ExpandTileOpOptions* expandOptions)
{
    ...
    if (failed(applyConfiguredPassManagerCLOptions(pm, "VPTO unified emission pipeline")))
        return failure();
...
static void appendVMISemanticPipeline(OpPassManager& pm)
{
    ...
    pm.addNestedPass<func::FuncOp>(pto::createVMINormalizeSignlessIntToUnsignedPass());
...
int mlir::pto::compilePTOASModule(
    OwningOpRef<ModuleOp>& module, PTOASContext& context, PTOBackend effectiveBackend, PTOASCompileResult& result,
    bool emitVPTOHostStub)
{
    ...
    if (effectiveBackend != PTOBackend::VPTO &&
        (emitVPTO || emitVPTOLLVMDialect || ptoPrintSeamIR || !ptoSeamIRFile.empty())) {
    ...
    module->getOperation()->setAttr("pto.target_arch", mlir::StringAttr::get(module->getContext(), arch));
    ...
    const bool enableA5FusionPath = opFusionEnabled && arch == "a5" && effectiveLevel != PTOBuildLevel::Level1;
    const bool enableA5EmitCFusionPath = enableA5FusionPath && effectiveBackend == PTOBackend::EmitC;
    const bool enableA5VPTOFusionPath = enableA5FusionPath && effectiveBackend == PTOBackend::VPTO;

    if (enableVfSimCostmodelOptimization && !(enableA5EmitCFusionPath || enableA5VPTOFusionPath)) {
    ...
    if (enableVfSimCostmodelOptimization && enableA5VPTOFusionPath && !enableUnrollAfterLoopFusion) {
        llvm::errs() << "Warning: --enable-vfsim-costmodel-optimization may "
    ...
    module->walk([&](mlir::func::FuncOp func) {
        ...
        func->setAttr("pto.auto_sync_tail_hint", mlir::StringAttr::get(module->getContext(), normalizedHint));
    });
    ...
    int enabledAutoSyncModes = (enableInsertSync ? 1 : 0) + (enableBufidSync ? 1 : 0) +
    ...
    module->walk([&](pto::AllocMultiTileOp op) {
        if (!op->hasAttr(pto::kPtoMultiBufferAddrsAttrName))
            return;
        op.emitError() << "attribute '" << pto::kPtoMultiBufferAddrsAttrName << "' is reserved for pto-plan-memory";
    ...
    if (effectiveBackend == PTOBackend::VPTO && hasTileOpsToExpand && tileLibBackend == TileLibBackend::PTODSL)
    ...
    if (effectiveBackend == PTOBackend::VPTO && !hasTileOpsToExpand) {
        ...
        if (failed(runVPTOBackendPipeline(
            ...
            return 1;
        return emitVPTOBackendResult(*module, result, emitVPTOHostStub, context.getCANNVersionOrDefault());
    }
    ...
    if (!isA2A3 && expandOptions && expandOptions->tileLibBackend == "ptodsl") {
        auto insertOptions = buildInsertTemplateAttributesOptions(*expandOptions);
        pm.addPass(pto::createInsertTemplateAttributesPass(insertOptions));
    }
    ...
    fusionPlanOpts.enableVfSimCostmodelOptimization = enableVfSimCostmodelOptimization;
    fusionPlanOpts.dumpVfSimUnrollTest = dumpVfSimUnrollTest;
    if (!isA2A3 && enableA5EmitCFusionPath) {
        pm.addNestedPass<mlir::func::FuncOp>(pto::createFusionPlanPass(fusionPlanOpts));
    ...
    pm.addNestedPass<mlir::func::FuncOp>(pto::createPTORematerializeFixpipeVectorQuantPass());
    ...
    if (effectiveLevel != PTOBuildLevel::Level3) {
        ...
        if (planMemoryImpl == "modern" && planMemoryOrderBySize.getNumOccurrences() == 0) {
    ...
    if (enableInsertSync) {
        if (emitMlirIR)
            pm.addPass(std::make_unique<SerialAutoSyncPass>(SerialAutoSyncPass::Mode::InsertSync, false, 0));
        else
            pm.addNestedPass<func::FuncOp>(pto::createPTOInsertSyncPass());
    } else if (enableBufidSync) {
        if (emitMlirIR) {
            pm.addPass(std::make_unique<SerialAutoSyncPass>(SerialAutoSyncPass::Mode::Bufid, enableBufidSyncDebug, 0));
        } else {
            PTOBufidSyncOptions options;
        ...
        if (emitMlirIR)
            pm.addPass(std::make_unique<SerialAutoSyncPass>(SerialAutoSyncPass::Mode::BarrierAll, false, 0));
        else
        ...
        if (emitMlirIR) {
            pm.addPass(std::make_unique<SerialAutoSyncPass>(
                SerialAutoSyncPass::Mode::GraphSolver, false, graphSyncSolverEventIdMax));
        } else {
            PTOGraphSyncSolverOptions options;
            options.eventIdNumMax = graphSyncSolverEventIdMax;
            pm.addNestedPass<func::FuncOp>(pto::createPTOGraphSyncSolverPass(options));
        }
    }
    ...
    if (effectiveBackend == PTOBackend::VPTO) {
        ...
        if (failed(runVPTOBackendPipeline(module, hasTileOpsToExpand, expandOptions ? &*expandOptions : nullptr)))
            return 1;
        return emitVPTOBackendResult(*module, result, emitVPTOHostStub, context.getCANNVersionOrDefault());
    }
    ...
    if (failed(applyConfiguredPassManagerCLOptions(emitcPM, "EmitC backend pipeline")))
        return 1;
