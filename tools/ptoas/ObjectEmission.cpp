...
static llvm::cl::opt<BishengVFAutoSyncMode> bishengVFAutoSyncMode(
    ...
    llvm::cl::values(
        clEnumValN(BishengVFAutoSyncMode::Off, "off", "Disable Bisheng VF auto-sync"),
        clEnumValN(BishengVFAutoSyncMode::Fused, "fused", "Use Bisheng fused VF auto-sync"),
        clEnumValN(BishengVFAutoSyncMode::Global, "global", "Use Bisheng global VF auto-sync")),
    llvm::cl::init(BishengVFAutoSyncMode::Unspecified));

static bool runCommandWithStderr(
    llvm::StringRef program, llvm::ArrayRef<std::string> ownedArgs, llvm::StringRef stderrPath,
    llvm::raw_ostream& diagOS, llvm::StringRef what, std::optional<llvm::StringRef> stdinPath = std::nullopt);

static bool writeTextFile(StringRef path, StringRef content, llvm::raw_ostream& diagOS)
{
    std::error_code ec;
    llvm::raw_fd_ostream os(path, ec, llvm::sys::fs::OF_Text);
    if (ec) {
        diagOS << "Error: failed to open " << path << " for write: " << ec.message() << "\n";
        return false;
    }
    os << content;
    os.flush();
...
static void stripUnsupportedBishengAttrs(llvm::Module& module)
{
    for (llvm::Function& function : module) {
        ...
        function.setAttributes(
            function.getAttributes().removeFnAttribute(module.getContext(), llvm::Attribute::Memory));
    }
}

static bool writeLLVMModuleFile(llvm::Module& module, StringRef path, llvm::raw_ostream& diagOS)
{
    std::error_code ec;
    llvm::raw_fd_ostream os(path, ec, llvm::sys::fs::OF_Text);
    if (ec) {
...
static std::optional<std::string> discoverCANNVersion(llvm::StringRef ascendHome)
{
    for (llvm::StringRef relPath :
         {"compiler/version.info", "x86_64-linux/ascend_toolkit_install.info",
          "x86_64-linux/ascend_all_cann_install.info", "aarch64-linux/ascend_toolkit_install.info",
          "aarch64-linux/ascend_all_cann_install.info", "ascend_toolkit_install.info", "ascend_all_cann_install.info",
          "opp/version.info"}) {
...
static std::optional<std::string> locateProgram(llvm::StringRef envPath, llvm::StringRef fallbackName)
{
...
static void addExistingIncludeDir(llvm::SmallVectorImpl<std::string>& dirs, llvm::StringRef path)
{
...
static void addPTOISAIncludeDirs(llvm::SmallVectorImpl<std::string>& dirs, llvm::StringRef ptoIsaPath)
{
...
static llvm::SmallVector<std::string, 8> discoverCppIncludeDirs(
    llvm::StringRef ascendHome, llvm::raw_ostream& diagOS, std::string& ptoIsaPath)
{
    ...
    std::string driverPath = getEnvPath("ASCEND_DRIVER_PATH").value_or("/usr/local/Ascend/driver");
...
static bool compileHostStubToObject(
    llvm::StringRef stubPath, llvm::StringRef outObjPath, llvm::StringRef moduleId, llvm::StringRef targetCPU,
    const mlir::pto::CANNToolchain& toolchain, llvm::StringRef deviceObjPath, llvm::StringRef stderrPath,
    llvm::raw_ostream& diagOS);
static bool mergeDeviceObjects(
    llvm::ArrayRef<std::string> deviceObjPaths, llvm::StringRef outObjPath, llvm::StringRef ldLldPath,
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS);

static llvm::StringRef getTargetCPU(mlir::pto::ObjectEmissionDeviceTarget target)
{
    switch (target) {
...
static std::string resolveTargetCPU(llvm::Module& module, mlir::pto::ObjectEmissionDeviceTarget fallback)
{
...
public:
    explicit VPTOFatobjArtifacts(mlir::pto::TempFileRegistry& tempFiles) : tempFiles(tempFiles) {}
    ...
    bool emitCubeObject(llvm::Module* module, const mlir::pto::CANNToolchain& toolchain, llvm::raw_ostream& diagOS)
    {
        if (!module)
            return true;
        ...
        if (failed(tempFiles.create("ptoas-device", ".o", cubeObjPath, diagOS)))
            return false;
        return succeeded(
            mlir::pto::emitVPTOCubeDeviceObject(*module, cubeLLPath, cubeObjPath, toolchain, stderrPath, diagOS));
    }

    bool emitVectorObject(
        llvm::Module* module, const mlir::pto::CANNToolchain& toolchain, mlir::pto::VFSIMTSizeFixMode vfsimtSizeFixMode,
        llvm::raw_ostream& diagOS)
    {
        if (!module)
            return true;
        ...
        if (failed(tempFiles.create("ptoas-device-vector-raw", ".o", rawVectorObjPath, diagOS)))
            return false;
        if (failed(mlir::pto::emitVPTOVectorDeviceObject(
                *module, vectorLLPath, rawVectorObjPath, toolchain, stderrPath, diagOS)))
            return false;
        ...
        if (failed(tempFiles.create("ptoas-device-vector-patched", ".o", patchedVectorObjPath, diagOS)))
            return false;
        mlir::FailureOr<mlir::pto::VFSIMTSizePatchResult> result = mlir::pto::verifyAndPatchVFSIMTSize(
            *module, rawVectorObjPath, patchedVectorObjPath, vfsimtSizeFixMode, diagOS);
        if (failed(result))
            return false;
    ...
    bool mergeDeviceObjects(const mlir::pto::CANNToolchain& toolchain, llvm::raw_ostream& diagOS)
    {
        ...
        if (failed(tempFiles.create("ptoas-device-merged", ".o", mergedDeviceObjPath, diagOS)))
            return false;
        return ::mergeDeviceObjects(deviceObjPaths, mergedDeviceObjPath, toolchain.ldLldPath, stderrPath, diagOS);
    }

    bool compileHostStub(
        const mlir::pto::CANNToolchain& toolchain, llvm::StringRef moduleId, llvm::StringRef targetCPU,
        llvm::raw_ostream& diagOS)
    {
        if (failed(tempFiles.create("ptoas-host-stub", ".o", hostStubObjPath, diagOS)))
            return false;
        return compileHostStubToObject(
            stubPath, hostStubObjPath, moduleId, targetCPU, toolchain, mergedDeviceObjPath, stderrPath, diagOS);
    }

    bool compileHostStubToFatobj(
        const mlir::pto::CANNToolchain& toolchain, llvm::StringRef moduleId, llvm::StringRef targetCPU,
        llvm::StringRef outputPath, llvm::raw_ostream& diagOS)
    {
        return compileHostStubToObject(
            stubPath, outputPath, moduleId, targetCPU, toolchain, mergedDeviceObjPath, stderrPath, diagOS);
    }
        ...
        llvm::StringRef outPath, llvm::raw_ostream& diagOS)
    {
        ...
        return runCommandWithStderr(toolchain.cceLdPath, args, stderrPath, diagOS, "fatobj repack");
    }
...
static bool runCommandWithStderr(
    llvm::StringRef program, llvm::ArrayRef<std::string> ownedArgs, llvm::StringRef stderrPath,
    llvm::raw_ostream& diagOS, llvm::StringRef what, std::optional<llvm::StringRef> stdinPath)
{
    llvm::SmallVector<llvm::StringRef, 16> args;
    args.reserve(ownedArgs.size());
    for (const std::string& arg : ownedArgs)
        args.push_back(arg);
    llvm::SmallVector<std::optional<llvm::StringRef>, 3> redirects = {stdinPath, stderrPath, stderrPath};

    std::string execErr;
    bool execFailed = false;
    int rc = llvm::sys::ExecuteAndWait(program, args, std::nullopt, redirects, 0, 0, &execErr, &execFailed);
    if (!execFailed && rc == 0)
        return true;
    ...
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    ...
    return runCommandWithStderr(bishengPath, args, stderrPath, diagOS, "device LLVM compilation", llPath);
}

static bool compileCppDeviceSourceToObject(
    llvm::StringRef cppPath, llvm::StringRef outObjPath, llvm::StringRef targetCPU,
    const mlir::pto::CANNToolchain& toolchain, llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    ...
    return runCommandWithStderr(toolchain.bishengPath, args, stderrPath, diagOS, "C++ device compilation");
}

static bool compileCppDeviceSourceToFatobj(
    llvm::StringRef cppPath, llvm::StringRef outObjPath, const mlir::pto::CANNToolchain& toolchain,
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    llvm::SmallVector<std::string, 32> args = {
        toolchain.bishengPath,
    ...
    return runCommandWithStderr(toolchain.bishengPath, args, stderrPath, diagOS, "C++ fatobj compilation");
}
    ...
    llvm::raw_ostream& diagOS)
{
    ...
    return runCommandWithStderr(toolchain.bishengCc1Path, args, stderrPath, diagOS, "host stub compilation");
}
    ...
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    ...
    llvm::SmallVector<std::string, 16> args = {
        ldLldPath.str(), "-m", "aicorelinux", "-Ttext", "0",
    };
    ...
    return runCommandWithStderr(ldLldPath, args, stderrPath, diagOS, "device object merge");
}

static bool linkFatobjFiles(
    llvm::ArrayRef<std::string> fatobjPaths, llvm::StringRef outObjPath, const mlir::pto::CANNToolchain& toolchain,
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    ...
    llvm::SmallVector<std::string, 32> args = {
        toolchain.bishengPath, "--cce-fatobj-link", "--cce-aicore-arch=dav-c310", "-r", "-o", outObjPath.str(),
    };
    for (const std::string& path : fatobjPaths)
        args.push_back(path);

    return runCommandWithStderr(toolchain.bishengPath, args, stderrPath, diagOS, "fatobj link");
}
...
mlir::LogicalResult mlir::pto::TempFileRegistry::create(
    llvm::StringRef prefix, llvm::StringRef suffix, std::string& path, llvm::raw_ostream& diagOS)
{
    llvm::SmallString<128> tempPath;
    int fd = -1;
    std::error_code ec = llvm::sys::fs::createTemporaryFile(prefix, suffix, fd, tempPath);
    if (ec) {
        diagOS << "Error: failed to create temporary file for " << prefix << suffix << ": " << ec.message() << "\n";
        return failure();
    }
...
std::optional<mlir::pto::CANNToolchain> mlir::pto::CANNToolchain::create(llvm::raw_ostream& diagOS)
{
    ...
    toolchain.bishengCc1Path = joinPath(toolchain.ascendHomePath, "tools/bisheng_compiler/bin/bisheng");
    toolchain.cceLdPath = joinPath(toolchain.ascendHomePath, "bin/cce-ld");
    toolchain.ldLldPath =
        locateProgram(joinPath(toolchain.ascendHomePath, "bin/ld.lld"), "ld.lld").value_or(std::string());
    toolchain.resourceDirPath = joinPath(toolchain.ascendHomePath, "tools/bisheng_compiler/lib/clang/15.0.5");
    toolchain.resourceIncludeDirPath = joinPath(toolchain.resourceDirPath, "include");
    toolchain.cceStubDirPath = joinPath(toolchain.resourceIncludeDirPath, "cce_stub");
    toolchain.bishengCompilerBinDirPath = joinPath(toolchain.ascendHomePath, "tools/bisheng_compiler/bin");
    toolchain.cannVersionString = discoverCANNVersion(toolchain.ascendHomePath).value_or("9.0.0-beta.1");
    llvm::SmallVector<std::string, 8> cppIncludeDirs =
        discoverCppIncludeDirs(toolchain.ascendHomePath, diagOS, toolchain.ptoIsaPath);
    toolchain.cppIncludeDirs.assign(cppIncludeDirs.begin(), cppIncludeDirs.end());
    if (failed(toolchain.validate(diagOS)))
        return std::nullopt;
    return toolchain;
}

mlir::LogicalResult mlir::pto::CANNToolchain::validate(llvm::raw_ostream& diagOS) const
{
    ...
    if (!llvm::sys::fs::exists(bishengCc1Path)) {
        diagOS << "Error: unable to locate bisheng cc1 frontend: " << bishengCc1Path << "\n";
        return failure();
    }
...
llvm::StringRef mlir::pto::CANNToolchain::vptoPublicABISuffix(ObjectEmissionDeviceTarget target) const
{
...
mlir::LogicalResult mlir::pto::writeLLVMModule(llvm::Module& module, llvm::StringRef path, llvm::raw_ostream& diagOS)
{
    return writeLLVMModuleFile(module, path, diagOS) ? success() : failure();
}

mlir::LogicalResult mlir::pto::writeCppSource(
    llvm::StringRef cppSource, llvm::StringRef path, llvm::raw_ostream& diagOS)
{
    return writeTextFile(path, cppSource, diagOS) ? success() : failure();
}

mlir::LogicalResult mlir::pto::writeHostStubSource(
    llvm::StringRef stubSource, llvm::StringRef path, llvm::raw_ostream& diagOS)
{
...
{
    return compileCppDeviceSourceToObject(cppPath, outObjPath, getTargetCPU(target), toolchain, stderrPath, diagOS) ?
               success() :
               failure();
}
    ...
    const CANNToolchain& toolchain, llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    return compileDeviceLLVMToObject(
               llPath, outObjPath, getTargetCPU(target), toolchain.bishengPath, stderrPath, diagOS) ?
               success() :
               failure();
}
    ...
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    if (failed(writeCppSource(cppSource, cppPath, diagOS)))
        return failure();
    return compileCppToDeviceObject(
        cppPath, outObjPath, ObjectEmissionDeviceTarget::Vector, toolchain, stderrPath, diagOS);
}
    ...
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    if (failed(writeCppSource(cppSource, cppPath, diagOS)))
        return failure();
    return compileCppToDeviceObject(
        cppPath, outObjPath, ObjectEmissionDeviceTarget::Cube, toolchain, stderrPath, diagOS);
}
    ...
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    if (failed(writeCppSource(cppSource, cppPath, diagOS)))
        return failure();
    return compileCppDeviceSourceToFatobj(cppPath, outObjPath, toolchain, stderrPath, diagOS) ? success() : failure();
}
    ...
    llvm::raw_ostream& diagOS)
{
    ...
    if (failed(tempFiles.create("ptoas-emitc", ".cpp", cppPath, diagOS)) ||
        failed(tempFiles.create("ptoas-emitc-fatobj", ".log", stderrPath, diagOS)))
        return failure();
    return emitCppFatobj(cppSource, cppPath, outputPath, toolchain, stderrPath, diagOS);
}
...
static mlir::LogicalResult renameLLVMFunction(
    llvm::Module& module, llvm::StringRef sourceName, llvm::StringRef abiName, llvm::raw_ostream& diagOS)
{
    ...
    if (!function)
        return mlir::success();
    if (llvm::Function* existing = module.getFunction(abiName); existing && existing != function) {
        diagOS << "Error: cannot rename LLVM symbol '" << sourceName << "' to '" << abiName
               << "': target symbol already exists.\n";
        return mlir::failure();
    }
...
static mlir::LogicalResult applyVPTOLLVMABINames(
    llvm::Module& module, llvm::StringRef suffix, llvm::raw_ostream& diagOS)
{
    for (llvm::Function& function : module) {
        if (function.isDeclaration() || !function.hasExternalLinkage())
            continue;
        llvm::StringRef name = function.getName();
        if (name.empty() || isVPTOKernelABISymbol(name) || isLegacyVPTOPublicABISymbol(name))
            continue;
...
{
    if (failed(
            applyVPTOLLVMABINames(module, toolchain.vptoPublicABISuffix(ObjectEmissionDeviceTarget::Vector), diagOS)))
        return failure();
    if (failed(writeLLVMModule(module, llPath, diagOS)))
        return failure();
    return compileDeviceLLVMToObject(
               llPath, outObjPath, resolveTargetCPU(module, ObjectEmissionDeviceTarget::Vector), toolchain.bishengPath,
               stderrPath, diagOS) ?
               success() :
               failure();
}
...
mlir::LogicalResult mlir::pto::emitVPTOCubeDeviceObject(
    ...
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    if (failed(applyVPTOLLVMABINames(module, toolchain.vptoPublicABISuffix(ObjectEmissionDeviceTarget::Cube), diagOS)))
        return failure();
    if (failed(writeLLVMModule(module, llPath, diagOS)))
        return failure();
    return compileDeviceLLVMToObject(
               llPath, outObjPath, resolveTargetCPU(module, ObjectEmissionDeviceTarget::Cube), toolchain.bishengPath,
               stderrPath, diagOS) ?
               success() :
               failure();
}
    ...
    VFSIMTSizeFixMode vfsimtSizeFixMode, llvm::raw_ostream& diagOS)
{
    ...
    if (!artifacts.emitCubeObject(cubeModule, toolchain, diagOS))
        return failure();
    if (!artifacts.emitVectorObject(vectorModule, toolchain, vfsimtSizeFixMode, diagOS))
        return failure();
    if (!artifacts.mergeDeviceObjects(toolchain, diagOS))
        return failure();

    constexpr llvm::StringLiteral targetCPU = "dav-c310";
    if (!artifacts.compileHostStubToFatobj(toolchain, moduleId, targetCPU, outputPath, diagOS))
        return failure();
    return success();
}
...
mlir::LogicalResult mlir::pto::mergeDeviceObjects(
    ...
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    return ::mergeDeviceObjects(deviceObjPaths, outObjPath, toolchain.ldLldPath, stderrPath, diagOS) ? success() :
                                                                                                       failure();
}
    ...
    const CANNToolchain& toolchain, llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    constexpr llvm::StringLiteral targetCPU = "dav-c310";
    return compileHostStubToObject(
               stubPath, outputPath, moduleId, targetCPU, toolchain, deviceObjPath, stderrPath, diagOS) ?
               success() :
               failure();
}
...
mlir::LogicalResult mlir::pto::linkFatobjs(
    ...
    llvm::StringRef stderrPath, llvm::raw_ostream& diagOS)
{
    return linkFatobjFiles(fatobjPaths, outputPath, toolchain, stderrPath, diagOS) ? success() : failure();
}
    ...
    VFSIMTSizeFixMode vfsimtSizeFixMode, llvm::raw_ostream& diagOS)
{
    ...
    if (!artifacts.repackFatObj(*toolchain, moduleId, hostTargetCPU, outputFile.getFilename(), diagOS))
        return failure();
    outputFile.keep();
    return success();
}
