...
static llvm::SmallVector<uint64_t, kValueIdInlineCapacity> readKnownOperandIds(
    BuildCtx& bc, Reader& r, uint16_t opcode, uint8_t variant, const ptobc::v0::OpInfo& info,
    const KnownOpImmediates& imms)
{
    ...
    switch (info.operand_mode) {
        ...
        case 0x04:
            return reorderLegacyIndexedTscatter(readValueIds(
                r, ((imms.optMask & 0x1) ? 1 : 0) + ((imms.optMask & 0x2) ? 1 : 0) + ((imms.optMask & 0x4) ? 1 : 0)));
        default:
            (void)bc;
    ...
    std::vector<std::vector<mlir::Operation*>>* opsByFuncOut)
{
    ...
    if (dbg) {
        llvm::errs() << "[ptobc] func body built ok: values=" << bc.values.size() << " ops=" << opsById.size() << "\n";
    }
    if (opsByFuncOut)
