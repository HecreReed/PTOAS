...
static bool shouldEncodeViaGenericV0CompatibilityShim(mlir::Operation& op)
{
    ...
    if (llvm::isa<mlir::pto::CmoCacheInvalidOp, mlir::pto::FenceBarrierAllOp>(&op))
        return true;
...
static bool isFrontendPipeOpWithDefaultId(mlir::Operation& op)
{
    ...
    return name == "pto.aic_initialize_pipe" || name == "pto.aiv_initialize_pipe" || name == "pto.talloc_to_aiv" ||
           name == "pto.talloc_to_aic" || name == "pto.tpush_to_aiv" || name == "pto.tpush_to_aic" ||
           name == "pto.tpop_from_aic" || name == "pto.tpop_from_aiv" || name == "pto.tfree_from_aic" ||
           name == "pto.tfree_from_aiv";
}

static mlir::DictionaryAttr dropDefaultZeroPipeIdForV0Encoding(mlir::Operation& op, mlir::DictionaryAttr dict)
{
    ...
    switch (info.imm_kind) {
        ...
        case 0x05: {
            ...
            if (auto intAttr = llvm::dyn_cast<mlir::IntegerAttr>(attr)) {
                ...
                uint64_t typeId = internType(file, cst.getType());
                constId = internConstFloatBits(typeId, floatAttr.getValue().bitcastToAPInt());
            } else {
        ...
        case 0x06: {
            ...
            imms.append({listMode, uint64_t(mtv.getShape().size()), uint64_t(mtv.getStrides().size())});
            return;
        }
        case 0x07: {
            ...
            imms.append({listMode, uint64_t(pv.getOffsets().size()), uint64_t(pv.getSizes().size())});
            return;
        }
        case 0x08: {
    ...
    llvm::ArrayRef<uint64_t> imms)
{
    ...
    switch (info.operand_mode) {
        ...
        case 0x04: {
            ...
            emitOperands(((mask & 0x1) ? 1 : 0) + ((mask & 0x2) ? 1 : 0) + ((mask & 0x4) ? 1 : 0));
            return;
        }
        default:
