...
static bool requiresGenericForExtendedTileOp(mlir::Operation &op) {
  if (hasOptionalOperand<mlir::pto::TCIOp>(
      ...
      hasOptionalOperand<mlir::pto::TExtractOp>(
          op, [](auto typedOp) {
            // The ND-to-2xNZ dual-output form must ride a generic v0 record:
            // it cannot reuse the shipped four/five-operand fixed-width
            // opcodes (design doc 8.2).
            return typedOp.getPreQuantScalar() || typedOp.isNdTo2xNzForm();
          }) ||
