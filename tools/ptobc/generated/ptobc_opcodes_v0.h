...
struct VariantOperandInfo {
  uint16_t opcode;
  uint8_t variant;
  uint8_t numOperands;
};

inline constexpr FullNameInfo kVariantFullNameTable[] = {
    ...
    {"pto.tgemv.mx.acc", {0x102A, 1, 4}},
    {"pto.tgemv.mx.bias", {0x102A, 1, 5}},
...
inline constexpr VariantOperandInfo kVariantOperandTable[] = {
    {0x102A, 0, 3}, {0x102A, 1, 4}, {0x102A, 2, 4},
    {0x102A, 3, 5}, {0x102A, 4, 6}, {0x102A, 5, 6},
...
inline std::optional<uint16_t> lookupOpcodeByName(llvm::StringRef name) {
  ...
  if (name == "pto.section")
    return static_cast<uint16_t>(0x0006);
  return std::nullopt;
}
...
inline std::optional<OpcodeAndVariant>
lookupVariantOpcodeByFullName(llvm::StringRef fullName) {
  for (const FullNameInfo &info : kVariantFullNameTable) {
...
lookupOpcodeAndVariantByFullName(llvm::StringRef fullName) {
  if (auto variantInfo = lookupVariantOpcodeByFullName(fullName))
    return variantInfo;
...
inline const char *fullNameFromOpcodeVariant(uint16_t opcode, uint8_t variant) {
  for (const FullNameInfo &info : kVariantFullNameTable) {
    if (info.value.opcode == opcode && info.value.variant == variant)
      return info.name;
  }
...
inline std::optional<int> lookupOperandsByVariant(uint16_t opcode, uint8_t variant) {
  for (const VariantOperandInfo &info : kVariantOperandTable) {
    if (info.opcode == opcode && info.variant == variant)
      return static_cast<int>(info.numOperands);
  }
  return std::nullopt;
}
