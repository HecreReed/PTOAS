...
from ptodsl._kernel_signature import (
    DeviceParameterSpec,
...
def explicit_layout_tensor_view_probe(
    ...
    k_view = pto.make_tensor_view(
        K_ptr, shape=[rows, cols], strides=[1, rows], layout="DN"
    )
    _ = k_view
...
def runtime_metadata_kernel(
    ...
    *,
    ...
    a_view = pto.make_tensor_view(
        A_ptr, shape=[rows, cols], strides=[row_stride, col_stride]
    )
    o_view = pto.make_tensor_view(
        O_ptr, shape=[rows, cols], strides=[row_stride, col_stride]
    )
    a_tile = pto.alloc_tile(shape=[1, BLOCK], dtype=pto.f32, valid_shape=[rows, cols])
    o_tile = pto.alloc_tile(shape=[1, BLOCK], dtype=pto.f32, valid_shape=[rows, cols])
    part = pto.partition_view(a_view, offsets=[0, 0], sizes=[rows, cols])
...
def authored_addr_tile_surface_probe(
    ...
    tile = pto.alloc_tile(
        shape=[1, 128], dtype=pto.f32, addr=0, valid_shape=[rows, cols]
    )
    _ = tile
...
def tile_surface_compute_probe():
    ...
    expect(
        reshape_1d.shape == (512,),
        "pto.tile.reshape(..., shape=[...]) should expose the authored logical result shape",
    )
    expect(
        reshape_1d.physical_shape == (1, 512),
        "rank-1 pto.tile.reshape(...) should materialize the authored 1D physical shape",
    )
    expect(
        str(reshape_1d.dtype) == str(reshape_src.dtype),
        "pto.tile.reshape(..., dtype omitted) should preserve the source element type",
    )
    expect(
        reshape_1d.memory_space == reshape_src.memory_space,
        "pto.tile.reshape(...) should preserve the source memory space",
    )
    expect(
    ...
    expect(
        reshape_col.shape == (8, 64),
        "same-rank pto.tile.reshape(...) should preserve the authored logical result rank",
    )
    expect(
    ...
    expect(
        "valid_shape" in reshape_col.surface_metadata
        and reshape_col.surface_metadata["valid_shape"] is None,
...
def nested_simd_probe():
    ...
    SUBKERNEL_OBSERVATIONS.append(
        (frame.role, frame.symbol_name, session.subkernel_stack_depth)
    )
...
def inline_subkernel_scope_probe(*, TRACE_TOKEN: pto.const_expr = 0):
    ...
    with pto.simt():
        frame = session.current_subkernel
        INLINE_SUBKERNEL_SCOPE_OBSERVATIONS.append(
            (frame.role, frame.symbol_name, session.subkernel_stack_depth)
        )
...
def simt_collective_math_probe():
    ...
    as_f32 = pto.convert(
        lane, pto.f32, rounding="r", saturation="nosat", signedness="signed"
    )
...
@pto.jit(target="a5")
def ast_runtime_for_static_slot_constexpr_index_probe(
    cols: pto.i32, *, SLOT: pto.const_expr = 2
):
    zero = pto.const(0, dtype=pto.index)
    accs = [zero for _ in pto.static_range(4)]
...
ast_signature_closure_default_kernel_probe = make_ast_signature_closure_default_kernel(
    2
)
...
def simt_reserved_buffer_peer_probe():
    ...
    imported = pto.import_reserved_buffer(
        "simt_c2v_fifo", peer_func=simt_reserved_buffer_peer
    )
    _ = imported
...
def simt_reserved_buffer_ambiguous_peer_probe():
    ...
    pto.import_reserved_buffer(
        "simt_c2v_fifo", peer_func=simt_reserved_buffer_ambiguous_peer
    )
...
@pto.tileop
def public_vector_surface_probe(
    inp_tile: pto.Tile, out_tile: pto.Tile, stats_tile: pto.Tile
):
...
def public_cube_tile_mx_probe(
    ...
    pto.tile.matmul_mx_acc(
        mat_acc, mat_lhs, mat_lhs_scale, mat_rhs, mat_rhs_scale, mat_acc
    )
    pto.tile.matmul_mx_bias(
        mat_lhs, mat_lhs_scale, mat_rhs, mat_rhs_scale, mat_bias, mat_acc
    )
    pto.tile.gemv_mx(gemv_lhs, gemv_lhs_scale, gemv_rhs, gemv_rhs_scale, gemv_acc)
    pto.tile.gemv_mx_acc(
        gemv_acc, gemv_lhs, gemv_lhs_scale, gemv_rhs, gemv_rhs_scale, gemv_acc
    )
    pto.tile.gemv_mx_bias(
        gemv_lhs, gemv_lhs_scale, gemv_rhs, gemv_rhs_scale, gemv_bias, gemv_acc
    )
...
def public_mask_surface_probe():
    ...
    packed_hi_b16 = pto.ppack(
        mask32_full, pto.PredicatePart.HIGHER, to_type=pto.mask_b16
    )
    unpacked_hi_b32 = pto.punpack(
        packed_hi_b16, pto.PredicatePart.HIGHER, to_type=pto.mask_b32
    )
...
def public_data_movement_surface_probe():
    ...
    pto.vstsx2(
        low0, high0, ub_dst, pto.const(0), pto.InterleaveDist.INTLV_B16, mask16_full
    )
...
def main() -> None:
    ...
    for name in expected_public_exports:
        expect(
            hasattr(pto, name),
            f"pto.{name} should be exported from the public namespace",
        )

    fake_tensor = _FakeTensor((2, 3, 4))
    fake_empty = pto.empty_like(fake_tensor)
    expect(
        isinstance(fake_empty, _FakeTensor),
        "pto.empty_like(...) should preserve host tensor factory type",
    )
    expect(
        fake_empty.shape == fake_tensor.shape,
        "pto.empty_like(...) should preserve the logical tensor shape",
    )
    expect(
        not hasattr(pto, "scalar"),
        "pto.scalar should not remain in the public pto namespace",
    )
    expect(
        hasattr(pto, "tile"), "pto.tile should be exported from the public namespace"
    )
    expect(hasattr(pto, "vmi"), "pto.vmi should be exported from the public namespace")
    expect(
        hasattr(pto.tile, "load"),
        "pto.tile.load should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.tile, "add"),
        "pto.tile.add should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.tile, "cmps"),
        "pto.tile.cmps should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.vmi, "vreg"),
        "pto.vmi.vreg should be exported from the public VMI namespace",
    )
    expect(
        hasattr(pto.vmi, "mask"),
        "pto.vmi.mask should be exported from the public VMI namespace",
    )
    expect(
        hasattr(pto.vmi, "vadd"),
        "pto.vmi.vadd should be exported from the public VMI namespace",
    )
    expect(
        hasattr(pto.vmi, "create_mask"),
        "pto.vmi.create_mask should be exported from the public VMI namespace",
    )
    expect(
        not hasattr(pto, "load_tile"),
        "pto.load_tile should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "store_tile"),
        "pto.store_tile should not remain on the public pto namespace",
    )
    expect(
        hasattr(pto.tile, "matmul"),
        "pto.tile.matmul should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.tile, "matmul_acc"),
        "pto.tile.matmul_acc should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.tile, "matmul_mx"),
        "pto.tile.matmul_mx should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.tile, "matmul_mx_acc"),
        "pto.tile.matmul_mx_acc should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.tile, "matmul_mx_bias"),
        "pto.tile.matmul_mx_bias should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.tile, "gemv_mx"),
        "pto.tile.gemv_mx should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.tile, "gemv_mx_acc"),
        "pto.tile.gemv_mx_acc should be exported from the public tile namespace",
    )
    expect(
        hasattr(pto.tile, "gemv_mx_bias"),
        "pto.tile.gemv_mx_bias should be exported from the public tile namespace",
    )
    expect(
        not hasattr(pto, "tload"),
        "legacy pto.tload should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "tstore"),
        "legacy pto.tstore should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "tadd"),
        "legacy pto.tadd should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "get_buf_dyn"),
        "pto.get_buf_dyn should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "rls_buf_dyn"),
        "pto.rls_buf_dyn should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "tile_buf_type"),
        "pto.tile_buf_type should not remain on the public pto namespace",
    )
    expect(
        hasattr(pto, "vecscope"),
        "pto.vecscope should be exported from the public pto namespace",
    )
    expect(
        not hasattr(pto, "as_ptr"),
        "pto.as_ptr should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "vbrc_load"),
        "pto.vbrc_load should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "vsts_1pt"),
        "pto.vsts_1pt should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "constexpr"),
        "pto.const_expr should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "copy_ubuf_to_ubuf"),
        "pto.copy_ubuf_to_ubuf should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "vmi_vreg_type"),
        "pto.vmi_vreg_type should not remain on the public pto namespace",
    )
    expect(
        not hasattr(pto, "vmi_mask_type"),
        "pto.vmi_mask_type should not remain on the public pto namespace",
    )
    expect(
        not hasattr(scalar, "sts"),
        "scalar.sts should not remain in the public scalar namespace",
    )
    expect(
        not hasattr(scalar, "cmpi"),
        "scalar.cmpi should not remain in the public scalar namespace",
    )
    expect(
        not hasattr(scalar, "cmpi_sgt"),
        "scalar.cmpi_sgt should not remain in the public scalar namespace",
    )
    removed_tile_buf_type = expect_raises(
        AttributeError, lambda: getattr(pto, "tile_buf_type")
    )
    expect(
        "pto.tile_buf_type is not a supported PTODSL public interface"
        in str(removed_tile_buf_type),
    ...
    expect(
        "pto.vbrc_load is not a supported PTODSL public interface"
        in str(removed_vbrc_load),
    ...
    expect(
        "pto.vsts_1pt is not a supported PTODSL public interface"
        in str(removed_vsts_1pt),
    ...
    expect(
        "pto.constexpr is not a supported PTODSL public interface"
        in str(removed_constexpr)
        and "Use pto.const_expr" in str(removed_constexpr),
        "removed pto.constexpr should diagnose pto.const_expr as the replacement",
    )
    removed_copy_ubuf_to_ubuf = expect_raises(
        AttributeError, lambda: getattr(pto, "copy_ubuf_to_ubuf")
    )
    expect(
        "pto.copy_ubuf_to_ubuf is not a supported PTODSL public interface"
        in str(removed_copy_ubuf_to_ubuf)
    ...
    for name in ("max", "min", "exp", "log", "sqrt", "abs"):
        expect(
            hasattr(scalar, name),
            f"scalar.{name} should be exported from the public scalar namespace",
        )
    ...
    with make_context() as ctx, Location.unknown(ctx):
        ...
        expect(
            hasattr(tile_buf_ty, "memory_space"),
            "TileBufType should expose a memory_space accessor",
        )
        expect(
            hasattr(tile_buf_ty, "shape"), "TileBufType should expose a shape accessor"
        )
        expect(
            hasattr(tile_buf_ty, "valid_shape"),
            "TileBufType should expose a valid_shape accessor",
        )
        expect(
            hasattr(tile_buf_ty, "element_type"),
            "TileBufType should expose an element_type accessor",
        )
        expect(
            tile_buf_ty.memory_space.value == pto.MemorySpace.MAT.value,
            "TileBufType.memory_space should preserve the authored address space",
        )
        expect(
            list(tile_buf_ty.shape) == [16, 32],
            "TileBufType.shape should preserve the authored physical shape",
        )
        expect(
            list(tile_buf_ty.valid_shape) == [16, 8],
            "TileBufType.valid_shape should preserve the authored valid shape",
        )
        expect(
            str(tile_buf_ty.element_type) == "f32",
            "TileBufType.element_type should preserve the authored element type",
        )
    ...
    with make_context() as ctx, Location.unknown(ctx):
        ...
        expect(
            str(pto.vmi.vreg(128, pto.f8e4m3).resolve())
            == "!pto.vmi.vreg<128xf8E4M3FN>",
    ...
    expect(
        default_compiled is explicit_default,
        "default constexpr compile should hit specialization cache",
    )
    expect(
        default_compiled is not block64,
        "different constexpr values should materialize different specializations",
    )
    expect(
        len(host_vec_copy.cached_specializations()) == 2,
        "expected exactly two cached specializations",
    )
    expect(
        default_compiled.constexpr_bindings == {"BLOCK": 128},
        "default constexpr binding mismatch",
    )
    expect(
        block64.constexpr_bindings == {"BLOCK": 64},
        "BLOCK=64 constexpr binding mismatch",
    )
    expect_raises(
        TypeError,
    ...
    expect(
        default_compiled.specialization_key.abi_signature
        == block64.specialization_key.abi_signature,
    ...
    pointer_explicit_default = pointer_runtime_shape_specialization_probe.compile(
        BLOCK=128
    )
    ...
    expect(
        pointer_default.specialization_key.abi_signature
        == pointer_block64.specialization_key.abi_signature,
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        expect(
            source_compiled_v1 is source_compiled_v1_again,
            "unchanged source-backed JIT should hit specialization cache",
        )
        expect(
        ...
        expect(
            source_compiled_v1.build_metadata()["source_path"]
        ...
        expect(
            source_compiled_v2.specialization_key
            != source_compiled_v1.specialization_key,
        ...
        @pto.jit(
            name="source_auto_native",
            target="a5",
            backend="vpto",
            source=str(source_auto_path),
        )
        ...
        source_no_insert_sync_path.write_text(
            source_no_insert_sync_text, encoding="utf-8"
        )
        ...
        source_no_insert_sync_native_build_compiled = (
            source_no_insert_sync_native.compile()
        )
        ...
        spec = spec_from_file_location(
            "ptodsl_relative_source_case", relative_module_path
        )
        expect(
            spec is not None and spec.loader is not None,
            "relative source test module should be importable",
        )
        ...
        expect(
            relative_compiled.build_metadata()["source_path"]
            == str(relative_source_path.resolve()),
    ...
    expect(
        pointer_artifacts_default.cache_dir
        == pointer_artifacts_explicit_default.cache_dir,
    ...
    marshaled_small = _marshal_launch_args(
        pointer_default._kernel_signature, (0x1000, 16, 32, 32)
    )
    marshaled_padded = _marshal_launch_args(
        pointer_default._kernel_signature, (0x1000, 7, 29, 64)
    )
    expect(
        len(marshaled_small) == 4,
        "pointer-first launch ABI should marshal one pointer plus rows/cols/stride scalars",
    )
    expect(
        len(marshaled_padded) == 4,
        "pointer-first launch ABI width should stay constant across dynamic shape launches",
    )
    expect(
        marshaled_small[1].value == 16
        and marshaled_small[2].value == 32
        and marshaled_small[3].value == 32,
        "contiguous runtime-shape launch should preserve rows/cols/stride values",
    )
    expect(
        marshaled_padded[1].value == 7
        and marshaled_padded[2].value == 29
        and marshaled_padded[3].value == 64,
    ...
    expect_parse_roundtrip_and_verify(
        default_text, "default host_vec_copy specialization"
    )
    expect_parse_roundtrip_and_verify(
        block64_text, "BLOCK=64 host_vec_copy specialization"
    )
    expect_parse_roundtrip_and_verify(
        explicit_text, "explicit host_vec_copy specialization"
    )
    expect(
        "!pto.tile_buf<vec, 1x128xf32>" in default_text,
        "default specialization MLIR missing BLOCK=128 tile",
    )
    expect(
        "!pto.tile_buf<vec, 1x64xf32>" in block64_text,
        "BLOCK=64 specialization MLIR missing specialized tile",
    )
    expect(
        "pto.entry" in default_text,
        "default @pto.jit entry child should carry the explicit entry marker",
    )
    expect(
        "pto.entry" in explicit_text,
        "explicit @pto.jit entry child should carry the explicit entry marker",
    )
    expect(
        default_text.count("module") == 2,
        "default @pto.jit should wrap an unspecified-kind kernel in a backend child module",
    )
    expect(
        block64_text.count("module") == 2,
        "specialized @pto.jit should keep the backend child module shape",
    )
    expect(
    ...
    expect(
        "pto.mode = " not in default_text,
        "generated PTODSL container IR should no longer expose public pto.mode",
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        emitc_entry_text, "emitc host_vec_copy specialization"
    )
    expect(
    ...
    emitc_helper_text = emitc_entry_calls_emitc_vector_kernel_module_metadata_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        emitc_helper_text,
    ...
    expect(
        isinstance(helper_params[0], HelperMarkerParameterSpec)
        and helper_params[0].annotation is pto.Tile,
        "entry=False kernel module should accept pto.Tile parameters",
    )
    expect(
        isinstance(helper_params[1], HelperMarkerParameterSpec)
        and helper_params[1].annotation is pto.TensorView,
        "entry=False kernel module should accept pto.TensorView parameters",
    )
    expect(
        isinstance(helper_params[2], HelperMarkerParameterSpec)
        and helper_params[2].annotation is pto.PartitionTensorView,
    ...
    expect(
        "helper_device_abi_surface_probe" in helper_repr
        and "CompiledKernelHandle" not in helper_repr,
        "repr(@pto.jit(entry=False)) should expose lightweight metadata without triggering compilation",
    )
    helper_cache_signature = (
        helper_device_abi_surface_probe.__ptodsl_cache_signature__()
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        kernel_module_call_text, "entry calling kernel-module specialization"
    )
    expect(
    ...
    expect(
        kernel_module_call_text.count("func.func private @process_tile_module__ptodsl_")
        >= 1,
    ...
    ast_rewrite_kernel_module_text = (
        entry_calls_ast_rewrite_kernel_module_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_rewrite_kernel_module_text,
    ...
    expect(
        "func.func public @ast_rewrite_kernel_module_probe__ptodsl_"
        in ast_rewrite_kernel_module_text,
    ...
    expect(
        kernel_module_graph.dependencies
        == (("entry_calls_kernel_module_probe", ("process_tile_module",)),),
        "kernel-module callsite lowering should record one caller->callee dependency edge",
    )
    expect(
        len(kernel_module_graph.imports) == 1
        and kernel_module_graph.imports[0].caller_symbol_name
        == "entry_calls_kernel_module_probe"
        and kernel_module_graph.imports[0].target_symbol_name.startswith(
            "process_tile_module__ptodsl_"
        )
    ...
    mixed_backend_text = (
        emitc_entry_calls_vpto_kernel_module_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        mixed_backend_text, "emitc entry calling vpto kernel-module specialization"
    )
    expect(
        'module attributes {pto.backend = "emitc", pto.target_arch = "a5"}'
    ...
    expect(
        "func.func public @process_row_ptr_kernel_module__ptodsl_("
        not in mixed_backend_text,
    ...
    expect(
        ...
        and "func.call @process_row_ptr_kernel_module__ptodsl_" in mixed_backend_text
        and ": (!pto.ptr<f32, gm>, !pto.ptr<f32, gm>, index) -> ()"
        in mixed_backend_text
    ...
    dynamic_buf_emitc_text = (
        emitc_entry_calls_dynamic_buf_kernel_module_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        dynamic_buf_emitc_text,
    ...
    expect(
        "emitc_vpto_kernel_module_callsite_simd_helper__ptodsl_"
        not in decorated_mixed_backend_text,
    ...
    expect(
        multi_abi_graph.dependencies
        == (
            ("entry_calls_kernel_module_multiple_abi_probe", ("process_tile_module",)),
        ),
    ...
    native_build_variants = (
        ...
        (
            ...
            emitc_entry_calls_vpto_kernel_module_probe.compile(),
            None,
        ),
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        def fake_compile_launch_cpp(
            ...
            *,
            ...
            expect(
                launch_cpp.is_file(),
                "native build should materialize launch.cpp before compiling it",
            )
            expect(
            ...
            expect(
                export_macro.endswith("_EXPORTS"),
                "native build should preserve launch export macro naming",
            )
            launch_object.write_text("fake launch object\n", encoding="utf-8")

        def fake_link_shared_library(
            launch_object, kernel_object, shared_library, *, kernel_kind
        ):
            expect(
                launch_object.is_file(),
                "native build should compile launch.cpp before linking",
            )
            expect(
                kernel_object.is_file(),
                "native build should run ptoas before shared-library link",
            )
            expect(
                kernel_kind in {None, "vector", "cube"},
        ...
        with (
            mock.patch.object(
                native_build_runtime, "artifact_paths", side_effect=fake_artifacts
            ),
            mock.patch.object(
                native_build_runtime, "is_native_build_current", return_value=False
            ),
            mock.patch.object(
                native_build_runtime, "_run_ptoas", side_effect=fake_run_ptoas
            ),
            mock.patch.object(
                native_build_runtime,
                "_compile_launch_cpp",
                side_effect=fake_compile_launch_cpp,
            ),
            mock.patch.object(
                native_build_runtime,
                "_link_shared_library",
                side_effect=fake_link_shared_library,
            ),
            mock.patch.object(
                native_build_runtime, "runtime_library_flags", return_value=("-laclrt",)
            ),
        ):
            for label, compiled, module_spec_override in native_build_variants:
                ...
                expect(
                    lib_path.is_file(),
                    f"{label} native build should materialize the shared library artifact",
                )
                expect(
    ...
    for (label, compiled, module_spec_override), observation, launch_target_arch in zip(
        ...
        expected_backend = (
            module_spec.backend if module_spec.jit_source is not None else None
        )
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        with (
            mock.patch.object(
                ...
                return_value=Path("/tmp/fake-ptoas"),
            ),
            mock.patch.object(
        ...
        expect(
            len(ptoas_cmds) == 1,
            "native build should issue exactly one ptoas command per kernel container",
        )
        ...
        expect(
            "--enable-tile-op-expand" in ptoas_cmd
            and str(mlir_path) in ptoas_cmd
            and str(kernel_object) in ptoas_cmd,
        ...
        expect(
            len(ptoas_cmds) == 1,
            "native build should issue exactly one ptoas command when insert_sync is forced on",
        )
        expect(
        ...
        expect(
            len(ptoas_cmds) == 1,
            "native build should issue exactly one ptoas command with explicit-mode PTOAS policy",
        )
    ...
    expect(
        "valid=?" not in default_text,
        "default alloc_tile() should keep full static valid-shape when valid_shape= is omitted",
    )
    ...
    merged_cross_mode_text = str(
        pto.merge_jit_modules(host_vec_copy.compile(), host_vec_copy_explicit.compile())
    )
    expect_parse_roundtrip_and_verify(
        merged_cross_mode_text, "merged cross-mode PTODSL container"
    )
    expect(
    ...
    merged_same_mode_text = str(
        pto.merge_jit_modules(host_vec_copy.compile(), host_vec_copy.compile(BLOCK=64))
    )
    expect_parse_roundtrip_and_verify(
        merged_same_mode_text, "merged same-mode PTODSL container"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        runtime_metadata_text, "runtime metadata specialization"
    )
    expect(
        re.search(
    ...
    expect_parse_roundtrip_and_verify(
        explicit_layout_text, "explicit tensor_view layout specialization"
    )
    expect(
        re.search(
    ...
    expect_parse_roundtrip_and_verify(
        tile_surface_text, "tile surface compute specialization"
    )
    tile_sort_gather_text = tile_sort_gather_surface_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        tile_sort_gather_text, "tile sort/gather surface specialization"
    )
    expect(
        "pto.texpands" in tile_surface_text,
        "pto.tile.expands should lower to pto.texpands",
    )
    expect(
        "pto.treshape" in tile_surface_text,
        "pto.tile.reshape should lower to pto.treshape",
    )
    ...
    expect(
        "pto.tsort32" in tile_sort_gather_text,
        "pto.tile.sort32 should lower to pto.tsort32",
    )
    expect(
        "pto.tmrgsort" in tile_sort_gather_text,
        "pto.tile.mrgsort should lower to pto.tmrgsort",
    )
    expect(
        tile_sort_gather_text.count("pto.tgather") == 2,
        "tile gather wrappers should lower to pto.tgather",
    )
    expect(
        "#pto.mask_pattern<P0101>" in tile_sort_gather_text,
        "pto.tile.gather should preserve P0101",
    )
    expect(
        "#pto.mask_pattern<P1010>" in tile_sort_gather_text,
        "pto.tgather should preserve P1010",
    )
    ...
    expect(
        "descending = true" in tile_ci_text,
        "pto.tile.ci(descending=True) should preserve the descending attribute in MLIR",
    )
    expect(
        re.search(
    ...
    expect_parse_roundtrip_and_verify(
        tile_window_matmul_text, "tile extract/insert/matmul specialization"
    )
    expect(
        "pto.textract" in tile_window_matmul_text,
        "pto.tile.extract should lower to pto.textract",
    )
    expect(
        "pto.tinsert" in tile_window_matmul_text,
        "pto.tile.insert should lower to pto.tinsert",
    )
    expect(
        "pto.tmatmul ins(" in tile_window_matmul_text,
        "pto.tile.matmul should lower to pto.tmatmul",
    )
    expect(
        "pto.tmatmul.acc ins(" in tile_window_matmul_text,
        "pto.tile.matmul_acc should lower to pto.tmatmul.acc",
    )
    expect(
        "!pto.tile_buf<left, 16x16xf16" in tile_window_matmul_text,
        "pto.tile.matmul lhs should preserve LEFT scratch tile typing",
    )
    expect(
        "!pto.tile_buf<right, 16x16xf16" in tile_window_matmul_text,
        "pto.tile.matmul rhs should preserve RIGHT scratch tile typing",
    )
    expect(
        "!pto.tile_buf<acc, 16x16xf32" in tile_window_matmul_text,
        "pto.tile.matmul/matmul_acc should preserve ACC destination typing",
    )
    expect(
        "pto.tinsert ins(" in tile_window_matmul_text
        and ", %c0, %c32 :" in tile_window_matmul_text,
        "pto.tile.insert should preserve the authored insertion offsets in MLIR",
    )
    expect(
        "pto.tmatmul.acc ins(" in tile_window_matmul_text
        and tile_window_matmul_text.count("%") >= 3,
    ...
    expect_parse_roundtrip_and_verify(
        tile_transfer_text, "tile transfer surface specialization"
    )
    expect(
        "pto.tload" in tile_transfer_text,
        "pto.tile.load(tensor, tile) should lower to pto.tload",
    )
    expect(
        "pto.tstore" in tile_transfer_text,
        "pto.tile.store(tile, tensor) should lower to pto.tstore",
    )
    expect(
        re.search(
            r"sizes = \[%[a-zA-Z0-9_]+, %[a-zA-Z0-9_]+\]",
    ...
    expect_parse_roundtrip_and_verify(
        authored_addr_tile_text, "authored alloc_tile addr specialization"
    )
    expect(
        re.search(
    ...
    expect_parse_roundtrip_and_verify(
        dynamic_addr_tile_text, "dynamic alloc_tile addr specialization"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        tile_valid_shape_text, "tile valid-shape update specialization"
    )
    expect(
        re.search(
    ...
    expect_parse_roundtrip_and_verify(
        tile_valid_shape_1d_text, "1D tile valid-shape update specialization"
    )
    expect(
        re.search(
            r"pto\.set_validshape %[a-zA-Z0-9_]+, %[a-zA-Z0-9_]+, %[a-zA-Z0-9_]+ : !pto\.tile_buf<vec, 1x128xf32, valid=\?x\?>",
    ...
    make_mask_index_roundtrip_text = (
        make_mask_index_roundtrip_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        make_mask_index_roundtrip_text, "make_mask index round-trip specialization"
    )
    expect(
        "pto.plt_b32 %arg2 : i32 -> !pto.mask<b32>, i32"
        in make_mask_index_roundtrip_text,
    ...
    expect_parse_roundtrip_and_verify(
        carry_static_pyint_init_text, "carry static pyint init specialization"
    )
    expect(
        re.search(
    ...
    fixed_integer_index_coercion_text = (
        fixed_integer_index_coercion_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        fixed_integer_index_coercion_text, "fixed integer index coercion specialization"
    )
    expect(
        re.search(
    ...
    shared_subkernel_text = shared_subkernel_lowering_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
        shared_subkernel_text, "shared subkernel lowering specialization"
    )
    expect(
        SUBKERNEL_OBSERVATIONS
        == [
    ...
    expect(
        re.search(
            r"call @top_level_cube_probe__ptodsl_[0-9a-f]+\(\)", shared_subkernel_text
        )
        is not None
        and re.search(
            r"call @top_level_simd_probe__ptodsl_[0-9a-f]+\(\)", shared_subkernel_text
        )
        is not None
        and re.search(
            r"call @nested_simd_probe__ptodsl_[0-9a-f]+\(\)", shared_subkernel_text
        )
        is not None,
    ...
    expect(
        ...
        in explicit_vector_inline_tileop_text
    ...
    inline_subkernel_scope_text = inline_subkernel_scope_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
        inline_subkernel_scope_text, "inline subkernel scope specialization"
    )
    expect(
        INLINE_SUBKERNEL_SCOPE_OBSERVATIONS
        == [
    ...
    expect(
        ...
        and re.search(r"pto\.section\.simt\s*<<<", inline_subkernel_scope_text)
        is not None,
        "inline pto.simt() should lower to one inline pto.section.simt region",
    )
    expect(
        re.search(
            r"call @inline_simt_[0-9]+__ptodsl_[0-9a-f]+\([^\\n]*\)",
            inline_subkernel_scope_text,
        )
        is None
        and re.search(
            r"call @inline_tileop_[0-9]+__ptodsl_[0-9a-f]+\([^\\n]*\)",
            inline_subkernel_scope_text,
        )
        is not None,
    ...
    inline_simt_launch_text = inline_simt_launch_dims_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
        inline_simt_launch_text, "inline simt launch-dims specialization"
    )
    expect(
    ...
    expect(
        re.search(
            r"func\.func @inline_simt_[0-9]+__ptodsl_[0-9a-f]+", inline_simt_launch_text
        )
        is None
    ...
    expect(
        "pto.get_tid_x" in simt_text, "SIMT helper body should contain pto.get_tid_x"
    )
    expect(
        "pto.get_tid_y" in simt_text, "SIMT helper body should contain pto.get_tid_y"
    )
    expect(
        "pto.get_tid_z" in simt_text, "SIMT helper body should contain pto.get_tid_z"
    )

    alloc_buffer_local_text = alloc_buffer_local_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        alloc_buffer_local_text, "alloc_buffer local specialization"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        alloc_buffer_top_level_text, "alloc_buffer top-level specialization"
    )
    expect(
        "llvm.alloca" in alloc_buffer_top_level_text
        and "pto.persistent" in alloc_buffer_top_level_text,
        "alloc_buffer outside a SIMT helper should be marked as a persistent fragment candidate",
    )
    alloc_buffer_tileop_helper_text = (
        alloc_buffer_tileop_helper_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        alloc_buffer_tileop_helper_text, "alloc_buffer tileop helper specialization"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        alloc_buffer_inline_simt_text, "alloc_buffer inline simt specialization"
    )
    expect(
        "llvm.alloca" in alloc_buffer_inline_simt_text
        and "pto.persistent" not in alloc_buffer_inline_simt_text,
    ...
    expect_parse_roundtrip_and_verify(
        rmsnorm_alloc_buffer_text, "RMSNorm hand-authored UB layout specialization"
    )
    ...
    expect(
        re.search(
            r"call @rmsnorm_alloc_buffer_frag_helper__simt_\d+\(",
            rmsnorm_alloc_buffer_text,
        )
        is not None,
    ...
    expect_parse_roundtrip_and_verify(
        simt_launch_text, "explicit simt launch specialization"
    )
    expect(
        re.search(r"pto\.simt_launch @simt_query_probe__simt_\d+<<<", simt_launch_text)
        is not None,
        "pto.simt_launch(...) should emit VPTO simt_launch sugar",
    )
    expect(
        re.search(
            r"func\.func @simt_query_probe__simt_\d+\(\) attributes \{pto\.simt_entry\}",
            simt_launch_text,
        )
        is not None,
        "explicit pto.simt_launch should materialize a reusable pto.simt_entry helper",
    )
    simt_launch_sugar_text = simt_launch_index_sugar_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
        simt_launch_sugar_text, "indexed simt launch specialization"
    )
    expect(
        re.search(
            r"pto\.simt_launch @simt_query_probe__simt_\d+<<<", simt_launch_sugar_text
        )
        is not None,
        "@pto.simt helper[x, y, z](...) should emit VPTO simt_launch sugar",
    )
    simt_grouped_query_text = simt_grouped_query_launch_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
        simt_grouped_query_text, "grouped simt query specialization"
    )
    expect(
        re.search(
            r"pto\.simt_launch @simt_grouped_query_probe__simt_\d+<<<",
            simt_grouped_query_text,
        )
        is not None,
    ...
    simt_resource_attr_text = simt_resource_attr_launch_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
        simt_resource_attr_text, "simt resource attr launch specialization"
    )
    expect(
        re.search(
    ...
    simt_arg_type_text = simt_specialized_arg_type_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
        simt_arg_type_text, "simt arg-type specialization"
    )
    expect(
        re.search(
            r"func\.func @simt_specialized_i32_ptr_probe__simt_\d+\(",
            simt_arg_type_text,
        )
        is not None
        and re.search(
            r"func\.func @simt_specialized_f32_ptr_probe__simt_\d+\(",
            simt_arg_type_text,
        )
        is not None,
    ...
    expect(
        "!pto.ptr<i32, gm>" in simt_arg_type_text
        and "!pto.ptr<f32, gm>" in simt_arg_type_text,
        "SIMT argument-type specializations should preserve distinct helper pointer types",
    )

    simt_static_kwarg_text = simt_specialized_static_kwarg_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
        simt_static_kwarg_text, "simt static kwarg specialization"
    )
    expect(
        len(
            re.findall(
                r"func\.func @simt_specialized_flag_probe__simt_\d+\(",
                simt_static_kwarg_text,
            )
        )
        == 2,
    ...
    expect(
        "pto.get_tid_x" in simt_static_kwarg_text,
        "FLAG=True SIMT specialization should emit get_tid_x",
    )
    expect(
        "pto.get_tid_y" in simt_static_kwarg_text,
        "FLAG=False SIMT specialization should emit get_tid_y",
    )

    simt_full_text = simt_full_surface_probe.compile(TRACE_TOKEN=1).mlir_text()
    expect_parse_roundtrip_and_verify(
        simt_full_text, "full simt surface specialization"
    )
    for op_name in (
        "pto.vote_all",
        "pto.vote_any",
    ...
    expect_raises(
        TypeError,
        lambda: simt_invalid_convert_signedness_launch.compile(
            TRACE_TOKEN=1
        ).mlir_text(),
        "requires signedness",
    )
    expect_raises(
        TypeError,
        lambda: simt_invalid_atomic_signedness_launch.compile(
            TRACE_TOKEN=1
        ).mlir_text(),
    ...
    ast_subkernel_runtime_for_text = (
        ast_subkernel_runtime_for_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_subkernel_runtime_for_text,
    ...
    expect(
        "pto.vlds" in ast_subkernel_runtime_for_text
        and "pto.vsts" in ast_subkernel_runtime_for_text,
    ...
    expect(
        "iter_args(" in carry_text,
        "carry loop should lower named state through scf.for iter_args",
    )
    expect(
        "scf.yield" in carry_text,
        "carry loop should lower loop.update(...) to scf.yield",
    )
    expect(
    ...
    expect(
        re.search(r"outs\(%[^\s]+#2 : !pto\.tile_buf<vec, 1x32xf32>\)", carry_text)
        is not None,
    ...
    expect_parse_roundtrip_and_verify(
        branch_then_only_text, "branch handle then-only specialization"
    )
    expect(
        branch_then_only_text.count("scf.if") == 1,
        "then-only branch handle should lower to one scf.if",
    )
    expect(
        "pto.barrier <PIPE_ALL>" in branch_then_only_text,
        "br.then_ body should lower into the scf.if then branch",
    )
    expect(
        "}, {" not in branch_then_only_text,
        "then-only branch handle should not materialize an else region",
    )

    branch_side_effect_text = branch_handle_side_effect_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        branch_side_effect_text, "branch handle side-effect specialization"
    )
    expect(
        branch_side_effect_text.count("scf.if") == 1,
        "side-effect branch handle should lower to one scf.if",
    )
    expect(
        "pto.barrier <PIPE_ALL>" in branch_side_effect_text,
        "br.then_ side effects should lower into the then branch",
    )
    expect(
        "pto.mem_bar" in branch_side_effect_text,
        "br.else_ side effects should lower into the else branch",
    )
    expect(
        "} else {" in branch_side_effect_text,
        "side-effect branch handle should materialize an explicit else region",
    )

    branch_merge_text = branch_handle_merge_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        branch_merge_text, "branch handle merge specialization"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        ast_if_side_effect_text, "AST-rewritten side-effect if specialization"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        ast_if_merge_text, "AST-rewritten if-merge specialization"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        ast_if_old_value_merge_text, "AST-rewritten old-value if-merge specialization"
    )
    expect(
    ...
    ast_if_branch_local_temp_liveness_text = (
        ast_if_branch_local_temp_liveness_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_if_branch_local_temp_liveness_text,
    ...
    expect_parse_roundtrip_and_verify(
        ast_nested_with_if_merge_text,
        "AST-rewritten nested with if-merge specialization",
    )
    expect(
        re.search(r"scf\.if %\d+ -> \(i32\)", ast_nested_with_if_merge_text)
        is not None,
    ...
    expect_parse_roundtrip_and_verify(
        ast_runtime_for_text, "AST-rewritten runtime for specialization"
    )
    expect(
    ...
    ast_nested_runtime_for_induction_scope_text = (
        ast_nested_runtime_for_induction_scope_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_nested_runtime_for_induction_scope_text,
    ...
    ast_nested_runtime_for_local_temp_scope_text = (
        ast_nested_runtime_for_local_temp_scope_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_nested_runtime_for_local_temp_scope_text,
    ...
    expect_parse_roundtrip_and_verify(
        ast_runtime_for_carry_text, "AST-rewritten runtime carry for specialization"
    )
    expect(
        "iter_args(" in ast_runtime_for_carry_text
        and "scf.yield" in ast_runtime_for_carry_text,
        "ast_rewrite=True accumulator loops should lower through scf.for iter_args",
    )

    ast_runtime_for_augassign_carry_text = (
        ast_runtime_for_augassign_carry_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_runtime_for_augassign_carry_text,
    ...
    expect(
        "iter_args(" in ast_runtime_for_augassign_carry_text
        and "scf.yield" in ast_runtime_for_augassign_carry_text,
        "ast_rewrite=True accumulator loops using += should lower through scf.for iter_args",
    )

    ast_runtime_for_branch_local_temp_text = (
        ast_runtime_for_branch_local_temp_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_runtime_for_branch_local_temp_text,
    ...
    ast_runtime_for_sibling_iv_reuse_text = (
        ast_runtime_for_sibling_iv_reuse_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_runtime_for_sibling_iv_reuse_text,
    ...
    ast_runtime_for_static_range_name_reuse_text = (
        ast_runtime_for_static_range_name_reuse_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_runtime_for_static_range_name_reuse_text,
    ...
    ast_runtime_for_static_slot_carry_text = (
        ast_runtime_for_static_slot_carry_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_runtime_for_static_slot_carry_text,
    ...
    ast_runtime_for_static_slot_global_index_text = (
        ast_runtime_for_static_slot_global_index_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_runtime_for_static_slot_global_index_text,
    ...
    ast_runtime_for_static_slot_binop_index_text = (
        ast_runtime_for_static_slot_binop_index_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_runtime_for_static_slot_binop_index_text,
    ...
    expect(
        ast_rewrite_disabled_nested_helper_python_control_text.count(
    ...
    ast_nested_helper_ast_rewrite_text = (
        ast_nested_helper_ast_rewrite_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_nested_helper_ast_rewrite_text,
    ...
    expect(
        "iter_args(" in ast_nested_helper_ast_rewrite_text
        and "scf.yield" in ast_nested_helper_ast_rewrite_text,
        "rewritten nested helpers should preserve loop-carried and branch live-out values",
    )

    ast_nested_helper_freevar_if_merge_text = (
        ast_nested_helper_freevar_if_merge_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_nested_helper_freevar_if_merge_text,
    ...
    expect(
        re.search(r"scf\.if %\d+ -> \(i32\)", ast_nested_helper_freevar_if_merge_text)
        is not None,
    ...
    ast_nested_helper_name_store_liveness_text = (
        ast_nested_helper_name_store_liveness_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_nested_helper_name_store_liveness_text,
    ...
    expect(
        re.search(r"scf\.if %\d+ ->", ast_nested_helper_name_store_liveness_text)
        is None,
    ...
    ast_control_flow_equiv_explicit_text = (
        ast_control_flow_equiv_explicit_probe.compile().mlir_text()
    )
    ast_control_flow_equiv_native_text = (
        ast_control_flow_equiv_native_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_control_flow_equiv_explicit_text,
    ...
    expect(
        mlir_op_sequence(ast_control_flow_equiv_native_text)
        == mlir_op_sequence(ast_control_flow_equiv_explicit_text),
    ...
    for pattern in ("scf.for", "scf.if", "iter_args(", "scf.yield", "arith.addi"):
        expect(
            ast_control_flow_equiv_native_text.count(pattern)
            == ast_control_flow_equiv_explicit_text.count(pattern),
    ...
    expect_parse_roundtrip_and_verify(
        ast_closure_kernel_text, "AST-rewritten closure kernel specialization"
    )
    expect(
    ...
    ast_rebound_closure_kernel_text = (
        ast_rebound_closure_kernel_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_rebound_closure_kernel_text,
    ...
    ast_signature_closure_default_text = (
        ast_signature_closure_default_kernel_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_signature_closure_default_text,
    ...
    ast_rebound_subkernel_text = ast_rebound_subkernel_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
    ...
    expect(
        ast_rebound_subkernel_text.count(
            "pto.simt_launch @tileop_noop_simt_probe__simt_"
        )
        == 4,
    ...
    sourceless_ast_rewrite_text = (
        sourceless_ast_rewrite_kernel_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        sourceless_ast_rewrite_text,
    ...
    sourceless_subkernel_text = sourceless_subkernel_entry_probe.compile(
        TRACE_TOKEN=1
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
    ...
    expect(
        sourceless_subkernel_text.count(
            "pto.simt_launch @tileop_noop_simt_probe__simt_"
        )
        == 1,
    ...
    ast_python_bool_guard_enabled_text = (
        ast_python_bool_guard_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_python_bool_guard_enabled_text,
    ...
    ast_static_enabled_text = ast_static_control_flow_probe.compile(
        ENABLE=True
    ).mlir_text()
    expect_parse_roundtrip_and_verify(
        ast_static_enabled_text, "AST static control-flow enabled specialization"
    )
    expect(
        "scf.if" not in ast_static_enabled_text
        and "scf.for" not in ast_static_enabled_text,
    ...
    ast_static_range_loop_target_live_after_text = (
        ast_static_range_loop_target_live_after_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_static_range_loop_target_live_after_text,
    ...
    expect(
        ast_static_range_loop_target_live_after_text.count("pto.barrier <PIPE_ALL>")
        == 3,
        "pto.static_range(...) should allow the Python loop target to remain live after unrolling",
    )
    ast_static_range_break_continue_text = (
        ast_static_range_break_continue_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_static_range_break_continue_text,
    ...
    ast_static_disabled_text = ast_static_control_flow_probe.compile(
        ENABLE=False
    ).mlir_text()
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        runtime_scalar_text, "runtime scalar operator specialization"
    )
    expect(
        "arith.index_cast" in runtime_scalar_text,
        "mixed i64/index runtime arithmetic should materialize index_cast",
    )
    expect(
        "arith.floordivsi" in runtime_scalar_text,
        "runtime // should lower to arith.floordivsi",
    )
    expect(
        "arith.remsi" in runtime_scalar_text, "runtime % should lower to arith.remsi"
    )
    expect(
        "arith.addf" in runtime_scalar_text,
        "runtime float + should lower to arith.addf",
    )
    expect(
        "arith.mulf" in runtime_scalar_text,
        "runtime float * should lower to arith.mulf",
    )
    expect(
        "arith.subf" in runtime_scalar_text,
        "runtime float - should lower to arith.subf",
    )
    expect(
        "arith.divf" in runtime_scalar_text,
        "runtime float / should lower to arith.divf",
    )
    expect(
        "arith.maximumf" in runtime_scalar_text,
        "scalar.max(float, float) should lower to arith.maximumf",
    )
    expect(
        "arith.minimumf" in runtime_scalar_text,
        "scalar.min(float, float) should lower to arith.minimumf",
    )
    expect(
        "math.exp" in runtime_scalar_text, "scalar.exp(...) should lower to math.exp"
    )
    expect(
        "math.log" in runtime_scalar_text, "scalar.log(...) should lower to math.log"
    )
    expect(
        "math.sqrt" in runtime_scalar_text, "scalar.sqrt(...) should lower to math.sqrt"
    )
    expect(
        "math.absf" in runtime_scalar_text,
        "scalar.abs(float) should lower to math.absf",
    )
    expect(
        "arith.cmpf ogt" in runtime_scalar_text,
        "float runtime '>' should lower to arith.cmpf ogt",
    )
    expect(
        "arith.cmpf oeq" in runtime_scalar_text,
        "float runtime '==' should lower to arith.cmpf oeq",
    )
    expect(
        "arith.cmpf oge" in runtime_scalar_text,
        "float runtime '>=' should lower to arith.cmpf oge",
    )
    expect(
        "arith.cmpf ole" in runtime_scalar_text,
        "float runtime '<=' should lower to arith.cmpf ole",
    )
    expect(
        "arith.andi" in runtime_scalar_text,
        "i1 conjunction from native '&' should lower to arith.andi",
    )

    host_runtime_scalar_entry_text = (
        host_runtime_scalar_entry_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        host_runtime_scalar_entry_text,
    ...
    expect(
        host_runtime_scalar_entry_probe.compile()
        is host_runtime_scalar_entry_probe.compile(),
    ...
    expect(
        "i32" in host_runtime_scalar_entry_text
        and "f32" in host_runtime_scalar_entry_text,
    ...
    expect_parse_roundtrip_and_verify(
        signed_integer_scalar_text, "signed integer scalar specialization"
    )
    expect(
    ...
    expect(
        "arith.addi" in signed_integer_scalar_text,
        "signed/unsigned scalar addition should lower through arith.addi",
    )
    expect(
        "arith.maxsi" in signed_integer_scalar_text,
        "signed scalar max should lower through arith.maxsi",
    )
    expect(
        "arith.minsi" in signed_integer_scalar_text,
        "signed scalar min should lower through arith.minsi",
    )
    expect(
        "arith.maxui" in signed_integer_scalar_text,
        "unsigned scalar max should lower through arith.maxui",
    )
    expect(
        "arith.minui" in signed_integer_scalar_text,
        "unsigned scalar min should lower through arith.minui",
    )
    expect(
        "math.absi" in signed_integer_scalar_text,
        "signed scalar abs should lower through math.absi",
    )
    expect(
        "arith.cmpi sgt" in signed_integer_scalar_text,
        "signed scalar cmp should preserve signed predicate",
    )
    expect(
        "arith.cmpi ugt" in signed_integer_scalar_text,
        "unsigned scalar cmp should preserve unsigned predicate",
    )
    expect(
        "pto.store" in runtime_scalar_text,
        "scalar.store(...) should lower to pto.store",
    )

    tile_slice_text = tile_slice_surface_probe.compile(BLOCK=128).mlir_text()
    expect_parse_roundtrip_and_verify(
        tile_slice_text, "tile slice surface specialization"
    )
    expect(
        "memref.subview" not in tile_slice_text,
        "tile[row, col:] should no longer lower through memref.subview",
    )
    expect(
        "pto.tile_buf_addr" in tile_slice_text,
        "tile[row, col:] should materialize a tile address pointer",
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        tile_slice_1d_text, "1D tile slice surface specialization"
    )
    expect(
        "memref.subview" not in tile_slice_1d_text,
        "tile[start:] should lower without memref.subview",
    )
    expect(
        "pto.vldas" in tile_slice_1d_text,
        "vldas(tile[start:]) should lower against the 1D slice view",
    )
    expect(
        "pto.vldus" in tile_slice_1d_text,
        "vldus(tile[start:], align) should lower against the 1D slice view",
    )
    expect(
        "pto.vsts" in tile_slice_1d_text,
        "vsts(vec, tile[start:], mask) should lower against the 1D slice view",
    )

    integer_loop_text = integer_loop_bound_probe.compile(BLOCK=8).mlir_text()
    expect_parse_roundtrip_and_verify(
        integer_loop_text, "integer loop bound specialization"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        scalar_pointer_offset_text, "scalar pointer offset specialization"
    )
    expect(
        re.search(r"pto\.store %c1_i32, %\d+\[%c1\]", scalar_pointer_offset_text)
        is not None,
        "scalar.store(ptr, 1) should lower as element offset 1",
    )
    expect(
        re.search(r"pto\.store %c2_i32, %\d+\[%c2\]", scalar_pointer_offset_text)
        is not None,
        "scalar.store(ptr + 2) should lower as element offset 2",
    )
    expect(
        re.search(r"pto\.load %\d+\[%c1(?:_\d+)?\]", scalar_pointer_offset_text)
        is not None,
        "scalar.load(ptr, 1) should lower as element offset 1",
    )
    expect(
        re.search(r"pto\.load %\d+\[%c2(?:_\d+)?\]", scalar_pointer_offset_text)
        is not None,
    ...
    expect_parse_roundtrip_and_verify(
        scalar_contiguous_text, "scalar contiguous vector specialization"
    )
    expect(
        "llvm.load" in scalar_contiguous_text,
        "scalar.load(..., contiguous=N) should lower to llvm.load",
    )
    expect(
        "llvm.store" in scalar_contiguous_text,
        "scalar.store(vector, ...) should lower to llvm.store",
    )
    expect(
        "vector<4xf32>" in scalar_contiguous_text,
        "contiguous=4 over f32 should produce vector<4xf32>",
    )
    expect(
        "llvm.insertelement" in scalar_contiguous_text,
        "pto.Vec(..., init=scalar) should broadcast with insertelement",
    )
    expect(
        "arith.mulf" in scalar_contiguous_text,
        "VecValue multiplication should lower to arith.mulf",
    )
    expect(
        "pto.load" not in scalar_contiguous_text
        and "pto.store" not in scalar_contiguous_text,
    ...
    expect_parse_roundtrip_and_verify(
        vec_arith_text, "scalar contiguous vector arithmetic specialization"
    )
    expect(
    ...
    expect(
        "arith.addf" in vec_arith_text, "VecValue addition should lower to arith.addf"
    )
    expect(
        "arith.subf" in vec_arith_text,
        "VecValue subtraction should lower to arith.subf",
    )
    expect(
        "arith.mulf" in vec_arith_text,
        "VecValue multiplication should lower to arith.mulf",
    )
    expect(
        "vector<4xf32>" in vec_arith_text,
        "vector arithmetic should keep vector<4xf32> type",
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        addptr_surface_text, "addptr surface specialization"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        simt_pointer_offset_text, "simt pointer offset specialization"
    )
    expect(
        re.search(
            r"call @simt_pointer_offset_helper__simt_\d+", simt_pointer_offset_text
        )
        is not None,
    ...
    expect(
        re.search(
            r"pto\.store %c9_i32, %(?:arg0|\d+)\[%c1(?:_\d+)?\]",
    ...
    scalar_store_coercion_text = (
        scalar_store_element_coercion_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        scalar_store_coercion_text, "scalar store coercion specialization"
    )
    expect(
    ...
    expect(
        re.search(r"pto\.store %\w+, %\d+\[%c0(?:_\d+)?\]", scalar_store_coercion_text)
        is not None,
    ...
    expect_parse_roundtrip_and_verify(
        shared_index_coercion_text, "shared index coercion specialization"
    )
    expect(
    ...
    expect(
        "scf.for" in shared_index_coercion_text,
        "shared index coercion should lower i32 loop bounds to scf.for",
    )
    expect(
        "pto.addptr" in shared_index_coercion_text,
        "shared index coercion should lower i32 pointer offsets",
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        public_surface_text, "public surface export specialization"
    )
    explicit_mx_scale_staging_text = (
        explicit_mx_scale_staging_surface_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        explicit_mx_scale_staging_text,
    ...
    expect_parse_roundtrip_and_verify(
        acc_store_pre_quant_text, "acc-store pre_quant public mode specialization"
    )
    expect(
        "pre_quant(%" in acc_store_pre_quant_text
        and "mode = f32_bf16" in acc_store_pre_quant_text,
    ...
    ast_runtime_index_bitwise_event_text = (
        ast_runtime_index_bitwise_event_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        ast_runtime_index_bitwise_event_text,
    ...
    data_movement_surface_text = (
        public_data_movement_surface_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        data_movement_surface_text, "public data movement surface specialization"
    )
    vector_conversion_surface_text = (
        public_vector_conversion_surface_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        vector_conversion_surface_text,
        "public vector conversion surface specialization",
    )
    low_precision_memory_surface_text = (
        low_precision_vector_memory_surface_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        low_precision_memory_surface_text,
        "low-precision vector memory surface specialization",
    )
    low_precision_vcvt_surface_text = (
        low_precision_vcvt_surface_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        low_precision_vcvt_surface_text, "low-precision vcvt surface specialization"
    )
    vdup_surface_text = vdup_surface_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        vdup_surface_text, "public vdup surface specialization"
    )
    vecscope_surface_text = vecscope_surface_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        vecscope_surface_text, "public vecscope surface specialization"
    )
    vmulscvt_surface_text = vmulscvt_surface_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        vmulscvt_surface_text, "public vmulscvt surface specialization"
    )
    vmula_surface_text = vmula_surface_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        vmula_surface_text, "public vmula surface specialization"
    )
    vmadd_surface_text = vmadd_surface_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        vmadd_surface_text, "public vmadd surface specialization"
    )
    vsstb_post_update_surface_text = (
        vsstb_post_update_surface_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        vsstb_post_update_surface_text, "vsstb post-update surface specialization"
    )
    vmi_wrapper_dispatch_text = vmi_wrapper_dispatch_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        vmi_wrapper_dispatch_text, "public VMI wrapper dispatch specialization"
    )
    vmi_vhist_signless_source_text = (
        vmi_vhist_signless_source_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        vmi_vhist_signless_source_text,
    ...
    expect_parse_roundtrip_and_verify(
        vmi_unpack_vload_text, "public VMI unpack vload specialization"
    )
    vmi_brc_vload_text = vmi_brc_vload_probe.compile().mlir_text()
    expect_parse_roundtrip_and_verify(
        vmi_brc_vload_text, "public VMI brc vload specialization"
    )
    expect(
    ...
    expect_parse_roundtrip_and_verify(
        vmi_group_brc_vload_text, "public VMI grouped brc vload specialization"
    )
    expect(
        'dist_mode = "brc"' in vmi_group_brc_vload_text
        and "group = 8" in vmi_group_brc_vload_text,
        "pto.vmi.vload should allow the grouped brc form exposed by the VMI IR contract",
    )
    fixed_width_integer_text = (
        fixed_width_integer_specialization_probe.compile().mlir_text()
    )
    expect_parse_roundtrip_and_verify(
        fixed_width_integer_text, "fixed-width integer specialization"
    )
    ...
    expect_parse_roundtrip_and_verify(
        vmi_round_r_vcvt_text, "VMI R-rounding vcvt specialization"
    )
    expect(
    ...
    expect(
        "pto.mte_gm_ub" in public_surface_text,
        "mte_load(...) should lower to pto.mte_gm_ub",
    )
    expect(
        "pto.mte_ub_gm" in public_surface_text,
        "mte_store(...) should lower to pto.mte_ub_gm",
    )
    expect(
        re.search(
    ...
    expect(
        public_surface_text.count("pto.mem_bar") >= 1,
        "mem_bar(...) should still lower explicit memory barriers",
    )
    expect(
        "pto.barrier <PIPE_ALL>" in public_surface_text,
        "pipe_barrier(Pipe.ALL) should lower to pto.barrier",
    )
    expect(
        "pto.trap" in trap_surface_text,
        "pto.trap() should lower to the public pto.trap operation",
    )
    expect("pto.vexp" in public_surface_text, "vexp(...) should lower to pto.vexp")
    expect(
        "pto.vcgmax" in public_surface_text, "vcgmax(...) should lower to pto.vcgmax"
    )
    expect(
        "pto.vcgadd" in public_surface_text, "vcgadd(...) should lower to pto.vcgadd"
    )
    expect("pto.vsqz" in public_surface_text, "vsqz(...) should lower to pto.vsqz")
    expect(
        "pto.vadds" in public_surface_text,
        "vsubs(...) should lower via scalar negation plus pto.vadds",
    )
    expect(
        "pto.mte_l1_l0a" in public_surface_text,
        "mte_l1_l0a(...) should lower to pto.mte_l1_l0a",
    )
    expect(
        "start(" not in public_surface_text,
        "mte_l1_l0a/l0b start_row/start_col should lower as operands",
    )
    expect(
        'pto.get_buf "PIPE_V", 0, 0' in sync_surface_text,
        "get_buf(Pipe.V, 0) should lower to pto.get_buf with PIPE_V",
    )
    expect(
        'pto.rls_buf "PIPE_MTE2", 1, 2' in sync_surface_text,
        "rls_buf(Pipe.MTE2, 1, 2) should lower to pto.rls_buf with PIPE_MTE2",
    )
    expect(
        "pto.get_buf_dyn" in dynamic_buf_sync_text,
        "get_buf_dyn should lower to pto.get_buf_dyn",
    )
    expect(
        "pto.rls_buf_dyn" in dynamic_buf_sync_text,
        "rls_buf_dyn should lower to pto.rls_buf_dyn",
    )
    expect(
    ...
    expect(
        "pto.set_flag[<PIPE_MTE2>, <PIPE_V>, <EVENT_ID0>]" in sync_surface_text,
        "set_flag(..., event_id=0) should lower static event ids to pto.set_flag",
    )
    expect(
        "pto.wait_flag[<PIPE_MTE2>, <PIPE_V>, <EVENT_ID0>]" in sync_surface_text,
        "wait_flag(..., event_id=0) should lower static event ids to pto.wait_flag",
    )
    expect(
        "pto.set_flag_dyn[<PIPE_V>, <PIPE_MTE3>, %c3]" in sync_surface_text,
        "set_flag(..., event_id=dynamic_event) should lower runtime event ids to pto.set_flag_dyn",
    )
    expect(
        "pto.wait_flag_dyn[<PIPE_V>, <PIPE_MTE3>, %c3]" in sync_surface_text,
        "wait_flag(..., event_id=dynamic_event) should lower runtime event ids to pto.wait_flag_dyn",
    )
    expect(
        "arith.andi" in explicit_runtime_index_bitwise_event_text,
        "explicit pto.for_ index & event id should lower to arith.andi",
    )
    expect(
        "arith.ori" in explicit_runtime_index_bitwise_event_text,
        "explicit pto.for_ index | event id should lower to arith.ori",
    )
    expect(
        "arith.xori" in explicit_runtime_index_bitwise_event_text,
        "explicit pto.for_ index ^ event id should lower to arith.xori",
    )
    expect(
        re.search(r"arith\.andi .* : index", explicit_runtime_index_bitwise_event_text)
        is not None,
    ...
    expect(
        explicit_runtime_index_integer_bitwise_event_text.count("arith.index_cast")
        >= 2,
        "index/integer bitwise event ids should coerce integer runtime scalars to index",
    )
    expect(
        explicit_runtime_index_integer_bitwise_event_text.count("pto.wait_flag_dyn")
        == 2,
        "index/integer bitwise event ids should lower waits to pto.wait_flag_dyn",
    )
    expect(
        explicit_runtime_index_integer_bitwise_event_text.count("pto.set_flag_dyn")
        == 1,
    ...
    expect(
        "pto.sync.set <PIPE_FIX>, 0" in sync_surface_text,
        "set_cross_flag(Pipe.FIX, 0) should lower to pto.sync.set",
    )
    expect(
        "pto.sync.wait <PIPE_FIX>, 0" in sync_surface_text,
        "wait_cross_flag(Pipe.FIX, 0) should lower to pto.sync.wait",
    )
    expect(
        "pto.sync.set <PIPE_MTE3>, %c3" in sync_surface_text,
        "set_intra_flag(Pipe.MTE3, dynamic_event) should lower dynamic event ids through pto.sync.set",
    )
    expect(
        "pto.sync.set <PIPE_FIX>, 4" in sync_surface_text,
        "set_intra_flag(Pipe.FIX, 4) should lower physical event ids through pto.sync.set",
    )
    expect(
        "pto.sync.wait <PIPE_V>, %c3" in sync_surface_text,
        "wait_intra_flag(Pipe.V, dynamic_event) should lower dynamic event ids through pto.sync.wait",
    )
    expect(
        "pto.sync.wait <PIPE_FIX>, 20" in sync_surface_text,
        "wait_intra_flag(Pipe.FIX, 20) should lower physical event ids through pto.sync.wait",
    )
    expect(
        "pto.sync.wait <PIPE_MTE3>, %c3" in sync_surface_text,
        "wait_intra_flag(Pipe.MTE3, dynamic_event) should lower dynamic event ids through pto.sync.wait",
    )
    expect(
        "pto.sync.wait <PIPE_MTE3>, 31" in sync_surface_text,
        "wait_intra_flag(Pipe.MTE3, 31) should lower the static physical event id through pto.sync.wait",
    )
    expect(
    ...
    expect(
        "pto.mte_ub_gm" in data_movement_surface_text,
        "public grouped UB->GM wrapper should lower to pto.mte_ub_gm",
    )
    expect(
    ...
    expect(
        "pto.mte_ub_ub" in data_movement_surface_text,
        "public grouped UB->UB wrapper should lower to pto.mte_ub_ub",
    )
    expect(
        "pto.mte_ub_l1" in data_movement_surface_text,
        "public grouped UB->L1 wrapper should lower to pto.mte_ub_l1",
    )
    expect(
        "pto.mte_gm_l1" in data_movement_surface_text,
        "public grouped GM->L1 wrapper should lower to pto.mte_gm_l1",
    )
    expect(
        "pto.mte_l1_ub" in data_movement_surface_text,
        "public grouped L1->UB wrapper should lower to pto.mte_l1_ub",
    )
    expect(
        "pto.mte_gm_l1_frac" in data_movement_surface_text,
        "public GM->L1 frac wrapper should lower to pto.mte_gm_l1_frac",
    )
    expect(
        "pto.mte_l1_bt" in data_movement_surface_text,
        "public L1->BT wrapper should lower to pto.mte_l1_bt",
    )
    expect(
        "pto.mte_l1_fb" in data_movement_surface_text,
        "public L1->FB wrapper should lower to pto.mte_l1_fb",
    )
    expect(
        "pto.vldas" in data_movement_surface_text,
        "vldas(...) should lower to pto.vldas",
    )
    expect(
        "pto.vldus" in data_movement_surface_text,
        "vldus(...) should lower to pto.vldus",
    )
    expect(
        "pto.vldsx2" in data_movement_surface_text,
        "vldsx2(...) should lower to pto.vldsx2",
    )
    expect(
        "pto.vstur" in data_movement_surface_text,
        "vstur(...) should lower to pto.vstur",
    )
    expect(
        "pto.vstus" in data_movement_surface_text,
        "vstus(...) should lower to pto.vstus",
    )
    expect(
        "pto.vstsx2" in data_movement_surface_text,
        "vstsx2(...) should lower to pto.vstsx2",
    )
    expect(
        "pto.vgather2" in data_movement_surface_text,
        "vgather2(...) should lower to pto.vgather2",
    )
    expect(
        "pto.vgather2_bc" in data_movement_surface_text,
        "vgather2_bc(...) should lower to pto.vgather2_bc",
    )
    expect(
        "pto.vgatherb" in data_movement_surface_text,
        "vgatherb(...) should lower to pto.vgatherb",
    )
    expect(
        "pto.vscatter" in data_movement_surface_text,
        "vscatter(...) should lower to pto.vscatter",
    )
    expect(
        "pto.vsldb" in data_movement_surface_text,
        "vsldb(...) should lower to pto.vsldb",
    )
    expect(
        "pto.vsstb" in data_movement_surface_text,
        "vsstb(...) should lower to pto.vsstb",
    )
    expect(
        re.search(
            r"arith\.index_cast %[a-zA-Z0-9_]+ : index to i16", fixed_width_integer_text
        )
        is not None,
    ...
    expect(
        "pto.vstar" in data_movement_surface_text,
        "vstar(...) should lower to pto.vstar",
    )
    expect(
        "pto.vstas" in data_movement_surface_text,
        "vstas(...) should lower to pto.vstas",
    )
    expect(
        "pto.vlds" in vector_conversion_surface_text,
        "vlds(..., post_update=ON) should lower through pto.vlds on the current VPTO Python surface",
    )
    expect(
        "-> !pto.vreg<64xf32>, !pto.ptr<f32, ub>" in vector_conversion_surface_text,
        "vlds(..., post_update=ON) should request the updated source pointer result",
    )
    expect(
        "pto.vcvt" in vector_conversion_surface_text,
        "vcvt(...) should lower to pto.vcvt",
    )
    expect(
        'rnd = "R"' in vector_conversion_surface_text,
        "vcvt(..., rnd=VcvtRoundMode.R) should preserve the authored rounding attr",
    )
    expect(
        'sat = "SAT"' in vector_conversion_surface_text,
        "vcvt(..., sat=VcvtSatMode.SAT) should preserve the authored saturation attr",
    )
    expect(
        'part = "EVEN"' in vector_conversion_surface_text,
        "vcvt(..., part=VcvtPartMode.EVEN) should preserve the authored part attr",
    )
    expect(
        "pto.vsts" in vector_conversion_surface_text,
        "vsts(..., post_update=ON) should lower through pto.vsts on the current VPTO Python surface",
    )
    expect(
        vector_conversion_surface_text.count("-> !pto.ptr<f16, ub>") >= 1,
        "vsts(..., post_update=ON) should request the updated destination pointer result",
    )
    expect(
        'dist = "PK_B32"' in vector_conversion_surface_text,
        "vsts(..., dist=VStoreDist.PK_B32) should preserve the authored store distribution",
    )
    expect(
        "pto.vpack" in vector_conversion_surface_text,
        "vpack(...) should lower to pto.vpack",
    )
    expect(
        "!pto.vreg<128xui16>" in vector_conversion_surface_text,
        "vpack(i32/u32 -> u16) should infer the unsigned packed result type",
    )
    expect(
        "!pto.vreg<256xf8E4M3FN>" in low_precision_memory_surface_text,
        "vlds/vsts should support f8e4m3 vreg storage",
    )
    expect(
        "!pto.vreg<256x!pto.hif8>" in low_precision_memory_surface_text,
        "vlds/vsts should support hif8 vreg storage",
    )
    expect(
        "!pto.ptr<f8E4M3FN, ub>" in low_precision_memory_surface_text,
        "low-precision f8 pointers should lower as UB pointers",
    )
    expect(
        "!pto.vreg<256xf8E4M3FN>" in low_precision_vcvt_surface_text,
        "vcvt(f32 -> f8e4m3) should infer the packed low-precision result type",
    )
    expect(
        "!pto.vreg<256x!pto.hif8>" in low_precision_vcvt_surface_text,
        "vcvt(f32 -> hif8) should infer the packed HiF8 result type",
    )
    expect(
        "!pto.vreg<256x!pto.f4E1M2x2>" in low_precision_vcvt_surface_text,
        "vcvt(bf16 -> f4e1m2x2) should infer the packed 4-bit result type",
    )
    expect(
        'rnd = "H"' in low_precision_vcvt_surface_text,
        "vcvt(..., rnd=VcvtRoundMode.H) should preserve the H rounding token",
    )
    expect(
        low_precision_vcvt_surface_text.count('part = "P0"') >= 6,
        "low-precision packed vcvt forms should preserve P0 part selectors",
    )
    expect(
        vdup_surface_text.count("pto.vdup") == 3,
        "vdup(...) should lower once per authored scalar/vector duplication",
    )
    expect(
        "f32, !pto.mask<b32> -> !pto.vreg<64xf32>" in vdup_surface_text,
        "vdup(scalar_f32, mask_b32) should infer an f32 vector result type",
    )
    expect(
        vdup_surface_text.count('position = "LOWEST"') >= 1,
        "vdup(vec, mask) should default position to LOWEST",
    )
    expect(
        'position = "HIGHEST"' in vdup_surface_text,
        "vdup(vec, mask, PositionMode.HIGHEST) should preserve the authored position",
    )
    expect(
        vecscope_surface_text.count("pto.vecscope") == 1,
        "pto.vecscope() should lower exactly one vecscope region",
    )
    expect(
        "pto.vlds" in vecscope_surface_text,
        "public vecscope body should allow vector loads",
    )
    expect(
        "pto.vsts" in vecscope_surface_text,
        "public vecscope body should allow vector stores",
    )
    expect(
        "pto.vmulscvt" in vmulscvt_surface_text,
        "vmulscvt(...) should lower to pto.vmulscvt",
    )
    expect(
        '"A"' in vmulscvt_surface_text,
        "vmulscvt(..., rnd=VcvtRoundMode.A) should preserve the authored round token",
    )
    expect(
        '"EVEN"' in vmulscvt_surface_text,
        "vmulscvt(..., part=PartMode.EVEN) should preserve the authored part token",
    )
    expect(
        "!pto.vreg<128xf16>" in vmulscvt_surface_text,
        "vmulscvt(f32 -> f16) should infer the packed f16 result type",
    )
    expect("pto.vmula" in vmula_surface_text, "vmula(...) should lower to pto.vmula")
    expect(
        "!pto.vreg<64xf32>" in vmula_surface_text,
        "vmula(f32, f32, f32) should infer the f32 vector result type",
    )
    expect("pto.vmadd" in vmadd_surface_text, "vmadd(...) should lower to pto.vmadd")
    expect(
        "!pto.vreg<64xf32>" in vmadd_surface_text,
        "vmadd(f32, f32, f32) should infer the f32 vector result type",
    )
    expect(
        "pto.vsstb" in vsstb_post_update_surface_text,
        "vsstb(..., post_update=ON) should still lower through pto.vsstb on the current VPTO IR",
    )
    expect(
        "-> !pto.ptr<f32, ub>" in vsstb_post_update_surface_text,
        "vsstb(..., post_update=ON) should request the updated destination pointer result",
    )
    expect(
        "pto.mte_l1_l0b" in public_surface_text,
        "mte_l1_l0b(...) should lower to pto.mte_l1_l0b",
    )
    expect(
        "pto.mte_l1_l0a_mx" in public_surface_text,
        "mte_l1_l0a_mx(...) should lower to pto.mte_l1_l0a_mx",
    )
    expect(
        "pto.mte_l1_l0b_mx" in public_surface_text,
        "mte_l1_l0b_mx(...) should lower to pto.mte_l1_l0b_mx",
    )
    expect(
    ...
    expect(
        "pto.tmatmul.mx" in public_surface_text,
        "pto.tile.matmul_mx should lower to pto.tmatmul.mx",
    )
    expect(
        "pto.tmatmul.mx.acc" in public_surface_text,
        "pto.tile.matmul_mx_acc should lower to pto.tmatmul.mx.acc",
    )
    expect(
        "pto.tmatmul.mx.bias" in public_surface_text,
        "pto.tile.matmul_mx_bias should lower to pto.tmatmul.mx.bias",
    )
    expect(
        "pto.tgemv.mx" in public_surface_text,
        "pto.tile.gemv_mx should lower to pto.tgemv.mx",
    )
    expect(
        "pto.tgemv.mx.acc" in public_surface_text,
        "pto.tile.gemv_mx_acc should lower to pto.tgemv.mx.acc",
    )
    expect(
        "pto.tgemv.mx.bias" in public_surface_text,
        "pto.tile.gemv_mx_bias should lower to pto.tgemv.mx.bias",
    )
    expect(
        "pto.mte_l0c_l1" in public_surface_text,
        "mte_l0c_l1(...) should lower to pto.mte_l0c_l1",
    )
    expect(
        "pto.mte_l0c_gm" in public_surface_text,
        "mte_l0c_gm(...) should lower to pto.mte_l0c_gm",
    )
    expect(
        public_surface_text.count("pto.mte_l0c_ub") >= 2,
        "mte_l0c_ub(...) should lower sub-block and split modes",
    )
    expect("pto.mad" in public_surface_text, "mad(...) should lower to pto.mad")
    expect(
        "unit_flag(check_only)" in public_surface_text,
        "mad unit_flag option should lower to PTO unit_flag clause",
    )
    expect(
        "disable_gemv" in public_surface_text,
        "mad disable_gemv option should lower to PTO disable_gemv clause",
    )
    expect(
        "tf32_mode(round_even)" in public_surface_text,
        "mad tf32_mode option should lower to PTO tf32_mode clause",
    )
    expect(
        "n_dir" in public_surface_text,
        "mad n_dir option should lower to PTO n_dir clause",
    )
    expect(
        "pto.mad_acc" in public_surface_text, "mad_acc(...) should lower to pto.mad_acc"
    )
    expect(
        "pto.mad_bias" in public_surface_text,
        "mad_bias(...) should lower to pto.mad_bias",
    )
    expect(
        "pto.mad_mx" in public_surface_text, "mad_mx(...) should lower to pto.mad_mx"
    )
    expect(
        "pto.mad_mx_acc" in public_surface_text,
        "mad_mx_acc(...) should lower to pto.mad_mx_acc",
    )
    expect(
        "pto.mad_mx_bias" in public_surface_text,
        "mad_mx_bias(...) should lower to pto.mad_mx_bias",
    )
    expect(
        "!pto.tile_buf<vec, 128x64xf8E4M3FN>" in low_precision_storage_text,
        "low-precision tile allocation should preserve float8 element types in MLIR",
    )
    expect(
        "!pto.tile_buf<vec, 64x64x!pto.hif8>" in low_precision_storage_text,
        "low-precision tile allocation should preserve HiF8 element types in MLIR",
    )
    expect(
        "pto.vlds" in pointer_vlds_text,
        "vlds(ptr, offset) should still lower to pto.vlds",
    )
    expect(
        "!pto.vreg<64xf32>" in pointer_vlds_text,
        "vlds(ptr, offset) should infer the result vreg type from the pointer element type",
    )
    expect(
        'dist = "BRC_B32"' in pointer_vlds_text,
        'vlds(ptr, offset, dist="BRC_B32") should lower the authored load distribution',
    )
    expect(
        "pto.vbitcast" in pointer_vlds_text,
        "vbitcast(...) should lower to pto.vbitcast",
    )
    expect(
        "!pto.vreg<128xf16>" in pointer_vlds_text,
        "vbitcast(vec, pto.f16) should preserve the 256-byte payload while adjusting the lane count",
    )
    expect(
        mask_bitcast_text.count("pto.pbitcast") == 2,
        "pbitcast(...) should lower to pto.pbitcast for each authored mask reinterpretation",
    )
    expect(
        "!pto.mask<b16>" in mask_bitcast_text,
        "pbitcast(mask, pto.mask_b16) should materialize the requested result mask type",
    )
    expect(
        "!pto.mask<b32>" in mask_bitcast_text,
        "pbitcast(mask, pto.mask_b32) should materialize the requested result mask type",
    )
    expect_raises(
        TypeError,
    ...
    expect(
        "pto.pset_b8" in mask_surface_text, "pset_b8(...) should lower to pto.pset_b8"
    )
    expect(
        "pto.pset_b16" in mask_surface_text,
        "pset_b16(...) should lower to pto.pset_b16",
    )
    expect(
        "pto.pset_b32" in mask_surface_text,
        "pset_b32(...) should lower to pto.pset_b32",
    )
    expect("pto.pge_b8" in mask_surface_text, "pge_b8(...) should lower to pto.pge_b8")
    expect(
        "pto.pge_b16" in mask_surface_text, "pge_b16(...) should lower to pto.pge_b16"
    )
    expect(
        "pto.pge_b32" in mask_surface_text, "pge_b32(...) should lower to pto.pge_b32"
    )
    expect("pto.plt_b8" in mask_surface_text, "plt_b8(...) should lower to pto.plt_b8")
    expect(
        "pto.plt_b16" in mask_surface_text, "plt_b16(...) should lower to pto.plt_b16"
    )
    expect(
        mask_surface_text.count("pto.plt_b32") >= 1,
        "plt_b32(...) should still lower to pto.plt_b32",
    )
    ...
    expect(
        "pto.punpack" in mask_surface_text, "punpack(...) should lower to pto.punpack"
    )
    expect(
    ...
    expect(
        "pto.pintlv_b8" in mask_surface_text,
        "pintlv_b8(...) should lower to pto.pintlv_b8",
    )
    expect(
        "pto.pintlv_b16" in mask_surface_text,
        "pintlv_b16(...) should lower to pto.pintlv_b16",
    )
    expect(
        "pto.pintlv_b32" in mask_surface_text,
        "pintlv_b32(...) should lower to pto.pintlv_b32",
    )
    expect(
        "pto.pdintlv_b8" in mask_surface_text,
        "pdintlv_b8(...) should lower to pto.pdintlv_b8",
    )
    expect(
        "pto.pdintlv_b16" in mask_surface_text,
        "pdintlv_b16(...) should lower to pto.pdintlv_b16",
    )
    expect(
        "pto.pdintlv_b32" in mask_surface_text,
        "pdintlv_b32(...) should lower to pto.pdintlv_b32",
    )
    ...
    expect(
        "pto.vcmp %" in mask_surface_text and ', "eq"' in mask_surface_text,
        "vcmp(..., pto.CmpMode.EQ) should normalize to the compare attribute spelling",
    )
    expect(
        "pto.vcmps %" in mask_surface_text and ', "gt"' in mask_surface_text,
        "vcmps(..., pto.CmpMode.GT) should normalize to the compare attribute spelling",
    )
    expect(
        mask_surface_text.count("pto.plds") == 3,
        "plds(...) should lower once per authored predicate load",
    )
    expect(
        mask_surface_text.count("pto.psts") == 2,
        "psts(...) should lower once per authored predicate store",
    )
    expect(
        mask_surface_text.count("pto.pstu") == 2,
        "pstu(...) should lower once per authored predicate unaligned store",
    )
    expect(
        ', "US"' in mask_surface_text,
        "plds(..., dist=pto.PredicateDist.US) should preserve the documented DIST token",
    )
    expect(
        ', "DS"' in mask_surface_text,
        "plds(..., dist=pto.PredicateDist.DS) should preserve the documented DIST token",
    )
    expect(
        ', "PK"' in mask_surface_text,
        "psts(..., dist=pto.PredicateDist.PK) should preserve the documented DIST token",
    )
    expect(
        "pto.init_align" in mask_surface_text,
        "init_align() should lower to pto.init_align",
    )

    launch_handle = block64[1, None]
    expect(
        callable(launch_handle),
        "compiled[grid, stream] should return a launch callable",
    )
    ...
    expect(
        callable(source_launch_handle),
        "source-backed compiled[grid, stream] should return a launch callable",
    )
    expect(
        hasattr(source_launch_handle, "__call__"),
        "source-backed launch handle should support __call__",
    )
