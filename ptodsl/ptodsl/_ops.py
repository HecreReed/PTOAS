...
def _normalize_struct_path(path, *, op_name: str) -> tuple[int, ...]:
    if isinstance(path, bool):
        raise TypeError(
            f"{op_name}: path must be a static int, tuple[int, ...], or list[int]"
        )
    ...
    for depth, index in enumerate(indices):
        ...
        if index < 0:
            raise ValueError(
                f"{op_name}: path[{depth}] must be non-negative, got {index}"
            )
    return indices
...
def vlds(src_ptr, offset=None, result_vreg_type=None, *, dist=None, post_update="OFF"):
    ...
    if isinstance(src_ptr, TileSliceValue):
        if offset is not None or result_vreg_type is not None:
            raise TypeError(
                "vlds(tile[row, col:]) infers its pointer slice and vreg type; do not pass offset/result_vreg_type"
            )
...
def vstsx2(low, high, dst_ptr, offset_or_dist, dist_or_mask=None, mask=None):
    ...
    if isinstance(dst_ptr, TileSliceValue):
        if mask is not None:
            raise TypeError(
                "vstsx2(low, high, tile[row, col:], dist, mask) does not accept a separate offset argument"
            )
        destination, destination_offset = _tile_slice_address(dst_ptr)
        _pto.Vstsx2Op(
...
def _resolve_l1_bypass_scalar_pointer(ptr_value, *, context: str):
    ...
    if not IntegerType.isinstance(elem_type) or IntegerType(elem_type).width not in (
        8,
        16,
        32,
        64,
...
def tscatter(src, dst, *, indexes=None, axis=None, mask_pattern=None):
    ...
    if indexes is None and (axis is None or mask_pattern is None):
        raise ValueError("non indexes mode must provide both axis and mask_pattern")
    if axis is not None and axis not in ("row", "col"):
        raise ValueError(f"axis must be 'row' or 'col', got {axis!r}")
    _pto.tscatter(
        unwrap_surface_value(src),
