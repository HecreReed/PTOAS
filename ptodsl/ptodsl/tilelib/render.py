...
def render_best(
    ...
    return select_and_specialize(
        op, target, tile_specs, context_attrs, candidate_id
    ).mlir_text()
...
def _parse_tile_arg(spec: str):
    ...
    if not name or not rest:
        raise argparse.ArgumentTypeError(
            f"invalid --tile {spec!r}; expected name=RxC@mem:dtype"
        )
    ...
    if dtype not in _DTYPES:
        raise argparse.ArgumentTypeError(
            f"unsupported dtype {dtype!r} in --tile {spec!r}"
        )
...
def _parse_scalar_arg(spec: str):
    ...
    if len(parts) not in {2, 3} or not parts[0]:
        raise argparse.ArgumentTypeError(
            f"invalid --scalar {spec!r}; expected name:dtype[:value]"
        )
    name, dtype = parts[0], parts[1]
    if dtype not in _DTYPES:
        raise argparse.ArgumentTypeError(
            f"unsupported dtype {dtype!r} in --scalar {spec!r}"
        )
...
def main(argv=None):
    ...
    parser.add_argument(
        "--tile", action="append", default=[], help="name=RxC@mem:dtype"
    )
    parser.add_argument(
        "--scalar", action="append", default=[], help="name:dtype[:value]"
    )
