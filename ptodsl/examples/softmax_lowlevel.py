...
def build():
    with Context() as ctx:
        ...
        with Location.unknown():
            ...
            ptv5d = pto.PartitionTensorViewType.get(
                [_dyn] * 5, f32
            ...
            with InsertionPoint(entry):
                ...
                has_rows = arith.CmpIOp(
                    arith.CmpIPredicate.sgt, remaining_rows, c0_i32
                ).result
                too_many_rows = arith.CmpIOp(
                    arith.CmpIPredicate.sgt, remaining_rows, c8_i32
                ).result
                row_count_i32 = arith.SelectOp(
                    too_many_rows, c8_i32, remaining_rows
                ).result
                ...
                with InsertionPoint(if_rows.then_block):
                    ...
                    oldmax_part = pto.PartitionViewOp(
                        ptv5d, oldmax_view, off5, sz1
                    ).result
                    oldsum_part = pto.PartitionViewOp(
                        ptv5d, oldsum_view, off5, sz1
                    ).result
                    qk_part = pto.PartitionViewOp(ptv5d, qk_view, off5, szs).result
                    newmax_part = pto.PartitionViewOp(
                        ptv5d, newmax_view, off5, sz1
                    ).result
                    newsum_part = pto.PartitionViewOp(
                        ptv5d, newsum_view, off5, sz1
                    ).result
                    expmax_part = pto.PartitionViewOp(
                        ptv5d, expmax_view, off5, sz1
                    ).result
                    out_part = pto.PartitionViewOp(ptv5d, out_view, off5, szs).result

                    # ── Tile allocation ───────────────────────────────────
                    oldmax_tile = pto.AllocTileOp(
                        tile_col, addr=c0_i64, valid_row=row_count
                    ).result
                    oldsum_tile = pto.AllocTileOp(
                        tile_col, addr=c128_i64, valid_row=row_count
                    ).result
                    qk_tile = pto.AllocTileOp(
                        tile_wide, addr=c256_i64, valid_row=row_count, valid_col=seq
                    ).result
                    out_tile = pto.AllocTileOp(
                        tile_wide, addr=c8448_i64, valid_row=row_count, valid_col=seq
                    ).result
                    newmax_tile = pto.AllocTileOp(
                        tile_col, addr=c16640_i64, valid_row=row_count
                    ).result
                    newsum_tile = pto.AllocTileOp(
                        tile_col, addr=c16768_i64, valid_row=row_count
                    ).result
                    expmax_tile = pto.AllocTileOp(
                        tile_col, addr=c16896_i64, valid_row=row_count
                    ).result
                    ...
                    with InsertionPoint(vs_block):
                        ...
                        with InsertionPoint(row_for.body):
                            ...
                            oldmax_bc = pto.VldsOp(
                                vreg, ub_oldmax, row, dist="BRC_B32"
                            ).result
                            oldsum_bc = pto.VldsOp(
                                vreg, ub_oldsum, row, dist="BRC_B32"
                            ).result

                            # ── for chunk in [0,128,64) with iter_args ────
                            chunk_for = scf.ForOp(c0, c128, c64, [oldmax_bc, oldsum_bc])
                            with InsertionPoint(chunk_for.body):
                                ...
                                has_chunk = arith.CmpIOp(
                                    arith.CmpIPredicate.sgt, remaining_cols, c0_i32
                                ).result

                                # ── if has_chunk -> (vreg, vreg) ──────────
                                c_if = scf.IfOp(has_chunk, [vreg, vreg], hasElse=True)
                                with InsertionPoint(c_if.then_block):
                                    cplt = pto.PltB32Op(mask_b32, i32, remaining_cols)
                                    chunk_mask = cplt.mask
                                    chunk_base = arith.AddIOp(row_qk, chunk).result
                                    vec = pto.VldsOp(vreg, ub_qk, chunk_base).result
                                    chunk_max = pto.VcmaxOp(
                                        vreg, vec, chunk_mask
                                    ).result
                                    chunk_max_bc = pto.VdupOp(
                                        vreg, chunk_max, active, position="LOWEST"
                                    ).result
                                    merged_max = pto.VmaxOp(
                                        vreg, running_max, chunk_max_bc, active
                                    ).result
                                    scaled_run = pto.VexpdifOp(
                                        vreg, running_max, merged_max, active, "ODD"
                                    ).result
                                    run_sum_sc = pto.VmulOp(
                                        vreg, scaled_run, running_sum, active
                                    ).result
                                    chunk_exp = pto.VexpdifOp(
                                        vreg, vec, merged_max, chunk_mask, "ODD"
                                    ).result
                                    chunk_sum = pto.VcaddOp(
                                        vreg, chunk_exp, chunk_mask
                                    ).result
                                    chunk_sum_bc = pto.VdupOp(
                                        vreg, chunk_sum, active, position="LOWEST"
                                    ).result
                                    merged_sum = pto.VaddOp(
                                        vreg, run_sum_sc, chunk_sum_bc, active
                                    ).result
                            ...
                            raw_expmax = pto.VexpdifOp(
                                vreg, oldmax_bc, final_max, active, "ODD"
                            ).result
                            scaled_oldsum = pto.VmulOp(
                                vreg, raw_expmax, oldsum_bc, active
                            ).result
                            expmax = pto.VdivOp(
                                vreg, scaled_oldsum, final_sum, active
                            ).result

                            pto.VstsOp(
                                final_max, ub_newmax, row, one_mask, dist="1PT_B32"
                            )
                            pto.VstsOp(
                                final_sum, ub_newsum, row, one_mask, dist="1PT_B32"
                            )
                            pto.VstsOp(expmax, ub_expmax, row, one_mask, dist="1PT_B32")

                            # ── Output normalisation loop ─────────────────
                            out_for = scf.ForOp(c0, c128, c64)
                            with InsertionPoint(out_for.body):
                                chunk2 = out_for.induction_variable
                                ci32_2 = arith.IndexCastOp(i32, chunk2).result
                                rem2 = arith.SubIOp(arg7, ci32_2).result
                                has_chunk2 = arith.CmpIOp(
                                    arith.CmpIPredicate.sgt, rem2, c0_i32
                                ).result

                                o_if = scf.IfOp(has_chunk2)
                                with InsertionPoint(o_if.then_block):
                                    oplt = pto.PltB32Op(mask_b32, i32, rem2)
                                    cmask2 = oplt.mask
                                    cbase2 = arith.AddIOp(row_qk, chunk2).result
                                    vec2 = pto.VldsOp(vreg, ub_qk, cbase2).result
                                    exp2 = pto.VexpdifOp(
                                        vreg, vec2, final_max, cmask2, "ODD"
                                    ).result
                                    out2 = pto.VdivOp(
                                        vreg, exp2, final_sum, cmask2
                                    ).result
