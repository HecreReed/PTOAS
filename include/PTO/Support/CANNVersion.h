...
struct CANNVersion {
    ...
    bool operator==(const CANNVersion& rhs) const
    {
        return major == rhs.major && minor == rhs.minor && patch == rhs.patch && beta == rhs.beta;
    }

    bool operator>=(const CANNVersion& rhs) const { return !(*this < rhs); }
};

inline std::optional<unsigned> parseCANNVersionComponent(llvm::StringRef value)
{
    unsigned result = 0;
...
inline std::optional<unsigned> parseCANNBetaVersion(llvm::StringRef prerelease)
{
