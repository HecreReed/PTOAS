...
namespace llvm {
template <>
struct DenseMapInfo<mlir::pto::syncsolver::CorePipeInfo> {
    ...
    static unsigned getHashValue(const mlir::pto::syncsolver::CorePipeInfo& val)
    {
        // Combine hashes of members
        return DenseMapInfo<CorePipePairTy>::getHashValue({val.coreType, val.pipe});
    }
    static bool isEqual(const mlir::pto::syncsolver::CorePipeInfo& lhs, const mlir::pto::syncsolver::CorePipeInfo& rhs)
    {
...
public:
    ...
    void insertConflictPair(ConflictPair* conflictPair) { conflictPairs[conflictPair] += 1; }
