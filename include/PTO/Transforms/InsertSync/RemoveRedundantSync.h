...
private:
    ...
    bool CheckRepeatSync(unsigned int begin, unsigned int end, SmallVector<bool>& syncFinder, SyncOperation* setFlag);
    ...
    bool CheckLoopBetween(LoopInstanceElement* loopElement, SyncOperation* setFlag, unsigned& i);

    /// 核心判断逻辑：检查遇到的 relatedSync 是否能构成对 setFlag 的覆盖
    bool CanMatchedSync(SmallVector<bool>& syncFinder, SyncOperation* relatedSync, SyncOperation* setFlag);
};
