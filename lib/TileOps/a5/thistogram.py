...
def _u8_vreg_type():
    return _resolve_type(
        _vreg_type(
            _elements_per_vreg(_resolve_type(_ui8_dtype)), _resolve_type(_ui8_dtype)
        )
    )
...
def _u16_vreg_type():
    return _resolve_type(
        _vreg_type(
            _elements_per_vreg(_resolve_type(_ui16_dtype)), _resolve_type(_ui16_dtype)
        )
    )
...
def _histogram_core(
    ...
    vb32_bins_even = pto.vcvt(
        vb16_bins_low, pto.ui32, preg_all_b16, part=pto.VcvtPartMode.EVEN
    )
    vb32_bins_odd = pto.vcvt(
        vb16_bins_low, pto.ui32, preg_all_b16, part=pto.VcvtPartMode.ODD
    )
    ...
    vb32_bins_even = pto.vcvt(
        vb16_bins_high, pto.ui32, preg_all_b16, part=pto.VcvtPartMode.EVEN
    )
    vb32_bins_odd = pto.vcvt(
        vb16_bins_high, pto.ui32, preg_all_b16, part=pto.VcvtPartMode.ODD
    )
...
def _deintlv_u32_bytes(src_ptr, offset):
    ...
    chunk0_lo, chunk0_hi = pto.vldsx2(
        src_addr, 0, pto.DeinterleaveDist.DINTLV_B16, result_vreg_type=_u16_vreg_type()
    )
    chunk1_addr = pto.addptr(src_addr, 128)
    chunk1_lo, chunk1_hi = pto.vldsx2(
        chunk1_addr,
        0,
    ...
    def template(src: pto.Tile, idx: pto.Tile, dst: pto.Tile):
        ...
        for row in range(valid_rows):
            ...
            for col in range(0, valid_cols, _ELEM_PER_REPEAT_B8):
                ...
                vb8_lsb, vb8_msb = pto.vldsx2(
                    src_addr,
                    0,
                    pto.DeinterleaveDist.DINTLV_B8,
...
def _make_u32_template(byte_val, template_id):
    ...
    @tilelib.tile_template(
        ...
        target="a5",
        name=name,
        dtypes=_HIST_U32_DTYPES,
        iteration_axis="none",
        op_engine="vector",
        op_class="other",
        constraints=[_is_byte(byte_val), _src_min_cols, _hist_layout, _hist_mem_space]
        + idx_layout_con,
        loop_depth=2,
        is_post_update=False,
        id=template_id,
    ...
    def template(src: pto.Tile, idx: pto.Tile, dst: pto.Tile):
        ...
        for row in range(valid_rows):
            ...
            for col in range(0, valid_cols, _ELEM_PER_REPEAT_B8):
                elem_mask, remained = pto.make_mask(pto.ui8, remained)
                byte0, byte1, byte2, byte3 = _deintlv_u32_bytes(
                    src_ptr, row * src_cols + col
                )
                if num_filters == 0:
                    filt_mask = elem_mask
                else:
