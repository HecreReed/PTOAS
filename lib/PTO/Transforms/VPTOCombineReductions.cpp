...
bool areEquivalentMasks(Value lhs, Value rhs)
{
    ...
    bool isPatternMask = isa<PsetB8Op, PsetB16Op, PsetB32Op, PgeB8Op, PgeB16Op, PgeB32Op>(lhsOp);
...
struct CombineEquivalentReductionTreePattern : OpRewritePattern<CombineOpTy> {
    ...
    LogicalResult matchAndRewrite(CombineOpTy op, PatternRewriter& rewriter) const override
    {
        ...
        if (failed(collect(op.getLhs(), op.getMask(), reductions, baseValues)) ||
            failed(collect(op.getRhs(), op.getMask(), reductions, baseValues)) || reductions.size() < 2 ||
            baseValues.size() > 1)
            return failure();
        ...
        for (ReductionLeaf leaf : llvm::drop_begin(reductions)) {
            if (leaf.source.getType() != sourceType)
                return failure();
            combinedSource =
                rewriter.create<CombineOpTy>(op.getLoc(), sourceType, combinedSource, leaf.source, reductionMask)
                    .getResult();
        }

        Value reduced =
            rewriter.create<ReduceOpTy>(op.getLoc(), op.getResult().getType(), combinedSource, reductionMask)
                .getResult();
        if (baseValues.empty()) {
        ...
        if (base.getType() != op.getResult().getType())
            return failure();
        rewriter.replaceOpWithNewOp<CombineOpTy>(op, op.getResult().getType(), reduced, base, op.getMask());
        return success();
    }

private:
    LogicalResult collect(
        Value value, Value combineMask, SmallVectorImpl<ReductionLeaf>& reductions,
        SmallVectorImpl<Value>& baseValues) const
    {
        ...
        if (auto combine = value.getDefiningOp<CombineOpTy>()) {
            if (!areEquivalentMasks(combineMask, combine.getMask()))
                return failure();
            if (failed(collect(combine.getLhs(), combineMask, reductions, baseValues)))
                return failure();
...
struct VPTOCombineReductionsPass : pto::impl::VPTOCombineReductionsBase<VPTOCombineReductionsPass> {
    void runOnOperation() override
    {
        RewritePatternSet patterns(&getContext());
        patterns.add<
            CombineEquivalentReductionTreePattern<VaddOp, VcaddOp>,
            ...
            CombineEquivalentReductionTreePattern<VminOp, VcgminOp>>(&getContext());
