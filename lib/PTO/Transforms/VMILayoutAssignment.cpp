...
struct LayoutSolver {
    explicit LayoutSolver(ModuleOp module) : module(module), ctx(module.getContext()) {}
    ...
    unsigned addDataValue(Value value)
    {
        ...
        if (inserted) {
            dataNodes.push_back(DataNode{value, type, it->second, type.getLayoutAttr(), {}});
            if (type.getLayoutAttr())
                dataLayoutSeeds.push_back(DataLayoutSeed{value, type.getLayoutAttr(), DataLayoutSeedPhase::Explicit});
        }
        return it->second;
    }
    ...
    unsigned addMaskValue(Value value)
    {
        ...
        if (inserted)
            maskNodes.push_back(MaskNode{value, type, it->second, type.getLayoutAttr()});
        return it->second;
    }
    ...
    LogicalResult uniteDataEquivalent(Value lhs, Value rhs, Operation* op)
    {
        ...
        if (lhsNode.naturalLayout && rhsNode.naturalLayout && lhsNode.naturalLayout != rhsNode.naturalLayout)
            return op->emitError() << kVMIDiagLayoutContractPrefix << "conflicting natural layouts "
                                   << lhsNode.naturalLayout << " and " << rhsNode.naturalLayout;
        if (lhsNode.preferredLayout && rhsNode.preferredLayout && lhsNode.preferredLayout != rhsNode.preferredLayout)
            return op->emitError() << kVMIDiagLayoutContractPrefix << "conflicting preferred layouts "
    ...
    LogicalResult uniteMask(Value lhs, Value rhs, Operation* op)
    {
        ...
        if (lhsNode.requestedLayout && rhsNode.requestedLayout && lhsNode.requestedLayout != rhsNode.requestedLayout)
            return op->emitError() << kVMIDiagLayoutContractPrefix << "conflicting mask layouts "
    ...
    LogicalResult setNaturalLayout(
        Value value, VMILayoutAttr layout, Operation* op, DataLayoutSeedPhase phase = DataLayoutSeedPhase::Other)
    {
        ...
        if (existing && existing != layout)
            return op->emitError() << kVMIDiagLayoutContractPrefix << "conflicting natural layouts " << existing
                                   << " and " << layout;
    ...
    LogicalResult setPreferredLayout(
        Value value, VMILayoutAttr layout, Operation* op, DataLayoutSeedPhase phase = DataLayoutSeedPhase::Other)
    {
        unsigned id = addDataValue(value);
        if (id == ~0u || !layout)
        ...
        if (existing && existing != layout)
            return op->emitError() << kVMIDiagLayoutContractPrefix << "conflicting preferred layouts " << existing
                                   << " and " << layout;
    ...
    VMILayoutAttr getContiguousLayout() { return VMILayoutAttr::getContiguous(ctx); }
    ...
    VMILayoutAttr getPreferredDenseStoreLayout(VMIVRegType type)
    {
        ...
        FailureOr<VMIStoreLayoutFact> fact = supports.getPreferredStoreLayoutFact(type);
        if (failed(fact))
            return {};
    ...
    FailureOr<VMIMaskedStoreLayoutFact> getPreferredDenseMaskedStoreLayout(VMIVRegType valueType, VMIMaskType maskType)
    {
        VMILayoutSupport supports;
        return supports.getPreferredMaskedStoreLayoutFact(valueType, maskType);
    }

    VMILayoutAttr getGroupSlotsLayout(int64_t numGroups) { return VMILayoutAttr::getGroupSlots(ctx, numGroups); }

    VMILayoutAttr getPreferredGroupSlotsLayout(VMIVRegType type, int64_t numGroups)
    {
        if (VMILayoutAttr existing = type.getLayoutAttr())
            if (existing.isGroupSlots() && existing.getSlots() > 0)
                return existing;
        VMILayoutSupport supports;
        FailureOr<VMIGroupReduceLayoutFact> fact = supports.getPreferredGroupReduceLayoutFact(type, numGroups);
        if (succeeded(fact))
            return fact->resultLayout;
        return getGroupSlotsLayout(numGroups);
    }

    VMILayoutAttr getPreferredGroupReduceSourceLayout(VMIVRegType type, int64_t numGroups)
    {
        if (VMILayoutAttr existing = type.getLayoutAttr())
            return existing;
        VMILayoutSupport supports;
    ...
    DataLayoutSeedPhase getGroupReduceUseSeedPhase(
        VMIVRegType sourceType, int64_t numGroups, VMIGroupReduceLayoutFact fact)
    {
        if (!fact.sourceLayout || !fact.sourceLayout.isContiguous() || fact.sourceLayout.getLaneStride() != 1)
            return DataLayoutSeedPhase::Reduce;

        VMILayoutSupport supports;
        FailureOr<SmallVector<VMIGroupReduceLayoutFact, 4>> resultFacts = supports.getGroupReduceLayoutFactsForLayout(
            sourceType, numGroups, VMIGroupReduceLayoutPort::Result, fact.resultLayout);
    ...
    VMILayoutAttr getPreferredGroupBroadcastLoadLayout(VMIGroupBroadcastLoadOp op)
    {
        auto type = cast<VMIVRegType>(op.getResult().getType());
        if (VMILayoutAttr existing = type.getLayoutAttr())
            return existing;
        ...
        VMILayoutSupport supports;
        FailureOr<VMIGroupBroadcastLoadDirectFact> fact = supports.getGroupBroadcastLoadDirectFact(
            type, op.getSource().getType(), op.getSourceGroupStride(), op.getNumGroupsAttr().getInt());
        if (failed(fact))
            return {};
    ...
    VMILayoutAttr getPreferredGroupBroadcastSourceLayout(Value value, int64_t numGroups)
    {
        auto type = dyn_cast<VMIVRegType>(value.getType());
        if (!type)
            return getContiguousLayout();
        ...
        if (solved && solved.isGroupSlots() && solved.getNumGroups() == numGroups && solved.getSlots() > 0)
            return solved;
    ...
    VMILayoutAttr getPreferredGroupBroadcastResultLayout(VMIGroupBroadcastOp op)
    {
        ...
        int64_t numGroups = op.getNumGroupsAttr().getInt();
        if (failed(lanesPerPart) || numGroups <= 0 || type.getElementCount() % numGroups != 0)
            return {};
    ...
    LogicalResult validateGroupLoadLayoutPlan(VMIGroupLoadOp op)
    {
        ...
        return op.emitError() << kVMIDiagLayoutContractPrefix << "pto.vmi.group_load group_size " << groupSize
    ...
    void requestDataUse(
        OpOperand& operand, VMILayoutAttr layout, bool late = false,
        DataLayoutSeedPhase phase = DataLayoutSeedPhase::Other)
    {
    ...
    LogicalResult constrainElementwiseBinary(OpOperand& lhs, OpOperand& rhs, Value result, Operation* op)
    {
    ...
    LogicalResult requestMaskUse(
        OpOperand& operand, VMILayoutAttr layout, Operation* op, DataLayoutSeedPhase phase = DataLayoutSeedPhase::Other)
    {
        if (!isa<VMIMaskType>(operand.get().getType()))
            return success();
        if (!layout)
            return op->emitError() << kVMIDiagLayoutContractPrefix << "cannot infer concrete mask use layout";
    ...
    LogicalResult addConstraints()
    {
        WalkResult result = module.walk([&](Operation* op) -> WalkResult {
            if (auto groupIota = dyn_cast<VMIGroupIotaOp>(op)) {
                if (failed(setNaturalLayout(groupIota.getResult(), getContiguousLayout(), op)))
            ...
            if (auto addf = dyn_cast<VMIAddFOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(addf.getLhsMutable(), addf.getRhsMutable(), addf.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto addi = dyn_cast<VMIAddIOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(addi.getLhsMutable(), addi.getRhsMutable(), addi.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto subf = dyn_cast<VMISubFOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(subf.getLhsMutable(), subf.getRhsMutable(), subf.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto subi = dyn_cast<VMISubIOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(subi.getLhsMutable(), subi.getRhsMutable(), subi.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto mulf = dyn_cast<VMIMulFOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(mulf.getLhsMutable(), mulf.getRhsMutable(), mulf.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto muli = dyn_cast<VMIMulIOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(muli.getLhsMutable(), muli.getRhsMutable(), muli.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto vmull = dyn_cast<VMIVmullOp>(op)) {
                if (failed(constrainElementwiseBinary(vmull.getAMutable(), vmull.getBMutable(), vmull.getLow(), op)) ||
                    failed(unite(vmull.getA(), vmull.getHigh(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto fma = dyn_cast<VMIFmaOp>(op)) {
                if (failed(unite(fma.getLhs(), fma.getRhs(), op)) || failed(unite(fma.getLhs(), fma.getAcc(), op)) ||
            ...
            if (auto divf = dyn_cast<VMIDivFOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(divf.getLhsMutable(), divf.getRhsMutable(), divf.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto minf = dyn_cast<VMIMinFOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(minf.getLhsMutable(), minf.getRhsMutable(), minf.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto mini = dyn_cast<VMIMinIOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(mini.getLhsMutable(), mini.getRhsMutable(), mini.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto maxf = dyn_cast<VMIMaxFOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(maxf.getLhsMutable(), maxf.getRhsMutable(), maxf.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto maxi = dyn_cast<VMIMaxIOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(maxi.getLhsMutable(), maxi.getRhsMutable(), maxi.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto fptosi = dyn_cast<VMIFPToSIOp>(op)) {
                ...
                VMILayoutSupport supports;
                FailureOr<VMICastLayoutFact> fact = supports.getPreferredCastLayoutFact(sourceType, resultType);
                VMILayoutAttr resultLayout = getContiguousLayout();
                if (succeeded(fact))
                    resultLayout = fact->resultLayout;
                if (failed(setPreferredLayout(fptosi.getResult(), resultLayout, op, DataLayoutSeedPhase::Cast)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto fptoui = dyn_cast<VMIFPToUIOp>(op)) {
                ...
                if (succeeded(fact))
                    resultLayout = fact->resultLayout;
                if (failed(setPreferredLayout(fptoui.getResult(), resultLayout, op, DataLayoutSeedPhase::Cast)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto sitofp = dyn_cast<VMISIToFPOp>(op)) {
                ...
                if (succeeded(fact))
                    resultLayout = fact->resultLayout;
                if (failed(setPreferredLayout(sitofp.getResult(), resultLayout, op, DataLayoutSeedPhase::Cast)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto andi = dyn_cast<VMIAndIOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(andi.getLhsMutable(), andi.getRhsMutable(), andi.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto ori = dyn_cast<VMIOrIOp>(op)) {
                if (failed(constrainElementwiseBinary(ori.getLhsMutable(), ori.getRhsMutable(), ori.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto xori = dyn_cast<VMIXOrIOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(xori.getLhsMutable(), xori.getRhsMutable(), xori.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto shli = dyn_cast<VMIShLIOp>(op)) {
                if (failed(
                        constrainElementwiseBinary(shli.getLhsMutable(), shli.getRhsMutable(), shli.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto shrui = dyn_cast<VMIShRUIOp>(op)) {
                if (failed(constrainElementwiseBinary(
                        shrui.getLhsMutable(), shrui.getRhsMutable(), shrui.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto shrsi = dyn_cast<VMIShRSIOp>(op)) {
                if (failed(constrainElementwiseBinary(
                        shrsi.getLhsMutable(), shrsi.getRhsMutable(), shrsi.getResult(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto vselr = dyn_cast<VMIVselrOp>(op)) {
                VMILayoutSupport supports;
                FailureOr<VMIVselrLayoutFact> fact = supports.getPreferredVselrLayoutFact(vselr);
                if (failed(fact))
                    return WalkResult::advance();
            ...
            if (auto activePrefix = dyn_cast<VMIActivePrefixIndexOp>(op)) {
                if (failed(setNaturalLayout(activePrefix.getResult(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto compress = dyn_cast<VMICompressOp>(op)) {
                requestDataUse(compress.getSourceMutable(), getContiguousLayout());
                if (failed(setNaturalLayout(compress.getResult(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto reduce = dyn_cast<VMIReduceAddIOp>(op)) {
                requestDataUse(
                    reduce.getSourceMutable(), getContiguousLayout(),
                    /*late=*/false, DataLayoutSeedPhase::Reduce);
                requestDataUse(
                    reduce.getInitMutable(), getContiguousLayout(),
                    /*late=*/false, DataLayoutSeedPhase::Reduce);
                if (failed(requestMaskUse(reduce.getMaskMutable(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                if (failed(
                        setNaturalLayout(reduce.getResult(), getContiguousLayout(), op, DataLayoutSeedPhase::Reduce)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto reduce = dyn_cast<VMIGroupReduceAddFOp>(op)) {
                ...
                VMILayoutAttr sourceLayout = succeeded(fact) ? fact->sourceLayout : getContiguousLayout();
                DataLayoutSeedPhase usePhase = succeeded(fact) ?
                                                   getGroupReduceUseSeedPhase(sourceType, numGroups, *fact) :
                                                   DataLayoutSeedPhase::Reduce;
                requestDataUse(reduce.getSourceMutable(), sourceLayout, /*late=*/false, usePhase);
                if (failed(requestMaskUse(reduce.getMaskMutable(), sourceLayout, op, usePhase)))
                    return WalkResult::interrupt();
                if (failed(setNaturalLayout(
                        reduce.getResult(),
                        succeeded(fact) ? fact->resultLayout : getPreferredGroupSlotsLayout(resultType, numGroups), op,
                        DataLayoutSeedPhase::Reduce)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto broadcast = dyn_cast<VMIGroupBroadcastOp>(op)) {
                ...
                if (failed(setPreferredLayout(
                        broadcast.getResult(), getPreferredGroupBroadcastResultLayout(broadcast), op,
            ...
            if (auto hist = dyn_cast<VMIVdhistOp>(op)) {
                requestDataUse(
                    hist.getAccMutable(), getContiguousLayout(),
                    /*late=*/false, DataLayoutSeedPhase::Reduce);
                requestDataUse(
                    hist.getSourceMutable(), getContiguousLayout(),
                    /*late=*/false, DataLayoutSeedPhase::Reduce);
                if (failed(
                        requestMaskUse(hist.getMaskMutable(), getContiguousLayout(), op, DataLayoutSeedPhase::Reduce)))
                    return WalkResult::interrupt();
                if (failed(setNaturalLayout(hist.getResult(), getContiguousLayout(), op, DataLayoutSeedPhase::Reduce)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto extf = dyn_cast<VMIExtFOp>(op)) {
                ...
                FailureOr<VMICastLayoutFact> fact = supports.getPreferredCastLayoutFact(sourceType, resultType);
                if (succeeded(fact)) {
                    if (failed(setPreferredLayout(extf.getResult(), fact->resultLayout, op, getCastSeedPhase(*fact))))
                        return WalkResult::interrupt();
                }
                return WalkResult::advance();
            }
            if (auto extsi = dyn_cast<VMIExtSIOp>(op)) {
                ...
                FailureOr<VMICastLayoutFact> fact = supports.getPreferredCastLayoutFact(sourceType, resultType);
                if (succeeded(fact)) {
                    if (failed(setPreferredLayout(extsi.getResult(), fact->resultLayout, op, getCastSeedPhase(*fact))))
                        return WalkResult::interrupt();
                }
                return WalkResult::advance();
            }
            if (auto extui = dyn_cast<VMIExtUIOp>(op)) {
                ...
                FailureOr<VMICastLayoutFact> fact = supports.getPreferredCastLayoutFact(sourceType, resultType);
                if (succeeded(fact)) {
                    if (failed(setPreferredLayout(extui.getResult(), fact->resultLayout, op, getCastSeedPhase(*fact))))
                        return WalkResult::interrupt();
                }
                return WalkResult::advance();
            }
            if (auto truncf = dyn_cast<VMITruncFOp>(op)) {
                ...
                DataLayoutSeedPhase phase = succeeded(fact) ? getCastSeedPhase(*fact) : DataLayoutSeedPhase::Cast;
                if (failed(setPreferredLayout(truncf.getResult(), resultLayout, op, phase)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto trunci = dyn_cast<VMITruncIOp>(op)) {
                ...
                DataLayoutSeedPhase phase = succeeded(fact) ? getCastSeedPhase(*fact) : DataLayoutSeedPhase::Cast;
                if (failed(setPreferredLayout(trunci.getResult(), resultLayout, op, phase)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto vintlv = dyn_cast<VMIVintlvOp>(op)) {
                ...
                FailureOr<VMIInterleaveLayoutFact> fact = supports.getPreferredVintlvLayoutFact(valueType);
                if (failed(fact))
                    return WalkResult::advance();
                requestDataUse(vintlv.getLhsMutable(), fact->lhsLayout);
                requestDataUse(vintlv.getRhsMutable(), fact->rhsLayout);
                if (failed(requestMaskUse(vintlv.getMaskMutable(), fact->maskLayout, op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto vdintlv = dyn_cast<VMIVdintlvOp>(op)) {
                ...
                FailureOr<VMIInterleaveLayoutFact> fact = supports.getPreferredVdintlvLayoutFact(valueType);
                if (failed(fact))
                    return WalkResult::advance();
                requestDataUse(vdintlv.getLhsMutable(), fact->lhsLayout);
                requestDataUse(vdintlv.getRhsMutable(), fact->rhsLayout);
                if (failed(requestMaskUse(vdintlv.getMaskMutable(), fact->maskLayout, op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto load = dyn_cast<VMIDeinterleaveLoadOp>(op)) {
                ...
                FailureOr<VMIDeinterleaveLoadLayoutFact> fact =
                    supports.getPreferredDeinterleaveLoadLayoutFact(cast<VMIVRegType>(load.getLow().getType()));
                if (failed(fact))
                    return WalkResult::advance();
            ...
            if (auto load = dyn_cast<VMIMaskedLoadOp>(op)) {
                ...
                if (failed(setNaturalLayout(load.getResult(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto gather = dyn_cast<VMIGatherOp>(op)) {
                ...
                if (failed(requestMaskUse(gather.getMaskMutable(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                if (failed(setNaturalLayout(gather.getResult(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto load = dyn_cast<VMIGroupLoadOp>(op)) {
                ...
                if (layout.isContiguous() && layout.getLaneStride() == 1) {
                    if (failed(setPreferredLayout(load.getResult(), layout, op)))
                        return WalkResult::interrupt();
                } else if (failed(setNaturalLayout(load.getResult(), layout, op, DataLayoutSeedPhase::GroupLoad))) {
                    return WalkResult::interrupt();
                }
                return WalkResult::advance();
            }
            ...
            if (auto load = dyn_cast<VMIGroupBroadcastLoadOp>(op)) {
                DataLayoutSeedPhase phase = hasDirectGroupBroadcastLoadCandidate(load) ?
                                                DataLayoutSeedPhase::GroupBroadcastLoad :
                                                DataLayoutSeedPhase::Other;
                if (failed(setNaturalLayout(load.getResult(), getPreferredGroupBroadcastLoadLayout(load), op, phase)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto load = dyn_cast<VMIStrideLoadOp>(op)) {
                if (failed(setNaturalLayout(load.getResult(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                if (failed(requestMaskUse(load.getMaskMutable(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto store = dyn_cast<VMIStoreOp>(op)) {
                ...
                if (!hasDataLayoutSeed(store.getValue()))
                    if (VMILayoutAttr layout = getPreferredDenseStoreLayout(valueType))
                        requestDataUse(store.getValueMutable(), layout, /*late=*/false, DataLayoutSeedPhase::Store);
                return WalkResult::advance();
            }
            ...
            if (auto store = dyn_cast<VMIGroupStoreOp>(op)) {
                ...
                if (succeeded(highPriorityFact)) {
                    ...
                    requestDataUse(
                        store.getValueMutable(), highPriorityLayout,
                ...
                if (succeeded(preferredFact) && preferredFact->valueLayout != highPriorityLayout)
                    requestDataUse(
                        store.getValueMutable(), preferredFact->valueLayout,
                        /*late=*/false, DataLayoutSeedPhase::GroupStoreFallback);
                return WalkResult::advance();
            }
            if (auto store = dyn_cast<VMIMaskedStoreOp>(op)) {
                ...
                if (!hasDataLayoutSeed(store.getValue())) {
                    FailureOr<VMIMaskedStoreLayoutFact> fact = getPreferredDenseMaskedStoreLayout(valueType, maskType);
                    if (succeeded(fact)) {
                        requestDataUse(
                            store.getValueMutable(), fact->valueLayout,
                            /*late=*/false, DataLayoutSeedPhase::Store);
                        if (failed(requestMaskUse(
                                store.getMaskMutable(), fact->maskLayout, op, DataLayoutSeedPhase::Store)))
                            return WalkResult::interrupt();
                    }
                }
                return WalkResult::advance();
            ...
            if (auto store = dyn_cast<VMIStrideStoreOp>(op)) {
                ...
                if (failed(requestMaskUse(store.getMaskMutable(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto scatter = dyn_cast<VMIScatterOp>(op)) {
                ...
                if (failed(requestMaskUse(scatter.getMaskMutable(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto split = dyn_cast<VMIChannelSplitOp>(op)) {
                ...
                requestDataUse(split.getSourceMutable(), VMILayoutAttr::getDeinterleaved(ctx, channels));
            ...
            if (auto merge = dyn_cast<VMIChannelMergeOp>(op)) {
                ...
                if (failed(setNaturalLayout(merge.getResult(), VMILayoutAttr::getDeinterleaved(ctx, channels), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto shuffle = dyn_cast<VMIShuffleOp>(op)) {
                ...
                if (isLane0SplatShuffle(shuffle))
                    return WalkResult::advance();
                if (failed(setNaturalLayout(shuffle.getResult(), getContiguousLayout(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto branchOp = dyn_cast<cf::BranchOp>(op)) {
                if (failed(addBranchConstraints(branchOp.getDest(), branchOp.getDestOperands(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto condBranchOp = dyn_cast<cf::CondBranchOp>(op)) {
                if (failed(addBranchConstraints(condBranchOp.getTrueDest(), condBranchOp.getTrueDestOperands(), op)) ||
                    failed(addBranchConstraints(condBranchOp.getFalseDest(), condBranchOp.getFalseDestOperands(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto switchOp = dyn_cast<cf::SwitchOp>(op)) {
                if (failed(addBranchConstraints(switchOp.getDefaultDestination(), switchOp.getDefaultOperands(), op)))
                    return WalkResult::interrupt();
                for (auto [dest, operands] : llvm::zip(switchOp.getCaseDestinations(), switchOp.getCaseOperands())) {
            ...
            if (auto funcOp = dyn_cast<func::FuncOp>(op)) {
                if (funcOp.empty() && hasVMIFunctionType(funcOp)) {
                    funcOp.emitError() << kVMIDiagLayoutContractPrefix
        ...
        for (OpResult result : ifOp->getResults()) {
            ...
            for (Region* region : {&ifOp.getThenRegion(), &ifOp.getElseRegion()}) {
                ...
                if (!yieldOp || resultNo >= yieldOp.getNumOperands())
                    continue;
                if (failed(uniteEquivalentValues(result, yieldOp.getOperand(resultNo), ifOp)))
                    return failure();
            }
        }
        return success();
    ...
    LogicalResult addYieldConstraints(ResultRange results, scf::YieldOp yieldOp, Operation* op)
    {
    ...
    LogicalResult addExecuteRegionConstraints(scf::ExecuteRegionOp executeOp)
    {
        WalkResult result = executeOp.getRegion().walk([&](scf::YieldOp yieldOp) {
            if (yieldOp->getParentOp() != executeOp.getOperation())
                return WalkResult::advance();
            if (failed(addYieldConstraints(executeOp->getResults(), yieldOp, executeOp)))
                return WalkResult::interrupt();
            return WalkResult::advance();
        });
        return failure(result.wasInterrupted());
    ...
    LogicalResult addIndexSwitchConstraints(scf::IndexSwitchOp indexSwitchOp)
    {
        auto addBlockTerminator = [&](Block& block) -> LogicalResult {
            ...
            if (!yieldOp)
                return success();
            return addYieldConstraints(indexSwitchOp->getResults(), yieldOp, indexSwitchOp);
        };
    ...
    LogicalResult addWhileConstraints(scf::WhileOp whileOp)
    {
        return VMIControlFlowSupport::addWhileConstraints(
            whileOp, [&](Value lhs, Value rhs, Operation* op) { return uniteEquivalentValues(lhs, rhs, op); });
    }

    LogicalResult addForConstraints(scf::ForOp forOp)
    {
        return VMIControlFlowSupport::addForConstraints(
            forOp, [&](Value lhs, Value rhs, Operation* op) { return uniteEquivalentValues(lhs, rhs, op); });
    }

    LogicalResult addBranchConstraints(Block* dest, OperandRange operands, Operation* op)
    {
        if (!dest)
            return success();
    ...
    bool hasVMIFunctionType(func::FuncOp func)
    {
        ...
        return llvm::any_of(type.getInputs(), containsVMIType) || llvm::any_of(type.getResults(), containsVMIType);
    }
    ...
    LogicalResult addCallConstraints(func::CallOp callOp)
    {
        ...
        auto callee = SymbolTable::lookupNearestSymbolFrom<func::FuncOp>(callOp, callOp.getCalleeAttr());
        if (!callee || callee.empty())
            return callOp.emitError() << kVMIDiagLayoutContractPrefix
                                      << "VMI typed call requires a direct internal callee with a body";

        for (auto [operand, argument] : llvm::zip(callOp.getOperands(), callee.getArguments())) {
        ...
        for (func::ReturnOp returnOp : returns) {
            for (auto [index, result] : llvm::enumerate(callOp.getResults())) {
                if (index >= returnOp.getNumOperands())
                    break;
                if (failed(uniteEquivalentValues(result, returnOp.getOperand(index), callOp)))
                    return failure();
            }
        }
        return success();
    ...
    void rewriteDataTypes()
    {
        for (DataNode& node : dataNodes) {
            ...
            node.value.setType(VMIVRegType::get(ctx, node.type.getElementCount(), node.type.getElementType(), layout));
        }
    }

    FailureOr<Value> materializeLayoutValue(Value value, Type targetType, Location loc, OpBuilder& builder)
    {
        ...
        if (auto sourceType = dyn_cast<VMIVRegType>(value.getType())) {
            ...
            if (!targetVRegType || sourceType.getElementCount() != targetVRegType.getElementCount() ||
                sourceType.getElementType() != targetVRegType.getElementType())
                return failure();
            return builder.create<VMIEnsureLayoutOp>(loc, targetVRegType, value).getResult();
        }

        if (auto sourceType = dyn_cast<VMIMaskType>(value.getType())) {
            auto targetMaskType = dyn_cast<VMIMaskType>(targetType);
            if (!targetMaskType || sourceType.getElementCount() != targetMaskType.getElementCount() ||
                sourceType.getGranularity() != targetMaskType.getGranularity())
                return failure();
            return builder.create<VMIEnsureMaskLayoutOp>(loc, targetMaskType, value).getResult();
        }
    ...
    SmallVector<Type> getCallResultTypes(func::FuncOp func)
    {
        ...
        module.walk([&](func::CallOp call) {
            if (call.getCallee() != func.getSymName())
                return;
            if (!found) {
                resultTypes.assign(call.getResultTypes().begin(), call.getResultTypes().end());
                found = true;
                return;
            }
        ...
        WalkResult callResult = module.walk([&](func::CallOp call) -> WalkResult {
            ...
            for (auto [index, operand] : llvm::enumerate(call.getOperands())) {
                ...
                if (!isa<VMIVRegType, VMIMaskType>(targetType))
                    continue;
                FailureOr<Value> materialized = materializeLayoutValue(operand, targetType, call.getLoc(), rewriter);
        ...
        WalkResult returnResult = module.walk([&](func::FuncOp func) -> WalkResult {
            ...
            WalkResult nested = func.walk([&](func::ReturnOp ret) -> WalkResult {
                ...
                for (auto [index, operand] : llvm::enumerate(ret.getOperands())) {
                    ...
                    if (!isa<VMIVRegType, VMIMaskType>(targetType))
                        continue;
                    FailureOr<Value> materialized = materializeLayoutValue(operand, targetType, ret.getLoc(), rewriter);
                    if (failed(materialized))
                        return WalkResult::interrupt();
            ...
            return nested.wasInterrupted() ? WalkResult::interrupt() : WalkResult::advance();
        });
    ...
    LogicalResult insertDataUseMaterializations()
    {
        ...
        for (DataUseRequest request : dataUseRequests) {
            ...
            auto resultType =
                VMIVRegType::get(ctx, sourceType.getElementCount(), sourceType.getElementType(), request.layout);
            builder.setInsertionPoint(request.operand->getOwner());
            auto ensure = builder.create<VMIEnsureLayoutOp>(request.operand->getOwner()->getLoc(), resultType, value);
    ...
    LogicalResult requestDataLayoutSeeds(
        VMILayoutPropagator& propagator, DataLayoutSeedPhase phase, bool skipAlreadyRequested)
    {
        ...
        if (skipAlreadyRequested) {
            for (DataLayoutSeed seed : dataLayoutSeeds) {
                if (seed.phase != phase)
                    continue;
                if (hasRequestedLayout(propagator, seed.value) && !llvm::is_contained(protectedValues, seed.value))
    ...
    LogicalResult requestDataUseSeeds(VMILayoutPropagator& propagator, DataLayoutSeedPhase phase, bool late)
    {
        for (DataUseRequest request : dataUseRequests)
            if (request.phase == phase && request.late == late) {
                if (hasLayoutAssignment(propagator, request.operand->get())) {
                    VMILayoutAttr assigned = propagator.getRequestedOrCurrentLayout(request.operand->get());
    ...
    LogicalResult requestMaskUseSeeds(VMILayoutPropagator& propagator, DataLayoutSeedPhase phase)
    {
    ...
    LogicalResult runSeedPhase(VMILayoutPropagator& propagator, DataLayoutSeedPhase phase)
    {
        if (failed(requestDataLayoutSeeds(
                propagator, phase,
    ...
    LogicalResult applyLayouts()
    {
        ...
        if (failed(requestDataLayoutSeeds(
                propagator, DataLayoutSeedPhase::Explicit,
                /*skipAlreadyRequested=*/false)))
            return failure();
        for (MaskNode& node : maskNodes) {
            MaskNode& root = maskNodes[findMask(maskIds.lookup(node.value))];
            if (root.requestedLayout && failed(propagator.request(node.value, root.requestedLayout)))
                return failure();
        }
        ...
        for (int64_t phase = static_cast<int64_t>(DataLayoutSeedPhase::SeedStart);
            ...
            if (failed(runSeedPhase(propagator, static_cast<DataLayoutSeedPhase>(phase))))
                return failure();

        for (DataUseRequest request : dataUseRequests)
            if (request.late && failed(propagator.request(*request.operand, request.layout)))
                return failure();
        if (failed(propagator.run()))
            return failure();
                ...
                for (Type type : functionType.getResults()) {
                    if (auto vregType = dyn_cast<VMIVRegType>(type)) {
                        results.push_back(VMIVRegType::get(
                            ctx, vregType.getElementCount(), vregType.getElementType(), getContiguousLayout()));
                    } else if (auto maskType = dyn_cast<VMIMaskType>(type)) {
                        results.push_back(
                            VMIMaskType::get(ctx, maskType.getElementCount(), "b32", getContiguousLayout()));
                    } else {
                        results.push_back(type);
                    }
                }
    ...
    LogicalResult run()
    {
        ...
        if (failed(materializeCallBoundaries()))
            return failure();
        rewriteFunctionType();
        return validateVMILayoutAssignedIR(
            module, /*diagOS=*/nullptr,
...
struct VMILayoutAssignmentPass : public mlir::pto::impl::VMILayoutAssignmentBase<VMILayoutAssignmentPass> {
...
std::unique_ptr<Pass> mlir::pto::createVMILayoutAssignmentPass() { return std::make_unique<VMILayoutAssignmentPass>(); }
