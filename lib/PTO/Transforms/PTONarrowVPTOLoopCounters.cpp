...
{
    return op->getParentOfType<pto::VecScopeOp>() || op->getParentOfType<pto::StrictVecScopeOp>();
}
...
static Value createI16Constant(PatternRewriter& rewriter, Location loc, int64_t value)
{
    Type i16Type = rewriter.getI16Type();
    return rewriter.create<arith::ConstantOp>(loc, i16Type, rewriter.getIntegerAttr(i16Type, value));
}

static Value restoreInductionVariableType(
    PatternRewriter& rewriter, Location loc, Value inductionVar, Type originalType)
{
...
struct NarrowVecScopeLoopCounterPattern : public OpRewritePattern<scf::ForOp> {
    ...
    LogicalResult matchAndRewrite(scf::ForOp forOp, PatternRewriter& rewriter) const override
    {
        ...
        if (!lower || !upper || !step || *step <= 0 || !exitValueFitsSignedI16(*lower, *upper, *step))
            return failure();
        ...
        auto newFor = rewriter.create<scf::ForOp>(loc, newLower, newUpper, newStep, forOp.getInitArgs());
        ...
        bodyArgumentReplacements.append(newFor.getRegionIterArgs().begin(), newFor.getRegionIterArgs().end());
...
struct PTONarrowVPTOLoopCounters : public pto::impl::PTONarrowVPTOLoopCountersBase<PTONarrowVPTOLoopCounters> {
    using pto::impl::PTONarrowVPTOLoopCountersBase<PTONarrowVPTOLoopCounters>::PTONarrowVPTOLoopCountersBase;
