...
static constexpr llvm::StringLiteral kOpLibAttrInstVariantId = "pto.oplib.instance.variant_id";
static constexpr llvm::StringLiteral kOpLibAttrInstOp = "pto.oplib.instance.op";
static constexpr llvm::StringLiteral kOpLibAttrInstDType = "pto.oplib.instance.dtype";
static constexpr llvm::StringLiteral kErrInstanceBodyMissing = "E_OPLIB_INSTANCE_BODY_MISSING";

static bool isInstanceFunc(func::FuncOp fn) { return fn->hasAttr(kOpLibAttrInstVariantId); }

static bool isTilelangInlineProcFunc(func::FuncOp fn) { return fn->hasAttr("pto.tilelang.inline_proc"); }

static bool isTileOpHelperFunc(func::FuncOp fn) { return fn->hasAttr("pto.tileop.helper"); }

static bool isTilelangTemplateFunc(func::FuncOp fn) { return fn->hasAttr("pto.tilelang.instance") && fn.isPrivate(); }

static bool isInlineableBackendHelperFunc(func::FuncOp fn) { return isTileOpHelperFunc(fn); }
...
static Operation* cloneOpForInlineWithFix(OpBuilder& builder, Operation& op, IRMapping& mapping)
{
    if (auto alloc = dyn_cast<pto::AllocTileOp>(&op)) {
        ...
        Value mappedAddr = mapOperand(alloc.getAddr(), alloc.getAddr() ? alloc.getAddr().getType() : Type());
        Value mappedValidRow =
            mapOperand(alloc.getValidRow(), alloc.getValidRow() ? alloc.getValidRow().getType() : Type());
        Value mappedValidCol =
            mapOperand(alloc.getValidCol(), alloc.getValidCol() ? alloc.getValidCol().getType() : Type());

        auto cloned = builder.create<pto::AllocTileOp>(
            alloc.getLoc(), alloc.getType(), mappedAddr, mappedValidRow, mappedValidCol);
...
static LogicalResult inlineCall(func::CallOp call, func::FuncOp callee)
{
    ...
    for (Operation& op : entry.without_terminator()) {
        FailureOr<bool> handledOr = pto::tryCloneOpLibInlineBridgeOp(builder, op, mapping);
        if (failed(handledOr))
    ...
    for (auto [callResult, returnOperand] : llvm::zip(call.getResults(), returnOp.getOperands())) {
...
{
    call.emitError() << kErrInstanceBodyMissing << ": OP-Lib instance body is missing for @" << callee.getSymName();
    if (auto variant = callee->getAttrOfType<StringAttr>(kOpLibAttrInstVariantId)) {
...
static LogicalResult validateInlineableCalleesHaveBodies(ModuleOp module, InlinePredicate&& shouldInline)
{
    for (ModuleOp funcModule : collectFuncModules(module)) {
        for (func::FuncOp func : funcModule.getOps<func::FuncOp>()) {
            ...
            func.walk([&](func::CallOp call) {
                ...
                func::FuncOp callee = funcModule.lookupSymbol<func::FuncOp>(calleeAttr.getValue());
...
static LogicalResult inlineMatchingCalls(
    ModuleOp module, InlinePredicate&& shouldInline, bool debug, llvm::StringRef debugTag, int& inlinedCalls,
    int& touchedFuncs)
{
    for (ModuleOp funcModule : collectFuncModules(module)) {
        for (func::FuncOp func : funcModule.getOps<func::FuncOp>()) {
            ...
            while (madeProgress) {
                ...
                for (func::CallOp oldCall : calls) {
                    ...
                    OpBuilder builder(call);
                    auto newCall = builder.create<func::CallOp>(call.getLoc(), callee, concreteOperands);
                    if (call.getNumResults() != newCall.getNumResults()) {
                        call.emitOpError("call result arity mismatch during inline staging");
                        return failure();
                    }
                    for (auto [oldResult, newResult] : llvm::zip(call.getResults(), newCall.getResults()))
                    ...
                    madeProgress = true;
                    if (debug) {
                        llvm::errs() << debugTag << ": inlined @" << callee.getSymName() << " into @"
                                     << func.getSymName() << "\n";
                    }
                }
            }
...
static void eraseDeadMatchingPrivateFuncs(ModuleOp module, FuncPredicate&& predicate)
{
    for (ModuleOp funcModule : collectFuncModules(module)) {
...
struct PTOInlineBackendHelpersPass : public pto::impl::PTOInlineBackendHelpersBase<PTOInlineBackendHelpersPass> {
    using pto::impl::PTOInlineBackendHelpersBase<PTOInlineBackendHelpersPass>::PTOInlineBackendHelpersBase;
    ...
    void runOnOperation() override
    {
        ...
        int inlinedCalls = 0;
        int touchedFuncs = 0;
        if (failed(inlineMatchingCalls(
                module, isInlineableBackendHelperFunc, debug, "[op-fusion] inline-backend-helpers", inlinedCalls,
                touchedFuncs))) {
            signalPassFailure();
            return;
        }
        ...
        if (debug) {
            llvm::errs() << "[op-fusion] inline-backend-helpers touched " << touchedFuncs << " function(s), inlined "
                         << inlinedCalls << " call(s)\n";
        }
...
struct PTOInlineLibCallPass : public pto::impl::PTOInlineLibCallBase<PTOInlineLibCallPass> {
    using pto::impl::PTOInlineLibCallBase<PTOInlineLibCallPass>::PTOInlineLibCallBase;
    ...
    void runOnOperation() override
    {
        ...
        if (failed(inlineMatchingCalls(
                module, isInlineableLibFunc, debug, "[op-fusion] inline-libcall", inlinedCalls, touchedFuncs))) {
            signalPassFailure();
            return;
        }

        if (debug) {
            llvm::errs() << "[op-fusion] inline-libcall touched " << touchedFuncs << " function(s), inlined "
                         << inlinedCalls << " call(s)\n";
        }
...
std::unique_ptr<Pass> mlir::pto::createPTOInlineLibCallPass(const PTOInlineLibCallOptions& options)
{
    return std::make_unique<PTOInlineLibCallPass>(options);
}

std::unique_ptr<Pass> mlir::pto::createPTOInlineBackendHelpersPass(const PTOInlineBackendHelpersOptions& options)
{
