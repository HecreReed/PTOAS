...
LogicalResult VMIControlFlowSupport::addForConstraints(scf::ForOp forOp, EquivalenceCallback addEquivalent)
{
    ...
    for (auto [index, initArg] : llvm::enumerate(forOp.getInitArgs())) {
        ...
        if (yieldOp && index < yieldOp.getNumOperands() &&
            ...
            return failure();
        if (index < forOp.getNumResults() && failed(addEquivalent(anchor, forOp.getResult(index), forOp)))
            return failure();
    }
    return success();
}

LogicalResult VMIControlFlowSupport::addWhileConstraints(scf::WhileOp whileOp, EquivalenceCallback addEquivalent)
{
    ...
    for (auto [index, init] : llvm::enumerate(whileOp.getInits())) {
        if (index < whileOp.getBeforeArguments().size() &&
            failed(addEquivalent(init, whileOp.getBeforeArguments()[index], whileOp)))
            return failure();
        if (index < yieldOp.getNumOperands() && failed(addEquivalent(init, yieldOp.getOperand(index), whileOp)))
            return failure();
    }

    for (auto [index, forwarded] : llvm::enumerate(conditionOp.getArgs())) {
        if (index < afterBlock.getNumArguments() &&
            failed(addEquivalent(forwarded, afterBlock.getArgument(index), whileOp)))
            return failure();
        if (index < whileOp.getNumResults() && failed(addEquivalent(forwarded, whileOp.getResult(index), whileOp)))
            return failure();
    }
    return success();
}
