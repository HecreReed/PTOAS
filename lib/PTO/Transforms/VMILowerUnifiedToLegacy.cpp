...
static bool hasMergePmode(Operation* op) { return getPmodeOrDefault(op) == "merge"; }

/// Create a zero-valued VMIConstantOp with the same type as \p vmiType.
static Value createZeroConstant(OpBuilder& builder, Location loc, VMIVRegType vmiType)
{
    ...
    if (auto floatType = dyn_cast<FloatType>(elemType)) {
        zeroAttr = DenseElementsAttr::get(shapedType, APFloat::getZero(floatType.getFloatSemantics()));
    } else if (auto intType = dyn_cast<IntegerType>(elemType)) {
        zeroAttr = DenseElementsAttr::get(shapedType, APInt::getZero(intType.getWidth()));
    } else {
...
static Value createReduceNeutralInit(
    OpBuilder& builder, Location loc, Type elemType, bool isAdd, bool isMax, Attribute layout = Attribute())
{
    auto oneLaneType = VMIVRegType::get(builder.getContext(), 1, elemType, layout);
    auto shapedType = RankedTensorType::get({1}, elemType);
    DenseElementsAttr attr;
    if (auto floatTy = dyn_cast<FloatType>(elemType)) {
        if (isAdd)
            attr = DenseElementsAttr::get(shapedType, APFloat::getZero(floatTy.getFloatSemantics()));
        else if (isMax)
            attr = DenseElementsAttr::get(shapedType, APFloat::getInf(floatTy.getFloatSemantics(), /*Negative=*/true));
        else
            attr = DenseElementsAttr::get(shapedType, APFloat::getInf(floatTy.getFloatSemantics(), /*Negative=*/false));
    } else {
        auto intTy = cast<IntegerType>(elemType);
        if (isAdd)
            attr = DenseElementsAttr::get(shapedType, APInt::getZero(intTy.getWidth()));
        else if (isMax)
            attr = DenseElementsAttr::get(
                shapedType,
                intTy.isUnsigned() ? APInt::getZero(intTy.getWidth()) : APInt::getSignedMinValue(intTy.getWidth()));
        else
            attr = DenseElementsAttr::get(
                shapedType,
                intTy.isUnsigned() ? APInt::getMaxValue(intTy.getWidth()) : APInt::getSignedMaxValue(intTy.getWidth()));
    }
...
static std::string mapCmpPredicate(StringRef cmp, Type elemType, bool isFloat)
{
    if (isFloat) {
...
/// (hif8, f8, f4, etc.).
static bool isFloatType(Type elemType) { return isa<FloatType>(elemType) || pto::isPTOLowPrecisionType(elemType); }

/// Return the element type of a VMIVRegType.
static Type getVMIElementType(Value v) { return cast<VMIVRegType>(v.getType()).getElementType(); }
...
static LogicalResult lowerBinaryIgnoringMask(
    UnifiedOp op, function_ref<Value(Location, Type, Value, Value)> createLegacy)
{
    if (hasMergePmode(op))
        return failure();
...
template <typename UnifiedOp>
static LogicalResult lowerMaskedUnary(
    UnifiedOp op, OpBuilder& builder, function_ref<Value(Location, Type, Value)> createLegacy)
{
    if (hasMergePmode(op))
        return failure();
...
static bool isCompactGroupCount(int64_t count) { return count == 1 || count == 2 || count == 4 || count == 8; }
...
static LogicalResult lowerVCmp(VMIVcmpOp op, OpBuilder& builder)
{
    ...
    if (isFloat) {
        rawMask = builder
                      .create<VMICmpFOp>(
                          loc, op.getResult().getType(), builder.getStringAttr(predicate), op.getLhs(), op.getRhs())
                      .getResult();
    } else {
        rawMask = builder
                      .create<VMICmpIOp>(
    ...
    if (!isAllActiveSeed(op.getSeed()))
        result = builder.create<VMIMaskAndOp>(loc, op.getResult().getType(), rawMask, op.getSeed()).getResult();
...
static LogicalResult lowerVCmps(VMIVcmpsOp op, OpBuilder& builder)
{
    ...
    Value brc = builder.create<VMIBroadcastOp>(loc, srcVmiType, scalar).getResult();

    // 2. Legacy cmpf or cmpi.
    Value rawMask;
    if (isFloat) {
        rawMask =
            builder.create<VMICmpFOp>(loc, op.getResult().getType(), builder.getStringAttr(predicate), op.getSrc(), brc)
                .getResult();
    } else {
        rawMask =
            builder.create<VMICmpIOp>(loc, op.getResult().getType(), builder.getStringAttr(predicate), op.getSrc(), brc)
                .getResult();
    }
...
static LogicalResult lowerVCvt(VMICvtOp op, OpBuilder& builder)
{
    ...
    if (direction == "widen_fp") {
        ...
        result = builder.create<VMITruncFOp>(loc, resultType, source, roundingAttr, saturateAttr).getResult();
    } else if (direction == "fptosi") {
        result = builder.create<VMIFPToSIOp>(loc, resultType, source, saturateAttr).getResult();
    } else if (direction == "fptoui") {
        result = builder.create<VMIFPToUIOp>(loc, resultType, source, saturateAttr).getResult();
        ...
        if (useSigned)
            result = builder.create<VMIExtSIOp>(loc, resultType, source).getResult();
        else
            result = builder.create<VMIExtUIOp>(loc, resultType, source).getResult();
    } else if (direction == "narrow_int") {
        result = builder.create<VMITruncIOp>(loc, resultType, source, saturateAttr).getResult();
    } else {
        return failure();
    }
...
static LogicalResult lowerVLoad(VMIvLoadOp op, OpBuilder& builder)
{
    ...
    if (op.getGroupAttr()) {
        ...
        if (op.getDistMode() && op.getDistMode() == "brc") {
            auto gbl = builder.create<VMIGroupBroadcastLoadOp>(
                op->getLoc(), resultType, op.getSource(), op.getOffset(), op.getStride(), op.getGroupAttr());
    ...
    if (op.getBlockStride()) {
        ...
        if (auto it = dyn_cast<IntegerType>(elemType))
            ...
            bits = ft.getWidth();
        auto gran = StringAttr::get(builder.getContext(), bits <= 8 ? "b8" : bits <= 16 ? "b16" : "b32");
        auto maskType =
            VMIMaskType::get(builder.getContext(), resultVMIType.getElementCount(), gran, resultVMIType.getLayout());
        auto fullLanes =
            builder.create<arith::ConstantOp>(op->getLoc(), builder.getIndexAttr(resultVMIType.getElementCount()));
        auto mask = builder.create<VMICreateMaskOp>(op->getLoc(), maskType, fullLanes.getResult());
        Value bs = op.getBlockStride();
        Value rs = op.getRepeatStride();
        auto strideLoad = builder.create<VMIStrideLoadOp>(
            op->getLoc(), resultType, op.getSource(), op.getOffset(), bs, rs, mask.getResult());
    ...
    if (distMode == "continuous") {
        ...
        if (isCompactGroupCount(numGroups)) {
            ...
            auto loadOp = builder.create<VMIGroupSlotLoadOp>(
                loc, resultType, source, offset, unitStride, builder.getI64IntegerAttr(numGroups));
        ...
        auto dloadOp = builder.create<VMIDeinterleaveLoadOp>(
            loc, op.getResults()[0].getType(), op.getResults()[1].getType(), source, offset);
        ...
        if (auto groupAttr = op.getGroupAttr()) {
            ...
            auto gbl = builder.create<VMIGroupBroadcastLoadOp>(
                loc, resultType, source, offset, stride, builder.getI64IntegerAttr(numGroups));
            op.getResults().front().replaceAllUsesWith(gbl.getResult());
        } else {
            Value zeroStride = builder.create<arith::ConstantOp>(loc, builder.getIndexType(), builder.getIndexAttr(0));
            auto gbl = builder.create<VMIGroupBroadcastLoadOp>(
                loc, resultType, source, offset, zeroStride, builder.getI64IntegerAttr(1));
            op.getResults().front().replaceAllUsesWith(gbl.getResult());
        }
    } else {
...
static LogicalResult lowerVStore(VMIvStoreOp op, OpBuilder& builder)
{
    ...
    if (op.getGroupAttr()) {
        builder.create<VMIGroupStoreOp>(
            op->getLoc(), op.getValues()[0], op.getDestination(), op.getOffset(), op.getStride(), op.getGroupAttr());
        op->erase();
        return success();
    }
    ...
    if (op.getBlockStride()) {
        ...
        if (!op.getMask().empty()) {
            ...
            auto gran = StringAttr::get(builder.getContext(), bits <= 8 ? "b8" : bits <= 16 ? "b16" : "b32");
            auto maskType =
                VMIMaskType::get(builder.getContext(), valueType.getElementCount(), gran, valueType.getLayout());
            auto fullLanes =
                builder.create<arith::ConstantOp>(op->getLoc(), builder.getIndexAttr(valueType.getElementCount()));
            mask = builder.create<VMICreateMaskOp>(op->getLoc(), maskType, fullLanes.getResult()).getResult();
        }
        Value bs = op.getBlockStride();
        Value rs = op.getRepeatStride();
        builder.create<VMIStrideStoreOp>(
            op->getLoc(), op.getValues()[0], op.getDestination(), op.getOffset(), bs, rs, mask);
        op->erase();
        return success();
    }
    ...
    if (distMode == "continuous") {
        ...
        if (compact && allActive) {
            Value unitStride = builder.create<arith::ConstantIndexOp>(loc, 1);
            builder.create<VMIGroupStoreOp>(
                loc, values[0], dest, offset, unitStride, builder.getI64IntegerAttr(numGroups));
            op->erase();
            return success();
        }
        if (mask) {
        ...
        if (values.size() < 2)
            return failure();
        builder.create<VMIInterleaveStoreOp>(loc, values[0], values[1], dest, offset);
    } else {
        return failure();
    }
...
static LogicalResult lowerPset(VMIPsetOp op, OpBuilder& builder)
{
    ...
    Value activeLanes = builder.create<arith::ConstantOp>(loc, indexType, builder.getIndexAttr(laneCount));
    Value result = builder.create<VMICreateMaskOp>(loc, maskType, activeLanes).getResult();
    op.getResult().replaceAllUsesWith(result);
    op->erase();
    return success();
}
...
static LogicalResult lowerPge(VMIPgeOp op, OpBuilder& builder)
{
    ...
    auto maskType = cast<VMIMaskType>(op.getResult().getType());
    auto indexType = IndexType::get(builder.getContext());
    Value activeLanes = builder.create<arith::ConstantOp>(loc, indexType, builder.getIndexAttr(numLanes));
    ...
    if (auto groupAttr = op.getGroupAttr()) {
        ...
        Value result = builder
                           .create<VMICreateGroupMaskOp>(
                               loc, maskType, activeLanes, builder.getI64IntegerAttr(numGroups),
...
static LogicalResult lowerVCadd(VMIvcaddOp op, OpBuilder& builder)
{
    ...
    if (std::optional<int64_t> numGroups = getReductionNumGroups(op)) {
        ...
        if (isFloat)
            result = builder
                         .create<VMIGroupReduceAddFOp>(
                             loc, resultType, source, mask, builder.getI64IntegerAttr(*numGroups), op.getReassocAttr())
                         .getResult();
        else
            result =
                builder
                    .create<VMIGroupReduceAddIOp>(loc, resultType, source, mask, builder.getI64IntegerAttr(*numGroups))
                    .getResult();
        op.getResult().replaceAllUsesWith(result);
    } else {
        // Full reduce path
        Value init = createReduceNeutralInit(
            builder, loc, elemType,
            /*isAdd=*/true, /*isMax=*/false, sourceType.getLayout());
        Value result;
        if (isFloat)
            result =
                builder.create<VMIReduceAddFOp>(loc, resultType, source, init, mask, op.getReassocAttr()).getResult();
        else
            result = builder.create<VMIReduceAddIOp>(loc, resultType, source, init, mask).getResult();
        op.getResult().replaceAllUsesWith(result);
    }
    op->erase();
    return success();
...
static LogicalResult lowerVcmax(VMIvcmaxOp op, OpBuilder& builder)
{
    ...
    if (std::optional<int64_t> numGroups = getReductionNumGroups(op)) {
        ...
        if (isFloat)
            result =
                builder
                    .create<VMIGroupReduceMaxFOp>(loc, resultType, source, mask, builder.getI64IntegerAttr(*numGroups))
                    .getResult();
        else
            result =
                builder
                    .create<VMIGroupReduceMaxIOp>(loc, resultType, source, mask, builder.getI64IntegerAttr(*numGroups))
                    .getResult();
        op.getResult().replaceAllUsesWith(result);
        op->erase();
        return success();
    ...
    Value init = createReduceNeutralInit(
        builder, loc, elemType,
        /*isAdd=*/false, /*isMax=*/true, sourceType.getLayout());
    Value result;
    if (isFloat)
        result = builder.create<VMIReduceMaxFOp>(loc, resultType, source, init, mask).getResult();
    else
        result = builder.create<VMIReduceMaxIOp>(loc, resultType, source, init, mask).getResult();
    op.getResult().replaceAllUsesWith(result);
    op->erase();
    return success();
}
...
static LogicalResult lowerVcmin(VMIvcminOp op, OpBuilder& builder)
{
    ...
    if (std::optional<int64_t> numGroups = getReductionNumGroups(op)) {
        ...
        if (isFloat)
            result =
                builder
                    .create<VMIGroupReduceMinFOp>(loc, resultType, source, mask, builder.getI64IntegerAttr(*numGroups))
                    .getResult();
        else
            result =
                builder
                    .create<VMIGroupReduceMinIOp>(loc, resultType, source, mask, builder.getI64IntegerAttr(*numGroups))
                    .getResult();
        op.getResult().replaceAllUsesWith(result);
        op->erase();
        return success();
    ...
    Value init = createReduceNeutralInit(
        builder, loc, elemType,
        /*isAdd=*/false, /*isMax=*/false, sourceType.getLayout());
    Value result;
    if (isFloat)
        result = builder.create<VMIReduceMinFOp>(loc, resultType, source, init, mask).getResult();
    else
        result = builder.create<VMIReduceMinIOp>(loc, resultType, source, init, mask).getResult();
    op.getResult().replaceAllUsesWith(result);
    op->erase();
    return success();
}
...
static LogicalResult lowerVmula(VMIVmulaOp op, OpBuilder& builder)
{
    ...
    Value result = builder.create<VMIFmaOp>(loc, resultType, op.getLhs(), op.getRhs(), op.getAcc()).getResult();
    op.getResult().replaceAllUsesWith(result);
    op->erase();
    return success();
}
...
static LogicalResult lowerVaxpy(VMIVaxpyOp op, OpBuilder& builder)
{
    ...
    Location loc = op.getLoc();
    Value alphaVec = builder.create<VMIBroadcastOp>(loc, resultType, op.getAlpha()).getResult();
    // fma(alpha, x, y) == alpha*x + y.
    Value raw = builder.create<VMIFmaOp>(loc, resultType, alphaVec, op.getX(), op.getAcc()).getResult();
    op.getResult().replaceAllUsesWith(raw);
    op->erase();
    return success();
}
...
static LogicalResult lowerPlt(VMIPltOp op, OpBuilder& builder)
{
    ...
    Value cL = builder.create<arith::ConstantOp>(loc, i32Type, builder.getIntegerAttr(i32Type, laneCount));
    Value act = builder.create<arith::MinSIOp>(loc, i32Type, op.getScalar(), cL);
    Value aidx = builder.create<arith::IndexCastOp>(loc, builder.getIndexType(), act);
...
static LogicalResult lowerVgather(VMIVgatherOp op, OpBuilder& builder)
{
    ...
    Value result = builder.create<VMIGatherOp>(loc, resultType, op.getSource(), op.getOffsets(), op.getMask(), passthru)
                       .getResult();
    op.getResult().replaceAllUsesWith(result);
    op->erase();
    return success();
...
static LogicalResult lowerVscatter(VMIVscatterOp op, OpBuilder& builder)
{
    ...
    Location loc = op.getLoc();
    builder.create<VMIScatterOp>(loc, op.getValue(), op.getDestination(), op.getOffsets(), op.getMask());
    op->erase();
    return success();
}
...
static LogicalResult lowerVlrelu(VMIVlreluOp op, OpBuilder& builder)
{
    ...
    Value pos = builder.create<VMIMaxFOp>(loc, resultType, x, zeroConst).getResult();
    Value neg = builder.create<VMIMinFOp>(loc, resultType, x, zeroConst).getResult();
    Value slopeVec = builder.create<VMIBroadcastOp>(loc, resultType, op.getSlope()).getResult();
    Value scaledNeg = builder.create<VMIMulFOp>(loc, resultType, slopeVec, neg).getResult();
    Value raw = builder.create<VMIAddFOp>(loc, resultType, pos, scaledNeg).getResult();
    op.getResult().replaceAllUsesWith(raw);
    op->erase();
    return success();
}
...
static LogicalResult lowerVprelu(VMIVpreluOp op, OpBuilder& builder)
{
    ...
    Value pos = builder.create<VMIMaxFOp>(loc, resultType, x, zeroConst).getResult();
    Value neg = builder.create<VMIMinFOp>(loc, resultType, x, zeroConst).getResult();
    Value scaledNeg = builder.create<VMIMulFOp>(loc, resultType, op.getAlpha(), neg).getResult();
    Value raw = builder.create<VMIAddFOp>(loc, resultType, pos, scaledNeg).getResult();
    op.getResult().replaceAllUsesWith(raw);
    op->erase();
    return success();
...
struct VMILowerUnifiedToLegacyPass : public mlir::pto::impl::VMILowerUnifiedToLegacyBase<VMILowerUnifiedToLegacyPass> {
...
void VMILowerUnifiedToLegacyPass::runOnOperation()
{
    ...
    module.walk([&](Operation* op) {
        ...
        if (isa<VMIVciOp, VMIVinterpretCastOp, VMIvSelOp, VMIVbrcOp>(op) ||
            // Category B — binary
            isa<VMIVaddOp, VMIVsubOp, VMIVmulOp, VMIVdivOp, VMIVminOp, VMIVmaxOp, VMIVandOp, VMIVorOp, VMIVxorOp,
                VMIVshlOp, VMIVshrOp>(op) ||
            // Category B — unary
            isa<VMIVnegOp, VMIVabsOp, VMIVsqrtOp, VMIVexpOp, VMIVlnOp, VMIVreluOp, VMIVnotOp>(op) ||
            // Category C1
        ...
        if (isa<VMIAddSOp, VMIMulSOp, VMIMaxSOp, VMIMinSOp, VMIShlSOp, VMIShrSOp, VMIVintlvOp, VMIVdintlvOp, VMIVselrOp,
    ...
    for (Operation* op : llvm::reverse(worklist)) {
        ...
        if (auto vop = dyn_cast<VMIVciOp>(op)) {
            ...
            if (groupAttr && groupAttr.getInt() > 1) {
                if (auto vmiTy = dyn_cast<VMIVRegType>(resultType)) {
                    ...
                    if (layout && !layout.isContiguous()) {
                        Type contigType = VMIVRegType::get(
                            op->getContext(), vmiTy.getElementCount(), vmiTy.getElementType(),
                            VMILayoutAttr::getContiguous(op->getContext()));
                        Value contig =
                            builder
                                .create<VMIGroupIotaOp>(op->getLoc(), contigType, vop.getBase(), orderAttr, groupAttr)
                                .getResult();
                        Value converted = builder.create<VMIEnsureLayoutOp>(op->getLoc(), vmiTy, contig).getResult();
                ...
                Value grouped =
                    builder.create<VMIGroupIotaOp>(op->getLoc(), resultType, vop.getBase(), orderAttr, groupAttr)
                        .getResult();
            ...
            Value iota = builder.create<VMIIotaOp>(op->getLoc(), resultType, vop.getBase(), orderAttr).getResult();
        ...
        if (auto vop = dyn_cast<VMIVinterpretCastOp>(op)) {
            ...
            builder.setInsertionPoint(op);
            Value result =
                builder.create<VMIBitcastOp>(op->getLoc(), vop.getResult().getType(), vop.getSource()).getResult();
        ...
        if (auto vop = dyn_cast<VMIvSelOp>(op)) {
            ...
            Value result =
                builder
                    .create<VMISelectOp>(
                        op->getLoc(), vop.getResult().getType(), vop.getMask(), vop.getTrueValue(), vop.getFalseValue())
                    .getResult();
            vop.getResult().replaceAllUsesWith(result);
            op->erase();
            continue;
            ...
            if (vop.getGroupAttr()) {
                result = builder
                             .create<VMIGroupBroadcastOp>(
                                 op->getLoc(), vop.getResult().getType(), vop.getValue(), vop.getGroupAttr())
                             .getResult();
            } else {
                result =
                    builder.create<VMIBroadcastOp>(op->getLoc(), vop.getResult().getType(), vop.getValue()).getResult();
            }
            vop.getResult().replaceAllUsesWith(result);
            op->erase();
            continue;
        ...
        if (auto vop = dyn_cast<VMIVsstbOp>(op)) {
            ...
            if (hasMergePmode(vop))
                continue;
            Value repeatStride = builder.create<arith::ConstantOp>(vop.getLoc(), builder.getI16IntegerAttr(0));
            builder.create<VMIStrideStoreOp>(
                vop.getLoc(), vop.getValue(), vop.getDestination(), vop.getOffset(), vop.getBlockStride(), repeatStride,
                vop.getMask());
            vop->erase();
            continue;
        }
        ...
        if (auto vop = dyn_cast<VMIVandOp>(op)) {
            ...
            if (isa<VMIMaskType>(vop.getLhs().getType())) {
                ...
                Value result =
                    builder.create<VMIMaskAndOp>(vop.getLoc(), vop.getResult().getType(), vop.getLhs(), vop.getRhs())
                        .getResult();
                vop.getResult().replaceAllUsesWith(result);
                op->erase();
                continue;
        ...
        if (auto vop = dyn_cast<VMIVorOp>(op)) {
            ...
            if (isa<VMIMaskType>(vop.getLhs().getType())) {
                ...
                Value result =
                    builder.create<VMIMaskOrOp>(vop.getLoc(), vop.getResult().getType(), vop.getLhs(), vop.getRhs())
                        .getResult();
                vop.getResult().replaceAllUsesWith(result);
                op->erase();
                continue;
        ...
        if (auto vop = dyn_cast<VMIVxorOp>(op)) {
            ...
            if (isa<VMIMaskType>(vop.getLhs().getType())) {
                ...
                Value result =
                    builder.create<VMIMaskXOrOp>(vop.getLoc(), vop.getResult().getType(), vop.getLhs(), vop.getRhs())
                        .getResult();
                vop.getResult().replaceAllUsesWith(result);
                op->erase();
                continue;
        ...
        if (auto vop = dyn_cast<VMIVabsOp>(op)) {
            ...
            auto createLegacy = [&](Location loc, Type ty, Value src) -> Value {
                ...
                if (elemType.isBF16()) {
                    ...
                    auto iTy = VMIVRegType::get(builder.getContext(), srcTy.getElementCount(), i16, srcTy.getLayout());
                    Value asI = builder.create<VMIBitcastOp>(loc, iTy, src);
                    Value c = builder.create<arith::ConstantOp>(loc, i16, builder.getIntegerAttr(i16, 0x7FFF));
                    Value maskVec = builder.create<VMIBroadcastOp>(loc, iTy, c).getResult();
                    Value cleared = builder.create<VMIAndIOp>(loc, iTy, asI, maskVec).getResult();
        ...
        if (auto vop = dyn_cast<VMIVnotOp>(op)) {
            ...
            if (isa<VMIMaskType>(vop.getSource().getType())) {
                ...
                Value result =
                    builder.create<VMIMaskNotOp>(vop.getLoc(), vop.getResult().getType(), vop.getSource()).getResult();
                vop.getResult().replaceAllUsesWith(result);
                op->erase();
                continue;
            }
