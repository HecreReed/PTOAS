...
{
    return getPTOStorageElemByteSize(ty) == 1 && isa<Type>(ty) && ty.getTypeID() == ty.getTypeID() && [&]() {
        std::string buffer;
...
static std::optional<Layout> inferNZLayout(ArrayRef<int64_t> shape, ArrayRef<int64_t> stride, unsigned elemBytes)
{
    ...
    bool alignMatch = (sh3 == kNZInnerRows) && (sh3 * sh4 * static_cast<int64_t>(elemBytes) == kNZFractalBytes);
...
{
    if (!mrTy.hasStaticShape() || mrTy.getRank() == 0 || mrTy.getRank() > kPaddedLayoutRank)
        return std::nullopt;
    SmallVector<int64_t> strideInts;
    int64_t offset = ShapedType::kDynamic;
    if (failed(mlir::pto::getPTOMemRefStridesAndOffset(mrTy, strideInts, offset)))
        return std::nullopt;
                    ...
                    if (auto dstTy = dyn_cast<TileBufType>(load.getDst().getType())) {
                        ...
                        if (dstSpace && dstSpace.getAddressSpace() == AddressSpace::MAT &&
                            ...
                            if (bl && sl) {
                                ...
                                } else if (bl.getValue() == BLayout::ColMajor && sl.getValue() == SLayout::ColMajor) {
    ...
    MakeTensorViewOp op, ArrayRef<int64_t> shape, ArrayRef<int64_t> strides, bool& isAmbiguous)
{
    ...
    if (!pref.conflict && pref.preferred && (*pref.preferred == Layout::MX_A_ZZ || *pref.preferred == Layout::MX_B_NN))
...
struct InferPTOLayoutPass : public mlir::pto::impl::InferPTOLayoutBase<InferPTOLayoutPass> {
    ...
    StringRef getDescription() const final { return "Infer GlobalTensor layout (ND/DN/NZ) for make_tensor_view"; }
    ...
    void runOnOperation() override
    {
        ...
        // ------------------------------------------------------------------
        func.walk([&](MakeTensorViewOp op) { inferMakeTensorViewLayoutAttr(op, [this] { signalPassFailure(); }); });

        // ------------------------------------------------------------------
        // 2) memref.reinterpret_cast (lowered from make_tensor_view)
        // ------------------------------------------------------------------
        func.walk(
            [&](memref::ReinterpretCastOp op) { inferReinterpretCastLayoutAttr(op, [this] { signalPassFailure(); }); });
        ...
        func.walk([&](memref::SubViewOp op) {
            ...
            SmallVector<int64_t> strideInts;
            int64_t offset = ShapedType::kDynamic;
            if (failed(mlir::pto::getPTOMemRefStridesAndOffset(srcTy, strideInts, offset)) ||
        ...
        func.walk([&](pto::TStoreOp op) {
            attachLoadStoreLayout(
                op, [](auto store) { return store.getDst(); }, [](auto store) { return store.getSrc(); });
        });
    }
};
