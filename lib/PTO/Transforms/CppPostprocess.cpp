...
static bool parseLastUseMarkerName(llvm::StringRef markerName, std::string& callee, std::string& lastUseArgs)
{
    ...
    while (pos < encoded.size()) {
        ...
        llvm::StringRef token = next == llvm::StringRef::npos ? encoded.drop_front(pos) : encoded.slice(pos, next);
        if (token.empty())
            return false;
