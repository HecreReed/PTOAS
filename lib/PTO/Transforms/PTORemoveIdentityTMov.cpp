...
{
    return llvm::all_of(value.getUses(), [currentOp](OpOperand& use) { return use.getOwner() == currentOp; });
}

static bool hasNoResultUses(Operation* op)
{
    return llvm::all_of(op->getResults(), [](OpResult result) { return result.use_empty(); });
}

static bool isDeadDstTMov(TMovOp op) { return hasOnlyCurrentOpUses(op.getDst(), op) && hasNoResultUses(op); }

static const BaseMemInfo* getSingleMemInfo(const Buffer2MemInfoMap& buffer2MemInfoMap, Value value)
{
...
static std::optional<int64_t> tryGetConcreteRootAddress(const BaseMemInfo* info)
{
    if (!info)
        return std::nullopt;
...
{
    return llvm::any_of(values, [](int64_t value) { return value == ShapedType::kDynamic; });
}
...
static bool isStaticallyAddressableValue(Value value)
{
    ...
    while (value && depth++ < kMaxDepth) {
        ...
        if (auto subView = dyn_cast<memref::SubViewOp>(defOp)) {
            if (hasDynamicStaticList(subView.getStaticOffsets()) || hasDynamicStaticList(subView.getStaticSizes()) ||
...
static bool hasExactSameAddressRange(const BaseMemInfo* srcInfo, const BaseMemInfo* dstInfo)
{
    if (!srcInfo || !dstInfo)
        return false;
...
static bool hasSameConcreteAddressRange(const BaseMemInfo* srcInfo, const BaseMemInfo* dstInfo)
{
...
static bool hasLaterUseOfSameAddressRange(
    TMovOp op, const BaseMemInfo* dstInfo, const Buffer2MemInfoMap& buffer2MemInfoMap)
{
    for (const auto& entry : buffer2MemInfoMap) {
        ...
        if (entry.first == op.getSrc())
            continue;
        bool sameRange = llvm::any_of(
            entry.second, [&](const auto& info) { return hasSameConcreteAddressRange(info.get(), dstInfo); });
...
static bool isIdentityTMovByMemInfo(TMovOp op, const Buffer2MemInfoMap& buffer2MemInfoMap)
{
    Value src = op.getSrc();
    Value dst = op.getDst();
    ...
    return isDeadDstTMov(op) && !hasLaterUseOfSameAddressRange(op, dstInfo, buffer2MemInfoMap);
}

struct PTORemoveIdentityTMovPass : public mlir::pto::impl::PTORemoveIdentityTMovBase<PTORemoveIdentityTMovPass> {
    void runOnOperation() override
    {
        ...
        func.walk([&](TMovOp op) {
            if (!hasPlainTMovSemantics(op) || !hasCompatibleIdentityTypes(op) || touchesLowPrecisionElement(op))
                return;
