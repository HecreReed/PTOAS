...
constexpr llvm::StringLiteral kAIVectorScopeAttrName = "llvm.loop.aivector_scope";
...
public:
    ...
    static bool isLegalityTypedValue(Type type) { return isa<VRegType, MaskType, AlignType>(type); }

    static bool isBufferLikeValue(Type type) { return isa<BaseMemRefType, PtrType>(type); }
    ...
    static bool isAIVectorScopeCarrier(scf::ForOp loop) { return loop && loop->hasAttr(kAIVectorScopeAttrName); }

    static bool isDedicatedVecScopeCarrier(Operation* op) { return isa_and_nonnull<VecScopeOp, StrictVecScopeOp>(op); }
    ...
    static std::optional<VPTOMaskGranularity> inferMaskGranularityFromType(Type type)
    {
    ...
    static std::optional<VPTOMaskGranularity> inferMaskGranularityFromFamily(Operation* op)
    {
    ...
    static VPTOBufferAddressFamily classifyBufferAddressFamily(Operation* op)
    {
        ...
        if (isa<CopyGmToUbufOp, CopyUbufToUbufOp, CopyUbufToGmOp, CopyCbufToUbufOp, CopyUbufToCbufOp>(op))
            return VPTOBufferAddressFamily::Copy;

        if (isa<VldasOp, VldusOp, PstuOp, VstusOp, VsturOp, MadOp, MadMxOp, CopyGmToCbufOp, LoadCbufToCaOp,
                LoadCbufToCbOp, CopyMatrixCcToGmOp>(op))
            return VPTOBufferAddressFamily::PtrOnly;

        if (isa<VldsOp, UvldOp, PldsOp, PldiOp, VstsOp, PstiOp, PstsOp, VbitsortOp, Vmrgsort4Op, Vgather2Op, VgatherbOp,
                Vgather2BcOp, VscatterOp, Vldsx2Op, Vstsx2Op, VsldbOp, VsstbOp, VstasOp, VstarOp>(op))
    ...
    {
        return classifyBufferAddressFamily(op) == VPTOBufferAddressFamily::BufferLike;
    }

    static bool isResidualEmissionScaffold(Operation* op)
    {
        return isa<memref::SubViewOp, memref::ReinterpretCastOp, memref::MemorySpaceCastOp>(op) ||
...
private:
    static bool isPTOp(Operation* op) { return op && op->getName().getStringRef().starts_with("pto."); }
    ...
    static bool isTrivialEmissionCastPtr(Operation* op)
    {
        ...
        return castOp && castOp.getInput().getType() == castOp.getResult().getType();
    }
...
public:
    VPTOLegalityValidator(ModuleOp module, VPTOLegalityStage stage, llvm::raw_ostream* diagOS)
    ...
    LogicalResult validate()
    {
        ...
        if (stage == VPTOLegalityStage::Emission && failed(validateEmissionRules()))
            return failure();
...
private:
    ...
    static std::string formatExpectedMaskType(VPTOMaskGranularity granularity)
    {
        ...
        os << "!pto.mask<" << VPTOLegalityHelper::stringifyMaskGranularity(granularity) << ">";
        return storage;
    }

    static LogicalResult validateMaskMatchesVectorFamily(
        Operation* op, Type maskType, StringRef maskRole, Type vectorType, StringRef vectorRole)
    {
        ...
        return op->emitOpError() << maskRole << " " << maskType << " does not match " << vectorRole << " " << vectorType
                                 << "; expected " << formatExpectedMaskType(*expected);
    }

    static std::optional<VPTOMaskGranularity> inferVstsMaskGranularityOverride(Operation* op)
    {
        Value value;
    ...
    static LogicalResult validateSameMaskGranularity(
        Operation* op, Type lhsType, StringRef lhsRole, Type rhsType, StringRef rhsRole)
    {
        ...
        return op->emitOpError() << lhsRole << " " << lhsType << " does not match " << rhsRole << " " << rhsType;
    }

    static bool isAdjacentMaskGranularityWidening(VPTOMaskGranularity input, VPTOMaskGranularity result)
    {
        return (input == VPTOMaskGranularity::B8 && result == VPTOMaskGranularity::B16) ||
               (input == VPTOMaskGranularity::B16 && result == VPTOMaskGranularity::B32);
    }

    static bool isAdjacentMaskGranularityNarrowing(VPTOMaskGranularity input, VPTOMaskGranularity result)
    {
        return (input == VPTOMaskGranularity::B16 && result == VPTOMaskGranularity::B8) ||
               (input == VPTOMaskGranularity::B32 && result == VPTOMaskGranularity::B16);
    }
    ...
    static LogicalResult validatePpackMaskGranularity(PpackOp op)
    {
        ...
        if (!input || !result || *input == *result || isAdjacentMaskGranularityNarrowing(*input, *result))
            return success();

        return op.emitOpError() << "input mask type " << op.getInput().getType() << " does not match result mask type "
                                << op.getResult().getType() << " for pto.ppack";
    }
    ...
    static LogicalResult validatePunpackMaskGranularity(PunpackOp op)
    {
        auto input = VPTOLegalityHelper::getMaskGranularity(op.getInput().getType());
        auto result = VPTOLegalityHelper::getMaskGranularity(op.getResult().getType());
        if (!input || !result || *input == *result || isAdjacentMaskGranularityWidening(*input, *result))
            return success();

        return op.emitOpError() << "input mask type " << op.getInput().getType() << " does not match result mask type "
                                << op.getResult().getType() << " for pto.punpack";
    }

    template <typename OpTy>
    static LogicalResult validateInputMaskVectorConsumer(OpTy op)
    {
        return validateMaskMatchesVectorFamily(
            op, op.getMask().getType(), "mask type", op.getInput().getType(), "input vector type");
    }
    ...
    {
        return validateMaskMatchesVectorFamily(
            op, op.getMask().getType(), "mask type", op.getLhs().getType(), "lhs vector type");
    }
    ...
    static LogicalResult validateValueMaskVectorConsumer(OpTy op)
    {
        if constexpr (std::is_same_v<OpTy, VstsOp>) {
            if (std::optional<VPTOMaskGranularity> expected = inferVstsMaskGranularityOverride(op.getOperation())) {
                auto actual = VPTOLegalityHelper::getMaskGranularity(op.getMask().getType());
                if (!actual || *actual == *expected)
                    return success();
                return op.emitOpError() << "mask type " << op.getMask().getType()
                                        << " does not match value vector type " << op.getValue().getType()
                                        << "; expected " << formatExpectedMaskType(*expected);
            }
        }
        return validateMaskMatchesVectorFamily(
            op, op.getMask().getType(), "mask type", op.getValue().getType(), "value vector type");
    }
    ...
    void emitHardwareSupportWarnings(Operation* op) const
    {
        auto emitForStore = [&](auto storeOp) {
            ...
            if (dist == "MRG4CHN_B8" || dist == "MRG2CHN_B8" || dist == "MRG2CHN_B16")
                writeDiagnostic((Twine("warning: ") + store->getName().getStringRef() + " dist " + dist +
    ...
    {
        return validateMaskMatchesVectorFamily(
            op, op.getMask().getType(), "mask type", op.getResult().getType(), "result vector type");
    }

    template <typename CarryOp>
    static LogicalResult validateCarryFamilyContract(CarryOp op)
    {
        if (failed(validateMaskMatchesVectorFamily(
                op, op.getMask().getType(), "mask type", op.getLhs().getType(), "lhs vector type")) ||
            failed(validateSameMaskGranularity(
                op, op.getMask().getType(), "mask type", op.getCarry().getType(), "carry type")))
            return failure();

        if constexpr (std::is_same_v<CarryOp, VaddcsOp> || std::is_same_v<CarryOp, VsubcsOp>) {
            if (failed(validateSameMaskGranularity(
                    op, op.getCarryIn().getType(), "carry_in type", op.getMask().getType(), "mask type")) ||
                failed(validateSameMaskGranularity(
                    op, op.getCarryIn().getType(), "carry_in type", op.getCarry().getType(), "carry type")))
                return failure();
        }
    ...
    {
        if (failed(validateMaskMatchesVectorFamily(
                op, op.getMask().getType(), "seed mask type", vecType, "input vector type")) ||
            failed(validateMaskMatchesVectorFamily(
                op, op.getResult().getType(), "result mask type", vecType, "input vector type")) ||
            failed(validateSameMaskGranularity(
                op, op.getMask().getType(), "seed mask type", op.getResult().getType(), "result mask type")))
            return failure();
        return success();
    }
    ...
    {
        return validateSameMaskGranularity(
            op, op.getInput().getType(), "input mask type", op.getResult().getType(), "result mask type");
    }

    static LogicalResult validateMaskOnlyPnotContract(PnotOp op)
    {
        if (failed(validateSameMaskGranularity(
                op, op.getInput().getType(), "input mask type", op.getMask().getType(), "mask type")) ||
            failed(validateSameMaskGranularity(
                op, op.getInput().getType(), "input mask type", op.getResult().getType(), "result mask type")))
            return failure();
        return success();
    ...
    {
        if (failed(validateSameMaskGranularity(
                op, op.getSrc0().getType(), "src0 mask type", op.getSrc1().getType(), "src1 mask type")) ||
            failed(validateSameMaskGranularity(
                op, op.getSrc0().getType(), "src0 mask type", op.getMask().getType(), "mask type")) ||
            failed(validateSameMaskGranularity(
                op, op.getSrc0().getType(), "src0 mask type", op.getResult().getType(), "result mask type")))
            return failure();
        return success();
    }

    template <typename PredicateMovementOp>
    static LogicalResult validatePredicateMovementContract(PredicateMovementOp op)
    {
        ...
        if (failed(validateSameMaskGranularity(
                op, op.getLhs().getType(), "lhs mask type", op.getRhs().getType(), "rhs mask type")) ||
            failed(validateSameMaskGranularity(
                op, op.getLhs().getType(), "lhs mask type", op.getLow().getType(), "low mask type")) ||
            failed(validateSameMaskGranularity(
                op, op.getLhs().getType(), "lhs mask type", op.getHigh().getType(), "high mask type")))
            return failure();
        ...
        return op.emitOpError() << "predicate movement family requires " << formatExpectedMaskType(*expected)
                                << " but got lhs mask type " << op.getLhs().getType();
    }

    static LogicalResult validateFamilySuffixMaskResult(Operation* op, Type resultType, StringRef resultRole)
    {
        auto expected = VPTOLegalityHelper::inferMaskGranularityFromFamily(op);
        ...
        return op->emitOpError() << "family suffix requires " << resultRole << " to be "
    ...
    static LogicalResult validateFamilySuffixMaskContracts(Operation* op)
    {
        return llvm::TypeSwitch<Operation*, LogicalResult>(op)
            .Case<PsetB8Op, PsetB16Op, PsetB32Op, PgeB8Op, PgeB16Op, PgeB32Op>([](auto concreteOp) {
                return validateFamilySuffixMaskResult(concreteOp, concreteOp.getResult().getType(), "result type");
            })
            .Case<PltB8Op, PltB16Op, PltB32Op>([](auto concreteOp) {
                return validateFamilySuffixMaskResult(concreteOp, concreteOp.getMask().getType(), "mask result type");
            })
    ...
    {
        return llvm::TypeSwitch<Operation*, LogicalResult>(op)
            .Case<VabsOp, VexpOp, VlnOp, VsqrtOp, VreluOp, VnotOp, VcaddOp, VcmaxOp, VcminOp>(
                [](auto concreteOp) { return validateInputMaskVectorConsumer(concreteOp); })
            .Case<VaddOp, VsubOp, VmulOp, VdivOp, VmaxOp, VminOp, VandOp, VorOp, VxorOp, VshlOp, VshrOp>(
                [](auto concreteOp) { return validateBinaryMaskVectorConsumer(concreteOp); })
            .Case<VaddcOp, VsubcOp, VaddcsOp, VsubcsOp>(
                [](auto concreteOp) { return validateCarryFamilyContract(concreteOp); })
            .Case<VcmpOp>([](VcmpOp concreteOp) {
                return validateCompareFamilyContract(concreteOp, concreteOp.getSrc0().getType());
            })
            .Case<VcmpsOp>([](VcmpsOp concreteOp) {
                return validateCompareFamilyContract(concreteOp, concreteOp.getSrc().getType());
            })
            .Case<PpackOp>([](PpackOp concreteOp) { return validatePpackMaskGranularity(concreteOp); })
            .Case<PunpackOp>([](PunpackOp concreteOp) { return validatePunpackMaskGranularity(concreteOp); })
            .Case<PnotOp>([](PnotOp concreteOp) { return validateMaskOnlyPnotContract(concreteOp); })
            .Case<PselOp>([](PselOp concreteOp) { return validateMaskOnlyPselContract(concreteOp); })
            .Case<PdintlvB8Op, PdintlvB16Op, PdintlvB32Op, PintlvB8Op, PintlvB16Op, PintlvB32Op>(
                [](auto concreteOp) { return validatePredicateMovementContract(concreteOp); })
            .Case<VselOp>([](VselOp concreteOp) {
                return validateMaskMatchesVectorFamily(
                    concreteOp, concreteOp.getMask().getType(), "mask type", concreteOp.getSrc0().getType(),
                    "src0 vector type");
            })
            .Case<Vgather2BcOp, VsldbOp>([](auto concreteOp) { return validateResultMaskVectorConsumer(concreteOp); })
            .Case<VstsOp, VsstbOp>([](auto concreteOp) { return validateValueMaskVectorConsumer(concreteOp); })
            .Case<Vstsx2Op>([](Vstsx2Op concreteOp) {
                return validateMaskMatchesVectorFamily(
                    concreteOp, concreteOp.getMask().getType(), "mask type", concreteOp.getLow().getType(),
                    "low vector type");
            })
            .Case<VmullOp, VmulaOp>([](auto concreteOp) {
                return validateMaskMatchesVectorFamily(
                    concreteOp, concreteOp.getMask().getType(), "mask type", concreteOp.getLhs().getType(),
                    "lhs vector type");
            })
            .Default([](Operation*) { return success(); });
    }
    ...
    LogicalResult validateAuthoringFunctionSurface()
    {
        for (func::FuncOp func : helper.getFunctions()) {
            auto validatePositiveI32FuncAttr = [&](StringRef attrName, int64_t upperBound,
                ...
                if (!intAttr || !intAttr.getType().isSignlessInteger(32))
                    return func.emitError() << "'" << attrName << "' must be a signless i32 integer attribute";

                if (intAttr.getInt() <= 0)
                    return func.emitError()
                           << "'" << attrName << "' must be a positive integer, got " << intAttr.getInt();

                if (intAttr.getInt() > upperBound)
                    return func.emitError()
                           << "'" << attrName << "' must be in range [1, " << upperBound << "] for " << description;

                if (!func->hasAttr(pto::kPTOSimtEntryAttrName))
                    return func.emitError() << "'" << attrName << "' is only allowed on functions marked '"
                                            << pto::kPTOSimtEntryAttrName << "'";

                return success();
            };

            if (failed(validatePositiveI32FuncAttr(pto::kPTOSimtMaxThreadsAttrName, 2048, "SIMT max threads")) ||
                failed(validatePositiveI32FuncAttr(pto::kPTOSimtMaxRegistersAttrName, 128, "SIMT max registers")))
                return failure();
            ...
            WalkResult walkResult = func.walk([&](StoreVfSimtInfoOp op) {
                op.emitOpError() << "must not appear inside a function marked with '" << pto::kPTOSimtEntryAttrName
        ...
        WalkResult keepResumeWalk = helper.getModule().walk([&](Operation* op) {
            ...
            if (!func || !func->hasAttr(pto::kPTOSimtEntryAttrName)) {
                op->emitOpError() << "must appear inside a function marked with '" << pto::kPTOSimtEntryAttrName << "'";
                return WalkResult::interrupt();
            }
            ...
            if (auto resume = dyn_cast<ResumeOp>(op)) {
                ...
                if (!first || !isa<ResumeOp>(first)) {
                    op->emitOpError() << "must be in the contiguous SIMT resume prologue group after "
            ...
            if (auto keep = dyn_cast<KeepOp>(op)) {
                ...
                if (!terminator || !isa<func::ReturnOp>(terminator)) {
                    op->emitOpError() << "must be placed in the SIMT epilogue before func.return";
                    return WalkResult::interrupt();
                }
                ...
                if (!isOpInRange(op, firstKeep, lastKeep)) {
                    op->emitOpError() << "must be in the contiguous SIMT keep epilogue group "
    ...
    {
        WalkResult constantWalkResult = helper.getModule().walk([&](arith::ConstantOp constant) {
            ...
            constant.emitOpError() << "does not support directly constructed FP8 constants in "
        ...
        WalkResult loopWalkResult = helper.getModule().walk([&](scf::ForOp loop) {
            ...
            Operation* parentScope = VPTOLegalityHelper::getEnclosingVectorScopeCarrier(loop);
            if (!parentScope)
                return WalkResult::advance();

            if (isa<scf::ForOp>(parentScope)) {
                loop.emitOpError() << "does not allow nested scf.for with '" << kAIVectorScopeAttrName << "'";
                return WalkResult::interrupt();
            }

            loop.emitOpError() << "does not allow legacy scf.for carrier nested inside dedicated "
        ...
        WalkResult vecScopeWalkResult = helper.getModule().walk([&](Operation* op) {
            ...
            op->emitOpError() << "does not allow nested dedicated pto.vecscope/pto.strict_vecscope";
            return WalkResult::interrupt();
        });
        ...
        WalkResult opWalkResult = helper.getModule().walk([&](Operation* op) {
            ...
            if (VPTOLegalityHelper::getEnclosingVectorScopeCarrier(op)) {
                if (failed(validateFamilySuffixMaskContracts(op)) || failed(validateUnaryElementTypeContracts(op)) ||
            ...
            op->emitOpError() << "requires enclosing scf.for with '" << kAIVectorScopeAttrName
        ...
        for (func::FuncOp func : helper.getFunctions()) {
            ...
            for (auto [idx, inputType] : llvm::enumerate(functionType.getInputs())) {
                if (!isa<BaseMemRefType>(inputType))
                    continue;
                return func.emitError() << "emission-stage VPTO legality rejects memref argument #" << idx << ": "
                                        << inputType;
            }
            ...
            for (auto [idx, resultType] : llvm::enumerate(functionType.getResults())) {
                if (!isa<BaseMemRefType>(resultType))
                    continue;
                return func.emitError() << "emission-stage VPTO legality rejects memref result #" << idx << ": "
                                        << resultType;
            }
        }
        return success();
    ...
    LogicalResult validateEmissionOperationSurface()
    {
        WalkResult walkResult = helper.getModule().walk([&](Operation* op) {
            if (isa<BuildAsyncSessionOp, TPutAsyncOp, TGetAsyncOp, WaitAsyncEventOp, TestAsyncEventOp>(op)) {
                op->emitOpError() << "is not supported by the VPTO backend; async SDMA session "
                                     "operations currently require the EmitC backend";
                return WalkResult::interrupt();
            }

            VPTOBufferAddressFamily family = VPTOLegalityHelper::classifyBufferAddressFamily(op);
            ...
            if (family == VPTOBufferAddressFamily::BufferLike) {
                for (OpOperand* operand : VPTOLegalityHelper::collectBufferOperands(op)) {
                    ...
                    op->emitOpError() << "emission-stage VPTO legality rejects memref-form buffer "
                                         "operand #"
            ...
            if (VPTOLegalityHelper::isResidualEmissionScaffold(op)) {
                op->emitOpError() << "must be eliminated before emission-stage VPTO validation";
                return WalkResult::interrupt();
            }
...
struct PTOValidateVPTOIRPass : public PassWrapper<PTOValidateVPTOIRPass, OperationPass<ModuleOp>> {
...
struct PTOValidateVPTOEmissionIRPass : public PassWrapper<PTOValidateVPTOEmissionIRPass, OperationPass<ModuleOp>> {
    MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(PTOValidateVPTOEmissionIRPass)

    StringRef getArgument() const final { return "pto-validate-vpto-emission-ir"; }
...
LogicalResult validateVPTOAuthoringIR(ModuleOp module, llvm::raw_ostream* diagOS)
{
    return detail::VPTOLegalityValidator(module, detail::VPTOLegalityStage::Authoring, diagOS).validate();
}

LogicalResult validateVPTOEmissionIR(ModuleOp module, llvm::raw_ostream* diagOS)
{
    return detail::VPTOLegalityValidator(module, detail::VPTOLegalityStage::Emission, diagOS).validate();
}

std::unique_ptr<Pass> createPTOValidateVPTOIRPass() { return std::make_unique<PTOValidateVPTOIRPass>(); }
