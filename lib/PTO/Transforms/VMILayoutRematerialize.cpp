...
static bool hasConcreteLayout(VMIVRegType type) { return type && static_cast<bool>(type.getLayoutAttr()); }

static bool hasConcreteLayout(VMIMaskType type) { return type && static_cast<bool>(type.getLayoutAttr()); }

static Value materializeDataLayout(Value value, VMIVRegType resultType, Location loc, OpBuilder& builder)
{
...
static std::optional<Value> rematerializeWidenExt(ExtOp op, VMIVRegType resultType, Location loc, OpBuilder& builder)
{
    ...
    FailureOr<VMILayoutAttr> sourceLayout =
        supports.getWidenSourceLayoutForResultLayout(sourceType, resultType, resultType.getLayoutAttr());
    if (failed(sourceLayout))
        return std::nullopt;

    auto rematSourceType = VMIVRegType::get(
        sourceType.getContext(), sourceType.getElementCount(), sourceType.getElementType(), *sourceLayout);
    if (sourceType != rematSourceType) {
        ...
        FailureOr<int64_t> rematSourceArity = getVMIPhysicalArity(rematSourceType);
        if (failed(sourceArity) || failed(rematSourceArity) || *rematSourceArity > *sourceArity)
            return std::nullopt;
    }
    Value rematSource = materializeDataLayout(op.getSource(), rematSourceType, loc, builder);
    return builder.create<ExtOp>(loc, resultType, rematSource).getResult();
}

static std::optional<Value> rematerializeBinaryDataOp(
    Operation* op, VMIVRegType resultType, Location loc, OpBuilder& builder)
{
    auto rebuild = [&](auto typedOp) -> std::optional<Value> {
        ...
        if (!lhsType || !rhsType)
            return std::nullopt;
        auto lhsResultType = VMIVRegType::get(
            lhsType.getContext(), lhsType.getElementCount(), lhsType.getElementType(), resultType.getLayoutAttr());
        auto rhsResultType = VMIVRegType::get(
            rhsType.getContext(), rhsType.getElementCount(), rhsType.getElementType(), resultType.getLayoutAttr());
        Value lhs = materializeDataLayout(typedOp.getLhs(), lhsResultType, loc, builder);
        Value rhs = materializeDataLayout(typedOp.getRhs(), rhsResultType, loc, builder);
        return builder.create<std::decay_t<decltype(typedOp)>>(loc, resultType, lhs, rhs).getResult();
    };
...
static std::optional<Value> rematerializeUnaryDataOp(
    Operation* op, VMIVRegType resultType, Location loc, OpBuilder& builder)
{
    auto rebuild = [&](auto typedOp) -> std::optional<Value> {
        ...
        auto sourceResultType = VMIVRegType::get(
            sourceType.getContext(), sourceType.getElementCount(), sourceType.getElementType(),
            resultType.getLayoutAttr());
        Value source = materializeDataLayout(typedOp.getSource(), sourceResultType, loc, builder);
        return builder.create<std::decay_t<decltype(typedOp)>>(loc, resultType, source).getResult();
    };
...
static std::optional<Value> rematerializeFma(VMIFmaOp fma, VMIVRegType resultType, Location loc, OpBuilder& builder)
{
    ...
    auto makeType = [&](VMIVRegType type) {
        return VMIVRegType::get(
            type.getContext(), type.getElementCount(), type.getElementType(), resultType.getLayoutAttr());
    };
    Value lhs = materializeDataLayout(fma.getLhs(), makeType(lhsType), loc, builder);
    Value rhs = materializeDataLayout(fma.getRhs(), makeType(rhsType), loc, builder);
    Value acc = materializeDataLayout(fma.getAcc(), makeType(accType), loc, builder);
    return builder.create<VMIFmaOp>(loc, resultType, lhs, rhs, acc).getResult();
}

static std::optional<Value> rematerializeDataProducer(
    Value value, VMIVRegType resultType, Location loc, OpBuilder& builder)
{
    ...
    if (auto constant = value.getDefiningOp<VMIConstantOp>()) {
        ...
        if (denseAttr && denseAttr.isSplat())
            return builder.create<VMIConstantOp>(loc, resultType, constant.getValue()).getResult();
    }

    if (auto broadcast = value.getDefiningOp<VMIBroadcastOp>())
        return builder.create<VMIBroadcastOp>(loc, resultType, broadcast.getValue()).getResult();

    if (auto iota = value.getDefiningOp<VMIIotaOp>()) {
        return builder.create<VMIIotaOp>(loc, resultType, iota.getBase(), iota.getOrderAttr()).getResult();
    }
    ...
    if (auto groupIota = value.getDefiningOp<VMIGroupIotaOp>()) {
        ...
        if (!resultLayout || !resultLayout.isContiguous())
            return std::nullopt;
        return builder
            .create<VMIGroupIotaOp>(
                loc, resultType, groupIota.getBase(), groupIota.getOrderAttr(), groupIota.getGroupAttr())
            .getResult();
    }
...
static std::optional<Value> rematerializeMaskProducer(
    Value value, VMIMaskType resultType, Location loc, OpBuilder& builder)
{
    if (!hasConcreteLayout(resultType))
        return std::nullopt;
...
static bool tryReplaceDataEnsure(VMIEnsureLayoutOp ensure)
{
    ...
    auto result = rematerializeDataProducer(ensure.getSource(), resultType, ensure->getLoc(), builder);
    if (!result)
        return false;
...
static bool tryRematerializeTruncIThroughSourceEnsure(VMITruncIOp trunc)
{
    ...
    FailureOr<VMICastLayoutFact> fact =
        supports.getCastLayoutFactForSourceLayout(originalSourceType, resultType, originalSourceLayout);
    if (failed(fact))
        return false;

    unsigned resultBits = pto::getPTOStorageElemBitWidth(resultType.getElementType());
    if (resultBits == 8 && !cast<IntegerType>(resultType.getElementType()).isUnsigned())
        return false;

    VMILayoutAttr rematResultLayout = fact->resultLayout;
    auto rematResultType = VMIVRegType::get(
        resultType.getContext(), resultType.getElementCount(), resultType.getElementType(), rematResultLayout);
    if (rematResultType == resultType)
        return false;

    OpBuilder builder(trunc);
    Value remat =
        builder.create<VMITruncIOp>(trunc->getLoc(), rematResultType, ensure.getSource(), trunc.getSaturateAttr())
            .getResult();
    Value replacement = materializeDataLayout(remat, resultType, trunc->getLoc(), builder);
...
static bool tryReplaceMaskEnsure(EnsureOp ensure)
{
    ...
    OpBuilder builder(ensure);
    auto result = rematerializeMaskProducer(ensure.getSource(), resultType, ensure->getLoc(), builder);
    if (!result)
        return false;
...
struct VMILayoutRematerializePass : public mlir::pto::impl::VMILayoutRematerializeBase<VMILayoutRematerializePass> {
    ...
    void runOnOperation() override
    {
        ...
        while (changed) {
            ...
            module.walk([&](Operation* op) {
                if (isa<VMIEnsureLayoutOp, VMIEnsureMaskLayoutOp, VMIEnsureMaskGranularityOp, VMITruncIOp>(op))
