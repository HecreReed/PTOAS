...
namespace mlir::pto {
LogicalResult runModernPlanMemory(func::FuncOp func, llvm::StringRef memMode, bool orderBySize);
} // namespace mlir::pto
...
static std::optional<AddressSpace> getBufferAddressSpace(Type type)
{
    if (auto tileType = dyn_cast<TileBufType>(type)) {
        if (auto attr = dyn_cast_or_null<AddressSpaceAttr>(tileType.getMemorySpace()))
...
{
    return space == AddressSpace::MAT || space == AddressSpace::LEFT || space == AddressSpace::RIGHT ||
           space == AddressSpace::ACC;
}

static bool isNameIn(StringRef name, ArrayRef<StringRef> names) { return llvm::is_contained(names, name); }
...
static bool isIgnoredA5TmpOperandUse(OpOperand& use)
{
    ...
    if (isNameIn(
            name, {"pto.trowargmax", "pto.trowargmin", "pto.trowmax", "pto.trowmin", "pto.trowsum", "pto.trowprod"}))
        return operandNo == 1;
    ...
    if (isNameIn(
            name,
            {"pto.tprelu", "pto.txor", "pto.tsels", "pto.trowexpand", "pto.tcolexpand", "pto.trowexpandadd",
             "pto.trowexpanddiv", "pto.trowexpandexpdif", "pto.trowexpandmax", "pto.trowexpandmin", "pto.trowexpandmul",
             "pto.trowexpandsub", "pto.tcolexpandadd", "pto.tcolexpanddiv", "pto.tcolexpandexpdif", "pto.tcolexpandmax",
             "pto.tcolexpandmin", "pto.tcolexpandmul", "pto.tcolexpandsub"}))
        return operandNo == 2;
...
static FailureOr<uint64_t> computeStaticBufferBytes(Value value)
{
    auto computeTileBytes = [](TileBufType type) -> FailureOr<uint64_t> {
        ...
        if (type.getCompactModeI32() == static_cast<int32_t>(CompactMode::RowPlusOne)) {
            if (shape.size() != 2 || llvm::is_contained(shape, ShapedType::kDynamic))
                return failure();

            bool rowMajor = type.getBLayoutValueI32() == static_cast<int32_t>(BLayout::RowMajor);
...
static bool isOneOf(StringRef name, ArrayRef<StringRef> names) { return llvm::is_contained(names, name); }
...
static InplacePolicy getInplacePolicy(Operation* op)
{
    ...
    policy.notInplaceSafe = isOneOf(
        name, {
                  "pto.tands",      "pto.tfillpad_expand", "pto.tfmod",      "pto.tfmods",
                  "pto.tgather",    "pto.tmrgsort",        "pto.tors",       "pto.trecip",
                  "pto.trsqrt",     "pto.tsort32",         "pto.ttrans",     "pto.trowargmax",
                  "pto.trowargmin", "pto.trowmax",         "pto.trowmin",    "pto.trowprod",
                  "pto.trowsum",    "pto.tcolargmax",      "pto.tcolargmin", "pto.tcvt",
                  "pto.txors",
              });
...
static SmallVector<Value> getWrittenNonDpsOperands(Operation* op, ValueRange dpsInits)
{
...
{
    return op->getParentOfType<scf::ForOp>() != nullptr || op->getParentOfType<scf::WhileOp>() != nullptr;
}
...
struct PlannerAnalysis {
    ...
    {
        facts.targetHazardEnabled = getTargetArch(func.getOperation()) == PTOArch::A3;
    }
    ...
    void addRoot(Value value, Operation* defOp)
    {
        ...
        else if (auto attr = defOp->getAttrOfType<IntegerAttr>(pto::kPtoMultiBufferAttrName))
    ...
    void addForbidAliasBetweenRoots(const RootList& lhsRoots, const RootList& rhsRoots)
    {
    ...
    bool isSplitTpopDerived(Value value) const { return splitTpopDerivedValues.contains(value); }

    bool operandsContainSplitTpopDerived(Operation* op) const
    {
        return llvm::any_of(op->getOperands(), [&](Value operand) { return isSplitTpopDerived(operand); });
    }

    bool operandsContainLoadDerivedRoot(Operation* op) const
    {
        return llvm::any_of(
            op->getOperands(), [&](Value operand) { return rootsContain(facts.loadDerivedRoots, getRoots(operand)); });
    }
    ...
    void propagateSplitTpopDerived(Value result, ValueRange sources)
    {
        if (!result)
            return;
        if (llvm::any_of(sources, [&](Value source) { return isSplitTpopDerived(source); }))
    ...
    void recordIfBranchExclusivity(scf::IfOp ifOp)
    {
        ...
        for (auto [thenVal, elseVal] : llvm::zip(thenYield.getResults(), elseYield.getResults())) {
    ...
    void recordTpopConsumerRoots(Operation* op, ValueRange dpsInits, unsigned opIndex)
    {
    ...
    void recordRootAccessStats(ArrayRef<Value> accessRoots, PIPE pipe, bool isWrite, bool inLoop)
    {
    ...
    void recordOpAccess(Operation* op, ValueRange dpsInits)
    {
        ...
        if (!access.reads.empty() || !access.writes.empty()) {
            recordRootAccessStats(access.reads, access.pipe, /*isWrite=*/false, access.inLoop);
            recordRootAccessStats(access.writes, access.pipe, /*isWrite=*/true, access.inLoop);
    ...
    void seedForIterArgAliases(scf::ForOp forOp)
    {
        ...
        for (auto [iterArg, initArg] : llvm::zip(body.getArguments().drop_front(1), forOp.getInitArgs())) {
    ...
    void seedRegionBranchEntryAliases(RegionBranchOpInterface branchOp)
    {
        ...
        for (RegionSuccessor successor : successors) {
            if (successor.isParent())
                continue;
            mapRegionBranchValues(branchOp.getEntrySuccessorOperands(successor), successor.getSuccessorInputs());
        }
    }

    void finalizeRegionBranchAliasesFromRegion(RegionBranchOpInterface branchOp, Region& region)
    {
        if (region.empty())
            return;
        ...
        for (RegionSuccessor successor : successors) {
            ...
            for (Block& block : region) {
                auto terminator = dyn_cast<RegionBranchTerminatorOpInterface>(block.getTerminator());
                if (!terminator)
                    continue;
                mapRegionBranchValues(terminator.getSuccessorOperands(successor), destinations);
            }
        }
    }
    ...
    void finalizeForLoopLiveness(scf::ForOp forOp, unsigned loopStartIndex, unsigned loopEndIndex)
    {
        ...
        for (auto [iterArg, result, initArg, yielded] :
             llvm::zip(forOp.getRegionIterArgs(), forOp.getResults(), forOp.getInitArgs(), yieldOp.getResults())) {
            RootList loopCarriedRoots = unionRoots(getRoots(initArg), getRoots(yielded));
    ...
    void finalizeWhileLoopLiveness(scf::WhileOp whileOp, unsigned loopStartIndex, unsigned loopEndIndex)
    {
        ...
        for (Block& block : region) {
            for (Operation& opRef : block) {
                ...
                if (auto multiGet = dyn_cast<pto::MultiTileGetOp>(op)) {
                    ...
                    propagateSplitTpopDerived(multiGet.getResult(), ValueRange{multiGet.getSource()});
                } else if (auto select = dyn_cast<arith::SelectOp>(op)) {
                    setRoots(
                        select.getResult(),
                        unionRoots(getRoots(select.getTrueValue()), getRoots(select.getFalseValue())));
                    propagateSplitTpopDerived(
                        select.getResult(), ValueRange{select.getTrueValue(), select.getFalseValue()});
                } else if (auto subview = dyn_cast<pto::SubViewOp>(op)) {
                    setRoots(subview.getResult(), getRoots(subview.getSource()));
                    propagateSplitTpopDerived(subview.getResult(), ValueRange{subview.getSource()});
                } else if (auto bitcast = dyn_cast<pto::BitcastOp>(op)) {
                    setRoots(bitcast.getResult(), getRoots(bitcast.getSrc()));
                    propagateSplitTpopDerived(bitcast.getResult(), ValueRange{bitcast.getSrc()});
                } else if (auto reshape = dyn_cast<pto::TReshapeOp>(op)) {
                    setRoots(reshape.getResult(), getRoots(reshape.getSrc()));
                    propagateSplitTpopDerived(reshape.getResult(), ValueRange{reshape.getSrc()});
                ...
                if (auto ifOp = dyn_cast<scf::IfOp>(op)) {
                    ...
                    unsigned loopEndIndex = linearOps.empty() ? index : linearOps.size() - 1;
                    ...
                    auto yieldOp = cast<pto::YieldOp>(fusionRegion.getBody().front().getTerminator());
                    for (auto [result, yielded] : llvm::zip(fusionRegion.getResults(), yieldOp.getOperands()))
...
{
    if (facts.loopCarriedRoots.contains(lhs) || facts.loopCarriedRoots.contains(rhs))
        return false;
...
static bool gateLifetimeAndPhi(const RootInfo& lhs, const RootInfo& rhs, const ConflictFacts& facts)
{
...
static bool hasTargetHazard(const RootInfo& input, const RootInfo& writer, const ConflictFacts& facts)
{
    if (!facts.targetHazardEnabled)
        return false;

    if (!facts.loadDerivedRoots.contains(input.root) || !facts.tpopConsumerRoots.contains(writer.root))
        return false;
...
static bool gateTargetHazard(const RootInfo& lhs, const RootInfo& rhs, const ConflictFacts& facts)
{
    return !hasTargetHazard(lhs, rhs, facts) && !hasTargetHazard(rhs, lhs, facts);
}

static bool gateSemanticNoAlias(const RootInfo& lhs, const RootInfo& rhs, const PlannerAnalysis& analysis)
{
    return !analysis.hasForbidAlias(lhs.root, rhs.root);
}

static bool canShare(const RootInfo& lhs, const RootInfo& rhs, const PlannerAnalysis& analysis)
{
    const ConflictFacts& facts = analysis.facts;
    return gateLifetimeAndPhi(lhs, rhs, facts) && gateTargetHazard(lhs, rhs, facts) &&
           gateSemanticNoAlias(lhs, rhs, analysis);
}

static bool canJoinReuseGroup(const RootInfo& info, const ReuseGroup& group, const PlannerAnalysis& analysis)
{
...
static bool containsRoot(ArrayRef<Value> roots, Value root) { return llvm::is_contained(roots, root); }
...
static bool accessPairHasMemoryDependency(const OpAccess& lhs, const OpAccess& rhs, Value lhsRoot, Value rhsRoot)
{
    auto depends = [&](Value writtenRoot, Value otherRoot) {
        return containsRoot(lhs.writes, writtenRoot) &&
               (containsRoot(rhs.reads, otherRoot) || containsRoot(rhs.writes, otherRoot));
    };
    auto reverseDepends = [&](Value readRoot, Value writtenRoot) {
        return containsRoot(lhs.reads, readRoot) && containsRoot(rhs.writes, writtenRoot);
    };

    return depends(lhsRoot, rhsRoot) || depends(rhsRoot, lhsRoot) || reverseDepends(lhsRoot, rhsRoot) ||
           reverseDepends(rhsRoot, lhsRoot);
}

static bool isNearNeighbor(const OpAccess& lhs, const OpAccess& rhs, unsigned lookahead)
{
    unsigned lhsIndex = lhs.opIndex;
    unsigned rhsIndex = rhs.opIndex;
    unsigned distance = lhsIndex > rhsIndex ? lhsIndex - rhsIndex : rhsIndex - lhsIndex;
    return distance > 0 && distance <= lookahead;
}

static uint64_t loopWeightedPenalty(uint64_t penalty, const OpAccess& lhs, const OpAccess& rhs)
{
    constexpr uint64_t kLoopWeight = 4;
    return (lhs.inLoop || rhs.inLoop) ? penalty * kLoopWeight : penalty;
}

static uint64_t getRootPairReuseCost(Value lhsRoot, Value rhsRoot, const ConflictFacts& facts)
{
    if (lhsRoot == rhsRoot)
        return 0;
    ...
    for (const OpAccess& lhs : facts.opAccesses) {
        ...
        for (const OpAccess& rhs : facts.opAccesses) {
            ...
            if (lhs.pipe == PIPE::PIPE_V && rhs.pipe == PIPE::PIPE_V && isNearNeighbor(lhs, rhs, kPipeVLookahead) &&
                accessPairHasMemoryDependency(lhs, rhs, lhsRoot, rhsRoot)) {
                cost += loopWeightedPenalty(kPipeVOverlapPenalty, lhs, rhs);
            }

            if (lhs.pipe == PIPE::PIPE_MTE3 && rhs.pipe == PIPE::PIPE_MTE2 && isNearNeighbor(lhs, rhs, kMteLookahead)) {
                bool storeSourceThenLoadDst = (containsRoot(lhs.reads, lhsRoot) && containsRoot(rhs.writes, rhsRoot)) ||
                                              (containsRoot(lhs.reads, rhsRoot) && containsRoot(rhs.writes, lhsRoot));
...
static uint64_t getGroupReuseCost(const RootInfo& info, const ReuseGroup& group, const ConflictFacts& facts)
{
    uint64_t cost = 0;
    for (const RootInfo* member : group.members)
...
{
    if (info.loopAccessCount > 0 && (info.accessCount >= 2 || info.pipeVAccessCount > 0 || info.mteAccessCount > 0))
        return true;
    ...
    return info.accessCount >= 2 && (info.pipeVAccessCount > 0 || info.mteAccessCount > 0);
}
...
static uint64_t getHotClusterReuseCost(const RootInfo& info, const ReuseGroup& group)
{
    if (!isHotRoot(info))
        return 0;
    ...
    for (const RootInfo* member : group.members) {
        ...
        uint64_t pairHotness = std::min<uint64_t>(infoHotness, getRootHotness(*member));
        ...
        if (info.alignmentBytes <= kBankConflictModuloBytes && member->alignmentBytes <= kBankConflictModuloBytes)
            cost += kBankConflictPenalty;
    }
    return cost;
}

static uint64_t getProjectedPackedBytes(
    ArrayRef<ReuseGroup> groups, const RootInfo& info, std::optional<unsigned> joinGroup)
{
    uint64_t cursor = 0;
...
static ReuseGroup* chooseReuseGroupByCost(
    RootInfo& info, SmallVectorImpl<ReuseGroup>& groups, const PlannerAnalysis& analysis, const MemSpec& spec)
{
    ...
    for (auto [index, group] : llvm::enumerate(groups)) {
        ...
        uint64_t projectedBytes = getProjectedPackedBytes(groups, info, static_cast<unsigned>(index));
        bool fits = projectedBytes <= spec.capacityBytes;
        uint64_t cost = getGroupReuseCost(info, group, analysis.facts) + getHotClusterReuseCost(info, group);
        if ((!bestGroup && !bestFits) || (fits != bestFits && fits) ||
            (fits == bestFits &&
             std::tie(cost, projectedBytes, index) < std::tie(bestCost, bestProjectedBytes, bestOrder))) {
            bestGroup = &group;
            bestCost = cost;
    ...
    uint64_t freshProjectedBytes = getProjectedPackedBytes(groups, info, std::nullopt);
    bool freshFits = freshProjectedBytes <= spec.capacityBytes;
    uint64_t freshSlack = freshFits ? spec.capacityBytes - freshProjectedBytes : 0;
    uint64_t pressureReserve = std::max(info.totalBytes, info.alignmentBytes);
    ...
    auto isBetter = [](bool lhsFits, uint64_t lhsCost, uint64_t lhsBytes, unsigned lhsOrder, bool rhsFits,
                       uint64_t rhsCost, uint64_t rhsBytes, unsigned rhsOrder) {
        if (lhsFits != rhsFits)
            return lhsFits;
        return std::tie(lhsCost, lhsBytes, lhsOrder) < std::tie(rhsCost, rhsBytes, rhsOrder);
    };

    if (isBetter(
            freshFits, freshCost, freshProjectedBytes, freshOrder, bestFits, bestCost, bestProjectedBytes, bestOrder))
        return nullptr;
    return bestGroup;
}

static SmallVector<uint64_t> buildSlotOffsets(uint64_t base, uint64_t slotBytes, uint64_t slotCount)
{
    SmallVector<uint64_t> offsets;
...
static FailureOr<uint64_t> planReserveBufferBase(
    pto::ReserveBufferOp reserveOp, const MemSpec& spec, SmallVectorImpl<std::pair<uint64_t, uint64_t>>& occupied)
{
    uint64_t sizeBytes = alignUp(static_cast<uint64_t>(reserveOp.getSize()), spec.alignmentBytes);
...
static LogicalResult validateManualReserveBufferBase(pto::ReserveBufferOp reserveOp, const MemSpec& spec)
{
    ...
    if (signedBase < 0)
        return reserveOp.emitError("expects 'base' to be non-negative when present");
    ...
    if (base % spec.alignmentBytes != 0) {
        return reserveOp.emitError("expects 'base' to be aligned to ")
               << spec.alignmentBytes << " bytes for " << stringifyEnum(reserveOp.getLocation().getAddressSpace());
    }
    ...
    if (base > spec.capacityBytes || size > spec.capacityBytes - base) {
        return reserveOp.emitError("reserved range exceeds ")
               << stringifyEnum(reserveOp.getLocation().getAddressSpace()) << " capacity: base " << base << " + size "
               << size << " > " << spec.capacityBytes << " bytes";
    }

    return success();
}

static LogicalResult validateReserveBufferForPlanMemory(pto::ReserveBufferOp reserveOp, PTOArch arch)
{
    MemSpec spec = getMemSpec(arch, reserveOp.getLocation().getAddressSpace());

    if (reserveOp.getAutoAlloc()) {
        if (reserveOp.getBaseAttr()) {
            return reserveOp.emitError("unexpected pre-populated 'base' on auto reserve_buffer before "
    ...
    return reserveOp.emitError("pto.reserve_buffer with explicit 'base' (auto = false) is not "
                               "supported in PlanMemory; use --pto-level=level3 or set auto = true");
}

class AllocTileOpAddPlannedAddressPattern : public OpRewritePattern<pto::AllocTileOp> {
public:
    explicit AllocTileOpAddPlannedAddressPattern(
        MLIRContext* context, DenseMap<Value, SmallVector<uint64_t>> buffer2Offsets)
        : OpRewritePattern<pto::AllocTileOp>(context), buffer2Offsets(std::move(buffer2Offsets))
    {}

    LogicalResult matchAndRewrite(pto::AllocTileOp op, PatternRewriter& rewriter) const override
    {
        if (op.getAddr())
            return failure();
        ...
        if (it->second.size() != 1) {
            return rewriter.notifyMatchFailure(op, "single alloc_tile root expects exactly one planned address");
        }

        Value addr = rewriter.create<arith::ConstantIntOp>(op.getLoc(), it->second.front(), 64);
        auto planned = rewriter.create<pto::AllocTileOp>(
            op.getLoc(), tileType, addr, op.getValidRow() ? op.getValidRow() : Value(),
...
class AllocMultiTileOpAddPlannedAddressesPattern : public OpRewritePattern<pto::AllocMultiTileOp> {
public:
    explicit AllocMultiTileOpAddPlannedAddressesPattern(
        MLIRContext* context, DenseMap<Value, SmallVector<uint64_t>> buffer2Offsets)
        : OpRewritePattern<pto::AllocMultiTileOp>(context), buffer2Offsets(std::move(buffer2Offsets))
    {}

    LogicalResult matchAndRewrite(pto::AllocMultiTileOp op, PatternRewriter& rewriter) const override
    {
        ...
        if (it->second.size() != op.getResult().getType().getCount()) {
            return rewriter.notifyMatchFailure(op, "planned address count does not match multi_tile_buf count");
        }
        ...
        rewriter.modifyOpInPlace(
            op, [&] { op->setAttr(pto::kPtoMultiBufferAddrsAttrName, rewriter.getDenseI64ArrayAttr(addrs)); });
        return success();
    }
...
static LogicalResult materializePlannedOffsets(func::FuncOp func, DenseMap<Value, SmallVector<uint64_t>> buffer2Offsets)
{
    RewritePatternSet patterns(func.getContext());
    patterns.add<AllocTileOpAddPlannedAddressPattern>(patterns.getContext(), buffer2Offsets);
    patterns.add<AllocMultiTileOpAddPlannedAddressesPattern>(patterns.getContext(), buffer2Offsets);
...
LogicalResult mlir::pto::runModernPlanMemory(func::FuncOp func, llvm::StringRef memMode, bool orderBySize)
{
    if (!memMode.equals_insensitive("local")) {
        func.emitError("unsupported mem-mode '") << memMode << "'; only 'local' is currently implemented";
        return failure();
    }
    ...
    for (auto& entry : rootsBySpace) {
        ...
        for (RootInfo* info : roots) {
            ...
            chosen->alignmentBytes = std::max(chosen->alignmentBytes, info->alignmentBytes);
        }
        ...
        for (ReuseGroup& group : groups) {
            ...
            for (RootInfo* member : group.members) {
                member->offsets = buildSlotOffsets(group.offsetBytes, member->slotBytes, member->slotCount);
            }
            scopeRequiredBytes = std::max(scopeRequiredBytes, group.offsetBytes + group.sizeBytes);
            cursor = group.offsetBytes + group.sizeBytes;
        }

        if (scopeRequiredBytes > spec.capacityBytes) {
            func.emitError() << stringifyEnum(space) << " overflow, requires " << (scopeRequiredBytes * 8)
                             << " bits while " << (spec.capacityBytes * 8) << " bits available";
            return failure();
        }
    ...
    DenseMap<AddressSpace, SmallVector<std::pair<uint64_t, uint64_t>>> occupiedBySpace;
    for (const RootInfo& info : analysis.roots) {
        if (info.offsets.empty())
            continue;
        occupiedBySpace[info.space].push_back({info.offsets.front(), info.offsets.front() + info.totalBytes});
    }
    ...
    func.walk([&](pto::ReserveBufferOp reserveOp) -> WalkResult {
        ...
        auto baseOr = planReserveBufferBase(reserveOp, spec, occupiedBySpace[space]);
        ...
        reserveOp->setAttr("base", IntegerAttr::get(IntegerType::get(ctx, 32), static_cast<int32_t>(*baseOr)));
        return WalkResult::advance();
    });

    if (analysis.failed || mlir::failed(materializePlannedOffsets(func, buffer2Offsets))) {
        return failure();
    }
...
namespace {
struct PlanMemoryModernPass : public PassWrapper<PlanMemoryModernPass, OperationPass<ModuleOp>> {
    ...
    StringRef getDescription() const final { return "Plan local memory using the modern PTO planner"; }
    ...
    void runOnOperation() override
    {
        ...
        for (func::FuncOp funcOp : funcs) {
            if (failed(mlir::pto::runModernPlanMemory(funcOp, memMode, orderBySize))) {
                signalPassFailure();
                return;
            }
        }
...
std::unique_ptr<Pass> mlir::pto::createPlanMemoryModernPass(const PlanMemoryOptions& options)
{
