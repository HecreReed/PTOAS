...
static llvm::cl::opt<bool> preferLaneStrideNarrowing(
    ...
    llvm::cl::desc("Prefer continuous-to-lane-stride layouts for 2x/4x VMI narrowing"), llvm::cl::init(true));
...
static constexpr PhysicalChunkCountPattern chunk()
{
    ...
    static_assert(((Counts == 1 || Counts == 2 || Counts == 4) && ...), "unsupported physical chunk count");
    return {{Counts...}, static_cast<int64_t>(sizeof...(Counts))};
}

static constexpr uint64_t elementBitsMask(int64_t bits)
{
    return bits == 8 ? 1ull << 0 : bits == 16 ? 1ull << 1 : bits == 32 ? 1ull << 2 : bits == 64 ? 1ull << 3 : 0;
}

template <int64_t... Bits>
static constexpr ElementBitsPattern bits()
{
    return {((uint64_t{0} | elementBitsMask(Bits)) | ...)};
}

template <int64_t... Counts>
static constexpr ElementCountPattern N()
{
...
template <int64_t... Counts>
static constexpr ElementCountPattern G()
{
    return N<Counts...>();
}
...
static bool matchesElementBitsPattern(ElementBitsPattern pattern, int64_t bits)
{
    uint64_t mask = elementBitsMask(bits);
    return mask != 0 && (pattern.mask & mask) != 0;
}

static bool matchesElementBitsPattern(ElementBitsPattern pattern, Type elementType)
{
    unsigned elementBits = pto::getPTOStorageElemBitWidth(elementType);
    return matchesElementBitsPattern(pattern, elementBits);
}

static bool matchesElementCountPattern(ElementCountPattern pattern, int64_t count)
{
    if (pattern.any)
        return true;
...
static bool matchesPhysicalChunkCountPattern(PhysicalChunkCountPattern pattern, int64_t chunkCount)
{
    if (chunkCount <= 0)
        return false;
    for (int64_t i = 0; i < pattern.count; ++i)
...
static bool matchesMaskGranularityPattern(MaskGranularityPattern pattern, StringRef granularity)
{
    uint8_t mask = granularity == "b8" ? 1u << 0 : granularity == "b16" ? 1u << 1 : granularity == "b32" ? 1u << 2 : 0;
    return mask != 0 && (pattern.mask & mask) != 0;
}

static VMILayoutAttr materializeLayoutPattern(MLIRContext* ctx, LayoutPattern pattern, int64_t numGroups = 0)
{
    switch (pattern.kind) {
        ...
        case LayoutPatternKind::GroupSlots:
            return numGroups > 0 ? VMILayoutAttr::getGroupSlots(ctx, numGroups, pattern.value, pattern.laneStride) :
                                   VMILayoutAttr();
    }
    llvm_unreachable("unknown layout pattern kind");
}

static bool matchesLayoutPattern(MLIRContext* ctx, LayoutPattern pattern, VMILayoutAttr layout, int64_t numGroups = 0)
{
    if (!layout)
        return false;
...
static constexpr LayoutPattern c() { return {LayoutPatternKind::Contiguous, 1, 1}; }

static constexpr LayoutPattern ls(int64_t laneStride) { return {LayoutPatternKind::LaneStride, laneStride, 1}; }

static constexpr LayoutPattern d(int64_t factor) { return {LayoutPatternKind::Deinterleaved, factor, 1}; }

static constexpr LayoutPattern bd(int64_t factor) { return {LayoutPatternKind::BlockDeinterleaved, factor, 1}; }
...
static constexpr GroupMemoryPattern memAny() { return {GroupMemoryPatternKind::Any}; }

static constexpr GroupMemoryPattern memContiguous() { return {GroupMemoryPatternKind::Contiguous}; }

static constexpr GroupMemoryPattern memBlockAligned() { return {GroupMemoryPatternKind::BlockAligned}; }

static constexpr GroupBlockPattern gb(int64_t numerator) { return {GroupBlockPatternKind::Ratio, numerator, 1}; }
...
static FailureOr<int64_t> getGroupSizeFromNumGroups(VMIVRegType type, int64_t numGroups, std::string* reason)
{
...
static constexpr GroupReduceLayoutPattern kGroupReduceLayoutPatterns[] = {
    {gb(1, 4), ls(4), gs(8)}, {gb(1, 2), ls(2), gs(8)}, {gb(1), c(), gs(8)},   {gb(2), d(2), gs(8)},
    {gb(2), bd(2), gs(8)},    {gb(4), d(4), gs(8)},     {gb(4), bd(4), gs(8)}, {gbFull(), c(), gs(1)},
    {gbFull(2), d(2), gs(1)}, {gbFull(4), d(4), gs(1)},
};
...
static constexpr PreferredCastLayoutPattern kPreferredLaneStrideNarrowCastLayoutPatterns[] = {
    {bits<16>(), bits<8>(), 0, c(), ls(2)},
    {bits<32>(), bits<16>(), 0, c(), ls(2)},
    {bits<32>(), bits<8>(), 0, c(), ls(4)},
};

static constexpr HighPriorityCastLayoutPattern kHighPriorityCastLayoutPatterns[] = {
...
static constexpr LegalMaskGranularityCastLayoutPattern kLegalMaskGranularityCastLayoutPatterns[] = {
    // Same-width fp-to-fp (bf16 -> f16): dense contiguous 1:1.
    {mb16(), mb16(), c(), c()},
...
static constexpr InterleaveLayoutPattern kVdintlvLayoutPatterns[] = {
    {bits<8, 16, 32, 64>(), chunk<2, 4>(), 0, d(2), d(2), d(2), c(), c()},
    {bits<8, 16, 32, 64>(), chunk<4>(), 0, d(4), d(4), d(4), d(2), d(2)},
...
static constexpr InterleaveLayoutPattern kVintlvLayoutPatterns[] = {
    {bits<8, 16, 32, 64>(), chunk<2, 4>(), 0, c(), c(), c(), d(2), d(2)},
    {bits<8, 16, 32, 64>(), chunk<4>(), 0, d(2), d(2), d(2), d(4), d(4)},
    {bits<8, 16, 32, 64>(), chunk<1>(), 0, c(), c(), c(), c(), c()},
    {bits<8, 16>(), chunk<1>(), 1, ls(2), ls(2), ls(2), ls(2), ls(2)},
    {bits<8>(), chunk<1>(), 1, ls(4), ls(4), ls(4), ls(4), ls(4)},
};
...
static constexpr DeinterleaveLoadLayoutPattern kDeinterleaveLoadLayoutPatterns[] = {
    {bits<8, 16, 32>(), c(), c()},
};
...
static constexpr DenseMaskedStoreLayoutPattern kDenseMaskedStoreLayoutPatterns[] = {
    {bits<8, 16, 32>(), c(), c()},
...
static constexpr GroupStoreLayoutPattern kGroupStoreLayoutPatterns[] = {
    ...
    {bits<8, 16, 32>(), gb(1), memBlockAligned(), c(), GroupStoreLayoutPriority::High},
    // Full physical group chunks use ordinary contiguous stores.
    {bits<8, 16, 32>(), gbFull(), memAny(), c(), GroupStoreLayoutPriority::Fallback},
...
static constexpr GroupBroadcastLoadLayoutPattern kGroupBroadcastLoadLayoutPatterns[] = {
    {gb(1, 4), bits<8, 16, 32>(), memContiguous(), ls(4)}, {gb(1, 2), bits<8, 16, 32>(), memContiguous(), ls(2)},
    {gb(1), bits<8, 16, 32>(), memContiguous(), c()},      {gb(2), bits<8, 16, 32>(), memContiguous(), c()},
    {gb(2), bits<8, 16, 32>(), memContiguous(), d(2)},     {gb(4), bits<8, 16, 32>(), memContiguous(), c()},
    {gb(4), bits<8, 16, 32>(), memContiguous(), d(4)},     {gbFull(), bits<8, 16, 32>(), memAny(), c()},
};
...
static constexpr GroupBroadcastLoadDirectPattern kGroupBroadcastLoadDirectPatterns[] = {
    {VMIGroupBroadcastLoadDirectKind::E2B, G<8>(), gb(1), bits<16, 32>(), memContiguous(), c()},
    {VMIGroupBroadcastLoadDirectKind::E2B, G<8>(), gb(2), bits<16, 32>(), memContiguous(), d(2)},
    {VMIGroupBroadcastLoadDirectKind::E2B, G<8>(), gb(4), bits<16, 32>(), memContiguous(), d(4)},
    {VMIGroupBroadcastLoadDirectKind::BRC, anyG(), gbFull(), bits<8, 16, 32>(), memAny(), c()},
};
...
static constexpr VselrLayoutPattern kVselrLayoutPatterns[] = {
    {bits<8>(), N<64>(), N<64>(), N<64>(), chunk<1>(), chunk<1>(), chunk<1>(), c(), c(), c()},
    {bits<8>(), N<128>(), N<128>(), N<128>(), chunk<1>(), chunk<1>(), chunk<1>(), c(), c(), c()},
    {bits<8>(), N<256>(), N<256>(), N<256>(), chunk<1>(), chunk<1>(), chunk<1>(), c(), c(), c()},
    {bits<16>(), N<64>(), N<64>(), N<64>(), chunk<1>(), chunk<1>(), chunk<1>(), c(), c(), c()},
    {bits<16>(), N<128>(), N<128>(), N<128>(), chunk<1>(), chunk<1>(), chunk<1>(), c(), c(), c()},
    {bits<32>(), N<64>(), N<64>(), N<64>(), chunk<1>(), chunk<1>(), chunk<1>(), c(), c(), c()},
};
...
static constexpr WidthChangingBitcastLayoutPattern kWidthChangingBitcastLayoutPatterns[] = {
    {c()},
};
    ...
    MLIRContext* ctx, const DeinterleaveLoadLayoutPattern& pattern)
{
    return VMIDeinterleaveLoadLayoutFact{
        materializeLayoutPattern(ctx, pattern.lowLayout), materializeLayoutPattern(ctx, pattern.highLayout)};
}

static bool isSameGroupBlockPattern(GroupBlockPattern lhs, GroupBlockPattern rhs)
{
    return lhs.kind == rhs.kind && lhs.numerator == rhs.numerator && lhs.denominator == rhs.denominator;
}

static VMIGroupBlockClass getGroupBlockClassFromPattern(GroupBlockPattern pattern)
{
...
static bool matchesGroupBroadcastLoadMemoryPattern(
    GroupMemoryPattern pattern, std::optional<int64_t> stride, int64_t elementBits)
{
    switch (pattern.kind) {
...
static bool matchesGroupLoadMemoryPattern(
    GroupMemoryPattern pattern, std::optional<int64_t> rowStride, int64_t groupSize, int64_t elementBits)
{
    switch (pattern.kind) {
        case GroupMemoryPatternKind::Any:
            return true;
...
static bool isSupportedGroupSlotMemoryLayout(VMILayoutAttr layout, int64_t numGroups)
{
    if (!layout || !layout.isGroupSlots() || layout.getNumGroups() != numGroups || layout.getSlots() <= 0)
        return false;
    for (const GroupSlotMemoryLayoutPattern& pattern : kGroupSlotMemoryLayoutPatterns)
        if (matchesLayoutPattern(layout.getContext(), pattern.layout, layout, numGroups))
            return true;
    return false;
}

static FailureOr<VMIGroupBlockClass> getGroupBlockClass(int64_t groupSize, int64_t vcgBlockElems)
{
    if (vcgBlockElems <= 0)
        return failure();
...
static bool matchesPhysicalChunkCountPattern(PhysicalChunkCountPattern pattern, InterleaveLayoutKey key)
{
    return matchesPhysicalChunkCountPattern(pattern, key.physicalChunkCount);
}

static bool matchesInterleaveLayoutPattern(
    const InterleaveLayoutPattern& pattern, VMIVRegType valueType, InterleaveLayoutKey key)
{
    if (!matchesElementBitsPattern(pattern.elementBits, valueType.getElementType()) ||
        !matchesPhysicalChunkCountPattern(pattern.chunks, key))
        return false;
    if (pattern.maxPhysicalParts <= 0)
        return true;

    VMILayoutAttr lhsLayout = materializeLayoutPattern(valueType.getContext(), pattern.lhsLayout);
    auto lhsType =
        VMIVRegType::get(valueType.getContext(), valueType.getElementCount(), valueType.getElementType(), lhsLayout);
...
static FailureOr<InterleaveLayoutKey> buildInterleaveLayoutKey(VMIVRegType valueType, std::string* reason)
{
    ...
    FailureOr<int64_t> lanesPerPart = getDataLanesPerPart(valueType.getElementType());
    ...
    int64_t physicalChunkCount =
        elementCount <= *lanesPerPart ? 1 : (elementCount % *lanesPerPart == 0 ? elementCount / *lanesPerPart : 0);
    return InterleaveLayoutKey{elementCount, *lanesPerPart, physicalChunkCount};
}

static bool matchesGroupBlockPattern(GroupBlockPattern pattern, GroupLayoutKey key)
{
...
static bool matchesGroupStoreLayoutPattern(
    const GroupStoreLayoutPattern& pattern, VMIVRegType valueType, GroupLayoutKey key, std::optional<int64_t> rowStride)
{
    unsigned elementBits = pto::getPTOStorageElemBitWidth(valueType.getElementType());
    return elementBits != 0 && matchesElementBitsPattern(pattern.elementBits, valueType.getElementType()) &&
           matchesGroupBlockPattern(pattern.block, key) &&
           matchesGroupLoadMemoryPattern(pattern.memory, rowStride, key.groupSize, elementBits);
}

static FailureOr<GroupLayoutKey> buildGroupLayoutKey(
    VMIVRegType type, int64_t numGroups, const Twine& unsupportedGroupSizeReason, std::string* reason)
{
    ...
    FailureOr<int64_t> groupSize = getGroupSizeFromNumGroups(type, numGroups, reason);
    if (failed(groupSize))
        return failure();
    ...
    FailureOr<VMIGroupBlockClass> blockClass = getGroupBlockClass(*groupSize, vcgBlockElems);
...
static VMIGroupReduceLayoutFact materializeGroupReduceLayoutFact(
    MLIRContext* ctx, const GroupReduceLayoutPattern& pattern, int64_t groupSize, int64_t lanesPerPart,
    int64_t vcgBlockElems, int64_t numGroups)
{
    ...
    fact.resultLayout = materializeLayoutPattern(ctx, pattern.resultLayout, numGroups);
...
static VMIGroupBroadcastLayoutFact materializeGroupBroadcastLayoutFact(
    MLIRContext* ctx, const GroupBroadcastLayoutPattern& pattern, int64_t groupSize, int64_t lanesPerPart,
    int64_t vcgBlockElems, int64_t numGroups)
{
    VMIGroupBroadcastLayoutFact fact;
    fact.blockClass = getGroupBlockClassFromPattern(pattern.block);
    fact.sourceLayout = materializeLayoutPattern(ctx, pattern.sourceLayout, numGroups);
    fact.resultLayout = materializeLayoutPattern(ctx, pattern.resultLayout, numGroups);
    fact.groupSize = groupSize;
    fact.lanesPerPart = lanesPerPart;
    fact.vcgBlockElems = vcgBlockElems;
...
static VMIGroupStoreLayoutFact materializeGroupStoreLayoutFact(
    MLIRContext* ctx, const GroupStoreLayoutPattern& pattern, GroupLayoutKey key)
{
...
static VMIInterleaveLayoutFact materializeInterleaveLayoutFact(
    MLIRContext* ctx, const InterleaveLayoutPattern& pattern, InterleaveLayoutKey key)
{
...
static VMIVselrLayoutFact materializeVselrLayoutFact(MLIRContext* ctx, const VselrLayoutPattern& pattern)
{
    return VMIVselrLayoutFact{
        materializeLayoutPattern(ctx, pattern.sourceLayout), materializeLayoutPattern(ctx, pattern.indexLayout),
        materializeLayoutPattern(ctx, pattern.resultLayout)};
}

static FailureOr<int64_t> getPhysicalArityForLayout(VMIVRegType type, VMILayoutAttr layout)
{
    auto assignedType = VMIVRegType::get(type.getContext(), type.getElementCount(), type.getElementType(), layout);
    return getVMIPhysicalArity(assignedType);
}

static bool matchesVselrLayoutPattern(const VselrLayoutPattern& pattern, VMIVselrOp op, const VMIVselrLayoutFact& fact)
{
    ...
    unsigned indexBits = pto::getPTOStorageElemBitWidth(indexType.getElementType());
    if (sourceType.getElementType() != resultType.getElementType() ||
        indexType.getElementCount() != resultType.getElementCount() || sourceBits != indexBits ||
        !matchesElementBitsPattern(pattern.elementBits, sourceBits) ||
        !matchesElementCountPattern(pattern.sourceElements, sourceType.getElementCount()) ||
        !matchesElementCountPattern(pattern.indexElements, indexType.getElementCount()) ||
        !matchesElementCountPattern(pattern.resultElements, resultType.getElementCount()))
        return false;

    FailureOr<int64_t> sourceArity = getPhysicalArityForLayout(sourceType, fact.sourceLayout);
    FailureOr<int64_t> indexArity = getPhysicalArityForLayout(indexType, fact.indexLayout);
    FailureOr<int64_t> resultArity = getPhysicalArityForLayout(resultType, fact.resultLayout);
    return succeeded(sourceArity) && succeeded(indexArity) && succeeded(resultArity) &&
...
FailureOr<VMIVselrLayoutFact> VMILayoutSupport::getPreferredVselrLayoutFact(VMIVselrOp op, std::string* reason) const
{
    ...
    for (const VselrLayoutPattern& pattern : kVselrLayoutPatterns) {
        VMIVselrLayoutFact fact = materializeVselrLayoutFact(op.getContext(), pattern);
...
FailureOr<VMIVselrLayoutFact> VMILayoutSupport::getVselrLayoutFact(VMIVselrOp op, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIVselrLayoutFact> {
        if (reason)
            *reason = message.str();
    ...
    auto indexType = cast<VMIVRegType>(op.getIndex().getType());
    auto resultType = cast<VMIVRegType>(op.getResult().getType());
    VMIVselrLayoutFact assignedFact{sourceType.getLayoutAttr(), indexType.getLayoutAttr(), resultType.getLayoutAttr()};
    if (!assignedFact.sourceLayout || !assignedFact.indexLayout || !assignedFact.resultLayout)
        return fail("vselr requires assigned source/index/result layouts");

    for (const VselrLayoutPattern& pattern : kVselrLayoutPatterns) {
        VMIVselrLayoutFact tableFact = materializeVselrLayoutFact(op.getContext(), pattern);
        if (assignedFact.sourceLayout != tableFact.sourceLayout || assignedFact.indexLayout != tableFact.indexLayout ||
...
LogicalResult VMILayoutSupport::getVselrSupport(VMIVselrOp op, std::string* reason) const
{
    return getVselrLayoutFact(op, reason);
}

FailureOr<VMIGroupReduceLayoutFact> VMILayoutSupport::getPreferredGroupReduceLayoutFact(
    VMIVRegType sourceType, int64_t numGroups, std::string* reason) const
{
    ...
    for (const GroupReduceLayoutPattern& pattern : kGroupReduceLayoutPatterns) {
        if (!matchesGroupBlockPattern(pattern.block, *key))
            continue;
        return materializeGroupReduceLayoutFact(
            sourceType.getContext(), pattern, key->groupSize, key->lanesPerPart, key->vcgBlockElems, numGroups);
    }
...
FailureOr<VMIGroupReduceLayoutFact> VMILayoutSupport::getGroupReduceLayoutFactForLayouts(
    VMIVRegType sourceType, VMIMaskType maskType, VMIVRegType resultType, int64_t numGroups, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIGroupReduceLayoutFact> {
        if (reason)
            *reason = message.str();
    ...
    FailureOr<GroupLayoutKey> key =
        buildGroupLayoutKey(sourceType, numGroups, "group_reduce layout table has no row for this group size", reason);
    if (failed(key))
        return failure();
    ...
    for (const GroupReduceLayoutPattern& pattern : kGroupReduceLayoutPatterns) {
        ...
        VMIGroupReduceLayoutFact candidate = materializeGroupReduceLayoutFact(
            sourceType.getContext(), pattern, key->groupSize, key->lanesPerPart, key->vcgBlockElems, numGroups);
        if (candidate.sourceLayout == sourceLayout && candidate.maskLayout == maskLayout &&
...
FailureOr<SmallVector<VMIGroupReduceLayoutFact, 4>> VMILayoutSupport::getGroupReduceLayoutFactsForLayout(
    VMIVRegType sourceType, int64_t numGroups, VMIGroupReduceLayoutPort port, VMILayoutAttr layout,
    std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<SmallVector<VMIGroupReduceLayoutFact, 4>> {
        if (reason)
            *reason = message.str();
        return failure();
    };
...
FailureOr<VMIGroupBroadcastLayoutFact> VMILayoutSupport::getGroupBroadcastLayoutFactForLayouts(
    VMIVRegType sourceType, VMIVRegType resultType, int64_t numGroups, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIGroupBroadcastLayoutFact> {
        if (reason)
            *reason = message.str();
        return failure();
    };
    ...
    FailureOr<GroupLayoutKey> key = buildGroupLayoutKey(
        resultType, numGroups, "group_broadcast layout table has no row for this group size", reason);
    if (failed(key))
        return failure();
    ...
    for (const GroupBroadcastLayoutPattern& pattern : kGroupBroadcastLayoutPatterns) {
        ...
        VMIGroupBroadcastLayoutFact candidate = materializeGroupBroadcastLayoutFact(
            sourceType.getContext(), pattern, key->groupSize, key->lanesPerPart, key->vcgBlockElems, numGroups);
        if (candidate.sourceLayout == sourceLayout && candidate.resultLayout == resultLayout)
            return candidate;
    }
...
FailureOr<SmallVector<VMIGroupBroadcastLayoutFact, 4>> VMILayoutSupport::getGroupBroadcastLayoutFactsForLayout(
    VMIVRegType sourceType, VMIVRegType resultType, int64_t numGroups, VMIGroupBroadcastLayoutPort port,
    VMILayoutAttr layout, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<SmallVector<VMIGroupBroadcastLayoutFact, 4>> {
        if (reason)
            *reason = message.str();
        return failure();
    };
...
FailureOr<VMIGroupBroadcastLoadLayoutFact> VMILayoutSupport::getGroupBroadcastLoadLayoutFact(
    VMIGroupBroadcastLoadOp op, std::string* reason) const
{
    return getGroupBroadcastLoadLayoutFact(
        cast<VMIVRegType>(op.getResult().getType()), op.getSourceGroupStride(), op.getNumGroupsAttr().getInt(), reason);
}

FailureOr<VMIGroupBroadcastLoadLayoutFact> VMILayoutSupport::getGroupBroadcastLoadLayoutFact(
    VMIVRegType resultType, Value sourceGroupStride, int64_t numGroups, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIGroupBroadcastLoadLayoutFact> {
        if (reason)
            *reason = message.str();
        return failure();
    };
    ...
    FailureOr<GroupLayoutKey> key = buildGroupLayoutKey(
        resultType, numGroups, "group_broadcast_load layout table has no row for this group size", reason);
    if (failed(key))
        return failure();

    for (const GroupBroadcastLoadLayoutPattern& pattern : kGroupBroadcastLoadLayoutPatterns) {
        if (!matchesGroupBlockPattern(pattern.block, *key))
            continue;
        if (!matchesElementBitsPattern(pattern.elementBits, elementBits))
            continue;
        if (!matchesGroupBroadcastLoadMemoryPattern(pattern.memory, stride, elementBits))
            continue;
        if (!matchesLayoutPattern(resultType.getContext(), pattern.resultLayout, resultLayout, numGroups))
            continue;
    ...
    int64_t alignedStrideElems = 256 / elementBits;
    return fail(
        Twine("group_broadcast_load requires a table row for result "
...
FailureOr<VMIGroupBroadcastLoadDirectFact> VMILayoutSupport::getGroupBroadcastLoadDirectFact(
    VMIGroupBroadcastLoadOp op, std::string* reason) const
{
    return getGroupBroadcastLoadDirectFact(
        cast<VMIVRegType>(op.getResult().getType()), op.getSource().getType(), op.getSourceGroupStride(),
        op.getNumGroupsAttr().getInt(), reason);
}

FailureOr<VMIGroupBroadcastLoadDirectFact> VMILayoutSupport::getGroupBroadcastLoadDirectFact(
    VMIVRegType resultType, Type sourceType, Value sourceGroupStride, int64_t numGroups, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIGroupBroadcastLoadDirectFact> {
        if (reason)
            *reason = message.str();
        return failure();
    };
    ...
    for (const GroupBroadcastLoadDirectPattern& pattern : kGroupBroadcastLoadDirectPatterns) {
        ...
        if (!matchesGroupBroadcastLoadMemoryPattern(pattern.memory, stride, elementBits))
            continue;
        VMILayoutAttr resultLayout = materializeLayoutPattern(resultType.getContext(), pattern.resultLayout, numGroups);
        if (existing && existing != resultLayout)
            continue;
        return VMIGroupBroadcastLoadDirectFact{
            pattern.kind, VMIGroupBroadcastLoadLayoutFact{
                              getGroupBlockClassFromPattern(pattern.block), resultLayout, key->groupSize,
                              key->lanesPerPart, key->vcgBlockElems, static_cast<int64_t>(elementBits)}};
    }
...
static std::pair<int64_t, int64_t> getCastElementBits(VMIVRegType sourceType, VMIVRegType resultType)
{
    unsigned sourceBits = pto::getPTOStorageElemBitWidth(sourceType.getElementType());
...
static VMICastLayoutFact makeCastLayoutFact(
    int64_t sourceBits, int64_t resultBits, VMILayoutAttr sourceLayout, VMILayoutAttr resultLayout,
...
static FailureOr<VMICastLayoutFact> getHighPriorityCastLayoutFactImpl(
    VMIVRegType sourceType, VMIVRegType resultType, bool allowLaneStrideNarrowing, std::string* reason)
{
    ...
    for (const HighPriorityCastLayoutPattern& pattern : kHighPriorityCastLayoutPatterns) {
        if (!matchesElementBitsPattern(pattern.sourceBits, sourceBits) ||
            !matchesElementBitsPattern(pattern.resultBits, resultBits))
            continue;

        VMILayoutAttr sourceLayout = materializeLayoutPattern(ctx, pattern.sourceLayout);
        VMILayoutAttr resultLayout = materializeLayoutPattern(ctx, pattern.resultLayout);
        auto assignedSourceType =
            VMIVRegType::get(ctx, sourceType.getElementCount(), sourceType.getElementType(), sourceLayout);
        auto assignedResultType =
            VMIVRegType::get(ctx, resultType.getElementCount(), resultType.getElementType(), resultLayout);
        ...
        selected = makeCastLayoutFact(sourceBits, resultBits, sourceLayout, resultLayout, VMICastLayoutPriority::High);
    }
    if (!selected) {
        if (reason)
...
static VMIMaskGranularityCastLayoutFact makeMaskGranularityCastLayoutFact(
    int64_t sourceBits, int64_t resultBits, VMILayoutAttr sourceLayout, VMILayoutAttr resultLayout)
{
...
static FailureOr<VMICastLayoutFact> getPreferredCastLayoutFactImpl(
    ArrayRef<PreferredCastLayoutPattern> patterns, VMIVRegType sourceType, VMIVRegType resultType, StringRef tableName,
    std::string* reason, VMICastLayoutPriority priority = VMICastLayoutPriority::Normal)
{
    auto [sourceBits, resultBits] = getCastElementBits(sourceType, resultType);
    ...
    return makeCastLayoutFact(
        sourceBits, resultBits, materializeLayoutPattern(ctx, selected->sourceLayout),
        materializeLayoutPattern(ctx, selected->resultLayout), priority);
}

static FailureOr<VMICastLayoutFact> getPreferredLaneStrideNarrowCastLayoutFactImpl(
    VMIVRegType sourceType, VMIVRegType resultType, std::string* reason)
{
    return getPreferredCastLayoutFactImpl(
        ...
        "preferred lane-stride narrow cast layout table", reason, VMICastLayoutPriority::LaneStrideNarrowing);
}
    ...
    VMIVRegType sourceType, VMIVRegType resultType, std::string* reason) const
{
    FailureOr<VMICastLayoutFact> highPriorityFact =
        getHighPriorityCastLayoutFactImpl(sourceType, resultType, preferLaneStrideNarrowing, reason);
    ...
    if (preferLaneStrideNarrowing) {
        FailureOr<VMICastLayoutFact> laneStrideFact =
            getPreferredLaneStrideNarrowCastLayoutFactImpl(sourceType, resultType, reason);
        if (succeeded(laneStrideFact))
            return laneStrideFact;
    }
    return getPreferredCastLayoutFactImpl(
        kPreferredCastLayoutPatterns, sourceType, resultType, "preferred cast layout table", reason);
}

FailureOr<SmallVector<VMICastLayoutFact, 4>> VMILayoutSupport::getCastLayoutFactsForLayout(
    VMIVRegType sourceType, VMIVRegType resultType, VMICastLayoutPort port, VMILayoutAttr layout,
    std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<SmallVector<VMICastLayoutFact, 4>> {
        if (reason)
            *reason = message.str();
        return failure();
    };
    ...
    int64_t numGroups = layout && layout.isGroupSlots() ? layout.getNumGroups() : 0;
    for (const LegalCastLayoutPattern& pattern : kLegalCastLayoutPatterns) {
        if (!matchesElementBitsPattern(pattern.sourceBits, sourceBits) ||
            !matchesElementBitsPattern(pattern.resultBits, resultBits))
            continue;

        VMILayoutAttr sourceLayout = materializeLayoutPattern(ctx, pattern.sourceLayout, numGroups);
        VMILayoutAttr resultLayout = materializeLayoutPattern(ctx, pattern.resultLayout, numGroups);
        if (!sourceLayout || !resultLayout)
            continue;
        ...
        facts.push_back(makeCastLayoutFact(sourceBits, resultBits, sourceLayout, resultLayout));
    }
...
static FailureOr<VMICastLayoutFact> getUniqueCastLayoutFact(
    FailureOr<SmallVector<VMICastLayoutFact, 4>> facts, std::string* reason)
{
...
FailureOr<VMICastLayoutFact> VMILayoutSupport::getCastLayoutFactForSourceLayout(
    VMIVRegType sourceType, VMIVRegType resultType, VMILayoutAttr sourceLayout, std::string* reason) const
{
    return getUniqueCastLayoutFact(
        getCastLayoutFactsForLayout(sourceType, resultType, VMICastLayoutPort::Source, sourceLayout, reason), reason);
}

FailureOr<VMICastLayoutFact> VMILayoutSupport::getCastLayoutFactForResultLayout(
    VMIVRegType sourceType, VMIVRegType resultType, VMILayoutAttr resultLayout, std::string* reason) const
{
    return getUniqueCastLayoutFact(
        getCastLayoutFactsForLayout(sourceType, resultType, VMICastLayoutPort::Result, resultLayout, reason), reason);
}

FailureOr<VMICastLayoutFact> VMILayoutSupport::getCastLayoutFactForLayouts(
    VMIVRegType sourceType, VMIVRegType resultType, VMILayoutAttr sourceLayout, VMILayoutAttr resultLayout,
    std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMICastLayoutFact> {
        if (reason)
    ...
    FailureOr<SmallVector<VMICastLayoutFact, 4>> facts =
        getCastLayoutFactsForLayout(sourceType, resultType, VMICastLayoutPort::Source, sourceLayout, reason);
    if (failed(facts))
        return failure();
...
VMILayoutSupport::getMaskGranularityCastLayoutFactsForLayout(
    VMIMaskType sourceType, VMIMaskType resultType, VMICastLayoutPort port, VMILayoutAttr layout,
    std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<SmallVector<VMIMaskGranularityCastLayoutFact, 4>> {
        if (reason)
            *reason = message.str();
        return failure();
    };
    ...
    for (const LegalMaskGranularityCastLayoutPattern& pattern : kLegalMaskGranularityCastLayoutPatterns) {
        if (!matchesMaskGranularityPattern(pattern.sourceGranularity, sourceType.getGranularity()) ||
            !matchesMaskGranularityPattern(pattern.resultGranularity, resultType.getGranularity()))
            continue;
        ...
        facts.push_back(makeMaskGranularityCastLayoutFact(sourceBits, resultBits, sourceLayout, resultLayout));
    }
...
FailureOr<VMIMaskGranularityCastLayoutFact> VMILayoutSupport::getMaskGranularityCastLayoutFactForLayouts(
    VMIMaskType sourceType, VMIMaskType resultType, VMILayoutAttr sourceLayout, VMILayoutAttr resultLayout,
    std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIMaskGranularityCastLayoutFact> {
        if (reason)
            *reason = message.str();
        return failure();
    };

    FailureOr<SmallVector<VMIMaskGranularityCastLayoutFact, 4>> facts = getMaskGranularityCastLayoutFactsForLayout(
        sourceType, resultType, VMICastLayoutPort::Source, sourceLayout, reason);
    if (failed(facts))
        return failure();
...
FailureOr<VMILayoutAttr> VMILayoutSupport::getWidenSourceLayoutForResultLayout(
    VMIVRegType sourceType, VMIVRegType resultType, VMILayoutAttr requestedResultLayout, std::string* reason) const
{
    FailureOr<VMICastLayoutFact> fact =
        getCastLayoutFactForResultLayout(sourceType, resultType, requestedResultLayout, reason);
    if (failed(fact))
        return failure();
...
static FailureOr<VMIInterleaveLayoutFact> getPreferredInterleaveLayoutFactImpl(
    ArrayRef<InterleaveLayoutPattern> patterns, VMIVRegType valueType, std::string* reason)
{
    ...
    FailureOr<InterleaveLayoutKey> key = buildInterleaveLayoutKey(valueType, reason);
    if (failed(key))
        return failure();
    ...
    for (const InterleaveLayoutPattern& pattern : patterns) {
        if (!matchesInterleaveLayoutPattern(pattern, valueType, *key))
            continue;
        return materializeInterleaveLayoutFact(valueType.getContext(), pattern, *key);
    }

    return fail("requires a preferred interleave layout table row");
}

static FailureOr<SmallVector<VMIInterleaveLayoutFact, 4>> getInterleaveLayoutFactsForLayoutImpl(
    ArrayRef<InterleaveLayoutPattern> patterns, VMIVRegType valueType, VMIInterleaveLayoutPort port,
    VMILayoutAttr layout, std::string* reason)
{
    auto fail = [&](const Twine& message) -> FailureOr<SmallVector<VMIInterleaveLayoutFact, 4>> {
        if (reason)
            *reason = message.str();
        return failure();
    };
    ...
    for (const InterleaveLayoutPattern& pattern : patterns) {
        if (!matchesInterleaveLayoutPattern(pattern, valueType, *key))
            continue;
        VMIInterleaveLayoutFact candidate = materializeInterleaveLayoutFact(valueType.getContext(), pattern, *key);
...
static FailureOr<VMIInterleaveLayoutFact> getInterleaveLayoutFactForLayoutsImpl(
    ArrayRef<InterleaveLayoutPattern> patterns, VMIVRegType lhsType, VMIVRegType rhsType, VMIMaskType maskType,
    VMIVRegType lowType, VMIVRegType highType, std::string* reason)
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIInterleaveLayoutFact> {
        if (reason)
            *reason = message.str();
    ...
    if (lhsType.getElementCount() != rhsType.getElementCount() ||
        ...
        return fail("interleave layout requires all ports to share logical lane "
                    "count");
    if (lhsType.getElementType() != rhsType.getElementType() || lhsType.getElementType() != lowType.getElementType() ||
    ...
    FailureOr<SmallVector<VMIInterleaveLayoutFact, 4>> facts =
        getInterleaveLayoutFactsForLayoutImpl(patterns, lhsType, VMIInterleaveLayoutPort::Lhs, lhsLayout, reason);
    if (failed(facts))
        return failure();

    std::optional<VMIInterleaveLayoutFact> selected;
    for (const VMIInterleaveLayoutFact& fact : *facts) {
        if (fact.rhsLayout != rhsLayout || fact.maskLayout != maskLayout || fact.lowLayout != lowLayout ||
            fact.highLayout != highLayout)
            continue;
        if (selected)
...
FailureOr<VMIInterleaveLayoutFact> VMILayoutSupport::getPreferredVintlvLayoutFact(
    VMIVRegType valueType, std::string* reason) const
{
    return getPreferredInterleaveLayoutFactImpl(kVintlvLayoutPatterns, valueType, reason);
}

FailureOr<VMIInterleaveLayoutFact> VMILayoutSupport::getPreferredVdintlvLayoutFact(
    VMIVRegType valueType, std::string* reason) const
{
    return getPreferredInterleaveLayoutFactImpl(kVdintlvLayoutPatterns, valueType, reason);
}

FailureOr<SmallVector<VMIInterleaveLayoutFact, 4>> VMILayoutSupport::getVintlvLayoutFactsForLayout(
    VMIVRegType valueType, VMIInterleaveLayoutPort port, VMILayoutAttr layout, std::string* reason) const
{
    return getInterleaveLayoutFactsForLayoutImpl(kVintlvLayoutPatterns, valueType, port, layout, reason);
}

FailureOr<SmallVector<VMIInterleaveLayoutFact, 4>> VMILayoutSupport::getVdintlvLayoutFactsForLayout(
    VMIVRegType valueType, VMIInterleaveLayoutPort port, VMILayoutAttr layout, std::string* reason) const
{
    return getInterleaveLayoutFactsForLayoutImpl(kVdintlvLayoutPatterns, valueType, port, layout, reason);
}

FailureOr<VMIInterleaveLayoutFact> VMILayoutSupport::getVintlvLayoutFactForLayouts(
    VMIVRegType lhsType, VMIVRegType rhsType, VMIMaskType maskType, VMIVRegType lowType, VMIVRegType highType,
    std::string* reason) const
{
    return getInterleaveLayoutFactForLayoutsImpl(
        kVintlvLayoutPatterns, lhsType, rhsType, maskType, lowType, highType, reason);
}

FailureOr<VMIInterleaveLayoutFact> VMILayoutSupport::getVdintlvLayoutFactForLayouts(
    VMIVRegType lhsType, VMIVRegType rhsType, VMIMaskType maskType, VMIVRegType lowType, VMIVRegType highType,
    std::string* reason) const
{
    return getInterleaveLayoutFactForLayoutsImpl(
        kVdintlvLayoutPatterns, lhsType, rhsType, maskType, lowType, highType, reason);
}

FailureOr<VMILoadLayoutFact> VMILayoutSupport::getLoadLayoutFact(VMIVRegType resultType, std::string* reason) const
{
    ...
    for (const DenseMemoryLayoutPattern& pattern : kDenseLoadLayoutPatterns) {
        if (!matchesElementBitsPattern(pattern.elementBits, resultType.getElementType()))
            continue;
        if (!matchesElementCountPattern(pattern.elementCounts, resultType.getElementCount()))
            continue;
...
FailureOr<VMIDeinterleaveLoadLayoutFact> VMILayoutSupport::getPreferredDeinterleaveLoadLayoutFact(
    VMIVRegType valueType, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIDeinterleaveLoadLayoutFact> {
        if (reason)
            *reason = message.str();
        return failure();
    };

    for (const DeinterleaveLoadLayoutPattern& pattern : kDeinterleaveLoadLayoutPatterns) {
        if (!matchesElementBitsPattern(pattern.elementBits, valueType.getElementType()))
            continue;
        return materializeDeinterleaveLoadLayoutFact(valueType.getContext(), pattern);
    }
    return fail("requires a preferred deinterleave_load layout table row");
}

FailureOr<SmallVector<VMIDeinterleaveLoadLayoutFact, 4>> VMILayoutSupport::getDeinterleaveLoadLayoutFactsForLayout(
    VMIVRegType valueType, VMIDeinterleaveLoadLayoutPort port, VMILayoutAttr layout, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<SmallVector<VMIDeinterleaveLoadLayoutFact, 4>> {
        if (reason)
            *reason = message.str();
        return failure();
    };
    ...
    for (const DeinterleaveLoadLayoutPattern& pattern : kDeinterleaveLoadLayoutPatterns) {
        ...
        VMILayoutAttr candidateLayout =
            port == VMIDeinterleaveLoadLayoutPort::Low ? candidate.lowLayout : candidate.highLayout;
        if (candidateLayout == layout)
            facts.push_back(candidate);
    }
    if (facts.empty())
...
FailureOr<VMIDeinterleaveLoadLayoutFact> VMILayoutSupport::getDeinterleaveLoadLayoutFactForLayouts(
    VMIVRegType lowType, VMIVRegType highType, std::string* reason) const
{
    ...
    FailureOr<SmallVector<VMIDeinterleaveLoadLayoutFact, 4>> facts =
        getDeinterleaveLoadLayoutFactsForLayout(lowType, VMIDeinterleaveLoadLayoutPort::Low, lowLayout, reason);
    if (failed(facts))
        return failure();
...
FailureOr<VMIStoreLayoutFact> VMILayoutSupport::getStoreLayoutFact(VMIVRegType valueType, std::string* reason) const
{
    ...
    for (const DenseMemoryLayoutPattern& pattern : kDenseStoreLayoutPatterns) {
        if (!matchesElementBitsPattern(pattern.elementBits, valueType.getElementType()))
            continue;
        if (!matchesElementCountPattern(pattern.elementCounts, valueType.getElementCount()))
            continue;
...
FailureOr<VMIStoreLayoutFact> VMILayoutSupport::getPreferredStoreLayoutFact(
    VMIVRegType valueType, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIStoreLayoutFact> {
        if (reason)
    ...
    for (const DenseMemoryLayoutPattern& pattern : kDenseStoreLayoutPatterns) {
        ...
        if (!matchesElementCountPattern(pattern.elementCounts, valueType.getElementCount()))
            continue;
        VMILayoutAttr layout = materializeLayoutPattern(valueType.getContext(), pattern.layout);
        if (!layout)
            continue;
        return VMIStoreLayoutFact{layout};
    }
    ...
    VMIVRegType valueType, VMIMaskType maskType, std::string* reason) const
{
    ...
    for (const DenseMaskedStoreLayoutPattern& pattern : kDenseMaskedStoreLayoutPatterns) {
        if (!matchesElementBitsPattern(pattern.elementBits, valueType.getElementType()))
            continue;
        if (!matchesElementCountPattern(pattern.elementCounts, valueType.getElementCount()))
            continue;
        if (!matchesLayoutPattern(valueType.getContext(), pattern.valueLayout, valueLayout))
            continue;
        if (!matchesLayoutPattern(maskType.getContext(), pattern.maskLayout, maskLayout))
            continue;
...
FailureOr<VMIMaskedStoreLayoutFact> VMILayoutSupport::getPreferredMaskedStoreLayoutFact(
    VMIVRegType valueType, VMIMaskType maskType, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIMaskedStoreLayoutFact> {
        if (reason)
    ...
    for (const DenseMaskedStoreLayoutPattern& pattern : kDenseMaskedStoreLayoutPatterns) {
        ...
        VMILayoutAttr valueLayout = materializeLayoutPattern(valueType.getContext(), pattern.valueLayout);
        VMILayoutAttr maskLayout = materializeLayoutPattern(maskType.getContext(), pattern.maskLayout);
        if (!valueLayout || !maskLayout)
            continue;
...
FailureOr<VMIMaskedLoadLayoutFact> VMILayoutSupport::getMaskedLoadLayoutFact(
    VMIVRegType resultType, VMIMaskType maskType, VMIVRegType passthruType, std::string* reason) const
{
    ...
    for (const DenseMaskedLoadLayoutPattern& pattern : kDenseMaskedLoadLayoutPatterns) {
        if (!matchesElementBitsPattern(pattern.elementBits, resultType.getElementType()))
            continue;
        if (!matchesLayoutPattern(resultType.getContext(), pattern.resultLayout, resultLayout))
            continue;
        if (!matchesLayoutPattern(maskType.getContext(), pattern.maskLayout, maskLayout))
            continue;
        if (!matchesLayoutPattern(passthruType.getContext(), pattern.passthruLayout, passthruLayout))
            continue;
...
static LogicalResult matchEnsureLayoutPattern(
    VMIVRegType sourceType, VMIVRegType resultType, VMILayoutAttr sourceLayout, VMILayoutAttr resultLayout,
    std::string* reason)
{
    ...
    int64_t numGroups = sourceLayout.isGroupSlots() ? sourceLayout.getNumGroups() :
                                                      (resultLayout.isGroupSlots() ? resultLayout.getNumGroups() : 0);

    for (const EnsureLayoutPattern& pattern : kEnsureLayoutPatterns) {
        if (!matchesElementBitsPattern(pattern.elementBits, sourceType.getElementType()))
            continue;
        if (!matchesElementCountPattern(pattern.elementCounts, sourceType.getElementCount()))
            continue;
        if (!matchesLayoutPattern(sourceType.getContext(), pattern.sourceLayout, sourceLayout, numGroups))
            continue;
        if (!matchesLayoutPattern(resultType.getContext(), pattern.resultLayout, resultLayout, numGroups))
            continue;
        return success();
...
static LogicalResult matchEnsureMaskLayoutPattern(
    VMIMaskType sourceType, VMIMaskType resultType, VMILayoutAttr sourceLayout, VMILayoutAttr resultLayout,
    std::string* reason)
{
    auto fail = [&](const Twine& message) -> LogicalResult {
    ...
    for (const EnsureMaskLayoutPattern& pattern : kEnsureMaskLayoutPatterns) {
        if (!matchesMaskGranularityPattern(pattern.granularity, sourceType.getGranularity()))
            continue;
        if (!matchesElementCountPattern(pattern.elementCounts, sourceType.getElementCount()))
            continue;
        if (!matchesLayoutPattern(sourceType.getContext(), pattern.sourceLayout, sourceLayout))
            continue;
        if (!matchesLayoutPattern(resultType.getContext(), pattern.resultLayout, resultLayout))
            continue;
        return success();
    ...
    VMIVRegType sourceType, VMIVRegType resultType, std::string* reason) const
{
    VMILayoutAttr sourceLayout = sourceType.getLayoutAttr();
    VMILayoutAttr resultLayout = resultType.getLayoutAttr();
    if (failed(matchEnsureLayoutPattern(sourceType, resultType, sourceLayout, resultLayout, reason)))
        return failure();
    ...
    VMIMaskType sourceType, VMIMaskType resultType, std::string* reason) const
{
    VMILayoutAttr sourceLayout = sourceType.getLayoutAttr();
    VMILayoutAttr resultLayout = resultType.getLayoutAttr();
    if (failed(matchEnsureMaskLayoutPattern(sourceType, resultType, sourceLayout, resultLayout, reason)))
        return failure();
...
FailureOr<VMIGroupLoadLayoutFact> VMILayoutSupport::getGroupLoadLayoutFact(VMIGroupLoadOp op, std::string* reason) const
{
    return getGroupLoadLayoutFact(
        cast<VMIVRegType>(op.getResult().getType()), op.getRowStride(), op.getNumGroupsAttr().getInt(), reason);
}

FailureOr<VMIGroupLoadLayoutFact> VMILayoutSupport::getGroupLoadLayoutFact(
    VMIVRegType resultType, Value rowStride, int64_t numGroups, std::string* reason) const
{
    ...
    FailureOr<GroupLayoutKey> key =
        buildGroupLayoutKey(resultType, numGroups, "group_load layout table has no row for this group size", reason);
    if (failed(key))
        return failure();
    ...
    for (const GroupLoadLayoutPattern& pattern : kGroupLoadLayoutPatterns) {
        ...
        if (!matchesGroupBlockPattern(pattern.block, *key))
            continue;
        if (!matchesGroupLoadMemoryPattern(pattern.memory, stride, key->groupSize, elementBits))
            continue;
        if (!matchesLayoutPattern(resultType.getContext(), pattern.resultLayout, layout))
            continue;
        return VMIGroupLoadLayoutFact{getGroupBlockClassFromPattern(pattern.block), layout, key->groupSize};
    }
    ...
    VMIGroupStoreOp op, VMIVRegType valueType, std::string* reason) const
{
    ...
    FailureOr<GroupLayoutKey> key =
        buildGroupLayoutKey(valueType, numGroups, "group_store layout table has no row for this group size", reason);
    if (failed(key))
        return failure();
    ...
    for (const GroupStoreLayoutPattern& pattern : kGroupStoreLayoutPatterns) {
        if (!matchesGroupStoreLayoutPattern(pattern, valueType, *key, rowStride))
            continue;
        if (!matchesLayoutPattern(valueType.getContext(), pattern.valueLayout, layout))
            continue;
        VMIGroupStoreLayoutFact fact = materializeGroupStoreLayoutFact(valueType.getContext(), pattern, *key);
...
FailureOr<SmallVector<VMIGroupStoreLayoutFact, 4>> VMILayoutSupport::getGroupStoreLayoutFactsForLayout(
    VMIGroupStoreOp op, VMIVRegType valueType, VMILayoutAttr layout, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<SmallVector<VMIGroupStoreLayoutFact, 4>> {
        if (reason)
            *reason = message.str();
        return failure();
    };
    ...
    auto sourceType = VMIVRegType::get(ctx, valueType.getElementCount(), valueType.getElementType(), layout);
    FailureOr<VMIGroupStoreLayoutFact> directFact = getGroupStoreLayoutFact(op, sourceType, nullptr);
    if (succeeded(directFact))
        return SmallVector<VMIGroupStoreLayoutFact, 4>{*directFact};

    FailureOr<GroupLayoutKey> key = buildGroupLayoutKey(
        valueType, op.getNumGroupsAttr().getInt(), "group_store layout table has no row for this group size", reason);
    if (failed(key))
        return failure();
    ...
    for (const GroupStoreLayoutPattern& pattern : kGroupStoreLayoutPatterns) {
        ...
        VMILayoutAttr useLayout = materializeLayoutPattern(ctx, pattern.valueLayout);
        bool duplicate =
            llvm::any_of(facts, [&](const VMIGroupStoreLayoutFact& fact) { return fact.valueLayout == useLayout; });
        if (!useLayout || duplicate)
            continue;

        auto useType = VMIVRegType::get(ctx, valueType.getElementCount(), valueType.getElementType(), useLayout);
        if (failed(getEnsureLayoutFact(sourceType, useType, nullptr)))
            continue;
        VMIGroupStoreLayoutFact fact = materializeGroupStoreLayoutFact(ctx, pattern, *key);
...
FailureOr<VMIGroupStoreLayoutFact> VMILayoutSupport::getPreferredGroupStoreLayoutFact(
    VMIGroupStoreOp op, VMIVRegType valueType, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIGroupStoreLayoutFact> {
        if (reason)
    ...
    if (valueType.getElementCount() == numGroups) {
        std::optional<int64_t> rowStride = getConstantIndexValue(op.getRowStride());
        bool packedSlots = rowStride && *rowStride == 1 && static_cast<int64_t>(numGroups) >= 8;
        VMILayoutAttr layout = VMILayoutAttr::getGroupSlots(ctx, numGroups, packedSlots ? 8 : 1);
        auto assignedType = VMIVRegType::get(ctx, valueType.getElementCount(), valueType.getElementType(), layout);
    ...
    if (pto::getPTOStorageElemBitWidth(valueType.getElementType()) == 0)
        return fail("group_store requires known element bit width");
    FailureOr<GroupLayoutKey> key = buildGroupLayoutKey(
        valueType, numGroups, "group_store preferred layout table has no row for this group size", reason);
    if (failed(key))
        return failure();
    ...
    for (const GroupStoreLayoutPattern& pattern : kGroupStoreLayoutPatterns) {
        ...
        if (!matchesGroupStoreLayoutPattern(pattern, valueType, *key, rowStride))
            continue;
        if (!selected || static_cast<unsigned>(pattern.priority) > static_cast<unsigned>(selected->priority))
            selected = &pattern;
    }
    if (selected)
...
FailureOr<VMIGroupStoreLayoutFact> VMILayoutSupport::getHighPriorityGroupStoreLayoutFact(
    VMIGroupStoreOp op, VMIVRegType valueType, std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<VMIGroupStoreLayoutFact> {
        if (reason)
    ...
    FailureOr<GroupLayoutKey> key = buildGroupLayoutKey(
        ...
        "high-priority group_store layout table has no row for this group size", reason);
    if (failed(key))
        return failure();
...
LogicalResult getGroupReduceAddSupportImpl(
    VMIVRegType sourceType, VMIMaskType maskType, VMIVRegType resultType, int64_t numGroups, std::string* reason)
{
    FailureOr<VMIGroupReduceLayoutFact> fact =
        VMILayoutSupport().getGroupReduceLayoutFactForLayouts(sourceType, maskType, resultType, numGroups, reason);
    if (failed(fact))
        return failure();
    return success();
}

LogicalResult VMILayoutSupport::getGroupReduceAddFSupport(VMIGroupReduceAddFOp op, std::string* reason) const
{
    return getGroupReduceAddSupportImpl(
        cast<VMIVRegType>(op.getSource().getType()), cast<VMIMaskType>(op.getMask().getType()),
        cast<VMIVRegType>(op.getResult().getType()), op.getNumGroupsAttr().getInt(), reason);
}

LogicalResult VMILayoutSupport::getGroupReduceMaxFSupport(VMIGroupReduceMaxFOp op, std::string* reason) const
{
    return getGroupReduceAddSupportImpl(
        cast<VMIVRegType>(op.getSource().getType()), cast<VMIMaskType>(op.getMask().getType()),
        cast<VMIVRegType>(op.getResult().getType()), op.getNumGroupsAttr().getInt(), reason);
...
LogicalResult VMILayoutSupport::getGroupReduceMinFSupport(VMIGroupReduceMinFOp op, std::string* reason) const
{
    return getGroupReduceAddSupportImpl(
        cast<VMIVRegType>(op.getSource().getType()), cast<VMIMaskType>(op.getMask().getType()),
        cast<VMIVRegType>(op.getResult().getType()), op.getNumGroupsAttr().getInt(), reason);
...
LogicalResult VMILayoutSupport::getGroupReduceAddISupport(VMIGroupReduceAddIOp op, std::string* reason) const
{
    return getGroupReduceAddSupportImpl(
        cast<VMIVRegType>(op.getSource().getType()), cast<VMIMaskType>(op.getMask().getType()),
        cast<VMIVRegType>(op.getResult().getType()), op.getNumGroupsAttr().getInt(), reason);
...
LogicalResult VMILayoutSupport::getGroupReduceMaxISupport(VMIGroupReduceMaxIOp op, std::string* reason) const
{
    return getGroupReduceAddSupportImpl(
        cast<VMIVRegType>(op.getSource().getType()), cast<VMIMaskType>(op.getMask().getType()),
        cast<VMIVRegType>(op.getResult().getType()), op.getNumGroupsAttr().getInt(), reason);
...
LogicalResult VMILayoutSupport::getGroupReduceMinISupport(VMIGroupReduceMinIOp op, std::string* reason) const
{
    return getGroupReduceAddSupportImpl(
        cast<VMIVRegType>(op.getSource().getType()), cast<VMIMaskType>(op.getMask().getType()),
        cast<VMIVRegType>(op.getResult().getType()), op.getNumGroupsAttr().getInt(), reason);
...
LogicalResult VMILayoutSupport::getGroupBroadcastSupport(VMIGroupBroadcastOp op, std::string* reason) const
{
    return getGroupBroadcastSupport(
        cast<VMIVRegType>(op.getSource().getType()), cast<VMIVRegType>(op.getResult().getType()),
        op.getNumGroupsAttr().getInt(), reason);
}

LogicalResult VMILayoutSupport::getGroupBroadcastLoadSupport(VMIGroupBroadcastLoadOp op, std::string* reason) const
{
...
LogicalResult VMILayoutSupport::getGroupBroadcastSupport(
    VMIVRegType sourceType, VMIVRegType resultType, int64_t numGroups, std::string* reason) const
{
    return success(succeeded(getGroupBroadcastLayoutFactForLayouts(sourceType, resultType, numGroups, reason)));
}

static LogicalResult getNarrowCastSupport(VMIVRegType sourceType, VMIVRegType resultType, std::string* reason)
{
...
LogicalResult VMILayoutSupport::getTruncFSupport(VMITruncFOp op, std::string* reason) const
{
    return getNarrowCastSupport(
        cast<VMIVRegType>(op.getSource().getType()), cast<VMIVRegType>(op.getResult().getType()), reason);
}

LogicalResult VMILayoutSupport::getExtFSupport(VMIExtFOp op, std::string* reason) const
{
    auto sourceType = cast<VMIVRegType>(op.getSource().getType());
    auto resultType = cast<VMIVRegType>(op.getResult().getType());
...
LogicalResult VMILayoutSupport::getExtSISupport(VMIExtSIOp op, std::string* reason) const
{
    return getExtISupportImpl(op, reason);
}

LogicalResult VMILayoutSupport::getExtUISupport(VMIExtUIOp op, std::string* reason) const
{
    return getExtISupportImpl(op, reason);
}

LogicalResult VMILayoutSupport::getTruncISupport(VMITruncIOp op, std::string* reason) const
{
    return getNarrowCastSupport(
        cast<VMIVRegType>(op.getSource().getType()), cast<VMIVRegType>(op.getResult().getType()), reason);
}

FailureOr<VMIBitcastLayoutFact> VMILayoutSupport::getBitcastLayoutFact(VMIBitcastOp op, std::string* reason) const
{
    ...
    int64_t numGroups = sourceLayout.isGroupSlots() ? sourceLayout.getNumGroups() : 0;
    unsigned sourceElementBits = pto::getPTOStorageElemBitWidth(sourceType.getElementType());
    unsigned resultElementBits = pto::getPTOStorageElemBitWidth(resultType.getElementType());
    ...
    if (sourceElementBits != resultElementBits) {
        ...
        for (const WidthChangingBitcastLayoutPattern& pattern : kWidthChangingBitcastLayoutPatterns) {
...
FailureOr<SmallVector<VMIBitcastLayoutFact, 4>> VMILayoutSupport::getBitcastLayoutFactsForLayout(
    VMIVRegType sourceType, VMIVRegType resultType, VMICastLayoutPort port, VMILayoutAttr layout,
    std::string* reason) const
{
    auto fail = [&](const Twine& message) -> FailureOr<SmallVector<VMIBitcastLayoutFact, 4>> {
        if (reason)
            *reason = message.str();
        return failure();
    };
    ...
    if (sourceElementBits == resultElementBits)
        return SmallVector<VMIBitcastLayoutFact, 4>{VMIBitcastLayoutFact{layout, layout}};

    int64_t numGroups = layout.isGroupSlots() ? layout.getNumGroups() : 0;
    MLIRContext* ctx = sourceType.getContext();
    SmallVector<VMIBitcastLayoutFact, 4> facts;
    for (const WidthChangingBitcastLayoutPattern& pattern : kWidthChangingBitcastLayoutPatterns) {
        VMILayoutAttr candidate = materializeLayoutPattern(ctx, pattern.layout, numGroups);
        if (!candidate)
            continue;
    ...
    if (facts.empty()) {
        if (port == VMICastLayoutPort::Source)
            return fail("requires a legal width-changing bitcast source layout relation");
        return fail("requires a legal width-changing bitcast result layout relation");
    }
    return facts;
}

LogicalResult VMILayoutSupport::getBitcastSupport(VMIBitcastOp op, std::string* reason) const
{
    return getBitcastLayoutFact(op, reason);
}

template <typename OpTy>
static FailureOr<VMIHistogramLayoutFact> getHistogramLayoutFactImpl(
    OpTy op, ArrayRef<HistogramLayoutPattern> patterns, StringRef opName, std::string* reason)
{
    ...
    return fail(
        opName + " acc/source/mask/result layouts do not match a "
                 "histogram layout table row");
}

FailureOr<VMIHistogramLayoutFact> VMILayoutSupport::getVdhistLayoutFact(VMIVdhistOp op, std::string* reason) const
{
    return getHistogramLayoutFactImpl(op, kVdhistLayoutPatterns, "vdhist", reason);
}

FailureOr<VMIHistogramLayoutFact> VMILayoutSupport::getVchistLayoutFact(VMIVchistOp op, std::string* reason) const
{
...
LogicalResult VMILayoutSupport::getVdhistSupport(VMIVdhistOp op, std::string* reason) const
{
    return getVdhistLayoutFact(op, reason);
}

LogicalResult VMILayoutSupport::getVchistSupport(VMIVchistOp op, std::string* reason) const
{
