...
static bool isOddPart(StringRef part) { return part == "ODD" || part == "PART_ODD"; }
...
{
    return dist == "BRC_B8" || dist == "BRC_B16" || dist == "BRC_B32" || dist == "US_B8" || dist == "US_B16" ||
           dist == "E2B_B16" || dist == "E2B_B32";
}
...
static bool isZeroGapLoad(Value value, AlignedUnsignedWidening widening)
{
    ...
    std::optional<StringRef> dist = load.getDist();
    if (!loadType || !dist || getPTOStorageElemBitWidth(loadType.getElementType()) != widening.payloadBits)
        return false;
...
static bool isZeroGapNarrowingVcvt(Value value, AlignedUnsignedWidening widening)
{
    ...
    unsigned inputBits = getPTOStorageElemBitWidth(inputType.getElementType());
    unsigned resultBits = getPTOStorageElemBitWidth(resultType.getElementType());
    if (inputBits != widening.carrierBits || resultBits != widening.payloadBits || inputBits <= resultBits ||
        inputType.getElementCount() * inputBits != resultType.getElementCount() * resultBits)
        return false;

    std::optional<StringRef> part = cvt.getPart();
    if (!part)
        return false;
    return (inputBits == resultBits * 2 && *part == "EVEN") || (inputBits == resultBits * 4 && *part == "P0");
}
...
static bool isCanonicalZeroGapCarrier(Value value, AlignedUnsignedWidening widening)
{
    ...
    unsigned sourceBits = getPTOStorageElemBitWidth(sourceType.getElementType());
    unsigned resultBits = getPTOStorageElemBitWidth(resultType.getElementType());
    if (sourceBits == 0 || resultBits != widening.carrierBits || resultBits != sourceBits * 2 ||
        widening.payloadBits > sourceBits)
        return false;

    if (widening.payloadBits == sourceBits)
        return true;
    return isCanonicalZeroGapCarrier(unpack.getSrc(), {widening.payloadBits, sourceBits});
}

static std::optional<AlignedUnsignedWidening> matchAlignedUnsignedWidening(VcvtOp op)
{
    auto inputType = dyn_cast<VRegType>(op.getInput().getType());
    auto resultType = dyn_cast<VRegType>(op.getResult().getType());
    if (!inputType || !resultType || op.getRndAttr() || op.getSatAttr() || !isAllTrueMask(op.getMask()))
        return std::nullopt;
    ...
    if (!inputElementType || !resultElementType || !inputElementType.isUnsigned() || !resultElementType.isUnsigned())
        return std::nullopt;

    unsigned inputBits = inputElementType.getWidth();
    unsigned resultBits = resultElementType.getWidth();
    if (inputBits >= resultBits || inputType.getElementCount() * inputBits != resultType.getElementCount() * resultBits)
        return std::nullopt;

    std::optional<StringRef> part = op.getPart();
    if (!part)
        return std::nullopt;
    if ((resultBits == inputBits * 2 && *part != "EVEN") || (resultBits == inputBits * 4 && *part != "P0") ||
...
struct CanonicalizeEquivalentPartPattern : public OpRewritePattern<VcvtOp> {
    ...
    LogicalResult matchAndRewrite(VcvtOp op, PatternRewriter& rewriter) const override
    {
        std::optional<StringRef> part = op.getPart();
        if (!part || !isOddPart(*part) || !isNarrowToWideVcvt(op) || !isAllTrueMask(op.getMask()) ||
            !hasEvenOddEquivalentLanes(op.getInput()))
            return failure();

        rewriter.modifyOpInPlace(op, [&] { op.setPartAttr(rewriter.getStringAttr("EVEN")); });
        return success();
    }
};
...
struct FoldZeroGapExtensionPattern : public OpRewritePattern<VcvtOp> {
    ...
    LogicalResult matchAndRewrite(VcvtOp op, PatternRewriter& rewriter) const override
    {
        std::optional<AlignedUnsignedWidening> widening = matchAlignedUnsignedWidening(op);
        if (!widening || !isCanonicalZeroGapCarrier(op.getInput(), *widening))
            return failure();

        Value carrier = stripVbitcasts(op.getInput());
        Value result =
            carrier.getType() == op.getResult().getType() ?
                carrier :
                rewriter.create<VbitcastOp>(op.getLoc(), op.getResult().getType(), op.getInput()).getResult();
        rewriter.replaceOp(op, result);
        return success();
    }
};

struct VPTOOptimizeVcvtPass : public pto::impl::VPTOOptimizeVcvtBase<VPTOOptimizeVcvtPass> {
    void runOnOperation() override
    {
        RewritePatternSet patterns(&getContext());
        patterns.add<CanonicalizeEquivalentPartPattern, FoldZeroGapExtensionPattern>(&getContext());
...
std::unique_ptr<Pass> mlir::pto::createVPTOOptimizeVcvtPass() { return std::make_unique<VPTOOptimizeVcvtPass>(); }
