...
{
    return isa<pto::PartitionTensorViewType, pto::TensorViewType, pto::TileBufType>(type);
}
...
static bool eraseDeadViewBridgeCasts(func::FuncOp func)
{
    ...
    func.walk([&](UnrealizedConversionCastOp castOp) {
        if (!castOp.use_empty() || castOp.getNumOperands() != 1 || castOp.getNumResults() != 1)
            return;
...
static bool eraseDeadMemrefViewOps(func::FuncOp func)
{
    ...
    func.walk([&](Operation* op) {
        if ((isa<memref::SubViewOp>(op) || isa<memref::ReinterpretCastOp>(op)) && op->use_empty())
...
{
    while (eraseDeadViewBridgeCasts(func) || eraseDeadMemrefViewOps(func) || eraseDeadTensorViewOps(func) ||
...
static bool isSCFTileCarrier(Value value)
{
    ...
    return blockArg && isa_and_nonnull<scf::ForOp>(blockArg.getOwner()->getParentOp());
}

static std::optional<TileHandleInfo> resolveTileHandle(Value tileBuf, Operation* user)
{
    if (auto regionResult = dyn_cast<OpResult>(tileBuf)) {
        if (auto fusionRegion = dyn_cast<pto::FusionRegionOp>(regionResult.getOwner())) {
            auto yieldOp = dyn_cast<pto::YieldOp>(fusionRegion.getBody().front().getTerminator());
            if (!yieldOp) {
    ...
    if (auto alloc = tileBuf.getDefiningOp<pto::AllocTileOp>()) {
        ...
        return TileHandleInfo{alloc.getAddr(), alloc.getValidRow(), alloc.getValidCol(), tileTy.getConfigAttr()};
    }
    ...
    if (auto reshape = tileBuf.getDefiningOp<pto::TReshapeOp>()) {
        ...
        return TileHandleInfo{
            sourceInfo->addr, validRow ? validRow : sourceInfo->validRow, validCol ? validCol : sourceInfo->validCol,
...
static std::optional<ViewChain> traceViewChain(Value tensorView, Operation* user)
{
    Value memrefVal;
    ...
    if (isa<MemRefType>(tensorView.getType())) {
        ...
        if (!castOp || castOp.getNumOperands() != 1) {
            user->emitError("FoldTileBufIntrinsics: expected tensor_view to be defined by a "
                            "single-operand builtin.unrealized_conversion_cast");
            return std::nullopt;
        }
        memrefVal = castOp.getOperand(0);
        if (!isa<MemRefType>(memrefVal.getType())) {
            user->emitError("FoldTileBufIntrinsics: expected cast operand to be a memref, got ") << memrefVal.getType();
            return std::nullopt;
        }
    }

    if (auto memrefCastOp = memrefVal.getDefiningOp<UnrealizedConversionCastOp>()) {
    ...
    if (!subviewOp) {
        user->emitError("FoldTileBufIntrinsics: expected memref to be defined by "
                        "memref.subview, got ")
            << (memrefVal.getDefiningOp() ? memrefVal.getDefiningOp()->getName().getStringRef() :
    ...
    if (!rcOp) {
        user->emitError("FoldTileBufIntrinsics: expected subview source to be defined by "
...
static Value getValueOrCreateConstant(OpBuilder& builder, Location loc, OpFoldResult ofr)
{
...
static Value computeResultStride(OpBuilder& builder, Location loc, OpFoldResult rcStride, OpFoldResult svStride)
{
...
static Value computeLinearOffset(
    OpBuilder& builder, Location loc, ArrayRef<OpFoldResult> rcOffsets, ArrayRef<OpFoldResult> svOffsets,
...
static Value projectSCFIfViewResult(
    Value view, PTOViewProjectionKind kind, int64_t dim, pto::PtrType resultPtrType, OpBuilder& builder,
    Operation* user);

static Value resolvePTOViewDim(Value view, int64_t dim, OpBuilder& builder, Operation* user)
{
    view = unwrapPTOViewBridge(view);
    if (Value projected = projectSCFIfViewResult(view, PTOViewProjectionKind::Dim, dim, {}, builder, user))
        return projected;
...
static Value resolvePTOViewStride(Value view, int64_t dim, OpBuilder& builder, Operation* user)
{
    view = unwrapPTOViewBridge(view);
    if (Value projected = projectSCFIfViewResult(view, PTOViewProjectionKind::Stride, dim, {}, builder, user))
        return projected;
    if (auto partition = view.getDefiningOp<pto::PartitionViewOp>())
...
static Value resolvePTOViewAddress(Value view, pto::PtrType resultType, OpBuilder& builder, Operation* user)
{
    view = unwrapPTOViewBridge(view);
    if (Value projected = projectSCFIfViewResult(view, PTOViewProjectionKind::Address, 0, resultType, builder, user))
        return projected;
    if (auto makeView = view.getDefiningOp<pto::MakeTensorViewOp>()) {
    ...
    for (unsigned index = 0; index < partition.getOffsets().size(); ++index) {
        Value stride = resolvePTOViewStride(partition.getSource(), index, builder, user);
        if (!stride)
            return {};
        Value offset = partition.getOffsets()[index];
        Value term = builder.create<arith::MulIOp>(user->getLoc(), offset, stride);
        linearOffset = linearOffset ? builder.create<arith::AddIOp>(user->getLoc(), linearOffset, term) : term;
    }
    ...
    if (!linearOffset)
        return base;
    return builder.create<pto::AddPtrOp>(user->getLoc(), resultType, base, linearOffset);
}

static void cloneBlockWithoutTerminator(Block* source, Block* target, IRMapping& mapping, OpBuilder& builder)
{
    ...
    Operation* user)
{
    ...
    Type projectionType = kind == PTOViewProjectionKind::Address ? Type(resultPtrType) : Type(builder.getIndexType());
    if (!projectionType)
        return {};
    ...
    auto cloneBranch = [&](Block* source, Block* target, scf::YieldOp oldYield, bool& failed) {
        IRMapping mapping;
        ...
        switch (kind) {
            case PTOViewProjectionKind::Dim:
                projection = resolvePTOViewDim(yieldedView, dim, ifBuilder, user);
                break;
            case PTOViewProjectionKind::Stride:
                projection = resolvePTOViewStride(yieldedView, dim, ifBuilder, user);
                break;
            case PTOViewProjectionKind::Address:
                projection = resolvePTOViewAddress(yieldedView, resultPtrType, ifBuilder, user);
                break;
        }
        if (!projection) {
            failed = true;
    ...
    for (auto [oldResult, newResult] :
         llvm::zip_equal(ifOp.getResults(), newIf.getResults().take_front(ifOp.getNumResults())))
...
struct FoldTileBufIntrinsicsPass : public pto::impl::FoldTileBufIntrinsicsBase<FoldTileBufIntrinsicsPass> {
    ...
    void runOnOperation() override
    {
        ...
        if (failed(mode)) {
            func.emitError() << "FoldTileBufIntrinsics: unsupported --fold-mode value '" << foldMode
                             << "' (expected all, shape-only, or addr-only)";
        ...
        if (shouldFoldAddrFamily(*mode)) {
            ...
            for (auto gvsOp : getValidShapeOps) {
                ...
                if (!validShape.empty() && validShape[0] != ShapedType::kDynamic)
                    rowReplacement = builder.create<arith::ConstantIndexOp>(gvsOp.getLoc(), validShape[0]);

                Value colReplacement;
                if (validShape.size() >= 2 && validShape[1] != ShapedType::kDynamic)
                    colReplacement = builder.create<arith::ConstantIndexOp>(gvsOp.getLoc(), validShape[1]);
            ...
            for (auto addrOp : addrOps) {
                if (auto srcMemrefType = dyn_cast<MemRefType>(addrOp.getSrc().getType())) {
                    ...
                    if (auto resultPtrType = dyn_cast<pto::PtrType>(addrOp.getDst().getType())) {
                        builder.setInsertionPoint(addrOp);
                        Value replacement =
                            builder.create<pto::CastPtrOp>(addrOp.getLoc(), resultPtrType, addrOp.getSrc());
                ...
                builder.setInsertionPoint(addrOp);
                Value replacement = builder.create<pto::CastPtrOp>(addrOp.getLoc(), resultPtrType, handleInfo->addr);
                addrOp.getDst().replaceAllUsesWith(replacement);
                addrOp.erase();
            }
        }
        ...
        if (shouldFoldShapeFamily(*mode)) {
            ...
            for (auto rowsOp : rowsOps) {
                ...
                if (vRow != ShapedType::kDynamic) {
                    replacement = builder.create<arith::ConstantIndexOp>(rowsOp.getLoc(), vRow);
                } else {
                    ...
                    if (!replacement) {
                        rowsOp.emitError("tile_valid_rows: dynamic v_row but the materialized tile "
                                         "handle has no valid_row operand");
                        return signalPassFailure();
                    }
                    assert(
                        replacement.getType() == rowsOp.getResult().getType() &&
            ...
            for (auto colsOp : colsOps) {
                ...
                if (vCol != ShapedType::kDynamic) {
                    replacement = builder.create<arith::ConstantIndexOp>(colsOp.getLoc(), vCol);
                } else {
                    ...
                    if (!replacement) {
                        colsOp.emitError("tile_valid_cols: dynamic v_col but the materialized tile "
                                         "handle has no valid_col operand");
                        return signalPassFailure();
                    }
                    assert(
                        replacement.getType() == colsOp.getResult().getType() &&
            ...
            for (auto dimOp : tvDimOps) {
                ...
                if (!getConstIndexValue(dimOp.getDimIndex(), dimIdx)) {
                    dimOp.emitError("FoldTileBufIntrinsics: get_tensor_view_dim requires a constant "
                                    "dim index");
                    return signalPassFailure();
                }

                builder.setInsertionPoint(dimOp);
                if (Value direct = resolvePTOViewDim(dimOp.getTensorView(), dimIdx, builder, dimOp)) {
                ...
                if (dimIdx < 0 || dimIdx >= svTy.getRank()) {
                    dimOp.emitError("FoldTileBufIntrinsics: get_tensor_view_dim dim index out of "
                                    "bounds");
                    return signalPassFailure();
                }
                ...
                if (!svTy.isDynamicDim(dimIdx)) {
                    replacement = builder.create<arith::ConstantIndexOp>(dimOp.getLoc(), svTy.getDimSize(dimIdx));
                } else {
                    replacement =
                        getValueOrCreateConstant(builder, dimOp.getLoc(), chain->subview.getMixedSizes()[dimIdx]);
                }
            ...
            for (auto strideOp : tvStrideOps) {
                ...
                if (!getConstIndexValue(strideOp.getDimIndex(), dimIdx)) {
                    strideOp.emitError("FoldTileBufIntrinsics: get_tensor_view_stride requires a "
                                       "constant dim index");
                    return signalPassFailure();
                }

                builder.setInsertionPoint(strideOp);
                if (Value direct = resolvePTOViewStride(strideOp.getTensorView(), dimIdx, builder, strideOp)) {
                ...
                auto svTy = cast<MemRefType>(chain->subview.getType());
                if (dimIdx < 0 || dimIdx >= svTy.getRank()) {
                    strideOp.emitError("FoldTileBufIntrinsics: get_tensor_view_stride dim index out of "
                                       "bounds");
                    return signalPassFailure();
                }
                ...
                Value replacement = computeResultStride(
                    builder, strideOp.getLoc(), chain->reinterpretCast.getMixedStrides()[dimIdx],
        ...
        if (shouldFoldAddrFamily(*mode)) {
            for (auto addrOp : tvAddrOps) {
                ...
                auto resultPtrType = dyn_cast<pto::PtrType>(addrOp.getDst().getType());
                if (resultPtrType) {
                    if (Value direct = resolvePTOViewAddress(addrOp.getSrc(), resultPtrType, builder, addrOp)) {
                ...
                if (!resultPtrType) {
                    ...
                    addrOp.emitError("FoldTileBufIntrinsics: tensor_view_addr result must be memref "
                                     "or !pto.ptr");
                    return signalPassFailure();
                }

                Value linearOffset = computeLinearOffset(
                    builder, addrOp.getLoc(), chain->reinterpretCast.getMixedOffsets(),
                    chain->subview.getMixedOffsets(), chain->reinterpretCast.getMixedStrides());

                Value basePtr = builder.create<pto::CastPtrOp>(addrOp.getLoc(), resultPtrType, chain->baseMemref);
                Value replacement = linearOffset ? builder.create<pto::AddPtrOp>(
                                                       addrOp.getLoc(), resultPtrType, basePtr, linearOffset) :
                                                   basePtr;
        ...
        SmallVector<UnrealizedConversionCastOp, 8> deadCasts;
        func.walk([&](UnrealizedConversionCastOp castOp) {
            if (castOp.use_empty() && castOp.getNumOperands() == 1 && isa<MemRefType>(castOp.getOperand(0).getType()) &&
                isa<MemRefType, pto::PartitionTensorViewType, pto::TileBufType>(castOp.getResult(0).getType()))
                deadCasts.push_back(castOp);
        });
        for (auto castOp : llvm::reverse(deadCasts))
            castOp.erase();
        ...
        for (auto op : llvm::reverse(setValidShapeOps)) {
            ...
            func.walk([&](pto::GetValidShapeOp reader) {
                if (reader.getSource() == op.getSource() && reader->hasAttr(kTileOpValidShapeReadAttr)) {
        ...
        while (tileDceChanged) {
            ...
            func.walk([&](Operation* op) {
                ...
                if (isa<pto::TReshapeOp, pto::AllocTileOp>(op))
                    ...
                    if (castOp.getNumOperands() == 1 && isa<pto::TileBufType>(castOp.getResult(0).getType()))
                        deadTileOps.push_back(op);
                }
            });
...
std::unique_ptr<Pass> createFoldTileBufIntrinsicsPass() { return std::make_unique<FoldTileBufIntrinsicsPass>(); }
