...
static void mergeSegmentSummary(UncoveredTopLevelSegment& dst, const UncoveredTopLevelSegment& src)
{
    if (!src.firstOp)
        return;
    if (!dst.firstOp)
    ...
    dst.ambiguousTileOps.append(src.ambiguousTileOps.begin(), src.ambiguousTileOps.end());
}

static bool isExplicitSection(Operation* op) { return isa<SectionCubeOp, SectionVectorOp>(op); }

static bool isTileLikeOp(Operation* op)
{
    if (!op)
        return false;
    return isa<OpPipeInterface>(op) && op->getName().getStringRef().starts_with("pto.t");
}
...
static bool isRawSectionCarrierOp(Operation* op)
{
    if (isa<SetFlagDynOp>(op) || isa<WaitFlagDynOp>(op))
        return false;
    return op && isa<VectorMicroOpInterface, CubeMicroOpInterface, MteOpInterface, SetFlagOp, WaitFlagOp, SyncSetOp,
                     SyncWaitOp>(op);
}

static std::optional<InferredSectionKind> classifyTileOpByAddressSpace(Operation* op);
...
static bool isSharedSyncPipe(PIPE pipe) { return pipe == PIPE::PIPE_S || pipe == PIPE::PIPE_MTE2; }

static std::optional<InferredSectionKind> classifyMteOpByAddressSpace(MteOpInterface mteOp)
{
    ...
    if (*destination == AddressSpace::MAT || *destination == AddressSpace::LEFT ||
        *destination == AddressSpace::RIGHT || *destination == AddressSpace::BIAS ||
...
static std::optional<InferredSectionKind> classifyRawSectionCarrierOp(Operation* op)
{
...
static bool isPipeLikeOp(Operation* op) { return op && isa<OpPipeInterface>(op); }

static bool isRawVPTOVectorTransientType(Type type) { return isa<VRegType, MaskType, AlignType>(type); }
...
{
    return funcOp && funcOp->hasAttrOfType<FunctionKernelKindAttr>(FunctionKernelKindAttr::name);
}
...
static std::optional<AddressSpace> getBufferAddressSpace(Type type)
{
    ...
    if (auto memrefType = dyn_cast<MemRefType>(type)) {
        if (auto attr = dyn_cast_or_null<AddressSpaceAttr>(memrefType.getMemorySpace()))
            return attr.getAddressSpace();
        return std::nullopt;
    }
    return std::nullopt;
...
static void collectTileAddressSpaces(Type type, SmallVectorImpl<AddressSpace>& spaces)
{
...
static std::optional<InferredSectionKind> classifyTileSectionByAddressSpace(std::optional<AddressSpace> space)
{
    if (!space)
        return std::nullopt;
...
static std::optional<InferredSectionKind> classifyInternalPipeTileOp(Operation* op)
{
    if (auto push = dyn_cast<TPushOp>(op)) {
        ...
        if (*dirMask == 2)
            return InferredSectionKind::Vector;
        return classifyTileSectionByAddressSpace(getBufferAddressSpace(push.getTile().getType()));
    }
    ...
    if (auto pop = dyn_cast<TPopOp>(op)) {
        ...
        if (*dirMask == 2)
            return InferredSectionKind::Cube;
        return classifyTileSectionByAddressSpace(getBufferAddressSpace(pop.getTile().getType()));
    }
    ...
    if (auto free = dyn_cast<TFreeOp>(op)) {
        ...
        if (!free.getEntry())
            return std::nullopt;
        return classifyTileSectionByAddressSpace(getBufferAddressSpace(free.getEntry().getType()));
    }
...
static std::optional<InferredSectionKind> classifyTileOpByName(Operation* op)
{
    ...
    if (name.starts_with("pto.tmatmul") || name.starts_with("pto.tgemv"))
        return InferredSectionKind::Cube;
    if (name == "pto.talloc_to_aiv" || name == "pto.tpush_to_aic" || name == "pto.tpop_from_aic" ||
        name == "pto.tfree_from_aic")
        return InferredSectionKind::Vector;
    if (name == "pto.talloc_to_aic" || name == "pto.tpush_to_aiv" || name == "pto.tpop_from_aiv" ||
...
static std::optional<InferredSectionKind> classifyTLoadByDestinationAddressSpace(Operation* op)
{
    if (!isa<TLoadOp>(op))
        return std::nullopt;

    auto tload = cast<TLoadOp>(op);
    std::optional<AddressSpace> dstSpace = getBufferAddressSpace(tload.getDst().getType());
    if (!dstSpace)
        return std::nullopt;
...
static std::optional<InferredSectionKind> classifyTStoreBySourceAddressSpace(Operation* op)
{
    if (!isa<TStoreOp>(op))
        return std::nullopt;

    auto tstore = cast<TStoreOp>(op);
    std::optional<AddressSpace> srcSpace = getBufferAddressSpace(tstore.getSrc().getType());
    if (!srcSpace)
        return std::nullopt;
...
static std::optional<InferredSectionKind> classifyTileOp(Operation* op)
{
    ...
    if (std::optional<InferredSectionKind> kind = classifyInternalPipeTileOp(op))
        return kind;
    if (std::optional<InferredSectionKind> kind = classifyTLoadByDestinationAddressSpace(op))
        return kind;
    if (std::optional<InferredSectionKind> kind = classifyTStoreBySourceAddressSpace(op))
        return kind;
...
static void inspectModuleKindOperation(Operation* op, ModuleKindSummary& summary)
{
    ...
    if (isRawSectionCarrierOp(op)) {
        if (std::optional<InferredSectionKind> kind = classifyRawSectionCarrierOp(op)) {
...
static FunctionKindCacheState encodeFunctionKind(std::optional<InferredSectionKind> kind)
{
    if (!kind)
        return FunctionKindCacheState::Unknown;
    return *kind == InferredSectionKind::Vector ? FunctionKindCacheState::Vector : FunctionKindCacheState::Cube;
}

static std::optional<InferredSectionKind> decodeFunctionKind(FunctionKindCacheState state)
{
    switch (state) {
...
static func::CallOp getTransparentWrapperCall(func::FuncOp funcOp)
{
    ...
    if (returnOp.getNumOperands() != callOp.getNumResults())
        return nullptr;
    for (auto [returned, forwarded] : llvm::zip(returnOp.getOperands(), callOp.getResults())) {
...
static std::optional<InferredSectionKind> inferWholeFunctionKind(
    func::FuncOp funcOp, llvm::DenseMap<Operation*, FunctionKindCacheState>& cache)
{
    ...
    if (!inferredKind) {
        if (func::CallOp callOp = getTransparentWrapperCall(funcOp)) {
            auto callee = SymbolTable::lookupNearestSymbolFrom<func::FuncOp>(funcOp, callOp.getCalleeAttr());
...
static void assignModuleKernelKind(ModuleOp module, InferredSectionKind kind)
{
    FunctionKernelKind kernelKind =
        kind == InferredSectionKind::Vector ? FunctionKernelKind::Vector : FunctionKernelKind::Cube;
    module->setAttr(FunctionKernelKindAttr::name, FunctionKernelKindAttr::get(module.getContext(), kernelKind));
}

static void assignFunctionKernelKind(func::FuncOp funcOp, InferredSectionKind kind)
{
    if (!funcOp)
        return;
    ...
    FunctionKernelKind kernelKind =
        kind == InferredSectionKind::Vector ? FunctionKernelKind::Vector : FunctionKernelKind::Cube;
    funcOp->setAttr(FunctionKernelKindAttr::name, FunctionKernelKindAttr::get(funcOp.getContext(), kernelKind));
}
...
static LogicalResult tryAssignWholeModuleKernelKind(ModuleOp module)
{
    ...
    for (func::FuncOp funcOp : defs) {
        if (hasAnySection(funcOp))
            return success();
        std::optional<InferredSectionKind> funcKind = inferWholeFunctionKind(funcOp, cache);
        if (!funcKind)
            return success();
        if (!commonKind) {
            commonKind = funcKind;
...
{
    if (!funcOp || funcOp.isDeclaration() || hasAnySection(funcOp) || hasKnownKernelKindContext(funcOp))
        return success();
...
static void inspectSegmentOperation(Operation* op, UncoveredTopLevelSegment& segment)
{
    if (!op)
        return;
...
static std::optional<InferredSectionKind> inferSegmentKind(const UncoveredTopLevelSegment& segment)
{
...
static void collectUncoveredTopLevelSegments(func::FuncOp funcOp, SmallVectorImpl<UncoveredTopLevelSegment>& segments)
{
    if (!funcOp || funcOp.isDeclaration() || !funcOp.getBody().hasOneBlock())
        return;
    ...
    for (Operation& op : entryBlock.getOperations()) {
        ...
        bool mustSplit = current.containsNestedExplicitSection || opSummary.containsNestedExplicitSection ||
                         !currentKind || !opKind || *currentKind != *opKind;
        if (mustSplit) {
            flushCurrent();
            current = std::move(opSummary);
            continue;
...
static void wrapUncoveredTopLevelSegment(func::FuncOp funcOp, const UncoveredTopLevelSegment& segment)
{
    Block& entryBlock = funcOp.getBody().front();
    ...
    sectionBlock->getOperations().splice(sectionBlock->end(), entryBlock.getOperations(), firstIt, afterLastIt);
}

static LogicalResult emitSegmentInferenceError(func::FuncOp funcOp, const UncoveredTopLevelSegment& segment)
{
    InFlightDiagnostic diag = funcOp.emitOpError("contains an uncovered top-level op segment whose "
...
static LogicalResult emitResidualUncoveredTileSegmentError(func::FuncOp funcOp, const UncoveredTopLevelSegment& segment)
{
    InFlightDiagnostic diag = funcOp.emitOpError("still contains an uncovered top-level op segment after section "
                                                 "normalization");
    ...
    diag << "; first uncovered op segment starts at '"
         << (segment.firstTileCarrierOp ? segment.firstTileCarrierOp : segment.firstOp)->getName().getStringRef()
         << '\'';
    return failure();
}
...
static LogicalResult verifyFunctionHasNoResidualUncoveredTileSegments(func::FuncOp funcOp)
{
    if (hasKnownKernelKindContext(funcOp))
        return success();
...
struct PTONormalizeUncoveredTileSectionsPass
    : public mlir::pto::impl::PTONormalizeUncoveredTileSectionsBase<PTONormalizeUncoveredTileSectionsPass> {
