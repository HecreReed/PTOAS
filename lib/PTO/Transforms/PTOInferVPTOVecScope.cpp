...
using SegmentRematCache = llvm::DenseMap<Value, llvm::DenseMap<Operation*, Value>>;

static VPTOInferenceOpClass classifyOperationForInference(Operation* op);
static LogicalResult buildResultlessScopePlan(
    ArrayRef<Operation*> ops, ResultlessScopePlan& plan, EscapingMovedValue& escapingValue);
static LogicalResult inferVecScopesInRegion(Region& region, MLIRContext* context);

static bool isVecScopeType(Type type) { return isa<pto::VRegType, pto::MaskType, pto::AlignType>(type); }

static bool isPTOOperation(Operation* op) { return op && op->getName().getStringRef().starts_with("pto."); }

static bool isExplicitVectorScopeCarrier(Operation* op) { return isa<pto::VecScopeOp, pto::StrictVecScopeOp>(op); }
...
static bool isForbiddenInsideInferredVectorScope(Operation* op)
{
    ...
    return isa<pto::VbitsortOp, pto::Vmrgsort4Op, pto::GetBlockIdxOp, pto::GetBlockNumOp>(op);
}

static bool isVectorScopeBoundaryOperation(Operation* op) { return isa<pto::BarrierOp, pto::BarrierSyncOp>(op); }
...
static bool requiresVectorScope(Operation* op)
{
    ...
    return hasVecScopeTypedOperandOrResult(op) || isa<pto::MemBarOp, pto::SprclrOp>(op);
}

static bool isAtomicControlFlowCandidate(Operation* op) { return isa<scf::IfOp, scf::ForOp>(op); }
...
static void summarizeNestedRegionForAtomicCluster(Region& region, NestedRegionSummary& summary)
{
...
{
    return llvm::any_of(
        ops, [](Operation* op) { return classifyOperationForInference(op) == VPTOInferenceOpClass::Vector; });
}

static bool isUserInsideCluster(Operation* user, const llvm::SmallPtrSetImpl<Operation*>& ops)
{
...
static bool anyUserIsMoved(Value result, const llvm::SmallPtrSetImpl<Operation*>& movedOps)
{
...
static llvm::SmallPtrSet<Operation*, 16> computeMovedOpsForResultlessScope(ArrayRef<Operation*> ops)
{
    ...
    while (changed) {
        ...
        for (Operation* op : llvm::reverse(ops)) {
            if (movedOps.contains(op) || classifyOperationForInference(op) != VPTOInferenceOpClass::SafeScalar)
                continue;
...
static FailureOr<Operation*> cloneVecScopeProducerForUse(
    Value value, Operation* user, Operation* logicalScopeAnchor, SegmentRematCache& cache, MLIRContext* context,
    llvm::DenseMap<Operation*, Operation*>& clones)
{
    ...
    for (Value operand : producer->getOperands()) {
        ...
        FailureOr<Operation*> clonedOperandProducer =
            cloneVecScopeProducerForUse(operand, user, logicalScopeAnchor, cache, context, clones);
        if (failed(clonedOperandProducer))
            return failure();

        auto operandResult = cast<OpResult>(operand);
        mapping.map(operand, (*clonedOperandProducer)->getResult(operandResult.getResultNumber()));
    }
...
static void collectGreedyLogicalScopePlans(ArrayRef<Operation*> ops, SmallVectorImpl<LogicalScopePlan>& plans)
{
    plans.clear();
    ...
    for (size_t begin = 0; begin < ops.size();) {
        ...
        for (size_t end = ops.size(); end > begin; --end) {
            ...
            if (succeeded(buildResultlessScopePlan(candidate, plan, candidateEscapingValue))) {
                bestEnd = end;
...
static void assignLogicalScopeAnchorsForCluster(
    ArrayRef<Operation*> ops, llvm::DenseMap<Operation*, Operation*>& logicalScopeAnchors)
{
...
static llvm::DenseMap<Operation*, Operation*> computeLogicalScopeAnchors(Block& block)
{
...
static LogicalResult rematerializeEscapingValueForUserSegments(
    Value value, const llvm::SmallPtrSetImpl<Operation*>& movedOps, Block& block, SegmentRematCache& cache,
    MLIRContext* context)
{
    auto result = dyn_cast<OpResult>(value);
    if (!result)
    ...
    llvm::DenseMap<Operation*, Operation*> logicalScopeAnchors = computeLogicalScopeAnchors(block);
    ...
    for (auto& entry : usesBySegment) {
        ...
        if (!replacement) {
            ...
            FailureOr<Operation*> clonedProducer =
                cloneVecScopeProducerForUse(value, logicalScopeAnchor, logicalScopeAnchor, cache, context, clones);
...
static bool findEscapingMovedResult(
    const llvm::SmallPtrSetImpl<Operation*>& movedOps, EscapingMovedValue& escapingValue)
{
...
static LogicalResult emitEscapingVectorScopeValueError(const EscapingMovedValue& escapingValue)
{
    Operation* producer = escapingValue.producer;
    if (!producer)
        return failure();

    InFlightDiagnostic diag = producer->emitOpError() << "cannot infer resultless pto.vecscope because "
    ...
    ArrayRef<Operation*> ops, ResultlessScopePlan& plan, EscapingMovedValue& escapingValue)
{
    ...
    llvm::SmallPtrSet<Operation*, 16> movedOps = computeMovedOpsForResultlessScope(ops);
    if (movedOps.empty())
        return failure();
...
static void wrapCluster(const ResultlessScopePlan& plan, MLIRContext* context)
{
    ...
    for (Operation* op : plan.moveOps) {
        scopeBody.getOperations().splice(scopeBody.end(), parentBlock->getOperations(), Block::iterator(op));
    }
}

static LogicalResult wrapGreedySubclusters(ArrayRef<Operation*> ops, MLIRContext* context)
{
    for (size_t begin = 0; begin < ops.size();) {
        size_t bestEnd = begin;
        ResultlessScopePlan bestPlan;
        ...
        if (bestEnd == begin) {
            if (classifyOperationForInference(ops[begin]) == VPTOInferenceOpClass::Vector && sawEscapingMovedResult &&
                escapingValue.requiresDiagnostic)
...
static FailureOr<bool> fixOneEscapingSubcluster(
    ArrayRef<Operation*> ops, SegmentRematCache& cache, MLIRContext* context)
{
    for (size_t begin = 0; begin < ops.size();) {
        size_t bestEnd = begin;
        EscapingMovedValue escapingValue;
        ...
        for (size_t end = ops.size(); end > begin; --end) {
            ...
            EscapingMovedValue candidateEscapingValue;
            if (succeeded(buildResultlessScopePlan(candidate, ignoredPlan, candidateEscapingValue))) {
                bestEnd = end;
                break;
            }
        ...
        if (bestEnd == begin) {
            if (classifyOperationForInference(ops[begin]) == VPTOInferenceOpClass::Vector && sawEscapingMovedResult &&
                ...
                ArrayRef<Operation*> escapingCandidate =
                    ops.slice(escapingCandidateBegin, escapingCandidateEnd - escapingCandidateBegin);
                llvm::SmallPtrSet<Operation*, 16> movedOps = computeMovedOpsForResultlessScope(escapingCandidate);
...
static LogicalResult repairEscapingSubclusters(Block& block, MLIRContext* context)
{
    ...
    for (unsigned iteration = 0; iteration < kMaxRepairIterations; ++iteration) {
        ...
        auto flush = [&]() -> FailureOr<bool> {
            FailureOr<bool> changed = fixOneEscapingSubcluster(pending, cache, context);
            pending.clear();
            return changed;
        };
...
struct PTOInferVPTOVecScopePass : public pto::impl::PTOInferVPTOVecScopeBase<PTOInferVPTOVecScopePass> {
