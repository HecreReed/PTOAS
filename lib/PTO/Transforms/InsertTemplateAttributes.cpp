...
{
    auto memorySpace = dyn_cast_or_null<pto::AddressSpaceAttr>(tileType.getMemorySpace());
    return memorySpace ? stringifyMemorySpace(memorySpace.getAddressSpace()) : "ub";
}

static std::string getMemorySpaceString(MemRefType memrefType)
{
    auto memorySpace = dyn_cast_or_null<pto::AddressSpaceAttr>(memrefType.getMemorySpace());
    return memorySpace ? stringifyMemorySpace(memorySpace.getAddressSpace()) : "gm";
}

static std::string getMemorySpaceString(pto::PartitionTensorViewType) { return "gm"; }
...
static void recordStaticSizes(ArrayRef<OpFoldResult> values, SmallVectorImpl<int64_t>& out)
{
    out.clear();
...
static SmallVector<int64_t> combineSubviewStrides(ArrayRef<int64_t> baseStrides, ArrayRef<OpFoldResult> steps)
{
    ...
    for (auto [baseStride, step] : llvm::zip(baseStrides, steps)) {
        ...
        if (baseStride == ShapedType::kDynamic || stepValue == ShapedType::kDynamic) {
...
static std::optional<pto::Layout> resolveViewLayout(Value value)
{
    ...
    while (definingOp) {
        ...
        if (auto reinterpret = dyn_cast<memref::ReinterpretCastOp>(definingOp)) {
...
static void populatePTOViewShapeAndStrides(
    Value value, SmallVectorImpl<int64_t>& shape, SmallVectorImpl<int64_t>& strides)
{
    if (!value)
        return;
    ...
    if (auto part = value.getDefiningOp<pto::PartitionViewOp>()) {
        if (shape.empty()) {
            ...
            if (shape.empty()) {
                auto partTy = dyn_cast<pto::PartitionTensorViewType>(part.getResult().getType());
                if (partTy)
        ...
        populatePTOViewShapeAndStrides(part.getSource(), sourceShape, sourceStrides);
...
static void populateViewShapeAndStrides(Value value, SmallVectorImpl<int64_t>& shape, SmallVectorImpl<int64_t>& strides)
{
    if (!value)
        return;
    ...
    if (auto memrefType = dyn_cast<MemRefType>(value.getType())) {
        ...
        if (strides.empty()) {
            ...
            (void)mlir::pto::getPTOMemRefStridesAndOffset(memrefType, strides, offset);
        }
    }
}
...
static bool tryAppendPrecisionType(Operation* op, SmallVectorImpl<std::pair<std::string, std::string>>& attrs)
{
    auto typed = dyn_cast<OpT>(op);
    if (!typed)
        return false;
    attrs.emplace_back("precisionType", getPrecisionTypeString(typed.getPrecisionType()).str());
    return true;
}

static void appendOpContextAttrs(Operation* op, SmallVectorImpl<std::pair<std::string, std::string>>& attrs)
{
    ...
    if (auto tcmp = dyn_cast<pto::TCmpOp>(op)) {
        if (auto cmpModeAttr = tcmp.getCmpModeAttr())
            attrs.emplace_back("cmp_mode", stringifyCmpMode(cmpModeAttr.getValue()).str());
    }
    ...
    if (auto tmrgsort = dyn_cast<pto::TMrgSortOp>(op))
        attrs.emplace_back("exhausted", tmrgsort.getExhausted() ? "1" : "0");
    if (auto tgather = dyn_cast<pto::TGatherOp>(op)) {
        if (auto maskPatternAttr = tgather.getMaskPatternAttr()) {
            attrs.emplace_back("mask_pattern", stringifyMaskPattern(maskPatternAttr.getValue()).str());
        }
    ...
    (void)(tryAppendPrecisionType<pto::TExpOp>(op, attrs) || tryAppendPrecisionType<pto::TLogOp>(op, attrs) ||
           tryAppendPrecisionType<pto::TSqrtOp>(op, attrs) || tryAppendPrecisionType<pto::TRecipOp>(op, attrs) ||
           tryAppendPrecisionType<pto::TRsqrtOp>(op, attrs) || tryAppendPrecisionType<pto::TDivOp>(op, attrs) ||
           tryAppendPrecisionType<pto::TDivSOp>(op, attrs) || tryAppendPrecisionType<pto::TRowExpandDivOp>(op, attrs) ||
...
static void appendTileOperandSpecJson(std::string& json, pto::TileBufType tileType)
{
    ...
    appendJsonIntArray(json, validShape.empty() ? tileType.getShape() : validShape);
    json += ",\"memory_space\":\"";
...
static void appendViewOperandSpecJson(std::string& json, Value operand, MemRefType memrefType)
{
...
static void appendViewOperandSpecJson(std::string& json, Value operand, pto::PartitionTensorViewType viewType)
{
...
static void appendVectorOperandSpecJson(std::string& json, VectorType vectorType)
{
...
static std::optional<std::string> buildOperandSpecsJson(Operation* operation)
{
    std::string json = "[";
    for (auto [index, operand] : llvm::enumerate(operation->getOperands())) {
        ...
        if (auto tileType = dyn_cast<pto::TileBufType>(type)) {
            if (getDtypeString(tileType.getElementType()).empty()) {
                operation->emitError("InsertTemplateAttributes encountered an unsupported tile dtype");
                return std::nullopt;
            }
        ...
        if (auto memrefType = dyn_cast<MemRefType>(type)) {
            if (getDtypeString(memrefType.getElementType()).empty()) {
                operation->emitError("InsertTemplateAttributes encountered an unsupported view dtype");
                return std::nullopt;
            }
        ...
        if (auto ptrType = dyn_cast<pto::PtrType>(type)) {
            if (getDtypeString(ptrType.getElementType()).empty()) {
                operation->emitError("InsertTemplateAttributes encountered an unsupported pointer dtype");
                return std::nullopt;
            }
        ...
        if (auto vectorType = dyn_cast<VectorType>(type)) {
            if (getDtypeString(vectorType.getElementType()).empty()) {
                operation->emitError("InsertTemplateAttributes encountered an unsupported vector dtype");
                return std::nullopt;
            }
        ...
        operation->emitError("InsertTemplateAttributes encountered an unsupported operand type ") << type;
        return std::nullopt;
    }
    json += "]";
    return json;
...
static std::optional<std::string> getTargetArch(Operation* operation)
{
    auto module = operation->getParentOfType<ModuleOp>();
    if (!module) {
        operation->emitError("InsertTemplateAttributes requires a parent module");
        return std::nullopt;
    }

    for (ModuleOp current = module; current; current = current->getParentOfType<ModuleOp>()) {
        if (auto target = current->getAttrOfType<StringAttr>("pto.target_arch"))
            return target.getValue().str();
    }

    operation->emitError("InsertTemplateAttributes requires pto.target_arch");
    return std::nullopt;
}

static std::optional<std::string> invokeMetadataHelper(
    Operation* operation, StringRef pythonExe, StringRef daemonSocketPath, StringRef tileLibPkgPath,
    StringRef daemonHelperModule)
{
    auto pythonPath = pto::resolvePythonExecutable(pythonExe);
    if (!pythonPath) {
        operation->emitError("InsertTemplateAttributes cannot find Python '") << pythonExe << "'";
        return std::nullopt;
    }
    ...
    int outputFd;
    if (auto error = llvm::sys::fs::createTemporaryFile("tilelib_metadata", "json", outputFd, outputPath)) {
    ...
    int errorFd;
    if (auto error = llvm::sys::fs::createTemporaryFile("tilelib_metadata", "err", errorFd, errorPath)) {
    ...
    int result = llvm::sys::ExecuteAndWait(
        *pythonPath, args, hasPythonPath ? std::optional<llvm::ArrayRef<StringRef>>(environment) : std::nullopt,
    ...
    if (result != 0) {
        ...
        operation->emitError("InsertTemplateAttributes metadata RPC failed: ") << detail;
        return std::nullopt;
    }
    ...
    llvm::sys::fs::remove(errorPath);
    if (!output) {
        operation->emitError("InsertTemplateAttributes cannot read metadata output");
        return std::nullopt;
    }
    return (*output)->getBuffer().str();
}

static FailureOr<ArrayAttr> parseCandidateAttributes(Operation* operation, StringRef metadataJson)
{
    auto parsed = llvm::json::parse(metadataJson);
    if (!parsed) {
        llvm::consumeError(parsed.takeError());
        operation->emitError("InsertTemplateAttributes received invalid metadata JSON");
        return failure();
    }
    ...
    for (const auto& entry : *candidates) {
        ...
        if (!metadata) {
            operation->emitError("InsertTemplateAttributes candidate metadata must be an object");
            return failure();
        }
        ...
        if (!name || !loopDepth || !postUpdate || !tail) {
            operation->emitError("InsertTemplateAttributes candidate metadata is missing name, "
                                 "loop_depth, is_post_update, or has_tail");
            return failure();
        }
        if (!id && candidates->size() != 1) {
            operation->emitError("InsertTemplateAttributes requires an id for every "
    ...
    llvm::sort(parsedCandidates, [](const CandidateMetadata& left, const CandidateMetadata& right) {
        if (left.id != right.id)
    ...
    for (auto [index, candidate] : llvm::enumerate(parsedCandidates)) {
        if (index != 0 && candidate.id == parsedCandidates[index - 1].id) {
            operation->emitError("InsertTemplateAttributes candidate ids must be unique");
            return failure();
        }
    }
    ...
    for (const CandidateMetadata& candidate : parsedCandidates) {
        attributes.push_back(DictionaryAttr::get(
            operation->getContext(),
            {
                ...
                builder.getNamedAttr("loop_depth", builder.getI64IntegerAttr(candidate.loopDepth)),
                builder.getNamedAttr("postupdate", builder.getI64IntegerAttr(candidate.postUpdate ? 1 : 0)),
                builder.getNamedAttr("tail", builder.getI64IntegerAttr(candidate.tail ? 1 : 0)),
            }));
    }
    return builder.getArrayAttr(attributes);
}

struct InsertTemplateAttributesPass : public pto::impl::InsertTemplateAttributesBase<InsertTemplateAttributesPass> {
    ...
    void runOnOperation() override
    {
        ...
        if (daemonSocketPath.empty()) {
            module.emitError("InsertTemplateAttributes requires a PTODSL daemon socket");
            return signalPassFailure();
        }

        for (Operation* operation : tileOperations) {
            auto metadata =
                invokeMetadataHelper(operation, pythonExe, daemonSocketPath, tileLibPkgPath, daemonHelperModule);
            if (!metadata)
                return signalPassFailure();
...
std::unique_ptr<Pass> createInsertTemplateAttributesPass() { return std::make_unique<InsertTemplateAttributesPass>(); }
