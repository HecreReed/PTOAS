...
void SyncEventIdAllocation::SetEventId(SyncOperation* sync)
{
    ...
    } else if (reallocatedPipePair.count(ScopePair(sync)) && (canAllocaEventId.size() < idSize)) {
...
SmallVector<int> SyncEventIdAllocation::UpdateBlockAvailableEventId(
    SyncOperation* sync, SmallVector<bool> eventIdLifetimeAvailableStatus, size_t eventIdNum)
{
    SmallVector<int> canAllocaEventId;
    size_t idSize = static_cast<size_t>(sync->eventIdNum);
...
SmallVector<int> SyncEventIdAllocation::GetAvailableEventId(
    SyncOperation* sync, SmallVector<bool> eventIdLifetimeAvailableStatus, SmallVector<bool> eventIdIdleStatus,
    size_t eventIdNum)
{
    SmallVector<int> canAllocaEventId;
    size_t idSize = static_cast<size_t>(sync->eventIdNum);
...
int SyncEventIdAllocation::ScopePair(PipelineType srcPipe, PipelineType dstPipe) const
{
...
void SyncEventIdAllocation::SetUseEventID(
    unsigned int begin, unsigned int end, int scopePair, unsigned int eventId, size_t poolSize)
{
    assert(begin < end);
...
void SyncEventIdAllocation::SeedHiddenMacroEventIds(
    const llvm::SmallSet<int, kReallocatedPipePairInlineCapacity>* scopeFilter)
{
