...
static bool isLocalBufferOverlapCrossRoot(const BaseMemInfo* a, const BaseMemInfo* b)
{
...
bool MemoryDependentAnalyzer::MemAlias(const BaseMemInfo* a, const BaseMemInfo* b)
{
    ...
    if (isTraceEnabled())
        llvm::errs() << "      -> Cross-root overlap check: " << (crossRootOverlap ? "true" : "false") << "\n";
    return crossRootOverlap;
}

bool MemoryDependentAnalyzer::isGMBufferOverlap(const BaseMemInfo* a, const BaseMemInfo* b)
{
    if (a->baseAddresses.empty() || b->baseAddresses.empty())
        return true;
    if (a->rootBuffer != b->rootBuffer) {
...
bool MemoryDependentAnalyzer::isBufferAddressRangeOverlap(const BaseMemInfo* a, const BaseMemInfo* b)
{
...
bool MemoryDependentAnalyzer::isBufferOverlap(const BaseMemInfo* a, const BaseMemInfo* b, int aIndex, int bIndex)
{
