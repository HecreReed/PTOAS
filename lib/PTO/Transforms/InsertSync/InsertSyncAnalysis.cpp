...
static std::optional<RepeatAccessShape> getKnownRepeatAccessShapeFromType(Type ty)
{
    if (auto tileTy = dyn_cast<TileBufType>(ty)) {
        ...
        if (fullShape.size() != 2 || validShape.size() != 2)
            return std::nullopt;
        ...
        return RepeatAccessShape{
            SmallVector<int64_t, 2>{fullShape[0], fullShape[1]}, SmallVector<int64_t, 2>{validShape[0], validShape[1]},
...
void InsertSyncAnalysis::MergeAlreadySync(
    SyncRecordList& syncRecordList, const SyncRecordList& syncRecordIfList, const SyncRecordList& syncRecordElseList)
{
    for (size_t bufferIdx = 0; bufferIdx < syncRecordList.size(); bufferIdx++) {
...
static bool isForwardDepDroppableBySlotAffine(const BaseMemInfo* a, const BaseMemInfo* b)
{
    if (!a || !b)
        return false;
    ...
    if (!slotA || !slotB)
        return false;
    return compareSlotSSA(slotA, slotB, static_cast<uint32_t>(n)) == SlotRelation::kDisjoint;
}
    ...
    const std::optional<unsigned>& forEndIndex)
{
    ...
    if (!forEndIndex.has_value()) {
        auto isDroppable = [](const std::pair<const BaseMemInfo*, const BaseMemInfo*>& pair) {
...
void InsertSyncAnalysis::UpdateAlreadySync(
    const SyncOps& syncVector, SyncRecordList& syncRecordList, const PipelineType nowPipeValue)
{
...
void InsertSyncAnalysis::UpdateSyncRecord(const SyncOperation* sync, SyncRecord& syncRecord, PipelineType nowPipeValue)
{
...
void InsertSyncAnalysis::UpdateSyncRecordInfo(CompoundInstanceElement* frontCompound, SyncRecordList& syncRecordList)
{
    (void)frontCompound;
...
SmallVector<Value> InsertSyncAnalysis::GetMemInfoBuffers(const DepBaseMemInfoPairVec& depBaseMemInfosVec)
{
...
int InsertSyncAnalysis::GetEventIdNum(const DepBaseMemInfoPairVec& depBaseMemInfosVec)
{
