...
static uint64_t getStaticBufferSizeInBytes(ArrayRef<int64_t> shape, Type elementType)
{
...
{
    if (auto attr = dyn_cast_or_null<pto::AddressSpaceAttr>(type.getMemorySpace()))
        return attr.getAddressSpace();
    return pto::AddressSpace::MAT;
}

static void appendUniqueMemInfo(
    SmallVectorImpl<std::unique_ptr<BaseMemInfo>>& infos, std::unique_ptr<BaseMemInfo> candidate)
{
    if (!candidate)
        return;
    if (llvm::any_of(infos, [&](const std::unique_ptr<BaseMemInfo>& info) { return info && *info == *candidate; }))
        return;
...
static func::FuncOp lookupSyncHelper(func::CallOp callOp)
{
    ...
    if (!callee)
        return {};
    if (!callee->hasAttr("pto.tileop.helper") && !callee->hasAttr("pto.ptodsl.subkernel_helper"))
        return {};
    return callee;
}
...
{
    if (auto roleAttr = callee->getAttrOfType<mlir::StringAttr>("pto.ptodsl.subkernel_helper")) {
        return llvm::StringSwitch<std::optional<pto::PipelineType>>(roleAttr.getValue())
    ...
    if (!kindAttr)
        return std::nullopt;
    return llvm::StringSwitch<std::optional<pto::PipelineType>>(kindAttr.getValue())
        .Case("cube", pto::PipelineType::PIPE_M)
...
{
    return isa<pto::PtrType, pto::TileBufType, pto::TensorViewType, pto::PartitionTensorViewType>(type);
}

static pto::TCoreType getSyncHelperCoreType(pto::PipelineType pipe)
{
    return pipe == pto::PipelineType::PIPE_M ? pto::TCoreType::CUBE : pto::TCoreType::VECTOR;
}
...
void PTOIRTranslator::UpdateKernelArgMemInfo()
{
    for (Value funcArg : func_.getArguments()) {
        ...
        if (auto ptrType = dyn_cast<pto::PtrType>(argType)) {
            ...
            sizeInBytes = getStaticBufferSizeInBytes(viewType.getShape(), viewType.getElementType());
        } else if (auto viewType = dyn_cast<pto::PartitionTensorViewType>(argType)) {
            space = pto::AddressSpace::GM;
            sizeInBytes = getStaticBufferSizeInBytes(viewType.getShape(), viewType.getElementType());
        } else {
            continue;
        ...
        buffer2MemInfoMap_[funcArg].emplace_back(
            std::make_unique<BaseMemInfo>(funcArg, funcArg, space, std::move(baseAddresses), sizeInBytes));
    }
}
...
LogicalResult PTOIRTranslator::UpdateAllocMultiTileOpMemInfo(pto::AllocMultiTileOp op)
{
    ...
    pto::AddressSpace space = pto::AddressSpace::MAT;
    if (auto attr = dyn_cast_or_null<pto::AddressSpaceAttr>(slotType.getMemorySpace()))
        space = attr.getAddressSpace();

    SmallVector<uint64_t> addresses;
    bool hasKnownAddresses = false;
    if (auto planned = op->getAttrOfType<DenseI64ArrayAttr>(pto::kPtoMultiBufferAddrsAttrName)) {
    ...
    if (addresses.empty()) {
        return op.emitError("requires planner-assigned slot addresses or a constant level3 base");
    }

    auto info = std::make_unique<BaseMemInfo>(
        result, result, space, std::move(addresses), slotBytes, hasKnownAddresses && isLocalAddressSpace(space));
...
LogicalResult PTOIRTranslator::UpdateDeclareTileOpMemInfo(pto::DeclareTileOp op)
{
    ...
    pto::AddressSpace space = pto::AddressSpace::MAT;
    if (auto attr = dyn_cast_or_null<pto::AddressSpaceAttr>(tileType.getMemorySpace()))
        space = attr.getAddressSpace();

    auto info = std::make_unique<BaseMemInfo>(result, result, space, SmallVector<uint64_t>{0}, sizeInBytes);
    buffer2MemInfoMap_[result].emplace_back(info->clone());
    return success();
}

LogicalResult PTOIRTranslator::UpdateDeclareGlobalOpMemInfo(pto::DeclareGlobalOp op)
{
    ...
    bool isStatic = llvm::all_of(shape, [](int64_t dim) { return dim != ShapedType::kDynamic; });
    if (isStatic) {
        int64_t elemSize = static_cast<int64_t>(pto::getPTOStorageElemByteSize(tensorViewType.getElementType()));
        if (elemSize == 0)
            return failure();
    ...
    auto newMemInfo =
        std::make_unique<BaseMemInfo>(res, res, pto::AddressSpace::GM, SmallVector<uint64_t>{0}, sizeInBytes);
...
void PTOIRTranslator::UpdatePTOOpInfoWithPipeline(Operation* op, pto::PipelineType pipe, bool skipIfNoMemInfo)
{
...
void PTOIRTranslator::MakeMacroCompound(
    Operation* op, PipelineType pipe, ValueRange defValues, ValueRange useValues, int macroPhaseId)
{
    SmallVector<const BaseMemInfo*> defVec;
    SmallVector<const BaseMemInfo*> useVec;
    ...
    auto compoundElement =
        std::make_unique<CompoundInstanceElement>(index, std::move(defVec), std::move(useVec), pipe, op->getName());
    compoundElement->elementOp = op;
    compoundElement->macroOpInstanceId = macroPhaseId;
    compoundElement->compoundCoreType = pipe == PipelineType::PIPE_M ? pto::TCoreType::CUBE : pto::TCoreType::VECTOR;
    syncIR_.emplace_back(std::move(compoundElement));
    index++;
}
...
void PTOIRTranslator::UpdateMacroOpInfo(Operation* op)
{
    ...
    for (const auto& phase : model->phases) {
        MakeMacroCompound(op, phase.pipe, ValueRange(phase.defValues), ValueRange(phase.useValues), phase.phaseId);
    }
}
...
void PTOIRTranslator::UpdateHelperCallInfo(func::CallOp callOp)
{
    ...
    auto compoundElement = std::make_unique<CompoundInstanceElement>(index, defVec, useVec, *pipe, callOp->getName());
...
void PTOIRTranslator::UpdateMultiTileGetAliasBufferInfo(pto::MultiTileGetOp op)
{
    UpdateSlotSelectedAliasBufferInfo(op.getResult(), op.getSource(), op.getSlot());
}

void PTOIRTranslator::UpdateSlotSelectedAliasBufferInfo(Value result, Value source, Value slot)
{
    if (!result || !source)
        return;
    if (!buffer2MemInfoMap_.contains(source))
    ...
    for (auto& parentInfo : buffer2MemInfoMap_[source]) {
        ...
        if (isConstSlot && constSlotIdx >= 0 && constSlotIdx < static_cast<int64_t>(newInfo->baseAddresses.size()) &&
            newInfo->baseAddresses.size() > 1) {
            uint64_t pickAddr = newInfo->baseAddresses[static_cast<size_t>(constSlotIdx)];
...
void PTOIRTranslator::printMemInfoList(
    llvm::raw_ostream& os, const SmallVector<const BaseMemInfo*>& list, AsmState& state)
{
    os << "[";
    bool first = true;
