...
void addPhase(SyncMacroModel& model, PipelineType pipe, ValueRange defValues, ValueRange useValues)
{
    model.phases.push_back(SyncMacroPhase{
        static_cast<unsigned>(model.phases.size()), pipe, SmallVector<Value>(defValues.begin(), defValues.end()),
        SmallVector<Value>(useValues.begin(), useValues.end())});
}

void addHiddenEvent(SyncMacroModel& model, PipelineType srcPipe, PipelineType dstPipe, ArrayRef<unsigned> eventIds)
{
    model.hiddenEvents.push_back(
        SyncMacroHiddenEvent{srcPipe, dstPipe, SmallVector<unsigned>(eventIds.begin(), eventIds.end())});
}

void addBidirectionalHiddenEvent(
    SyncMacroModel& model, PipelineType firstPipe, PipelineType secondPipe, ArrayRef<unsigned> eventIds)
{
...
std::optional<SyncMacroModel> getP2PCommSyncMacroModel(Operation* op)
{
    ...
    addBidirectionalHiddenEvent(
        model, PipelineType::PIPE_MTE2, PipelineType::PIPE_MTE3, getSequentialEventIds(laneCount));
    return model;
}
...
std::optional<SyncMacroModel> getCollectiveCommSyncMacroModel(Operation* op)
{
    ...
    if (auto tgather = dyn_cast<pto::CommTGatherOp>(op)) {
        ...
        SmallVector<Value> staging = getPingPongValues(tgather.getPing(), tgather.getPong());
        addPhase(model, PipelineType::PIPE_MTE2, staging, tgather.getGroup());
        addPhase(model, PipelineType::PIPE_MTE3, ValueRange{tgather.getDst()}, staging);
        ...
        SmallVector<Value> staging = getPingPongValues(tscatter.getPing(), tscatter.getPong());
        addPhase(model, PipelineType::PIPE_MTE2, staging, ValueRange{tscatter.getSrc()});
        addPhase(model, PipelineType::PIPE_MTE3, tscatter.getGroup(), staging);
        ...
        // destination through MTE3, using ping/pong as staging tiles.
        SmallVector<Value> staging = getPingPongValues(tbroadcast.getPing(), tbroadcast.getPong());
        addPhase(model, PipelineType::PIPE_MTE2, staging, ValueRange{tbroadcast.getSrc()});
        addPhase(model, PipelineType::PIPE_MTE3, tbroadcast.getGroup(), staging);
        ...
        SmallVector<Value> recvStaging = getPingPongValues(treduce.getRecvPing(), treduce.getRecvPong());
        ...
        addPhase(model, PipelineType::PIPE_V, ValueRange{treduce.getAcc()}, reduceUses);
        addPhase(model, PipelineType::PIPE_MTE3, ValueRange{treduce.getDst()}, ValueRange{treduce.getAcc()});
    } else {
        return std::nullopt;
    }

    SmallVector<unsigned> eventIds = getSequentialEventIds(laneCount);
    addBidirectionalHiddenEvent(model, PipelineType::PIPE_MTE2, PipelineType::PIPE_MTE3, eventIds);
    if (isa<pto::TReduceOp>(op)) {
        addHiddenEvent(model, PipelineType::PIPE_MTE2, PipelineType::PIPE_V, eventIds);
        addHiddenEvent(model, PipelineType::PIPE_V, PipelineType::PIPE_MTE2, eventIds);
        addHiddenEvent(model, PipelineType::PIPE_V, PipelineType::PIPE_MTE3, eventIds);
    }
...
std::optional<SyncMacroModel> getTScatterSyncMacroModel(pto::TScatterOp op)
{
    ...
    addPhase(model, PipelineType::PIPE_S, ValueRange{op.getDst()}, ValueRange{op.getSrc(), op.getIndexes()});
    addHiddenEvent(model, PipelineType::PIPE_V, PipelineType::PIPE_S, ArrayRef<unsigned>{0});
    return model;
}
...
std::optional<SyncMacroModel> getTGatherSyncMacroModel(pto::TGatherOp op)
{
    ...
    addPhase(model, PipelineType::PIPE_V, ValueRange{tmp}, ValueRange{op.getSrc()});
    addPhase(model, PipelineType::PIPE_S, ValueRange{tmp}, ValueRange{});
    addPhase(model, PipelineType::PIPE_V, ValueRange{op.getDst()}, ValueRange{tmp});
    addPhase(model, PipelineType::PIPE_S, ValueRange{cdst}, ValueRange{});
    addHiddenEvent(model, PipelineType::PIPE_S, PipelineType::PIPE_V, ArrayRef<unsigned>{0});
    return model;
}

// Resolve the PTO address space of an MGatherOp tile operand type.
static std::optional<pto::AddressSpace> getMGatherOperandAddressSpace(::mlir::Type ty)
{
    if (auto tb = ::mlir::dyn_cast<::mlir::pto::TileBufType>(ty)) {
        if (auto as = ::mlir::dyn_cast_or_null<::mlir::pto::AddressSpaceAttr>(tb.getMemorySpace()))
...
static std::optional<SmallVector<int64_t>> getMGatherOperandShapeFromType(::mlir::Type ty, bool useValidShape = true)
{
    if (auto tb = ::mlir::dyn_cast<::mlir::pto::TileBufType>(ty)) {
        ArrayRef<int64_t> shape = useValidShape ? tb.getValidShape() : tb.getShape();
...
static std::optional<SmallVector<int64_t>> getMGatherOperandShape(Value value, bool useValidShape = true)
{
...
std::optional<SyncMacroModel> getMGatherSyncMacroModel(pto::MGatherOp op)
{
    ...
    if (isGm2L1) {
        ...
        addPhase(model, PipelineType::PIPE_S, ValueRange(sDefs), ValueRange{op.getIdx()});
        if (coalesce == pto::Coalesce::Elem && op.getScratch()) {
            SmallVector<Value> mte2Uses;
            mte2Uses.push_back(op.getMem());
            mte2Uses.push_back(op.getScratch());
            addPhase(model, PipelineType::PIPE_MTE2, ValueRange{op.getDst()}, ValueRange(mte2Uses));
        } else {
            addPhase(model, PipelineType::PIPE_MTE2, ValueRange{op.getDst()}, ValueRange{op.getMem()});
        }
        addHiddenEvent(model, PipelineType::PIPE_S, PipelineType::PIPE_MTE2, ArrayRef<unsigned>{0});
        return model;
    }
    ...
    if (arch == PTOArch::A5) {
        ...
        const bool isElem1x1 = coalesce == pto::Coalesce::Elem && dstShape && dstShape->size() == 2 &&
                               (*dstShape)[0] == 1 && (*dstShape)[1] == 1;
        if (isElem1x1) {
            addPhase(model, PipelineType::PIPE_V, ValueRange{}, ValueRange{op.getIdx()});
            addPhase(model, PipelineType::PIPE_S, ValueRange{op.getDst()}, ValueRange{op.getMem()});
            addBidirectionalHiddenEvent(model, PipelineType::PIPE_V, PipelineType::PIPE_S, ArrayRef<unsigned>{0});
        } else {
            addPhase(model, PipelineType::PIPE_V, ValueRange{op.getDst()}, ValueRange{op.getMem(), op.getIdx()});
        }
        return model;
    }
    ...
    if (coalesce == pto::Coalesce::Row) {
        addPhase(model, PipelineType::PIPE_S, ValueRange{}, ValueRange{op.getIdx()});
        addPhase(model, PipelineType::PIPE_MTE2, ValueRange{op.getDst()}, ValueRange{op.getMem()});
        addHiddenEvent(model, PipelineType::PIPE_V, PipelineType::PIPE_S, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_MTE3, PipelineType::PIPE_S, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_S, PipelineType::PIPE_MTE2, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_MTE2, PipelineType::PIPE_V, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_MTE2, PipelineType::PIPE_MTE3, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_S, PipelineType::PIPE_V, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_S, PipelineType::PIPE_MTE3, ArrayRef<unsigned>{0});
    } else {
        SmallVector<Value> sUses;
        sUses.push_back(op.getIdx());
        sUses.push_back(op.getMem());
        addPhase(model, PipelineType::PIPE_S, ValueRange{op.getDst()}, ValueRange(sUses));
        addHiddenEvent(model, PipelineType::PIPE_V, PipelineType::PIPE_S, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_MTE3, PipelineType::PIPE_S, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_MTE2, PipelineType::PIPE_S, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_S, PipelineType::PIPE_V, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_S, PipelineType::PIPE_MTE2, ArrayRef<unsigned>{0});
        addHiddenEvent(model, PipelineType::PIPE_S, PipelineType::PIPE_MTE3, ArrayRef<unsigned>{0});
    }
