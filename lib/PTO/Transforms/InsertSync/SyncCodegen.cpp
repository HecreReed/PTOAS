...
void SyncCodegen::CreateBarrierOp(IRRewriter& rewriter, Operation* op, SyncOperation* sync, bool beforeInsert)
{
    ...
    IRRewriter& rewriter, Operation* op, SyncOperation* sync, bool beforeInsert)
{
    ...
    assert(sync->eventIds.size() == n && "multi-buffer codegen expects N event ids");
    ...
    if (slot.getType() != rewriter.getIndexType()) {
        slot = rewriter.create<arith::IndexCastOp>(loc, rewriter.getIndexType(), slot);
    }
    ...
    // through 1..N-1.
    Value selected = rewriter.create<arith::ConstantIndexOp>(loc, sync->eventIds[0]);
    for (uint32_t i = 1; i < n; ++i) {
        Value iIdx = rewriter.create<arith::ConstantIndexOp>(loc, i);
        Value isThis = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, slotMod, iIdx);
        Value idI = rewriter.create<arith::ConstantIndexOp>(loc, sync->eventIds[i]);
