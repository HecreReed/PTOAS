...
{
    const bool isC220 = options.march == "dav-c220-vec" || options.march == "dav-c220-cube";
    return !isC220 && options.cannVersion >= CANNVersion::release(9, 0, 0);
}
...
static LogicalResult verifyLdStDevTarget(ModuleOp module, const VPTOEmissionOptions& options, llvm::raw_ostream& diagOS)
{
    ...
    llvm::raw_ostream& diagOS)
{
    ...
    if (usesCANN900Lowering(options))
        return lowerVPTOModuleToLLVMModulesCANN900(module, options, cubeModule, vectorModule, diagOS);
    ...
    ModuleOp module, const VPTOEmissionOptions& options, std::string& output, llvm::raw_ostream& diagOS)
{
    ...
    if (failed(lowerVPTOModuleToLLVMModules(module, options, cubeModule, vectorModule, diagOS)))
        return failure();
    ...
    size_t offset = 0;
    while ((offset = output.find(modernArgMemOnly.str(), offset)) != std::string::npos) {
