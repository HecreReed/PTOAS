...
static bool getTilePointerStrides(pto::TileBufType type, int64_t& rowStride, int64_t& colStride)
{
    ...
    if (sl == 0) {
        bool rowPlusOne = type.getCompactModeI32() == static_cast<int32_t>(pto::CompactMode::RowPlusOne);
    ...
    if (bl == 1) {
        if (sl != 1)
            return false;
        rowStride = innerCols;
        colStride = shape[0] + (type.getCompactModeI32() == static_cast<int32_t>(pto::CompactMode::RowPlusOne) ? 1 : 0);
    } else {
        rowStride = shape[1] + (type.getCompactModeI32() == static_cast<int32_t>(pto::CompactMode::RowPlusOne) ? 1 : 0);
        colStride = innerRows;
    }
    return true;
}
...
static Value computeTileAddress(Value value, IRRewriter& rewriter, Location loc)
{
    ...
    if (auto subview = value.getDefiningOp<pto::SubViewOp>()) {
        ...
        int64_t rowStride = 0;
        int64_t colStride = 0;
        if (!base || !getTilePointerStrides(sourceType, rowStride, colStride) || subview.getOffsets().size() != 2)
            return {};
        ...
        int64_t elemBytes = static_cast<int64_t>(pto::getPTOStorageElemByteSize(sourceType.getElementType()));
        if (elemBytes == 0)
            return {};
...
static pto::TileBufType getSubviewPhysicalType(pto::SubViewOp op)
{
    ...
    auto compactChildType = pto::TileBufType::get(
        op.getContext(), resultType.getShape(), resultType.getElementType(), resultType.getMemorySpace(),
        resultType.getValidShape(), resultType.getConfigAttr());
    ...
    if (getTilePointerStrides(sourceType, inheritedRowStride, inheritedColStride) &&
        getTilePointerStrides(compactChildType, childRowStride, childColStride) &&
        inheritedRowStride == childRowStride && inheritedColStride == childColStride)
        physicalShape = resultType.getShape();

    return pto::TileBufType::get(
        op.getContext(), physicalShape, resultType.getElementType(), resultType.getMemorySpace(),
        resultType.getValidShape(), resultType.getConfigAttr());
}

static Value getSubviewValidOperand(
    pto::SubViewOp op, pto::TileBufType physicalType, unsigned dim, IRRewriter& rewriter)
{
...
static LogicalResult resolveTileNativeSubviews(ModuleOp module, MLIRContext* ctx)
{
    ...
    for (pto::SubViewOp op : subviews) {
        ...
        auto alloc = rewriter.create<pto::AllocTileOp>(
            op.getLoc(), physicalType, addr, getSubviewValidOperand(op, physicalType, 0, rewriter),
...
static LogicalResult getMultiTileAddresses(
    pto::AllocMultiTileOp alloc, IRRewriter& rewriter, SmallVectorImpl<Value>& addrs)
{
    uint32_t count = alloc.getResult().getType().getCount();
    if (auto planned = alloc->getAttrOfType<DenseI64ArrayAttr>(pto::kPtoMultiBufferAddrsAttrName)) {
        ...
        for (int64_t address : planned.asArrayRef())
            addrs.push_back(rewriter.create<arith::ConstantIntOp>(alloc.getLoc(), address, 64));
        return success();
    }

    Value base = alloc.getAddr();
    if (!base)
        return alloc.emitError("has neither a level3 base address nor planner-assigned slot addresses");
    auto slotBytes = getStaticSlotBytes(alloc.getResult().getType().getSlotType());
    if (failed(slotBytes))
        return alloc.emitError("requires a static slot shape and known element byte size");

    uint64_t slotStride = alignUp(*slotBytes, getTileAddressAlignmentBytes(alloc.getResult().getType().getSlotType()));

    addrs.push_back(base);
    for (uint32_t slot = 1; slot < count; ++slot) {
        Value offset =
            rewriter.create<arith::ConstantIntOp>(alloc.getLoc(), static_cast<int64_t>(slot * slotStride), 64);
        addrs.push_back(rewriter.create<arith::AddIOp>(alloc.getLoc(), base, offset));
    }
    return success();
}

static LogicalResult resolveTileNativeMultiGets(ModuleOp module, MLIRContext* ctx)
{
    ...
    for (pto::MultiTileGetOp op : gets) {
        ...
        if (!alloc)
            return op.emitError("currently requires a direct pto.alloc_multi_tile source");
        ...
        if (matchPattern(op.getSlot(), m_Constant(&constSlotAttr))) {
            ...
            for (uint32_t slot = 1; slot < addrs.size(); ++slot) {
                ...
                Value matches =
                    rewriter.create<arith::CmpIOp>(op.getLoc(), arith::CmpIPredicate::eq, op.getSlot(), slotValue);
                selectedAddr = rewriter.create<arith::SelectOp>(op.getLoc(), matches, addrs[slot], selectedAddr);
            }
        }

        auto slotHandle = rewriter.create<pto::AllocTileOp>(
            op.getLoc(), op.getResult().getType(), selectedAddr, alloc.getValidRow() ? alloc.getValidRow() : Value(),
    ...
    for (pto::AllocMultiTileOp alloc : allocs) {
        if (!alloc.getResult().use_empty())
            return alloc.emitError("has unsupported uses after resolving pto.multi_tile_get");
        alloc.erase();
    }
    return success();
}

struct PTOResolveBufferSelectPass : public mlir::pto::impl::PTOResolveBufferSelectBase<PTOResolveBufferSelectPass> {
