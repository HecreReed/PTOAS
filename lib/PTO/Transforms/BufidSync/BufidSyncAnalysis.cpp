...
bool BufidSyncAnalysis::isGMDependency(const SmallVector<const BaseMemInfo*>& tiles) const
{
    for (auto* tile : tiles) {
...
bool BufidSyncAnalysis::isSamePipe(CompoundInstanceElement* a, CompoundInstanceElement* b) const
{
...
void BufidSyncAnalysis::collectDependencies()
{
    ...
    if (debugEnabled_) {
        llvm::outs() << "[bufid_sync] collectDependencies: compounds count=" << compounds.size() << "\n";
    }
    ...
    for (unsigned i = 0; i < compounds.size(); ++i) {
        for (unsigned j = i + 1; j < compounds.size(); ++j) {
            ...
            bool hasDep = memAnalyzer_.DepBetween(src->defVec, dst->useVec, depBaseMemInfosVec);
            if (!hasDep) {
                depBaseMemInfosVec.clear();
                hasDep = memAnalyzer_.DepBetween(src->useVec, dst->defVec, depBaseMemInfosVec);
            }
            if (!hasDep) {
                depBaseMemInfosVec.clear();
                hasDep = memAnalyzer_.DepBetween(src->defVec, dst->defVec, depBaseMemInfosVec);
            }
            ...
            for (auto& pair : depBaseMemInfosVec) {
                if (pair.first->scope == pto::AddressSpace::GM || pair.second->scope == pto::AddressSpace::GM)
                    continue;
...
void BufidSyncAnalysis::collectTilesFromDepPairs()
{
    ...
    for (auto& dp : depPairs_) {
        for (auto* memInfo : dp.depTiles) {
            ...
            ti.baseAddr = memInfo->baseAddresses.empty() ? 0 : memInfo->baseAddresses[0];
            ti.size = memInfo->allocateSize;
            ti.tileValue = baseBuf;
            Value rootBuf = memInfo->rootBuffer;
            if (rootBuf && rootBuf.getDefiningOp() && rootBuf.getDefiningOp()->hasTrait<mlir::OpTrait::ConstantLike>())
                rootBuf = baseBuf;
    ...
    if (!removed.empty()) {
        ...
        if (debugEnabled_) {
            llvm::outs() << "[bufid_sync] allTiles dedup: " << allTiles_.size() << " -> " << deduped.size() << "\n";
        }
    ...
    for (auto& [space, indices] : spaceGroups) {
        ...
        for (unsigned i = 0; i < n; ++i) {
            for (unsigned j = i + 1; j < n; ++j) {
                if (memAnalyzer_.MemAlias(allTiles_[indices[i]].memInfo, allTiles_[indices[j]].memInfo)) {
                    unite(i, j);
                }
            }
        }
...
void BufidSyncAnalysis::bronKerbosch(
    const SmallVector<unsigned>& R, SmallVector<unsigned> P, SmallVector<unsigned> X,
    const SmallVector<SmallVector<bool>>& adjMatrix, SmallVector<SmallVector<unsigned>>& maximalCliques)
{
    if (P.empty() && X.empty()) {
        if (!R.empty())
    ...
    if (maximalCliques.size() > 100000) {
        llvm::errs() << "[bufid_sync] bronKerbosch WARNING: too many cliques (" << maximalCliques.size()
                     << "), aborting!\n";
        return;
    }
...
void BufidSyncAnalysis::allocateVirtualBufIds()
{
    ...
    for (unsigned gi = 0; gi < tileGroups_.size(); ++gi) {
        ...
        SmallVector<SmallVector<bool>> adjMatrix(n, SmallVector<bool>(n, false));
        for (unsigned i = 0; i < n; ++i) {
            for (unsigned j = i + 1; j < n; ++j) {
                if (memAnalyzer_.MemAlias(allTiles_[group[i]].memInfo, allTiles_[group[j]].memInfo)) {
                    adjMatrix[i][j] = true;
                    adjMatrix[j][i] = true;
                }
            }
        ...
        if (debugEnabled_) {
            llvm::outs() << "[bufid_sync] allocateVirtualBufIds: group[" << gi
                         << "] bronKerbosch done, maximalCliques=" << maximalCliques.size() << "\n";
        }
...
bool BufidSyncAnalysis::virtualBufIdContainsTile(const VirtualBufId& vbid, const BaseMemInfo* tile) const
{
    if (!tile)
        return false;
    ...
    for (auto& t : vbid.tiles) {
        ...
        bool aliasMatch = (!ptrMatch && !valMatch && memAnalyzer_.MemAlias(t.memInfo, tile));
    ...
    for (auto& vbid : virtualBufIds_) {
        if (virtualBufIdContainsTile(vbid, tile) &&
            (vbid.tiles.size() > bestTileCount ||
             (vbid.tiles.size() == bestTileCount && (bestLogicId < 0 || vbid.logicId < bestLogicId)))) {
...
int BufidSyncAnalysis::findBestVirtualBufId(const DepPair& depPair) const
{
    ...
    for (auto& vbid : virtualBufIds_) {
        ...
        bool isBetter = matchedTiles > bestMatchedTiles ||
                        (matchedTiles == bestMatchedTiles && vbid.tiles.size() > bestCliqueSize) ||
                        (matchedTiles == bestMatchedTiles && vbid.tiles.size() == bestCliqueSize &&
...
void BufidSyncAnalysis::optimizeSamePipeMerge()
{
    ...
    virtualBufIds_.erase(
        std::remove_if(
            virtualBufIds_.begin(), virtualBufIds_.end(),
            [&](const VirtualBufId& vbid) { return mergeMap.count(vbid.logicId); }),
        virtualBufIds_.end());

    if (debugEnabled_) {
        printOp2BufSync(llvm::outs(), op2BufSync_, func_, "After optimizeSamePipeMerge");
    }
}
...
void BufidSyncAnalysis::mergeGetRls()
{
    ...
    std::function<void(Block*, RlsMap&)> processBlock = [&](Block* block, RlsMap& rlsMap) {
        for (auto& op : *block) {
                                ...
                                rlsPipeAfter.erase(
                                    std::remove_if(
                                        ...
                                        [&](const BufSyncOperation& s) {
                                            return s.type == BufSyncType::RLS_BUF && s.logicId == sync.logicId &&
                                                   s.pipe == sync.pipe;
                                        }),
                                    rlsPipeAfter.end());
                            }
    ...
    if (debugEnabled_) {
        printOp2BufSync(llvm::outs(), op2BufSync_, func_, "After mergeGetRls");
    }
}
