...
enum class BufSyncType { GET_BUF, RLS_BUF };
...
inline void printTileValue(llvm::raw_ostream& os, Value val) { val.printAsOperand(os, OpPrintingFlags()); }
...
inline void printTileInfo(llvm::raw_ostream& os, const TileInfo& t)
{
    ...
    os << " scope=" << static_cast<int>(t.scope) << "(" << stringifyAddressSpace(t.scope) << ")";
...
inline void printDepPairs(llvm::raw_ostream& os, const SmallVector<DepPair>& depPairs)
{
    ...
    for (unsigned i = 0; i < depPairs.size(); ++i) {
        ...
        os << "  depPair[" << i << "] src[syncIR=" << dp.srcElement->GetIndex()
           << "] dst[syncIR=" << dp.dstElement->GetIndex() << "] srcPipe=" << static_cast<int>(dp.srcPipe)
           << " dstPipe=" << static_cast<int>(dp.dstPipe) << "\n";
        for (unsigned j = 0; j < dp.depTiles.size(); ++j) {
            os << "    depTile[" << j << "] ";
            printTileValue(os, dp.depTiles[j]->baseBuffer);
            os << " scope=" << static_cast<int>(dp.depTiles[j]->scope) << " rootBuffer=";
...
inline void printAllTiles(llvm::raw_ostream& os, const SmallVector<TileInfo>& allTiles)
{
...
inline void printTileGroups(
    llvm::raw_ostream& os, const SmallVector<SmallVector<unsigned>>& tileGroups, const SmallVector<TileInfo>& allTiles)
{
...
inline void printVirtualBufIds(llvm::raw_ostream& os, const SmallVector<VirtualBufId>& virtualBufIds)
{
    os << "[bufid_sync] VirtualBufId count: " << virtualBufIds.size() << "\n";
    for (auto& vbid : virtualBufIds) {
        os << "  logicId=" << vbid.logicId << " scope=" << static_cast<int>(vbid.scope)
...
inline void printOp2BufSync(
    llvm::raw_ostream& os, const DenseMap<Operation*, BufSyncPipeBuild>& op2BufSync, func::FuncOp func,
    const char* title = nullptr)
{
    ...
    std::sort(sortedOps.begin(), sortedOps.end(), [&](Operation* a, Operation* b) { return opOrder[a] < opOrder[b]; });

    os << "[bufid_sync] op2BufSync count: " << op2BufSync.size() << "\n";
    unsigned printIdx = 0;
    for (auto* op : sortedOps) {
        auto& build = op2BufSync.find(op)->second;
        auto firstSyncIdx = build.pipeBefore.empty() ? (build.pipeAfter.empty() ? 0 : build.pipeAfter[0].syncIRIndex) :
        ...
        for (auto& s : build.pipeBefore) {
            os << " [GET_BUF pipe=" << static_cast<int>(s.pipe) << " logicId=" << s.logicId
               << " syncIR=" << s.syncIRIndex << "]";
        }
        os << "\n    pipeAfter:";
        for (auto& s : build.pipeAfter) {
            os << " [RLS_BUF pipe=" << static_cast<int>(s.pipe) << " logicId=" << s.logicId
               << " syncIR=" << s.syncIRIndex << "]";
        }
        os << "\n";
        ++printIdx;
...
inline void printLifeIntervals(llvm::raw_ostream& os, const SmallVector<BufIdInterval>& intervals)
{
    os << "[bufid_sync] LifeIntervals count: " << intervals.size() << "\n";
    for (auto& iv : intervals) {
        os << "  logicId=" << iv.logicId << " [" << iv.startPos << "," << iv.endPos << "] pipes=";
        for (auto p : iv.pipes)
...
inline void printLogicToPhysical(
    llvm::raw_ostream& os, const DenseMap<int, int>& logicToPhysical, const char* label = "")
{
    os << "[bufid_sync] " << label << ":\n";
...
public:
    BufidSyncAnalysis(
        SyncIRs& syncIR, MemoryDependentAnalyzer& memAnalyzer, func::FuncOp func, bool debugEnabled = false)
        : syncIR_(syncIR), memAnalyzer_(memAnalyzer), func_(func), debugEnabled_(debugEnabled)
    {}
...
private:
    ...
    bool virtualBufIdContainsTile(const VirtualBufId& vbid, const BaseMemInfo* tile) const;
