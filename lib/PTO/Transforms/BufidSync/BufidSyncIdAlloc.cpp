...
void BufidSyncIdAlloc::collectPipeSignature(int logicId, SmallVector<PipelineType>& pipes) const
{
...
unsigned BufidSyncIdAlloc::getOutermostLoopBegin(Operation* op) const
{
    ...
    while (auto forOp = current->getParentOfType<scf::ForOp>()) {
        ...
        for (auto& e : syncIR_) {
            ...
            if (loop && loop->getLoopKind() == KindOfLoop::LOOP_BEGIN && loop->elementOp == forOpOp) {
...
unsigned BufidSyncIdAlloc::getOutermostLoopEnd(Operation* op) const
{
    ...
    while (auto forOp = current->getParentOfType<scf::ForOp>()) {
        ...
        for (auto& e : syncIR_) {
            auto* loop = dyn_cast<LoopInstanceElement>(e.get());
            if (loop && loop->getLoopKind() == KindOfLoop::LOOP_END && loop->elementOp == forOpOp) {
...
void BufidSyncIdAlloc::computeLifeIntervals()
{
    ...
    std::sort(intervals_.begin(), intervals_.end(), [](const BufIdInterval& a, const BufIdInterval& b) {
...
void BufidSyncIdAlloc::linearScanAllocate()
{
    ...
    if (debugEnabled_) {
        llvm::outs() << "[bufid_sync] LinearScan result: maxPhysicalIdUsed=" << maxPhysicalIdUsed_ << "\n";
...
void BufidSyncIdAlloc::compactPhysicalIds()
{
    ...
    if (debugEnabled_) {
        llvm::outs() << "[bufid_sync] After compactPhysicalIds: maxPhysicalIdUsed=" << maxPhysicalIdUsed_ << "\n";
...
void BufidSyncIdAlloc::reuseIds()
{
    ...
    auto isConsecutiveOnPipe = [&](const SmallVector<int>& ids, PipelineType pipe) -> bool {
    ...
    while (maxPhysicalIdUsed_ >= (int)physicalBufIdCount_) {
        ...
        if (debugEnabled_) {
            llvm::outs() << "[bufid_sync] reuseIds iteration=" << iteration << " sigGroups=" << sigGroups.size()
                         << "\n";
        ...
        if (debugEnabled_) {
            llvm::outs() << "[bufid_sync] reuseIds: bestSig=" << bestSigKey << " groupSize=" << groupIds.size()
                         << " checkPipe=" << static_cast<int>(checkPipe) << " consecutive=" << consecutive << "\n";
        }
        ...
        if (debugEnabled_) {
            ...
            for (auto& [lid, donorLid] : mergeMap) {
                llvm::outs() << "  merge logicId=" << lid << " -> donor logicId=" << donorLid
                             << " physicalId=" << logicToPhysical_[lid] << "\n";
            }
        }
...
bool BufidSyncIdAlloc::validateNoSamePhysicalIdNesting(std::string* error) const
{
    struct Event {
    ...
    for (auto& [op, build] : op2BufSync_) {
        ...
        for (auto& sync : build.pipeBefore) {
            ...
            if (it == logicToPhysical_.end()) {
                if (error)
                    *error = "missing physical bufid for logic id " + std::to_string(sync.logicId);
                return false;
            }
            events.push_back({sync.syncIRIndex, 0, sync.type, sync.logicId, it->second, sync.pipe});
        }
        for (auto& sync : build.pipeAfter) {
            ...
            events.push_back({sync.syncIRIndex, 1, sync.type, sync.logicId, it->second, sync.pipe});
        }
    }

    std::sort(events.begin(), events.end(), [](const Event& a, const Event& b) {
        return std::make_tuple(
                   a.syncIRIndex, a.phase, static_cast<int>(a.type), a.physicalId, a.logicId,
                   static_cast<int>(a.pipe)) <
               std::make_tuple(
                   b.syncIRIndex, b.phase, static_cast<int>(b.type), b.physicalId, b.logicId, static_cast<int>(b.pipe));
    });
    ...
    for (const Event& event : events) {
        if (event.type == BufSyncType::GET_BUF) {
            ...
            if (activeIt != activeByPhysicalId.end()) {
                if (error) {
                    ...
                    *error = "nested get_buf for physical bufid " + std::to_string(event.physicalId) + " at SyncIR " +
                             std::to_string(event.syncIRIndex) + " while logic id " + std::to_string(active.logicId) +
                             " from SyncIR " + std::to_string(active.syncIRIndex) + " is still active";
                }
                return false;
            }
        ...
        if (activeIt == activeByPhysicalId.end()) {
            if (error) {
                *error = "rls_buf without active get_buf for physical bufid " + std::to_string(event.physicalId) +
                         " at SyncIR " + std::to_string(event.syncIRIndex);
            }
            return false;
        }
    ...
    if (!activeByPhysicalId.empty()) {
        if (error) {
            ...
            const Event& active = activeIt->second;
            *error = "unclosed get_buf for physical bufid " + std::to_string(active.physicalId) + " from SyncIR " +
