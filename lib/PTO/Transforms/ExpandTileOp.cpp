...
struct OperandTypeInfo {
    ...
    bool operator==(const OperandTypeInfo& rhs) const
    {
        ...
        if (kind == OperandKind::Tile)
            return tileShape == rhs.tileShape && tileValidShape == rhs.tileValidShape &&
                   tileMemorySpace == rhs.tileMemorySpace && blayout == rhs.blayout && slayout == rhs.slayout &&
        ...
        return viewShape == rhs.viewShape && viewStrides == rhs.viewStrides && viewMemorySpace == rhs.viewMemorySpace &&
...
struct SpecKey {
    ...
    {
        return opName == rhs.opName && targetArch == rhs.targetArch && operands == rhs.operands &&
               contextAttrs == rhs.contextAttrs;
    }
};
...
struct SpecKeyInfo : public llvm::DenseMapInfo<SpecKey> {
    ...
    static unsigned getHashValue(const SpecKey& key)
    {
        ...
        for (const auto& op : key.operands) {
            ...
            if (op.kind == OperandKind::Tile) {
                h = llvm::hash_combine(h, op.tileMemorySpace, op.blayout, op.slayout, op.fractal, op.pad);
    ...
    static bool isEqual(const SpecKey& lhs, const SpecKey& rhs) { return lhs == rhs; }
};
// ============================================================================
// Helpers
// ============================================================================
...
static Value bridgeOperandToType(OpBuilder& builder, Location loc, Value operand, Type dstTy)
{
    ...
    return builder.create<UnrealizedConversionCastOp>(loc, dstTy, operand).getResult(0);
}

static StringRef getTileOpName(Operation* op) { return op->getName().stripDialect(); }

static std::string getTargetArchString(Operation* op)
{
    if (!op)
        return "";
    for (ModuleOp current = op->getParentOfType<ModuleOp>(); current; current = current->getParentOfType<ModuleOp>()) {
...
static bool tryAppendPrecisionType(
    Operation* op, SmallVectorImpl<std::pair<std::string, std::string>>& attrs, PrecisionT highPrecision)
{
    ...
    if (precision == highPrecision && !highPrecisionImplementedOps().contains(op->getName().getStringRef())) {
...
static std::string getTRandomRoundsString(pto::TRandomOp op) { return std::to_string(op.getRounds()); }
...
static void appendOpContextAttrs(Operation* op, SmallVectorImpl<std::pair<std::string, std::string>>& attrs)
{
    ...
    (void)(tryAppendPrecisionType<pto::TExpOp>(op, attrs, pto::ExpPrecision::HighPrecision) ||
           tryAppendPrecisionType<pto::TLogOp>(op, attrs, pto::LogPrecision::HighPrecision) ||
           tryAppendPrecisionType<pto::TSqrtOp>(op, attrs, pto::SqrtPrecision::HighPrecision) ||
           tryAppendPrecisionType<pto::TRecipOp>(op, attrs, pto::RecipPrecision::HighPrecision) ||
           tryAppendPrecisionType<pto::TRsqrtOp>(op, attrs, pto::RsqrtPrecision::HighPrecision) ||
           tryAppendPrecisionType<pto::TDivOp>(op, attrs, pto::DivPrecision::HighPrecision) ||
           tryAppendPrecisionType<pto::TDivSOp>(op, attrs, pto::DivPrecision::HighPrecision) ||
           tryAppendPrecisionType<pto::TRowExpandDivOp>(op, attrs, pto::DivPrecision::HighPrecision) ||
           tryAppendPrecisionType<pto::TColExpandDivOp>(op, attrs, pto::DivPrecision::HighPrecision));
}
...
static void recordStaticSizes(ArrayRef<OpFoldResult> inputs, SmallVectorImpl<int64_t>& out)
{
    out.clear();
...
static void populateViewShapeAndStrides(Value value, SmallVectorImpl<int64_t>& shape, SmallVectorImpl<int64_t>& strides)
{
    ...
    if (auto memrefTy = dyn_cast<MemRefType>(value.getType())) {
        ...
        if (strides.empty()) {
            ...
            if (succeeded(mlir::pto::getPTOMemRefStridesAndOffset(memrefTy, strides, offset))) {
...
static std::optional<OperandTypeInfo> buildOperandTypeInfo(Value value)
{
    ...
    if (auto tbTy = dyn_cast<pto::TileBufType>(ty)) {
        ...
        if (auto config = tbTy.getConfigAttr()) {
            ...
            info.fractal = config.getSFractalSize() ? static_cast<int32_t>(config.getSFractalSize().getInt()) : 0;
    ...
    if (auto mrTy = dyn_cast<MemRefType>(ty)) {
        ...
        if (info.viewStrides.empty()) {
            int64_t offset = ShapedType::kDynamic;
            if (succeeded(mlir::pto::getPTOMemRefStridesAndOffset(mrTy, info.viewStrides, offset))) {
                // strides populated — dynamic dims remain ShapedType::kDynamic.
            }
        }
        return info;
...
struct ExpandState {
    ...
    std::optional<std::string> invokeTileLibHelper(const SpecKey& key, StringRef candidateId = {});
    func::FuncOp invokeTileLib(const SpecKey& key, Operation* tileOp, ModuleOp mod, MLIRContext* ctx);
    func::FuncOp invokeTileLibDaemon(const SpecKey& key, StringRef candidateId, ModuleOp mod, MLIRContext* ctx);

    LogicalResult expandTileOpsInFunction(func::FuncOp func, ModuleOp mod, MLIRContext* ctx);
};

// ============================================================================
// The Pass
// ============================================================================
struct ExpandTileOpPass : public mlir::pto::impl::ExpandTileOpBase<ExpandTileOpPass> {
...
static void appendJsonDimArray(std::string& json, ArrayRef<int64_t> arr, bool negativeIsDynamic = false)
{
    json += "[";
    for (size_t i = 0; i < arr.size(); ++i) {
        if (i > 0)
...
// ============================================================================
std::optional<std::string> ExpandState::invokeTileLibHelper(const SpecKey& key, StringRef candidateId)
{
    ...
    int tmpFD;
    if (auto ec = llvm::sys::fs::createTemporaryFile("tilelib_helper", "out", tmpFD, tmpPath)) {
    ...
    SmallVector<StringRef> args = {
        *pythonPath, "-m",   daemonHelperModule, "--socket",       daemonSocketPath, "--target", key.targetArch,
        "--op",      opName, "--operand-specs",  operandSpecsJson,
    };
    ...
    std::optional<StringRef> redirects[] = {std::nullopt, StringRef(tmpPath), std::nullopt};
    ...
    int rc = llvm::sys::ExecuteAndWait(
        *pythonPath, args, hasPythonPath ? std::optional<ArrayRef<StringRef>>(envp) : std::nullopt, redirects,
        /*secondsToWait=*/30, /*memoryLimit=*/0, &errMsg);

    if (rc != 0) {
        llvm::errs() << "ExpandTileOp: daemon helper instantiate failed (rc=" << rc << "): " << errMsg << "\n";
...
// ============================================================================
func::FuncOp ExpandState::invokeTileLibDaemon(const SpecKey& key, StringRef candidateId, ModuleOp mod, MLIRContext* ctx)
{
    ...
    if (!cloned->hasAttr("pto.tilelang.instance")) {
        llvm::errs() << "ExpandTileOp: warning: daemon output function @" << cloned.getSymName()
...
// ============================================================================
func::FuncOp ExpandState::invokeTileLib(const SpecKey& key, Operation* tileOp, ModuleOp mod, MLIRContext* ctx)
{
    ...
    func::FuncOp daemonResult = invokeTileLibDaemon(key, selectedName.getValue(), mod, ctx);
    if (daemonResult)
        return daemonResult;
...
// ============================================================================
LogicalResult ExpandState::expandTileOpsInFunction(func::FuncOp func, ModuleOp mod, MLIRContext* ctx)
{
    OpBuilder builder(ctx);
    ...
    for (auto* op : tileOps) {
        ...
        if (!specKeyOpt) {
            op->emitError("ExpandTileOp: cannot build specialization key for this operand schema");
            return failure();
        }
        ...
        if (!dslFn) {
            ...
            op->emitError("ExpandTileOp: failed to instantiate TileLib template for " + opName);
            return failure();
        }
        ...
        for (unsigned i = 0; i < op->getNumOperands(); ++i) {
            ...
            if (i < fnArgTypes.size() && operand.getType() != fnArgTypes[i]) {
                operand = bridgeOperandToType(builder, op->getLoc(), operand, fnArgTypes[i]);
            }
...
void ExpandTileOpPass::runOnOperation()
{
    ...
    if (tileLibBackend != "ptodsl") {
        mod.emitError("ExpandTileOp received unsupported tile-lib-backend '" + std::string(tileLibBackend) + "'");
        signalPassFailure();
        return;
    }
...
std::unique_ptr<Pass> createExpandTileOpPass() { return std::make_unique<ExpandTileOpPass>(); }
