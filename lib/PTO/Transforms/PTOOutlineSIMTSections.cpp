...
static LogicalResult collectCaptures(pto::SectionSimtOp sectionOp, SmallVectorImpl<Value>& captures)
{
...
static void cloneExternalConstants(pto::SectionSimtOp sectionOp, OpBuilder& builder, IRMapping& mapping)
{
    ...
    sectionOp.getBody().walk([&](Operation* op) {
        for (Value operand : op->getOperands()) {
            ...
            if (!defOp || isDefinedInside(scope, operand) || !defOp->hasTrait<OpTrait::ConstantLike>())
                continue;
...
static LogicalResult verifySectionCanBeOutlined(pto::SectionSimtOp sectionOp)
{
    ...
    if (parentFunc->hasAttr(pto::kPTOSimtEntryAttrName)) {
        return sectionOp.emitOpError() << "must not be nested in a function marked with '" << pto::kPTOSimtEntryAttrName
                                       << "'";
    }
    ...
    if (escapeCheck.wasInterrupted()) {
        return sectionOp.emitOpError("does not support values defined in pto.section.simt escaping the "
                                     "section");
    }
...
static std::string getUniqueHelperName(ModuleOp module, func::FuncOp parentFunc, unsigned& outlineIndex)
{
    std::string parentName = parentFunc.getSymName().str();
    do {
        std::string candidate = (Twine(parentName) + "_simt_" + Twine(outlineIndex++)).str();
...
static FailureOr<int64_t> getSimtThreadCount(pto::SectionSimtOp sectionOp)
{
    ...
    for (int64_t dimension : {dimX, dimY, dimZ}) {
        if (dimension <= 0 || threadCount > kMaxSimtThreads / dimension) {
            sectionOp.emitOpError() << "SIMT launch dimensions must have a positive product no greater "
                                       "than "
                                    << kMaxSimtThreads << ", got (" << dimX << ", " << dimY << ", " << dimZ << ")";
            return failure();
        }
...
static func::FuncOp createOutlinedHelper(
    ModuleOp module, pto::SectionSimtOp sectionOp, StringRef helperName, int64_t maxThreads, ValueRange captures)
{
    ...
    auto helper = moduleBuilder.create<func::FuncOp>(loc, helperName, FunctionType::get(ctx, argTypes, TypeRange{}));
    helper.setPrivate();
    ...
    helper->setAttr(pto::kPTOSimtMaxThreadsAttrName, moduleBuilder.getI32IntegerAttr(maxThreads));
...
static void replaceSectionWithLaunch(pto::SectionSimtOp sectionOp, StringRef helperName, ValueRange captures)
{
    Location loc = sectionOp.getLoc();
    OpBuilder builder(sectionOp);
    Value dimX = builder.create<arith::ConstantIntOp>(
        loc, sectionOp.getDimX(),
        /*width=*/32);
    Value dimY = builder.create<arith::ConstantIntOp>(
        loc, sectionOp.getDimY(),
        /*width=*/32);
    Value dimZ = builder.create<arith::ConstantIntOp>(
        loc, sectionOp.getDimZ(),
        /*width=*/32);
    builder.create<pto::SimtLaunchOp>(loc, helperName, dimX, dimY, dimZ, captures);
    sectionOp.erase();
}

static LogicalResult outlineSection(ModuleOp module, pto::SectionSimtOp sectionOp, unsigned& outlineIndex)
{
    ...
    func::FuncOp parentFunc = sectionOp->getParentOfType<func::FuncOp>();
    std::string helperName = getUniqueHelperName(module, parentFunc, outlineIndex);
...
struct PTOOutlineSIMTSectionsPass : public pto::impl::PTOOutlineSIMTSectionsBase<PTOOutlineSIMTSectionsPass> {
    using pto::impl::PTOOutlineSIMTSectionsBase<PTOOutlineSIMTSectionsPass>::PTOOutlineSIMTSectionsBase;

    void runOnOperation() override
    {
        ModuleOp module = getOperation();
        SmallVector<pto::SectionSimtOp> sections;
        module.walk([&](pto::SectionSimtOp sectionOp) { sections.push_back(sectionOp); });
