...
static FailureOr<StringRef> buildCANN900ModeTypedCallee(
...
static FailureOr<StringRef> buildCANN900SignedModeTypedCallee(
    MLIRContext* context, Type vectorType, StringRef stem, StringRef mode)
{
    std::string vec = getCANN900VectorTypeFragment(vectorType);
...
static FailureOr<StringRef> buildCANN900WideningReductionCallee(
    MLIRContext* context, Type inputType, Type resultType, StringRef stem, StringRef mode)
{
    ...
    return StringAttr::get(
               context,
               "llvm.hivm." + stem.str() + "." + signedness + "." + mode.str() + "." + resultVec + "." + inputVec)
        .getValue();
}
    ...
    MLIRContext* context, Type vectorType, StringRef stem, StringRef mode)
{
    ...
    if (!abi || !lanes)
        return failure();
    return StringAttr::get(
               context, "llvm.hivm." + stem.str() + "." + mode.str() + ".v" + std::to_string(*lanes) +
...
template <typename UnaryOp>
static FailureOr<StringRef> buildUnaryMaskedCallee(MLIRContext* context, Type resultType)
{
...
static FailureOr<StringRef> buildVbrCallee(MLIRContext* context, Type resultType)
{
    std::string vec = getCANN900VectorTypeFragment(resultType);
    if (vec.empty())
        return failure();
...
static FailureOr<StringRef> buildCopyGmToUbCallee(MLIRContext* context, Type sourceType)
{
...
static FailureOr<StringRef> buildVldsx2Callee(MLIRContext* context, Type resultType, bool post)
{
    ...
    if (vec.empty() || !lanes)
        return failure();
    return StringAttr::get(
               context, "llvm.hivm.vldsx2" + std::string(post ? ".post" : "") + ".v" + std::to_string(*lanes) + vec)
        .getValue();
}
...
public:
        ...
        ReductionOp op, typename ReductionOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        FailureOr<StringRef> calleeName = buildCANN900WideningReductionCallee(
            op.getContext(), op.getInput().getType(), op.getResult().getType(), stem, "x");
        if (failed(calleeName))
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::PTOLdDevOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto llvmPtrType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getPtr().getType());
        if (!llvmPtrType)
            return rewriter.notifyMatchFailure(op, "expected LLVM pointer operand");
        ...
        FailureOr<Value> gmPtr =
            reinterpretPointerToAddrSpace(op, elemPtr, static_cast<unsigned>(pto::AddressSpace::GM));
        if (failed(gmPtr))
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{gmPtr->getType(), rewriter.getI64Type()}, TypeRange{rewriter.getI64Type()});
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), calleeName, TypeRange{rewriter.getI64Type()}, ValueRange{*gmPtr, intrinsicOffset});
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::PTOStDevOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto llvmPtrType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getPtr().getType());
        if (!llvmPtrType)
            return rewriter.notifyMatchFailure(op, "expected LLVM pointer operand");
        ...
        Value intrinsicOffset = getI64Constant(rewriter, op.getLoc(), 0);
        auto funcType = rewriter.getFunctionType(
            TypeRange{rewriter.getI64Type(), gmPtr->getType(), rewriter.getI64Type()}, TypeRange{});
        rewriter.create<func::CallOp>(
            op.getLoc(), calleeName, TypeRange{}, ValueRange{payload, *gmPtr, intrinsicOffset});
        state.plannedDecls.push_back(PlannedDecl{calleeName, funcType});
        rewriter.eraseOp(op);
        return success();
    }
...
static void populateVPTOOpLoweringPatterns(
    VPTOTypeConverter& typeConverter, RewritePatternSet& patterns, LoweringState& state)
{
    patterns.add<
...
static void configureVPTOOpLoweringTarget(ConversionTarget& target, VPTOTypeConverter& typeConverter)
{
    (void)typeConverter;
...
static LogicalResult lowerVPTOTypes(ModuleOp module, llvm::raw_ostream& diagOS)
{
    ...
    target.addIllegalOp<
        ...
        pto::PTOLdgOp, pto::PTOStgOp, pto::PTOLdDevOp, pto::PTOStDevOp, pto::DeclareStructOp, pto::StructGetOp,
        pto::StructSetOp>();
    ...
    patterns.add<
        ConvertPtoAddPtrOp, ConvertPtoCastPtrOp, ConvertPtoLoadScalarOp, ConvertPtoStoreScalarOp,
        ConvertPtoDeclareStructOp, ConvertPtoStructGetOp, ConvertPtoStructSetOp>(typeConverter, context);
    patterns.add<
        ConvertPtoLoadOp, ConvertPtoStoreOp, ConvertPtoLdgOp, ConvertPtoStgOp, ConvertPtoLdDevOp, ConvertPtoStDevOp>(
...
static LogicalResult runPipeline(ModuleOp module, llvm::raw_ostream& diagOS, EmitFn&& emit)
{
