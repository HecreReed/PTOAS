...
static Value getPipeInitLocalAddr(Operation *initOp) {
  if (auto init = dyn_cast_or_null<pto::InitializeL2LPipeOp>(initOp))
    return init.getLocalAddr();
...
static FailureOr<Operation *> findPeerFixpipeConsumerInit(Operation *producerInit) {
  Value localAddr = getPipeInitLocalAddr(producerInit);
  if (!localAddr)
    return failure();

  auto importOp = localAddr.getDefiningOp<ImportReservedBufferOp>();
  if (!importOp)
    return failure();

  auto peerFunc =
      lookupPeerFuncAcrossContainer(importOp.getOperation(), importOp.getPeerFuncAttr());
  if (!peerFunc)
    return failure();
  ...
  peerFunc.walk([&](Operation *candidate) {
    Value peerLocalAddr = getPipeInitLocalAddr(candidate);
    if (!peerLocalAddr)
      return WalkResult::advance();
    auto reserveOp = peerLocalAddr.getDefiningOp<ReserveBufferOp>();
...
static FailureOr<TileBufType> resolveFixpipeConsumerTileType(Value pipeHandle) {
  ...
  if (auto peerInitOr = findPeerFixpipeConsumerInit(producerInit);
      succeeded(peerInitOr)) {
  ...
  if (!resolvedType && !hasMismatch) {
    auto producerFunc = producerInit->getParentOfType<func::FuncOp>();
    auto moduleOp = producerInit->getParentOfType<ModuleOp>();
    auto producerPipeId = getFrontendPipeIdFromInit(producerInit);
    auto producerAccPushEpilogue = getPipeInitAccPushEpilogue(producerInit);
    if (producerFunc && moduleOp && producerPipeId) {
      moduleOp.walk([&](pto::TPopOp pop) {
        auto popFunc = pop->getParentOfType<func::FuncOp>();
        if (!popFunc || popFunc == producerFunc)
          return WalkResult::advance();
        Operation *popInit = getPipeInitDef(pop.getPipeHandle());
        if (!popInit)
          return WalkResult::advance();
        auto popPipeId = getFrontendPipeIdFromInit(popInit);
        if (!popPipeId || *popPipeId != *producerPipeId)
          return WalkResult::advance();
        if (producerAccPushEpilogue) {
          auto popAccPushEpilogue = getPipeInitAccPushEpilogue(popInit);
          if (!popAccPushEpilogue ||
              popAccPushEpilogue != producerAccPushEpilogue)
            return WalkResult::advance();
        }
        if (!resolvedType) {
          resolvedType = pop.getTile().getType();
  ...
  for (auto funcOp : mop.getOps<func::FuncOp>()) {
    for (Block &block : funcOp.getBlocks()) {
      ...
      for (Operation *op : originalOps) {
        ...
        auto tpush = dyn_cast<TPushOp>(op);
        if (!tpush)
          continue;
