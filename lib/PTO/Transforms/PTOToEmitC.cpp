...
static void buildGlobalTensorShapeAndStride(
...
static std::string getGlobalTensorTypeStringFromShapeAndStrides(
    Type elemTy, ArrayRef<int64_t> shape, ArrayRef<int64_t> strides, StringRef layoutEnum = "pto::Layout::ND");
...
static Type getEmitCVariableResultType(Type valueType) { return valueType; }

static Value loadEmitCVariableIfNeeded(OpBuilder& builder, Location loc, Value value)
{
    (void)builder;
    (void)loc;
    return value;
...
static void appendRawLocationNameHints(Location loc, SmallVectorImpl<std::string>& hints)
{
...
static constexpr llvm::StringLiteral kPipePeerOwnerFuncAttrName = "__pto.peer_owner_func";
static constexpr llvm::StringLiteral kPipePeerReserveNameAttrName = "__pto.peer_reserve_name";
static constexpr llvm::StringLiteral kPipePeerDirMaskAttrName = "__pto.peer_dir_mask";
static constexpr llvm::StringLiteral kEmitCScalarOutTypeAttrName = "__pto.emitc_scalar_out_type";
...
static int64_t getAPIntSignedValue(const APInt& value) { return value.getBitWidth() == 0 ? 0 : value.getSExtValue(); }
...
static int64_t getIntegerAttrSignedValue(IntegerAttr attr) { return getAPIntSignedValue(attr.getValue()); }
...
static SmallVector<unsigned, 4> buildDefaultLastUseTileSlotOrder(Operation* op)
{
...
static std::optional<std::string> buildLastUseMarkerCallee(
    Operation* op, StringRef callee, ArrayRef<unsigned> tileSlotOrder = {})
{
    auto lastUseAttr = dyn_cast_or_null<DenseI64ArrayAttr>(op->getAttr(kLastUseAttrName));
    if (!lastUseAttr)
        return std::nullopt;
...
static StringRef getLastUseAwareCallee(
    Operation* op, StringRef callee, std::string& storage, ArrayRef<unsigned> tileSlotOrder = {})
{
    std::optional<std::string> marker = buildLastUseMarkerCallee(op, callee, tileSlotOrder);
    if (!marker)
        return callee;
...
static void createLastUseAwareOpaqueCall(
    ConversionPatternRewriter& rewriter, Operation* op, TypeRange resultTypes, StringRef callee, ValueRange operands,
    ArrayAttr args = ArrayAttr{}, ArrayAttr templateArgs = ArrayAttr{}, ArrayRef<unsigned> tileSlotOrder = {})
{
    std::string calleeStorage;
    StringRef effectiveCallee = getLastUseAwareCallee(op, callee, calleeStorage, tileSlotOrder);
    rewriter.create<emitc::CallOpaqueOp>(op->getLoc(), resultTypes, effectiveCallee, args, templateArgs, operands);
}

static Value buildGlobalTensorFromMemref(
...
static std::optional<mlir::pto::Layout> resolveLayoutForGlobalTensor(Operation* anchor, Value basePtr)
{
...
static Value peelGlobalTensorConversionBridge(Value value)
{
    ...
    if (isEmitCGlobalTensorLikeType(input.getType()) && isEmitCGlobalTensorLikeType(value.getType()))
        return input;
    return value;
}

static bool isF8E8M0ElemType(Type elemTy) { return mlir::pto::isPTOF8E8M0Type(elemTy); }
...
static std::string getEmitCScalarTypeToken(Type elemTy)
{
    ...
    if (elemTy.isInteger(8))
        return (elemTy.isSignlessInteger(8) || elemTy.isSignedInteger(8)) ? "int8_t" : "uint8_t";
    if (elemTy.isInteger(16))
        return (elemTy.isSignlessInteger(16) || elemTy.isSignedInteger(16)) ? "int16_t" : "uint16_t";
    if (elemTy.isInteger(32))
        return (elemTy.isSignlessInteger(32) || elemTy.isSignedInteger(32)) ? "int32_t" : "uint32_t";
...
static std::string renderStructFieldDecl(Type fieldTy, const std::string& name)
{
...
static int64_t renderTileTemplateDim(int64_t rawDim, Type elemTy, pto::BLayout blayout, int dimIdx);
static bool isLowPrecisionCubeOperandType(Type elemTy)
{
    return pto::isPTOFloat8Type(elemTy) || isa<pto::F4E1M2x2Type>(elemTy) || isa<pto::F4E2M1x2Type>(elemTy);
}
...
static std::optional<SpecialGlobalTensorTypeSpec> getSpecialScaleGlobalTensorTypeSpecForTileValue(
    Value dstValue, ArrayRef<int64_t> shape, Type elemTy)
{
    ...
    auto dstSpace = dyn_cast_or_null<pto::AddressSpaceAttr>(dstTileTy.getMemorySpace());
    ...
    if (blayout == pto::BLayout::RowMajor && slayout == pto::SLayout::RowMajor) {
        if (effectiveShape[0] == 1)
            return std::nullopt;
        return SpecialGlobalTensorTypeSpec{
            "TileShape2D<" + elemTypeStr + ", " + std::to_string(effectiveShape[0]) + ", " +
                std::to_string(effectiveShape[1]) + ", pto::Layout::MX_A_ZZ>",
            "BaseShape2D<" + elemTypeStr + ", " + std::to_string(effectiveShape[0]) + ", " +
                std::to_string(effectiveShape[1]) + ", pto::Layout::MX_A_ZZ>",
            "pto::Layout::MX_A_ZZ",
        };
    }

    if (blayout == pto::BLayout::ColMajor && slayout == pto::SLayout::ColMajor) {
        return SpecialGlobalTensorTypeSpec{
            "TileShape2D<" + elemTypeStr + ", " + std::to_string(effectiveShape[0]) + ", " +
...
static std::optional<SpecialGlobalTensorTypeSpec> getSpecialGlobalTensorTypeSpecForLayout(
    std::optional<mlir::pto::Layout> layout, ArrayRef<int64_t> shape, Type elemTy)
{
    ...
    switch (*layout) {
        case mlir::pto::Layout::MX_A_ZZ: {
            ...
            return SpecialGlobalTensorTypeSpec{
                "TileShape2D<" + elemTypeStr + ", " + std::to_string(rows) + ", " + std::to_string(cols) +
                    ", pto::Layout::MX_A_ZZ>",
                "BaseShape2D<" + elemTypeStr + ", " + std::to_string(rows) + ", " + std::to_string(cols) +
                    ", pto::Layout::MX_A_ZZ>",
                "pto::Layout::MX_A_ZZ",
            };
        }
...
static std::optional<SpecialGlobalTensorTypeSpec> getSpecialScaleGlobalTensorTypeSpec(
    Operation* anchor, MemRefType mrTy)
{
    auto load = dyn_cast_or_null<pto::TLoadOp>(anchor);
    if (!load)
        return std::nullopt;
    return getSpecialScaleGlobalTensorTypeSpecForTileValue(load.getDst(), mrTy.getShape(), mrTy.getElementType());
}

static const char* scalingRoleToken(Type elemTy, pto::TileBufConfigAttr configAttr)
{
    if (!isF8E8M0ElemType(elemTy))
...
static const char* tileRoleToken(
    Attribute memorySpace, std::optional<Type> elemType = std::nullopt,
...
static std::optional<std::string> getEmitCTileTypeString(pto::TileBufType type)
{
    ...
    auto render = [&](int64_t dim, int dimIdx) { return renderTileTemplateDim(dim, elemTy, blayout, dimIdx); };
    ...
    if (auto asAttr = dyn_cast_or_null<pto::AddressSpaceAttr>(type.getMemorySpace())) {
        if (isLowPrecisionCubeOperandType(elemTy)) {
            if (asAttr.getAddressSpace() == pto::AddressSpace::LEFT && shape[0] != 1 &&
                validShape[1] != ShapedType::kDynamic) {
                vcolTok = std::to_string(render(cols, 1));
            } else if (asAttr.getAddressSpace() == pto::AddressSpace::RIGHT && validShape[0] != ShapedType::kDynamic) {
    ...
    return std::string("Tile<") + tileRoleToken(type.getMemorySpace(), elemTy, type.getConfigAttr()) + ", " +
           getEmitCScalarTypeToken(elemTy) + ", " + std::to_string(render(rows, 0)) + ", " +
           std::to_string(render(cols, 1)) + ", " + tileBufBLayoutToken(configAttr) + ", " + vrowTok + ", " + vcolTok +
           ", " + tileBufSLayoutToken(configAttr) + ", " + std::to_string(fractal) + ", " +
           tileBufPadToken(configAttr) + ", " + tileBufCompactToken(configAttr) + ">";
}
...
public:
    PTOToEmitCTypeConverter(MLIRContext* Ctx, PTOArch targetArch)
    {
        ...
        addConversion([Ctx](IndexType type) -> Type { return emitc::OpaqueType::get(Ctx, "int64_t"); });

        // vector<4xi16> (e.g. TMRGSORT executedNumList) -> pto::MrgSortExecutedNumList
        addConversion([Ctx](VectorType type) -> Type {
            if (type.getRank() == 1 && type.getNumElements() == 4 && type.getElementType().isInteger(16))
        ...
        addConversion([this, Ctx](pto::PtrType type) -> std::optional<Type> {
            ...
            std::string qualifier = addrSpaceQualifier(getAddressSpaceOrGM(type.getMemorySpace()));
        ...
        addConversion([Ctx](pto::StructType type) -> Type {
            return emitc::PointerType::get(emitc::OpaqueType::get(Ctx, getStructTypeName(type)));
        });
        ...
        addConversion([Ctx](pto::TensorViewType type) -> Type {
            return getRuntimeGlobalTensorOpaqueType(Ctx, type.getElementType(), type.getShape());
        });
...
static constexpr unsigned kPTOIndexBitWidth = 64; // keep consistent with IndexType conversion
...
static Value makeEmitCOpaqueConstant(
    ConversionPatternRewriter& rewriter, Location loc, Type type, llvm::StringRef literal);
static Value makeEmitCIntConstant(ConversionPatternRewriter& rewriter, Location loc, Type type, int64_t value);
static Value emitCCast(ConversionPatternRewriter& rewriter, Location loc, Type dstType, Value src);
static FailureOr<std::string> buildEmitCOpaqueConstantLiteral(Type targetType, Attribute valueAttr);
static Value castSignlessIntToUnsignedSameWidth(
...
static FailureOr<std::string> buildTPipeTokenFromInitOp(Operation* op, PTOArch targetArch)
{
    if (auto initOp = dyn_cast<pto::InitializeL2G2LPipeOp>(op)) {
        ...
        int32_t localSlotNum = initOp.getLocalSlotNumAttr() ?
                                   static_cast<int32_t>(getIntegerAttrSignedValue(initOp.getLocalSlotNumAttr())) :
                                   initOp.getSlotNum();
        return buildTPipeToken(
            static_cast<int32_t>(getIntegerAttrSignedValue(initOp.getFlagBaseAttr())), *dirTok, initOp.getSlotSize(),
            initOp.getSlotNum(), localSlotNum, initOp.getNosplitAttr() && initOp.getNosplitAttr().getValue());
    }
    ...
    if (auto initOp = dyn_cast<pto::InitializeL2LPipeOp>(op)) {
        ...
        return buildTPipeToken(
            static_cast<int32_t>(getIntegerAttrSignedValue(initOp.getFlagBaseAttr())), *dirTok, initOp.getSlotSize(),
            initOp.getSlotNum(), 2, initOp.getNosplitAttr() && initOp.getNosplitAttr().getValue());
    }
...
static FailureOr<std::string> buildFixpipeConfigTypeToken(AccPushEpilogueAttr accPushEpilogue)
{
...
{
    auto ownerFuncAttr = producerInit->getAttrOfType<FlatSymbolRefAttr>(kPipePeerOwnerFuncAttrName);
    auto reserveNameAttr = producerInit->getAttrOfType<StringAttr>(kPipePeerReserveNameAttrName);
    auto dirMaskAttr = producerInit->getAttrOfType<IntegerAttr>(kPipePeerDirMaskAttrName);
    if (!ownerFuncAttr || !reserveNameAttr || !dirMaskAttr || dirMaskAttr.getInt() != 1)
        return failure();

    auto peerFunc = lookupPeerFuncAcrossContainer(producerInit, ownerFuncAttr);
    if (!peerFunc)
        return failure();
    ...
    peerFunc.walk([&](Operation* candidate) {
        ...
        auto candidateOwnerFuncAttr = candidate->getAttrOfType<FlatSymbolRefAttr>(kPipePeerOwnerFuncAttrName);
        auto candidateReserveNameAttr = candidate->getAttrOfType<StringAttr>(kPipePeerReserveNameAttrName);
        auto candidateDirMaskAttr = candidate->getAttrOfType<IntegerAttr>(kPipePeerDirMaskAttrName);
        if (!candidateOwnerFuncAttr || !candidateReserveNameAttr || !candidateDirMaskAttr)
            return WalkResult::advance();

        if (candidateOwnerFuncAttr != ownerFuncAttr || candidateReserveNameAttr != reserveNameAttr ||
...
static FailureOr<TileBufType> resolveFixpipeConsumerTileType(Value pipeHandle)
{
    ...
    auto collectFromFunc = [&](func::FuncOp funcOp, llvm::function_ref<bool(pto::TPopOp)> matchesPop) {
    ...
    collectFromFunc((*peerInitOr)->getParentOfType<func::FuncOp>(), [&](pto::TPopOp pop) {
                    ...
                    if (isScalarFixpipeQuant(accPushEpilogue.getQuant())) {
                        ...
                        if (it != activeScalarById.end()) {
                            ...
                            cloned->setAttr(
                                ...
                                builder.getStringAttr(getEmitCScalarTypeToken((*consumerTileTy).getElementType())));
                        }
...
static LogicalResult insertFixpipeConfigAliases(ModuleOp mop)
{
    for (auto funcOp : mop.getOps<func::FuncOp>()) {
        ...
        for (const auto& [pipeId, configTok] : aliases) {
            std::string line = "using " + buildFixpipeConfigAliasName(pipeId) + " = " + configTok + ";";
            builder.create<emitc::VerbatimOp>(funcOp.getLoc(), builder.getStringAttr(line));
        }
    }
    return success();
}
...
static bool isSetFFTsPointerLikeType(Type ty) { return isEmitCPointerLikeType(ty); }

static bool tileDataReturnsIntegralAddress(pto::AddressSpace as) { return as == pto::AddressSpace::BIAS; }
...
static Value materializeTileDataValue(
    ConversionPatternRewriter& rewriter, Location loc, Value tile, pto::AddressSpace as, StringRef elemTok)
{
...
static Value materializeAddressAsPointer(
...
static FailureOr<Value> adaptCallOperandForEmitC(
    const TypeConverter* typeConverter, ConversionPatternRewriter& rewriter, Location loc, Type originalCalleeArgTy,
    Value originalOperand, Value loweredOperand)
{
    Type elemTy;
    ...
    if (elemTy && as) {
        ...
        auto materializeForCall = [&](Value tileLike) -> FailureOr<Value> {
            Value extracted = materializeTileDataValue(rewriter, loc, tileLike, *as, elemTok);
            if (!typeConverter)
                return extracted;
...
static Value createFFTSMsg(ConversionPatternRewriter& rewriter, Location loc, Value eventId, int64_t fftsMode)
{
    auto* ctx = rewriter.getContext();
    ...
    IntegerAttr eventIdAttr, int64_t fftsMode)
{
    ...
    if (targetArch == PTOArch::A3) {
        ...
        Value eventVal = makeEmitCIntConstant(rewriter, loc, indexTy, getIntegerAttrSignedValue(eventIdAttr));
...
struct ArithUnsignedBitwiseBinaryToEmitC : public OpConversionPattern<ArithOp> {
        ...
        ArithOp op, typename ArithOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (bitWidth == 1) {
            rewriter.replaceOpWithNewOp<EmitCOp>(op, dstTy, adaptor.getLhs(), adaptor.getRhs());
            return success();
        }
...
struct ArithCeilDivSIToEmitC : public OpConversionPattern<arith::CeilDivSIOp> {
        ...
        arith::CeilDivSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value q0 = rewriter.create<emitc::DivOp>(loc, dstTy, adaptor.getLhs(), adaptor.getRhs());
        Value r = rewriter.create<emitc::RemOp>(loc, dstTy, adaptor.getLhs(), adaptor.getRhs());

        Value rNeZero = rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::ne, r, zero);
        Value lhsLt0 =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, adaptor.getLhs(), zero);
        Value rhsLt0 =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, adaptor.getRhs(), zero);
        Value signsSame =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::eq, lhsLt0, rhsLt0);
        Value adjust = rewriter.create<emitc::LogicalAndOp>(loc, rewriter.getI1Type(), rNeZero, signsSame);

        Value qPlusOne = rewriter.create<emitc::AddOp>(loc, dstTy, q0, one);
        Value result = rewriter.create<emitc::ConditionalOp>(loc, dstTy, adjust, qPlusOne, q0);
        rewriter.replaceOp(op, result);
        return success();
    }
};
...
struct ArithFloorDivSIToEmitC : public OpConversionPattern<arith::FloorDivSIOp> {
        ...
        arith::FloorDivSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value signsDifferent =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::ne, lhsLt0, rhsLt0);
        Value adjust = rewriter.create<emitc::LogicalAndOp>(loc, rewriter.getI1Type(), rNeZero, signsDifferent);

        Value qMinusOne = rewriter.create<emitc::SubOp>(loc, dstTy, q0, one);
        Value result = rewriter.create<emitc::ConditionalOp>(loc, dstTy, adjust, qMinusOne, q0);
        rewriter.replaceOp(op, result);
        return success();
    }
};
...
struct ArithShiftLeftToEmitC : public OpConversionPattern<arith::ShLIOp> {
        ...
        arith::ShLIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (bitWidth == 1) {
            ...
            Value sh = rewriter.create<emitc::BitwiseLeftShiftOp>(loc, u8Ty, lhsU8, rhsU8);
            Value masked =
                rewriter.create<emitc::BitwiseAndOp>(loc, u8Ty, sh, makeEmitCIntConstant(rewriter, loc, u8Ty, 1));
        ...
        Value lhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getLhs(), bitWidth);
        Value rhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getRhs(), bitWidth);
        Value shU = rewriter.create<emitc::BitwiseLeftShiftOp>(loc, uTy, lhsU, rhsU);
...
struct ArithShiftRightUIToEmitC : public OpConversionPattern<arith::ShRUIOp> {
        ...
        arith::ShRUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value lhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getLhs(), bitWidth);
        Value rhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getRhs(), bitWidth);
        Value shU = rewriter.create<emitc::BitwiseRightShiftOp>(loc, uTy, lhsU, rhsU);
        Value result = emitCCast(rewriter, loc, dstTy, shU);
        rewriter.replaceOp(op, result);
        return success();
    }
...
struct ArithCastToEmitC : public OpConversionPattern<CastOp> {
    ...
    LogicalResult matchAndRewrite(
        CastOp op, typename CastOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type dstTy = this->getTypeConverter()->convertType(op.getType());
        if (!dstTy)
            return failure();
...
struct ArithMulExtendedToEmitC : public OpConversionPattern<ArithOp> {
        ...
        ArithOp op, typename ArithOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Type wideTy = isUnsigned ? (Type)getWiderUnsignedIntOpaqueType(rewriter.getContext(), bitWidth) :
                                   (Type)getWiderSignedIntOpaqueType(rewriter.getContext(), bitWidth);
...
struct ArithMaxSIToEmitC : public OpConversionPattern<arith::MaxSIOp>, ArithMinMaxIToEmitCBase {
        ...
        arith::MaxSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value cond = rewriter
                         .create<emitc::CmpOp>(
                             loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, adaptor.getLhs(), adaptor.getRhs())
                         .getResult();
        Value res = makeSelect(rewriter, loc, dstTy, cond, adaptor.getRhs(), adaptor.getLhs());
        ...
        arith::MinSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value cond = rewriter
                         .create<emitc::CmpOp>(
                             loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, adaptor.getLhs(), adaptor.getRhs())
                         .getResult();
        Value res = makeSelect(rewriter, loc, dstTy, cond, adaptor.getLhs(), adaptor.getRhs());
        rewriter.replaceOp(op, res);
        return success();
    }
};
...
struct ArithMaxUIToEmitC : public OpConversionPattern<arith::MaxUIOp>, ArithMinMaxIToEmitCBase {
        ...
        arith::MaxUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value rhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getRhs(), bitWidth);
        Value cond =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, lhsU, rhsU).getResult();
        Value res = makeSelect(rewriter, loc, dstTy, cond, adaptor.getRhs(), adaptor.getLhs());
        rewriter.replaceOp(op, res);
        return success();
    }
...
struct ArithMaxNumFToEmitC : public OpConversionPattern<arith::MaxNumFOp>, ArithFloatMinMaxToEmitCBase {
        ...
        arith::MaxNumFOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value cmpLt = rewriter
                          ...
                          .getResult();
        Value maxNoNaN =
            rewriter.create<emitc::ConditionalOp>(loc, dstTy, cmpLt, adaptor.getRhs(), adaptor.getLhs()).getResult();

        Value rhsOrMax =
            rewriter.create<emitc::ConditionalOp>(loc, dstTy, rhsNaN, adaptor.getLhs(), maxNoNaN).getResult();
        Value res = rewriter.create<emitc::ConditionalOp>(loc, dstTy, lhsNaN, adaptor.getRhs(), rhsOrMax).getResult();
        rewriter.replaceOp(op, res);
        return success();
    }
};
...
struct ArithMinNumFToEmitC : public OpConversionPattern<arith::MinNumFOp>, ArithFloatMinMaxToEmitCBase {
        ...
        arith::MinNumFOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value cmpLt = rewriter
                          ...
                          .getResult();
        Value minNoNaN =
            rewriter.create<emitc::ConditionalOp>(loc, dstTy, cmpLt, adaptor.getLhs(), adaptor.getRhs()).getResult();

        Value rhsOrMin =
            rewriter.create<emitc::ConditionalOp>(loc, dstTy, rhsNaN, adaptor.getLhs(), minNoNaN).getResult();
        Value res = rewriter.create<emitc::ConditionalOp>(loc, dstTy, lhsNaN, adaptor.getRhs(), rhsOrMin).getResult();
        rewriter.replaceOp(op, res);
        return success();
    }
};
...
static FailureOr<std::string> buildEmitCOpaqueConstantLiteral(Type targetType, Attribute valueAttr)
{
    ...
    if (opaqueTy.getValue() == "pto::MrgSortExecutedNumList") {
        ...
        if (!vecTy || vecTy.getRank() != 1 || vecTy.getNumElements() != 4 || !vecTy.getElementType().isInteger(16))
            return failure();
...
struct ArithMulIToEmitC : public OpConversionPattern<arith::MulIOp> {
        ...
        arith::MulIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (bitWidth == 1) {
            rewriter.replaceOpWithNewOp<emitc::BitwiseAndOp>(op, opTy, adaptor.getLhs(), adaptor.getRhs());
            return success();
        }
...
struct ArithAddIToEmitC : public OpConversionPattern<arith::AddIOp> {
        ...
        arith::AddIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (bitWidth == 1) {
            rewriter.replaceOpWithNewOp<emitc::BitwiseXorOp>(op, opTy, adaptor.getLhs(), adaptor.getRhs());
            return success();
        }
...
struct ArithDivSIToEmitC : public OpConversionPattern<arith::DivSIOp> {
        ...
        arith::DivSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (!newTy)
            return failure();
        rewriter.replaceOpWithNewOp<emitc::DivOp>(op, newTy, adaptor.getLhs(), adaptor.getRhs());
        return success();
    }
};
...
struct ArithRemSIToEmitC : public OpConversionPattern<arith::RemSIOp> {
        ...
        arith::RemSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (!newTy)
            return failure();
        rewriter.replaceOpWithNewOp<emitc::RemOp>(op, newTy, adaptor.getLhs(), adaptor.getRhs());
        return success();
    }
};
...
struct PTOMGatherToMGATHER : public OpConversionPattern<pto::MGatherOp> {
        ...
        pto::MGatherOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value memArg = mem;
        auto coalescePropAttr = dyn_cast_or_null<pto::CoalesceAttr>(op.getProperties().coalesce);
        auto gatherOobAttr = dyn_cast_or_null<pto::GatherOOBAttr>(op.getProperties().gatherOob);
        pto::GatherOOB gatherOob = gatherOobAttr ? gatherOobAttr.getValue() : pto::GatherOOB::Undefined;
        ...
        if (!coalescePropAttr)
            return op.emitError("expects mgather to specify an explicit coalesce attribute (row or "
                                "elem)");

        SmallVector<Attribute, 2> templateArgVec;
        templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, coalesceTok(coalescePropAttr.getValue())));
        if (op.getGatherOob() != pto::GatherOOB::Undefined) {
            templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, gatherOobTok(gatherOob)));
        }
        ArrayAttr templateArgs = templateArgVec.empty() ? ArrayAttr{} : rewriter.getArrayAttr(templateArgVec);
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "MGATHER", ArrayAttr{}, templateArgs, ValueRange(callArgs));
...
struct FuncToEmitC : public OpConversionPattern<func::FuncOp> {
        ...
        func::FuncOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (op.isDeclaration()) {
            emitcFunc.setSpecifiersAttr(rewriter.getStrArrayAttr({"extern \"C\"", "AICORE"}));
            rewriter.eraseOp(op);
            return success();
        }

        if (pto::isPTOEntryFunction(op)) {
            emitcFunc.setSpecifiersAttr(rewriter.getStrArrayAttr({"extern \"C\"", "__global__ AICORE"}));
        ...
        if (failed(rewriter.convertRegionTypes(&emitcFunc.getBody(), *getTypeConverter(), &entryConv)))
            return failure();
...
struct SubviewToEmitCPattern : public OpConversionPattern<memref::SubViewOp> {
        ...
        memref::SubViewOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto mkIndex = [&](int64_t v) -> Value {
            return rewriter.create<emitc::ConstantOp>(loc, indexTy, emitc::OpaqueAttr::get(ctx, std::to_string(v)));
        };
        ...
        if (auto rc = op.getSource().getDefiningOp<memref::ReinterpretCastOp>()) {
            ...
            bool useTypeStrides = succeeded(mlir::pto::getPTOMemRefStridesAndOffset(srcType, strideInts, offset));
            (void)offset;
            if (useTypeStrides) {
        ...
        } else if (auto uc = sourcePtr.getDefiningOp<UnrealizedConversionCastOp>()) {
        ...
        Value newPtr;
        {
            ...
            if (typedSourcePtr.getType() != typedPtrTy)
                typedSourcePtr = rewriter.create<emitc::CastOp>(loc, typedPtrTy, typedSourcePtr);
        ...
        for (int i = 0; i < resTy.getRank(); ++i) {
            ...
            if (i < (int)mixedSizes.size())
                ...
                sizeValues.push_back(mkIndex(resShape[i] == ShapedType::kDynamic ? 1 : resShape[i]));
        }
        ...
        for (int i = 0; i < rank; ++i) {
            OpFoldResult srcStrideOfr = (i < (int)sourceStrides.size()) ? sourceStrides[i] : rewriter.getIndexAttr(1);
            OpFoldResult stepOfr = (i < (int)subViewSteps.size()) ? subViewSteps[i] : rewriter.getIndexAttr(1);
            ...
            if (stepStatic && *stepStatic == 1)
                ...
                strideValues.push_back(rewriter.create<emitc::MulOp>(loc, indexTy, srcV, stepV));
        }
        ...
        buildGlobalTensorShapeAndStride(shapeParamsVec, strideTemplateVec, finalShape, finalStride);
        ...
        for (int i = 3; i >= 0; --i) {
            ...
            if (finalShape[i + 1] == 1) {
                ...
                finalStrideValues[i] =
                    rewriter.create<emitc::MulOp>(loc, indexTy, finalShapeValues[i + 1], finalStrideValues[i + 1]);
            }
        }
        ...
        auto specialScaleSpec =
            getSpecialGlobalTensorTypeSpecForLayout(resolvedLayout, resTy.getShape(), resTy.getElementType());

        // Spelled-out C++ types.
        std::string shapeCppType =
            specialScaleSpec ? specialScaleSpec->shapeTypeExpr : "pto::Shape<" + shapeParams + ">";
        std::string strideCppType =
            specialScaleSpec ? specialScaleSpec->strideTypeExpr : "pto::Stride<" + strideParams + ">";
        ...
        if (specialScaleSpec) {
            ...
            if (allStatic) {
                if (finalShape[2] == 16 && finalShape[2] * finalShape[3] * elemBytes == 512 && finalStride[4] == 1 &&
                    finalStride[3] == finalShape[4]) {
                    layoutTag = 2; // NZ
                } else {
                    bool isRow = finalStride[4] == 1;
                    for (int i = 3; i >= 0; --i)
                        isRow &= (finalStride[i] == multiplyOrDynamic(finalStride[i + 1], finalShape[i + 1]));
                    bool isCol = finalStride[0] == 1;
                    for (int i = 0; i < 4; ++i)
                        isCol &= (finalStride[i + 1] == multiplyOrDynamic(finalStride[i], finalShape[i]));
                    if (isCol)
                        layoutTag = 1; // DN
                    else
        ...
        std::string gtCppType =
            "GlobalTensor<" + elemTypeStr + ", " + shapeCppType + ", " + strideCppType + ", " + layoutEnum + ">";
...
static std::string getElemTypeStringForGT(Type elemTy) { return getEmitCScalarTypeToken(elemTy); }
...
static bool getStaticMemrefLayout(MemRefType mrTy, SmallVectorImpl<int64_t>& strides, int64_t& offset)
{
    if (failed(mlir::pto::getPTOMemRefStridesAndOffset(mrTy, strides, offset))) {
        strides.clear();
        int64_t stride = 1;
...
static Value applyStaticMemrefOffset(ConversionPatternRewriter& rewriter, Location loc, Value basePtr, int64_t offset)
{
    if (offset == 0)
        return basePtr;
    auto* ctx = rewriter.getContext();
    ...
    MLIRContext* ctx, Type elemTy, ArrayRef<int64_t> shape, StringRef layoutEnum)
{
    ...
    for (auto [index, dim] : llvm::enumerate(shape))
        shape5D[shift + static_cast<int64_t>(index)] = ShapedType::isDynamic(dim) ? -1 : dim;

    std::string elemTypeStr = getElemTypeStringForGT(elemTy);
    std::string shapeType = "pto::Shape<" + joinIntTemplateParams(shape5D) + ">";
    std::string strideType = "pto::Stride<" + joinIntTemplateParams(stride5D) + ">";
    return emitc::OpaqueType::get(
        ctx, "GlobalTensor<" + elemTypeStr + ", " + shapeType + ", " + strideType + ", " + layoutEnum.str() + ">");
}
...
static std::string inferFallbackGlobalTensorLayout(ArrayRef<int64_t> shape5D, ArrayRef<int64_t> stride5D, Type elemTy)
{
    ...
    if (elemBytes == 0)
        return "pto::Layout::ND";
    if (shape5D[2] == 16 && multiplyOrDynamic(shape5D[2], shape5D[3]) * elemBytes == 512 && stride5D[4] == 1 &&
        stride5D[3] == shape5D[4]) {
        return "pto::Layout::NZ";
    }
...
static GlobalTensorTypeNames getGlobalTensorTypeNames(Operation* anchor, StringRef tag = {})
{
...
static Value buildGlobalTensorFromMemref(
    ConversionPatternRewriter& rewriter, Location loc, Value basePtr, MemRefType mrTy, Operation* anchor, StringRef tag)
{
    auto* ctx = rewriter.getContext();
    ...
    if (auto spec = getSpecialScaleGlobalTensorTypeSpec(anchor, mrTy)) {
        rewriter.create<emitc::VerbatimOp>(loc, "using " + names.shapeTypeName + " = " + spec->shapeTypeExpr + ";");
        rewriter.create<emitc::VerbatimOp>(loc, "using " + names.strideTypeName + " = " + spec->strideTypeExpr + ";");
        ...
        layoutEnum = resolveGlobalTensorLayout(anchor, basePtr, shape5D, stride5D, mrTy.getElementType());
    }
    ...
    rewriter.create<emitc::VerbatimOp>(
        loc, "using " + names.tensorTypeName + " = GlobalTensor<" + elemTypeStr + ", " + names.shapeTypeName + ", " +
    ...
    StringRef tag)
{
    ...
    if (Value gt = buildGlobalTensorFromMemref(rewriter, loc, loweredValue, mrTy, anchor, tag))
        return gt;
    return loweredValue;
}

static Value castToGMBytePointer(ConversionPatternRewriter& rewriter, Location loc, Value value)
{
    auto* ctx = rewriter.getContext();
...
static Value materializeGlobalTensorDataPointer(
    ConversionPatternRewriter& rewriter, Location loc, Value value, Type sourceType)
{
    ...
    else if (auto partitionTy = dyn_cast<pto::PartitionTensorViewType>(sourceType))
...
static std::string tileBufSLayoutToken(pto::TileBufConfigAttr configAttr)
{
    ...
    if (auto slAttr = dyn_cast<SLayoutAttr>(configAttr.getSLayout())) {
        ...
        slTok = (slVal == 1) ? "SLayout::RowMajor" : (slVal == 2) ? "SLayout::ColMajor" : "SLayout::NoneBox";
    }
    return slTok;
}
...
static int64_t renderTileTemplateDim(int64_t rawDim, Type elemTy, pto::BLayout blayout, int dimIdx)
{
    assert(dimIdx >= 0 && dimIdx < 2 && "renderTileTemplateDim expects a rank-2 rows/cols dimension index");
    ...
    ConversionPatternRewriter& rewriter, Location loc, Value originalScratch, Value emittedScratch)
{
    ...
    std::string elemTypeStr = getEmitCScalarTypeToken(elemTy);
    std::string tileTypeStr = "Tile<TileType::Vec, " + elemTypeStr + ", " + std::to_string(templateRows) + ", " +
                              std::to_string(templateCols) + ", " + tileBufBLayoutToken(configAttr) + ", " +
    ...
    Value tile = rewriter
                     .create<emitc::VariableOp>(
                         loc, getEmitCVariableResultType(emitc::OpaqueType::get(ctx, tileTypeStr)),
                         emitc::OpaqueAttr::get(ctx, ""))
                     .getResult();
    tile = loadEmitCVariableIfNeeded(rewriter, loc, tile);
    auto addr = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, "uint64_t")});
    Value scratchAddr = rewriter
                            .create<emitc::CallOpaqueOp>(
                                loc, emitc::OpaqueType::get(ctx, "uint64_t"), "reinterpret_cast", ArrayAttr{}, addr,
                                ValueRange{scratch})
                            .getResult(0);
    rewriter.create<emitc::CallOpaqueOp>(
...
struct CastPtrConversion : public OpConversionPattern<pto::CastPtrOp> {
        ...
        pto::CastPtrOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (auto resultPtrTy = dyn_cast<pto::PtrType>(op.getResult().getType())) {
            ...
            std::optional<pto::AddressSpace> as = getAddressSpaceOrGM(resultPtrTy.getMemorySpace());
            if (!as)
            ...
            if (isEmitCTileLikeType(peeledInput.getType())) {
                Value ptr = rewriter
                                .create<emitc::CallOpaqueOp>(
                                    op.getLoc(), convertedResultType, "PTOAS__TILE_DATA", ArrayAttr{}, ArrayAttr{},
                                    ValueRange{peeledInput})
                                .getResult(0);
                rewriter.replaceOp(op, ptr);
                return success();
            }

            Value ptr = materializeAddressAsPointer(rewriter, op.getLoc(), peeledInput, *as, elemTok);
            if (ptr.getType() != convertedResultType)
                ptr = rewriter.create<emitc::CastOp>(op.getLoc(), convertedResultType, ptr).getResult();
            rewriter.replaceOp(op, ptr);
            return success();
        }

        if (isa<IntegerType>(op.getResult().getType()) && emitc::isSupportedEmitCType(convertedResultType)) {
            Value source = input;
            if (!emitc::isSupportedEmitCType(source.getType())) {
                if (auto inputPtrTy = dyn_cast<pto::PtrType>(op.getInput().getType())) {
                    std::string elemTok = getEmitCScalarTypeToken(inputPtrTy.getElementType());
                    std::optional<pto::AddressSpace> as = getAddressSpaceOrGM(inputPtrTy.getMemorySpace());
                    if (!as)
                        return rewriter.notifyMatchFailure(op, "unsupported ptr address space");
                    if (isEmitCTileLikeType(peeledInput.getType())) {
                        Type convertedInputType = getTypeConverter()->convertType(op.getInput().getType());
                        if (!convertedInputType)
                            return rewriter.notifyMatchFailure(op, "failed to convert input ptr type");
                        source = rewriter
                                     .create<emitc::CallOpaqueOp>(
                                         op.getLoc(), convertedInputType, "PTOAS__TILE_DATA", ArrayAttr{}, ArrayAttr{},
                                         ValueRange{peeledInput})
                                     .getResult(0);
                    } else {
                        source = materializeAddressAsPointer(rewriter, op.getLoc(), peeledInput, *as, elemTok);
                    }
                }
            }
            if (!emitc::isSupportedEmitCType(source.getType()))
            ...
            auto templateArgs = rewriter.getArrayAttr({emitc::OpaqueAttr::get(
                rewriter.getContext(), cast<emitc::OpaqueType>(convertedResultType).getValue())});
            auto cast = rewriter.create<emitc::CallOpaqueOp>(
                op.getLoc(), convertedResultType, "reinterpret_cast", ArrayAttr{}, templateArgs, ValueRange{source});
            rewriter.replaceOp(op, cast.getResult(0));
            return success();
        }

        if (emitc::isSupportedEmitCType(input.getType()) && emitc::isSupportedEmitCType(convertedResultType)) {
...
struct PTOTLoadToTLOAD : public OpConversionPattern<pto::TLoadOp> {
        ...
        pto::TLoadOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "TLOAD", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, src});
...
struct PTOTPrefetchToTPREFETCH : public OpConversionPattern<pto::TPrefetchOp> {
        ...
        pto::TPrefetchOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "TPREFETCH", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, src});
        rewriter.eraseOp(op);
        return success();
    }
};
...
struct CallToEmitC : public OpConversionPattern<func::CallOp> {
        ...
        func::CallOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (originalArgCount != adaptor.getOperands().size())
            return rewriter.notifyMatchFailure(op, "call operand count mismatch after type conversion");

        for (auto [index, loweredOperand] : llvm::enumerate(adaptor.getOperands())) {
            FailureOr<Value> adapted = adaptCallOperandForEmitC(
                getTypeConverter(), rewriter, op.getLoc(), calleeType.getInput(index), op.getOperand(index),
                loweredOperand);
            if (failed(adapted))
                return rewriter.notifyMatchFailure(op, "failed to adapt call operand for EmitC ABI");
            operands.push_back(*adapted);
        }

        rewriter.replaceOpWithNewOp<emitc::CallOp>(op, op.getCalleeAttr(), resultTypes, operands);
        return success();
    }
};
...
static void emitDsbDdr(ConversionPatternRewriter& rewriter, Location loc)
{
    ...
    rewriter.create<emitc::CallOpaqueOp>(loc, TypeRange{}, "dsb", args, ArrayAttr{}, ValueRange{});
}

static void emitPipeBarrier(ConversionPatternRewriter& rewriter, Location loc, StringRef pipeTok)
{
    auto* ctx = rewriter.getContext();
    auto args = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, pipeTok)});
    rewriter.create<emitc::CallOpaqueOp>(loc, TypeRange{}, "pipe_barrier", args, ArrayAttr{}, ValueRange{});
}

static void emitConservativeGmFencePipeDrains(ConversionPatternRewriter& rewriter, Location loc)
{
...
static bool isInVectorKernel(Operation* op)
{
    for (Operation* parent = op->getParentOp(); parent; parent = parent->getParentOp()) {
        ...
        auto kernelKindAttr = parent->getAttrOfType<FunctionKernelKindAttr>(FunctionKernelKindAttr::name);
        if (kernelKindAttr)
...
struct PTOFenceToEmitC : public OpConversionPattern<FenceOp> {
    ...
    {
        (void)adaptor;
        if (op.getScope().getScope() != pto::FenceScope::GM && op.getScope().getScope() != pto::FenceScope::All)
...
struct PTOSyncAllToEmitC : public OpConversionPattern<mlir::pto::SyncAllOp> {
        ...
        mlir::pto::SyncAllOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto buildGmWorkspace = [&]() -> FailureOr<Value> {
            ...
            if (!ptrTy)
                return failure();
            return buildSyncAllGlobalTensorFromPointer(rewriter, op.getLoc(), gm, ptrTy.getElementType());
        };
        ...
        Value usedCores = adaptor.getUsedCores() ?
                              ...
                              rewriter.create<emitc::LiteralOp>(op.getLoc(), i32Ty, "int32_t{0}").getResult();
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, callee, ArrayAttr{}, ArrayAttr{}, ValueRange{*gmWorkspace, usedCores});
        rewriter.eraseOp(op);
        return success();
    }
};
...
struct PTOGetBufDynToEmitC : public OpConversionPattern<mlir::pto::GetBufDynOp> {
    ...
    LogicalResult matchAndRewrite(
        mlir::pto::GetBufDynOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto* ctx = rewriter.getContext();
...
struct PTORlsBufDynToEmitC : public OpConversionPattern<mlir::pto::RlsBufDynOp> {
    ...
    LogicalResult matchAndRewrite(
        mlir::pto::RlsBufDynOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto* ctx = rewriter.getContext();
...
struct PTOSyncSetToEmitC : public OpConversionPattern<mlir::pto::SyncSetOp> {
        ...
        mlir::pto::SyncSetOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (targetArch == PTOArch::A5) {
            ...
            auto emitSet = [&](Value eventOperand, IntegerAttr eventLiteral, bool isDynamic) {
                if (isDynamic) {
...
struct PTOMScatterToMSCATTER : public OpConversionPattern<pto::MScatterOp> {
        ...
        pto::MScatterOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value idx = adaptor.getIdx();
        Value mem = adaptor.getMem();
        auto coalesceAttr = dyn_cast_or_null<pto::CoalesceAttr>(op.getProperties().coalesce);
        auto scatterAtomicAttr = dyn_cast_or_null<pto::ScatterAtomicOpAttr>(op.getProperties().scatterAtomicOp);
        auto scatterOobAttr = dyn_cast_or_null<pto::ScatterOOBAttr>(op.getProperties().scatterOob);
        auto scatterConflictAttr = dyn_cast_or_null<pto::ScatterConflictAttr>(op.getProperties().scatterConflict);
        pto::ScatterAtomicOp scatterAtomicOp =
            scatterAtomicAttr ? scatterAtomicAttr.getValue() : pto::ScatterAtomicOp::None;
        pto::ScatterOOB scatterOob = scatterOobAttr ? scatterOobAttr.getValue() : pto::ScatterOOB::Undefined;
        ...
        if (coalesceAttr) {
            templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, coalesceTok(coalesceAttr.getValue())));
            if (scatterConflictAttr) {
                templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, scatterAtomicTok(scatterAtomicOp)));
                templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, scatterOobTok(scatterOob)));
                templateArgVec.push_back(
                    emitc::OpaqueAttr::get(ctx, scatterConflictTok(scatterConflictAttr.getValue())));
            } else if (scatterAtomicOp != pto::ScatterAtomicOp::None || scatterOob != pto::ScatterOOB::Undefined) {
                templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, scatterAtomicTok(scatterAtomicOp)));
        ...
        pto::THistogramOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (auto legacyIsMSB = op->getAttrOfType<BoolAttr>("isMSB")) {
            ...
            if (byteAttr && byte != legacyByte)
                return rewriter.notifyMatchFailure(op, "conflicting 'byte' and legacy 'isMSB' attributes");
            byte = legacyByte;
        }
        switch (byte) {
            case 0:
        ...
        auto templateArgs = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, histByte)});
        rewriter.create<emitc::CallOpaqueOp>(
...
struct PTOGetScaleAddrToEmitC : public OpConversionPattern<pto::TGetScaleAddrOp> {
        ...
        pto::TGetScaleAddrOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (!srcSpace || !srcElemTy)
            return rewriter.notifyMatchFailure(op, "failed to resolve src address space or element type");

        std::string srcElemTok = getEmitCScalarTypeToken(srcElemTy);
        auto isEmitCTileLike = [](Type ty) {
            auto opaqueTy = dyn_cast<emitc::OpaqueType>(ty);
            return opaqueTy && (opaqueTy.getValue().contains("Tile<") || opaqueTy.getValue().contains("ConvTile<"));
        };
        Type convertedSrcTy = getTypeConverter()->convertType(op.getSrc().getType());
        if (!convertedSrcTy || !isEmitCTileLike(convertedSrcTy))
            return rewriter.notifyMatchFailure(op, "expected src to lower to a tile-like value");
        Value rawPtr = src;
        rawPtr = materializeTileDataValue(rewriter, loc, src, *srcSpace, srcElemTok);
        if (tileDataReturnsIntegralAddress(*srcSpace))
            rawPtr = materializeAddressAsPointer(rewriter, loc, rawPtr, *srcSpace, srcElemTok);
...
struct PTOGetValidShapeToEmitC : public OpConversionPattern<pto::GetValidShapeOp> {
        ...
        pto::GetValidShapeOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value row = rewriter
                        .create<emitc::CallOpaqueOp>(
                            rowLoc, resultTy, "PTOAS__TILE_GET_VALID_ROW", ArrayAttr{}, ArrayAttr{}, ValueRange{src})
                        .getResult(0);
        Value col = rewriter
                        .create<emitc::CallOpaqueOp>(
                            colLoc, resultTy, "PTOAS__TILE_GET_VALID_COL", ArrayAttr{}, ArrayAttr{}, ValueRange{src})
                        .getResult(0);
...
struct PTOTAssignToEmitC : public OpConversionPattern<pto::TAssignOp> {
        ...
        pto::TAssignOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (isa<emitc::PointerType>(addr.getType()) ||
            ...
            addr =
                rewriter
                    .create<emitc::CallOpaqueOp>(loc, u64Ty, "reinterpret_cast", ArrayAttr{}, rcU64, ValueRange{addr})
                    .getResult(0);
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            loc, TypeRange{}, "TASSIGN", ArrayAttr{}, ArrayAttr{}, ValueRange{tile, addr});
...
static void emitInvalidateGmCacheAll(ConversionPatternRewriter& rewriter, Location loc)
{
    auto* ctx = rewriter.getContext();
    auto args = rewriter.getArrayAttr({
    ...
    rewriter.create<emitc::CallOpaqueOp>(loc, TypeRange{}, "dcci", args, ArrayAttr{}, ValueRange{});
}

static void emitInvalidateGmCacheSingleLine(ConversionPatternRewriter& rewriter, Location loc, Value addr)
{
    rewriter.create<emitc::CallOpaqueOp>(
        loc, TypeRange{}, "PTOAS__DCCI_SINGLE_CACHE_LINE", ArrayAttr{}, ArrayAttr{}, ValueRange{addr});
}
...
struct PTOCmoCacheInvalidToEmitC : public OpConversionPattern<pto::CmoCacheInvalidOp> {
    using OpConversionPattern<pto::CmoCacheInvalidOp>::OpConversionPattern;

    LogicalResult matchAndRewrite(
        pto::CmoCacheInvalidOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (op.getAddr()) {
            ...
            addr = materializeGlobalTensorDataPointer(rewriter, op.getLoc(), addr, op.getAddr().getType());
...
struct PTOStoreScalarToEmitC : public OpConversionPattern<pto::StoreScalarOp> {
        ...
        pto::StoreScalarOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "PTOAS__SCALAR_GM_STORE_FLUSH", ArrayAttr{}, ArrayAttr{}, ValueRange{ptr});
...
struct PTOTAddToTADD : public OpConversionPattern<pto::TAddOp> {
    ...
    LogicalResult matchAndRewrite(pto::TAddOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TADD", ValueRange{dst, src0, src1});
...
struct PTOInitializeL2G2LPipeToEmitC : public OpConversionPattern<mlir::pto::InitializeL2G2LPipeOp> {
        ...
        mlir::pto::InitializeL2G2LPipeOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        gmAddr = materializeGlobalTensorDataPointer(rewriter, op.getLoc(), gmAddr, op.getGmAddr().getType());
        Value localAddr = op.getLocalAddr() ? adaptor.getLocalAddr() : Value();
...
struct PTOBuildAsyncSessionToEmitC : public OpConversionPattern<mlir::pto::BuildAsyncSessionOp> {
        ...
        mlir::pto::BuildAsyncSessionOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value workspace = castToGMBytePointer(rewriter, loc, adaptor.getWorkspace());

        Value session =
            rewriter
                .create<emitc::VariableOp>(loc, getEmitCVariableResultType(sessionTy), emitc::OpaqueAttr::get(ctx, ""))
                .getResult();
        ...
        uint64_t syncId = op.getSyncIdAttr() ? static_cast<uint64_t>(getIntegerAttrSignedValue(op.getSyncIdAttr())) : 0;
        uint64_t blockBytes = op.getBlockBytesAttr() ?
                                  static_cast<uint64_t>(getIntegerAttrSignedValue(op.getBlockBytesAttr())) :
                                  32 * 1024;
        uint64_t commBlockOffset = op.getCommBlockOffsetAttr() ?
                                       static_cast<uint64_t>(getIntegerAttrSignedValue(op.getCommBlockOffsetAttr())) :
                                       0;
        uint64_t queueNum =
            op.getQueueNumAttr() ? static_cast<uint64_t>(getIntegerAttrSignedValue(op.getQueueNumAttr())) : 1;
        uint64_t channelGroupIdx = op.getChannelGroupIdxAttr() ?
                                       static_cast<uint64_t>(getIntegerAttrSignedValue(op.getChannelGroupIdxAttr())) :
                                       UINT32_MAX;
...
struct PTOAsyncTransferToEmitC : public OpConversionPattern<AsyncOp> {
    ...
    explicit PTOAsyncTransferToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, StringRef callee)
        ...
        AsyncOp op, typename AsyncOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Value dst = peelGlobalTensorConversionBridge(adaptor.getDst());
        Value src = peelGlobalTensorConversionBridge(adaptor.getSrc());
        Type convertedDstTy = this->getTypeConverter()->convertType(op.getDst().getType());
        Type convertedSrcTy = this->getTypeConverter()->convertType(op.getSrc().getType());
        if (!convertedDstTy || !convertedSrcTy || !isEmitCGlobalTensorLikeType(convertedDstTy) ||
            !isEmitCGlobalTensorLikeType(convertedSrcTy))
            return rewriter.notifyMatchFailure(op, "expected GlobalTensor-like src and dst");
        ...
        rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(
            op, TypeRange{eventTy}, callee, ArrayAttr{}, ArrayAttr{}, ValueRange{dst, src, adaptor.getSession()});
        return success();
    }
...
struct PTOAsyncEventToEmitC : public OpConversionPattern<AsyncEventOp> {
    explicit PTOAsyncEventToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, StringRef callee)
        : OpConversionPattern<AsyncEventOp>(typeConverter, ctx), callee(callee.str())
    {}

    LogicalResult matchAndRewrite(
        AsyncEventOp op, typename AsyncEventOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(
            ...
            ValueRange{adaptor.getEvent(), adaptor.getSession()});
        return success();
    }
    ...
    ConversionPatternRewriter& rewriter, Location loc, ArrayRef<Value> groupGTs, int64_t root)
{
    ...
    auto groupArray = cast<TypedValue<emitc::ArrayType>>(
        rewriter.create<emitc::VariableOp>(loc, getEmitCVariableResultType(arrayTy), emitc::OpaqueAttr::get(ctx, "{}"))
            .getResult());
...
static void emitTNotifyReleaseActions(ConversionPatternRewriter& rewriter, Location loc, bool drainMte2, bool drainMte3)
{
    if (drainMte2)
        emitPipeBarrier(rewriter, loc, "PIPE_MTE2");
    if (drainMte3)
...
struct PTOCommCollectiveToEmitC : public OpConversionPattern<CollectiveOp> {
    ...
    explicit PTOCommCollectiveToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, StringRef apiName)
        ...
        CollectiveOp op, typename CollectiveOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if constexpr (std::is_same_v<CollectiveOp, pto::TBroadcastOp>) {
            FailureOr<Value> srcGT =
                buildCommGlobalTensorValue(rewriter, loc, op.getSrc(), adaptor.getSrc(), op.getOperation());
            ...
            if (op.getPong()) {
                FailureOr<Value> pongTile = buildPong(op.getPong(), adaptor.getPong(), "__pong");
                if (failed(pongTile))
            ...
            if (op.getRecvPong()) {
                FailureOr<Value> recvPong = buildPong(op.getRecvPong(), adaptor.getRecvPong(), "__recv_pong");
                if (failed(recvPong))
...
struct PTOP2PCommToEmitC : public OpConversionPattern<OpTy> {
    ...
    explicit PTOP2PCommToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, StringRef callee)
...
struct PTOSignalCommToEmitC : public OpConversionPattern<SignalOp> {
    ...
    explicit PTOSignalCommToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, StringRef callee)
        ...
        SignalOp op, typename SignalOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if constexpr (std::is_same_v<SignalOp, pto::TNotifyOp>) {
            ...
            SmallVector<Value> operands{*signalGT, adaptor.getValue(), notifyOp};
            ...
            SmallVector<Value> operands{*signalGT, adaptor.getCmpValue(), waitCmp};
...
struct PTODeclareGlobalToEmitC : public OpConversionPattern<mlir::pto::DeclareGlobalOp> {
        ...
        mlir::pto::DeclareGlobalOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto var = rewriter.create<emitc::VariableOp>(
            op.getLoc(), getEmitCVariableResultType(convertedType), emitc::OpaqueAttr::get(rewriter.getContext(), ""));
        rewriter.replaceOp(op, loadEmitCVariableIfNeeded(rewriter, op.getLoc(), var.getResult()));
        return success();
    }
};
...
struct PTODeclareEventIdArrayToEmitC : public OpConversionPattern<mlir::pto::DeclareEventIdArrayOp> {
        ...
        mlir::pto::DeclareEventIdArrayOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto array =
            rewriter
                .create<emitc::VariableOp>(
                    op.getLoc(), getEmitCVariableResultType(arrayTy), emitc::OpaqueAttr::get(rewriter.getContext(), ""))
                .getResult();
...
struct PTOEventIdArrayGetToEmitC : public OpConversionPattern<mlir::pto::EventIdArrayGetOp> {
        ...
        mlir::pto::EventIdArrayGetOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto subscript = rewriter.create<emitc::SubscriptOp>(op.getLoc(), resultTy, array, ValueRange{index});
...
struct PTOEventIdArraySetToEmitC : public OpConversionPattern<mlir::pto::EventIdArraySetOp> {
        ...
        mlir::pto::EventIdArraySetOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value slot =
            rewriter.create<emitc::SubscriptOp>(op.getLoc(), value.getType(), array, ValueRange{index}).getResult();
...
struct PTOLocalArrayGetToEmitC : public OpConversionPattern<mlir::pto::LocalArrayGetOp> {
        ...
        mlir::pto::LocalArrayGetOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto sub = rewriter.create<emitc::SubscriptOp>(op.getLoc(), resultTy, array, indices);
        auto snapshot =
            rewriter.create<emitc::VariableOp>(op.getLoc(), resultTy, emitc::OpaqueAttr::get(rewriter.getContext(), ""))
                .getResult();
...
struct PTODeclareStructToEmitC : public OpConversionPattern<mlir::pto::DeclareStructOp> {
    using OpConversionPattern<mlir::pto::DeclareStructOp>::OpConversionPattern;

    LogicalResult matchAndRewrite(
        mlir::pto::DeclareStructOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        ...
        if (!ptrTy)
            return rewriter.notifyMatchFailure(op, "!pto.struct did not map to a pointer");

        Value storage = rewriter
                            .create<emitc::VariableOp>(
                                op.getLoc(), ptrTy.getPointee(), emitc::OpaqueAttr::get(rewriter.getContext(), ""))
                            .getResult();
...
static FailureOr<Value> buildStructMemberChain(
    ConversionPatternRewriter& rewriter, Location loc, const TypeConverter* tc, Value root,
    mlir::pto::StructType rootPtoTy, llvm::ArrayRef<int64_t> path)
{
    ...
    if (applyOp && applyOp.getApplicableOperator() == "&") {
        ...
        if (!isa<emitc::PointerType>(ptr.getType()))
            return failure();
        ptrSlot =
            rewriter.create<emitc::VariableOp>(loc, ptr.getType(), emitc::OpaqueAttr::get(rewriter.getContext(), ""))
                .getResult();
    ...
    for (int64_t idx : path) {
        ...
        structLValue = structLValue ? rewriter.create<emitc::MemberOp>(loc, resultTy, name, structLValue).getResult() :
                                      rewriter.create<emitc::MemberOfPtrOp>(loc, resultTy, name, ptrSlot).getResult();
        curPtoTy = fieldPtoTy;
    }
    return structLValue;
}
...
struct PTOStructGetToEmitC : public OpConversionPattern<mlir::pto::StructGetOp> {
    using OpConversionPattern<mlir::pto::StructGetOp>::OpConversionPattern;

    LogicalResult matchAndRewrite(
        mlir::pto::StructGetOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        FailureOr<Value> member = buildStructMemberChain(
            rewriter, op.getLoc(), getTypeConverter(), adaptor.getS(), op.getS().getType(), op.getPath());
        if (failed(member))
            return rewriter.notifyMatchFailure(op, "failed to map struct field type");
...
struct PTOStructSetToEmitC : public OpConversionPattern<mlir::pto::StructSetOp> {
    using OpConversionPattern<mlir::pto::StructSetOp>::OpConversionPattern;

    LogicalResult matchAndRewrite(
        mlir::pto::StructSetOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        FailureOr<Value> member = buildStructMemberChain(
            rewriter, op.getLoc(), getTypeConverter(), adaptor.getS(), op.getS().getType(), op.getPath());
        if (failed(member))
...
static FailureOr<Value> buildGlobalTensorViewFromPointer(
    ...
    ArrayRef<int64_t> strides = {}, std::optional<SpecialGlobalTensorTypeSpec> specialSpec = std::nullopt,
    ...
    ConversionPatternRewriter& rewriter, Location loc, Value ptr, Type elemTy)
{
    ...
    return buildGlobalTensorViewFromPointer(rewriter, loc, ptr, elemTy, shape, strides);
}
...
static Value castViewIndexToEmitC(ConversionPatternRewriter& rewriter, Location loc, Value value)
{
...
static Value makeViewIndexConstant(ConversionPatternRewriter& rewriter, Location loc, int64_t value)
{
    return makeEmitCIntConstant(rewriter, loc, emitc::OpaqueType::get(rewriter.getContext(), "int64_t"), value);
}

static Value getRuntimeGlobalTensorMetadata(
    ConversionPatternRewriter& rewriter, Location loc, Value tensor, Value logicalDim, int64_t rank, bool isStride)
{
    Value dim = castViewIndexToEmitC(rewriter, loc, logicalDim);
    int64_t shift = 5 - rank;
    if (shift != 0) {
        dim = rewriter.create<emitc::AddOp>(loc, dim.getType(), dim, makeViewIndexConstant(rewriter, loc, shift))
                  .getResult();
    }
    StringRef marker =
        isStride ? StringRef("PTOAS__GLOBAL_TENSOR_GET_STRIDE") : StringRef("PTOAS__GLOBAL_TENSOR_GET_SHAPE");
    return rewriter
        .create<emitc::CallOpaqueOp>(loc, dim.getType(), marker, ArrayAttr{}, ArrayAttr{}, ValueRange{tensor, dim})
        .getResult(0);
}

static FailureOr<Value> buildRuntimeGlobalTensor(
    ConversionPatternRewriter& rewriter, Location loc, Value ptr, Type elemTy, ArrayRef<int64_t> staticShape,
    ValueRange runtimeShape, ValueRange runtimeStrides, StringRef layoutEnum = "pto::Layout::ND")
{
    ...
    for (auto [index, value] : llvm::enumerate(runtimeStrides))
        strideValues[shift + static_cast<int64_t>(index)] = castViewIndexToEmitC(rewriter, loc, value);
    if (shift == 5) {
        ...
        for (int64_t dim = shift - 1; dim >= 0; --dim) {
            strideValues[dim] =
                rewriter
                    .create<emitc::MulOp>(
                        loc, strideValues[dim + 1].getType(), shapeValues[dim + 1], strideValues[dim + 1])
                    .getResult();
        }
    }
    ...
    Value shape = rewriter
                      .create<emitc::CallOpaqueOp>(
                          loc, emitc::OpaqueType::get(rewriter.getContext(), shapeType), shapeType, ArrayAttr{},
                          ArrayAttr{}, shapeValues)
                      .getResult(0);
    Value stride = rewriter
                       .create<emitc::CallOpaqueOp>(
                           loc, emitc::OpaqueType::get(rewriter.getContext(), strideType), strideType, ArrayAttr{},
                           ArrayAttr{}, strideValues)
                       .getResult(0);
    auto resultType = getRuntimeGlobalTensorOpaqueType(rewriter.getContext(), elemTy, staticShape, layoutEnum);
    return rewriter
        .create<emitc::CallOpaqueOp>(
            loc, resultType, resultType.getValue(), ArrayAttr{}, ArrayAttr{}, ValueRange{ptr, shape, stride})
        .getResult(0);
}
...
static bool isDeadPureEmitCValueOp(Operation* op)
{
    ...
    if (auto call = dyn_cast<emitc::CallOpaqueOp>(op)) {
        ...
        return callee.starts_with("pto::Shape<") || callee.starts_with("pto::Stride<") ||
...
struct PTOMakeTensorViewToEmitC : public OpConversionPattern<mlir::pto::MakeTensorViewOp> {
    using OpConversionPattern<mlir::pto::MakeTensorViewOp>::OpConversionPattern;

    LogicalResult matchAndRewrite(
        mlir::pto::MakeTensorViewOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        if (op->use_empty()) {
            rewriter.eraseOp(op);
            return success();
        ...
        auto layout = op.getLayoutAttr() ? layoutToEmitCString(op.getLayoutAttr().getLayout()) : "pto::Layout::ND";
        auto result = buildRuntimeGlobalTensor(
            rewriter, op.getLoc(), adaptor.getPtr(), resultType.getElementType(), resultType.getShape(),
            adaptor.getShape(), adaptor.getStrides(), layout);
        if (failed(result))
            return rewriter.notifyMatchFailure(op, "failed to build runtime GlobalTensor descriptor");
        rewriter.replaceOp(op, *result);
        return success();
    }
};
...
struct PTOGetTensorViewMetadataToEmitC : public OpConversionPattern<OpTy> {
        ...
        OpTy op, typename OpTy::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value result = getRuntimeGlobalTensorMetadata(
            rewriter, op.getLoc(), peelGlobalTensorConversionBridge(adaptor.getTensorView()), adaptor.getDimIndex(),
            rank, IsStride);
        rewriter.replaceOp(op, result);
        return success();
    }
...
struct PTOPartitionViewToEmitC : public OpConversionPattern<mlir::pto::PartitionViewOp> {
        ...
        mlir::pto::PartitionViewOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto resultType = dyn_cast<pto::PartitionTensorViewType>(op.getResult().getType());
        ...
        if (auto makeView = op.getSource().getDefiningOp<pto::MakeTensorViewOp>()) {
            if (makeView.getStrides().size() != static_cast<size_t>(sourceType.getRank()))
            ...
            for (Value stride : makeView.getStrides()) {
                ...
                sourceStrides.push_back(castViewIndexToEmitC(rewriter, op.getLoc(), mapped));
            }
        } else {
        ...
        for (auto [offset, stride] : llvm::zip(adaptor.getOffsets(), sourceStrides)) {
            Value term = rewriter
                             .create<emitc::MulOp>(
                                 op.getLoc(), linearOffset.getType(),
                                 castViewIndexToEmitC(rewriter, op.getLoc(), offset), stride)
                             .getResult();
            linearOffset =
                rewriter.create<emitc::AddOp>(op.getLoc(), linearOffset.getType(), linearOffset, term).getResult();
        }

        std::string elemTypeStr = getElemTypeStringForGT(sourceType.getElementType());
        auto ptrType = emitc::PointerType::get(emitc::OpaqueType::get(rewriter.getContext(), "__gm__ " + elemTypeStr));
        Value data =
            rewriter
                .create<emitc::CallOpaqueOp>(
                    op.getLoc(), ptrType, "PTOAS__GLOBAL_TENSOR_DATA", ArrayAttr{}, ArrayAttr{}, ValueRange{source})
                .getResult(0);
        Value ptr = rewriter.create<emitc::AddOp>(op.getLoc(), ptrType, data, linearOffset).getResult();

        auto layout = resolveLayoutForGlobalTensor(op.getOperation(), op.getSource());
        std::string layoutString = layout ? layoutToEmitCString(*layout) : "pto::Layout::ND";
        auto result = buildRuntimeGlobalTensor(
            rewriter, op.getLoc(), ptr, resultType.getElementType(), resultType.getShape(), adaptor.getSizes(),
            sourceStrides, layoutString);
        if (failed(result))
            return rewriter.notifyMatchFailure(op, "failed to build partition GlobalTensor descriptor");
        rewriter.replaceOp(op, *result);
        return success();
    }
};

struct PTOPartitionViewStaticToEmitC : public OpConversionPattern<mlir::pto::PartitionViewOp> {
    using OpConversionPattern<mlir::pto::PartitionViewOp>::OpConversionPattern;
        ...
        mlir::pto::PartitionViewOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (!partitionViewHasStaticResultShape(op))
            return rewriter.notifyMatchFailure(op, "globaltensor partition_view requires static result shape");
        ...
        if (auto makeView = op.getSource().getDefiningOp<pto::MakeTensorViewOp>()) {
            ...
            if (data.getType() != ptrTy)
                data = rewriter.create<emitc::CastOp>(op.getLoc(), ptrTy, data).getResult();
        } else {
        ...
        if (!dynamicOffsetTerms.empty()) {
            Type indexTy = emitc::OpaqueType::get(ctx, "int64_t");
            auto makeIndex = [&](int64_t value) { return makeEmitCIntConstant(rewriter, op.getLoc(), indexTy, value); };
            auto asIndex = [&](Value value) -> Value {
                if (value.getType() == indexTy)
                    return value;
                return rewriter.create<emitc::CastOp>(op.getLoc(), indexTy, value).getResult();
            };
            ...
            for (auto [offsetValue, stride] : dynamicOffsetTerms) {
                ...
                if (stride != 1) {
                    ...
                    term = rewriter.create<emitc::MulOp>(op.getLoc(), indexTy, term, strideValue).getResult();
                }
                totalOffset = rewriter.create<emitc::AddOp>(op.getLoc(), indexTy, totalOffset, term).getResult();
            }
            ptr = rewriter.create<emitc::AddOp>(op.getLoc(), data.getType(), data, totalOffset).getResult();
        } else {
            ptr = applyStaticMemrefOffset(rewriter, op.getLoc(), data, staticLinearOffset);
        }
        ...
        auto resultOr = buildGlobalTensorViewFromPointer(
            ...
            getSpecialGlobalTensorTypeSpecForLayout(
                resolveLayoutForGlobalTensor(op.getOperation(), op.getSource()), resTy.getShape(),
                resTy.getElementType()),
            resolveLayoutForGlobalTensor(op.getOperation(), op.getSource()) ?
                layoutToEmitCString(*resolveLayoutForGlobalTensor(op.getOperation(), op.getSource())) :
                "pto::Layout::ND");
        if (failed(resultOr))
...
struct PTOTAllocToEmitC : public OpConversionPattern<mlir::pto::TAllocOp> {
        ...
        mlir::pto::TAllocOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(
            op, TypeRange{}, callee, ArrayAttr{}, ArrayAttr{}, ValueRange{adaptor.getPipeHandle(), entry});
        return success();
    }
...
struct PTOSetQuantScalarToEmitC : public OpConversionPattern<mlir::pto::SetQuantScalarOp> {
    PTOSetQuantScalarToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
        : OpConversionPattern<mlir::pto::SetQuantScalarOp>(typeConverter, ctx), targetArch(targetArch)
    {}

    LogicalResult matchAndRewrite(
        mlir::pto::SetQuantScalarOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto outTypeAttr = op->getAttrOfType<StringAttr>(kEmitCScalarOutTypeAttrName);
        if (!outTypeAttr)
            return rewriter.notifyMatchFailure(
        ...
        ArrayAttr targs = rewriter.getArrayAttr({emitc::OpaqueAttr::get(rewriter.getContext(), outTok)});
        rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(
            op, TypeRange{}, "SET_QUANT_SCALAR", ArrayAttr{}, targs, ValueRange{scale});
        return success();
    }
...
struct PTOSetQuantVectorToEmitC : public OpConversionPattern<mlir::pto::SetQuantVectorOp> {
    PTOSetQuantVectorToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
        : OpConversionPattern<mlir::pto::SetQuantVectorOp>(typeConverter, ctx), targetArch(targetArch)
    {}

    LogicalResult matchAndRewrite(
        mlir::pto::SetQuantVectorOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(
            op, TypeRange{}, "SET_QUANT_VECTOR", ArrayAttr{}, ArrayAttr{}, ValueRange{adaptor.getScalingTile()});
        return success();
    }
...
struct PTOTPushToEmitC : public OpConversionPattern<mlir::pto::TPushOp> {
        ...
        mlir::pto::TPushOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        std::string callee;
        if (auto accPushEpilogue = getPipeInitAccPushEpilogue(getPipeInitDef(op.getPipeHandle()))) {
        ...
        if (Value aivSubblockId = adaptor.getAivSubblockid()) {
            Value aivSubblockIdI32 =
                rewriter.create<emitc::CastOp>(op.getLoc(), rewriter.getI32Type(), peelUnrealized(aivSubblockId));
...
struct PTOTPopToEmitC : public OpConversionPattern<mlir::pto::TPopOp> {
        ...
        mlir::pto::TPopOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        std::string callee = "TPOP<" + *pipeTok + ", " + *tileTok + ", " + *splitTok + ">";
        SmallVector<Value> callOperands{adaptor.getPipeHandle(), convertedTile};
        if (Value aivSubblockId = adaptor.getAivSubblockid()) {
            Value aivSubblockIdI32 =
                rewriter.create<emitc::CastOp>(op.getLoc(), rewriter.getI32Type(), peelUnrealized(aivSubblockId));
...
struct ReinterpretCastToEmitC : public OpConversionPattern<memref::ReinterpretCastOp> {
        ...
        memref::ReinterpretCastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        std::string tileTypeStr =
            std::string("Tile<") + roleTok + ", " + elemTok + ", " + std::to_string(templateRows) + ", " +
            std::to_string(templateCols) + ", BLayout::RowMajor, " + std::to_string(templateRows) + ", " +
            std::to_string(templateCols) + ", SLayout::NoneBox, 512, PadValue::Null, CompactMode::Null>";

        auto tileType = emitc::OpaqueType::get(ctx, tileTypeStr);
        Value tile =
            rewriter
                .create<emitc::VariableOp>(loc, getEmitCVariableResultType(tileType), emitc::OpaqueAttr::get(ctx, ""))
                .getResult();
        tile = loadEmitCVariableIfNeeded(rewriter, loc, tile);
...
struct MemRefCastToEmitC : public OpConversionPattern<memref::CastOp> {
    ...
    LogicalResult matchAndRewrite(
        memref::CastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct PTOAddSToTADDS : public OpConversionPattern<pto::TAddSOp> {
        ...
        pto::TAddSOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TADDS", ValueRange{dst, src, scalar});
...
struct PTOTCIToEmitC : public OpConversionPattern<pto::TCIOp> {
    ...
    LogicalResult matchAndRewrite(pto::TCIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (auto ot = mlir::dyn_cast<emitc::OpaqueType>(dst.getType())) {
            SmallVector<Attribute, 4> templateArgVec;
            templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, ot.getValue().str()));
            if (tmp) {
                auto tmpOt = mlir::dyn_cast<emitc::OpaqueType>(tmp.getType());
                if (!tmpOt)
                    return rewriter.notifyMatchFailure(op, "expected tmp tile to lower to emitc::OpaqueType");
                templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, tmpOt.getValue().str()));
            }
...
struct PTOColExpandDivToEmitC : public OpConversionPattern<pto::TColExpandDivOp> {
        ...
        pto::TColExpandDivOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (op.getPrecisionType() != pto::DivPrecision::Default) {
            ...
            templateArgs = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, precisionTok)});
        }
...
struct PTOColMaxToEmitC : public OpConversionPattern<pto::TColMaxOp> {
        ...
        pto::TColMaxOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TCOLMAX", ValueRange{dst, src});
...
struct PTOColMinToEmitC : public OpConversionPattern<pto::TColMinOp> {
        ...
        pto::TColMinOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TCOLMIN", ValueRange{dst, src});
        ...
        pto::TColSumOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (op.getTmp()) {
            ...
            createLastUseAwareOpaqueCall(
                rewriter, op.getOperation(), TypeRange{}, "TCOLSUM", ValueRange{dst, src, tmp, isBinaryVal},
            ...
            tileSlotOrder.push_back(op.getSrcMutable().getOperandNumber());
            createLastUseAwareOpaqueCall(
                rewriter, op.getOperation(), TypeRange{}, "TCOLSUM", ValueRange{dst, src}, ArrayAttr{}, ArrayAttr{},
                tileSlotOrder);
        }
...
struct PTODivToTDIV : public OpConversionPattern<pto::TDivOp> {
    ...
    LogicalResult matchAndRewrite(pto::TDivOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(
            rewriter, op.getOperation(), TypeRange{}, "TDIV", ValueRange{dst, src0, src1}, ArrayAttr{}, templateArgs);
...
struct PTODivSToEmitC : public OpConversionPattern<pto::TDivSOp> {
        ...
        pto::TDivSOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TDIVS", ValueRange{dst, src, scalar});
...
struct PTOExpToEmitC : public OpConversionPattern<pto::TExpOp> {
    ...
    LogicalResult matchAndRewrite(pto::TExpOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(
            rewriter, op.getOperation(), TypeRange{}, "TEXP", ValueRange{dst, src}, ArrayAttr{}, templateArgs);
...
struct PTOExpandsToEmitC : public OpConversionPattern<pto::TExpandsOp> {
        ...
        pto::TExpandsOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TEXPANDS", ValueRange{dst, scalar});
...
struct PTOExtractToEmitC : public OpConversionPattern<pto::TExtractOp> {
        ...
        pto::TExtractOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (hasMode || reluNonDefault) {
            ...
            if (!dstOT || !srcOT)
                return rewriter.notifyMatchFailure(op, "textract template lowering expects opaque dst/src types");
        ...
        rewriter.create<emitc::CallOpaqueOp>(loc, TypeRange{}, "TEXTRACT", ArrayAttr{}, templateArgs, operands);
        rewriter.eraseOp(op);
        return success();
    }
};
...
struct PTOExtractFPToEmitC : public OpConversionPattern<pto::TExtractFPOp> {
        ...
        pto::TExtractFPOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (hasMode || reluNonDefault) {
            ...
            if (!dstOT || !srcOT || !fpOT)
                return rewriter.notifyMatchFailure(op, "textract_fp template lowering expects opaque dst/src/fp types");
...
struct PTOInsertToEmitC : public OpConversionPattern<pto::TInsertOp> {
        ...
        pto::TInsertOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (hasTInsertMode) {
            templateArgs =
                rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, getTInsertModeToken(tinsertModeAttr.getValue()))});
        } else if (hasFp || hasPreQuantScalar || hasMode || reluNonDefault) {
            auto dstOT = mlir::dyn_cast<emitc::OpaqueType>(dst.getType());
            auto srcOT = mlir::dyn_cast<emitc::OpaqueType>(src.getType());
            if (!dstOT || !srcOT)
                return rewriter.notifyMatchFailure(op, "tinsert template lowering expects opaque dst/src types");
            SmallVector<Attribute, 5> args{
                emitc::OpaqueAttr::get(ctx, dstOT.getValue().str()),
                emitc::OpaqueAttr::get(ctx, srcOT.getValue().str()),
            };
            if (hasFp) {
                auto fpOT = mlir::dyn_cast<emitc::OpaqueType>(fp.getType());
                if (!fpOT)
                    return rewriter.notifyMatchFailure(op, "tinsert template lowering expects opaque fp type");
        ...
        rewriter.create<emitc::CallOpaqueOp>(loc, TypeRange{}, "TINSERT", ArrayAttr{}, templateArgs, operands);
        rewriter.eraseOp(op);
        return success();
    }
};
...
struct PTOInsertFPToEmitC : public OpConversionPattern<pto::TInsertFPOp> {
        ...
        pto::TInsertFPOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (hasMode || reluNonDefault) {
            ...
            auto fpOT = mlir::dyn_cast<emitc::OpaqueType>(fp.getType());
            if (!dstOT || !srcOT || !fpOT)
                return rewriter.notifyMatchFailure(op, "tinsert_fp template lowering expects opaque dst/src/fp types");
            SmallVector<Attribute, 5> args{
                emitc::OpaqueAttr::get(ctx, dstOT.getValue().str()),
                emitc::OpaqueAttr::get(ctx, srcOT.getValue().str()),
                emitc::OpaqueAttr::get(ctx, fpOT.getValue().str()),
...
struct PTOFillPadToEmitC : public OpConversionPattern<pto::TFillPadOp> {
        ...
        pto::TFillPadOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (auto padValueAttr = op.getPadValueAttr()) {
            ...
            templateArgs = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, padValueTok(padValueAttr.getValue()))});
        }
...
struct PTOMaxToEmitC : public OpConversionPattern<pto::TMaxOp> {
    ...
    LogicalResult matchAndRewrite(pto::TMaxOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TMAX", operands);
...
struct PTOMaxSToEmitC : public OpConversionPattern<pto::TMaxSOp> {
        ...
        pto::TMaxSOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TMAXS", operands);
...
struct PTOMinToEmitC : public OpConversionPattern<pto::TMinOp> {
    ...
    LogicalResult matchAndRewrite(pto::TMinOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        SmallVector<Value, 3> operands{dst, src0, src1};
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TMIN", operands);
...
struct PTOMinsToEmitC : public OpConversionPattern<pto::TMinSOp> {
        ...
        pto::TMinSOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TMINS", operands);
...
struct PTOMovToEmitC : public OpConversionPattern<pto::TMovOp> {
    ...
    LogicalResult matchAndRewrite(pto::TMovOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        ArrayAttr templateArgs =
            templateArgVec.size() == 2 && !hasFp && !hasPreQuantScalar && !hasMode && !reluNonDefault ?
                ArrayAttr{} :
...
struct PTOQuantToEmitC : public OpConversionPattern<pto::TQuantOp> {
        ...
        pto::TQuantOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (op.getOffset()) {
            ...
            if (!offsetLValue) {
                offsetLValue = rewriter
                                   .create<emitc::VariableOp>(
                                       loc, getEmitCVariableResultType(offsetValueTy), emitc::OpaqueAttr::get(ctx, ""))
                                   .getResult();
                rewriter.create<emitc::AssignOp>(loc, offsetLValue, offset);
            }
            offsetPtr = rewriter.create<emitc::ApplyOp>(loc, emitc::PointerType::get(offsetValueTy), "&", offsetLValue)
                            .getResult();
        }
        ...
        if (dstOT && srcOT && fpOT) {
            ...
            if (tmp) {
                auto tmpOT = mlir::dyn_cast<emitc::OpaqueType>(tmp.getType());
                if (!tmpOT)
                    return rewriter.notifyMatchFailure(op, "tquant tmp lowering expects opaque tmp type");
        ...
        rewriter.create<emitc::CallOpaqueOp>(loc, TypeRange{}, "TQUANT", ArrayAttr{}, templateArgs, operands);
...
struct PTOQuantMxToEmitC : public OpConversionPattern<pto::TQuantMxOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TQuantMxOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        auto* ctx = rewriter.getContext();
        ...
        Value expZz = adaptor.getExpZz() ? adaptor.getExpZz() : Value{};
        ...
        auto expZzOT = expZz ? mlir::dyn_cast<emitc::OpaqueType>(expZz.getType()) : emitc::OpaqueType{};
        if (!dstOT || !srcOT || !expOT || !maxOT || !scalingOT)
            return rewriter.notifyMatchFailure(op, "expected all operands to be emitc::OpaqueType");
        if (expZz && !expZzOT)
            return rewriter.notifyMatchFailure(op, "expected exp_zz operand to be emitc::OpaqueType");

        auto makePtr = [&](Value v, emitc::OpaqueType ot) -> Value {
            if (Value variable = getSourceEmitCVariable(v))
                return rewriter.create<emitc::ApplyOp>(loc, emitc::PointerType::get(v.getType()), "&", variable)
                    .getResult();

            Value tmp =
                rewriter.create<emitc::VariableOp>(loc, getEmitCVariableResultType(ot), emitc::OpaqueAttr::get(ctx, ""))
                    .getResult();
            rewriter.create<emitc::AssignOp>(loc, tmp, v);
            return rewriter.create<emitc::ApplyOp>(loc, emitc::PointerType::get(ot), "&", tmp).getResult();
        };
        ...
        std::string quantTypeStr =
            op.getQuantType() == pto::QuantType::MXFP8 ? "pto::QuantType::MXFP8" : "pto::QuantType::MXFP4_E2M1";
        ...
        if (auto storeMode = op.getStoreMode())
            templateArgsStorage.push_back(emitc::OpaqueAttr::get(ctx, vecStoreModeTok(*storeMode)));
        templateArgsStorage.push_back(emitc::OpaqueAttr::get(ctx, dstOT.getValue().str()));
        templateArgsStorage.push_back(emitc::OpaqueAttr::get(ctx, srcOT.getValue().str()));
        templateArgsStorage.push_back(emitc::OpaqueAttr::get(ctx, expOT.getValue().str()));
        templateArgsStorage.push_back(emitc::OpaqueAttr::get(ctx, maxOT.getValue().str()));
        templateArgsStorage.push_back(emitc::OpaqueAttr::get(ctx, scalingOT.getValue().str()));
        if (!op.getStoreMode() && op.getQuantScaleAlg() != pto::QuantScaleAlg::OCP)
            templateArgsStorage.push_back(emitc::OpaqueAttr::get(ctx, quantScaleAlgTok(op.getQuantScaleAlg())));
...
struct PTOMulToEmitC : public OpConversionPattern<pto::TMulOp> {
    ...
    LogicalResult matchAndRewrite(pto::TMulOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        SmallVector<Value, 3> operands{dst, src0, src1};
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TMUL", operands);
...
struct PTOMulsToEmitC : public OpConversionPattern<pto::TMulSOp> {
        ...
        pto::TMulSOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        SmallVector<Value, 3> operands{dst, src, scalar};
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TMULS", operands);
...
struct PTOPowToEmitC : public OpConversionPattern<pto::TPowOp> {
    ...
    LogicalResult matchAndRewrite(pto::TPowOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        auto* ctx = rewriter.getContext();
        ...
        if (op.getPrecisionType() == pto::PowPrecision::HighPrecision) {
            templateArgs = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, "pto::PowAlgorithm::HIGH_PRECISION")});
        }
        rewriter.create<emitc::CallOpaqueOp>(
            loc, TypeRange{}, "TPOW",
            /*args=*/ArrayAttr{}, /*templateArgs=*/templateArgs,
...
struct PTOPowSToEmitC : public OpConversionPattern<pto::TPowSOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TPowSOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        auto* ctx = rewriter.getContext();
...
static void replaceOrEraseWithOpaqueCallAndReturnDst(
    Operation* op, Value dst, StringRef callee, ArrayRef<Value> args, ArrayAttr templateArgs,
...
struct PTOTGemvBiasToTGEMV_BIAS : public OpConversionPattern<pto::TGemvBiasOp> {
        ...
        pto::TGemvBiasOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        replaceOrEraseWithOpaqueCallAndReturnDst(
            op.getOperation(), dst, "TGEMV_BIAS", {dst, a, b, bias}, templateArgs, rewriter);
        return success();
    }
};
...
struct PTOTGemvMXToTGEMV_MX : public OpConversionPattern<pto::TGemvMxOp> {
        ...
        pto::TGemvMxOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        ArrayAttr templateArgs = buildAccPhaseTemplateArgs(rewriter, op.getAccPhase());
        replaceOrEraseWithOpaqueCallAndReturnDst(
            op.getOperation(), dst, "TGEMV_MX", {dst, a, aScale, b, bScale}, templateArgs, rewriter);
        return success();
    }
};
...
struct PTOTGemvMXAccToTGEMV_MX : public OpConversionPattern<pto::TGemvMxAccOp> {
        ...
        pto::TGemvMxAccOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        ArrayAttr templateArgs = buildAccPhaseTemplateArgs(rewriter, op.getAccPhase());
        replaceOrEraseWithOpaqueCallAndReturnDst(
            op.getOperation(), dst, "TGEMV_MX", {dst, cIn, a, aScale, b, bScale}, templateArgs, rewriter);
        return success();
    }
};
...
struct PTOTGemvMXBiasToTGEMV_MX : public OpConversionPattern<pto::TGemvMxBiasOp> {
        ...
        pto::TGemvMxBiasOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        replaceOrEraseWithOpaqueCallAndReturnDst(
            op.getOperation(), dst, "TGEMV_MX", {dst, a, aScale, b, bScale, bias}, ArrayAttr{}, rewriter);
        return success();
    }
};
...
struct PTOTMatmulBiasToTMATMUL_BIAS : public OpConversionPattern<pto::TMatmulBiasOp> {
        ...
        pto::TMatmulBiasOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        ArrayAttr templateArgs = buildAccPhaseTemplateArgs(rewriter, op.getAccPhase());
        replaceOrEraseWithOpaqueCallAndReturnDst(
            op.getOperation(), dst, "TMATMUL_BIAS", {dst, a, b, bias}, templateArgs, rewriter);
        return success();
    }
};
...
struct PTOTMatmulMXToTMATMUL_MX : public OpConversionPattern<pto::TMatmulMxOp> {
        ...
        pto::TMatmulMxOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        ArrayAttr templateArgs = buildAccPhaseTemplateArgs(rewriter, op.getAccPhase());
        replaceOrEraseWithOpaqueCallAndReturnDst(
            op.getOperation(), dst, "TMATMUL_MX", {dst, a, aScale, b, bScale}, templateArgs, rewriter);
        return success();
    }
};
...
struct PTOTMatmulMXAccToTMATMUL_MX_ACC : public OpConversionPattern<pto::TMatmulMxAccOp> {
        ...
        pto::TMatmulMxAccOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        ArrayAttr templateArgs = buildAccPhaseTemplateArgs(rewriter, op.getAccPhase());
        replaceOrEraseWithOpaqueCallAndReturnDst(
            op.getOperation(), dst, "TMATMUL_MX", {dst, cIn, a, aScale, b, bScale}, templateArgs, rewriter);
        return success();
    }
};
...
struct PTOTMatmulMXBiasToTMATMUL_MX_BIAS : public OpConversionPattern<pto::TMatmulMxBiasOp> {
        ...
        pto::TMatmulMxBiasOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        replaceOrEraseWithOpaqueCallAndReturnDst(
            op.getOperation(), dst, "TMATMUL_MX", {dst, a, aScale, b, bScale, bias}, ArrayAttr{}, rewriter);
        return success();
    }
};
...
struct PTORowExpandDivToEmitC : public OpConversionPattern<pto::TRowExpandDivOp> {
        ...
        pto::TRowExpandDivOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(
            rewriter, op.getOperation(), TypeRange{}, "TROWEXPANDDIV", operands, ArrayAttr{}, templateArgs);
...
struct PTORowExpandMulToEmitC : public OpConversionPattern<pto::TRowExpandMulOp> {
        ...
        pto::TRowExpandMulOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (tmp)
            ...
            operands.assign({dst, src0, src1});
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TROWEXPANDMUL", operands);
...
struct PTORowMaxToEmitC : public OpConversionPattern<pto::TRowMaxOp> {
        ...
        pto::TRowMaxOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TROWMAX", operands);
...
struct PTORowMinToEmitC : public OpConversionPattern<pto::TRowMinOp> {
        ...
        pto::TRowMinOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        SmallVector<Value, 3> operands{dst, src, tmp};
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TROWMIN", operands);
...
struct PTORowSumToEmitC : public OpConversionPattern<pto::TRowSumOp> {
        ...
        pto::TRowSumOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        SmallVector<Value, 3> operands{dst, src, tmp};
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TROWSUM", operands);
        ...
        pto::TScatterOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (auto mp = op.getMaskPatternAttr()) {
            ...
            if (auto axisAttr = op.getAxisAttr()) {
                ...
                std::string scatterAxis =
                    (axisVal == "col") ? "pto::ScatterAxis::SCATTER_COL" : "pto::ScatterAxis::SCATTER_ROW";
...
struct PTOSubSToEmitC : public OpConversionPattern<pto::TSubOp> {
    ...
    LogicalResult matchAndRewrite(pto::TSubOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        SmallVector<Value, 4> operands{dst, src0, src1};
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TSUB", operands);
...
struct PTOSubSSToEmitC : public OpConversionPattern<pto::TSubSOp> {
        ...
        pto::TSubSOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        SmallVector<Value, 4> operands{dst, src, scalar};
        createLastUseAwareOpaqueCall(rewriter, op.getOperation(), TypeRange{}, "TSUBS", operands);
...
struct PTOPrintToTPRINT : public OpConversionPattern<pto::TPrintOp> {
        ...
        pto::TPrintOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value src = adaptor.getSrc();
        if (isa<MemRefType>(op.getSrc().getType()) || isa<mlir::pto::PartitionTensorViewType>(op.getSrc().getType())) {
            src = maybeWrapGlobalMemrefAsGlobalTensor(rewriter, loc, src, op.getSrc().getType(), op.getOperation());
        }

        SmallVector<Value, 4> operands{src};
        if (Value tmp = op->getNumOperands() > 1 ? op->getOperand(1) : Value()) {
            Value tmpValue = adaptor.getOperands().size() > 1 ? adaptor.getOperands()[1] : Value();
            tmpValue = peelUnrealized(tmpValue);
            if (isa<MemRefType>(tmp.getType()) || isa<mlir::pto::PartitionTensorViewType>(tmp.getType())) {
                tmpValue =
                    maybeWrapGlobalMemrefAsGlobalTensor(rewriter, loc, tmpValue, tmp.getType(), op.getOperation());
            }
            operands.push_back(tmpValue);
        }

        SmallVector<Attribute, 1> templateArgVec;
        if (auto formatAttr = dyn_cast_or_null<pto::PrintFormatAttr>(op.getProperties().printFormat)) {
            templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, printFormatTok(formatAttr.getValue())));
        }
        ArrayAttr templateArgs = templateArgVec.empty() ? ArrayAttr{} : rewriter.getArrayAttr(templateArgVec);
        rewriter.create<emitc::CallOpaqueOp>(
...
struct PTOAllocTileToEmitC : public OpConversionPattern<pto::AllocTileOp> {
        ...
        pto::AllocTileOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        bool hasDynamicValidDim = llvm::any_of(validShape, [](int64_t dim) { return dim < 0; });
        ...
        if (useConstructor) {
            ...
            if (validShape.size() > 0 && validShape[0] < 0) {
                ...
                if (!validRow)
                    return rewriter.notifyMatchFailure(op, "dynamic alloc_tile valid row must have an operand");
                if (validRow)
            ...
            if (validShape.size() > 1 && validShape[1] < 0) {
                ...
                if (!validCol)
                    return rewriter.notifyMatchFailure(op, "dynamic alloc_tile valid col must have an operand");
                if (validCol)
        ...
        if (useConstructor) {
            ...
            tile = rewriter
                       .create<emitc::VariableOp>(
                           loc, getEmitCVariableResultType(convertedTy), emitc::OpaqueAttr::get(ctx, ""))
                       .getResult();
            tile = loadEmitCVariableIfNeeded(rewriter, loc, tile);
        }
        ...
        if (addr) {
            ...
            if (isa<emitc::PointerType>(addr.getType()) ||
                ...
                addr = rewriter
                           .create<emitc::CallOpaqueOp>(
                               loc, u64Ty, "reinterpret_cast", ArrayAttr{}, rcU64, ValueRange{addr})
                           .getResult(0);
            } else if (addr.getType() != u64Ty) {
                addr = rewriter.create<emitc::CastOp>(loc, u64Ty, addr).getResult();
            }
    ...
    bool initializeDynamicValidToShape = false)
{
    ...
    if (initializeDynamicValidToShape && tileTy.hasDynamicValid()) {
        ...
        constructorArgs.push_back(makeEmitCIntConstant(
            rewriter, loc, i32Ty, renderTileTemplateDim(shape[0], tileTy.getElementType(), blayout, 0)));
        constructorArgs.push_back(makeEmitCIntConstant(
            rewriter, loc, i32Ty, renderTileTemplateDim(shape[1], tileTy.getElementType(), blayout, 1)));
        return rewriter
            .create<emitc::CallOpaqueOp>(loc, convertedTy, *tileTypeString, ArrayAttr{}, ArrayAttr{}, constructorArgs)
            .getResult(0);
    }
    ...
    Value tile =
        rewriter
            .create<emitc::VariableOp>(
                loc, getEmitCVariableResultType(convertedTy), emitc::OpaqueAttr::get(rewriter.getContext(), ""))
            .getResult();
    return loadEmitCVariableIfNeeded(rewriter, loc, tile);
}

struct PTODeclareTileToEmitC : public OpConversionPattern<pto::DeclareTileOp> {
    using OpConversionPattern::OpConversionPattern;

    LogicalResult matchAndRewrite(
        pto::DeclareTileOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        ...
        if (failed(tile))
            return rewriter.notifyMatchFailure(op, "only rank-2 declare_tile handles can be converted to EmitC");
        rewriter.replaceOp(op, *tile);
        return success();
    }
};
...
struct PTOBitcastToEmitC : public OpConversionPattern<pto::BitcastOp> {
        ...
        pto::BitcastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value addr = rawPtr;
        if (isSetFFTsPointerLikeType(rawPtr.getType())) {
            auto rcU64 = rewriter.getArrayAttr({emitc::OpaqueAttr::get(rewriter.getContext(), "uint64_t")});
            addr = rewriter
                       .create<emitc::CallOpaqueOp>(
                           op.getLoc(), u64Ty, "reinterpret_cast", ArrayAttr{}, rcU64, ValueRange{rawPtr})
                       .getResult(0);
        } else if (addr.getType() != u64Ty) {
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "TASSIGN", ArrayAttr{}, ArrayAttr{}, ValueRange{*dst, addr});
        rewriter.replaceOp(op, *dst);
        return success();
    }
};
...
struct PTOTileBufAddrToEmitC : public OpConversionPattern<pto::TileBufAddrOp> {
        ...
        pto::TileBufAddrOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (isEmitCTileLikeType(src.getType())) {
            rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(
                op, TypeRange{dstTy}, "PTOAS__TILE_DATA", ArrayAttr{}, ArrayAttr{}, ValueRange{src});
            return success();
        }
...
struct SectionToEmitC : public OpConversionPattern<SectionOpTy> {
    ...
    LogicalResult matchAndRewrite(
        SectionOpTy op, typename SectionOpTy::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Location loc = op.getLoc();
        bool needsNoSplitGuard = needsA5NoSplitVectorGuard(op.getOperation());
...
struct SCFIndexSwitchToCF : public OpRewritePattern<scf::IndexSwitchOp> {
    ...
    static void addContinuationArguments(
        PatternRewriter& rewriter, scf::IndexSwitchOp op, Location loc, Block* continueBlock)
    {
        SmallVector<BlockArgument> contArgs;
        contArgs.reserve(op.getNumResults());
...
struct SCFWhileToCF : public OpRewritePattern<scf::WhileOp> {
    ...
    static void addWhileExitArguments(PatternRewriter& rewriter, scf::WhileOp op, Location loc, Block* afterWhileBlock)
    {
    ...
    RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
{
    ...
    patterns.add<PTOQuantToEmitC, PTOQuantMxToEmitC>(typeConverter, ctx);
    ...
    patterns.add<
        PTOMakeTensorViewToEmitC, PTOPartitionViewToEmitC,
        PTOGetTensorViewMetadataToEmitC<pto::GetTensorViewDimOp, false>,
        PTOGetTensorViewMetadataToEmitC<pto::GetTensorViewStrideOp, true>>(typeConverter, ctx);
    patterns.add<PTOPartitionViewStaticToEmitC>(typeConverter, ctx, PatternBenefit(2));
    ...
    patterns.add<
        ...
        PTOTMatmulMXBiasToTMATMUL_MX_BIAS, PTOTMatmulBiasToTMATMUL_BIAS, PTOTMatmulMXToTMATMUL_MX,
        PTOTMatmulMXAccToTMATMUL_MX_ACC, PTOTMatmulMXBiasToTMATMUL_MX_BIAS, PTOTGemvBiasToTGEMV_BIAS,
        PTOTGemvMXToTGEMV_MX, PTOTGemvMXAccToTGEMV_MX, PTOTGemvMXBiasToTGEMV_MX, PTOBarrierToEmitC,
        PTOFenceToEmitC<pto::FenceBarrierAllOp>, PTOCmoCacheInvalidToEmitC>(typeConverter, ctx);
    ...
    void runOnOperation() override
    {
        ...
        // content-derived !emitc.opaque tokens.
        {
            ...
            for (pto::StructType st : structDefs)
                builder.create<emitc::VerbatimOp>(loc, builder.getStringAttr(renderStructDef(st)));
        }
        ...
        // 1.5 Pre-lower SCF constructs not handled by SCFToEmitC.
        {
            ...
            scfLoweringPatterns
                .add<SCFExecuteRegionInline, SCFExecuteRegionToCF, SCFIndexSwitchToCF, SCFWhileToCF, CFSwitchToCondBr>(
                    ctx);
        ...
        mop.walk([&](UnrealizedConversionCastOp cast) {
            ...
            if (Type convertedOutTy = typeConverter.convertType(outTy); convertedOutTy && convertedOutTy == inTy) {
                output.replaceAllUsesWith(input);
                castsToErase.push_back(cast);
                return;
            }
            ...
            if (isEmitCTileLikeType(inTy) && isEmitCPointerLikeType(outTy)) {
                ...
                auto extracted = builder.create<emitc::CallOpaqueOp>(
                    cast.getLoc(), outTy, "PTOAS__TILE_DATA", ArrayAttr{}, ArrayAttr{}, ValueRange{input});
            ...
            cast.emitError() << "cannot lower unrealized_conversion_cast(" << inTy << " -> " << outTy
                             << ") to emitc.cast";
            castCleanupFailed = true;
        });
            ...
            for (emitc::CallOpaqueOp callOp : tileDataReadsToSink) {
                ...
                for (OpOperand& use : llvm::make_early_inc_range(oldRes.getUses())) {
                    ...
                    auto newRead = b.create<emitc::CallOpaqueOp>(
                        callOp.getLoc(), dstTy, "PTOAS__TILE_DATA", ArrayAttr{}, ArrayAttr{}, ValueRange{src});
