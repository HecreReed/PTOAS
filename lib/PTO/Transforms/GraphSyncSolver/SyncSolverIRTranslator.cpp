...
bool isTransparentGraphSyncRegionOp(Operation& op) { return isa<pto::SectionCubeOp, pto::SectionVectorOp>(op); }
} // namespace
...
llvm::SmallVector<Value> IRTranslator::tracebackMemValsStep(Value val)
{
    ...
    } else if (auto castPtr = dyn_cast<pto::CastPtrOp>(defOp); castPtr &&
...
llvm::SmallVector<Value> IRTranslator::tracebackMemVals(Value val)
{
    ...
    while (!queue.empty()) {
        ...
        if (isa<pto::AllocTileOp, pto::AllocMultiTileOp, tensor::EmptyOp, pto::MultiTileGetOp>(defOp)) {
...
std::unique_ptr<OperationBase> IRTranslator::getScalarMemoryOp(Operation* op, OperationBase* parentOp)
{
    auto [reads, writes] = getReadWriteMemoryOps(op);
    if (reads.empty() && writes.empty())
        return nullptr;
    return std::make_unique<RWOperation>(
        op, parentOp, TCoreType::CUBE_OR_VECTOR, pto::PIPE::PIPE_S, pto::PIPE::PIPE_S, reads, writes);
}
...
std::unique_ptr<OperationBase> IRTranslator::getCallOp(func::CallOp callOp, OperationBase* parentOp) { return nullptr; }
...
std::optional<int64_t> IRTranslator::getLoopMultibufferUnrollNum(Loop* loopOp) { return {}; }

std::optional<int64_t> IRTranslator::getScopePreloadNum(Scope* scopeOp) { return {}; }

std::optional<int64_t> IRTranslator::getScopeMaxPreloadNum(Scope* scopeOp) { return {}; }
...
std::unique_ptr<Scope> IRTranslator::funcIrBuilder(Region& region, OperationBase* parentOp, bool skipEmptyScopes)
{
    ...
    translateRegionIntoScope(region, scopeOp.get(), skipEmptyScopes, isFunctionRegion);
    return scopeOp;
}

void IRTranslator::translateRegionIntoScope(
    Region& region, Scope* scopeOp, bool skipEmptyScopes, bool createFunctionBlocks)
{
...
void IRTranslator::translateBlockIntoScope(Block& block, Scope* parScope, bool skipEmptyScopes)
{
