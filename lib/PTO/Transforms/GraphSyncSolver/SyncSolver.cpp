...
namespace {
mlir::pto::SlotRelation compareMemInfoSlotSSA(const MemInfo& memInfo1, const MemInfo& memInfo2)
{
...
bool isForwardDepDroppableBySlotAffine(const MemInfo& memInfo1, const MemInfo& memInfo2)
{
    return compareMemInfoSlotSSA(memInfo1, memInfo2) == mlir::pto::SlotRelation::kDisjoint;
}
} // namespace
...
Occurrence* Solver::getFirstIterOcc(Occurrence* occ, Occurrence* parOcc)
{
    ...
    if (parOcc->depth + 1 < occ->depth) {
        auto* newParOcc = getFirstIterOcc(occ->getNthParent(occ->depth - parOcc->depth - 1), parOcc);
        return getFirstIterOcc(occ, newParOcc);
    }
    auto* it = std::find_if(parOcc->childOccs.begin(), parOcc->childOccs.end(), [occ](Occurrence* curOcc) {
...
Occurrence* Solver::getLastIterOcc(Occurrence* occ, Occurrence* parOcc)
{
    assert(occ != nullptr && parOcc != nullptr);
    if (parOcc->depth + 1 < occ->depth) {
        auto* newParOcc = getLastIterOcc(occ->getNthParent(occ->depth - parOcc->depth - 1), parOcc);
        return getLastIterOcc(occ, newParOcc);
    }
    auto it = std::find_if(parOcc->childOccs.rbegin(), parOcc->childOccs.rend(), [occ](Occurrence* curOcc) {
        return occ->op == curOcc->op;
    });
...
bool Solver::checkAlreadySyncedWithUnitFlag(Occurrence* occ1, Occurrence* occ2)
{
    assert(occ1 != nullptr && occ2 != nullptr);
    ...
    RWOperation* rwOp1, RWOperation* rwOp2)
{
    ...
    if (options.isCrossCoreMode()) {
        ...
        assert(coreSrc == pto::TCoreType::VECTOR || coreSrc == pto::TCoreType::CUBE);
    ...
    if (checkMemInfoConflict(rwOp1, rwOp2, rwOp1->readMemInfo, rwOp2->writeMemInfo)) {
        collectedConflictsSet.insert({CorePipeInfo(coreSrc, rwOp1->pipeRead), CorePipeInfo(coreDst, rwOp2->pipeWrite)});
    }
    if (checkMemInfoConflict(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->readMemInfo)) {
        collectedConflictsSet.insert({CorePipeInfo(coreSrc, rwOp1->pipeWrite), CorePipeInfo(coreDst, rwOp2->pipeRead)});
    }
    if (checkMemInfoConflict(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->writeMemInfo)) {
        collectedConflictsSet.insert(
            {CorePipeInfo(coreSrc, rwOp1->pipeWrite), CorePipeInfo(coreDst, rwOp2->pipeWrite)});
    }
...
llvm::SmallVector<std::tuple<CorePipeInfo, CorePipeInfo>> Solver::checkMemoryConflictsForOcc(
    Occurrence* occ1, Occurrence* occ2, RWOperation* rwOp1, RWOperation* rwOp2)
{
    ...
    auto hasForwardConflict = [&](const llvm::SmallVector<MemInfo>& memInfoList1,
    ...
    Occurrence* occ1, Occurrence* occ2, RWOperation* rwOp1, RWOperation* rwOp2)
{
    ...
    if (options.isTestMode()) {
        ...
        if (!parLoop1->isProperAncestor(setOcc) || !parLoop1->isProperAncestor(waitOcc)) {
            return {};
        }
    } else {
        ...
        auto [setOcc, waitOcc] = getSetWaitOcc(occ1, occ2);
        if (!setOcc->getParentWithOp(
                multibufferLoop,
                /*assertExists=*/false) ||
            !waitOcc->getParentWithOp(
                multibufferLoop,
                /*assertExists=*/false)) {
            return {};
        }
...
std::optional<EventIdInfo> Solver::checkCVMultiBufferUnrollEventIdInfo(RWOperation* rwOp1, RWOperation* rwOp2)
{
    ...
    assert(parentLoop1->multibufferUnrollNum.value() == parentLoop2->multibufferUnrollNum.value());
...
std::optional<EventIdInfo> Solver::checkCVMultiBufferPreloadEventIdInfo(RWOperation* rwOp1, RWOperation* rwOp2)
{
    ...
    assert(parentScope1->maxPreloadNum.value() == parentScope2->maxPreloadNum.value());
    ...
    const llvm::SmallVector<ConflictPair*>& ignoreConflictPairs)
{
    ...
    auto handleConflictPair = [&](ConflictPair* conflictPair) {
        ...
        if (conflictPair->endIndex < startIndex.value() || conflictPair->startIndex > endIndex.value()) {
            return;
        }
        if (conflictPair->isInnerBackward) {
            if ((eventIdInfo.eventIdNum * eventIdInfo.eventIdRepeatNum) <
                (conflictPair->eventIdInfo.eventIdNum * conflictPair->eventIdInfo.eventIdRepeatNum)) {
                return;
            }
        }
    ...
    if (options.enableUnitFlagFeature) {
        mnDistance = graphSolver.runDijkstraUnitFlagEnabled(
            occ1, occ2, corePipeSrc, corePipeDst, startIndex.value(), endIndex.value());
    } else {
        mnDistance = graphSolver.runDijkstra(corePipeSrc, corePipeDst, startIndex.value(), endIndex.value());
    }
...
bool Solver::checkSyncOpsConflicts(ConflictPair* conflictPair1, ConflictPair* conflictPair2)
{
    ...
    if (conflictPair1->setCorePipeInfo != conflictPair2->setCorePipeInfo) {
        ...
        assert(occ1 != nullptr && occ2 != nullptr);
        result = result || checkGraphConflict(
                               occ1, occ2, corePipeSrc, corePipeDst, conflictPair1->eventIdInfo, startIndex, endIndex,
...
std::optional<UnitFlagInfo> Solver::checkUnitFlagPatterns(Occurrence* occ1, Occurrence* occ2) { return {}; }
...
std::pair<Occurrence*, Occurrence*> Solver::getSetWaitLCAPairOcc(Occurrence* occ1, Occurrence* occ2)
{
    assert(occ1 != nullptr && occ2 != nullptr);

    auto [grandParOcc1, grandParOcc2] = Occurrence::getLCAPair(occ1, occ2);
    assert(grandParOcc1 != nullptr && grandParOcc2 != nullptr);
    assert(grandParOcc1->parentOcc != nullptr && grandParOcc2->parentOcc != nullptr);
...
std::pair<Occurrence*, Occurrence*> Solver::getFixedSetWaitOcc(Occurrence* occ1, Occurrence* occ2)
{
    ...
    if (setOcc->op != waitOcc->op) {
        if (auto* parLoopOp = llvm::dyn_cast_if_present<Loop>(setOcc->parentOcc->op)) {
    ...
    if (auto* unlikelyParCondOcc1 = Occurrence::getUnlikelyParentCondition(occ1)) {
        if (!unlikelyParCondOcc1->isProperAncestor(occ2)) {
            ...
            if (parentLoopOcc == nullptr || parentLoopOcc->isProperAncestor(occ2)) {
                auto* placeHolderOcc =
                    getScopeEndPlaceHolderOcc(occ1->getNthParent(occ1->depth - unlikelyParCondOcc1->depth - 1));
    ...
    if (auto* unlikelyParCondOcc2 = Occurrence::getUnlikelyParentCondition(occ2)) {
        if (!unlikelyParCondOcc2->isProperAncestor(occ1)) {
            ...
            if (parentLoopOcc == nullptr || parentLoopOcc->isProperAncestor(occ1)) {
                auto* placeHolderOcc =
                    getScopeBeginPlaceHolderOcc(occ2->getNthParent(occ2->depth - unlikelyParCondOcc2->depth - 1));
                return std::make_pair(placeHolderOcc, occ2);
            }
        }
    }
...
Occurrence* Solver::getBarrierWaitOcc(Occurrence* occ1, Occurrence* occ2)
{
    ...
    while (allParents.size() >= 2 && llvm::isa_and_present<Condition>(allParents.back()->op)) {
        allParents.pop_back();
...
void Solver::insertBarrierAllBeforeOp(OperationBase* op, bool isUseless, bool isPersistent)
{
    assert(op != nullptr);
...
bool Solver::reuseCmp(ConflictPair* conflictPair1, ConflictPair* conflictPair2)
{
    assert(conflictPair1 != nullptr && conflictPair2 != nullptr);
...
bool Solver::reuseConflictPair(ConflictPair* conflictPair, Occurrence* scopeOcc1, Occurrence* scopeOcc2)
{
    ...
    for (auto* opt : {opt1, opt2, opt3, opt4, opt5}) {
        if (opt != nullptr) {
            if (reusableConflictPair == nullptr || reuseCmp(reusableConflictPair, opt)) {
    ...
    bool isUseless)
{
    ...
    if (auto setWaitOccs = checkAndApplyMmadl0LoopOpt(conflictPair.get(), occ1, occ2, setOcc, waitOcc)) {
    ...
    auto checkColorable = [&]() -> bool {
        ...
        insertBarrierAllBeforeOp(
            occ2->op, conflictPair->isUseless,
            /*isPersistent=*/false);
    ...
    if (auto* oldEventIdNode = getOldEventIdNodeIfExists(conflictPair.get())) {
        ...
        conflictPair->eventIdNode =
            curEventIdSolver->createNode(conflictPair.get(), conflictPair->eventIdInfo.eventIdNum, reversedPriority);
    }
    ...
    auto intersectingConflictPairs = getIntersectingConflictPairs(conflictPair.get());
    ...
    llvm::SmallVector<std::pair<std::unique_ptr<ConflictPair>, Occurrence*>> extraConflictPairs;

    auto insertExtraConflictPair = [&](Occurrence* setOcc, Occurrence* waitOcc, Occurrence* parentScope,
    ...
    if (conflictPair->isInnerBackward && conflictPair->eventIdNode != nullptr) {
        ...
        if (insertOuterBwdConflictPair) {
            ...
            if (!insertExtraConflictPair(
                    parentLCALoopBeforePHOcc, parentLCALoopAfterPHOcc, parentLCALoopOcc->parentOcc)) {
                return;
            }
        }
    }
    ...
    if (conflictPair->isInnerBackward && conflictPair->eventIdNode != nullptr) {
        ...
        if (!insertExtraConflictPair(parentLCALoopBeforePHOcc, loopOpOcc1, parentLCALoopOcc, /*couldNotRun=*/true)) {
            return;
        }
        if (!insertExtraConflictPair(loopOpOcc2, parentLCALoopAfterPHOcc, parentLCALoopOcc, /*couldNotRun=*/true)) {
            return;
        }
    }
    ...
    Scope* scopeOp, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, int64_t eventId, bool shouldBeUsedAtleastOnce)
{
    ...
    if (shouldBeUsedAtleastOnce) {
        ...
        bool usedAtleastOnce = it != index.end() && it->first < setWaitEndIndexInclusive[scopeOp];
        if (!usedAtleastOnce) {
            return false;
        }
    }
    ...
    if (auto* loopOp = llvm::dyn_cast<Loop>(scopeOp)) {
        for (auto& childOp : loopOp->body) {
            if (auto* childScopeOp = llvm::dyn_cast<Scope>(childOp.get())) {
                if (!checkMergeable(childScopeOp, corePipeSrc, corePipeDst, eventId, false)) {
                    return false;
                }
            }
        }
        for (auto& childOp : loopOp->body) {
            if (auto* childScopeOp = llvm::dyn_cast<Scope>(childOp.get())) {
                if (checkMergeable(childScopeOp, corePipeSrc, corePipeDst, eventId, true)) {
                    return true;
                }
            }
        }
    ...
    for (auto& childOp : scopeOp->body) {
        auto it1 = index.lower_bound({setWaitStartIndexInclusive[childOp.get()], nullptr});
        auto it2 = index.lower_bound({setWaitEndIndex[childOp.get()], nullptr});
        bool usedAtleastOnce = it1 != index.end() && it1->first < setWaitEndIndexInclusive[childOp.get()];
        if (!usedAtleastOnce) {
            continue;
        }
        bool before = it1 != index.end() && it1->first < setWaitStartIndex[childOp.get()];
        bool after = it2 != index.end() && it2->first < setWaitEndIndexInclusive[childOp.get()];
        if (before || after) {
            return false;
        }
        if (!checkBackwardSyncEventsContains(childOp.get(), corePipeSrc, corePipeDst, eventId)) {
            return false;
        }
        if (checkBackwardSyncEventsContainsAfterMerge(childOp.get(), corePipeSrc, corePipeDst)) {
            return false;
        }
    }
    return true;
...
{
    for (auto [corePipeSrc, corePipeDst] : checkMemoryConflictsForOcc(occ1, occ2, rwOp1, rwOp2)) {
