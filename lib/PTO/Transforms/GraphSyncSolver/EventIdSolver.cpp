...
llvm::SmallVector<int64_t> EventIdSolver::getChosenEventIds(EventIdNode* node, int64_t eventIdMax)
{
    ...
    if (!node->reversePriority) {
        ...
        while ((curEventId >= 0) && (static_cast<int64_t>(chosenEventIds.size()) < node->eventIdNum)) {
        ...
        if (int64_t rem = node->eventIdNum - static_cast<int64_t>(chosenEventIds.size()); rem > 0) {
...
void EventIdSolver::undoActions()
{
    while (!actionsStack.empty() && (actionsStack.top()->actionType != ACTION_TYPE::NONE)) {
        ...
        llvm::TypeSwitch<Action*, void>(action.get())
            ...
            .Default([](Action* action) { llvm_unreachable("EventIdSolver: unhandled action_type."); });
    }
