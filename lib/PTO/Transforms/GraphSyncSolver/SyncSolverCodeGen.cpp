...
void CodeGenerator::emitSyncOp(IRRewriter& rewriter, SyncOp* syncOp)
{
    ...
    if (!setWait->allAtOnce && setWait->eventIds.size() > 1 && setWait->slotSSAExpr) {
        ...
        Value selected = rewriter.create<arith::ConstantIndexOp>(loc, setWait->eventIds[0]);
        for (int64_t i = 1; i < n; ++i) {
            ...
            Value idI = rewriter.create<arith::ConstantIndexOp>(loc, setWait->eventIds[static_cast<size_t>(i)]);
