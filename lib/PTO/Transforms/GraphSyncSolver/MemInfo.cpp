...
static std::optional<int64_t> getTileBufferBitSize(pto::TileBufType type)
{
    ...
    if (type.getCompactModeI32() == static_cast<int32_t>(pto::CompactMode::RowPlusOne)) {
        ...
        if (major == 0 || minor == 0)
            return 0;
        return ((major - 1) * (minor + 1) + minor) * static_cast<int64_t>(bitWidth);
    }
...
static std::optional<pto::AddressSpace> getValueAddressSpace(Value value)
{
    ...
    if (auto memRefType = dyn_cast<BaseMemRefType>(value.getType()); memRefType && !memRefType.getMemorySpace()) {
...
static PointerLikeInfo getPointerLikeInfo(pto::AllocMultiTileOp alloc)
{
    ...
    if (auto space = dyn_cast_or_null<pto::AddressSpaceAttr>(slotType.getMemorySpace()))
...
static MemInfo getMemInfoForMultiTileGet(pto::MultiTileGetOp get)
{
    ...
    if (matchPattern(get.getSlot(), m_Constant(&slotAttr)) && info.addresses.size() > 1) {
...
{
    if (!pointerLikeInfo1.addressSpace.has_value() || !pointerLikeInfo2.addressSpace.has_value()) {
        return false;
    }
    if (pointerLikeInfo1.addressSpace.value() != pointerLikeInfo2.addressSpace.value()) {
        return false;
    }

    if (pointerLikeInfo1.aliasesUnknownRange || pointerLikeInfo2.aliasesUnknownRange) {
        return true;
    }
    ...
    for (int64_t i = 0; i < len1; i++) {
        for (int64_t j = 0; j < len2; j++) {
            ...
            if ((allocSz1 != ShapedType::kDynamic) && (offset1 + allocSz1 < offset2 + 1)) {
                continue;
            }
            if ((allocSz2 != ShapedType::kDynamic) && (offset2 + allocSz2 < offset1 + 1)) {
                continue;
            }
            return true;
        }
    ...
    const MemInfo& memInfo1, const MemInfo& memInfo2, std::optional<int64_t> lcmLen, std::optional<int64_t> eventIdNum)
{
    ...
    auto getUnknownAddressSpace = [](const MemInfo& memInfo) -> std::optional<pto::AddressSpace> {
        if (!memInfo.pointerLikeInfo || !memInfo.pointerLikeInfo->aliasesUnknownRange) {
            return std::nullopt;
        }
        return memInfo.pointerLikeInfo->addressSpace;
    };
    auto aliasesAddressSpace = [](const MemInfo& memInfo, pto::AddressSpace space) -> bool {
