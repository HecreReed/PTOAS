...
struct PersistentSectionWorklist {
    explicit PersistentSectionWorklist(pto::SectionSimtOp section) : section(section) {}

    pto::SectionSimtOp section;
    SmallVector<PersistentElementWorkItem> elements;
    llvm::DenseMap<Operation*, SmallVector<PersistentLaneRewrite>> laneRewritesByAccess;
};
...
static LogicalResult rewireScalarAccess(Operation* access, LLVM::AllocaOp proxy)
{
    ...
    return access->emitOpError("expected a scalar llvm.load or llvm.store during persistent "
...
static FailureOr<Type> getPersistentAccessType(Operation* access)
{
    ...
    access->emitOpError("expected an llvm.load or llvm.store persistent fragment access");
    return failure();
}
    ...
    llvm::DenseMap<Operation*, SmallVector<PersistentLaneRewrite>>& laneRewritesByAccess)
{
    for (auto& [access, laneRewrites] : laneRewritesByAccess) {
        ...
        llvm::sort(laneRewrites, [](const PersistentLaneRewrite& lhs, const PersistentLaneRewrite& rhs) {
        ...
        if (laneRewrites.size() != expectedLaneCount) {
            return access->emitOpError() << "persistent fragment access expected " << expectedLaneCount
                                         << " scalar lane rewrite(s), got " << laneRewrites.size();
        }
        for (auto [expectedLaneIndex, laneRewrite] : llvm::enumerate(laneRewrites)) {
            if (laneRewrite.laneIndex != expectedLaneIndex) {
                return access->emitOpError() << "persistent fragment access has non-contiguous lane "
                                                "mapping at lane "
                                             << expectedLaneIndex;
            }
        }
...
static bool isFragmentActiveInSection(const PersistentFragmentAnalysis& fragment, pto::SectionSimtOp section)
{
    if (fragment.initSection == section)
        return true;
    return llvm::any_of(fragment.carrySections, [&](pto::SectionSimtOp carry) { return carry == section; });
}
...
// immutable result.
static LogicalResult validateSectionWorklist(
    const PersistentMaterializationPlan& plan, PersistentSectionWorklist& sectionWorklist,
    llvm::DenseMap<Operation*, llvm::DenseSet<unsigned>>& assignedAccessLanes)
{
    ...
    if (!section || !section.getBody().hasOneBlock()) {
        return section ? section.emitOpError("persistent fragment transform requires a single-"
                                             "block SIMT section") :
                         failure();
    }
    ...
    for (const PersistentFragmentAnalysis& fragment : plan.fragments) {
        ...
        for (const ResidentElementPlan& expectedElement : fragment.residentElements) {
            if (expectedElementIndex >= sectionWorklist.elements.size()) {
                LLVM::AllocaOp allocaOp = fragment.allocaOp;
                return allocaOp.emitOpError("persistent fragment section worklist is missing the complete "
                                            "resident element set");
            }

            const PersistentElementWorkItem& element = sectionWorklist.elements[expectedElementIndex];
            if (element.fragment != &fragment || element.residentElement != &expectedElement) {
                return section.emitOpError("persistent fragment section worklist does not preserve fragment "
            ...
            for (const AccessLane& accessLane : element.accesses) {
                if (!accessLane.op) {
                    LLVM::AllocaOp allocaOp = fragment.allocaOp;
                    return allocaOp.emitOpError("persistent fragment worklist contains a null access "
                                                "operation");
                }

                bool isOwnedAccess = llvm::any_of(residentElement.accesses, [&](const AccessLane& candidate) {
                    return candidate.op == accessLane.op && candidate.laneIndex == accessLane.laneIndex;
                });
                if (!isOwnedAccess) {
                    return accessLane.op->emitOpError("persistent fragment worklist access lane is owned by a "
                                                      "different resident element");
                }

                pto::SectionSimtOp accessSection = accessLane.op->getParentOfType<pto::SectionSimtOp>();
                if (accessSection != section || accessLane.op->getBlock() != &section.getBody().front()) {
                    return accessLane.op->emitOpError("persistent fragment access does not belong to its planned "
                                                      "SIMT section body");
                }
                if (!assignedAccessLanes[accessLane.op].insert(accessLane.laneIndex).second) {
                    return accessLane.op->emitOpError("persistent fragment access lane was assigned to more than one "
                                                      "transform work item");
                }
    ...
    if (expectedElementIndex != sectionWorklist.elements.size()) {
        return section.emitOpError("persistent fragment section worklist contains an unexpected "
                                   "resident element");
    }
...
static LogicalResult buildPersistentTransformWorklist(
    const PersistentMaterializationPlan& plan, PersistentTransformWorklist& worklist)
{
    ...
    for (PersistentSectionWorklist& sectionWorklist : worklist.sections) {
        if (!sectionWorklist.section)
            return failure();
        auto [it, inserted] = worklistBySection.try_emplace(sectionWorklist.section.getOperation(), &sectionWorklist);
        (void)it;
        if (!inserted) {
    ...
    for (const PersistentFragmentAnalysis& fragment : plan.fragments) {
        ...
        if (!fragment.initSection) {
            return allocaOp.emitOpError("persistent fragment has no init section in its analysis plan");
        }
        ...
        auto addToSection = [&](pto::SectionSimtOp section) -> LogicalResult {
            if (!addedSections.insert(section.getOperation()).second) {
                return allocaOp.emitOpError("persistent fragment init/carry sections contain a duplicate");
            }
            auto sectionIt = worklistBySection.find(section.getOperation());
            if (sectionIt == worklistBySection.end()) {
                return allocaOp.emitOpError("persistent fragment init/carry section is not in the plan");
            }

            PersistentSectionWorklist& sectionWorklist = *sectionIt->second;
            for (const ResidentElementPlan& residentElement : fragment.residentElements) {
                ...
                for (const AccessLane& accessLane : residentElement.accesses) {
                    pto::SectionSimtOp accessSection =
                        accessLane.op ? accessLane.op->getParentOfType<pto::SectionSimtOp>() : pto::SectionSimtOp();
                    if (accessSection == section)
                        localAccesses.push_back(accessLane);
                }
                sectionWorklist.elements.push_back({&fragment, &residentElement, std::move(localAccesses)});
            }
            return success();
        };
    ...
    llvm::DenseMap<Operation*, llvm::DenseSet<unsigned>> assignedAccessLanes;
    for (PersistentSectionWorklist& sectionWorklist : worklist.sections) {
        if (failed(validateSectionWorklist(plan, sectionWorklist, assignedAccessLanes)))
            return failure();
    }
    ...
    for (const PersistentFragmentAnalysis& fragment : plan.fragments) {
        for (const ResidentElementPlan& element : fragment.residentElements) {
            for (const AccessLane& accessLane : element.accesses) {
                if (!accessLane.op) {
                    LLVM::AllocaOp allocaOp = fragment.allocaOp;
                    return allocaOp.emitOpError("persistent fragment analysis contains a null access "
                                                "operation");
                }
                auto assignedIt = assignedAccessLanes.find(accessLane.op);
                if (assignedIt == assignedAccessLanes.end() || !assignedIt->second.contains(accessLane.laneIndex)) {
                    accessLane.op->emitOpError("persistent fragment access lane was not assigned to a "
                                               "transform work item");
                    return failure();
                }
            }
    ...
    MutableArrayRef<PersistentElementRewrite> elementRewrites)
{
    ...
    if (!isa<VectorType>(*accessType)) {
        assert(
            laneRewrites.size() == 1 && laneRewrites.front().laneIndex == 0 &&
            "scalar access must map to exactly one scalar lane");
        LLVM::AllocaOp proxy = elementRewrites[laneRewrites.front().elementIndex].proxy;
    ...
    if (auto store = dyn_cast<LLVM::StoreOp>(access)) {
        for (const PersistentLaneRewrite& laneRewrite : laneRewrites) {
            ...
            Value laneIndex = builder.create<arith::ConstantIntOp>(loc, laneRewrite.laneIndex, /*width=*/32);
            Value laneValue = builder.create<LLVM::ExtractElementOp>(loc, store.getValue(), laneIndex);
    ...
    for (const PersistentLaneRewrite& laneRewrite : laneRewrites) {
        ...
        Value laneValue = builder.create<LLVM::LoadOp>(loc, vectorType.getElementType(), proxy.getRes());
        Value laneIndex = builder.create<arith::ConstantIntOp>(loc, laneRewrite.laneIndex, /*width=*/32);
        rebuiltVector = builder.create<LLVM::InsertElementOp>(loc, vectorType, rebuiltVector, laneValue, laneIndex);
    }
...
static LogicalResult materializeSection(
    const PersistentSectionWorklist& sectionWorklist, const DataLayout& dataLayout, DominanceInfo& dominance)
{
    ...
    if (hasLocalAccess) {
        proxyArraySize = entryBuilder.create<arith::ConstantIntOp>(section.getLoc(), 1, /*width=*/32);
    }
    ...
    for (auto [elementIndex, element] : llvm::enumerate(elements)) {
        ...
        rewrites[elementIndex].resumeValue =
            entryBuilder
                .create<pto::ResumeOp>(
                    section.getLoc(), allocaOp.getElemType(), static_cast<uint64_t>(residentElement.slot))
                .getResult();
    }
    ...
    for (auto [elementIndex, element] : llvm::enumerate(elements)) {
        ...
        LLVM::AllocaOp proxy = entryBuilder.create<LLVM::AllocaOp>(
            elementLoc, allocaOp.getRes().getType(), allocaOp.getElemType(), proxyArraySize);
        rewrites[elementIndex].proxy = proxy;

        if (section != fragment.initSection) {
            assert(rewrites[elementIndex].resumeValue && "carry element must have a resume value");
            entryBuilder.create<LLVM::StoreOp>(elementLoc, rewrites[elementIndex].resumeValue, proxy.getRes());
        }
    }
    ...
    if (rewrittenAccessCount != laneRewritesByAccess.size()) {
        return section.emitOpError("failed to rewrite every persistent fragment access in block order");
    }
    ...
    for (auto [elementIndex, element] : llvm::enumerate(elements)) {
        ...
        if (rewrites[elementIndex].proxy) {
            keepPayloads.push_back(
                exitBuilder
                    .create<LLVM::LoadOp>(
                        section.getLoc(), allocaOp.getElemType(), rewrites[elementIndex].proxy.getRes())
                    .getResult());
            continue;
        }
        assert(rewrites[elementIndex].resumeValue && "an element without local accesses must pass through resume");
        keepPayloads.push_back(rewrites[elementIndex].resumeValue);
    }

    for (auto [element, payload] : llvm::zip_equal(elements, keepPayloads)) {
        const ResidentElementPlan& residentElement = *element.residentElement;
        exitBuilder.create<pto::KeepOp>(section.getLoc(), payload, static_cast<uint64_t>(residentElement.slot));
    }
    ...
    for (auto [elementIndex, element] : llvm::enumerate(elements)) {
        ...
        if (failed(tryToPromoteMemorySlots(allocators, promotionBuilder, dataLayout, dominance))) {
            return section.emitOpError() << "failed to promote scalar proxy for persistent fragment "
                                            "element offset "
...
static LogicalResult eraseDeadPersistentGEPs(Value pointer)
{
    ...
    for (Operation* user : pointer.getUsers()) {
        ...
        if (!gep) {
            return user->emitOpError("persistent fragment access remained after scalar proxy promotion");
        }
    ...
    for (LLVM::GEPOp gep : derivedPointers) {
        ...
        if (!gep.getRes().use_empty()) {
            return gep.emitOpError("persistent fragment derived pointer still has live uses after "
...
static LogicalResult materializePersistentFragments(
    func::FuncOp func, DominanceInfo& dominance, const PersistentMaterializationPlan& plan,
    const PersistentTransformWorklist& worklist)
{
    ...
    for (const PersistentFragmentAnalysis& fragment : plan.fragments) {
        ...
        if (!allocaOp.getRes().use_empty()) {
            return allocaOp.emitOpError("persistent fragment still has live uses after materialization");
        }
        allocaOp.erase();
    }
    return success();
...
struct PTOMaterializeSIMTPersistentFragmentPass
    : public pto::impl::PTOMaterializeSIMTPersistentFragmentBase<PTOMaterializeSIMTPersistentFragmentPass> {
    using pto::impl::PTOMaterializeSIMTPersistentFragmentBase<
        PTOMaterializeSIMTPersistentFragmentPass>::PTOMaterializeSIMTPersistentFragmentBase;
    ...
    void runOnOperation() override
    {
        ...
        if (failed(materializePersistentFragments(func, dominance, plan, worklist))) {
            signalPassFailure();
            return;
        }
    }
...
std::unique_ptr<Pass> mlir::pto::createPTOMaterializeSIMTPersistentFragmentPass()
{
