...
static SmallVector<VMILayoutRelation, 4> makeSingleRelation(SmallVector<VMILayoutFact, 4> facts)
{
...
static bool relationContainsOperandLayout(const VMILayoutRelation& relation, OpOperand& operand, VMILayoutAttr layout)
{
...
static bool relationContainsValueLayout(const VMILayoutRelation& relation, Value value, VMILayoutAttr layout)
{
    for (const VMILayoutFact& fact : relation.facts)
...
static Type getValueTypeWithLayout(Value value, VMILayoutAttr layout)
{
    if (auto type = dyn_cast<VMIVRegType>(value.getType()))
        return VMIVRegType::get(type.getContext(), type.getElementCount(), type.getElementType(), layout);
    if (auto type = dyn_cast<VMIMaskType>(value.getType()))
        return VMIMaskType::get(type.getContext(), type.getElementCount(), type.getGranularity(), layout);
    return {};
}
...
public:
    ...
    virtual FailureOr<SmallVector<VMILayoutRelation, 4>> query(
        Operation* op, Value changedValue, VMILayoutAttr changedLayout, const VMILayoutPropagator& propagator,
        OpOperand* changedOperand) const = 0;

    LogicalResult propagate(
        Operation* op, Value changedValue, VMILayoutAttr changedLayout, VMILayoutPropagator& propagator,
...
public:
    FailureOr<SmallVector<VMILayoutRelation, 4>> query(
        Value source, VMILayoutAttr sourceLayout, VMILayoutAttr resultLayout) const
    {
        if (!source || !sourceLayout || !resultLayout)
            return failure();
        if (sourceLayout == resultLayout)
            return makeSingleRelation(SmallVector<VMILayoutFact, 4>{valueFact(source, sourceLayout)});
        ...
        if (auto sourceVRegType = dyn_cast<VMIVRegType>(sourceAssignedType)) {
            ...
            if (!resultVRegType || failed(supports.getEnsureLayoutFact(sourceVRegType, resultVRegType)))
                return failure();
            return makeSingleRelation(SmallVector<VMILayoutFact, 4>{valueFact(source, sourceLayout)});
        }
        ...
        if (auto sourceMaskType = dyn_cast<VMIMaskType>(sourceAssignedType)) {
            ...
            if (!resultMaskType || failed(supports.getEnsureMaskLayoutFact(sourceMaskType, resultMaskType)))
                return failure();
            return makeSingleRelation(SmallVector<VMILayoutFact, 4>{valueFact(source, sourceLayout)});
        }
        return failure();
...
{
    return isa<
        VMIAddFOp, VMIAddIOp, VMISubFOp, VMISubIOp, VMIMulFOp, VMIMulIOp, VMIAddSOp, VMIMulSOp, VMIMaxSOp, VMIMinSOp,
        VMIShlSOp, VMIShrSOp, VMIVmullOp, VMIFmaOp, VMIDivFOp, VMIMinFOp, VMIMinIOp, VMIMaxFOp, VMIMaxIOp, VMINegFOp,
        VMIAbsFOp, VMIAbsIOp, VMISqrtOp, VMIExpOp, VMILnOp, VMIReluOp, VMIVexpdifOp, VMIFPToSIOp, VMISIToFPOp,
        VMIAndIOp, VMIOrIOp, VMIXOrIOp, VMIShLIOp, VMIShRUIOp, VMIShRSIOp, VMINotOp, VMICmpFOp, VMICmpIOp, VMISelectOp,
        VMIMaskAndOp, VMIMaskOrOp, VMIMaskXOrOp, VMIMaskNotOp, VMIActivePrefixIndexOp, VMICompressOp, VMIExpandLoadOp>(
        op);
}

static bool isCastOp(Operation* op)
{
    return isa<VMIExtFOp, VMIExtSIOp, VMIExtUIOp, VMITruncFOp, VMITruncIOp, VMIFPToSIOp, VMIFPToUIOp, VMISIToFPOp>(op);
}
        ...
        OpOperand* changedOperand) const override
    {
        ...
        if (changedValue == op->getOperand(0)) {
            FailureOr<SmallVector<VMICastLayoutFact, 4>> facts =
                supports.getCastLayoutFactsForLayout(sourceType, resultType, VMICastLayoutPort::Source, changedLayout);
        ...
        if (changedValue == op->getResult(0)) {
            FailureOr<SmallVector<VMICastLayoutFact, 4>> facts =
                supports.getCastLayoutFactsForLayout(sourceType, resultType, VMICastLayoutPort::Result, changedLayout);
            if (failed(facts) || facts->empty())
                return failure();
            SmallVector<VMILayoutRelation, 4> relations;
            for (const VMICastLayoutFact& fact : *facts)
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        if (changedOperand == &bitcast.getSourceMutable() || changedValue == bitcast.getSource()) {
        ...
        FailureOr<SmallVector<VMIBitcastLayoutFact, 4>> facts =
            supports.getBitcastLayoutFactsForLayout(sourceType, resultType, port, changedLayout);
        if (failed(facts) || facts->empty())
            return failure();
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        if (isa<OpResult>(changedValue) && changedValue.getDefiningOp() == op)
            return makeSingleRelation(SmallVector<VMILayoutFact, 4>{valueFact(changedValue, changedLayout)});
        return failure();
    }
};
...
public:
    FailureOr<SmallVector<VMILayoutRelation, 4>> query(
        ...
        OpOperand* changedOperand) const override
    {
        if (!isa<OpResult>(changedValue) || changedValue.getDefiningOp() != op || !changedLayout.isContiguous())
            return failure();
        return makeSingleRelation(SmallVector<VMILayoutFact, 4>{valueFact(changedValue, changedLayout)});
    }
};
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        auto assignedType = VMIVRegType::get(
            resultType.getContext(), resultType.getElementCount(), resultType.getElementType(), changedLayout);
        VMILayoutSupport supports;
        if (failed(supports.getLoadLayoutFact(assignedType)))
            return failure();
        return makeSingleRelation(SmallVector<VMILayoutFact, 4>{valueFact(load.getResult(), changedLayout)});
    }
};
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        FailureOr<SmallVector<VMIDeinterleaveLoadLayoutFact, 4>> facts =
            supports.getDeinterleaveLoadLayoutFactsForLayout(valueType, port, changedLayout);
        if (failed(facts) || facts->empty())
            return failure();
        ...
        for (const VMIDeinterleaveLoadLayoutFact& fact : *facts) {
            relations.push_back(makeRelation(SmallVector<VMILayoutFact, 4>{
                valueFact(load.getLow(), fact.lowLayout), valueFact(load.getHigh(), fact.highLayout)}));
        }
        return relations;
    }
};
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        VMILayoutSupport supports;
        if (failed(
                supports.getGroupLoadLayoutFact(assignedType, load.getRowStride(), load.getNumGroupsAttr().getInt())))
            return failure();
        return makeSingleRelation(SmallVector<VMILayoutFact, 4>{valueFact(load.getResult(), changedLayout)});
    }
};
...
private:
    ...
    FailureOr<SmallVector<VMILayoutRelation, 4>> queryReduce(
        OpTy reduce, Value changedValue, VMILayoutAttr changedLayout, OpOperand* changedOperand) const
    {
        ...
        VMILayoutSupport supports;
        FailureOr<SmallVector<VMIGroupReduceLayoutFact, 4>> facts = supports.getGroupReduceLayoutFactsForLayout(
            sourceType, reduce.getNumGroupsAttr().getInt(), port, changedLayout);
        if (failed(facts) || facts->empty())
            return failure();
        SmallVector<VMILayoutRelation, 4> relations;
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        auto assignedType = VMIVRegType::get(
            resultType.getContext(), resultType.getElementCount(), resultType.getElementType(), changedLayout);
        VMILayoutSupport supports;
        if (failed(supports.getGroupSlotLoadLayoutFact(assignedType, load.getNumGroupsAttr().getInt())))
            return failure();
        return makeSingleRelation(SmallVector<VMILayoutFact, 4>{valueFact(load.getResult(), changedLayout)});
    }
};
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        if (failed(supports.getGroupBroadcastLoadLayoutFact(
                assignedType, load.getSourceGroupStride(), load.getNumGroupsAttr().getInt())))
            return failure();
        return makeSingleRelation(SmallVector<VMILayoutFact, 4>{valueFact(load.getResult(), changedLayout)});
    }
};
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        FailureOr<SmallVector<VMIGroupBroadcastLayoutFact, 4>> facts =
            supports.getGroupBroadcastLayoutFactsForLayout(sourceType, resultType, numGroups, port, changedLayout);
        if (failed(facts) || facts->empty())
            return failure();
        SmallVector<VMILayoutRelation, 4> relations;
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        if (auto vintlv = dyn_cast<VMIVintlvOp>(op))
            return queryInterleave(vintlv, /*vintlv=*/true, changedValue, changedLayout, changedOperand);
        if (auto vdintlv = dyn_cast<VMIVdintlvOp>(op))
            return queryInterleave(vdintlv, /*vintlv=*/false, changedValue, changedLayout, changedOperand);
        return failure();
    }

private:
    template <typename OpTy>
    FailureOr<SmallVector<VMILayoutRelation, 4>> queryInterleave(
        OpTy op, bool vintlv, Value changedValue, VMILayoutAttr changedLayout, OpOperand* changedOperand) const
    {
        ...
        } else if (changedOperand == &op.getRhsMutable() || changedValue == op.getRhs()) {
            port = VMIInterleaveLayoutPort::Rhs;
        } else if (changedOperand == &op.getMaskMutable() || changedValue == op.getMask()) {
        ...
        FailureOr<SmallVector<VMIInterleaveLayoutFact, 4>> facts =
            vintlv ? supports.getVintlvLayoutFactsForLayout(lowType, port, changedLayout) :
                     supports.getVdintlvLayoutFactsForLayout(lowType, port, changedLayout);
        if (failed(facts) || facts->empty())
            return failure();
        ...
        for (const VMIInterleaveLayoutFact& fact : *facts) {
            relations.push_back(makeRelation(SmallVector<VMILayoutFact, 4>{
                operandFact(op.getLhsMutable(), fact.lhsLayout), operandFact(op.getRhsMutable(), fact.rhsLayout),
                operandFact(op.getMaskMutable(), fact.maskLayout), valueFact(op.getLow(), fact.lowLayout),
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        return makeSingleRelation(SmallVector<VMILayoutFact, 4>{
            operandFact(gather.getIndicesMutable(), changedLayout), operandFact(gather.getMaskMutable(), changedLayout),
            operandFact(gather.getPassthruMutable(), changedLayout), valueFact(gather.getResult(), changedLayout)});
    }
};
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        auto assignedValueType = VMIVRegType::get(
            valueType.getContext(), valueType.getElementCount(), valueType.getElementType(), changedLayout);
        VMILayoutSupport supports;
        ...
        return makeSingleRelation(SmallVector<VMILayoutFact, 4>{operandFact(store.getValueMutable(), useLayout)});
    }
};
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        FailureOr<SmallVector<VMIGroupStoreLayoutFact, 4>> facts =
            supports.getGroupStoreLayoutFactsForLayout(store, valueType, changedLayout);
        if (failed(facts) || facts->empty())
            return failure();
        SmallVector<VMILayoutRelation, 4> relations;
        for (const VMIGroupStoreLayoutFact& fact : *facts)
            relations.push_back(
                makeRelation(SmallVector<VMILayoutFact, 4>{operandFact(store.getValueMutable(), fact.valueLayout)}));
        return relations;
    }
};
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        if (!load)
            return failure();
        if (changedValue != load.getResult() && changedValue != load.getMask() && changedValue != load.getPassthru())
            return failure();
        ...
        auto assignedMaskType = VMIMaskType::get(
            maskType.getContext(), maskType.getElementCount(), maskType.getGranularity(), changedLayout);
        auto assignedPassthruType = VMIVRegType::get(
            passthruType.getContext(), passthruType.getElementCount(), passthruType.getElementType(), changedLayout);
        VMILayoutSupport supports;
        if (failed(supports.getMaskedLoadLayoutFact(assignedResultType, assignedMaskType, assignedPassthruType)))
            return failure();

        return makeSingleRelation(SmallVector<VMILayoutFact, 4>{
            valueFact(load.getResult(), changedLayout), operandFact(load.getMaskMutable(), changedLayout),
...
public:
        ...
        OpOperand* changedOperand) const override
    {
        ...
        VMILayoutAttr useLayout = changedLayout;
        auto assignedValueType = VMIVRegType::get(
            valueType.getContext(), valueType.getElementCount(), valueType.getElementType(), useLayout);
        auto assignedMaskType =
            VMIMaskType::get(maskType.getContext(), maskType.getElementCount(), maskType.getGranularity(), useLayout);
        if (failed(supports.getMaskedStoreLayoutFact(assignedValueType, assignedMaskType)))
            useLayout = VMILayoutAttr::getContiguous(valueType.getContext());

        return makeSingleRelation(SmallVector<VMILayoutFact, 4>{
            operandFact(store.getValueMutable(), useLayout), operandFact(store.getMaskMutable(), useLayout)});
    }
};
...
const VMILayoutTransfer* getTransfer(Operation* op)
{
    ...
    if (isa<VMIConstantOp, VMIBroadcastOp, VMIIotaOp, VMICreateMaskOp, VMICreateGroupMaskOp, VMIConstantMaskOp>(op))
    ...
    if (isa<VMIGroupReduceAddFOp, VMIGroupReduceMaxFOp, VMIGroupReduceMinFOp, VMIGroupReduceAddIOp,
...
VMILayoutPropagator::VMILayoutPropagator(Operation* scope) : scope(scope), ctx(scope ? scope->getContext() : nullptr) {}

bool VMILayoutPropagator::isLayoutValue(Value value) const { return isa<VMIVRegType, VMIMaskType>(value.getType()); }
...
Type VMILayoutPropagator::getTypeWithLayout(Value value, VMILayoutAttr layout) const
{
    if (auto type = dyn_cast<VMIVRegType>(value.getType()))
        return VMIVRegType::get(ctx, type.getElementCount(), type.getElementType(), layout);
    if (auto type = dyn_cast<VMIMaskType>(value.getType()))
        return VMIMaskType::get(ctx, type.getElementCount(), type.getGranularity(), layout);
    return {};
}

bool VMILayoutPropagator::canUseOperandLayout(OpOperand& operand, VMILayoutAttr layout) const
{
    if (!layout)
        return false;
    ...
    if (!transfer)
        return false;
    FailureOr<SmallVector<VMILayoutRelation, 4>> relations =
        transfer->query(operand.getOwner(), operand.get(), layout, *this, &operand);
    if (failed(relations))
        return false;
...
VMILayoutAttr VMILayoutPropagator::getRequestedOrCurrentLayout(Value value) const
{
...
LogicalResult VMILayoutPropagator::addUseConflict(
    OpOperand& operand, VMIValueLayoutAssignment& assignment, VMILayoutAttr layout)
{
...
bool VMILayoutPropagator::canProduceValueLayout(Value value, VMILayoutAttr layout) const
{
    ...
    if (auto result = dyn_cast<OpResult>(value)) {
        ...
        if (!transfer)
            return isTypeRewriteable(value);
        FailureOr<SmallVector<VMILayoutRelation, 4>> relations =
            transfer->query(definingOp, value, layout, *this, /*changedOperand=*/nullptr);
        if (failed(relations))
            return false;
        for (const VMILayoutRelation& relation : *relations)
...
LogicalResult VMILayoutPropagator::request(OpOperand& operand, VMILayoutAttr layout)
{
    if (!layout)
        return failure();
...
LogicalResult VMILayoutPropagator::propagateFact(Value value, VMILayoutAttr layout)
{
    if (!isLayoutValue(value))
        return success();
    ...
    for (OpOperand* use : uses)
        if (getOperandLayout(*use) == layout && failed(propagateThrough(use->getOwner(), value, layout)))
            return failure();

    return success();
}

LogicalResult VMILayoutPropagator::propagateOperandFact(OpOperand& operand, VMILayoutAttr layout)
{
...
LogicalResult VMILayoutPropagator::propagateThrough(
    Operation* op, Value changedValue, VMILayoutAttr changedLayout, OpOperand* changedOperand)
{
    const VMILayoutTransfer* transfer = op ? getTransfer(op) : nullptr;
    if (!transfer)
        return success();
    return transfer->propagate(op, changedValue, changedLayout, *this, changedOperand);
}
    ...
    for (Value value : orderedValues) {
        ...
        if (!assignment.layout)
            return emitError(value.getLoc())
                   << kVMIDiagLayoutContractPrefix << "layout assignment has conflicts but no primary layout";

        VMILayoutAttr currentLayout = getCurrentLayout(value);
        if (currentLayout != assignment.layout && !isTypeRewriteable(value)) {
            if (!currentLayout)
                return emitError(value.getLoc()) << kVMIDiagLayoutContractPrefix
                                                 << "cannot materialize primary VMI layout from an unassigned "
                                                    "boundary value";
            if (!canMaterializeLayout(value, currentLayout, assignment.layout))
                return emitError(value.getLoc())
                       << kVMIDiagLayoutContractPrefix << "cannot materialize primary VMI layout " << assignment.layout
                       << " from " << currentLayout;
        }

        for (const VMILayoutConflict& conflict : assignment.conflicts) {
            if (!canMaterializeLayout(value, assignment.layout, conflict.layout))
                return emitError(conflict.operand ? conflict.operand->getOwner()->getLoc() : value.getLoc())
                       << kVMIDiagLayoutContractPrefix << "cannot materialize requested VMI layout " << conflict.layout
...
FailureOr<Value> VMILayoutPropagator::materializeAt(
    Value source, VMILayoutAttr layout, RewriterBase& rewriter, Location loc)
{
    ...
    if (isa<VMIVRegType>(source.getType()))
        return rewriter.create<VMIEnsureLayoutOp>(loc, resultType, source).getResult();
    if (isa<VMIMaskType>(source.getType()))
        return rewriter.create<VMIEnsureMaskLayoutOp>(loc, resultType, source).getResult();
    return failure();
}
    ...
    DenseMap<Value, Value>& assignedValues)
{
    ...
    if (!getCurrentLayout(value)) {
        ...
        if (!owner)
            return failure();
        return owner->emitError() << kVMIDiagLayoutContractPrefix
    ...
    FailureOr<Value> materialized = materializeAt(value, assignment.layout, rewriter, value.getLoc());
    ...
    Value assignedValue, VMILayoutConflict conflict, RewriterBase& rewriter)
{
    ...
    FailureOr<Value> materialized = materializeAt(assignedValue, conflict.layout, rewriter, owner->getLoc());
    if (failed(materialized))
        return owner->emitError() << kVMIDiagLayoutContractPrefix << "cannot materialize requested VMI operand layout "
                                  << conflict.layout;
...
LogicalResult VMILayoutPropagator::apply(RewriterBase& rewriter)
{
    ...
    for (Value value : orderedValues) {
        ...
        if (it == assignments.end())
            continue;
        if (failed(materializePrimary(value, it->second, rewriter, assignedValues)))
            return failure();
    }
