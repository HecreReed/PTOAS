...
static LogicalResult eraseDeadVPTOMemRefScaffold(ModuleOp module)
{
    ...
    while (erasedAny) {
        ...
        module.walk([&](Operation* op) {
            ...
            if (!op->use_empty())
                return;
            if (isa<memref::ReinterpretCastOp, memref::SubViewOp, memref::MemorySpaceCastOp>(op))
                deadOps.push_back(op);
        });
...
static bool needsPtrCanonicalization(Value value) { return isa<BaseMemRefType, pto::TileBufType>(value.getType()); }

static bool isSupportedVPTOBufferLikeBoundaryOp(Operation* op)
{
    return isa<
        pto::VldsOp, pto::UvldOp, pto::PldsOp, pto::PldiOp, pto::VstsOp, pto::PstsOp, pto::PstiOp, pto::Vldsx2Op,
        pto::Vstsx2Op, pto::VsldbOp, pto::VsstbOp, pto::VstasOp, pto::VstarOp, pto::LoadScalarOp, pto::StoreScalarOp,
        pto::MteGmL1FracOp, pto::MteL1FbOp, pto::MteL1L0aOp, pto::MteL1L0bOp, pto::MteL0cL1Op, pto::MteL0cGmOp>(op);
}

static LogicalResult canonicalizeBoundaryCastPtrOps(ModuleOp module, llvm::raw_ostream* diagOS)
{
    ...
    for (pto::CastPtrOp castOp : castsToRewrite) {
        ...
        Value ptrValue = pto::materializeBufferPointer(
            castOp.getInput(), resultType.getElementType(), resultType.getMemorySpace(), rewriter, castOp.getLoc());
        if (!ptrValue) {
            if (diagOS) {
...
static LogicalResult canonicalizeSupportedVPTOBufferLikeOps(ModuleOp module, llvm::raw_ostream* diagOS)
{
    ...
    for (Operation* op : opsToRewrite) {
        ...
        for (Value operand : op->getOperands()) {
            ...
            Value ptrValue = pto::materializeBufferPointer(operand, elementType, memorySpace, rewriter, op->getLoc());
            if (!ptrValue) {
                if (diagOS) {
                    *diagOS << "VPTO emission-boundary ptr rewrite failed: could not "
...
struct PTOVPTOPtrBoundaryPass : public pto::impl::PTOVPTOPtrBoundaryBase<PTOVPTOPtrBoundaryPass> {
    using pto::impl::PTOVPTOPtrBoundaryBase<PTOVPTOPtrBoundaryPass>::PTOVPTOPtrBoundaryBase;
...
LogicalResult mlir::pto::convertVPTOEmissionBoundaryToPtr(ModuleOp module, llvm::raw_ostream* diagOS)
{
    ...
    for (func::FuncOp func : module.getOps<func::FuncOp>()) {
        ...
        SmallVector<Type> newInputs(functionType.getInputs().begin(), functionType.getInputs().end());
        bool changed = false;
        ...
        for (auto [idx, inputType] : llvm::enumerate(functionType.getInputs())) {
            ...
            for (Operation* user : users) {
                ...
                if (isa<memref::ReinterpretCastOp, memref::SubViewOp, memref::MemorySpaceCastOp>(user) &&
                    user->use_empty()) {
                    user->erase();
                    continue;
                }
                ...
                if (diagOS) {
                    *diagOS << "VPTO emission-boundary ptr rewrite failed: argument " << idx << " of " << func.getName()
        ...
        if (changed) {
            func.setFunctionType(FunctionType::get(module.getContext(), newInputs, functionType.getResults()));
        }
    }
...
std::unique_ptr<Pass> mlir::pto::createPTOVPTOPtrBoundaryPass() { return std::make_unique<PTOVPTOPtrBoundaryPass>(); }
