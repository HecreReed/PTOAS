...
std::optional<PhysicalSectionKind> inferPhysicalSectionKindFromPipe(Operation* op)
{
...
std::optional<AddressSpaceAttr> GetBufferSpaceAttr(Value operand)
{
    if (auto tileTy = dyn_cast<pto::TileBufType>(operand.getType())) {
        if (auto memorySpaceAttr = dyn_cast_or_null<AddressSpaceAttr>(tileTy.getMemorySpace()))
            return memorySpaceAttr;
        return std::nullopt;
    }
    if (auto multiTy = dyn_cast<pto::MultiTileBufType>(operand.getType())) {
        if (auto memorySpaceAttr = dyn_cast_or_null<AddressSpaceAttr>(multiTy.getSlotType().getMemorySpace()))
            return memorySpaceAttr;
        return std::nullopt;
    }
