...
enum PlanStatus { PLAN_SUCCESS = 0, RESTART_NEW_PLAN, CONTINUE_PLAN, PLAN_FAILED };
...
private:
    ...
    void UpdateFusionRegionBufferAlias(pto::FusionRegionOp fusionRegion, pto::YieldOp yieldOp);
    ...
    BufferInfo GetBufferInfo(Operation* op, Value operand, pto::AddressSpace bufferScope);
...
public:
    ...
    inline void SetGenKillMap(DenseMap<OpInfo*, GenKillEntry> gkMap) { genKillMap = gkMap; }
...
private:
    ...
    void PlanBuffersWithoutReuse(StorageEntry* rootStorageEntry, size_t alignUnit);
    ...
    inline bool VerifyConflictStage0(StorageEntry* e, const std::shared_ptr<MemoryBound>& last);
    ...
    void ReorderContinuousPingPongEntry(SmallVector<StorageEntry*>& storageEntryVec);
