...
struct MaskGranularitySolver {
    explicit MaskGranularitySolver(ModuleOp module) : module(module), ctx(module.getContext()) {}
    ...
    LogicalResult uniteMask(Value lhs, Value rhs, Operation* op)
    {
        ...
        if (!lhsNode.granularity.empty() && !rhsNode.granularity.empty() && lhsNode.granularity != rhsNode.granularity)
            return op->emitError() << kVMIDiagLayoutContractPrefix << "conflicting mask granularities "
                                   << lhsNode.granularity << " and " << rhsNode.granularity;
    ...
    LogicalResult requestMask(Value mask, StringRef granularity, Operation* op)
    {
        ...
        if (granularity.empty())
            return op->emitError() << kVMIDiagLayoutContractPrefix << "cannot infer concrete mask granularity";
        MaskNode& node = maskNodes[findMask(id)];
        if (!node.granularity.empty() && node.granularity != granularity)
            return op->emitError() << kVMIDiagLayoutContractPrefix << "conflicting mask granularities "
                                   << node.granularity << " and " << granularity;
        node.granularity = granularity.str();
        return success();
    }

    LogicalResult requestMaskUse(OpOperand& operand, StringRef granularity, Operation* op)
    {
        if (!isa<VMIMaskType>(operand.get().getType()))
            return success();
        if (granularity.empty())
            return op->emitError() << kVMIDiagLayoutContractPrefix << "cannot infer concrete mask use granularity";
    ...
    LogicalResult addConstraints()
    {
        WalkResult result = module.walk([&](Operation* op) -> WalkResult {
            ...
            if (auto cmpf = dyn_cast<VMICmpFOp>(op)) {
                ...
                if (failed(requestMask(cmpf.getResult(), getMaskGranularityForElement(lhsType.getElementType()), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto cmpi = dyn_cast<VMICmpIOp>(op)) {
                auto lhsType = cast<VMIVRegType>(cmpi.getLhs().getType());
                if (failed(requestMask(cmpi.getResult(), getMaskGranularityForElement(lhsType.getElementType()), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto select = dyn_cast<VMISelectOp>(op)) {
                ...
                if (failed(requestMaskUse(
                        select.getMaskMutable(), getMaskGranularityForElement(resultType.getElementType()), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto vecScalar = dyn_cast<VMIAddSOp>(op)) {
                if (failed(requestMaskUseForSource(vecScalar.getMaskMutable(), vecScalar.getSrc(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto activePrefix = dyn_cast<VMIActivePrefixIndexOp>(op)) {
                ...
                if (failed(requestMaskUse(
                        activePrefix.getMaskMutable(), getMaskGranularityForElement(resultType.getElementType()), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto compress = dyn_cast<VMICompressOp>(op)) {
                ...
                if (failed(requestMaskUse(
                        compress.getMaskMutable(), getMaskGranularityForElement(resultType.getElementType()), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto vintlv = dyn_cast<VMIVintlvOp>(op)) {
                if (failed(requestMaskUseForSource(vintlv.getMaskMutable(), vintlv.getLhs(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto vdintlv = dyn_cast<VMIVdintlvOp>(op)) {
                if (failed(requestMaskUseForSource(vdintlv.getMaskMutable(), vdintlv.getLhs(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            if (auto reduce = dyn_cast<VMIReduceAddIOp>(op)) {
                if (failed(requestMaskUseForSource(reduce.getMaskMutable(), reduce.getSource(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto load = dyn_cast<VMIStrideLoadOp>(op)) {
                ...
                if (failed(requestMaskUse(
                        load.getMaskMutable(), getMaskGranularityForElement(resultType.getElementType()), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto gather = dyn_cast<VMIGatherOp>(op)) {
                ...
                if (failed(requestMaskUse(
                        gather.getMaskMutable(), getMaskGranularityForElement(resultType.getElementType()), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto store = dyn_cast<VMIMaskedStoreOp>(op)) {
                if (failed(requestMaskUseForSource(store.getMaskMutable(), store.getValue(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto scatter = dyn_cast<VMIScatterOp>(op)) {
                if (failed(requestMaskUseForSource(scatter.getMaskMutable(), scatter.getValue(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (auto condBranchOp = dyn_cast<cf::CondBranchOp>(op)) {
                if (failed(addBranchConstraints(condBranchOp.getTrueDest(), condBranchOp.getTrueDestOperands(), op)) ||
                    failed(addBranchConstraints(condBranchOp.getFalseDest(), condBranchOp.getFalseOperands(), op)))
                    return WalkResult::interrupt();
                return WalkResult::advance();
            }
            ...
            if (op->getName().getStringRef() == "func.call_indirect" && hasVMIValueTypes(op)) {
    ...
    LogicalResult requestMaskUseForSource(OpOperand& mask, Value source, Operation* op)
    {
        auto sourceType = dyn_cast<VMIVRegType>(source.getType());
        if (!sourceType)
            return success();
        return requestMaskUse(mask, getMaskGranularityForElement(sourceType.getElementType()), op);
    }

    LogicalResult uniteEquivalentValues(Value lhs, Value rhs, Operation* op) { return uniteMask(lhs, rhs, op); }
    ...
    void rewriteMaskTypes()
    {
        for (MaskNode& node : maskNodes) {
            ...
            StringRef granularity = root.granularity.empty() ? StringRef("b32") : StringRef(root.granularity);
            node.value.setType(
                VMIMaskType::get(ctx, node.type.getElementCount(), granularity, node.type.getLayoutAttr()));
        }
    }
            ...
            if (!callResultTypes.empty()) {
                ...
                for (Type type : functionType.getResults()) {
                    if (auto maskType = dyn_cast<VMIMaskType>(type)) {
                        StringRef granularity = VMIMaskType::isConcreteGranularity(maskType.getGranularity()) ?
                                                    maskType.getGranularity() :
                                                    StringRef("b32");
                        results.push_back(
                            VMIMaskType::get(ctx, maskType.getElementCount(), granularity, maskType.getLayoutAttr()));
                    } else {
    ...
    LogicalResult insertMaskUseMaterializations()
    {
        ...
        for (MaskUseRequest request : maskUseRequests) {
            ...
            auto resultType =
                VMIMaskType::get(ctx, sourceType.getElementCount(), request.granularity, sourceType.getLayoutAttr());
            Value current =
                rematerializeMaskProducer(value, resultType, request.operand->getOwner()->getLoc(), builder);
            if (!current)
    ...
    Value rematerializeMaskProducer(Value value, VMIMaskType resultType, Location loc, OpBuilder& builder)
    {
        if (auto createMask = value.getDefiningOp<VMICreateMaskOp>())
            return builder.create<VMICreateMaskOp>(loc, resultType, createMask.getActiveLanes()).getResult();

        if (auto createGroupMask = value.getDefiningOp<VMICreateGroupMaskOp>())
            return builder
                .create<VMICreateGroupMaskOp>(
                    loc, resultType, createGroupMask.getActiveElemsPerGroup(), createGroupMask.getNumGroupsAttr(),
                    createGroupMask.getGroupSizeAttr())
                .getResult();

        if (auto constantMask = value.getDefiningOp<VMIConstantMaskOp>())
            return builder.create<VMIConstantMaskOp>(loc, resultType, constantMask.getValueAttr()).getResult();
...
struct VMIMaskGranularityAssignmentPass
    : public mlir::pto::impl::VMIMaskGranularityAssignmentBase<VMIMaskGranularityAssignmentPass> {
