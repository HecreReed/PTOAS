...
static const PostUpdateTable& getPostUpdateTable()
{
    static const PostUpdateTable table = [] {
        ...
        t["pto.pldi"] = {0, 1, StrideUnit::Alignment, 1, StrideConstraint::Constant};
        t["pto.vsts"] = {1, 2, StrideUnit::Element, 0};
        t["pto.vstus"] = {3, 1, StrideUnit::Element, 1, StrideConstraint::Dynamic, false};
        t["pto.psts"] = {1, 2, StrideUnit::Byte, 0};
        t["pto.psti"] = {1, 2, StrideUnit::Alignment, 0, StrideConstraint::Constant};
        t["pto.sprsts"] = {0, 1, StrideUnit::Byte, 0};
        t["pto.sprsti"] = {0, 1, StrideUnit::Alignment, 0, StrideConstraint::SignedI8};
...
static std::optional<int64_t> strideUnitBytes(Operation* op, StrideUnit unit, int64_t elemBytes)
{
    switch (unit) {
...
static void extractBaseAndStrideOperand(Operation* op, const PostUpdateOpInfo& info, Value& base, Value& strideOperand)
{
    base = op->getOperand(info.baseOperandIdx);
    strideOperand = info.strideOperandIdx ? op->getOperand(*info.strideOperandIdx) : Value();
}
...
static bool isDirectlyInForBody(Operation* op, scf::ForOp forOp) { return op->getParentOp() == forOp.getOperation(); }
...
static StrideExprRef makeBinary(StrideExpr::Kind kind, StrideExprRef a, StrideExprRef b)
{
    auto e = std::make_shared<StrideExpr>();
    e->kind = kind;
    e->lhs = std::move(a);
...
static bool sameAffineAtom(const StrideExprRef& a, const StrideExprRef& b)
{
    ...
    return a->castOp->getName() == b->castOp->getName() &&
           a->castOp->getOperand(0).getType() == b->castOp->getOperand(0).getType() &&
           a->castOp->getResult(0).getType() == b->castOp->getResult(0).getType() && sameAffineAtom(a->lhs, b->lhs);
}
...
static bool accumulateAffine(const StrideExprRef& e, int64_t scale, AffineForm& form)
{
    if (!e)
        return false;
    switch (e->kind) {
        case StrideExpr::Kind::Const: {
            int64_t scaled;
            return !llvm::MulOverflow(e->constant, scale, scaled) && addAffineConstant(form, scaled);
        }
        case StrideExpr::Kind::Leaf: {
            if (auto c = getConstantIntValue(e->leaf)) {
                int64_t scaled;
                return !llvm::MulOverflow(*c, scale, scaled) && addAffineConstant(form, scaled);
            }
        ...
        case StrideExpr::Kind::Cast:
            ...
            return addAffineTerm(form, e, scale);
        case StrideExpr::Kind::Add:
            return accumulateAffine(e->lhs, scale, form) && accumulateAffine(e->rhs, scale, form);
        case StrideExpr::Kind::Sub: {
            int64_t negScale;
            return !llvm::SubOverflow(int64_t{0}, scale, negScale) && accumulateAffine(e->lhs, scale, form) &&
                   accumulateAffine(e->rhs, negScale, form);
        }
        case StrideExpr::Kind::Mul: {
            if (auto lhsConst = foldConst(e->lhs)) {
                int64_t newScale;
                return !llvm::MulOverflow(scale, *lhsConst, newScale) && accumulateAffine(e->rhs, newScale, form);
            }
            if (auto rhsConst = foldConst(e->rhs)) {
                int64_t newScale;
                return !llvm::MulOverflow(scale, *rhsConst, newScale) && accumulateAffine(e->lhs, newScale, form);
            }
            return false;
        }
    }
...
static bool equalAffineForms(const AffineForm& a, const AffineForm& b)
{
    ...
    for (const AffineTerm& termA : a.terms) {
        bool found = llvm::any_of(b.terms, [&](const AffineTerm& termB) {
            return termA.coeff == termB.coeff && sameAffineAtom(termA.atom, termB.atom);
        });
        if (!found)
            return false;
    }
...
static bool isZeroAffineForm(const AffineForm& form) { return form.constant == 0 && form.terms.empty(); }
...
static std::optional<LinearDecomp> decomposeLinear(
    Value v, BlockArgument blockArg, scf::ForOp forOp, DecompCache& cache)
{
    // v == blockArg → {1, 0}
    if (v == blockArg)
    ...
    if (forOp.isDefinedOutsideOfLoop(v) || defOp->hasTrait<OpTrait::ConstantLike>())
        return record(LinearDecomp{0, makeLeaf(v)});
    ...
    if (isa<arith::AddIOp, arith::SubIOp>(defOp)) {
        ...
        return record(LinearDecomp{
            ...
            isSub ? makeSub(da->increment, db->increment) : makeAdd(da->increment, db->increment)});
    }
    ...
    if (auto mulOp = dyn_cast<arith::MulIOp>(defOp)) {
        ...
        if (!constMul)
            return record(std::nullopt);
        return record(LinearDecomp{withBA.coeff * *constMul, makeMul(withBA.increment, makeConst(*constMul))});
    }
    ...
    if (auto addPtrOp = dyn_cast<pto::AddPtrOp>(defOp)) {
        ...
        if (dp->coeff == 0)
            return record(LinearDecomp{0, makeLeaf(v)});
        return record(LinearDecomp{dp->coeff, makeAdd(dp->increment, makeLeaf(addPtrOp.getOffset()))});
    }
...
static StrideResult getIterArgIncrement(Value v, scf::ForOp forOp)
{
    ...
    while (true) {
        if (auto blockArg = dyn_cast<BlockArgument>(current)) {
            ...
            DecompCache cache;
            auto decomp = decomposeLinear(yieldOp.getOperand(idx), blockArg, forOp, cache);
        ...
        if (!defOp || forOp.isDefinedOutsideOfLoop(current) || defOp->hasTrait<OpTrait::ConstantLike>())
            return {StrideStatus::NotIterArg, nullptr};
...
static std::optional<__int128> getConstantIterArgIncrement(BlockArgument iterArg, scf::ForOp forOp, bool isUnsigned)
{
    ...
    if (auto subOp = yielded.getDefiningOp<arith::SubIOp>()) {
        ...
        if (constant) {
            ...
            if (signedConstant == -(int64_t{1} << (kCanonicalAddressWidth - 1)))
                return std::nullopt;
...
// them.
static bool canonicalAddressRecurrenceDoesNotWrap(Value input, Operation* castOp, scf::ForOp forOp)
{
    ...
    if (input == forOp.getInductionVar()) {
        ...
        if (!iterArg || iterArg.getOwner() != forOp.getBody() || iterArg.getArgNumber() == 0)
            return false;
        unsigned idx = iterArg.getArgNumber() - 1;
    ...
    __int128 initial = isUnsigned ? static_cast<__int128>(initialBits.getZExtValue()) :
    ...
    if (isUnsigned)
        return minValue >= 0 && maxValue <= static_cast<__int128>(llvm::maxUIntN(kCanonicalAddressWidth));
    return minValue >= -static_cast<__int128>(uint64_t{1} << (kCanonicalAddressWidth - 1)) &&
           maxValue <= static_cast<__int128>((uint64_t{1} << (kCanonicalAddressWidth - 1)) - 1);
}
...
static bool castPreservesLoopDelta(Operation* castOp, scf::ForOp forOp)
{
    ...
    if (castOp->getResult(0).getType().isIndex() && isa<IntegerType>(input.getType()))
    ...
    if (isa<arith::IndexCastUIOp>(castOp))
        return *lower >= 0 && llvm::isUIntN(resultWidth, static_cast<uint64_t>(*lower)) &&
...
static StrideExprRef computeDelta(Value v, scf::ForOp forOp, DeltaCache& cache)
{
    // IV: delta = step
    ...
    if (auto mulOp = dyn_cast<arith::MulIOp>(defOp)) {
        ...
        for (auto [invariant, variant] : {std::pair{rhs, lhs}, std::pair{lhs, rhs}}) {
...
static Value materializeAtLoopEntry(Value v, scf::ForOp forOp, OpBuilder& builder)
{
    // IV → lower bound
    if (v == forOp.getInductionVar())
...
static bool canScaleInitialOffset(Value strideOperand, int64_t elemBytes, int64_t unitBytes)
{
...
static Value truncateElementOffsetToI32(Value offset, Location loc, OpBuilder& builder)
{
    if (!offset.getType().isIndex())
        return offset;
    Value offsetI64 = builder.create<arith::IndexCastOp>(loc, builder.getI64Type(), offset);
    Value offsetI32 = builder.create<arith::TruncIOp>(loc, builder.getI32Type(), offsetI64);
    return builder.create<arith::IndexCastOp>(loc, builder.getIndexType(), offsetI32);
}
...
static Value normalizeAddPtrOffsetToIndex(Value offset, StrideUnit strideUnit, Location loc, OpBuilder& builder)
{
    if (offset.getType().isIndex())
        return offset;
    if (strideUnit == StrideUnit::Block)
        return builder.create<arith::IndexCastUIOp>(loc, builder.getIndexType(), offset);
    return builder.create<arith::IndexCastOp>(loc, builder.getIndexType(), offset);
}
...
static Value createInitialPtr(
    Value base, Value strideOperand, StrideUnit strideUnit, int64_t elemBytes, int64_t unitBytes, Location loc,
    OpBuilder& builder)
{
    if (!strideOperand)
        return base;
    ...
    Value scaledOffset =
        strideUnit == StrideUnit::Element ? truncateElementOffsetToI32(strideOperand, loc, builder) : strideOperand;
    if (unitBytes != elemBytes) {
        if (unitBytes % elemBytes == 0) {
            Value soIndex = strideOperand;
            if (strideOperand.getType() != builder.getIndexType())
                soIndex = builder.create<arith::IndexCastUIOp>(loc, builder.getIndexType(), strideOperand);
            Value factor = builder.create<arith::ConstantIndexOp>(loc, unitBytes / elemBytes);
            scaledOffset = builder.create<arith::MulIOp>(loc, soIndex, factor);
        } else {
            int64_t divisor = elemBytes / unitBytes;
            scaledOffset = builder.create<arith::ConstantIndexOp>(loc, *constSo / divisor);
        }
    }
    scaledOffset = normalizeAddPtrOffsetToIndex(scaledOffset, strideUnit, loc, builder);
...
static Value computeInitialPtr(
    Value base, Value strideOperand, StrideUnit strideUnit, int64_t elemBytes, int64_t unitBytes, scf::ForOp forOp,
    OpBuilder& builder)
{
    ...
    builder.setInsertionPoint(forOp);
    return createInitialPtr(baseAtEntry, soAtEntry, strideUnit, elemBytes, unitBytes, forOp.getLoc(), builder);
}
...
static StrideExprRef scaleBaseDelta(StrideExprRef deltaBase, int64_t elemBytes, int64_t unitBytes)
{
    if (unitBytes == elemBytes)
        return deltaBase;
...
static StrideExprRef combineStride(
    StrideExprRef deltaBase, StrideExprRef deltaOffset, int64_t elemBytes, int64_t unitBytes)
{
...
static bool canHoistBefore(Value v, Operation* insertPt, scf::ForOp forOp, DenseMap<Value, bool>& memo)
{
...
static Value hoistBefore(
    Value v, Operation* insertPt, scf::ForOp forOp, OpBuilder& builder, DenseMap<Value, Value>& memo)
{
    if (forOp.isDefinedOutsideOfLoop(v) || isa<BlockArgument>(v))
        return v;
    Operation* defOp = v.getDefiningOp();
...
static StrideExprRef makeAvailableAt(
    const StrideExprRef& e, Operation* insertPt, scf::ForOp forOp, OpBuilder& builder, DenseMap<Value, Value>& memo)
{
    switch (e->kind) {
        case StrideExpr::Kind::Const:
            return e;
        ...
        case StrideExpr::Kind::Cast:
            return makeCast(e->castOp, makeAvailableAt(e->lhs, insertPt, forOp, builder, memo));
        case StrideExpr::Kind::Add:
            return makeAdd(
...
static Value materializeConst(int64_t c, Type ty, Location loc, scf::ForOp forOp, ConstCache& cache, OpBuilder& builder)
{
    if (!ty)
    ...
    if (ty.isIndex())
        ...
        v = builder.create<arith::ConstantIntOp>(loc, c, ty.getIntOrFloatBitWidth());
    cache[key] = v;
    return v;
}
...
static bool constantsFitType(const StrideExprRef& e, Type wantType)
{
    switch (e->kind) {
        ...
        case StrideExpr::Kind::Sub:
        case StrideExpr::Kind::Mul:
            return constantsFitType(e->lhs, wantType) && constantsFitType(e->rhs, wantType);
    }
    return false;
}

static bool satisfiesStrideConstraint(const StrideExprRef& stride, StrideConstraint constraint)
{
    ...
    if (!constant)
        return false;
    return constraint == StrideConstraint::Constant || (*constant >= -128 && *constant <= 127);
}
...
static Value materialize(
    const StrideExprRef& e, Type wantType, Location loc, scf::ForOp forOp, ConstCache& cache, OpBuilder& builder)
{
    switch (e->kind) {
        case StrideExpr::Kind::Const:
        ...
        case StrideExpr::Kind::Leaf:
            return e->leaf;
        case StrideExpr::Kind::Cast: {
            Value in = materialize(e->lhs, e->castOp->getOperand(0).getType(), loc, forOp, cache, builder);
...
static bool canMaterializeAs(const StrideExprRef& e, Type wantType)
{
    switch (e->kind) {
        ...
        case StrideExpr::Kind::Sub:
        case StrideExpr::Kind::Mul:
            return canMaterializeAs(e->lhs, wantType) && canMaterializeAs(e->rhs, wantType);
    }
    return false;
}
...
static bool canHoistBefore(Value v, Operation* insertPt, DominanceInfo& dominance, DenseMap<Value, bool>& memo)
{
...
static Value hoistBefore(
    Value v, Operation* insertPt, DominanceInfo& dominance, OpBuilder& builder, DenseMap<Value, Value>& memo)
{
    if (dominance.dominates(v, insertPt))
        return v;
    ...
    SmallVector<Value> newOperands;
    for (Value operand : defOp->getOperands())
        newOperands.push_back(hoistBefore(operand, insertPt, dominance, builder, memo));
...
static StrideExprRef makeAvailableAt(
    const StrideExprRef& e, Operation* insertPt, DominanceInfo& dominance, OpBuilder& builder,
    DenseMap<Value, Value>& memo)
{
    switch (e->kind) {
        ...
        case StrideExpr::Kind::Cast:
            return makeCast(e->castOp, makeAvailableAt(e->lhs, insertPt, dominance, builder, memo));
        case StrideExpr::Kind::Add:
            return makeAdd(
                makeAvailableAt(e->lhs, insertPt, dominance, builder, memo),
...
static Value materializeSequential(const StrideExprRef& e, Type wantType, Location loc, OpBuilder& builder)
{
    switch (e->kind) {
        case StrideExpr::Kind::Const:
            if (wantType.isIndex())
                return builder.create<arith::ConstantIndexOp>(loc, e->constant);
            return builder.create<arith::ConstantIntOp>(loc, e->constant, wantType.getIntOrFloatBitWidth());
        case StrideExpr::Kind::Leaf:
            return e->leaf;
        case StrideExpr::Kind::Cast: {
            Type inputType = e->castOp->getOperand(0).getType();
...
static Operation* createPostUpdateOp(
    Operation* op, const PostUpdateOpInfo& info, Value base, Value stride, OpBuilder& builder)
{
...
static Operation* createNormalOp(
    Operation* op, const PostUpdateOpInfo& info, Value base, Value zeroStride, OpBuilder& builder)
{
    OperationState state(op->getLoc(), op->getName());
    for (auto [i, operand] : llvm::enumerate(op->getOperands())) {
        if (static_cast<int>(i) == info.baseOperandIdx)
...
static scf::ForOp pruneDeadLoopCarriedValues(scf::ForOp forOp, OpBuilder& builder)
{
    ...
    for (Operation& op : forOp.getBody()->without_terminator()) {
        bool hasLiveResult = llvm::any_of(op.getResults(), [&](Value result) { return liveValues.contains(result); });
...
static scf::ForOp applyPostUpdateRewrites(scf::ForOp forOp, ArrayRef<PostUpdateRewrite> rewrites, OpBuilder& builder)
{
    if (rewrites.empty())
        return nullptr;
    ...
    for (auto [rwIdx, rw] : llvm::enumerate(rewrites)) {
        ...
        Operation* newOp = createPostUpdateOp(clonedOp, *info, ptr, strideNew, builder);
...
static StrideExprRef buildSequentialExpr(Value value, SequentialExprCache& cache)
{
    if (!value)
        return makeConst(0);
    ...
    if (auto add = dyn_cast_or_null<arith::AddIOp>(defOp)) {
        result = makeAdd(buildSequentialExpr(add.getLhs(), cache), buildSequentialExpr(add.getRhs(), cache));
    } else if (auto sub = dyn_cast_or_null<arith::SubIOp>(defOp)) {
        result = makeSub(buildSequentialExpr(sub.getLhs(), cache), buildSequentialExpr(sub.getRhs(), cache));
    } else if (auto mul = dyn_cast_or_null<arith::MulIOp>(defOp)) {
        if (auto lhsConst = getConstantIntValue(mul.getLhs()))
            result = makeMul(makeConst(*lhsConst), buildSequentialExpr(mul.getRhs(), cache));
        else if (auto rhsConst = getConstantIntValue(mul.getRhs()))
            result = makeMul(buildSequentialExpr(mul.getLhs(), cache), makeConst(*rhsConst));
        else
...
static NormalizedBase normalizeSequentialBase(Value base, int64_t elemBytes, SequentialExprCache& cache)
{
    Value root = base;
...
static std::optional<SequentialStep> analyzeSequentialStep(
    const SequentialCandidate& previous, const SequentialCandidate& current)
{
    if (previous.elemBytes != current.elemBytes || previous.unitBytes != current.unitBytes)
        return std::nullopt;
    ...
    StrideExprRef step = combineStride(deltaBase, deltaStride, current.elemBytes, current.unitBytes);
    if (!step)
        return std::nullopt;
...
static bool validateSequentialRun(SequentialRun& run, DominanceInfo& dominance)
{
    if (run.candidates.size() < 3)
        return false;

    SequentialCandidate* first = run.candidates.front();
    run.strideType = first->strideOperand ? first->strideOperand.getType() : IndexType::get(first->op->getContext());
    Value initialOffsetOperand = first->info->strideIsInitialOffset ? first->strideOperand : Value();
    if (!canMaterializeAs(run.step, run.strideType) || !constantsFitType(run.step, run.strideType) ||
        !satisfiesStrideConstraint(run.step, first->info->strideConstraint) ||
        !canScaleInitialOffset(initialOffsetOperand, first->elemBytes, first->unitBytes))
        return false;

    for (SequentialCandidate* candidate : run.candidates) {
        Type candidateStrideType =
            candidate->strideOperand ? candidate->strideOperand.getType() : IndexType::get(candidate->op->getContext());
    ...
    return llvm::all_of(leaves, [&](Value leaf) { return canHoistBefore(leaf, first->op, dominance, canCache); });
}
...
static bool isRunStrideUse(OpOperand& use, const SequentialRun& run)
{
    return llvm::any_of(run.candidates, [&](SequentialCandidate* candidate) {
        return candidate->info->strideOperandIdx && use.getOwner() == candidate->op &&
...
static void collectCumulativeOffsetOps(Value value, DenseSet<Operation*>& ops)
{
    Operation* defOp = value.getDefiningOp();
    if (!isa_and_nonnull<arith::AddIOp, arith::SubIOp>(defOp) || !ops.insert(defOp).second)
        return;
    for (Value operand : defOp->getOperands())
        collectCumulativeOffsetOps(operand, ops);
}

static bool allUsesDisappearAfterRewrite(Operation* op, const DenseSet<Operation*>& deadOps, const SequentialRun& run)
{
...
static bool cumulativeOffsetChainDefinitelyDies(const SequentialRun& run, DenseSet<Operation*>& deadOps)
{
    ...
    return !deadOps.empty() &&
           llvm::all_of(deadOps, [&](Operation* op) { return allUsesDisappearAfterRewrite(op, deadOps, run); });
}

static bool collectLatePureDefinitions(
    Value value, Operation* runHead, DominanceInfo& dominance, DenseSet<Operation*>& clonedOps)
{
...
static bool isStepMaterializationCostNeutral(
    const SequentialRun& run, const DenseSet<Operation*>& deadOffsetOps, DominanceInfo& dominance)
{
    SequentialCandidate* first = run.candidates.front();
    ...
    if (atom->kind == StrideExpr::Kind::Cast) {
        ...
        atom->kind == StrideExpr::Kind::Leaf &&
        !collectLatePureDefinitions(atom->leaf, first->op, dominance, clonedOps)) {
        return false;
    }

    DenseSet<Operation*> disappearing = deadOffsetOps;
    disappearing.insert(clonedOps.begin(), clonedOps.end());
    return llvm::all_of(clonedOps, [&](Operation* op) { return allUsesDisappearAfterRewrite(op, disappearing, run); });
}
...
static bool isProfitableDirectSymbolicLeafRun(const SequentialRun& run, DominanceInfo& dominance)
{
    ...
    if (run.stepForm.constant != 0 || run.stepForm.terms.size() != 1 || run.stepForm.terms.front().coeff != 1)
        return false;

    // This class covers a direct fixed base with offsets 0, step, 2*step, ...
    // Dynamic base chains are handled independently above.
    if (!llvm::all_of(
            run.candidates, [](SequentialCandidate* candidate) { return candidate->base == candidate->rootBase; }))
        return false;
    Value firstStrideOperand = run.candidates.front()->strideOperand;
    auto firstOffset = firstStrideOperand ? getConstantIntValue(firstStrideOperand) : std::optional<int64_t>(0);
...
static bool isProfitableSequentialRun(const SequentialRun& run, DominanceInfo& dominance)
{
    return isProfitableDynamicBaseRun(run) || isProfitableDirectSymbolicLeafRun(run, dominance);
}

static void collectNestedBlocks(Operation* op, pto::VecScopeOp owner, SmallVectorImpl<Block*>& blocks)
{
    for (Region& region : op->getRegions()) {
        for (Block& block : region) {
            blocks.push_back(&block);
            for (Operation& nested : block) {
                if (auto nestedScope = dyn_cast<pto::VecScopeOp>(nested); nestedScope && nestedScope != owner)
                    continue;
...
static void processSequentialBlock(Block* block, DominanceInfo& dominance, OpBuilder& builder)
{
    ...
    for (Operation& op : *block) {
        ...
        if (!unitBytes)
            continue;
        NormalizedBase normalized = normalizeSequentialBase(base, *elemBytes, exprCache);

        auto bucketIt = llvm::find_if(buckets, [&](const SequentialBucket& bucket) {
            return bucket.opName == op.getName().getStringRef() && bucket.rootBase == normalized.root;
        });
    ...
    for (SequentialBucket& bucket : buckets) {
        ...
        while (start + 1 < candidates.size()) {
            auto firstStep = analyzeSequentialStep(candidates[start], candidates[start + 1]);
            if (!firstStep) {
                ++start;
                continue;
            }
            ...
            while (end < candidates.size()) {
                auto nextStep = analyzeSequentialStep(candidates[end - 1], candidates[end]);
            ...
            if (validateSequentialRun(run, dominance) && isProfitableSequentialRun(run, dominance)) {
    ...
    for (SequentialRun& run : runs) {
        ...
        StrideExprRef available = makeAvailableAt(run.step, first->op, dominance, builder, hoistMemo);
        builder.setInsertionPoint(first->op);
        run.strideValue = materializeSequential(available, run.strideType, first->op->getLoc(), builder);
        run.zeroStride = materializeSequential(makeConst(0), run.strideType, first->op->getLoc(), builder);
        builder.setInsertionPoint(first->op);
        Value initialOffsetOperand = first->info->strideIsInitialOffset ? first->strideOperand : Value();
        run.currentPtr = createInitialPtr(
            first->base, initialOffsetOperand, first->info->strideUnit, first->elemBytes, first->unitBytes,
    ...
    for (Operation* op : originalOps) {
        ...
        Operation* newOp = isLast ? createNormalOp(op, *info, run.currentPtr, run.zeroStride, builder) :
...
struct VPTOSoftPostUpdatePass : public pto::impl::VPTOSoftPostUpdateBase<VPTOSoftPostUpdatePass> {
    using pto::impl::VPTOSoftPostUpdateBase<VPTOSoftPostUpdatePass>::VPTOSoftPostUpdateBase;
    ...
    void runOnOperation() override
    {
        ...
        module.walk([&](pto::VecScopeOp vecscope) { processVecScope(vecscope, builder); });
    }
...
private:
    ...
    void processForOp(scf::ForOp forOp, OpBuilder& builder)
    {
        ...
        for (Operation& op : *forOp.getBody()) {
            ...
            StrideExprRef deltaOffset = strideOperand ? getStride(strideOperand, forOp, deltaCache) : makeConst(0);

            if (!deltaBase || !deltaOffset)
                continue;

            StrideExprRef total = combineStride(deltaBase, deltaOffset, *elemBytes, *unitBytes);
            if (!total)
                continue;
            ...
            if (!exprType(total, exprResultType))
                continue;
            Type strideType = strideOperand ? strideOperand.getType() : builder.getIndexType();
            ...
            bool allInvariant = llvm::all_of(leaves, [&](Value l) { return forOp.isDefinedOutsideOfLoop(l); });
            ...
            if (!allInvariant) {
                ...
                DenseMap<Value, bool> canCache;
                if (!llvm::all_of(leaves, [&](Value l) { return canHoistBefore(l, &op, forOp, canCache); }))
                    continue;
                DenseMap<Value, Value> hoistMemo;
            ...
            Value strideNew = materialize(finalExpr, strideType, op.getLoc(), forOp, constCache, builder);

            Value initialOffsetOperand = info->strideIsInitialOffset ? strideOperand : Value();
            Value initPtr =
                computeInitialPtr(base, initialOffsetOperand, info->strideUnit, *elemBytes, *unitBytes, forOp, builder);
            if (!initPtr)
                continue;

            rewrites.push_back({&op, base, strideOperand, strideNew, initPtr, *unitBytes});
        }
...
std::unique_ptr<Pass> mlir::pto::createVPTOSoftPostUpdatePass() { return std::make_unique<VPTOSoftPostUpdatePass>(); }
