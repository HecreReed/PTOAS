...
{
    return lhs && rhs && lhs.getElementCount() == rhs.getElementCount() && lhs.getElementType() == rhs.getElementType();
}
...
static void tryFoldLoadEnsures(VMILoadOp load, SmallVectorImpl<VMIEnsureLayoutOp>& maybeDeadEnsures)
{
...
static void tryFoldNestedEnsureLayout(VMIEnsureLayoutOp ensure, SmallVectorImpl<VMIEnsureLayoutOp>& maybeDeadEnsures)
{
...
static bool isFoldableStoreEnsure(VMIEnsureLayoutOp ensure)
{
    ...
    if (!isUnitContiguousLayout(resultLayout) || isUnitContiguousLayout(sourceLayout))
        return false;
...
static void tryFoldEnsureLayoutIntoOperand(OpOperand& operand, SmallVectorImpl<VMIEnsureLayoutOp>& maybeDeadEnsures)
{
...
static void tryFoldEnsureLayoutIntoMaskedStore(
    VMIMaskedStoreOp store, SmallVectorImpl<VMIEnsureLayoutOp>& maybeDeadEnsures,
...
struct VMILayoutFoldPass : public mlir::pto::impl::VMILayoutFoldBase<VMILayoutFoldPass> {
    ...
    void runOnOperation() override
    {
        ...
        module.walk([&](VMILoadOp load) { tryFoldLoadEnsures(load, maybeDeadEnsures); });

        module.walk([&](VMIEnsureLayoutOp ensure) { tryFoldNestedEnsureLayout(ensure, maybeDeadEnsures); });

        module.walk([&](Operation* op) {
            if (auto store = dyn_cast<VMIStoreOp>(op))
                tryFoldEnsureLayoutIntoOperand(store.getValueMutable(), maybeDeadEnsures);
            if (auto maskedStore = dyn_cast<VMIMaskedStoreOp>(op))
                tryFoldEnsureLayoutIntoMaskedStore(maskedStore, maybeDeadEnsures, maybeDeadMaskEnsures);
        });
...
std::unique_ptr<Pass> mlir::pto::createVMILayoutFoldPass() { return std::make_unique<VMILayoutFoldPass>(); }
