...
static StringRef stringifyUnprovenReason(pto::IterationDomainUnprovenReason reason)
{
    switch (reason) {
...
static StringRef stringifyWriteInstanceEscapeClass(pto::FusionWriteInstanceEscapeClass escapeClass)
{
    switch (escapeClass) {
...
static void appendOptionalIndex(llvm::raw_ostream& os, std::optional<unsigned> value)
{
    if (!value) {
        os << "<none>";
        return;
...
static void appendDomain(llvm::raw_ostream& os, const pto::IterationDomainInfo& info)
{
...
static DenseMap<Value, std::string> buildValueLabels(Block& block, const pto::FusionBlockAnalysis& analysis)
{
    ...
    for (const pto::FusionComputeNode& node : analysis.computeNodes) {
        for (auto [idx, output] : llvm::enumerate(node.semantics.tileOutputs))
            labels.try_emplace(output, (llvm::Twine("node") + llvm::Twine(node.id) + ".out" + llvm::Twine(idx)).str());
    }
...
static void printLocalBoundaries(llvm::raw_ostream& os, Block& block, DenseMap<Value, std::string>& valueLabels)
{
    unsigned boundaryId = 0;
    for (Operation& op : block) {
        FailureOr<pto::FusionOpSemantics> semanticsOr = pto::getFusionOpSemantics(&op);
        if (failed(semanticsOr) || semanticsOr->kind != pto::FusionOpKind::LocalBoundary)
            continue;

        os << "    local_boundary[" << boundaryId++ << "] op=" << semanticsOr->opName << " inputs=[";
...
struct PrintPreFusionAnalysisPass : public pto::impl::PrintPreFusionAnalysisBase<PrintPreFusionAnalysisPass> {
    using pto::impl::PrintPreFusionAnalysisBase<PrintPreFusionAnalysisPass>::PrintPreFusionAnalysisBase;
    ...
    void runOnOperation() override
    {
        ...
        for (auto [blockIndex, blockAnalysis] : llvm::enumerate(analysis.getResult().blocks)) {
            os << "  block[" << blockIndex << "]\n";
            DenseMap<Value, std::string> valueLabels = buildValueLabels(*blockAnalysis.block, blockAnalysis);
            printLocalBoundaries(os, *blockAnalysis.block, valueLabels);

            for (const pto::IterationDomainClass& klass : blockAnalysis.iterationDomainClasses) {
                os << "    domain_class[" << klass.id << "] domain=";
                appendDomain(os, klass.info);
                os << " proof=" << stringifyIterationProof(klass.info.proof)
                   << " reason=" << stringifyUnprovenReason(klass.info.unprovenReason) << " members=";
            ...
            for (auto [edgeIndex, edge] : llvm::enumerate(blockAnalysis.edges)) {
                os << "    edge[" << edgeIndex << "] producer=" << edge.producerNode
                   << " consumer=" << edge.consumerNode << " value=" << valueLabels.lookup(edge.value) << "\n";
            }

            for (const pto::FusionValueLiveness& live : blockAnalysis.liveness) {
                os << "    liveness value=" << valueLabels.lookup(live.value) << " producer=";
                ...
                os << " external_users=" << (live.hasExternalUsers ? "true" : "false")
                   ...
                   << " boundary_users=" << (live.hasLocalBoundaryUsers ? "true" : "false")
                   << " hard_boundary_users=" << (live.hasLocalHardBoundaryUsers ? "true" : "false") << "\n";
            }

            for (const pto::FusionWriteInstanceLiveness& writeInstance : blockAnalysis.writeInstances) {
                os << "    write_instance[" << writeInstance.id << "] value=" << valueLabels.lookup(writeInstance.value)
                   << " storage=" << valueLabels.lookup(writeInstance.storageValue) << " producer=";
                ...
                os << " escape_class=" << stringifyWriteInstanceEscapeClass(writeInstance.escapeClass)
                   << " external_users=" << (writeInstance.hasExternalUsers ? "true" : "false")
                   << " escapes_block=" << (writeInstance.escapesBlock ? "true" : "false")
                   << " boundary_users=" << (writeInstance.hasLocalBoundaryUsers ? "true" : "false")
                   << " hard_boundary_users=" << (writeInstance.hasLocalHardBoundaryUsers ? "true" : "false") << "\n";
            }
        }
