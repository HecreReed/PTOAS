...
static bool areEquivalentMaskValues(Value lhs, Value rhs) { return areEquivalentValues(lhs, rhs); }
...
static std::optional<FusionRegionStoreContext> buildFusionRegionStoreContext(pto::FusionRegionOp fusionRegion)
{
...
static bool isSupportedLoopRoot(scf::ForOp loop)
{
    if (!loop)
        return false;
    return isa<pto::FusionRegionOp, pto::VecScopeOp, pto::StrictVecScopeOp>(loop->getParentOp());
}
...
static int findTrackedStoreIndex(ArrayRef<TrackedStore> stores, Value base, ArrayRef<Value> indices, Value mask)
{
    for (int index = static_cast<int>(stores.size()) - 1; index >= 0; --index) {
        const TrackedStore& store = stores[index];
        if (areEquivalentValues(store.base, base) && areEquivalentValueRanges(store.indices, indices) &&
...
static void pruneTrackedStoresForLoadBase(SmallVectorImpl<TrackedStore>& stores, Value base)
{
    if (!base) {
        stores.clear();
        return;
    }
    llvm::erase_if(stores, [&](const TrackedStore& store) { return areEquivalentValues(store.base, base); });
}

static bool shouldElideTailStore(
    const TrackedStore& store, const FusionRegionStoreContext& context, Operation* scopeOp,
    ...
    for (OpOperand& use : canonicalBase.getUses()) {
        ...
        if (context.regionOp->isProperAncestor(owner)) {
            ...
            if (topLevelUser == localScopeOp)
                continue;
            if (localScopeOp->getBlock() == topLevelUser->getBlock() && localScopeOp->isBeforeInBlock(topLevelUser))
                return false;
            continue;
        }
        ...
        Operation* topLevelUser = getTopLevelAncestorInBlock(owner, context.parentBlock);
        if (!topLevelUser) {
...
static bool elideLoadStoreRoundTripsInLeafBody(Block& body, const FusionRegionStoreContext* context, Operation* scopeOp)
{
    ...
    for (Operation& op : body.without_terminator()) {
        if (auto load = dyn_cast<pto::VldsOp>(op)) {
            ...
            int matchIndex = findTrackedStoreIndex(trackedStores, base, loadIndices, inferredMask);
            if (matchIndex >= 0) {
        ...
        if (auto store = dyn_cast<pto::VstsOp>(op)) {
            ...
            int matchIndex = findTrackedStoreIndex(trackedStores, base, storeIndices, mask);
            if (matchIndex >= 0) {
...
struct PTOFusionLoadStoreElisionPass : public pto::impl::PTOFusionLoadStoreElisionBase<PTOFusionLoadStoreElisionPass> {
    using pto::impl::PTOFusionLoadStoreElisionBase<PTOFusionLoadStoreElisionPass>::PTOFusionLoadStoreElisionBase;
    ...
    void runOnOperation() override
    {
        ...
        func.walk([&](pto::FusionRegionOp fusionRegion) {
            std::optional<FusionRegionStoreContext> context = buildFusionRegionStoreContext(fusionRegion);
            if (!context)
                return;
        ...
        auto runElisionForLeafBody = [&](Block* leafBody, Operation* scopeOp, pto::FusionRegionOp fusionRegion) {
            ...
            changed |= elideLoadStoreRoundTripsInLeafBody(*leafBody, &it->second, scopeOp);
        };
        ...
        func.walk([&](pto::VecScopeOp vecscope) {
            if (auto fusionRegion = vecscope->getParentOfType<pto::FusionRegionOp>()) {
                if (isSupportedStraightLineBlock(vecscope.getBody().front()))
                    runElisionForLeafBody(&vecscope.getBody().front(), vecscope, fusionRegion);
            }
        });
        ...
        func.walk([&](scf::ForOp loop) {
            if (!isSupportedLoopRoot(loop))
                return;
            runElisionForLeafBody(
                getLeafLoopBody(loop), loop.getOperation(), loop->getParentOfType<pto::FusionRegionOp>());
        });
