...
static LogicalResult flattenFusionRegion(pto::FusionRegionOp fusionRegion)
{
    ...
    SmallVector<Value, 8> yieldedValues(yieldOp.getValues().begin(), yieldOp.getValues().end());
    ...
    for (auto [result, replacement] : llvm::zip(anchor->getResults(), yieldedValues))
...
struct PTOFlattenFusionRegionPass : public pto::impl::PTOFlattenFusionRegionBase<PTOFlattenFusionRegionPass> {
    using pto::impl::PTOFlattenFusionRegionBase<PTOFlattenFusionRegionPass>::PTOFlattenFusionRegionBase;
    ...
    void runOnOperation() override
    {
        ...
        func.walk<WalkOrder::PostOrder>(
            [&](pto::FusionRegionOp fusionRegion) { fusionRegions.push_back(fusionRegion); });
