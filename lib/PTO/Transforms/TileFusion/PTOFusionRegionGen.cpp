...
struct FusionBlockAnalysisIndex {
    ...
    DenseMap<unsigned, SmallVector<const pto::FusionWriteInstanceLiveness*, 2>> writeInstancesByProducerNode;
};
...
static LogicalResult collectGroupSpansInBlock(Block& block, SmallVectorImpl<GroupSpan>& spans)
{
    ...
    auto flush = [&]() -> LogicalResult {
        ...
        current.block = &block;
        auto [it, inserted] = spanIndexByGroupId.try_emplace(current.groupId, spans.size());
        if (!inserted) {
            current.members.front().op->emitError("expected one contiguous span per pto.fusion.group_id within a basic "
                                                  "block");
            return failure();
        }
...
{
    Operation* topLevel = getTopLevelAncestorInBlock(use.getOwner(), boundary->getBlock());
    if (!topLevel || topLevel == boundary)
        return false;
    return boundary->isBeforeInBlock(topLevel);
}

static bool hasReplaceableUseOutsideSpan(Value value, const DenseSet<Operation*>& spanOps, Operation* boundary)
{
...
static bool hasAnyUseOutsideSpan(Value value, const DenseSet<Operation*>& spanOps)
{
    for (OpOperand& use : value.getUses())
        if (!isNestedInSpan(use.getOwner(), spanOps))
            return true;
...
static const FusionBlockAnalysisIndex* getBlockAnalysisIndex(const PreFusionAnalysisIndex* analysisIndex, Block* block)
{
    if (!analysisIndex)
        return nullptr;
...
static const pto::FusionWriteInstanceLiveness* getProducedWriteInstance(
    const FusionBlockAnalysisIndex* blockAnalysis, Operation* op, unsigned tileOutputIndex)
{
    if (!blockAnalysis)
        return nullptr;
    ...
    auto writeIt = blockAnalysis->writeInstancesByProducerNode.find(nodeIt->second);
...
static bool writeInstanceEscapesSpan(
    const pto::FusionWriteInstanceLiveness& writeInstance, const DenseSet<unsigned>& spanNodeIds)
{
    if (writeInstance.hasExternalUsers || writeInstance.escapesBlock || writeInstance.hasLocalBoundaryUsers ||
...
static bool canSinkAllocTileDefToRegion(Value value, const GroupSpan& span, const DenseSet<Operation*>& spanOps)
{
...
static GroupSpanInterface buildGroupSpanInterface(const GroupSpan& span, const PreFusionAnalysisIndex* analysisIndex)
{
    GroupSpanInterface iface;
    DenseSet<Value> seenOutputs;
    DenseSet<Operation*> spanOps;
    Operation* boundary = span.members.front().op;
    const FusionBlockAnalysisIndex* blockAnalysis = getBlockAnalysisIndex(analysisIndex, span.block);
    ...
    for (const GroupSpanMember& member : span.members) {
        ...
        if (auto dpsIface = dyn_cast<pto::PTO_DpsInitOpInterface>(member.op)) {
            ...
            for (Value init : dpsIface.getDpsInits()) {
                ...
                if (writeInstance)
                    escapesSpan = escapesSpan && writeInstanceEscapesSpan(*writeInstance, spanNodeIds);
    ...
    DenseSet<Value> visibleValues(iface.externallyVisibleValues.begin(), iface.externallyVisibleValues.end());
...
static void replaceEscapingUsesOutsideRegion(pto::FusionRegionOp fusionRegion, ArrayRef<Value> oldValues)
{
    for (auto [oldValueRef, newValue] : llvm::zip(oldValues, fusionRegion.getOutputs())) {
...
static FailureOr<std::optional<int64_t>> getCommonSpanI64Attr(const GroupSpan& span, StringRef attrName)
{
    ...
    for (const GroupSpanMember& member : span.members) {
        ...
        if (unroll < 1) {
            member.op->emitError("expected ") << attrName << " to be a positive integer";
            return failure();
        }
        if (!commonUnroll) {
            commonUnroll = unroll;
        ...
        if (*commonUnroll != unroll) {
            member.op->emitError("inconsistent ") << attrName << " within one pto.fusion.group_id";
            return failure();
        }
    }
    if (commonUnroll && sawMissingAttr) {
        span.members.front().op->emitError("incomplete ") << attrName << " within one pto.fusion.group_id";
        return failure();
    }
    return commonUnroll;
}

static LogicalResult encapsulateGroupSpan(const GroupSpan& span, const PreFusionAnalysisIndex* analysisIndex)
{
    if (span.members.empty())
        return success();

    GroupSpanInterface iface = buildGroupSpanInterface(span, analysisIndex);
    FailureOr<std::optional<int64_t>> commonRowUnroll = getCommonSpanI64Attr(span, kFusionRowUnrollAttr);
    if (failed(commonRowUnroll))
        return failure();
    FailureOr<std::optional<int64_t>> commonColUnroll = getCommonSpanI64Attr(span, kFusionColUnrollAttr);
    ...
    auto fusionRegion = builder.create<pto::FusionRegionOp>(loc, TypeRange(outputTypes));
    fusionRegion->setAttr(kFusionGroupIdAttr, builder.getI64IntegerAttr(span.groupId));
    if (*commonRowUnroll)
        fusionRegion->setAttr(kFusionRowUnrollAttr, builder.getI64IntegerAttr(**commonRowUnroll));
    if (*commonColUnroll)
        fusionRegion->setAttr(kFusionColUnrollAttr, builder.getI64IntegerAttr(**commonColUnroll));
...
static LogicalResult processRegion(Region& region, const PreFusionAnalysisIndex* analysisIndex)
{
...
struct PTOFusionRegionGenPass : public pto::impl::PTOFusionRegionGenBase<PTOFusionRegionGenPass> {
    using pto::impl::PTOFusionRegionGenBase<PTOFusionRegionGenPass>::PTOFusionRegionGenBase;
    ...
    void runOnOperation() override
    {
        ...
        for (const pto::FusionBlockAnalysis& blockAnalysis : analysis.blocks) {
            ...
            for (const pto::FusionWriteInstanceLiveness& writeInstance : blockAnalysis.writeInstances) {
                if (!writeInstance.producerNode)
                    continue;
                index.writeInstancesByProducerNode[*writeInstance.producerNode].push_back(&writeInstance);
            }
        }
...
std::unique_ptr<Pass> mlir::pto::createPTOFusionRegionGenPass() { return std::make_unique<PTOFusionRegionGenPass>(); }
