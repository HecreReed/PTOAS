...
static SmallVector<int64_t, 4> getValidShapeVec(Type type)
{
    if (auto tileType = dyn_cast<pto::TileBufType>(type)) {
        return SmallVector<int64_t, 4>(tileType.getValidShape().begin(), tileType.getValidShape().end());
    }
    if (auto shapedType = dyn_cast<ShapedType>(type)) {
        return SmallVector<int64_t, 4>(shapedType.getShape().begin(), shapedType.getShape().end());
    }
    return {};
}
...
static bool isShapeComputableOp(Operation* op)
{
    ...
    if (opName.starts_with("arith.")) {
        return opName == "arith.minsi" || opName == "arith.maxsi" || opName == "arith.minui" ||
               opName == "arith.maxui" || opName == "arith.cmpi" || opName == "arith.select" ||
               opName == "arith.addi" || opName == "arith.subi" || opName == "arith.muli" || opName == "arith.divsi" ||
...
public:
    ...
    struct KeyInfo : public DenseMapInfo<Key> {
        static Key getEmptyKey() { return Key{DenseMapInfo<void*>::getEmptyKey(), {}, Attribute()}; }
        static Key getTombstoneKey() { return Key{DenseMapInfo<void*>::getTombstoneKey(), {}, Attribute()}; }
...
static Value canonicalizeValue(
    Value value, DenseMap<Value, Value>& canonicalByValue, StructuralSignatureMap& signatureMap)
{
    if (!value)
        return value;
    ...
    for (Value operand : op->getOperands())
        canonicalOperands.push_back(canonicalizeValue(operand, canonicalByValue, signatureMap));
...
struct ShapeValueDims {
    ...
    bool isValid() const { return rows != kInvalidShapeDim && cols != kInvalidShapeDim; }
};
...
public:
    ...
    void merge(unsigned lhs, unsigned rhs)
    {
        ...
        if (constants[lhsRoot] && constants[rhsRoot] && *constants[lhsRoot] != *constants[rhsRoot])
...
static void bindDimToValue(
    ShapeConstraintSolver& solver, DenseMap<Value, unsigned>& symbolDimByValue,
    DenseMap<Value, Value>& canonicalByValue, StructuralSignatureMap& signatureMap, unsigned dim, Value value)
{
...
static void bindExplicitValidDims(
    ShapeConstraintSolver& solver, DenseMap<Value, unsigned>& symbolDimByValue,
    DenseMap<Value, Value>& canonicalByValue, StructuralSignatureMap& signatureMap, Value value, ShapeValueDims dims)
{
    if (auto alloc = value.getDefiningOp<pto::AllocTileOp>()) {
        bindDimToValue(solver, symbolDimByValue, canonicalByValue, signatureMap, dims.rows, alloc.getValidRow());
        bindDimToValue(solver, symbolDimByValue, canonicalByValue, signatureMap, dims.cols, alloc.getValidCol());
        return;
    }
    if (auto subview = value.getDefiningOp<pto::SubViewOp>()) {
        bindDimToValue(solver, symbolDimByValue, canonicalByValue, signatureMap, dims.rows, subview.getValidRow());
        bindDimToValue(solver, symbolDimByValue, canonicalByValue, signatureMap, dims.cols, subview.getValidCol());
        return;
    }
    ...
    for (Operation* user : value.getUsers()) {
        if (auto getVS = dyn_cast<pto::GetValidShapeOp>(user)) {
            bindDimToValue(solver, symbolDimByValue, canonicalByValue, signatureMap, dims.rows, getVS.getValidRow());
            bindDimToValue(solver, symbolDimByValue, canonicalByValue, signatureMap, dims.cols, getVS.getValidCol());
            return;
        }
    }
}
...
static ShapeValueDims getValueDims(
    ...
    DenseMap<Value, unsigned>& symbolDimByValue, DenseMap<Value, Value>& canonicalByValue,
    StructuralSignatureMap& signatureMap, Value value)
{
    ...
    if (validShape.size() >= 2) {
        ...
        bindExplicitValidDims(solver, symbolDimByValue, canonicalByValue, signatureMap, value, dims);
    }
...
static void mergeRows(ShapeConstraintSolver& solver, ShapeValueDims lhs, ShapeValueDims rhs)
{
    solver.merge(lhs.rows, rhs.rows);
}

static void mergeCols(ShapeConstraintSolver& solver, ShapeValueDims lhs, ShapeValueDims rhs)
{
    solver.merge(lhs.cols, rhs.cols);
}

static void mergeShapes(ShapeConstraintSolver& solver, ShapeValueDims lhs, ShapeValueDims rhs)
{
    ...
    StructuralSignatureMap& signatureMap, ArrayRef<Value> values)
{
    ...
    ShapeValueDims anchor =
        getValueDims(solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, values.front());
    for (Value value : values.drop_front())
        mergeShapes(
            solver, anchor, getValueDims(solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, value));
}
    ...
    StructuralSignatureMap& signatureMap, const FusionComputeNode& node)
{
    ...
    switch (semantics.computeFamily) {
        case FusionComputeFamily::Elementwise: {
            ...
            mergeAllShapes(solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, values);
            return;
        }
        case FusionComputeFamily::ScalarExpand:
            mergeAllShapes(
                solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, semantics.tileOutputs);
            return;
        case FusionComputeFamily::RowBroadcastBinary: {
            if (semantics.tileOutputs.empty())
                return;
            ShapeValueDims output = getValueDims(
                solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, semantics.tileOutputs.front());
            if (!semantics.tileInputs.empty())
                mergeShapes(
                    solver,
                    getValueDims(
                        solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, semantics.tileInputs[0]),
                    output);
            if (semantics.tileInputs.size() >= 2) {
                ShapeValueDims rowInput = getValueDims(
                    solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, semantics.tileInputs[1]);
            ...
            for (Value extraOutput : ArrayRef<Value>(semantics.tileOutputs).drop_front())
                mergeShapes(
                    solver, output,
                    getValueDims(solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, extraOutput));
            return;
        }
        case FusionComputeFamily::ReduceRow:
        case FusionComputeFamily::ReduceCol: {
            mergeAllShapes(solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, semantics.tileInputs);
            if (semantics.tileInputs.empty() || semantics.tileOutputs.empty())
                return;
            ShapeValueDims input = getValueDims(
                solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, semantics.tileInputs.front());
            ShapeValueDims output = getValueDims(
                solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, semantics.tileOutputs.front());
...
static IterationDomainInfo buildIterationDomainInfo(ShapeConstraintSolver& solver, ShapeValueDims dims)
{
...
static unsigned assignShapeInferredDomainClass(
    ...
    DenseMap<std::pair<unsigned, unsigned>, unsigned>& provenClassByRoot, ShapeValueDims dims,
    const IterationDomainInfo& info, unsigned nodeId)
{
    if (info.proof == IterationDomainProof::Proven) {
        std::pair<unsigned, unsigned> key{solver.find(dims.rows), solver.find(dims.cols)};
...
static void mergeIterationDim(int64_t& mergedDim, int64_t dim, IterationDomainInfo& info)
{
...
static IterationDomainInfo inferConsensusIterationDomain(ArrayRef<Value> anchorValues)
{
    IterationDomainInfo info;
    info.unprovenReason = IterationDomainUnprovenReason::None;
    ...
    std::optional<Rank2IterationSpace> firstSpace = getRank2IterationSpace(anchorValues.front());
    if (!firstSpace)
        return info;
    ...
    if (info.unprovenReason == IterationDomainUnprovenReason::None && info.vRow != ShapedType::kDynamic &&
        info.vCol != ShapedType::kDynamic) {
        info.proof = IterationDomainProof::Proven;
        return info;
    }
...
static IterationDomainInfo inferIterationDomainInfo(const FusionOpSemantics& semantics)
{
    switch (semantics.computeFamily) {
        case FusionComputeFamily::Elementwise: {
...
static unsigned assignStaticIterationDomainClass(
    SmallVectorImpl<IterationDomainClass>& classes, DenseMap<std::pair<int64_t, int64_t>, unsigned>& provenClassByKey,
    const IterationDomainInfo& info, unsigned nodeId)
{
    if (info.proof == IterationDomainProof::Proven) {
...
static LogicalResult inferStaticIterationDomain(FusionBlockAnalysis& analysis)
{
    ...
    for (FusionComputeNode& node : analysis.computeNodes) {
        ...
        node.iterationDomainClass =
            assignStaticIterationDomainClass(analysis.iterationDomainClasses, provenClassByKey, domainInfo, node.id);
    }
    return success();
}
...
static LogicalResult inferDynamicIterationDomain(FusionBlockAnalysis& analysis)
{
    ...
    for (const FusionComputeNode& node : analysis.computeNodes) {
        for (Value input : node.semantics.tileInputs)
            (void)getValueDims(solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, input);
        for (Value output : node.semantics.tileOutputs)
            (void)getValueDims(solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, output);
    }

    for (const FusionComputeNode& node : analysis.computeNodes)
        applyShapeConstraintsForNode(solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, node);
    ...
    for (FusionComputeNode& node : analysis.computeNodes) {
        ShapeValueDims domainDims =
            getIterationDomainDimsForNode(solver, dimsByValue, symbolDimByValue, canonicalByValue, signatureMap, node);
        ...
        node.iterationDomainClass = assignShapeInferredDomainClass(
            solver, analysis.iterationDomainClasses, provenClassByRoot, domainDims, info, node.id);
    }
    return success();
}
...
static FusionWriteInstanceEscapeClass classifyEscapeClass(const FusionWriteInstanceLiveness& live)
{
    if (live.hasExternalUsers || live.escapesBlock || live.hasLocalHardBoundaryUsers) {
...
static Value getWriteInstanceStorageValue(Operation* op, unsigned outputIndex, Value output)
{
...
static unsigned getOrCreateLivenessSlot(
    DenseMap<Value, unsigned>& slotByValue, SmallVectorImpl<MutableLiveness>& slots, Value value)
{
...
static void recordLastLocalConsumer(std::optional<unsigned>& lastLocalConsumer, unsigned consumerId)
{
...
static void finalizeBlockLiveness(
    Block& block, DenseMap<Operation*, FusionOpKind>& kindByOp, DenseMap<Operation*, unsigned>& computeNodeByOp,
...
static std::optional<unsigned> findReachingWriteInstance(
    ArrayRef<unsigned> writeInstanceIds, ArrayRef<MutableWriteInstance> mutableWriteInstances,
    std::optional<unsigned> userBlockOrder)
{
    ...
    for (unsigned writeInstanceId : llvm::reverse(writeInstanceIds)) {
        if (mutableWriteInstances[writeInstanceId].producerBlockOrder < *userBlockOrder)
...
static void finalizeWriteInstances(
    Block& block, DenseMap<Operation*, FusionOpKind>& kindByOp, DenseMap<Operation*, unsigned>& computeNodeByOp,
    DenseMap<Operation*, unsigned>& blockOrderByOp, ArrayRef<MutableLiveness> mutableLiveness,
    ...
    for (const MutableLiveness& storageState : mutableLiveness) {
        ...
        for (OpOperand& use : storageState.live.value.getUses()) {
            ...
            std::optional<unsigned> writeInstanceId =
                findReachingWriteInstance(storageState.live.writeInstances, mutableWriteInstances, userBlockOrder);
            if (!writeInstanceId)
                continue;

            FusionWriteInstanceLiveness& writeLive = mutableWriteInstances[*writeInstanceId].live;
...
static FailureOr<FusionBlockAnalysis> analyzeBlockDFG(Block& block)
{
    ...
    for (Operation& op : block) {
        ...
        for (auto [outputIdx, output] : llvm::enumerate(node.semantics.tileOutputs)) {
            ...
            unsigned liveSlot = getOrCreateLivenessSlot(livenessSlotByValue, mutableLiveness, output);
            ...
            writeInstance.live.storageValue = getWriteInstanceStorageValue(&op, outputIdx, output);
            writeInstance.live.producerNode = node.id;
            writeInstance.producerBlockOrder = blockOrder;
            mutableLiveness[liveSlot].live.writeInstances.push_back(writeInstance.live.id);
            mutableWriteInstances.push_back(std::move(writeInstance));
        }

        for (Value input : node.semantics.tileInputs) {
            unsigned liveSlot = getOrCreateLivenessSlot(livenessSlotByValue, mutableLiveness, input);
            appendUniqueNode(mutableLiveness[liveSlot].live.consumerNodes, node.id);
            recordLastLocalConsumer(mutableLiveness[liveSlot].live.lastLocalConsumer, node.id);
    ...
    finalizeWriteInstances(block, kindByOp, computeNodeByOp, blockOrderByOp, mutableLiveness, mutableWriteInstances);
...
static LogicalResult analyzeRegionDFG(Region& region, SmallVectorImpl<FusionBlockAnalysis>& blocks)
{
...
FailureOr<PreFusionAnalysisResult> buildPreFusionAnalysis(func::FuncOp func, bool enableShapeInference)
{
