...
static constexpr llvm::StringLiteral kRowUnrollFactorAttr = "pto.fusion.row_unroll_factor";
static constexpr llvm::StringLiteral kColUnrollFactorAttr = "pto.fusion.col_unroll_factor";

namespace {

/// Read an i64 unroll factor from `region`; return the value only if it is
/// present and strictly greater than 1, else 0.
static int64_t getEffectiveFactor(pto::FusionRegionOp region, llvm::StringRef attrName)
{
...
static LogicalResult tryUnrollLeafForOp(scf::ForOp forOp)
{
    ...
    if (!region) {
        LLVM_DEBUG(
            llvm::dbgs() << "PTOUnrollAfterLoopFusion: skip non-fusion "
    ...
    if (hasChildForOp(forOp)) {
        LLVM_DEBUG(
            llvm::dbgs() << "PTOUnrollAfterLoopFusion: skip non-leaf "
                         << "(outer) scf.for at " << forOp.getLoc() << " -- unrolling outer breaks LoadStoreElision;"
    ...
    if (colF > 1) {
        ...
        LLVM_DEBUG(
            llvm::dbgs() << "PTOUnrollAfterLoopFusion: skip no factor>1 "
                         << "scf.for at " << forOp.getLoc() << " row_f=" << rowF << " col_f=" << colF << "\n");
        return failure();
    }
    ...
    if (!trip) {
        LLVM_DEBUG(
            llvm::dbgs() << "PTOUnrollAfterLoopFusion: skip non-constant "
                         << "trip scf.for at " << forOp.getLoc() << " factor(from " << src << ")=" << factor << "\n");
        return failure();
    }
    if (*trip % factor != 0) {
        LLVM_DEBUG(
            llvm::dbgs() << "PTOUnrollAfterLoopFusion: skip indivisible "
                         << "trip scf.for at " << forOp.getLoc() << " trip=" << *trip << " factor=" << factor
                         << "(from " << src << ")\n");
        return failure();
    }

    LLVM_DEBUG(
        llvm::dbgs() << "PTOUnrollAfterLoopFusion: unroll scf.for at " << forOp.getLoc() << " factor=" << factor
                     << "(from " << src << ") trip=" << *trip << " row_f=" << rowF << " col_f=" << colF << "\n");
    ...
    if (failed(unrolled)) {
        LLVM_DEBUG(
            llvm::dbgs() << "  loopUnrollByFactor failed "
                         << "(iter_args live-out?) at " << forOp.getLoc() << "\n");
        return failure();
    }
    (void)unrolled; // mainLoopOp/epilogueLoopOp unused: divisibility => no tail.

    llvm::StringRef consumedAttr = src == "col" ? kColUnrollFactorAttr : kRowUnrollFactorAttr;
    region->setAttr(consumedAttr, Builder(region->getContext()).getI64IntegerAttr(/*value=*/1));
    return success();
}

struct PTOUnrollAfterLoopFusion : public pto::impl::PTOUnrollAfterLoopFusionBase<PTOUnrollAfterLoopFusion> {
    using pto::impl::PTOUnrollAfterLoopFusionBase<PTOUnrollAfterLoopFusion>::PTOUnrollAfterLoopFusionBase;
