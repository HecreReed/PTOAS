...
static Value mapValueOrSelf(Value value, IRMapping& mapping) { return mapping.lookupOrDefault(value); }
...
static bool sameForHeader(scf::ForOp lhs, scf::ForOp rhs)
{
    return areEquivalentValues(lhs.getLowerBound(), rhs.getLowerBound()) &&
           ...
           areEquivalentValues(lhs.getStep(), rhs.getStep()) && lhs->getAttrs() == rhs->getAttrs();
}

static bool isPureNoRegionOp(Operation* op) { return op->getNumRegions() == 0 && isMemoryEffectFree(op); }
...
static bool isSupportedPreludeOp(Operation* op) { return isPureNoRegionOp(op) || isMovableMemoryPreludeOp(op); }
...
static bool areEquivalentOperations(Operation* lhs, Operation* rhs)
{
    ...
    for (auto [lhsOperand, rhsOperand] : llvm::zip(lhs->getOperands(), rhs->getOperands())) {
...
static Value traceAliasRootOneStep(Value value)
{
    if (auto arg = dyn_cast<BlockArgument>(value)) {
        ...
        if (auto forOp = dyn_cast_or_null<scf::ForOp>(parentOp)) {
            if (arg.getArgNumber() > 0 && forOp.getInitArgs().size() >= arg.getArgNumber())
...
static LogicalResult collectAliasRelevantRoots(Operation* op, SmallVectorImpl<Value>& roots)
{
...
{
    return llvm::any_of(roots, [&](Value root) { return areEquivalentValues(root, candidate); });
}
...
static bool canMoveAcrossStages(Operation* movableOp, ArrayRef<StageInfo> crossStages, llvm::raw_ostream* debugOS)
{
    SmallVector<Value, 4> movableRoots;
    if (failed(collectAliasRelevantRoots(movableOp, movableRoots))) {
        if (debugOS)
            *debugOS << "[op-fusion] reject movable op " << movableOp->getName() << " at " << movableOp->getLoc()
    ...
    for (const StageInfo& crossStage : crossStages) {
        for (Operation* op : crossStage.leafOps) {
            ...
            if (failed(collectAliasRelevantRoots(op, opRoots))) {
                if (debugOS)
                    *debugOS << "[op-fusion] reject movable op " << movableOp->getName() << " at "
                             << movableOp->getLoc() << ": crossed effects of " << op->getName()
            ...
            for (Value movableRoot : movableRoots) {
                if (containsEquivalentRoot(opRoots, movableRoot)) {
                    if (debugOS)
                        *debugOS << "[op-fusion] reject movable op " << movableOp->getName() << " at "
                                 << movableOp->getLoc() << ": touched root may alias a crossed stage memory op\n";
                    return false;
                }
            }
        }
            ...
            for (Operation* op : level.epilogueOps) {
                ...
                for (Value movableRoot : movableRoots) {
                    if (containsEquivalentRoot(opRoots, movableRoot)) {
                        if (debugOS)
                            *debugOS << "[op-fusion] reject movable op " << movableOp->getName() << " at "
                                     << movableOp->getLoc() << ": touched root may alias a crossed stage epilogue op\n";
                        return false;
                    }
                }
            }
...
static bool arePreludeReordersLegal(ArrayRef<StageInfo> stages, llvm::raw_ostream* debugOS)
{
        ...
        for (const StageInfo& priorStage : priorStages) {
            for (const LoopLevelInfo& priorLevel : priorStage.levels) {
                for (Operation* epilogueOp : priorLevel.epilogueOps) {
                    if (!canMoveAcrossStages(epilogueOp, ArrayRef<StageInfo>(&stages[stageIndex], 1), debugOS))
                        return false;
                }
            }
        }
...
static SmallVector<StageInfo, 8> collectStageRunFrom(scf::ForOp firstLoop, llvm::raw_ostream* debugOS)
{
...
static void cloneOpAndMapResults(OpBuilder& builder, Operation* op, IRMapping& mapping)
{
...
static void appendMappedValues(ValueRange values, IRMapping& mapping, SmallVectorImpl<Value>& mappedValues)
{
    for (Value value : values)
        mappedValues.push_back(mapValueOrSelf(value, mapping));
}

static scf::ForOp buildFusedLoopNestAtLevel(
    OpBuilder& builder, MutableArrayRef<StageInfo> stages, MutableArrayRef<IRMapping> mappings, unsigned levelIndex)
{
    ...
    for (auto [stageIndex, stage] : llvm::enumerate(stages))
        appendMappedValues(
            ValueRange(stage.levels[levelIndex].loop.getInitArgs()), mappings[stageIndex], fusedInitArgs);

    auto fusedLoop = builder.create<scf::ForOp>(
        firstLoop.getLoc(), mapValueOrSelf(firstLoop.getLowerBound(), mappings.front()),
    ...
    for (auto [stageIndex, stage] : llvm::enumerate(stages)) {
        ...
        mappings[stageIndex].map(originalLoop.getInductionVar(), fusedLoop.getInductionVar());
        for (auto [argIndex, originalArg] : llvm::enumerate(originalLoop.getRegionIterArgs()))
            mappings[stageIndex].map(originalArg, fusedLoop.getRegionIterArgs()[iterArgOffset + argIndex]);
    ...
    if (levelIndex + 1 < stages.front().getDepth()) {
        (void)buildFusedLoopNestAtLevel(bodyBuilder, stages, mappings, levelIndex + 1);
    } else {
        for (auto [stageIndex, stage] : llvm::enumerate(stages))
    ...
    for (auto [stageIndex, stage] : llvm::enumerate(stages)) {
        auto originalYield = cast<scf::YieldOp>(stage.levels[levelIndex].loop.getBody()->getTerminator());
        appendMappedValues(ValueRange(originalYield.getOperands()), mappings[stageIndex], fusedYieldOperands);
    }

    Block* fusedBody = fusedLoop.getBody();
    Operation* fusedTerminator = nullptr;
    if (!fusedBody->empty() && fusedBody->back().hasTrait<OpTrait::IsTerminator>())
    ...
    for (auto [stageIndex, stage] : llvm::enumerate(stages)) {
        ...
        for (Value originalResult : originalLoop.getResults())
            mappings[stageIndex].map(originalResult, fusedLoop.getResults()[resultOffset++]);
    }

    return fusedLoop;
}

static bool fuseStageRun(SmallVectorImpl<StageInfo>& stages, llvm::raw_ostream* debugOS)
{
    if (stages.size() < 2) {
        if (debugOS)
    ...
    auto fusedOuterLoop = buildFusedLoopNestAtLevel(blockBuilder, stages, stageMappings, 0);
...
static bool fuseStageRunsInBlock(Block& block, llvm::raw_ostream* debugOS)
{
    ...
    while (localChange) {
        ...
        for (Operation& op : block) {
            ...
            SmallVector<StageInfo, 8> stages = collectStageRunFrom(firstLoop, debugOS);
...
struct PTOLowLevelLoopFusionPass : public pto::impl::PTOLowLevelLoopFusionBase<PTOLowLevelLoopFusionPass> {
    using pto::impl::PTOLowLevelLoopFusionBase<PTOLowLevelLoopFusionPass>::PTOLowLevelLoopFusionBase;

    void runOnOperation() override
    {
        ModuleOp module = getOperation();
        const bool traceEnabled = debug || (std::getenv("PTO_LL_LOOP_FUSION_TRACE") != nullptr);
        ...
        if (traceEnabled) {
            llvm::errs() << "[op-fusion] low-level loop fusion changed " << fusedFuncs << " function(s)\n";
        }
    }
};
...
std::unique_ptr<Pass> mlir::pto::createPTOLowLevelLoopFusionPass(const PTOLowLevelLoopFusionOptions& options)
{
