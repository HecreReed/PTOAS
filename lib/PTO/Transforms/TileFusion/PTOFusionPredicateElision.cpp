...
struct ValueEquivalenceContext {
    ...
    llvm::DenseMap<Value, llvm::SmallDenseMap<Value, EquivalenceState, 4>> states;
};
...
static bool areEquivalentValues(Value lhs, Value rhs, ValueEquivalenceContext& context);
...
static bool areSameValuePair(Value lhs, Value rhs, Value expectedLhs, Value expectedRhs)
{
...
static std::optional<EquivalenceState> lookupEquivalenceState(ValueEquivalenceContext& context, Value lhs, Value rhs)
{
    normalizeValuePair(lhs, rhs);
...
static void setEquivalenceState(ValueEquivalenceContext& context, Value lhs, Value rhs, EquivalenceState state)
{
    normalizeValuePair(lhs, rhs);
...
static std::optional<PltCandidate> buildPltCandidate(Operation* op)
{
    if (auto plt = dyn_cast<pto::PltB8Op>(op)) {
        return PltCandidate{op, plt.getScalar(), plt.getMask(), plt.getScalarOut(), 8, {}};
    }
    if (auto plt = dyn_cast<pto::PltB16Op>(op)) {
        return PltCandidate{op, plt.getScalar(), plt.getMask(), plt.getScalarOut(), 16, {}};
    }
    if (auto plt = dyn_cast<pto::PltB32Op>(op)) {
        return PltCandidate{op, plt.getScalar(), plt.getMask(), plt.getScalarOut(), 32, {}};
    }
    return std::nullopt;
}
...
static std::optional<ForIterArgInfo> getForIterArgInfo(Value value)
{
    ...
    auto forOp = dyn_cast_or_null<scf::ForOp>(arg.getParentRegion()->getParentOp());
...
static bool areEquivalentLoopCarriedValues(Value lhs, Value rhs, ValueEquivalenceContext& context)
{
    ...
    if (lhsInfo->forOp.getRegionIterArgs().size() != lhsInfo->forOp.getInitArgs().size())
        return false;
    if (lhsInfo->forOp.getRegionIterArgs().size() != lhsInfo->forOp.getYieldedValues().size())
        return false;

    ValueRange initArgs = lhsInfo->forOp.getInitArgs();
    if (!areEquivalentValues(initArgs[lhsInfo->iterArgIndex], initArgs[rhsInfo->iterArgIndex], context))
        return false;

    ValueRange yieldedValues = lhsInfo->forOp.getYieldedValues();
    std::optional<PltScalarOutInfo> lhsYieldInfo = getPltScalarOutInfo(yieldedValues[lhsInfo->iterArgIndex]);
    std::optional<PltScalarOutInfo> rhsYieldInfo = getPltScalarOutInfo(yieldedValues[rhsInfo->iterArgIndex]);
    ...
    return areEquivalentValues(lhsYieldInfo->scalar, rhsYieldInfo->scalar, context);
}

static bool areEquivalentOperations(Operation* lhs, Operation* rhs, ValueEquivalenceContext& context)
{
    if (!lhs || !rhs)
        return false;
...
static bool areEquivalentValues(Value lhs, Value rhs, ValueEquivalenceContext& context)
{
    ...
    if (std::optional<EquivalenceState> state = lookupEquivalenceState(context, lhs, rhs)) {
    ...
    if (lhsArg || rhsArg) {
        bool equivalent = lhsArg && rhsArg &&
                          ((lhsArg.getOwner() == rhsArg.getOwner() && lhsArg.getArgNumber() == rhsArg.getArgNumber()) ||
                           areEquivalentLoopCarriedValues(lhs, rhs, context));
        setEquivalenceState(
            context, lhs, rhs, equivalent ? EquivalenceState::Equivalent : EquivalenceState::NotEquivalent);
        return equivalent;
    }

    bool equivalent = areEquivalentOperations(lhs.getDefiningOp(), rhs.getDefiningOp(), context);
    setEquivalenceState(context, lhs, rhs, equivalent ? EquivalenceState::Equivalent : EquivalenceState::NotEquivalent);
    return equivalent;
}

static void populateDominatingCandidateIndices(MutableArrayRef<PltCandidate> candidates, DominanceInfo& dominanceInfo)
{
    for (unsigned current = 0; current < candidates.size(); ++current) {
        for (unsigned previous = 0; previous < current; ++previous) {
            ...
            if (!dominanceInfo.properlyDominates(candidates[previous].op, candidates[current].op))
                continue;
...
static FusionRegionPredicateContext buildFusionRegionPredicateContext(
    pto::FusionRegionOp fusionRegion, DominanceInfo& dominanceInfo)
{
...
static std::optional<unsigned> findEquivalentDominatingCandidate(
    FusionRegionPredicateContext& context, ValueEquivalenceContext& valueContext, unsigned currentIndex,
    const llvm::DenseSet<unsigned>& erased)
{
    ...
    for (unsigned previousIndex : current.dominatingCandidates) {
        ...
        if (areEquivalentValues(getCurrentScalarOperand(previous), currentScalar, valueContext))
            return previousIndex;
    }
    return std::nullopt;
}

static bool elideEquivalentPltCandidates(FusionRegionPredicateContext& context)
{
    bool changed = false;
    ...
    for (unsigned currentIndex = 0; currentIndex < context.pltCandidates.size(); ++currentIndex) {
        if (erased.contains(currentIndex))
            continue;

        std::optional<unsigned> previousIndex =
            findEquivalentDominatingCandidate(context, valueContext, currentIndex, erased);
        if (!previousIndex)
            continue;
...
struct PTOFusionPredicateElisionPass : public pto::impl::PTOFusionPredicateElisionBase<PTOFusionPredicateElisionPass> {
    using pto::impl::PTOFusionPredicateElisionBase<PTOFusionPredicateElisionPass>::PTOFusionPredicateElisionBase;
    ...
    void runOnOperation() override
    {
        ...
        func.walk([&](pto::FusionRegionOp fusionRegion) {
            FusionRegionPredicateContext context = buildFusionRegionPredicateContext(fusionRegion, dominanceInfo);
