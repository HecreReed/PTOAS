...
static std::optional<BinaryVRegOperands> getSinkableCompareOperands(Operation* op)
{
...
static std::optional<SelectOperands> getSinkableSelectOperands(Operation* op)
{
    if (auto select = dyn_cast<VMISelectOp>(op))
        return SelectOperands{&select.getMaskMutable(), &select.getTrueValueMutable(), &select.getFalseValueMutable()};
    return std::nullopt;
}

static std::optional<TernaryVRegOperands> getSinkableTernaryOperands(Operation* op)
{
    if (auto fma = dyn_cast<VMIFmaOp>(op))
        return TernaryVRegOperands{&fma.getLhsMutable(), &fma.getRhsMutable(), &fma.getAccMutable()};
    return std::nullopt;
}
...
static std::optional<BinaryMaskOperands> getSinkableBinaryMaskOperands(Operation* op)
{
    if (auto maskAnd = dyn_cast<VMIMaskAndOp>(op))
        return BinaryMaskOperands{&maskAnd.getLhsMutable(), &maskAnd.getRhsMutable()};
    ...
    if (auto maskXor = dyn_cast<VMIMaskXOrOp>(op))
        return BinaryMaskOperands{&maskXor.getLhsMutable(), &maskXor.getRhsMutable()};
    return std::nullopt;
}

static std::optional<UnaryMaskOperand> getSinkableUnaryMaskOperand(Operation* op)
{
...
static bool isSameMaterialization(VMIEnsureLayoutOp ensure, VMIVRegType resultType)
{
...
static bool isSameMaterialization(VMIEnsureLayoutOp lhsEnsure, VMIEnsureLayoutOp rhsEnsure, VMIVRegType resultType)
{
    ...
    return lhsSourceType == rhsSourceType && lhsResultType == rhsResultType && lhsResultType == resultType &&
           lhsSourceType != resultType;
}

static bool isSameMaterialization(
    VMIEnsureLayoutOp lhsEnsure, VMIEnsureLayoutOp rhsEnsure, VMIEnsureLayoutOp accEnsure, VMIVRegType resultType)
{
    ...
    if (!lhsSourceType || !rhsSourceType || !accSourceType || !lhsResultType || !rhsResultType || !accResultType)
        return false;

    return lhsSourceType == rhsSourceType && lhsSourceType == accSourceType && lhsResultType == rhsResultType &&
           lhsResultType == accResultType && lhsResultType == resultType && lhsSourceType != resultType;
}

static bool hasEnsureLayoutSupport(VMIVRegType sourceType, VMIVRegType resultType)
{
...
template <typename EnsureOp>
static bool isSameMaskMaterialization(EnsureOp lhsEnsure, EnsureOp rhsEnsure, VMIMaskType resultType)
{
    if (!lhsEnsure || !rhsEnsure || !resultType)
        return false;
...
static bool hasEnsureMaskSupport(VMIEnsureMaskLayoutOp, VMIMaskType sourceType, VMIMaskType resultType)
{
    VMILayoutSupport supports;
    return succeeded(supports.getEnsureMaskLayoutFact(sourceType, resultType));
}

static bool hasEnsureMaskSupport(VMIEnsureMaskGranularityOp, VMIMaskType sourceType, VMIMaskType resultType)
{
    return sourceType.getElementCount() == resultType.getElementCount() &&
           sourceType.getLayoutAttr() == resultType.getLayoutAttr() && !sourceType.isPred() && !resultType.isPred();
}
...
static bool trySinkBinaryMaterialization(Operation* op)
{
    ...
    auto resultEnsure = builder.create<VMIEnsureLayoutOp>(op->getLoc(), resultType, newOp->getResult(0));
...
static bool trySinkSelectMaterialization(Operation* op)
{
    ...
    auto maskEnsure = operands->mask->get().getDefiningOp<VMIEnsureMaskLayoutOp>();
    auto trueEnsure = operands->trueValue->get().getDefiningOp<VMIEnsureLayoutOp>();
    auto falseEnsure = operands->falseValue->get().getDefiningOp<VMIEnsureLayoutOp>();
    if (!maskEnsure || !trueEnsure || !falseEnsure)
        return false;

    auto trueSourceType = dyn_cast<VMIVRegType>(trueEnsure.getSource().getType());
    auto falseSourceType = dyn_cast<VMIVRegType>(falseEnsure.getSource().getType());
    auto trueResultType = dyn_cast<VMIVRegType>(trueEnsure.getResult().getType());
    auto falseResultType = dyn_cast<VMIVRegType>(falseEnsure.getResult().getType());
    ...
    if (!trueSourceType || !falseSourceType || !trueResultType || !falseResultType || !maskSourceType ||
        !maskResultType)
        return false;

    if (trueSourceType != falseSourceType || trueResultType != falseResultType || trueResultType != resultType ||
    ...
    OpBuilder builder(op);
    OperationState state(op->getLoc(), op->getName());
    state.addOperands({maskEnsure.getSource(), trueEnsure.getSource(), falseEnsure.getSource()});
...
static bool trySinkCompareMaterialization(Operation* op)
{
    ...
    if (!lhsSourceType || !rhsSourceType || !lhsResultType || !rhsResultType)
        return false;
    if (lhsSourceType != rhsSourceType || lhsResultType != rhsResultType || lhsSourceType == lhsResultType)
        return false;
    ...
    auto sourceMaskType = VMIMaskType::get(
        op->getContext(), resultMaskType.getElementCount(), resultMaskType.getGranularity(),
        lhsSourceType.getLayoutAttr());
    VMILayoutSupport supports;
    ...
    builder.setInsertionPointAfter(newOp);
    auto resultEnsure = builder.create<VMIEnsureMaskLayoutOp>(op->getLoc(), resultMaskType, newOp->getResult(0));
    op->getResult(0).replaceAllUsesWith(resultEnsure.getResult());
    op->erase();
...
static bool trySinkTernaryMaterialization(Operation* op)
{
    ...
    OpBuilder builder(op);
    OperationState state(op->getLoc(), op->getName());
    state.addOperands({lhsEnsure.getSource(), rhsEnsure.getSource(), accEnsure.getSource()});
    state.addTypes(sourceType);
    state.addAttributes(op->getAttrs());
    Operation* newOp = builder.create(state);
    ...
    if (rhsEnsure != lhsEnsure && rhsEnsure->use_empty())
        rhsEnsure.erase();
    if (accEnsure != lhsEnsure && accEnsure != rhsEnsure && accEnsure->use_empty())
        accEnsure.erase();
    return true;
}
...
static bool trySinkBinaryMaskMaterialization(Operation* op)
{
    ...
    builder.setInsertionPointAfter(newOp);
    auto resultEnsure = builder.create<EnsureOp>(op->getLoc(), resultType, newOp->getResult(0));
    op->getResult(0).replaceAllUsesWith(resultEnsure.getResult());
    op->erase();
...
struct VMILayoutSinkMaterializationPass
    : public mlir::pto::impl::VMILayoutSinkMaterializationBase<VMILayoutSinkMaterializationPass> {
    ...
    void runOnOperation() override
    {
        ...
        module.walk([&](Operation* op) {
            if (getSinkableBinaryOperands(op) || getSinkableCompareOperands(op) || getSinkableSelectOperands(op) ||
                getSinkableTernaryOperands(op) || getSinkableUnaryOperand(op) || getSinkableBinaryMaskOperands(op) ||
