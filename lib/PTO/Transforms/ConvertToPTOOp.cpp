...
std::optional<Value> getLeftPadNum(PatternRewriter& rewriter, std::optional<memref::AllocOp> maybeAlloc)
{
    ...
    for (auto* user : maybeAlloc.value()->getUsers()) {
        if (auto subviewOp = llvm::dyn_cast<memref::SubViewOp>(user)) {
            ...
            if (!offsetValue) {
                offsetValue =
                    rewriter.create<arith::ConstantIndexOp>(subviewOp->getLoc(), getConstantIntValue(offset).value());
            }
            return offsetValue;
        }
    }
...
std::pair<std::optional<Operation*>, std::optional<Value>> getInitInfo(Operation* op, pto::TLoadOp loadOp)
{
    (void)op;
    (void)loadOp;
...
std::pair<std::optional<Operation*>, std::optional<Value>> getUniqueInitInfo(
    PatternRewriter& rewriter, std::optional<memref::AllocOp> maybeAlloc, pto::TLoadOp loadOp)
{
    if (!maybeAlloc.has_value())
        return {std::nullopt, std::nullopt};
