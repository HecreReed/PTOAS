...
static bool isA5ScaleTileTMov(pto::TMovOp op)
{
    ...
    return srcAS && dstAS && *srcAS == pto::AddressSpace::MAT && *dstAS == pto::AddressSpace::SCALING;
}

static bool hasInterveningUsesOfDst(Operation* start, Operation* end, Value dst)
{
    for (Operation* cursor = start->getNextNode(); cursor && cursor != end; cursor = cursor->getNextNode()) {
    ...
    void runOnOperation() override
    {
        ...
        for (pto::TMovOp op : riskyOps) {
            ...
            if (failed(srcRowTy) || failed(dstRowTy)) {
                op.emitOpError("cannot normalize A5 vec->vec col_major TMOV: requires static 2D "
