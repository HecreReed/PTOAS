...
constexpr llvm::StringLiteral kTileOpValidShapeReadAttr = "__pto.tileop_valid_shape_abi";

using ValidShapeRequirements = DenseMap<Operation*, SmallVector<unsigned, 2>>;
using ExpandedValidShapeArguments = DenseMap<Operation*, DenseMap<unsigned, std::pair<Value, Value>>>;
...
static bool isTileOrScalarType(Type type) { return isa<TileBufType>(type) || isScalarType(type); }

static bool isMemoryReferenceType(Type type)
{
    return isa<TileBufType, PtrType, MemRefType, TensorViewType, PartitionTensorViewType>(type);
}

static bool isDirectSimtOperation(Operation* op) { return isa<SimtOpInterface>(op); }

static bool isForbiddenPipeOperation(Operation* op)
{
    return isa<
        RecordEventOp, WaitEventOp, BarrierSyncOp, AicInitializePipeOp, AivInitializePipeOp, InitializeL2G2LPipeOp,
        InitializeL2LPipeOp, DeclareEventIdArrayOp, EventIdArrayGetOp, EventIdArraySetOp, SetFlagOp, WaitFlagOp,
        SetFlagDynOp, WaitFlagDynOp, GetBufOp, RlsBufOp, SyncSetOp, SyncWaitOp, BarrierOp, FenceBarrierAllOp, TSyncOp,
        SyncAllOp, DsbOp>(op);
}

static bool isMteDataMovementOperation(Operation* op) { return isa<MteOpInterface>(op); }

static bool isForbiddenTileOperation(Operation* op)
{
    return isa<TileOpInterface>(op) ||
           isa<AllocTileOp, AllocMultiTileOp, ReserveBufferOp, TPrefetchAsyncOp, TAssignOp, TensorViewAddrOp>(op);
}

static std::optional<unsigned> traceToFunctionArgument(Value value, func::FuncOp function)
{
...
static void applyEffectToAllTileArguments(func::FuncOp function, uint8_t effect, SmallVectorImpl<uint8_t>& effects)
{
...
static SmallVector<uint8_t> collectDirectArgumentEffects(func::FuncOp function)
{
    ...
    function.walk([&](Operation* op) {
        ...
        SmallVector<SideEffects::EffectInstance<MemoryEffects::Effect>, 8> instances;
        ...
        for (const auto& instance : instances) {
            ...
            if (auto argument = traceToFunctionArgument(instance.getValue(), function))
...
static void summarizeSimtLaunchEffects(func::FuncOp helper, SimtLaunchOp launch, SmallVectorImpl<uint8_t>& effects)
{
    auto module = helper->getParentOfType<ModuleOp>();
    auto callee = module ? module.lookupSymbol<func::FuncOp>(launch.getCallee()) : func::FuncOp();
    ...
    for (auto [argument, calleeEffect] : llvm::zip_equal(launch.getArgs(), calleeEffects)) {
...
static void summarizeTileOpEffects(func::FuncOp helper)
{
    ...
    helper.walk([&](SimtLaunchOp launch) { summarizeSimtLaunchEffects(helper, launch, effects); });

    SmallVector<Attribute> effectAttrs;
    effectAttrs.reserve(effects.size());
    for (auto [type, effect] : llvm::zip_equal(helper.getArgumentTypes(), effects)) {
    ...
    helper->setAttr(kTileOpEffectsAttr, ArrayAttr::get(helper.getContext(), effectAttrs));
}

static bool addValidShapeRequirement(
    ValidShapeRequirements& requirements, func::FuncOp function, unsigned argumentIndex)
{
...
static LogicalResult collectTileOpValidShapeRequirements(func::FuncOp helper, ValidShapeRequirements& requirements)
{
    ...
    helper.walk([&](Operation* op) {
        ...
        if (!argument || argument.getOwner() != &helper.front()) {
            status = op->emitError("tileop valid-shape metadata must be read directly from a Tile "
                                   "argument");
        ...
        if (!tileType || tileType.getValidShape().size() != 2) {
            status = op->emitError("tileop valid-shape metadata requires a rank-2 Tile argument");
            return WalkResult::interrupt();
        }
...
static LogicalResult propagateValidShapeRequirements(ModuleOp module, ValidShapeRequirements& requirements)
{
    ...
    while (changed) {
        ...
        for (func::CallOp call : calls) {
            auto callee = SymbolTable::lookupNearestSymbolFrom<func::FuncOp>(call.getOperation(), call.getCalleeAttr());
            if (!callee)
                continue;
            ...
            if (!caller)
                return call.emitOpError("cannot propagate Tile valid-shape metadata without a caller "
                                        "function");
            for (unsigned calleeArgument : requiredArguments) {
                if (calleeArgument >= call.getNumOperands())
                    return call.emitOpError("TileOp call has fewer operands than its helper ABI");
                auto callerArgument = traceToFunctionArgument(call.getOperand(calleeArgument), caller);
                if (!callerArgument)
                    continue;
                auto tileType = dyn_cast<TileBufType>(caller.getArgument(*callerArgument).getType());
                if (!tileType || !tileType.hasDynamicValid())
                    continue;
                changed |= addValidShapeRequirement(requirements, caller, *callerArgument);
            }
        }
    }
    return success();
...
static LogicalResult expandValidShapeFunctionArguments(
    ValidShapeRequirements& requirements, ExpandedValidShapeArguments& expandedArguments)
{
    for (auto& [functionOperation, argumentIndices] : requirements) {
        auto function = cast<func::FuncOp>(functionOperation);
        if (function.isExternal())
            return function.emitOpError("cannot propagate dynamic Tile valid-shape metadata through an "
                                        "external function");

        unsigned originalArgumentCount = function.getNumArguments();
        SmallVector<unsigned> insertionIndices(argumentIndices.size() * 2, originalArgumentCount);
        SmallVector<Type> argumentTypes(argumentIndices.size() * 2, IndexType::get(function.getContext()));
        SmallVector<DictionaryAttr> argumentAttrs(
            argumentIndices.size() * 2, DictionaryAttr::get(function.getContext()));
        SmallVector<Location> argumentLocations(argumentIndices.size() * 2, function.getLoc());
        function.insertArguments(insertionIndices, argumentTypes, argumentAttrs, argumentLocations);
        ...
        for (auto [position, originalIndex] : llvm::enumerate(argumentIndices)) {
            ...
            functionArguments[originalIndex] =
                std::make_pair(function.getArgument(rowIndex), function.getArgument(rowIndex + 1));
        }
        ...
        if (auto effects = function->getAttrOfType<ArrayAttr>(kTileOpEffectsAttr)) {
            ...
            function->setAttr(kTileOpEffectsAttr, ArrayAttr::get(function.getContext(), effectAttrs));
        }
    }
    return success();
}

static std::optional<std::pair<Value, Value>> resolveCallValidShape(
    Value tile, Operation* anchor, func::FuncOp caller, const ExpandedValidShapeArguments& expandedArguments,
    OpBuilder& builder)
{
    if (!tile)
        return std::nullopt;
    ...
    if (tileType && tileType.getValidShape().size() == 2 && tileType.getValidShape()[0] >= 0 &&
        tileType.getValidShape()[1] >= 0) {
        Value row = builder.create<arith::ConstantIndexOp>(anchor->getLoc(), tileType.getValidShape()[0]);
        Value col = builder.create<arith::ConstantIndexOp>(anchor->getLoc(), tileType.getValidShape()[1]);
    ...
    validShape->setAttr(kTileOpValidShapeReadAttr, UnitAttr::get(tile.getContext()));
    return std::make_pair(validShape.getValidRow(), validShape.getValidCol());
}

static LogicalResult expandValidShapeCallOperands(
    ModuleOp module, const ValidShapeRequirements& requirements, const ExpandedValidShapeArguments& expandedArguments)
{
    SmallVector<func::CallOp> calls;
    module.walk([&](func::CallOp call) { calls.push_back(call); });
    ...
    for (func::CallOp call : calls) {
        ...
        for (unsigned argumentIndex : required->second) {
            if (argumentIndex >= call.getNumOperands())
                return call.emitOpError("TileOp call has fewer operands than its helper ABI");
            auto metadata =
                resolveCallValidShape(call.getOperand(argumentIndex), call, caller, expandedArguments, builder);
            if (!metadata)
                return call.emitOpError() << "cannot resolve dynamic valid_row/valid_col for Tile "
                                             "operand #"
                                          << argumentIndex
...
static LogicalResult replaceTileOpValidShapeReads(
    ArrayRef<func::FuncOp> helpers, const ExpandedValidShapeArguments& expandedArguments)
{
    for (func::FuncOp helper : helpers) {
        ...
        for (Operation* read : reads) {
            ...
            auto tileType = argument ? dyn_cast<TileBufType>(argument.getType()) : TileBufType();
            if (!argument || !tileType || tileType.getValidShape().size() != 2)
                return read->emitError("tileop valid-shape metadata must be read directly from a rank-2 "
                                       "Tile argument");
            ...
            if (tileType.getValidShape()[dimension] >= 0) {
                replacement =
                    builder.create<arith::ConstantIndexOp>(read->getLoc(), tileType.getValidShape()[dimension]);
            } else {
                auto function = expandedArguments.find(helper.getOperation());
                if (function == expandedArguments.end())
                    return read->emitError("missing internal Tile valid-shape ABI arguments");
                auto metadata = function->second.find(argument.getArgNumber());
                if (metadata == function->second.end())
                    return read->emitError("missing internal Tile valid-shape ABI arguments");
                replacement = dimension == 0 ? metadata->second.first : metadata->second.second;
            }
...
static LogicalResult materializeTileOpValidShapeABI(ModuleOp module, ArrayRef<func::FuncOp> helpers)
{
    ...
    if (failed(expandValidShapeFunctionArguments(requirements, expandedArguments)) ||
        failed(expandValidShapeCallOperands(module, requirements, expandedArguments)) ||
...
static LogicalResult verifyTileOpABI(func::FuncOp helper)
{
    for (auto [index, type] : llvm::enumerate(helper.getArgumentTypes())) {
        if (!isTileOrScalarType(type))
            return helper.emitOpError() << "tileop argument #" << index
...
static bool hasRawVPTOVectorTransientType(Type type) { return isa<VRegType, MaskType, AlignType>(type); }
...
static std::optional<PhysicalSectionKind> inferRawVPTOComputeKind(Operation* op)
{
...
static LogicalResult inferTileOpKind(func::FuncOp helper, PhysicalSectionKind& kind)
{
    ...
    helper.walk([&](Operation* op) {
        if (failed(status) || op == helper.getOperation() || isa<func::ReturnOp>(op))
            return WalkResult::advance();

        if (isa<SectionCubeOp, SectionVectorOp>(op)) {
            status = op->emitError("tileop helpers must not contain pre-existing sections");
            return WalkResult::interrupt();
        }
        ...
        if (isForbiddenPipeOperation(op)) {
            status = op->emitError("tileop helpers must not contain pipe synchronization");
            return WalkResult::interrupt();
        }
    ...
    if (firstVector && firstCube) {
        InFlightDiagnostic diag = helper.emitOpError("mixes Vector and Cube compute operations in one tileop helper");
...
static LogicalResult materializeTileOpSection(func::FuncOp helper, PhysicalSectionKind kind)
{
    ...
    Operation* sectionOperation = kind == PhysicalSectionKind::Vector ?
                                      builder.create<SectionVectorOp>(roots.front()->getLoc()).getOperation() :
                                      builder.create<SectionCubeOp>(roots.front()->getLoc()).getOperation();
    ...
    helper->setAttr(
        kTileOpKindAttr, StringAttr::get(helper.getContext(), kind == PhysicalSectionKind::Vector ? "vector" : "cube"));
    return success();
}
...
struct PTOMaterializeTileOpSectionsPass
    : public mlir::pto::impl::PTOMaterializeTileOpSectionsBase<PTOMaterializeTileOpSectionsPass> {
    void runOnOperation() override
    {
        ...
        if (failed(status) || failed(materializeTileOpValidShapeABI(module, helpers)))
            signalPassFailure();
    }
};
