...
static bool hasKernelKind(ModuleOp module) { return module->hasAttr(FunctionKernelKindAttr::name); }

static bool hasKernelKindChildModule(ModuleOp module)
{
    return llvm::any_of(module.getOps<ModuleOp>(), [](ModuleOp child) { return hasKernelKind(child); });
}
...
static bool flattenSingleUnpartitionedChild(ModuleOp module)
{
    ...
    if (!child || !isVPTOBackendModule(child) || hasKernelKind(child) || !hasCVSections(child) ||
        hasConflictingContainerAttrs(module, child))
        return false;

    SmallVector<NamedAttribute> childAttrs(child->getAttrs().begin(), child->getAttrs().end());
    Region childBody;
...
static bool hasCVSections(ModuleOp module)
{
    ...
    module.walk([&](func::FuncOp funcOp) {
        ...
        return result.wasInterrupted() ? WalkResult::interrupt() : WalkResult::advance();
    });
    return found;
}
...
static bool hasSectionKind(ModuleOp module, FunctionKernelKind kind)
{
    ...
    module.walk([&](func::FuncOp funcOp) {
        if (found || !isSectionSplitCandidate(funcOp))
            return WalkResult::advance();
        WalkResult result = funcOp.walk([&](Operation* op) {
            bool matches = kind == FunctionKernelKind::Cube ? isa<SectionCubeOp>(op) : isa<SectionVectorOp>(op);
            if (matches) {
                found = true;
                return WalkResult::interrupt();
            }
...
static bool isSectionSplitCandidate(func::FuncOp funcOp)
{
    return funcOp && !funcOp.isDeclaration() && (pto::isPTOEntryFunction(funcOp) || hasAnySection(funcOp));
}
...
{
    auto kindAttr = module->getAttrOfType<FunctionKernelKindAttr>(FunctionKernelKindAttr::name);
    if (!kindAttr)
        return success();
...
static LogicalResult verifySectionSplitCandidatesUseSections(ModuleOp module)
{
    ...
    module.walk([&](func::FuncOp funcOp) {
        ...
        if (!hasAnySection(funcOp)) {
            status = funcOp.emitOpError("must contain pto.section.cube or pto.section.vector in section "
...
static void eraseSectionSplitCandidatesWithoutSectionKind(ModuleOp module, FunctionKernelKind kind)
{
...
static void replaceSectionWithBody(Operation* sectionOp)
{
    ...
    parentBlock->getOperations().splice(Block::iterator(sectionOp), body.getOperations());
    sectionOp->erase();
}
...
static ModuleOp cloneModuleForKind(ModuleOp source, FunctionKernelKind kind, OpBuilder& builder)
{
    auto cloned = cast<ModuleOp>(source->clone());
    cloned->setAttr(FunctionKernelKindAttr::name, FunctionKernelKindAttr::get(cloned.getContext(), kind));
...
static LogicalResult materializeExplicitKernelKindSections(ModuleOp module)
{
    ...
    if (!kindAttr)
        return success();
    if (failed(verifyNoNestedSections(module)) || failed(verifyExplicitKernelKindMatchesSections(module)))
        return failure();
...
struct VPTOSplitCVModulePass : public mlir::pto::impl::VPTOSplitCVModuleBase<VPTOSplitCVModulePass> {
...
std::unique_ptr<Pass> mlir::pto::createVPTOSplitCVModulePass() { return std::make_unique<VPTOSplitCVModulePass>(); }
