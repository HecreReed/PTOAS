...
static FailureOr<int64_t> getSignedInt64(const APInt& value, Operation* anchor, StringRef description)
{
...
static FailureOr<int64_t> getConstantInt64(Value value, Operation* anchor, StringRef description)
{
    APInt constant;
    if (!matchPattern(value, m_ConstantInt(&constant))) {
        anchor->emitOpError() << description << " must be a compile-time integer constant";
        return failure();
    }
    return getSignedInt64(constant, anchor, description);
}

static FailureOr<int64_t> getFixedTypeByteSize(
    Type type, Operation* anchor, StringRef description, const DataLayout& dataLayout)
{
    llvm::TypeSize typeSize = dataLayout.getTypeSize(type);
    if (typeSize.isScalable() || typeSize.getFixedValue() == 0 ||
        typeSize.getFixedValue() > static_cast<uint64_t>(std::numeric_limits<int64_t>::max())) {
        anchor->emitOpError() << description << " must have a non-zero fixed byte size, got " << type;
        return failure();
    }
    return static_cast<int64_t>(typeSize.getFixedValue());
}

static FailureOr<FragmentShape> getFragmentShape(LLVM::AllocaOp allocaOp, const DataLayout& dataLayout)
{
    Type elementType = allocaOp.getElemType();
    if (!isSupportedPersistentScalarType(elementType)) {
        return allocaOp.emitOpError() << "persistent SIMT fragment requires an integer scalar element "
                                         "type up to 64 bits or f16/bf16/f32, got "
                                      << elementType;
    }

    FailureOr<int64_t> elementCount =
        getConstantInt64(allocaOp.getArraySize(), allocaOp, "persistent fragment element count");
    if (failed(elementCount))
        return failure();
    if (*elementCount <= 0) {
        return allocaOp.emitOpError("persistent SIMT fragment element count must be positive");
    }

    FailureOr<int64_t> elementByteSize =
        getFixedTypeByteSize(elementType, allocaOp, "persistent fragment element type", dataLayout);
    if (failed(elementByteSize))
        return failure();

    int64_t totalByteSize;
    if (llvm::MulOverflow(*elementCount, *elementByteSize, totalByteSize)) {
        return allocaOp.emitOpError("persistent SIMT fragment byte size overflows signed i64");
    }
...
static FailureOr<int64_t> getStaticGEPIndex(LLVM::GEPOp gep)
{
    ...
    if (indices.size() != 1) {
        gep.emitOpError("persistent SIMT fragment currently requires llvm.getelementptr "
...
static FailureOr<int64_t> getStaticGEPByteOffset(LLVM::GEPOp gep, const DataLayout& dataLayout)
{
    FailureOr<int64_t> index = getStaticGEPIndex(gep);
    if (failed(index))
        return failure();

    FailureOr<int64_t> elementByteSize = getFixedTypeByteSize(gep.getElemType(), gep, "GEP element type", dataLayout);
    if (failed(elementByteSize))
        return failure();
...
static LogicalResult validateScalarizableVectorAccess(Operation* access)
{
    ...
    if (isVolatile || ordering != LLVM::AtomicOrdering::not_atomic) {
        return access->emitOpError("persistent SIMT fragment vector access cannot be volatile or "
                                   "atomic");
    }
    return success();
}
...
static FailureOr<unsigned> getPersistentAccessLaneCount(Operation* access, Type accessType, Type allocationElementType)
{
    ...
    if (auto vectorType = dyn_cast<VectorType>(accessType)) {
        if (vectorType.getRank() != 1 || vectorType.isScalable()) {
            access->emitOpError("persistent SIMT fragment vector access must be fixed-length and "
                                "one-dimensional");
            return failure();
        }
        int64_t vectorLaneCount = vectorType.getDimSize(0);
        if (vectorLaneCount != 2 && vectorLaneCount != 4) {
            access->emitOpError() << "persistent SIMT fragment currently supports only 2- or 4-lane "
                                     "vector accesses, got "
    ...
    if (!isSupportedPersistentScalarType(scalarType)) {
        return access->emitOpError() << "persistent SIMT fragment access requires an integer scalar "
                                        "element type up to 64 bits or f16/bf16/f32, got "
                                     << scalarType;
    }
    if (scalarType != allocationElementType) {
        return access->emitOpError() << "persistent SIMT fragment access element type " << scalarType
                                     << " must match alloca element type " << allocationElementType;
    }
    return laneCount;
}

static LogicalResult recordAccess(
    Operation* access, Type accessType, int64_t byteOffset, func::FuncOp parentFunc, LLVM::AllocaOp allocaOp,
    const FragmentShape& shape, PersistentAccessDiscovery& discovery)
{
    ...
    if (!section) {
        return access->emitOpError("persistent SIMT fragment access must be inside pto.section.simt");
    }
    if (section->getParentOfType<func::FuncOp>() != parentFunc) {
        return access->emitOpError("persistent SIMT fragment access must remain in its defining "
                                   "function");
    }
    if (!section.getBody().hasOneBlock() || access->getBlock() != &section.getBody().front()) {
        return access->emitOpError("persistent SIMT fragment access must be directly inside the "
                                   "single top-level block of the pto.section.simt body");
    }

    Type elementType = allocaOp.getElemType();
    FailureOr<unsigned> laneCount = getPersistentAccessLaneCount(access, accessType, elementType);
    if (failed(laneCount))
        return failure();
    if (byteOffset < 0 || byteOffset % shape.elementByteSize != 0) {
        return access->emitOpError() << "persistent SIMT fragment byte offset " << byteOffset
                                     << " must be non-negative and aligned to element byte size "
                                     << shape.elementByteSize;
    }

    int64_t accessByteSize;
    if (llvm::MulOverflow(static_cast<int64_t>(*laneCount), shape.elementByteSize, accessByteSize)) {
        return access->emitOpError("persistent SIMT fragment access byte size overflows signed i64");
    }
    int64_t accessEnd;
    if (llvm::AddOverflow(byteOffset, accessByteSize, accessEnd) || accessEnd > shape.totalByteSize) {
        if (*laneCount == 1) {
            return access->emitOpError() << "persistent SIMT fragment access at byte offset " << byteOffset
                                         << " exceeds allocation size " << shape.totalByteSize;
        }
        return access->emitOpError() << "persistent SIMT fragment access at byte offset " << byteOffset << " with "
                                     << *laneCount << " scalar lane(s) exceeds allocation "
                                     << "size " << shape.totalByteSize;
    }

    if (discovery.accessIndices.count(access)) {
        return access->emitOpError("persistent SIMT fragment access was visited more than once");
    }
    ...
    for (unsigned laneIndex = 0; laneIndex < *laneCount; ++laneIndex) {
        int64_t elementOffset;
        if (llvm::AddOverflow(firstElementOffset, static_cast<int64_t>(laneIndex), elementOffset)) {
            return access->emitOpError("persistent SIMT fragment lane element offset overflows signed "
                                       "i64");
        }
...
static bool haveSameLaunchDimensions(pto::SectionSimtOp lhs, pto::SectionSimtOp rhs)
{
    return lhs.getDimX() == rhs.getDimX() && lhs.getDimY() == rhs.getDimY() && lhs.getDimZ() == rhs.getDimZ();
}

// Find the unique fragment access section that dominates every other access.
static FailureOr<pto::SectionSimtOp> findInitSection(
    DominanceInfo& dominance, const PersistentMaterializationPlan& plan, const PersistentAccessDiscovery& discovery,
    PersistentFragmentAnalysis& fragment)
{
    ...
    for (const NormalizedPersistentAccess& access : discovery.accesses) {
        if (!access.section) {
            return fragment.allocaOp.emitOpError("persistent fragment access has no containing SIMT section");
        }
    ...
    if (accessedSections.size() != accessedSectionSet.size()) {
        return fragment.allocaOp.emitOpError("failed to locate every persistent fragment access section in the "
                                             "materialization plan");
    }

    pto::SectionSimtOp initSection;
    for (pto::SectionSimtOp candidate : accessedSections) {
        bool dominatesAllAccessSections = llvm::all_of(accessedSections, [&](pto::SectionSimtOp section) {
            if (section == candidate)
                return true;
            return dominance.dominates(candidate.getOperation(), section.getOperation());
        });
        if (!dominatesAllAccessSections)
            continue;

        if (initSection) {
            fragment.allocaOp.emitOpError("persistent SIMT fragment has multiple init section candidates "
    ...
    if (!initSection) {
        fragment.allocaOp.emitOpError("persistent SIMT fragment requires a unique init section that "
...
static LogicalResult collectResidentElements(
    pto::SectionSimtOp initSection, const PersistentAccessDiscovery& discovery, PersistentFragmentAnalysis& fragment,
    llvm::DenseSet<int64_t>& residentElementSet)
{
    for (Operation& op : initSection.getBody().front()) {
        ...
        for (unsigned accessIndex : accessIt->second) {
            const NormalizedPersistentAccess& access = discovery.accesses[accessIndex];
            if (!residentElementSet.insert(access.elementOffset).second)
                continue;

            if (!isa<LLVM::StoreOp>(access.op)) {
                return access.op->emitOpError() << "persistent SIMT fragment element offset " << access.elementOffset
                                                << " must be initialized by a store before its first load in "
                                                   "the init section";
            }
            fragment.residentElements.push_back({access.elementOffset, 0, /*accesses=*/{}});
        }
    }

    if (fragment.residentElements.empty()) {
        return initSection.emitOpError("persistent SIMT fragment init section must initialize at least one "
                                       "element");
    }
    llvm::sort(fragment.residentElements, [](const ResidentElementPlan& lhs, const ResidentElementPlan& rhs) {
...
static LogicalResult validateResidentAccesses(
    const llvm::DenseSet<int64_t>& residentElementSet, const PersistentAccessDiscovery& discovery)
{
    for (const NormalizedPersistentAccess& access : discovery.accesses) {
...
static LogicalResult validateCarrySectionDimensions(
    pto::SectionSimtOp initSection, DominanceInfo& dominance, const PersistentMaterializationPlan& plan,
    PersistentFragmentAnalysis& fragment)
{
    ...
    for (pto::SectionSimtOp section : plan.sections) {
        ...
        if (!dominance.dominates(initSection.getOperation(), section.getOperation()))
            continue;
        if (!haveSameLaunchDimensions(initSection, section)) {
            return section.emitOpError() << "persistent SIMT fragment carry section launch dimensions ("
                                         << section.getDimX() << ", " << section.getDimY() << ", " << section.getDimZ()
                                         << ") must match init section dimensions (" << initSection.getDimX() << ", "
                                         << initSection.getDimY() << ", " << initSection.getDimZ() << ")";
        }
...
static LogicalResult analyzeResidentElements(
    DominanceInfo& dominance, const PersistentMaterializationPlan& plan, PersistentFragmentAnalysis& fragment,
    const PersistentAccessDiscovery& discovery)
{
    FailureOr<pto::SectionSimtOp> initSection = findInitSection(dominance, plan, discovery, fragment);
    if (failed(initSection))
        return failure();
    fragment.initSection = *initSection;

    llvm::DenseSet<int64_t> residentElementSet;
    if (failed(collectResidentElements(*initSection, discovery, fragment, residentElementSet)))
        return failure();
    if (failed(validateResidentAccesses(residentElementSet, discovery)))
        return failure();

    return validateCarrySectionDimensions(*initSection, dominance, plan, fragment);
}
...
static FailureOr<int64_t> getPersistentSlotWidth(Type type, Operation* anchor)
{
    ...
    anchor->emitOpError() << "persistent SIMT fragment type " << type << " has no keep/resume slot-width mapping";
    return failure();
}

static LogicalResult allocatePersistentSlots(func::FuncOp func, PersistentMaterializationPlan& plan)
{
    int64_t nextSlot = 0;
    for (PersistentFragmentAnalysis& fragment : plan.fragments) {
        Type elementType = fragment.allocaOp.getElemType();
        FailureOr<int64_t> slotWidth = getPersistentSlotWidth(elementType, fragment.allocaOp);
        if (failed(slotWidth))
            return failure();
        ...
        for (ResidentElementPlan& residentElement : fragment.residentElements) {
            ...
            if (slot > kPersistentSlotLimit - *slotWidth) {
                return fragment.allocaOp.emitOpError()
                       << "persistent SIMT fragment slot allocation in function '" << func.getSymName()
                       << "' exceeds the " << kPersistentSlotLimit << "-slot limit: element offset " << elementOffset
                       << " of type " << elementType << " needs " << *slotWidth << " slot(s), next candidate slot is "
                       << slot;
            }
...
static LogicalResult materializeResidentAccessLanes(
    const PersistentMaterializationPlan& plan, PersistentFragmentAnalysis& fragment,
    const PersistentAccessDiscovery& discovery)
{
    size_t materializedAccessCount = 0;
    for (pto::SectionSimtOp section : plan.sections) {
        if (!section.getBody().hasOneBlock()) {
            return section.emitOpError("persistent fragment access materialization requires a "
        ...
        for (Operation& op : section.getBody().front()) {
            ...
            for (unsigned accessIndex : accessIt->second) {
                if (accessIndex >= discovery.accesses.size()) {
                    return fragment.allocaOp.emitOpError("persistent fragment discovery has an invalid access index");
                }
                const NormalizedPersistentAccess& access = discovery.accesses[accessIndex];
                if (access.op != &op || access.section != section) {
                    return access.op->emitOpError("persistent fragment normalized access does not match its "
                                                  "section operation");
                }

                ResidentElementPlan* residentElement = fragment.findResidentElement(access.elementOffset);
                if (!residentElement) {
                    return access.op->emitOpError() << "persistent fragment normalized access element offset "
    ...
    if (materializedAccessCount != discovery.accesses.size()) {
        return fragment.allocaOp.emitOpError("failed to materialize every normalized persistent fragment access "
                                             "under a resident element");
    }
    return success();
}

static LogicalResult analyzePersistentFragment(
    LLVM::AllocaOp allocaOp, DominanceInfo& dominance, const PersistentMaterializationPlan& plan,
    PersistentFragmentAnalysis& fragment, PersistentAccessDiscovery& discovery)
{
    ...
    if (!isa<UnitAttr>(allocaOp->getAttr(kPersistentAttrName))) {
        return allocaOp.emitOpError() << "expects '" << kPersistentAttrName << "' to be a unit attribute";
    }
    if (allocaOp->getParentOfType<pto::SectionSimtOp>()) {
        return allocaOp.emitOpError("persistent SIMT fragment must be defined outside pto.section.simt");
    }
    if (parentFunc->hasAttr(pto::kPTOSimtEntryAttrName)) {
        return allocaOp.emitOpError("persistent SIMT fragment must be defined before SIMT outlining");
    }
    ...
    while (!pointerWorklist.empty()) {
        ...
        auto [offsetIt, inserted] = pointerOffsets.try_emplace(item.pointer, item.byteOffset);
        if (!inserted) {
            if (offsetIt->second != item.byteOffset) {
                return allocaOp.emitOpError("persistent fragment pointer has inconsistent byte offsets");
            }
            continue;
        }
        ...
        for (OpOperand& use : item.pointer.getUses()) {
            ...
            if (user->getParentOfType<func::FuncOp>() != parentFunc) {
                return user->emitOpError("persistent SIMT fragment pointer must remain in its defining "
                                         "function");
            }
            if (!dominance.dominates(allocaOp.getOperation(), user)) {
                return user->emitOpError("persistent SIMT fragment definition must dominate every use");
            }

            if (auto gep = dyn_cast<LLVM::GEPOp>(user)) {
                if (use.getOperandNumber() != 0) {
                    return gep.emitOpError("persistent SIMT fragment pointer must be the base of "
                                           "llvm.getelementptr");
                }
                ...
                if (llvm::AddOverflow(item.byteOffset, *gepByteOffset, derivedByteOffset)) {
                    return gep.emitOpError("persistent fragment cumulative byte offset overflows signed "
                                           "i64");
                }
            ...
            if (auto load = dyn_cast<LLVM::LoadOp>(user)) {
                if (failed(recordAccess(
                        load, load.getRes().getType(), item.byteOffset, parentFunc, allocaOp, *shape, discovery)))
                    return failure();
                continue;
            }
            ...
            if (auto store = dyn_cast<LLVM::StoreOp>(user)) {
                if (use.getOperandNumber() != 1) {
                    return store.emitOpError("persistent SIMT fragment pointer must not be stored as a "
                                             "value");
                }
                if (failed(recordAccess(
                        store, store.getValue().getType(), item.byteOffset, parentFunc, allocaOp, *shape, discovery)))
                    return failure();
                continue;
            }

            return user->emitOpError() << "unsupported use of persistent SIMT fragment pointer by '" << user->getName()
                                       << "'";
        }
    }

    if (discovery.accesses.empty()) {
        return allocaOp.emitOpError("persistent SIMT fragment requires at least one llvm.load or "
...
SIMTPersistentFragmentAnalysis::SIMTPersistentFragmentAnalysis(func::FuncOp func)
{
    ...
    if (!pto::isPTOEntryFunction(func)) {
        persistentAllocas.front().emitOpError("persistent SIMT fragment must be defined in a PTO kernel entry");
        return;
    }

    PersistentMaterializationPlan candidate;
    func.walk([&](pto::SectionSimtOp section) { candidate.sections.push_back(section); });
    ...
    for (LLVM::AllocaOp allocaOp : persistentAllocas) {
        ...
        if (failed(analyzePersistentFragment(
                allocaOp, dominance, candidate, candidate.fragments.back(), accessDiscoveries.back())))
            return;
    }

    if (failed(allocatePersistentSlots(func, candidate)))
        return;
    for (unsigned fragmentIndex = 0; fragmentIndex < static_cast<unsigned>(candidate.fragments.size());
         ++fragmentIndex) {
        if (failed(materializeResidentAccessLanes(
                candidate, candidate.fragments[fragmentIndex], accessDiscoveries[fragmentIndex])))
            return;
    }
