...
static std::pair<int64_t, int64_t> computeContMaskValues(unsigned nElements)
{
    int64_t mask0 = (nElements >= kMaskLen) ? static_cast<int64_t>(0xFFFFFFFFFFFFFFFFULL) :
                                              static_cast<int64_t>((1ULL << nElements) - 1ULL);
    int64_t mask1 = (nElements > kMaskLen) ? static_cast<int64_t>((1ULL << (nElements - kMaskLen)) - 1ULL) : 0LL;
    return {mask0, mask1};
}
...
static std::optional<TileShapeInfo> extractTileShapeInfoFromValue(Value opDst, const TileShapeMap& tileShapes)
{
    ...
    int64_t vRows = (!validShape.empty() && validShape[0] != ShapedType::kDynamic) ? validShape[0] : rows;
    int64_t vCols = (validShape.size() >= 2 && validShape[1] != ShapedType::kDynamic) ? validShape[1] : cols;
    if (vRows == ShapedType::kDynamic || vCols == ShapedType::kDynamic || rows == ShapedType::kDynamic ||
        cols == ShapedType::kDynamic)
        return std::nullopt;
...
static std::optional<TileShapeInfo> extractTileShapeInfo(Operation* op, const TileShapeMap& tileShapes)
{
...
struct LowerPTOToUBufOpsPass : public pto::impl::LowerPTOToUBufOpsBase<LowerPTOToUBufOpsPass> {
    ...
    void runOnOperation() override
    {
        ...
        TileShapeMap tileShapes;
        {
            ...
            for (auto op : allocOps) {
                ...
                auto ptrTy = pto::PtrType::get(
                    ctx, tbTy.getElementType(), pto::AddressSpaceAttr::get(ctx, pto::AddressSpace::VEC));
                auto pc = builder.create<pto::CastPtrOp>(op.getLoc(), ptrTy, addr);
                tileShapes[pc.getResult()] = {
                    SmallVector<int64_t, 2>(shape), SmallVector<int64_t, 2>(tbTy.getValidShape())};
        ...
        // ---- tadd → pto.ub.vadd ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!info)
                    continue;
                auto [dstPtr, src0Ptr, src1Ptr, ptrType] =
                    lowerBinaryOpCommon(builder, ctx, op, op.getDst(), op.getSrc0(), op.getSrc1(), tileShapes);
                if (!dstPtr)
                    continue;
                dispatch<pto::UBVaddOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- taddrelu -> pto.ub.vaddrelu ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatch<pto::UBVaddReluOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tsub → pto.ub.vsub ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatch<pto::UBVsubOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tmul → pto.ub.vmul ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatch<pto::UBVmulOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tdiv → pto.ub.vdiv ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatch<pto::UBVdivOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tmax → pto.ub.vmax ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatch<pto::UBVmaxOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tmin → pto.ub.vmin ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatch<pto::UBVminOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tand → pto.ub.vand ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatch<pto::UBVandOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tor → pto.ub.vor ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatch<pto::UBVorOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // De Morgan: src0 ^ src1 = ~(src0 & src1) & (src0 | src1)
        {
            ...
            for (auto op : ops) {
                ...
                if (!info)
                    continue;
                auto [dstPtr, src0Ptr, src1Ptr, tmpPtr, ptrType] = lowerXorOpCommon(
                    builder, ctx, op, op.getDst(), op.getSrc0(), op.getSrc1(), op.getTmp(), tileShapes);
                if (!dstPtr || !tmpPtr)
                    continue;
                auto pipeV = pto::PipeAttr::get(ctx, pto::PIPE::PIPE_V);
                // tmp = src0 | src1
                dispatch<pto::UBVorOp>(op.getLoc(), builder, tmpPtr, src0Ptr, src1Ptr, ptrType, *info);
                builder.create<pto::BarrierOp>(op.getLoc(), pipeV);
                // dst = src0 & src1
                dispatch<pto::UBVandOp>(op.getLoc(), builder, dstPtr, src0Ptr, src1Ptr, ptrType, *info);
                builder.create<pto::BarrierOp>(op.getLoc(), pipeV);
                // dst = ~dst
                dispatchUnary<pto::UBVnotOp>(op.getLoc(), builder, dstPtr, dstPtr, ptrType, *info);
                builder.create<pto::BarrierOp>(op.getLoc(), pipeV);
                // dst = dst & tmp
                dispatch<pto::UBVandOp>(op.getLoc(), builder, dstPtr, dstPtr, tmpPtr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tnot → pto.ub.vnot ----
        {
            ...
            for (auto op : ops) {
                ...
                auto [dstPtr, srcPtr, ptrType] =
                    lowerShiftOpCommon(builder, ctx, op, op.getDst(), op.getSrc(), tileShapes);
                if (!dstPtr)
                    continue;
                dispatchUnary<pto::UBVnotOp>(op.getLoc(), builder, dstPtr, srcPtr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tabs → pto.ub.vabs ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatchUnary<pto::UBVabsOp>(op.getLoc(), builder, dstPtr, srcPtr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- trelu → pto.ub.vrelu ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatchUnary<pto::UBVreluOp>(op.getLoc(), builder, dstPtr, srcPtr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tneg → pto.ub.vmuls(dst, src, -1) ----
        {
            ...
            for (auto op : ops) {
                ...
                if (elemTy.isF32() || elemTy.isF16()) {
                    minusOneScalar = builder.create<arith::ConstantOp>(op.getLoc(), builder.getFloatAttr(elemTy, -1.0));
                } else {
                    minusOneScalar = builder.create<arith::ConstantOp>(op.getLoc(), builder.getIntegerAttr(elemTy, -1));
                }
                Value minusOne = convertScalarToI64(builder, op.getLoc(), minusOneScalar);
                dispatchShift<pto::UBVmulSOp>(op.getLoc(), builder, dstPtr, srcPtr, minusOne, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- trecip → vector_dup(dst, 1) + vdiv(dst, dst, src) ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                Type elemTy = ptrType.getElementType();
                Value oneScalar = builder.create<arith::ConstantOp>(op.getLoc(), builder.getFloatAttr(elemTy, 1.0));
                Value one = convertScalarToI64(builder, op.getLoc(), oneScalar);
                dispatchDup(op.getLoc(), builder, dstPtr, one, ptrType, *info);
                builder.create<pto::BarrierOp>(op.getLoc(), pto::PipeAttr::get(ctx, pto::PIPE::PIPE_V));
                dispatch<pto::UBVdivOp>(op.getLoc(), builder, dstPtr, dstPtr, srcPtr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- texp → pto.ub.vexp ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatchUnary<pto::UBVexpOp>(op.getLoc(), builder, dstPtr, srcPtr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tlog → pto.ub.vln ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatchUnary<pto::UBVlnOp>(op.getLoc(), builder, dstPtr, srcPtr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tsqrt → pto.ub.vsqrt ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatchUnary<pto::UBVsqrtOp>(op.getLoc(), builder, dstPtr, srcPtr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- trsqrt → pto.ub.vrsqrt ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatchUnary<pto::UBVrsqrtOp>(op.getLoc(), builder, dstPtr, srcPtr, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tadds → pto.ub.vadds ----
        {
            ...
            for (auto op : ops) {
                ...
                dispatchShift<pto::UBVaddSOp>(op.getLoc(), builder, dstPtr, srcPtr, scalarI64, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tmuls → pto.ub.vmuls ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!info)
                    continue;
                auto [dstPtr, srcPtr, ptrType] =
                    lowerShiftOpCommon(builder, ctx, op, op.getDst(), op.getSrc0(), tileShapes);
                if (!dstPtr)
                    continue;
                Value scalarI64 = convertScalarToI64(builder, op.getLoc(), op.getScalar());
                dispatchShift<pto::UBVmulSOp>(op.getLoc(), builder, dstPtr, srcPtr, scalarI64, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tmaxs → pto.ub.vmaxs ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                Value scalarI64 = convertScalarToI64(builder, op.getLoc(), op.getScalar());
                dispatchShift<pto::UBVmaxSOp>(op.getLoc(), builder, dstPtr, srcPtr, scalarI64, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tmins → pto.ub.vmins ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                Value scalarI64 = convertScalarToI64(builder, op.getLoc(), op.getScalar());
                dispatchShift<pto::UBVminSOp>(op.getLoc(), builder, dstPtr, srcPtr, scalarI64, ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tshls → pto.ub.vshl (scalar shift) ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatchShift<pto::UBVshlOp>(op.getLoc(), builder, dstPtr, srcPtr, op.getScalar(), ptrType, *info);
                op.erase();
            }
        }
        ...
        // ---- tshrs → pto.ub.vshr (scalar shift) ----
        {
            ...
            for (auto op : ops) {
                ...
                if (!dstPtr)
                    continue;
                dispatchShift<pto::UBVshrOp>(op.getLoc(), builder, dstPtr, srcPtr, op.getScalar(), ptrType, *info);
                op.erase();
            }
        }
        ...
        func.walk([&](Operation* op) {
            if (isa<pto::PartitionViewOp, pto::MakeTensorViewOp, memref::SubViewOp, memref::ReinterpretCastOp,
                    memref::CastOp>(op))
...
private:
    ...
    {
        return b.create<arith::ConstantOp>(loc, b.getIntegerAttr(b.getIndexType(), val)).getResult();
    }
    ...
    void emitUBBinOp(Location loc, OpBuilder& b, Value dst, Value s0, Value s1, Value repeat, Value repStride)
    {
        b.create<UBop>(
            loc, dst, s0, s1, repeat, i64c1(loc, b), i64c1(loc, b), i64c1(loc, b), repStride, repStride, i64c0(loc, b));
    }

    void vadd(Location loc, OpBuilder& b, Value dst, Value s0, Value s1, Value repeat, Value repStride)
    {
        emitUBBinOp<pto::UBVaddOp>(loc, b, dst, s0, s1, repeat, repStride);
    }
    void vsub(Location loc, OpBuilder& b, Value dst, Value s0, Value s1, Value repeat, Value repStride)
    {
        emitUBBinOp<pto::UBVsubOp>(loc, b, dst, s0, s1, repeat, repStride);
    }
    void vmul(Location loc, OpBuilder& b, Value dst, Value s0, Value s1, Value repeat, Value repStride)
    {
        emitUBBinOp<pto::UBVmulOp>(loc, b, dst, s0, s1, repeat, repStride);
    }
    void vdiv(Location loc, OpBuilder& b, Value dst, Value s0, Value s1, Value repeat, Value repStride)
    {
        emitUBBinOp<pto::UBVdivOp>(loc, b, dst, s0, s1, repeat, repStride);
    }

    std::tuple<Value, Value, Value, pto::PtrType> lowerBinaryOpCommon(
        OpBuilder& builder, MLIRContext* ctx, Operation* op, Value dstVal, Value src0Val, Value src1Val,
    ...
    std::tuple<Value, Value, Value, Value, pto::PtrType> lowerXorOpCommon(
        OpBuilder& builder, MLIRContext* ctx, Operation* op, Value dstVal, Value src0Val, Value src1Val, Value tmpVal,
        const TileShapeMap& tileShapes)
    {
        Location loc = op->getLoc();
        builder.setInsertionPoint(op);
    ...
    std::tuple<Value, Value, pto::PtrType> lowerShiftOpCommon(
        OpBuilder& builder, MLIRContext* ctx, Operation* op, Value dstVal, Value srcVal, const TileShapeMap& tileShapes)
    {
        Location loc = op->getLoc();
        builder.setInsertionPoint(op);
        Type elemTy = getStoredElemType(dstVal.getType());
    ...
    template <typename UBop>
    void dispatchShift(
        Location loc, OpBuilder& b, Value dst, Value src, Value scalar, pto::PtrType ptrTy, const TileShapeInfo& info)
    {
        if (info.vRows > 1 && info.vCols != info.cols) {
            auto forOp = b.create<scf::ForOp>(loc, idxc0(loc, b), idxc(info.vRows, loc, b), idxc1(loc, b));
            b.setInsertionPointToStart(forOp.getBody());
            Value off = b.create<arith::MulIOp>(loc, forOp.getInductionVar(), idxc(info.cols, loc, b));
            TileShapeInfo rowInfo = info;
            rowInfo.rows = 1;
            rowInfo.vRows = 1;
            dispatchShift<UBop>(
                loc, b, addPtr(loc, b, dst, ptrTy, off), addPtr(loc, b, src, ptrTy, off), scalar, ptrTy, rowInfo);
        ...
        auto emitShift = [&](Value d, Value s) {
            ...
            b.create<UBop>(
                loc, d, s, scalarI64, i64c1(loc, b), i64c1(loc, b), i64c1(loc, b), i64c8(loc, b), i64c8(loc, b));
        };

        if (headRepeats > 1 || tailElements > 0) {
            if (headRepeats > 0) {
                auto forOp = b.create<scf::ForOp>(loc, idxc0(loc, b), idxc(headRepeats, loc, b), idxc1(loc, b));
                b.setInsertionPointToStart(forOp.getBody());
    ...
    template <typename UBop>
    void dispatchUnary(Location loc, OpBuilder& b, Value dst, Value src, pto::PtrType ptrTy, const TileShapeInfo& info)
    {
        if (info.vRows > 1 && info.vCols != info.cols) {
            auto forOp = b.create<scf::ForOp>(loc, idxc0(loc, b), idxc(info.vRows, loc, b), idxc1(loc, b));
            b.setInsertionPointToStart(forOp.getBody());
            ...
            rowInfo.vRows = 1;
            dispatchUnary<UBop>(
                loc, b, addPtr(loc, b, dst, ptrTy, off), addPtr(loc, b, src, ptrTy, off), ptrTy, rowInfo);
            b.setInsertionPointAfter(forOp);
            return;
        }
        auto emit = [&](Value rd, Value rs) {
            b.create<UBop>(loc, rd, rs, i64c1(loc, b), i64c1(loc, b), i64c1(loc, b), i64c8(loc, b), i64c8(loc, b));
        };
        int64_t epr = info.elementsPerRepeat;
        int64_t totalV = info.vRows * info.vCols;
        int64_t headRepeats = totalV / epr;
    ...
    void dispatchDup(Location loc, OpBuilder& b, Value dst, Value scalar, pto::PtrType ptrTy, const TileShapeInfo& info)
    {
        if (info.vRows > 1 && info.vCols != info.cols) {
            auto forOp = b.create<scf::ForOp>(loc, idxc0(loc, b), idxc(info.vRows, loc, b), idxc1(loc, b));
            b.setInsertionPointToStart(forOp.getBody());
            ...
            rowInfo.rows = 1;
            rowInfo.vRows = 1;
            dispatchDup(loc, b, addPtr(loc, b, dst, ptrTy, off), scalar, ptrTy, rowInfo);
            b.setInsertionPointAfter(forOp);
            return;
        }
        auto emit = [&](Value rd) {
            ...
            if (scalarI64.getType() != b.getI64Type())
                scalarI64 = b.create<arith::ExtSIOp>(loc, b.getI64Type(), scalar);
            b.create<pto::UBVdupOp>(
                loc, rd, scalarI64, i64c1(loc, b), i64c1(loc, b), i64c1(loc, b), i64c8(loc, b), i64c0(loc, b));
        };
    ...
    template <typename UBop>
    void modeNorm1L(
        Location loc, OpBuilder& b, Value dst, Value s0, Value s1, pto::PtrType ptrTy, const TileShapeInfo& info)
    {
        int64_t epr = info.elementsPerRepeat;
        int64_t totalV = info.vRows * info.vCols;
        int64_t headRepeats = totalV / epr;
    ...
    void fullMask(Location loc, OpBuilder& b) { b.create<pto::UBSetMaskOp>(loc, i64cM1(loc, b), i64cM1(loc, b)); }

    Value addPtr(Location loc, OpBuilder& b, Value base, pto::PtrType ptrTy, Value off)
    {
    ...
    static FailureOr<DmaViewInfo> extractDmaViewInfo(pto::TLoadOp op)
    {
        ...
        if (pvOp) {
            ...
            if (info.strides.empty() || !matchPattern(info.strides.back(), m_One())) {
    ...
    static FailureOr<DmaViewInfo> extractDmaMemRefViewInfo(Location loc, Value view, MLIRContext* ctx)
    {
        auto memTy = dyn_cast<MemRefType>(view.getType());
        if (!memTy)
            return failure();
        ...
        info.strides.assign(metadata.getStrides().begin(), metadata.getStrides().end());
        return info;
    }
    ...
    Value computeGMByteOffset(Location loc, OpBuilder& b, const DmaViewInfo& viewInfo, unsigned elemSize)
    {
        if (viewInfo.linearOffset) {
            Value offset = b.create<arith::IndexCastOp>(loc, b.getI64Type(), viewInfo.linearOffset);
            return b.create<arith::MulIOp>(loc, offset, i64c(elemSize, loc, b));
        }
        Value totalOff = idxc0(loc, b);
        for (size_t i = 0; i < viewInfo.offsets.size() && i < viewInfo.strides.size(); ++i) {
            APInt constOff;
            if (matchPattern(viewInfo.offsets[i], m_ConstantInt(&constOff)) && constOff.isZero())
                continue;
            Value dimOff = b.create<arith::MulIOp>(loc, viewInfo.offsets[i], viewInfo.strides[i]).getResult();
            totalOff = b.create<arith::AddIOp>(loc, totalOff, dimOff).getResult();
        }
        if (elemSize > 1)
            totalOff = b.create<arith::MulIOp>(loc, totalOff, idxc(elemSize, loc, b)).getResult();
        return totalOff;
    }

    Value offsetGMPtrByBytes(Location loc, OpBuilder& b, Value gmPtr, Value byteOff)
    {
        APInt constOff;
        if (matchPattern(byteOff, m_ConstantInt(&constOff)) && constOff.isZero())
            return gmPtr;
        auto origPtrTy = cast<pto::PtrType>(gmPtr.getType());
        auto bytePtrTy = pto::PtrType::get(b.getContext(), b.getI8Type(), origPtrTy.getMemorySpace());
        Value bytePtr = b.create<pto::CastPtrOp>(loc, bytePtrTy, gmPtr);
        Value offIdx = byteOff;
        if (!offIdx.getType().isIndex())
            offIdx = b.create<arith::IndexCastOp>(loc, b.getIndexType(), byteOff).getResult();
        Value offsetBytePtr = b.create<pto::AddPtrOp>(loc, bytePtrTy, bytePtr, offIdx);
        return b.create<pto::CastPtrOp>(loc, origPtrTy, offsetBytePtr);
    }

    Value i64Cast(Location loc, OpBuilder& b, Value indexVal)
    {
        return b.create<arith::IndexCastOp>(loc, b.getI64Type(), indexVal).getResult();
    }

    LogicalResult emitMteGmUb(
        Location loc, OpBuilder& b, Value gmPtr, Value ubPtr, const DmaViewInfo& viewInfo, Type elemTy,
        ArrayRef<int64_t> tileShape)
    {
        ...
        Value lenBurst = b.create<arith::MulIOp>(
                              loc, b.create<arith::IndexCastOp>(loc, b.getI64Type(), lenBurstElts).getResult(),
                              i64c(elemSize, loc, b))
                             .getResult();
        Value nburstSrcStride =
            b.create<arith::MulIOp>(
                 loc, b.create<arith::IndexCastOp>(loc, b.getI64Type(), viewInfo.strides[nd - 2]).getResult(),
                 i64c(elemSize, loc, b))
                .getResult();
        Value ubRowStride = b.create<arith::MulIOp>(loc, i64c(ubCols, loc, b), i64c(elemSize, loc, b)).getResult();
        pto::DmaLoopConfig nburst{i64Cast(loc, b, nburstCount), nburstSrcStride, ubRowStride};

        SmallVector<pto::DmaLoopConfig> loops;
        for (int i = nd - 3; i >= 0; --i) {
            Value count = b.create<arith::IndexCastOp>(loc, b.getI64Type(), viewInfo.sizes[i]).getResult();
            Value srcStride =
                b.create<arith::MulIOp>(
                     loc, b.create<arith::IndexCastOp>(loc, b.getI64Type(), viewInfo.strides[i]).getResult(),
                     i64c(elemSize, loc, b))
                    .getResult();
            Value innerElems = i64c1(loc, b);
            for (int j = i + 1; j < (int)nd; ++j) {
                Value dimSize = b.create<arith::IndexCastOp>(loc, b.getI64Type(), viewInfo.sizes[j]).getResult();
                innerElems = b.create<arith::MulIOp>(loc, innerElems, dimSize).getResult();
            }
            Value dstStride = b.create<arith::MulIOp>(loc, innerElems, i64c(elemSize, loc, b)).getResult();
            loops.push_back({count, srcStride, dstStride});
        }
        b.create<pto::MteGmUbOp>(
            loc, gmPtr, ubPtr, i64c0(loc, b), lenBurst, nburst, llvm::ArrayRef(loops), std::nullopt);
        return success();
    }

    LogicalResult emitMteUbGm(
        Location loc, OpBuilder& b, Value ubPtr, Value gmPtr, const DmaViewInfo& viewInfo, Type elemTy,
        ArrayRef<int64_t> tileShape)
    {
        if (tileShape.size() < 2)
            return failure();
        ...
        Value lenBurst = b.create<arith::MulIOp>(
                              ...
                              i64c(elemSize, loc, b))
                             .getResult();
        Value nburstSrcStride = b.create<arith::MulIOp>(loc, i64c(ubCols, loc, b), i64c(elemSize, loc, b)).getResult();
        Value nburstDstStride =
            b.create<arith::MulIOp>(
                 loc, b.create<arith::IndexCastOp>(loc, b.getI64Type(), viewInfo.strides[nd - 2]).getResult(),
                 i64c(elemSize, loc, b))
                .getResult();
        pto::DmaLoopConfig nburst{i64Cast(loc, b, nburstCount), nburstSrcStride, nburstDstStride};
        ...
        for (int i = nd - 3; i >= 0; --i) {
            ...
            Value srcStride = b.create<arith::MulIOp>(loc, innerElems, i64c(elemSize, loc, b)).getResult();
            Value dstStride =
                b.create<arith::MulIOp>(
                     loc, b.create<arith::IndexCastOp>(loc, b.getI64Type(), viewInfo.strides[i]).getResult(),
                     i64c(elemSize, loc, b))
        ...
        b.create<pto::MteUbGmOp>(loc, ubPtr, gmPtr, lenBurst, nburst, Value{}, llvm::ArrayRef(loops));
        return success();
    }

    LogicalResult lowerTLoad(pto::TLoadOp op, OpBuilder& b, const TileShapeMap& tileShapes)
    {
        Location loc = op.getLoc();
        ...
        return emitMteGmUb(loc, b, gmPtr, op.getDst(), *viewInfo, elemTy, llvm::ArrayRef(it->second.shape));
    }

    LogicalResult lowerTStore(pto::TStoreOp op, OpBuilder& b, const TileShapeMap& tileShapes)
    {
        Location loc = op.getLoc();
        ...
        Value byteOff = computeGMByteOffset(loc, b, *viewInfo, elemSize);
        Value gmPtr = offsetGMPtrByBytes(loc, b, viewInfo->gmPtr, byteOff);
        return emitMteUbGm(loc, b, op.getSrc(), gmPtr, *viewInfo, elemTy, llvm::ArrayRef(it->second.shape));
    }
        ...
        Location loc, OpBuilder& b, Value dst, Value s0, Value s1, pto::PtrType ptrTy, const TileShapeInfo& info)
    {
        ...
        b.setInsertionPointToStart(forOp.getBody());
        Value iv = forOp.getInductionVar();
        Value off = b.create<arith::MulIOp>(loc, iv, idxc(rowStride, loc, b)).getResult();
        Value rd = addPtr(loc, b, dst, ptrTy, off);
        ...
        int64_t rowStride, int64_t rs)
    {
        ...
        if (info.vRows > static_cast<int64_t>(epr)) {
            ...
            if (remainElem > 0) {
                ...
                tailRows<UBop>(
                    loc, b, addPtr(loc, b, dst, ptrTy, off), addPtr(loc, b, s0, ptrTy, off),
                    addPtr(loc, b, s1, ptrTy, off), ptrTy, info, rowStride, rs, remainElem);
            }
        } else {
            if (remainElem == 0) {
                headRows<UBop>(loc, b, dst, s0, s1, ptrTy, info, rowStride, info.vCols / epr);
        ...
        int64_t rowStride, int64_t rptPerLine)
    {
        ...
        b.setInsertionPointToStart(forOp.getBody());
        Value iv = forOp.getInductionVar();
        Value rowBase = b.create<arith::MulIOp>(loc, iv, idxc(rowStride, loc, b)).getResult();

        if (numLoop > 0) {
            auto inner = b.create<scf::ForOp>(loc, idxc0(loc, b), idxc(numLoop, loc, b), idxc1(loc, b));
            b.setInsertionPointToStart(inner.getBody());
            Value jv = inner.getInductionVar();
            Value co = b.create<arith::MulIOp>(loc, jv, idxc(chunkElems, loc, b)).getResult();
            Value off = b.create<arith::AddIOp>(loc, rowBase, co).getResult();
            emitUBBinOp<UBop>(
                loc, b, addPtr(loc, b, dst, ptrTy, off), addPtr(loc, b, s0, ptrTy, off), addPtr(loc, b, s1, ptrTy, off),
    ...
    {
        bool strideOver = (rowStride / info.blockSizeElem > kRepeatStrideMax);
        ...
        if (info.vRows > kRepeatMax) {
            ...
            auto forOp = b.create<scf::ForOp>(loc, idxc0(loc, b), idxc(numLoop, loc, b), idxc1(loc, b));
            b.setInsertionPointToStart(forOp.getBody());
            Value iv = forOp.getInductionVar();
            if (strideOver)
        ...
        if (remainAfterLoop > 0) {
            if (strideOver)
                tailStrideOverRemain<UBop>(loc, b, dst, s0, s1, ptrTy, rowStride, numLoop, remainAfterLoop);
            else
                tailStrideOkRemain<UBop>(loc, b, dst, s0, s1, ptrTy, rowStride, rs, numLoop, remainAfterLoop);
        }
    ...
    template <typename UBop>
    void tailStrideOverChunk(
        Location loc, OpBuilder& b, Value iv, Value dst, Value s0, Value s1, pto::PtrType ptrTy, int64_t rowStride)
    {
        auto forOp = b.create<scf::ForOp>(loc, idxc0(loc, b), idxc(kRepeatMax, loc, b), idxc1(loc, b));
        b.setInsertionPointToStart(forOp.getBody());
        Value jv = forOp.getInductionVar();
        Value baseOff = b.create<arith::MulIOp>(loc, iv, idxc(kRepeatMax * rowStride, loc, b)).getResult();
        Value rowOff = b.create<arith::MulIOp>(loc, jv, idxc(rowStride, loc, b)).getResult();
    ...
    template <typename UBop>
    void tailStrideOkChunk(
        Location loc, OpBuilder& b, Value iv, Value dst, Value s0, Value s1, pto::PtrType ptrTy, int64_t rowStride,
        int64_t rs)
    {
        Value off = b.create<arith::MulIOp>(loc, iv, idxc(kRepeatMax * rowStride, loc, b)).getResult();
        emitUBBinOp<UBop>(
            loc, b, addPtr(loc, b, dst, ptrTy, off), addPtr(loc, b, s0, ptrTy, off), addPtr(loc, b, s1, ptrTy, off),
    ...
    template <typename UBop>
    void tailStrideOverRemain(
        Location loc, OpBuilder& b, Value dst, Value s0, Value s1, pto::PtrType ptrTy, int64_t rowStride,
        int64_t numLoop, int64_t remain)
    {
        auto forOp = b.create<scf::ForOp>(loc, idxc0(loc, b), idxc(remain, loc, b), idxc1(loc, b));
        b.setInsertionPointToStart(forOp.getBody());
        Value jv = forOp.getInductionVar();
    ...
    template <typename UBop>
    void tailStrideOkRemain(
        Location loc, OpBuilder& b, Value dst, Value s0, Value s1, pto::PtrType ptrTy, int64_t rowStride, int64_t rs,
        int64_t numLoop, int64_t remain)
    {
...
namespace mlir {
namespace pto {
std::unique_ptr<Pass> createLowerPTOToUBufOpsPass() { return std::make_unique<LowerPTOToUBufOpsPass>(); }
} // namespace pto
} // namespace mlir
