...
static std::optional<int64_t> getTileBufferFootprintBytes(TileBufType type)
{
    ...
    if (major == 0 || minor == 0)
        return 0;
    return ((major - 1) * (minor + 1) + minor) * static_cast<int64_t>(elemBytes);
}
...
static size_t plannerAlignBitsFromBytes(size_t alignBytes) { return std::max<size_t>(alignBytes, 1) * kBitsPerByte; }
...
static SmallVector<Value> getMemoryEffectBufferOperands(Operation* op, const StableValueOrderMap& stableValueOrder)
{
    ...
    const DenseMap<Value, SmallVector<uint64_t>>& buffer2Offsets)
{
    ...
    for (const auto& it : bufferInfos) {
        ...
        if (bufferInfo.operation) {
            ...
            occupiedSizeBytes = alignUpBytes(occupiedSizeBytes, std::max<int64_t>(spec.alignBytes, 1));
        }
...
void MemLivenessAnalysis::RecursionIR(Region* region, Liveness live)
{
    auto result = region->walk<WalkOrder::PreOrder>([&](Operation* op) {
        ...
        if (mayAliasOp.has_value()) {
            ...
            if (auto attr = allocTileOp->getAttrOfType<IntegerAttr>(mlir::pto::kPtoMultiBufferAttrName)) {
                uint64_t n = attr.getValue().getZExtValue();
                if (n < mlir::pto::kPtoMultiBufferMinNum || n > mlir::pto::kPtoMultiBufferMaxNum) {
                    allocTileOp.emitError() << "pto.multi_buffer must be in [" << mlir::pto::kPtoMultiBufferMinNum
            ...
            for (const auto& conflictPair :
                 getScratchConflictPairsFromEffects(op, ptoDpsOp.getDpsInits(), stableValueOrder)) {
            ...
            OpKillHandle(curOpInfo, live, op->getBlock());
        } else if (isa<pto::TAllocOp, pto::TPushOp, pto::TFreeOp, pto::SetQuantScalarOp, pto::SetQuantVectorOp,
                       pto::InitializeL2LPipeOp, pto::InitializeL2G2LPipeOp, pto::BuildAsyncSessionOp, pto::TPutAsyncOp,
                       pto::TGetAsyncOp, pto::TPutOp, pto::TGetOp, pto::TNotifyOp, pto::TWaitOp, pto::TTestOp,
                       pto::SyncAllOp, pto::TBroadcastOp, pto::CommTGatherOp, pto::CommTScatterOp, pto::TReduceOp>(
                       op)) {
            UpdateOpGenInfo(curOpInfo, llvm::to_vector(op->getOperands()));
            OpKillHandle(curOpInfo, live, op->getBlock());
            ...
            SmallVector<Value> buffers = getMemoryEffectBufferOperands(op, stableValueOrder);
...
void MemLivenessAnalysis::UpdateFusionRegionBufferAlias(pto::FusionRegionOp fusionRegion, pto::YieldOp yieldOp)
{
    if (fusionRegion.getResults().empty())
        return;
    if (fusionRegion->getResults().size() != yieldOp->getOperands().size()) {
        llvm::report_fatal_error("pto.fusion_region result/yield sizes are inconsistent");
    }
...
void MemLivenessAnalysis::RecursiveFusionRegionOp(pto::FusionRegionOp fusionRegion, Liveness live)
{
...
SetVector<Value> MemLivenessAnalysis::Union(SetVector<Value> set1, SetVector<Value> set2)
{
...
BufferInfo MemLivenessAnalysis::GenerateBufferInfo(Operation* op, Value operand)
{
...
BufferInfo MemLivenessAnalysis::GetBufferInfo(Operation* op, Value operand, pto::AddressSpace bufferScope)
{
    ...
    if (!footprintBytes.has_value())
        llvm::report_fatal_error("failed to obtain buffer static physical footprint");
...
LogicalResult MemPlan::plan()
{
    ...
    for (size_t i = 0; i < plannedEntries.size(); ++i) {
        for (size_t j = i + 1; j < plannedEntries.size(); ++j) {
            ...
            func_.emitError() << "PlanMemory produced overlapping local buffers in "
    ...
    for (auto& e : StorageEntryVec) {
        ...
        for (Value& buffer : e->inplaceBuffers) {
            ...
            for (auto* sibling : e->relationOtherBuffers) {
                if (!sibling)
                    continue;
                buffer2Offsets[buffer].push_back((sibling->bitsOffset + kBitsToByte - 1) / kBitsToByte);
            }
        }
    }
...
void MemPlan::PlanBuffersWithoutReuse(StorageEntry* rootStorageEntry, size_t alignUnit)
{
    uint offset = 0;
...
void MemPlan::ReorderContinuousPingPongEntry(SmallVector<StorageEntry*>& storageEntryVec)
{
    SmallVector<StorageEntry*> reorderedStorageEntryVec;
...
StorageEntry* MemPlan::GetSizeOrderedRootStorageEntry(StorageEntry* rootStorageEntry)
{
    ...
    entries.insert(entries.end(), rootStorageEntry->mergedChildren.begin(), rootStorageEntry->mergedChildren.end());
    ...
    // are left untouched.
    std::stable_sort(entries.begin(), entries.end(), [](const StorageEntry* a, const StorageEntry* b) {
...
std::pair<size_t, size_t> MemPlan::GetBufferSpaceInfo(pto::AddressSpace& space) const
{
    switch (space) {
        ...
        case pto::AddressSpace::ACC:
            return std::make_pair(plannerAlignBitsFromBytes(l0cAlignSize), l0cSpaceSize);
        case pto::AddressSpace::LEFT:
            return std::make_pair(plannerAlignBitsFromBytes(l0aAlignSize), l0aSpaceSize);
        case pto::AddressSpace::RIGHT:
            return std::make_pair(plannerAlignBitsFromBytes(l0bAlignSize), l0bSpaceSize);
        case pto::AddressSpace::BIAS:
            return std::make_pair(plannerAlignBitsFromBytes(biasAlignSize), biasSpaceSize);
        case pto::AddressSpace::SCALING:
            return std::make_pair(plannerAlignBitsFromBytes(scalingAlignSize), scalingSpaceSize);
    ...
    for (MemBoundListConstIter start = outline.begin(); start != outline.end(); ++start) {
        ...
        for (MemBoundListConstIter end = start; end != outline.end(); ++end) {
            ...
            if (localLevel == SPEC_LEVEL_1 &&
                VerifyConflictStage1(outline, his, e, OutlineSectionInfo(start, end, size, false), pongOffset)) {
                break;
            }
            ...
            UpdateOutline(outline, his, e, OutlineSectionInfo(start, end, size, false), localLevel);
...
LoopLikeOpInterface MemPlan::GetBufferParentLoop(const SmallVector<Value>& buffers)
{
...
inline bool MemPlan::VerifyConflictStage0(StorageEntry* e, const std::shared_ptr<MemoryBound>& last)
{
...
class LegacyAllocTileOpAddPlannedAddressPattern : public OpRewritePattern<pto::AllocTileOp> {
public:
...
class LegacyAllocMultiTileOpAddPlannedAddressesPattern : public OpRewritePattern<pto::AllocMultiTileOp> {
public:
...
static FailureOr<MemPlanMode> parseLegacyMemPlanMode(func::FuncOp func, llvm::StringRef memMode)
{
    if (memMode.equals_insensitive("local") || memMode.equals_insensitive("local-mem-plan"))
    ...
    func.emitError("unsupported mem-mode '") << memMode << "'; only 'local' is supported by the PTOAS pipeline";
    return failure();
}
...
private:
    void populateBufferAddressToAllocOp(
        RewritePatternSet& patterns, MemPlanMode mode, DenseMap<Value, SmallVector<uint64_t>> buffer2Offsets)
    {
        if (mode == MemPlanMode::LOCAL_MEM_PLAN) {
            patterns.add<LegacyAllocTileOpAddPlannedAddressPattern>(patterns.getContext(), buffer2Offsets);
            patterns.add<LegacyAllocMultiTileOpAddPlannedAddressesPattern>(patterns.getContext(), buffer2Offsets);
        }
    }
};
} // namespace
...
void PlanMemoryPass::runOnOperation()
{
    ...
    for (func::FuncOp funcOp : funcs) {
        ...
        if (mode == MemPlanMode::LOCAL_MEM_PLAN && failed(analyzeReserveBufferPlans(funcOp, reservePlans))) {
            return signalPassFailure();
        }
        if (mode == MemPlanMode::LOCAL_MEM_PLAN) {
        ...
        MemPlan memPlan(
            mode, enableGlobalReuse, enablePrintMemoryAllocatedSize, restrictInplaceAsISA, this->orderBySize);
...
std::unique_ptr<Pass> mlir::pto::createPlanMemoryPass(const PlanMemoryOptions& options)
{
