...
{
    return isa<
        LLVM::LLVMHiFloat8Type, LLVM::LLVMFloat8E4M3Type, LLVM::LLVMFloat8E5M2Type, LLVM::LLVMFloat4E1M2x2Type,
        LLVM::LLVMFloat4E2M1x2Type>(type);
}

static Type getLLVMCompatibleVectorType(ArrayRef<int64_t> shape, Type elementType, ArrayRef<bool> scalableDims = {})
{
...
static Type normalizeGEPElementTypeForLLVMLowering(Type type, Builder& builder)
{
    if (pto::isPTOHiFloat8x2Type(type))
...
static Type convertVPTOType(Type type, Builder& builder)
{
    ...
    if (auto ptrType = dyn_cast<pto::PtrType>(type)) {
        return LLVM::LLVMPointerType::get(
            builder.getContext(), static_cast<unsigned>(ptrType.getMemorySpace().getAddressSpace()));
    }
    return normalizePayloadTypeForLLVMLowering(type, builder);
}
...
static Value materializeVPTOCast(OpBuilder& builder, Type resultType, ValueRange inputs, Location loc)
{
    if (inputs.size() != 1)
        return {};
...
static LLVM::LLVMStructType getVPTOStructStorageType(pto::StructType structType, Builder& builder)
{
    struct Frame {
        pto::StructType type;
        bool materialize;
...
static FailureOr<Value> getVPTOStructFieldAddress(
    ConversionPatternRewriter& rewriter, Location loc, Value root, pto::StructType rootType, ArrayRef<int64_t> path)
{
    ...
    for (auto [depth, index] : llvm::enumerate(path)) {
        ...
        address = rewriter.create<LLVM::GEPOp>(
            loc, pointerType, storageType, address, ArrayRef<LLVM::GEPArg>{0, static_cast<int32_t>(index)});
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::TrapOp op, pto::TrapOp::Adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
[[maybe_unused]] static Value getI1Constant(OpBuilder& builder, Location loc, bool value)
{
    return builder.create<arith::ConstantOp>(loc, builder.getIntegerAttr(builder.getI1Type(), value ? 1 : 0))
        .getResult();
}
...
static FailureOr<StringRef> buildMadMxCalleeName(MLIRContext* context, Type lhsElem, Type rhsElem)
{
...
static bool isMadE4M3ElementType(Type type) { return pto::isPTOFloat8E4M3LikeType(type); }

static bool isMadE5M2ElementType(Type type) { return pto::isPTOFloat8E5M2LikeType(type); }
...
static FailureOr<StringRef> buildMadTypedCalleeName(MLIRContext* context, Type lhsElem, Type rhsElem, Type dstElem)
{
    ...
    if (isSignedOrSignlessInteger(dyn_cast<IntegerType>(lhsElem), 8) && rhs == "s8" && dst == "s32")
        return StringAttr::get(context, "llvm.hivm.MAD.s8.c310").getValue();
    if (isMadE4M3ElementType(lhsElem) && isMadE4M3ElementType(rhsElem) && dst == "f32")
        return StringAttr::get(context, "llvm.hivm.MAD.e4m3e4m3.c310").getValue();
    if (isMadE4M3ElementType(lhsElem) && isMadE5M2ElementType(rhsElem) && dst == "f32")
        return StringAttr::get(context, "llvm.hivm.MAD.e4m3e5m2.c310").getValue();
    if (isMadE5M2ElementType(lhsElem) && isMadE4M3ElementType(rhsElem) && dst == "f32")
        return StringAttr::get(context, "llvm.hivm.MAD.e5m2e4m3.c310").getValue();
    if (isMadE5M2ElementType(lhsElem) && isMadE5M2ElementType(rhsElem) && dst == "f32")
        return StringAttr::get(context, "llvm.hivm.MAD.e5m2e5m2.c310").getValue();
    if (pto::isPTOHiFloat8Type(lhsElem) && pto::isPTOHiFloat8Type(rhsElem) && dst == "f32")
        return StringAttr::get(context, "llvm.hivm.MAD.e4m3e4m3.c310").getValue();
...
static FailureOr<StringRef> buildLaneTypedCallee(
    MLIRContext* context, Type resultType, StringRef stem, StringRef suffix)
{
...
static FailureOr<StringRef> buildLaneTypedCalleeFromInput(
    MLIRContext* context, Type inputType, StringRef stem, StringRef suffix)
{
    std::string vec = getElementTypeFragment(getElementTypeFromVectorLike(inputType));
...
static std::optional<LowpPayloadABI> getLowpPayloadABI(Type elementType, MLIRContext* context)
{
...
static FailureOr<StringRef> buildDirectLowpVLogicCallee(
    MLIRContext* context, Type vectorType, StringRef stem, StringRef mode)
{
...
static FailureOr<StringRef> buildLowpPayloadVLogicCallee(
    MLIRContext* context, Type vectorType, StringRef stem, StringRef mode)
{
    Type elementType = getElementTypeFromVectorLike(vectorType);
    auto lanes = getElementCountFromVectorLike(vectorType);
    ...
    return StringAttr::get(
               context, "llvm.hivm." + stem.str() + ".v" + std::to_string(*lanes) +
                            abi->intrinsicElementFragment.str() + "." + mode.str())
        .getValue();
}
...
static Type getPayloadABIType(Type semanticType, Type convertedType, MLIRContext* context)
{
...
static Value castToPayloadABI(Location loc, Value value, Type semanticType, ConversionPatternRewriter& rewriter)
{
...
static Value castFromPayloadABI(
    Location loc, Value value, Type semanticType, Type convertedType, ConversionPatternRewriter& rewriter)
{
    Type carrierType = getLowpPayloadCarrierType(semanticType, rewriter.getContext());
...
static std::string getAtomicElementTypeFragment(Type type, Attribute signednessAttr)
{
    if (auto vecType = dyn_cast<VectorType>(type)) {
...
static std::string getReduxIntrinsicTypeFragment(Type type, Attribute signednessAttr)
{
    if (auto intType = dyn_cast<IntegerType>(type)) {
...
static FailureOr<Value> reinterpretPointerToAddrSpace(Operation* anchor, Value value, unsigned targetAddressSpace)
{
...
static FailureOr<Value> normalizeVdupScalarOperand(OpBuilder& builder, Location loc, Value input, Type resultType)
{
...
static Value normalizeByteScalarOperandForHivmCall(
    ...
    switch (src) {
        ...
        case VcvtElemKind::BF16:
            switch (dst) {
                ...
                case VcvtElemKind::F16:
                    return VcvtContract{"llvm.hivm.vcvtff.bf162f16.x", true, true, false, 16, true};
                case VcvtElemKind::F32:
...
static std::optional<uint64_t> parseLoadDistImmediate(StringRef dist, Type elementType)
{
...
static std::optional<uint64_t> parseLoadX2DistImmediate(StringRef dist, Type elementType)
{
    auto width = getDistElementWidth(elementType);
    if (dist == "BDINTLV")
        return 10;
...
static std::optional<uint64_t> parseStoreDistImmediate(StringRef dist, Type elementType)
{
    auto width = getDistElementWidth(elementType);
    if (dist.empty()) {
        if (!width)
...
static bool isOnePointStoreDist(StringRef dist) { return dist == "1PT_B8" || dist == "1PT_B16" || dist == "1PT_B32"; }
...
static std::optional<uint64_t> parseStoreX2DistImmediate(StringRef dist, Type elementType)
{
    auto width = getDistElementWidth(elementType);
    if (!width)
        return std::nullopt;
...
static Value packBlockRepeatStride(Operation* anchor, Value blockStride, Value repeatStride)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
...
static FailureOr<Value> packCopyGmToUbConfig0(Operation* anchor, ValueRange operands)
{
    if (operands.size() != 11)
        return failure();
    ...
    if (!sid || !nBurst || !lenBurst || !leftPadding || !rightPadding || !dataSelect || !cacheCtl)
        return failure();
...
static FailureOr<Value> packCopyGmToUbConfig1(Operation* anchor, ValueRange operands)
{
    if (operands.size() != 11)
        return failure();
...
static FailureOr<Value> packCopyGmToUbCfgV220(Operation* anchor, ValueRange operands)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
    ...
    Value cfg = sid;
    auto oneI64 = builder.create<arith::ConstantOp>(loc, builder.getI64IntegerAttr(1)).getResult();
    cfg = bitOr(cfg, shl(oneI64, 4));
    auto bytesPer32B = builder.create<arith::ConstantOp>(loc, builder.getI64IntegerAttr(5)).getResult();
    auto lenIn32B = builder.create<arith::ShRUIOp>(loc, lenBurst, bytesPer32B).getResult();
    cfg = bitOr(cfg, shl(lenIn32B, 16));
    return cfg;
}

[[maybe_unused]] static FailureOr<Value> packCopyGmToUbConfig0(
    Operation* anchor, Value sid, Value nBurst, Value lenBurst, Value leftPadding, Value rightPadding, Value dataSelect,
    Value cacheCtl)
{
...
static FailureOr<Value> packCopyUbToGmConfig0(Operation* anchor, ValueRange operands)
{
    if (operands.size() != 8)
        return failure();
...
static FailureOr<Value> packCopyUbToGmConfig1(Operation* anchor, ValueRange operands)
{
    if (operands.size() != 8)
        return failure();
...
static FailureOr<Value> packCopyUbToGmCfgV220(Operation* anchor, ValueRange operands)
{
    if (operands.size() != 8)
        return failure();
...
[[maybe_unused]] static FailureOr<Value> packCopyUbToGmConfig0(
    Operation* anchor, Value sid, Value nBurst, Value lenBurst, Value l2CacheCtl)
{
...
static FailureOr<Value> packCopyUbToUbConfig(Operation* anchor, ValueRange operands)
{
    if (operands.size() != 7)
        return failure();
    OpBuilder builder(anchor);
...
static FailureOr<Value> packCopyCbufToUbConfig(Operation* anchor, ValueRange operands)
{
    if (operands.size() != 7)
        return failure();
    OpBuilder builder(anchor);
...
static FailureOr<Value> packCopyUbToCbufConfig(Operation* anchor, ValueRange operands)
{
    if (operands.size() != 7)
        return failure();
    OpBuilder builder(anchor);
...
static FailureOr<Value> packCopyGmToCbufConfig0(Operation* anchor, Value nBurst, Value lenBurst)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packCopyGmToCbufConfig1(Operation* anchor, Value srcStride, Value dstStride)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packCopyGmToCbufMultiConfig0(
    Operation* anchor, Value sid, Value loop1SrcStride, Value l2CacheCtl, Value nValue)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packCopyGmToCbufMultiConfig1(
    Operation* anchor, Value dValue, Value loop4SrcStride, Value smallC0En)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packCopyCbufToBtConfig(
    Operation* anchor, Value convControl, Value nBurst, Value lenBurst, Value sourceGap, Value dstGap)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();

    Value convControlI64 = castIntegerLikeTo(anchor, convControl, builder.getI64Type());
    Value nBurstI64 = castIntegerLikeTo(anchor, nBurst, builder.getI64Type());
    Value lenBurstI64 = castIntegerLikeTo(anchor, lenBurst, builder.getI64Type());
    ...
    if (!convControlI64 || !nBurstI64 || !lenBurstI64 || !sourceGapI64 || !dstGapI64)
        return failure();
...
static FailureOr<Value> packCopyCbufToFbufConfig(
    Operation* anchor, Value nBurst, Value lenBurst, Value sourceGap, Value dstGap)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packLoadCbufToS4Config0(Operation* anchor, Value mStart, Value kStart, Value mStep, Value kStep)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packLoadCbufToS4Config1(Operation* anchor, Value srcStride, Value dstStride)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packLoadCbufToCaConfig0(Operation* anchor, Value mStart, Value kStart, Value mStep, Value kStep)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packLoadCbufToCaConfig1(Operation* anchor, Value srcStride, Value dstStride)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packLoadCbufToCbConfig0(Operation* anchor, Value mStart, Value kStart, Value mStep, Value kStep)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static FailureOr<Value> packLoadCbufToCbConfig1(Operation* anchor, Value srcStride, Value dstStride)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
static Value buildMadBiasDestination(Operation* anchor, ConversionPatternRewriter& rewriter, Value dst, Value bias)
{
...
static FailureOr<Value> convertElementOffsetToBytes(Operation* anchor, Value offset, Type elementType)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    ...
    Value scale = builder.create<arith::ConstantOp>(anchor->getLoc(), builder.getI32IntegerAttr(bitWidth / 8));
    return builder.create<arith::MulIOp>(anchor->getLoc(), offsetI32, scale).getResult();
}

[[maybe_unused]] static FailureOr<Value> materializeDynamicPltMask(
    ConversionPatternRewriter& rewriter, LoweringState& state, Location loc, Value laneCount, Type vectorElemType)
{
...
static FailureOr<StringRef> buildCarryBinaryCallee(MLIRContext* context, Type resultType, StringRef stem)
{
...
static FailureOr<StringRef> buildVselCallee(MLIRContext* context, Type resultType)
{
    ...
    if (vec.empty() || !lanes)
        return failure();
    return StringAttr::get(context, "llvm.hivm.vsel.v" + std::to_string(*lanes) + vec).getValue();
}
...
static FailureOr<StringRef> buildVselrCallee(MLIRContext* context, Type resultType)
{
    ...
    if (auto floatType = dyn_cast<FloatType>(elemType); floatType && floatType.isF32())
        vec = "u32";
    if (std::optional<LowpPayloadABI> abi = getLowpPayloadABI(elemType, context))
        vec = abi->intrinsicElementFragment.str();
    if (vec.empty())
        return failure();

    return StringAttr::get(context, "llvm.hivm.vselr.v" + std::to_string(*lanes) + vec).getValue();
}
...
static FailureOr<StringRef> buildVdupCallee(MLIRContext* context, pto::VdupOp op)
{
    ...
    if (isa<VectorType, pto::VRegType>(inputType)) {
        ...
        return StringAttr::get(context, "llvm.hivm." + family.str() + ".v" + std::to_string(*lanes) + vec + ".z")
            .getValue();
    }

    return StringAttr::get(context, "llvm.hivm.vdups.v" + std::to_string(*lanes) + vec + ".z").getValue();
}

static FailureOr<StringRef> buildVbrCallee(MLIRContext* context, Type semanticElementType)
{
...
static FailureOr<StringRef> buildVstusCallee(MLIRContext* context, Type valueType)
{
...
static FailureOr<StringRef> buildVstusPostCallee(MLIRContext* context, Type valueType)
{
    std::string vec = getMemoryElementTypeFragment(getElementTypeFromVectorLike(valueType));
    auto lanes = getElementCountFromVectorLike(valueType);
    if (vec.empty() || !lanes)
...
static FailureOr<StringRef> buildShuffleCallee(MLIRContext* context, Type valueType);

template <>
FailureOr<StringRef> buildShuffleCallee<pto::ShuffleIdxOp>(MLIRContext* context, Type valueType)
{
...
template <>
FailureOr<StringRef> buildShuffleCallee<pto::ShuffleUpOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getShuffleIntrinsicTypeFragment(valueType);
    if (elem.empty())
        return failure();
...
template <>
FailureOr<StringRef> buildShuffleCallee<pto::ShuffleDownOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getShuffleIntrinsicTypeFragment(valueType);
    if (elem.empty())
        return failure();
...
template <>
FailureOr<StringRef> buildShuffleCallee<pto::ShuffleBflyOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getShuffleIntrinsicTypeFragment(valueType);
    if (elem.empty())
        return failure();
...
static Value buildShuffleControlValue(
    OpBuilder& builder, Location loc, Value controlValue, int64_t widthValue, unsigned controlMask)
{
...
static FailureOr<StringRef> buildReduxCallee(MLIRContext* context, Type valueType, Attribute signednessAttr);

template <>
FailureOr<StringRef> buildReduxCallee<pto::ReduxAddOp>(MLIRContext* context, Type valueType, Attribute signednessAttr)
{
...
template <>
FailureOr<StringRef> buildReduxCallee<pto::ReduxMaxOp>(MLIRContext* context, Type valueType, Attribute signednessAttr)
{
    std::string elem = getReduxIntrinsicTypeFragment(valueType, signednessAttr);
    if (elem.empty())
        return failure();
...
template <>
FailureOr<StringRef> buildReduxCallee<pto::ReduxMinOp>(MLIRContext* context, Type valueType, Attribute signednessAttr)
{
    std::string elem = getReduxIntrinsicTypeFragment(valueType, signednessAttr);
    if (elem.empty())
        return failure();
...
static FailureOr<StringRef> buildAtomicCallee(
    MLIRContext* context, Type ptrType, Type valueType, Attribute signednessAttr);

static FailureOr<StringRef> buildAtomicCalleeName(
    MLIRContext* context, Type ptrType, Type valueType, Attribute signednessAttr, StringRef opName)
{
...
static FailureOr<StringRef> buildL1CacheLoadCallee(MLIRContext* context, Type resultType, pto::L1Cache l1cache)
{
    std::string elem;
...
static FailureOr<StringRef> buildL1CacheStoreCallee(MLIRContext* context, Type valueType, pto::L1Cache l1cache)
{
    std::string elem;
...
static FailureOr<StringRef> buildMulhiCallee(MLIRContext* context, Type resultType, pto::Signedness signedness)
{
    if (resultType.isInteger(32)) {
        return StringAttr::get(
                   context, signedness == pto::Signedness::Unsigned ? "llvm.hivm.mulhi.ui" : "llvm.hivm.mulhi.i")
            .getValue();
    }
...
static FailureOr<StringRef> buildMulI32ToI64Callee(MLIRContext* context, pto::Signedness signedness)
{
    return StringAttr::get(
               context,
               signedness == pto::Signedness::Unsigned ? "llvm.hivm.mul.i32toi64.ui" : "llvm.hivm.mul.i32toi64.i")
        .getValue();
}
...
template <typename UnaryOp>
static FailureOr<StringRef> buildUnaryScalarMathCallee(MLIRContext* context, Type valueType);

template <>
FailureOr<StringRef> buildUnaryScalarMathCallee<pto::AbsFOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getLLVMFloatBuiltinFragment(valueType);
...
template <>
FailureOr<StringRef> buildUnaryScalarMathCallee<pto::ExpOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getLLVMFloatBuiltinFragment(valueType);
    if (elem != "f32" && elem != "f16" && elem != "v2f16")
        return failure();
...
template <>
FailureOr<StringRef> buildUnaryScalarMathCallee<pto::LogOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getLLVMFloatBuiltinFragment(valueType);
    if (elem != "f32" && elem != "f16" && elem != "v2f16")
        return failure();
...
template <>
FailureOr<StringRef> buildUnaryScalarMathCallee<pto::CeilOp>(MLIRContext* context, Type valueType)
{
...
template <>
FailureOr<StringRef> buildUnaryScalarMathCallee<pto::FloorOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getScalarHIVMFloatShortFragment(valueType);
    if (elem.empty())
        return failure();
...
template <>
FailureOr<StringRef> buildUnaryScalarMathCallee<pto::RintOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getScalarHIVMFloatShortFragment(valueType);
    if (elem.empty())
        return failure();
...
template <>
FailureOr<StringRef> buildUnaryScalarMathCallee<pto::RoundOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getScalarHIVMFloatShortFragment(valueType);
    if (elem.empty())
        return failure();
...
template <typename BinaryOp>
static FailureOr<StringRef> buildBinaryScalarMathCallee(MLIRContext* context, Type valueType);

template <>
FailureOr<StringRef> buildBinaryScalarMathCallee<pto::FMinOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getLLVMFloatBuiltinFragment(valueType);
    if (elem != "f32" && elem != "bf16" && elem != "v2f16" && elem != "v2bf16")
        return failure();
    return StringAttr::get(context, "llvm.minnum." + elem).getValue();
}

template <>
FailureOr<StringRef> buildBinaryScalarMathCallee<pto::FMaxOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getLLVMFloatBuiltinFragment(valueType);
    if (elem != "f32" && elem != "bf16" && elem != "v2f16" && elem != "v2bf16")
        return failure();
...
template <>
FailureOr<StringRef> buildBinaryScalarMathCallee<pto::PowOp>(MLIRContext* context, Type valueType)
{
    std::string elem = getLLVMFloatBuiltinFragment(valueType);
    if (elem != "f32" && elem != "f16" && elem != "v2f16")
        return failure();
...
static std::string getConvertScalarFragment(Type type, Attribute signednessAttr)
{
    if (auto vecType = dyn_cast<VectorType>(type)) {
        if (vecType.getRank() != 1 || vecType.getDimSize(0) != 2)
            return {};
...
static FailureOr<StringRef> buildConvertCallee(
    MLIRContext* context, Type srcType, Type dstType, Attribute signednessAttr)
{
...
static FailureOr<StringRef> buildVldsPostCallee(MLIRContext* context, Type resultType)
{
...
static FailureOr<StringRef> buildVstsPostCallee(MLIRContext* context, Type valueType)
{
    std::string vec = getMemoryElementTypeFragment(getElementTypeFromVectorLike(valueType));
    auto lanes = getElementCountFromVectorLike(valueType);
    if (vec.empty() || !lanes)
...
static FailureOr<StringRef> buildVldusCallee(MLIRContext* context, Type resultType)
{
    std::string vec = getMemoryElementTypeFragment(getElementTypeFromVectorLike(resultType));
    auto lanes = getElementCountFromVectorLike(resultType);
    if (vec.empty() || !lanes)
...
static FailureOr<StringRef> buildVldusPostCallee(MLIRContext* context, Type resultType)
{
    std::string vec = getMemoryElementTypeFragment(getElementTypeFromVectorLike(resultType));
    auto lanes = getElementCountFromVectorLike(resultType);
    if (vec.empty() || !lanes)
...
static FailureOr<StringRef> buildVcmpCallee(
    MLIRContext* context, Type inputType, StringRef cmpMode, bool isScalarCompare)
{
    std::string elem = getElementTypeFragment(getElementTypeFromVectorLike(inputType));
    if (elem.empty())
        return failure();
    StringRef stem = isScalarCompare ? "vcmps" : "vcmp";
    return StringAttr::get(context, "llvm.hivm." + stem.str() + "." + cmpMode.str() + "." + elem + ".z").getValue();
}
...
template <typename ExtremaOp>
static FailureOr<StringRef> buildExtremaPredicateCallee(MLIRContext* context, Type resultType)
{
    return buildLaneTypedCallee(context, resultType, getExtremaPredicateStem<ExtremaOp>(), ".x");
}

static FailureOr<StringRef> buildCopyGmToUbCallee(
    MLIRContext* context, Type sourceType, const std::string& march, bool hasPadding)
{
    ...
    if (march == "dav-c220-vec") {
        if (hasPadding) {
            ...
            if (elem.empty())
                return failure();
            return StringAttr::get(context, "llvm.hivm.MOV.OUT.TO.UB.ALIGN.V2." + elem).getValue();
        }
...
static StringRef buildCopyUbToGmCallee(MLIRContext* context, const std::string& march)
{
    if (march == "dav-c220-vec")
        return StringAttr::get(context, "llvm.hivm.MOV.UB.TO.OUT.v220.1").getValue();
...
static FailureOr<StringRef> buildOrdinaryMadCallee(MLIRContext* context, pto::MadRawOpInterface op)
{
    ...
    return buildMadTypedCalleeName(
        context, lhsType.getElementType(), rhsType.getElementType(), dstType.getElementType());
}

static FailureOr<StringRef> buildMxMadCallee(MLIRContext* context, pto::MadRawOpInterface op)
{
    auto lhsType = dyn_cast<pto::PtrType>(op.getLhs().getType());
    auto rhsType = dyn_cast<pto::PtrType>(op.getRhs().getType());
    if (!lhsType || !rhsType)
...
static FailureOr<StringRef> buildCopyGmToCbufCallee(MLIRContext* context, Type sourceType)
{
    auto ptrType = dyn_cast<pto::PtrType>(sourceType);
    if (!ptrType)
        return failure();
...
static FailureOr<StringRef> buildCopyGmToCbufMultiNd2NzCallee(MLIRContext* context, Type sourceType)
{
    auto ptrType = dyn_cast<pto::PtrType>(sourceType);
    if (!ptrType)
        return failure();
...
static FailureOr<StringRef> buildCopyGmToCbufMultiDn2NzCallee(MLIRContext* context, Type sourceType)
{
    auto ptrType = dyn_cast<pto::PtrType>(sourceType);
    if (!ptrType)
        return failure();
...
static FailureOr<StringRef> buildLoadCbufToCaCallee(MLIRContext* context, Type sourceType)
{
    auto ptrType = dyn_cast<pto::PtrType>(sourceType);
    if (!ptrType)
        return failure();
...
static FailureOr<StringRef> buildLoadCbufToCbCallee(MLIRContext* context, Type sourceType)
{
    auto ptrType = dyn_cast<pto::PtrType>(sourceType);
    if (!ptrType)
        return failure();
...
static FailureOr<StringRef> buildLoadCbufToCaS4Callee(MLIRContext* context, Type sourceType)
{
    auto ptrType = dyn_cast<pto::PtrType>(sourceType);
    if (!ptrType)
        return failure();
...
static FailureOr<StringRef> buildLoadCbufToCbS4Callee(MLIRContext* context, Type sourceType)
{
    auto ptrType = dyn_cast<pto::PtrType>(sourceType);
    if (!ptrType)
        return failure();
...
[[maybe_unused]] static StringRef buildLoadCbufToCbMxCallee(MLIRContext* context)
{
...
static FailureOr<StringRef> buildCopyMatrixCcToUbCallee(MLIRContext* context, Type destinationType)
{
...
static StringRef buildPselCallee(MLIRContext* context) { return StringAttr::get(context, "llvm.hivm.psel").getValue(); }
...
static StringRef buildPorCallee(MLIRContext* context) { return StringAttr::get(context, "llvm.hivm.por.z").getValue(); }
...
static FailureOr<StringRef> buildInterleaveCallee(MLIRContext* context, Type resultType, StringRef stem)
{
    return buildLaneTypedCallee(context, resultType, stem, "");
}

static FailureOr<StringRef> buildUnpackCallee(MLIRContext* context, Type inputType, Type resultType, StringRef stem)
{
    ...
    if (input.empty() || result.empty())
        return failure();
    return StringAttr::get(context, "llvm.hivm." + stem.str() + "." + input + "2" + result).getValue();
}

static FailureOr<StringRef> buildVpackCallee(MLIRContext* context, Type inputType, Type resultType)
{
    std::string input = getElementTypeFragment(getElementTypeFromVectorLike(inputType));
    std::string result = getElementTypeFragment(getElementTypeFromVectorLike(resultType));
    if (input.empty() || result.empty())
    ...
    return StringAttr::get(context, "llvm.hivm.vpack." + input + "2" + result + ".x").getValue();
}
...
template <>
StringRef getPredicateStoreCallee<pto::PstiOp>(MLIRContext* context, bool post)
{
    return buildPstiCallee(context, post);
}

template <>
StringRef getPredicateStoreCallee<pto::PstsOp>(MLIRContext* context, bool post)
{
...
static FailureOr<StringRef> buildVldsx2Callee(MLIRContext* context, Type resultType, bool post)
{
    ...
    if (element.empty())
        return failure();
    return StringAttr::get(
               context, "llvm.hivm.vldsx2" + std::string(post ? ".post" : "") + ".v" + std::to_string(*lanes) + element)
        .getValue();
}

static FailureOr<StringRef> buildBlockStridedMemoryCallee(
    MLIRContext* context, Type vectorType, StringRef stem, bool post)
{
    Type elementType = getElementTypeFromVectorLike(vectorType);
    auto lanes = getElementCountFromVectorLike(vectorType);
    if (!elementType || !lanes)
    ...
    return StringAttr::get(
               context,
               "llvm.hivm." + stem.str() + std::string(post ? ".post" : "") + ".v" + std::to_string(*lanes) + element)
        .getValue();
}

static FailureOr<StringRef> buildVsldbCallee(MLIRContext* context, Type resultType, bool post)
{
...
static FailureOr<StringRef> buildVsstbCallee(MLIRContext* context, Type valueType, bool post)
{
...
static FailureOr<StringRef> buildVgather2Callee(MLIRContext* context, Type sourceType, Type resultType)
{
...
static FailureOr<Type> getVgather2OffsetsCarrierType(
    PatternRewriter& rewriter, Type sourceType, Type resultType, Type offsetsType)
{
    Type sourceElemType = getVgather2SourceElementType(sourceType);
    Type elementType = getElementTypeFromVectorLike(resultType);
    auto lanes = getElementCountFromVectorLike(resultType);
...
static FailureOr<StringRef> buildVgather2BcCallee(MLIRContext* context, Type resultType)
{
    return buildLaneTypedCallee(context, resultType, "vgather2.bc", "");
}

static FailureOr<StringRef> buildVgatherbCallee(MLIRContext* context, Type resultType)
{
    return buildLaneTypedCallee(context, resultType, "vgatherb.v310", "");
}

static FailureOr<StringRef> buildVscatterCallee(MLIRContext* context, Type valueType)
{
    return buildLaneTypedCallee(context, valueType, "vscatter", ".v300");
}

static FailureOr<Type> getVscatterOffsetsCarrierType(Type offsetsType) { return offsetsType; }

static FailureOr<StringRef> buildVaxpyCallee(MLIRContext* context, Type resultType)
{
    return buildLaneTypedCallee(context, resultType, "vaxpy", ".m");
}

static FailureOr<StringRef> buildVmulscvtCallee(MLIRContext* context, Type inputType, Type resultType)
{
...
static FailureOr<StringRef> buildVciCallee(MLIRContext* context, Type resultType)
{
    ...
    if (vec.empty() || !lanes)
        return failure();
    if (vec == "f16" || vec == "f32")
        return StringAttr::get(context, "llvm.hivm.vci.v" + std::to_string(*lanes) + vec + "." + vec).getValue();
    return StringAttr::get(context, "llvm.hivm.vci.v" + std::to_string(*lanes) + vec).getValue();
}
...
static FailureOr<StringRef> buildVexpdifCallee(MLIRContext* context, Type inputType, Type resultType)
{
    std::string srcVec = getElementTypeFragment(getElementTypeFromVectorLike(inputType));
    auto srcLanes = getElementCountFromVectorLike(inputType);
    std::string dstElem = getElementTypeFragment(getElementTypeFromVectorLike(resultType));
    if (srcVec.empty() || dstElem.empty() || !srcLanes)
        return failure();
    return StringAttr::get(context, "llvm.hivm.vexpdif.v" + std::to_string(*srcLanes) + srcVec + dstElem).getValue();
}

static FailureOr<StringRef> buildVbitsortCallee(MLIRContext* context, pto::VbitsortOp op)
{
...
static FailureOr<StringRef> buildVmrgsort4Callee(MLIRContext* context, pto::Vmrgsort4Op op)
{
...
static FailureOr<Value> packVmrgsort4SourceAddr(
    Operation* anchor, Value source0, Value source1, Value source2, Value source3, Type elemType)
{
    OpBuilder builder(anchor);
    builder.setInsertionPoint(anchor);
    Location loc = anchor->getLoc();
...
template <>
StringRef buildUnaryConfigCallee<pto::SetLoop2StrideOutToL1Op>(MLIRContext* context)
{
    return StringAttr::get(context, "llvm.hivm.SET.LOOP2.STRIDE.OUTTOL1").getValue();
}

template <>
StringRef buildUnaryConfigCallee<pto::SetLoop1StrideOutToL1Op>(MLIRContext* context)
{
    return StringAttr::get(context, "llvm.hivm.SET.LOOP1.STRIDE.OUTTOL1").getValue();
}

template <>
StringRef buildUnaryConfigCallee<pto::SetLoopSizeOutToL1Op>(MLIRContext* context)
{
...
template <>
StringRef buildUnaryConfigCallee<pto::SetPadValOutToL1Op>(MLIRContext* context)
{
...
template <>
StringRef buildUnaryConfigCallee<pto::SetStoreAtomicCfgOp>(MLIRContext* context)
{
...
static FailureOr<Value> encodeMovPadValue(Location loc, Value value, ConversionPatternRewriter& rewriter)
{
...
static uint64_t getDsbMemImmediate(DsbMem kind) { return static_cast<uint64_t>(kind); }

static uint64_t getDcciCacheLineImmediate(DcciCacheLine kind) { return static_cast<uint64_t>(kind); }

static uint64_t getDcciDstImmediate(DcciDst kind) { return static_cast<uint64_t>(kind); }

static StringRef buildDcciCallee(unsigned addressSpace, bool hasDst, MLIRContext* context)
{
...
template <>
StringRef buildSimtBlockQueryCallee<pto::GetBlockIdxOp>(MLIRContext* context)
{
    return StringAttr::get(context, "llvm.hivm.tpe.get.BLOCK.IDX").getValue();
}

template <>
StringRef buildSimtBlockQueryCallee<pto::GetBlockNumOp>(MLIRContext* context)
{
    return StringAttr::get(context, "llvm.hivm.tpe.get.BLOCK.NUM").getValue();
}

static LogicalResult materializeDecls(ModuleOp module, ArrayRef<PlannedDecl> plannedDecls, llvm::raw_ostream& diagOS)
{
...
public:
    ...
    LogicalResult matchAndRewrite(
        UnaryOp op, typename UnaryOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        StringRef stem = getUnaryMaskedStem<UnaryOp>();
        FailureOr<StringRef> calleeName = buildLaneTypedCallee(op.getContext(), op.getResult().getType(), stem, ".x");
        ...
        auto call =
            rewriter.create<func::CallOp>(op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{input, mask});
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VsqzOp op, pto::VsqzOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{input, mask, storeHint});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VusqzOp op, pto::VusqzOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto call =
            rewriter.create<func::CallOp>(op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{src, mask});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VmulaOp op, pto::VmulaOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{acc, lhs, rhs, mask});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VmullOp op, pto::VmullOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        BinaryOp op, typename BinaryOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if constexpr (
            std::is_same_v<BinaryOp, pto::VandOp> || std::is_same_v<BinaryOp, pto::VorOp> ||
            ...
            if (elementType && pto::isPTOLowPrecisionType(elementType)) {
                ...
                if (failed(calleeName)) {
                    Type carrierType = getLowpPayloadCarrierType(op.getResult().getType(), rewriter.getContext());
                    if (!carrierType)
                        return rewriter.notifyMatchFailure(op, "unsupported low-precision binary payload ABI");
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{callResultType}, ValueRange{callLhs, callRhs, mask});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), call.getCalleeType()});
        Value result =
            castFromPayloadABI(op.getLoc(), call.getResult(0), op.getResult().getType(), resultType, rewriter);
...
public:
    ...
    LogicalResult matchAndRewrite(
        TernaryOp op, typename TernaryOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        StringRef stem = getTernaryMaskedStem<TernaryOp>();
        FailureOr<StringRef> calleeName = buildLaneTypedCallee(op.getContext(), op.getResult().getType(), stem, ".m");
        if (failed(calleeName))
...
public:
    ...
    LogicalResult matchAndRewrite(
        CarryOp op, typename CarryOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    explicit LowerCopyOpPattern(
        TypeConverter& typeConverter, MLIRContext* context, LoweringState& state, const std::string& march)
        : OpConversionPattern<CopyOp>(typeConverter, context), state(state), march(march)
    {}

    LogicalResult matchAndRewrite(
        CopyOp op, typename CopyOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if constexpr (isGmUb)
            calleeName = buildCopyGmToUbCallee(op.getContext(), op.getSource().getType(), march, hasPadding);
        else
        ...
        SmallVector<Value> args{adaptor.getOperands()[1], adaptor.getOperands()[0], *config0};
        SmallVector<Type> argTypes{llvmDestType, llvmSourceType, rewriter.getI64Type()};
        if (!useSingleConfig) {
...
public:
    explicit LowerUBufBinaryOpPattern(TypeConverter& typeConverter, MLIRContext* context, LoweringState& state)
        : OpConversionPattern<UBOp>(typeConverter, context), state(state)
    {}

    LogicalResult matchAndRewrite(
        UBOp op, typename UBOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (elemFrag.empty())
            return rewriter.notifyMatchFailure(op, "unsupported element type for ubuf binary op");
        ...
        if (!dst || !src0 || !src1 || !isa<LLVM::LLVMPointerType>(dst.getType()) ||
            !isa<LLVM::LLVMPointerType>(src0.getType()) || !isa<LLVM::LLVMPointerType>(src1.getType()))
            return rewriter.notifyMatchFailure(op, "unexpected converted ubuf binary operand types");
        ...
        auto maskByte = [&](Value v) -> Value {
            return rewriter.create<arith::AndIOp>(
                loc, v, rewriter.create<arith::ConstantOp>(loc, rewriter.getI64IntegerAttr(0xff)));
        };
        auto shl = [&](Value v, uint64_t amount) -> Value {
            return rewriter.create<arith::ShLIOp>(
                loc, v, rewriter.create<arith::ConstantOp>(loc, rewriter.getI64IntegerAttr(amount)));
        };
        Value config = rewriter.create<arith::ConstantOp>(loc, rewriter.getI64IntegerAttr(1LL << 56));
        config = rewriter.create<arith::OrIOp>(loc, config, maskByte(getI64(adaptor.getRepeat())));
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getDstBlockStride())), 8));
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getSrc0BlockStride())), 16));
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getSrc1BlockStride())), 24));
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getDstRepeatStride())), 32));
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getSrc0RepeatStride())), 40));
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getSrc1RepeatStride())), 48));

        auto funcType = rewriter.getFunctionType(
            TypeRange{dst.getType(), src0.getType(), src1.getType(), rewriter.getI64Type()}, TypeRange{});
        auto call =
            rewriter.create<func::CallOp>(op.getLoc(), calleeName, TypeRange{}, ValueRange{dst, src0, src1, config});
        (void)call;
...
public:
    explicit LowerUBufShiftOpPattern(TypeConverter& typeConverter, MLIRContext* context, LoweringState& state)
        : OpConversionPattern<ShiftOp>(typeConverter, context), state(state)
    {}

    LogicalResult matchAndRewrite(
        ShiftOp op, typename ShiftOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto ptrType = mlir::cast<pto::PtrType>(op.getSrc().getType());
        Type elemType = ptrType.getElementType();
        std::string elemFrag = getElementTypeFragment(elemType);
        if (elemFrag.empty())
            return rewriter.notifyMatchFailure(op, "unsupported element type for ubuf shift op");
        ...
        Value dst = adaptor.getDst();
        Value src = adaptor.getSrc();
        if (!dst || !src || !isa<LLVM::LLVMPointerType>(dst.getType()) || !isa<LLVM::LLVMPointerType>(src.getType()))
            return rewriter.notifyMatchFailure(op, "unexpected converted ubuf shift operand types");
        ...
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getRepeat())), 56));
        config = rewriter.create<arith::OrIOp>(loc, config, maskByte(getI64(adaptor.getDstBlockStride())));
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getSrcBlockStride())), 16));
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getDstRepeatStride())), 32));
        config = rewriter.create<arith::OrIOp>(loc, config, shl(maskByte(getI64(adaptor.getSrcRepeatStride())), 40));

        Value shiftDist = getI64(adaptor.getShiftDist());

        if constexpr (std::is_same_v<ShiftOp, pto::UBVshlOp>) {
            auto funcType =
                rewriter.getFunctionType(TypeRange{dst.getType(), src.getType(), i64Ty, i64Ty}, TypeRange{});
            rewriter.create<func::CallOp>(loc, calleeName, TypeRange{}, ValueRange{dst, src, shiftDist, config});
            state.plannedDecls.push_back(PlannedDecl{calleeName, funcType});
        } else {
            Value roundZero = rewriter.create<arith::ConstantOp>(loc, rewriter.getI64IntegerAttr(0));
            auto funcType =
                rewriter.getFunctionType(TypeRange{dst.getType(), src.getType(), i64Ty, i64Ty, i64Ty}, TypeRange{});
            rewriter.create<func::CallOp>(
                loc, calleeName, TypeRange{}, ValueRange{dst, src, shiftDist, config, roundZero});
            state.plannedDecls.push_back(PlannedDecl{calleeName, funcType});
        }
...
public:
    explicit LowerUBufScalarBinaryPattern(TypeConverter& typeConverter, MLIRContext* context, LoweringState& state)
        : OpConversionPattern<ScalarOp>(typeConverter, context), state(state)
    {}

    LogicalResult matchAndRewrite(
        ScalarOp op, typename ScalarOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto ptrType = mlir::cast<pto::PtrType>(op.getSrc().getType());
        Type elemType = ptrType.getElementType();
        std::string elemFrag = getElementTypeFragment(elemType);
        if (elemFrag.empty())
            return rewriter.notifyMatchFailure(op, "unsupported element type for ubuf scalar mul op");
        ...
        Value src = adaptor.getSrc();
        if (!dst || !src || !isa<LLVM::LLVMPointerType>(dst.getType()) || !isa<LLVM::LLVMPointerType>(src.getType()))
            return rewriter.notifyMatchFailure(op, "unexpected converted ubuf scalar binary operand types");
        ...
        if (elemType.isF32() || elemType.isF16()) {
            ...
            Type floatTy = elemType.isF32() ? rewriter.getF32Type() : rewriter.getF16Type();
            ...
            auto funcType =
                rewriter.getFunctionType(TypeRange{dst.getType(), src.getType(), floatTy, i64Ty}, TypeRange{});
            rewriter.create<func::CallOp>(loc, calleeName, TypeRange{}, ValueRange{dst, src, scalarFloat, config});
            state.plannedDecls.push_back(PlannedDecl{calleeName, funcType});
        } else {
            // Integer: VMULS/VADDS/etc .s16/s32 takes i64 scalar directly.
            auto funcType =
                rewriter.getFunctionType(TypeRange{dst.getType(), src.getType(), i64Ty, i64Ty}, TypeRange{});
            rewriter.create<func::CallOp>(loc, calleeName, TypeRange{}, ValueRange{dst, src, scalarI64, config});
            state.plannedDecls.push_back(PlannedDecl{calleeName, funcType});
        }
...
public:
    explicit LowerUBufVdupPattern(TypeConverter& typeConverter, MLIRContext* context, LoweringState& state)
        : OpConversionPattern<pto::UBVdupOp>(typeConverter, context), state(state)
    {}

    LogicalResult matchAndRewrite(
        pto::UBVdupOp op, pto::UBVdupOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto funcType = rewriter.getFunctionType(TypeRange{dst.getType(), i64Ty, i64Ty}, TypeRange{});
        rewriter.create<func::CallOp>(loc, calleeName, TypeRange{}, ValueRange{dst, scalar, config});
        state.plannedDecls.push_back(PlannedDecl{calleeName, funcType});
...
public:
    explicit LowerUBufUnaryOpPattern(TypeConverter& typeConverter, MLIRContext* context, LoweringState& state)
        : OpConversionPattern<UnaryOp>(typeConverter, context), state(state)
    {}
        ...
        UnaryOp op, typename UnaryOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        std::string elemFrag = getElementTypeFragment(elemType);
        if (elemFrag.empty())
            return rewriter.notifyMatchFailure(op, "unsupported element type for ubuf unary op");
        ...
        if constexpr (std::is_same_v<UnaryOp, pto::UBVnotOp>)
            ...
            if (elemFrag == "u16" || elemFrag == "u32")
                return rewriter.notifyMatchFailure(op, "VRELU not available for unsigned integer types");
        ...
        Value src = adaptor.getSrc();
        if (!dst || !src || !isa<LLVM::LLVMPointerType>(dst.getType()) || !isa<LLVM::LLVMPointerType>(src.getType()))
            return rewriter.notifyMatchFailure(op, "unexpected converted ubuf unary operand types");
        ...
        auto funcType = rewriter.getFunctionType(TypeRange{dst.getType(), src.getType(), i64Ty}, TypeRange{});
        rewriter.create<func::CallOp>(loc, calleeName, TypeRange{}, ValueRange{dst, src, config});
        state.plannedDecls.push_back(PlannedDecl{calleeName, funcType});
...
public:
    explicit LowerUBSetMaskOpPattern(TypeConverter& typeConverter, MLIRContext* context, LoweringState& state)
        : OpConversionPattern<pto::UBSetMaskOp>(typeConverter, context), state(state)
    {}

    LogicalResult matchAndRewrite(
        pto::UBSetMaskOp op, typename pto::UBSetMaskOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        StringRef calleeName = "llvm.hivm.MOVEMASK";
        Location loc = op.getLoc();

        auto funcType = rewriter.getFunctionType(TypeRange{rewriter.getI64Type(), rewriter.getI64Type()}, TypeRange{});

        Value c0Idx = rewriter.create<arith::ConstantOp>(loc, rewriter.getI64IntegerAttr(0));
        rewriter.create<func::CallOp>(loc, calleeName, TypeRange{}, ValueRange{c0Idx, adaptor.getMask0()});

        Value c1Idx = rewriter.create<arith::ConstantOp>(loc, rewriter.getI64IntegerAttr(1));
        rewriter.create<func::CallOp>(loc, calleeName, TypeRange{}, ValueRange{c1Idx, adaptor.getMask1()});
...
public:
    explicit LowerUBSetMaskCountOpPattern(TypeConverter& typeConverter, MLIRContext* context)
        : OpConversionPattern<pto::UBSetMaskCountOp>(typeConverter, context)
    {}

    LogicalResult matchAndRewrite(
        pto::UBSetMaskCountOp op, typename pto::UBSetMaskCountOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        auto i64Ty = rewriter.getI64Type();
        Value ctrl = rewriter.create<pto::GetCtrlOp>(loc, i64Ty).getResult();
        Value bit56 = rewriter.create<arith::ConstantOp>(loc, rewriter.getI64IntegerAttr(56));
        Value set = rewriter.create<pto::Sbitset1Op>(loc, i64Ty, ctrl, bit56).getResult();
...
public:
    explicit LowerUBSetMaskNormOpPattern(TypeConverter& typeConverter, MLIRContext* context)
        : OpConversionPattern<pto::UBSetMaskNormOp>(typeConverter, context)
    {}

    LogicalResult matchAndRewrite(
        pto::UBSetMaskNormOp op, typename pto::UBSetMaskNormOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        auto i64Ty = rewriter.getI64Type();
        Value ctrl = rewriter.create<pto::GetCtrlOp>(loc, i64Ty).getResult();
        Value bit56 = rewriter.create<arith::ConstantOp>(loc, rewriter.getI64IntegerAttr(56));
        Value reset = rewriter.create<pto::Sbitset0Op>(loc, i64Ty, ctrl, bit56).getResult();
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::CopyUbufToUbufOp op, pto::CopyUbufToUbufOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        auto llvmSourceType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getOperands()[0].getType());
        auto llvmDestType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getOperands()[1].getType());
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::CopyCbufToUbufOp op, pto::CopyCbufToUbufOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::CopyUbufToCbufOp op, pto::CopyUbufToCbufOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        Value sourceRaw = adaptor.getSource();
        Value destinationRaw = adaptor.getDestination();
...
static LogicalResult lowerMadRawOp(
    pto::MadRawOpInterface op, ValueRange convertedOperands, ConversionPatternRewriter& rewriter, LoweringState& state)
{
    ...
    if (!lhsRaw || !rhsRaw || !dstRaw || !xt || (op.hasBiasOperand() && !biasRaw))
        return rewriter.notifyMatchFailure(op, "expected converted mad raw operands");

    if (!isa<LLVM::LLVMPointerType>(lhsRaw.getType()) || !isa<LLVM::LLVMPointerType>(rhsRaw.getType()) ||
        !isa<LLVM::LLVMPointerType>(dstRaw.getType()) || (biasRaw && !isa<LLVM::LLVMPointerType>(biasRaw.getType()))) {
    ...
    auto funcType =
        rewriter.getFunctionType(TypeRange{dst->getType(), lhs->getType(), rhs->getType(), i64Ty}, TypeRange{});
    auto call =
        rewriter.create<func::CallOp>(op->getLoc(), *calleeName, TypeRange{}, ValueRange{callDst, *lhs, *rhs, xt});
    state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
    rewriter.replaceOp(op, call.getResults());
    return success();
}
...
public:
    ...
    LogicalResult matchAndRewrite(
        RawOp op, typename RawOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::CopyGmToCbufOp op, pto::CopyGmToCbufOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        Value sourceRaw = adaptor.getSource();
        Value destinationRaw = adaptor.getDestination();
        ...
        rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{}, ValueRange{*destination, *source, *config0, *config1});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.eraseOp(op);
        return success();
    }
...
template <typename CopyOp>
class LowerCopyGmToCbufMultiOpPattern final : public OpConversionPattern<CopyOp> {
public:
        ...
        CopyOp op, typename CopyOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        FailureOr<Value> config0 = packCopyGmToCbufMultiConfig0(
            op, adaptor.getSid(), adaptor.getLoop1SrcStride(), adaptor.getL2CacheCtrl(), adaptor.getNValue());
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::CopyCbufToBtOp op, pto::CopyCbufToBtOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        Value sourceRaw = adaptor.getSource();
        Value destinationRaw = adaptor.getDestination();
        ...
        FailureOr<Value> config = packCopyCbufToBtConfig(
            op, adaptor.getConvControl(), adaptor.getNBurst(), adaptor.getLenBurst(), adaptor.getSourceGap(),
            adaptor.getDstGap());
        if (failed(config))
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::CopyCbufToFbufOp op, pto::CopyCbufToFbufOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        Value sourceRaw = adaptor.getSource();
        Value destinationRaw = adaptor.getDestination();
        ...
        FailureOr<Value> config = packCopyCbufToFbufConfig(
            op, adaptor.getNBurst(), adaptor.getLenBurst(), adaptor.getSourceGap(), adaptor.getDstGap());
        if (failed(config))
        ...
        auto funcType =
            rewriter.getFunctionType(TypeRange{destination->getType(), source->getType(), i64Ty}, TypeRange{});
        rewriter.create<func::CallOp>(op.getLoc(), calleeName, TypeRange{}, ValueRange{*destination, *source, *config});
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.eraseOp(op);
        return success();
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::LoadCbufToCaOp op, pto::LoadCbufToCaOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        Value sourceRaw = adaptor.getSource();
        Value destinationRaw = adaptor.getDestination();
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{destination->getType(), source->getType(), i64Ty, i64Ty, i64Ty}, TypeRange{});
        rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{}, ValueRange{*destination, *source, *config0, *config1, transpose});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.eraseOp(op);
        return success();
    }
...
public:
        ...
        LoadOp op, typename LoadOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        FailureOr<StringRef> calleeName = std::is_same_v<LoadOp, pto::LoadCbufToCaS4Op> ?
                                              buildLoadCbufToCaS4Callee(op.getContext(), op.getSource().getType()) :
                                              buildLoadCbufToCbS4Callee(op.getContext(), op.getSource().getType());
        if (failed(calleeName))
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::LoadCbufToCbOp op, pto::LoadCbufToCbOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        Value sourceRaw = adaptor.getSource();
        Value destinationRaw = adaptor.getDestination();
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::LoadCbufToCaMxOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        FailureOr<Value> config0 = packLoadCbufToCaConfig0(
            op, adaptor.getXStartPosition(), adaptor.getYStartPosition(), adaptor.getXStep(), adaptor.getYStep());
        ...
        rewriter.create<func::CallOp>(
            op.getLoc(), calleeName, TypeRange{}, ValueRange{dstAddr, *src, *config0, *config1});
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.eraseOp(op);
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::LoadCbufToCbMxOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Value srcRaw = adaptor.getSource();
        Value dstRaw = adaptor.getDestination();
        if (!srcRaw || !dstRaw || !adaptor.getXStartPosition() || !adaptor.getYStartPosition() || !adaptor.getXStep() ||
...
template <typename CopyOp>
class LowerCopyMatrixCcToBufOpPattern final : public OpConversionPattern<CopyOp> {
public:
...
template <typename VecScalarOp>
class LowerVecScalarMaskedOpPattern final : public OpConversionPattern<VecScalarOp> {
public:
    ...
    LogicalResult matchAndRewrite(
        VecScalarOp op, typename VecScalarOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{input, scalar, mask});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), call.getCalleeType()});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
template <typename ReductionOp>
class LowerReductionUnaryOpPattern final : public OpConversionPattern<ReductionOp> {
public:
    ...
    LogicalResult matchAndRewrite(
        ReductionOp op, typename ReductionOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        HistOp op, typename HistOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{resultType, sourceType, maskType, rewriter.getI32Type()}, TypeRange{resultType});
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), calleeName, TypeRange{resultType}, ValueRange{acc, source, mask, bin});
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
template <typename ExtremaOp>
class LowerExtremaPredicateOpPattern final : public OpConversionPattern<ExtremaOp> {
public:
    ...
    LogicalResult matchAndRewrite(
        ExtremaOp op, typename ExtremaOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        FailureOr<StringRef> calleeName =
            buildExtremaPredicateCallee<ExtremaOp>(op.getContext(), op.getValue().getType());
        if (failed(calleeName))
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{valueType, predicateType}, ValueRange{input, mask});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), call.getCalleeType()});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    explicit LowerWideningReductionUnaryOpPattern(
        TypeConverter& typeConverter, MLIRContext* context, LoweringState& state)
        : OpConversionPattern<ReductionOp>(typeConverter, context), state(state)
    {}
        ...
        ReductionOp op, typename ReductionOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        FailureOr<StringRef> calleeName = buildLaneTypedCalleeFromInput(
            op.getContext(), op.getInput().getType(), getReductionUnaryStem<ReductionOp>(), ".x");
        if (failed(calleeName))
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VselOp op, pto::VselOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{src0, src1, mask});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), call.getCalleeType()});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VdupOp op, pto::VdupOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (vectorInput) {
            ...
            Value scalarForCall =
                normalizeByteScalarOperandForHivmCall(rewriter, op.getLoc(), *normalizedScalar, scalarType);
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VbrOp op, pto::VbrOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        FailureOr<StringRef> calleeName =
            buildVbrCallee(op.getContext(), cast<pto::VRegType>(op.getResult().getType()).getElementType());
        if (failed(calleeName))
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VselrOp op, pto::VselrOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (auto floatType = dyn_cast<FloatType>(resultElementType); floatType && floatType.isF32()) {
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{intrinsicResultType}, ValueRange{src0, src1});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::PnotOp op, pto::PnotOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
...
class LowerInterleaveOpPattern final : public OpConversionPattern<InterleaveOp> {
public:
    ...
    LogicalResult matchAndRewrite(
        InterleaveOp op, typename InterleaveOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        PackOp op, typename PackOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
...
public:
    ...
    LogicalResult matchAndRewrite(
        UnpackOp op, typename UnpackOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto call =
            rewriter.create<func::CallOp>(op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{src, part});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VpackOp op, pto::VpackOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        FailureOr<StringRef> calleeName =
...
public:
    ...
    LogicalResult matchAndRewrite(
        PredicateMaskOp op, typename PredicateMaskOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
...
class LowerPredicatePairReorderOpPattern final : public OpConversionPattern<ReorderOp> {
public:
    explicit LowerPredicatePairReorderOpPattern(
        TypeConverter& typeConverter, MLIRContext* context, LoweringState& state)
        : OpConversionPattern<ReorderOp>(typeConverter, context), state(state)
    {}

    LogicalResult matchAndRewrite(
        ReorderOp op, typename ReorderOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        SmallVector<Type> resultTypes;
...
public:
    ...
    LogicalResult matchAndRewrite(
        CmpOp op, typename CmpOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        PltOp op, typename PltOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        PltmOp op, typename PltmOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        SmallVector<Type> resultTypes;
        if (failed(this->getTypeConverter()->convertTypes(op->getResultTypes(), resultTypes)))
...
public:
    ...
    LogicalResult matchAndRewrite(
        PsetOp op, typename PsetOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
...
public:
    ...
    LogicalResult matchAndRewrite(
        PgeOp op, typename PgeOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        auto pattern = parsePredicatePatternImmediate(op.getPattern());
        if (!pattern)
...
public:
    ...
    LogicalResult matchAndRewrite(pto::VldsOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Type callValueType = getPayloadABIType(ptoResultType, resultTypes[0], rewriter.getContext());
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                adaptor.getSource().getType(), (*offsetBytes).getType(), distValue.getType(), postValue.getType()},
            callResultTypes);
        auto call = rewriter.create<func::CallOp>(op.getLoc(), *calleeName, callResultTypes, args);
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        Value loaded = castFromPayloadABI(op.getLoc(), call.getResult(0), ptoResultType, resultTypes[0], rewriter);
        if (usePostIntrinsic)
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::Vldsx2Op op, pto::Vldsx2Op::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value high =
            castFromPayloadABI(op.getLoc(), call.getResult(1), op.getHigh().getType(), resultTypes[1], rewriter);
        if (usePostIntrinsic)
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VsldbOp op, pto::VsldbOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto basePtr = dyn_cast<LLVM::LLVMPointerType>(adaptor.getSource().getType());
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                adaptor.getSource().getType(), packedStride.getType(), postValue.getType(),
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::InitAlignOp op, pto::InitAlignOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VldasOp op, pto::VldasOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), calleeName, TypeRange{resultType}, ValueRange{adaptor.getSource()});
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VldusOp op, pto::VldusOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto sourceType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getSource().getType());
        SmallVector<Type> resultTypes;
        bool usePostIntrinsic = static_cast<bool>(op.getUpdatedBase());
        ...
        FailureOr<StringRef> calleeName = usePostIntrinsic ?
        ...
        if (usePostIntrinsic) {
            Type elementType = getElementTypeFromVectorLike(op.getResult().getType());
            auto incrementBytes = convertElementOffsetToBytes(op, adaptor.getIncrement(), elementType);
        ...
        auto call = rewriter.create<func::CallOp>(op.getLoc(), *calleeName, intrinsicResultTypes, args);
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        Value loaded =
            castFromPayloadABI(op.getLoc(), call.getResult(0), op.getResult().getType(), resultTypes[0], rewriter);
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::SprclrOp op, pto::SprclrOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
...
public:
    ...
    LogicalResult matchAndRewrite(
        SprStoreOp op, typename SprStoreOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto spr = parseSprImmediate(op.getSpr());
        if (!spr)
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                sprValue.getType(), adaptor.getDestination().getType(), adaptor.getOffset().getType(),
                postValue.getType()},
            resultTypes);
...
public:
    ...
    LogicalResult matchAndRewrite(pto::VstsOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto offsetBytes = convertElementOffsetToBytes(op, adaptor.getOffset(), offsetElementType);
        ...
        Value distValue = rewriter.create<arith::ConstantOp>(op.getLoc(), rewriter.getI32IntegerAttr(*dist));
        Value zero =
            rewriter.create<arith::ConstantOp>(op.getLoc(), rewriter.getI32IntegerAttr(usePostIntrinsic ? 1 : 0));
        ...
        SmallVector<Value> args{value, adaptor.getDestination(), *offsetBytes, distValue, zero, mask};
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                value.getType(), adaptor.getDestination().getType(), rewriter.getI32Type(), rewriter.getI32Type(),
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VsstbOp op, pto::VsstbOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto basePtr = dyn_cast<LLVM::LLVMPointerType>(adaptor.getDestination().getType());
        Value packedStride = packBlockRepeatStride(op, adaptor.getBlockStride(), adaptor.getRepeatStride());
        if (!basePtr || !packedStride)
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                value.getType(), adaptor.getDestination().getType(), packedStride.getType(), zeroValue.getType(),
                adaptor.getMask().getType()},
            resultTypes);
        auto call = rewriter.create<func::CallOp>(op.getLoc(), *calleeName, resultTypes, args);
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::Vstsx2Op op, pto::Vstsx2Op::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type elementType = getElementTypeFromVectorLike(op.getLow().getType());
        if (!elementType)
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                low.getType(), high.getType(), adaptor.getDestination().getType(), (*offsetBytes).getType(),
                distValue.getType(), zeroValue.getType(), adaptor.getMask().getType()},
            TypeRange{});
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::PstuOp op, pto::PstuOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VstusOp op, pto::VstusOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type elementType = getElementTypeFromVectorLike(op.getValue().getType());
        if (!elementType)
        ...
        Value value = castToPayloadABI(op.getLoc(), adaptor.getValue(), op.getValue().getType(), rewriter);
        SmallVector<Value> args{value, adaptor.getBase(), *offsetBytes, adaptor.getAlignIn()};
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                value.getType(), adaptor.getBase().getType(), (*offsetBytes).getType(), adaptor.getAlignIn().getType()},
            resultTypes);
        auto call = rewriter.create<func::CallOp>(op.getLoc(), *calleeName, resultTypes, args);
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.replaceOp(op, call.getResults());
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VsturOp op, pto::VsturOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                value.getType(), adaptor.getBase().getType(), adaptor.getAlignIn().getType(), modeValue.getType(),
                zeroValue.getType()},
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VstarOp op, pto::VstarOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{adaptor.getValue().getType(), adaptor.getDestination().getType(), zeroValue.getType()},
            TypeRange{});
        rewriter.create<func::CallOp>(op.getLoc(), calleeName, TypeRange{}, args);
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.eraseOp(op);
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VstasOp op, pto::VstasOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto baseType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getDestination().getType());
        Type alignType = this->getTypeConverter()->convertType(op.getValue().getType());
        ...
        auto offsetBytes = convertElementOffsetToBytes(op, adaptor.getOffset(), dstType.getElementType());
        if (failed(offsetBytes))
        ...
        Value postValue = getI32Constant(rewriter, op.getLoc(), usePostIntrinsic ? 1 : 0);
        SmallVector<Value> args{adaptor.getValue(), adaptor.getDestination(), *offsetBytes, postValue};
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                adaptor.getValue().getType(), adaptor.getDestination().getType(), (*offsetBytes).getType(),
                postValue.getType()},
            resultTypes);
        auto call = rewriter.create<func::CallOp>(op.getLoc(), calleeName, resultTypes, args);
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::Vgather2Op op, pto::Vgather2Op::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        FailureOr<Type> offsetsCarrierType = getVgather2OffsetsCarrierType(
            rewriter, op.getSource().getType(), op.getResult().getType(), offsets.getType());
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::Vgather2BcOp op, pto::Vgather2BcOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto basePtr = dyn_cast<LLVM::LLVMPointerType>(adaptor.getSource().getType());
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VgatherbOp op, pto::VgatherbOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto basePtr = dyn_cast<LLVM::LLVMPointerType>(adaptor.getSource().getType());
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!basePtr || !resultType)
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VscatterOp op, pto::VscatterOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                adaptor.getValue().getType(), adaptor.getDestination().getType(), *offsetsCarrierType,
                adaptor.getMask().getType()},
            TypeRange{});
        rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{},
            ValueRange{adaptor.getValue(), adaptor.getDestination(), adaptor.getOffsets(), adaptor.getMask()});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType, true});
        rewriter.eraseOp(op);
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VaxpyOp op, pto::VaxpyOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type elemType = getElementTypeFromVectorLike(op.getResult().getType());
        if (!elemType)
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                adaptor.getSrc1().getType(), adaptor.getSrc0().getType(), adaptor.getAlpha().getType(),
                adaptor.getMask().getType()},
            TypeRange{resultType});
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType},
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VmulscvtOp op, pto::VmulscvtOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                adaptor.getInput().getType(), adaptor.getScalar().getType(), adaptor.getMask().getType(),
                partValue.getType()},
            TypeRange{resultType});
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType},
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VciOp op, pto::VciOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Type resultElemType = cast<pto::VRegType>(op.getResult().getType()).getElementType();
        if (auto intType = dyn_cast<IntegerType>(resultElemType)) {
            if (intType.getWidth() == 8) {
                Type loweredIndexType = rewriter.getI16Type();
                if (intType.isUnsigned())
                    indexValue = rewriter.create<arith::ExtUIOp>(op.getLoc(), loweredIndexType, indexValue);
                else
                    indexValue = rewriter.create<arith::ExtSIOp>(op.getLoc(), loweredIndexType, indexValue);
            }
        }
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{indexValue, orderValue});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VexpdifOp op, pto::VexpdifOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto part = parsePartImmediate(op.getPart());
        if (!part)
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                adaptor.getInput().getType(), adaptor.getMax().getType(), adaptor.getMask().getType(),
                partValue.getType()},
            TypeRange{resultType});
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType},
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VbitsortOp op, pto::VbitsortOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                adaptor.getDestination().getType(), adaptor.getSource().getType(), adaptor.getIndices().getType(),
                (*config).getType()},
            TypeRange{});
        rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{},
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::Vmrgsort4Op op, pto::Vmrgsort4Op::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto dstType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getDestination().getType());
        ...
        FailureOr<Value> packedSrc = packVmrgsort4SourceAddr(
            op, adaptor.getSource0(), adaptor.getSource1(), adaptor.getSource2(), adaptor.getSource3(), elemType);
        if (failed(packedSrc))
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{
                (*dst).getType(), (*packedSrc).getType(), adaptor.getCount().getType(), adaptor.getConfig().getType()},
            TypeRange{});
        rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{},
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VcvtOp op, pto::VcvtOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto appendRndArg = [&]() -> LogicalResult {
            auto roundMode = op.getRndAttr() ? parseRoundModeImmediate(*op.getRnd()) : std::nullopt;
            if (!roundMode)
        ...
        auto appendSatArg = [&]() -> LogicalResult {
            auto saturation = op.getSatAttr() ? parseSaturationImmediate(*op.getSat()) : std::nullopt;
            if (!saturation)
        ...
        if ((*contract).requiresPart) {
            auto part = op.getPartAttr() ? parseVcvtPartImmediate(*op.getPart()) : std::nullopt;
            if (!part)
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VbitcastOp op, pto::VbitcastOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::PbitcastOp op, pto::PbitcastOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::VtrcOp op, pto::VtrcOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        StoreOp op, typename StoreOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (!offset)
            return rewriter.notifyMatchFailure(op, "failed to convert predicate-store offset to i32");
...
public:
        ...
        LoadOp op, typename LoadOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        Value offset = castIntegerLikeTo(op, adaptor.getOffset(), rewriter.getI32Type());
        if (!offset)
            return rewriter.notifyMatchFailure(op, "failed to convert predicate-load offset to i32");
        ...
        args.push_back(
            rewriter.create<arith::ConstantOp>(op.getLoc(), rewriter.getI32IntegerAttr(usePostIntrinsic ? 1 : 0)));
        auto funcType = rewriter.getFunctionType(
            TypeRange{llvmSourceType, rewriter.getI32Type(), rewriter.getI32Type(), rewriter.getI32Type()},
            resultTypes);
        auto call = rewriter.create<func::CallOp>(op.getLoc(), calleeName, resultTypes, args);
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.replaceOp(op, call.getResults());
...
public:
    ...
    LogicalResult matchAndRewrite(
        LoopOp op, typename LoopOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        FailureOr<Value> packed = failure();
        if constexpr (
            std::is_same_v<LoopOp, pto::SetLoopSizeOutToUbOp> || std::is_same_v<LoopOp, pto::SetLoopSizeUbToOutOp>) {
...
public:
    ...
    LogicalResult matchAndRewrite(
        ConfigOp op, typename ConfigOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::StoreVfSimtInfoOp op, pto::StoreVfSimtInfoOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        Location loc = op.getLoc();
...
public:
    ...
    LogicalResult matchAndRewrite(
        FenceOp op, typename FenceOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
...
static std::string buildSimtKeepResumeConstraints(ArrayRef<SimtKeepResumePhysicalRegister> physicalRegs, bool tieInputs)
{
    std::string result;
...
static Value unpackSimtKeepResumePayload(
    Location loc, Value value, Type resultType, ConversionPatternRewriter& rewriter)
{
...
static FailureOr<SmallVector<SimtKeepResumePhysicalRegister, 4>> computeSimtKeepResumePhysicalRegs(
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::KeepOp op, pto::KeepOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        ...
        for (pto::KeepOp keep : keepOps) {
            ...
            if (!isValidSimtKeepResumeSlot(slot, registerCount))
                return rewriter.notifyMatchFailure(
                    keep, "slot must be in range [0, 122] and 64-bit slots must be even");
        ...
        rewriter.create<LLVM::InlineAsmOp>(
            op.getLoc(), TypeRange{asmResultType}, payloads, "", buildSimtKeepResumeConstraints(*physicalRegs, true),
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::ResumeOp op, pto::ResumeOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        if (hasPreviousSameOp(op.getOperation()))
        ...
        for (pto::ResumeOp resume : resumeOps) {
            ...
            if (!resultType || !getSimtKeepResumeBitWidth(resultType))
                return rewriter.notifyMatchFailure(resume, "expected integer scalar up to 64 bits or f16/bf16/f32");
            int64_t slot = resume.getSlot();
            unsigned registerCount = getSimtKeepResumeRegisterCount(resultType);
            if (!isValidSimtKeepResumeSlot(slot, registerCount))
                return rewriter.notifyMatchFailure(
                    resume, "slot must be in range [0, 122] and 64-bit slots must be even");
            logicalSlots.push_back({slot, registerCount});
            asmResultTypes.push_back(rewriter.getIntegerType(*getSimtKeepResumeBitWidth(resultType) > 32 ? 64 : 32));
        }
        FailureOr<SmallVector<SimtKeepResumePhysicalRegister, 4>> physicalRegs =
            computeSimtKeepResumePhysicalRegs(logicalSlots);
        if (failed(physicalRegs))
        ...
        auto asmOp = rewriter.create<LLVM::InlineAsmOp>(
            ...
            LLVM::AsmDialectAttr::get(op.getContext(), LLVM::AsmDialect::AD_ATT), ArrayAttr{});
        ...
        for (auto [index, resume] : llvm::enumerate(resumeOps)) {
            auto extract = rewriter.create<LLVM::ExtractValueOp>(
                resume.getLoc(), asmOp.getRes(), ArrayRef<int64_t>{static_cast<int64_t>(index)});
            Type resultType = getTypeConverter()->convertType(resume.getType());
...
public:
    ...
    LogicalResult matchAndRewrite(
        SyncOp op, typename SyncOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
...
public:
        ...
        SyncOp op, typename SyncOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (IntegerAttr eventIdAttr = op.getEventIdAttr()) {
            ...
            if (!eventIdDyn)
                return rewriter.notifyMatchFailure(op, "expected static or dynamic event-id operand");

            eventValue = castIntegerLikeTo(op, eventIdDyn, rewriter.getI64Type());
            if (!eventValue) {
                return rewriter.notifyMatchFailure(op, "failed to cast dynamic event-id to i64");
            }
        }

        StringRef calleeName = buildSyncCallee<SyncOp>(op.getContext());
        auto funcType = rewriter.getFunctionType(TypeRange{rewriter.getI64Type(), rewriter.getI64Type()}, TypeRange{});
        rewriter.create<func::CallOp>(op.getLoc(), calleeName, TypeRange{}, ValueRange{pipeValue, eventValue});
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.eraseOp(op);
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::MemBarOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
...
public:
    ...
    LogicalResult matchAndRewrite(
        MemoryConsistencyOp op, typename MemoryConsistencyOp::Adaptor adaptor,
        ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        (void)rewriter;
...
public:
    ...
    LogicalResult matchAndRewrite(
        BufSyncOp op, typename BufSyncOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{rewriter.getI64Type(), rewriter.getI64Type(), rewriter.getI64Type()}, TypeRange{});
        rewriter.create<func::CallOp>(
            op.getLoc(), calleeName, TypeRange{}, ValueRange{pipeValue, bufIdValue, modeValue});
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.eraseOp(op);
        return success();
    }
...
class LowerBufDynSyncOpPattern final : public OpConversionPattern<BufDynSyncOp> {
public:
    ...
    LogicalResult matchAndRewrite(
        BufDynSyncOp op, typename BufDynSyncOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        PIPE pipe = PIPE::PIPE_UNASSIGNED;
        if (auto pipeAttr = dyn_cast<PipeAttr>(op.getOpTypeAttr())) {
            pipe = pipeAttr.getPipe();
...
public:
    ...
    LogicalResult matchAndRewrite(
        QueryOp op, typename QueryOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
...
template <typename QueryOp>
class LowerBlockRuntimeQueryOpPattern final : public OpConversionPattern<QueryOp> {
public:
...
public:
    ...
    LogicalResult matchAndRewrite(
        VoteOp op, typename VoteOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), calleeName, TypeRange{resultType}, ValueRange{adaptor.getPred()});
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        ShuffleOp op, typename ShuffleOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{adaptor.getValue(), control});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        ReduxOp op, typename ReduxOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        if (!resultType)
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{adaptor.getValue()});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        AtomicOp op, typename AtomicOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::AtomicCasOp op, pto::AtomicCasOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getOld().getType());
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{ptrType, compareType, valueType, rewriter.getI32Type()}, TypeRange{resultType});
        Value modeValue = getI32Constant(
            rewriter, op.getLoc(),
            static_cast<uint64_t>(op.getL2cacheAttr() ? op.getL2cacheAttr().getValue() : pto::StL2Cache::NMFV));
        auto call = rewriter.create<func::CallOp>(
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::MulhiOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (succeeded(calleeName)) {
            ...
            auto call = rewriter.create<func::CallOp>(
                op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{adaptor.getLhs(), adaptor.getRhs()});
            state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
            rewriter.replaceOp(op, call.getResults());
            return success();
        }
        ...
        auto unsignedCall = rewriter.create<func::CallOp>(
            op.getLoc(), *unsignedCalleeName, TypeRange{resultType}, ValueRange{adaptor.getLhs(), adaptor.getRhs()});
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::MulI32ToI64Op op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = getTypeConverter()->convertType(op.getResult().getType());
        Type lhsType = getTypeConverter()->convertType(op.getLhs().getType());
        Type rhsType = getTypeConverter()->convertType(op.getRhs().getType());
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::SqrtOp op, pto::SqrtOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        Type valueType = this->getTypeConverter()->convertType(op.getValue().getType());
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::FmaOp op, pto::FmaOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = this->getTypeConverter()->convertType(op.getResult().getType());
        Type lhsType = this->getTypeConverter()->convertType(op.getLhs().getType());
        Type rhsType = this->getTypeConverter()->convertType(op.getRhs().getType());
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::ConvertOp op, pto::ConvertOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{resultType}, ValueRange{adaptor.getSrc(), rounding, saturation});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::GetVms4SrOp op, pto::GetVms4SrOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        SmallVector<Type> resultTypes;
        if (failed(this->getTypeConverter()->convertTypes(op->getResultTypes(), resultTypes)) ||
        ...
        auto call =
            rewriter.create<func::CallOp>(op.getLoc(), calleeName, TypeRange{rewriter.getI64Type()}, ValueRange{});
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        ...
        for (unsigned i = 0; i < 4; ++i) {
            ...
            if (i != 0)
                shifted =
                    rewriter.create<arith::ShRUIOp>(op.getLoc(), raw, getI64Constant(rewriter, op.getLoc(), i * 16));
...
public:
        ...
        BinaryOp op, typename BinaryOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{adaptor.getFirst().getType(), adaptor.getSecond().getType()}, TypeRange{resultType});
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), calleeName, TypeRange{resultType}, ValueRange{adaptor.getFirst(), adaptor.getSecond()});
        state.plannedDecls.push_back(PlannedDecl{calleeName.str(), funcType});
        rewriter.replaceOp(op, call.getResults());
        return success();
    }
...
class ConvertPtoTileBufAddrOp final : public OpConversionPattern<pto::TileBufAddrOp> {
public:
    using OpConversionPattern::OpConversionPattern;

    LogicalResult matchAndRewrite(
        pto::TileBufAddrOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (isa<MemRefType>(input.getType())) {
            Value alignedIdx =
                rewriter.create<memref::ExtractAlignedPointerAsIndexOp>(op.getLoc(), rewriter.getIndexType(), input);
            Value i64 = rewriter.create<arith::IndexCastUIOp>(op.getLoc(), rewriter.getI64Type(), alignedIdx);
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::DeclareStructOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        auto resultType = dyn_cast<LLVM::LLVMPointerType>(getTypeConverter()->convertType(op.getS().getType()));
        if (!resultType)
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::StructGetOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type resultType = getTypeConverter()->convertType(op.getValue().getType());
        if (!resultType)
            return rewriter.notifyMatchFailure(op, "could not convert result type");
        FailureOr<Value> address = getVPTOStructFieldAddress(
            rewriter, op.getLoc(), adaptor.getS(), cast<pto::StructType>(op.getS().getType()), op.getPath());
        if (failed(address))
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::StructSetOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        FailureOr<Value> address = getVPTOStructFieldAddress(
            rewriter, op.getLoc(), adaptor.getS(), cast<pto::StructType>(op.getS().getType()), op.getPath());
        if (failed(address))
        ...
        rewriter.replaceOpWithNewOp<LLVM::StoreOp>(
            op, adaptor.getValue(), *address, getNaturalByteAlignment(adaptor.getValue().getType()));
        return success();
    }
};
...
public:
        ...
        arith::SelectOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        rewriter.replaceOpWithNewOp<arith::SelectOp>(
            op, convertedResultType, adaptor.getCondition(), trueValue, falseValue);
        return success();
    }
};
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::AddPtrOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type convertedResultType = getTypeConverter()->convertType(op.getResult().getType());
        auto llvmPtrType = dyn_cast<LLVM::LLVMPointerType>(convertedResultType);
        if (!llvmPtrType)
        ...
        auto gep = rewriter.create<LLVM::GEPOp>(
            ...
            normalizeGEPElementTypeForLLVMLowering(
                cast<pto::PtrType>(op.getPtr().getType()).getElementType(), rewriter),
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::CastPtrOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type convertedResultType = getTypeConverter()->convertType(op.getResult().getType());
        if (!convertedResultType)
        ...
        if (auto llvmPtrType = dyn_cast<LLVM::LLVMPointerType>(convertedResultType)) {
            ...
            if (isa<MemRefType>(inputType)) {
                Value alignedIdx = rewriter.create<memref::ExtractAlignedPointerAsIndexOp>(
        ...
        pto::LoadScalarOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (!matchPattern(offset, m_Zero())) {
            elemPtr = rewriter.create<LLVM::GEPOp>(
                op.getLoc(), llvmPtrType, normalizeGEPElementTypeForLLVMLowering(convertedValueType, rewriter),
                adaptor.getPtr(), ValueRange{offset});
        }

        rewriter.replaceOpWithNewOp<LLVM::LoadOp>(
            op, convertedValueType, elemPtr, getNaturalByteAlignment(convertedValueType));
        return success();
    }
};
...
public:
        ...
        pto::StoreScalarOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (!matchPattern(offset, m_Zero())) {
            elemPtr = rewriter.create<LLVM::GEPOp>(
                op.getLoc(), llvmPtrType,
                normalizeGEPElementTypeForLLVMLowering(adaptor.getValue().getType(), rewriter), adaptor.getPtr(),
                ValueRange{offset});
        }

        rewriter.create<LLVM::StoreOp>(
            op.getLoc(), adaptor.getValue(), elemPtr, getNaturalByteAlignment(adaptor.getValue().getType()));
        rewriter.eraseOp(op);
        return success();
    }
};
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::PTOLoadOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto llvmPtrType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getPtr().getType());
        if (!llvmPtrType)
            return rewriter.notifyMatchFailure(op, "expected LLVM pointer operand");
        ...
        if (!matchPattern(offset, m_Zero())) {
            elemPtr = rewriter.create<LLVM::GEPOp>(
                op.getLoc(), llvmPtrType, convertedValueType, adaptor.getPtr(), ValueRange{offset});
        }
...
static Value convertLdgCallResult(
    Location loc, Type valueType, Type convertedValueType, Value callResult, ConversionPatternRewriter& rewriter)
{
    if (auto intType = dyn_cast<IntegerType>(valueType)) {
        unsigned width = intType.getWidth();
        if (width == 8 || width == 16)
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::PTOLdgOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto llvmPtrType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getPtr().getType());
        if (!llvmPtrType)
            return rewriter.notifyMatchFailure(op, "expected LLVM pointer operand");
        ...
        auto call = rewriter.create<func::CallOp>(
            op.getLoc(), *calleeName, TypeRange{callResultType}, ValueRange{*ptr, modeValue});
        state.plannedDecls.push_back(PlannedDecl{calleeName->str(), funcType});
        Value result =
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::PTOStoreOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto llvmPtrType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getPtr().getType());
        if (!llvmPtrType)
            return rewriter.notifyMatchFailure(op, "expected LLVM pointer operand");
        ...
        if (!matchPattern(offset, m_Zero())) {
            elemPtr = rewriter.create<LLVM::GEPOp>(
                op.getLoc(), llvmPtrType, adaptor.getValue().getType(), adaptor.getPtr(), ValueRange{offset});
        }

        rewriter.replaceOpWithNewOp<LLVM::StoreOp>(
            op, adaptor.getValue(), elemPtr, getNaturalByteAlignment(adaptor.getValue().getType()));
        return success();
    }
};
...
public:
    ...
    LogicalResult matchAndRewrite(
        pto::PTOStgOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto llvmPtrType = dyn_cast<LLVM::LLVMPointerType>(adaptor.getPtr().getType());
        if (!llvmPtrType)
            return rewriter.notifyMatchFailure(op, "expected LLVM pointer operand");
        ...
        auto funcType = rewriter.getFunctionType(
            TypeRange{ptr->getType(), storedValue.getType(), rewriter.getI32Type()}, TypeRange{});
...
static void populateVPTOOpLoweringPatterns(
    VPTOTypeConverter& typeConverter, RewritePatternSet& patterns, LoweringState& state, const std::string& march)
{
    patterns.add<
        LowerUnaryMaskedOpPattern<pto::VabsOp>, LowerUnaryMaskedOpPattern<pto::VexpOp>,
        ...
        LowerPredicatePackOpPattern<pto::PpackOp>, LowerPredicatePackOpPattern<pto::PunpackOp>, LowerVselOpPattern,
        LowerVselrOpPattern, LowerPnotOpPattern, LowerPredicateMaskBinaryOpPattern<pto::PselOp>,
        LowerPredicateMaskBinaryOpPattern<pto::PandOp>, LowerPredicateMaskBinaryOpPattern<pto::PorOp>,
        LowerPredicateMaskBinaryOpPattern<pto::PxorOp>, LowerPredicatePairReorderOpPattern<pto::PdintlvB8Op>,
        LowerPredicatePairReorderOpPattern<pto::PdintlvB16Op>, LowerPredicatePairReorderOpPattern<pto::PdintlvB32Op>,
        LowerPredicatePairReorderOpPattern<pto::PintlvB8Op>, LowerPredicatePairReorderOpPattern<pto::PintlvB16Op>,
        LowerPredicatePairReorderOpPattern<pto::PintlvB32Op>, LowerUnpackOpPattern<pto::VsunpackOp>,
        LowerUnpackOpPattern<pto::VzunpackOp>, LowerVpackOpPattern, LowerInterleaveOpPattern<pto::VintlvOp>,
        LowerInterleaveOpPattern<pto::VdintlvOp>, LowerCmpOpPattern<pto::VcmpOp>, LowerCmpOpPattern<pto::VcmpsOp>,
        LowerPltOpPattern<pto::PltB8Op>, LowerPltOpPattern<pto::PltB16Op>, LowerPltOpPattern<pto::PltB32Op>,
        LowerPltmOpPattern<pto::PltmB8Op>, LowerPltmOpPattern<pto::PltmB16Op>, LowerPltmOpPattern<pto::PltmB32Op>,
        LowerPsetOpPattern<pto::PsetB8Op>, LowerPsetOpPattern<pto::PsetB16Op>, LowerPsetOpPattern<pto::PsetB32Op>,
        LowerPgeOpPattern<pto::PgeB8Op>, LowerPgeOpPattern<pto::PgeB16Op>, LowerPgeOpPattern<pto::PgeB32Op>,
        ...
        LowerSetLoopConfigOpPattern<pto::SetLoopSizeUbToOutOp>, LowerSetLoopConfigOpPattern<pto::SetLoop3ParaOp>,
        LowerSetLoopConfigOpPattern<pto::SetChannelParaOp>, LowerUnaryI64ConfigOpPattern<pto::SetCtrlOp>,
        LowerStoreVfSimtInfoOpPattern, LowerUnaryConfigOpPattern<pto::SetMovPadValOp>,
        LowerUnaryI64ConfigOpPattern<pto::SetQuantPreOp>, LowerUnaryI64ConfigOpPattern<pto::SetReluAlphaOp>,
        LowerUnaryI64ConfigOpPattern<pto::SetFixClipReluOp>, LowerUnaryI64ConfigOpPattern<pto::SetLoop2StrideOutToL1Op>,
        ...
        LowerVldsOpPattern, LowerVldsx2OpPattern, LowerVsldbOpPattern, LowerVldasOpPattern, LowerInitAlignOpPattern,
        LowerVldusOpPattern, LowerSprclrOpPattern, LowerSprStoreOpPattern<pto::SprstiOp>,
        LowerSprStoreOpPattern<pto::SprstsOp>, LowerVstsOpPattern, LowerVsstbOpPattern, LowerVstsx2OpPattern,
        LowerVstarOpPattern, LowerVstasOpPattern, LowerVgather2OpPattern, LowerVgather2BcOpPattern,
        LowerVgatherbOpPattern, LowerVscatterOpPattern, LowerVaxpyOpPattern, LowerVmulscvtOpPattern, LowerVciOpPattern,
        LowerVexpdifOpPattern, LowerVbitsortOpPattern, LowerVmrgsort4OpPattern, LowerVtrcOpPattern, LowerVcvtOpPattern,
        LowerVbitcastOpPattern, LowerPbitcastOpPattern, LowerPredicateLoadOpPattern<pto::PldiOp>,
        LowerPredicateLoadOpPattern<pto::PldsOp>, LowerPredicateStoreOpPattern<pto::PstiOp>,
        LowerPredicateStoreOpPattern<pto::PstsOp>, LowerPstuOpPattern, LowerVstusOpPattern, LowerVsturOpPattern,
        LowerInterCoreSyncOpPattern<pto::SyncSetOp>, LowerInterCoreSyncOpPattern<pto::SyncWaitOp>,
        LowerCopyGmToCbufOpPattern, LowerLoadCbufToCaOpPattern, LowerLoadCbufToCbOpPattern,
        LowerLoadCbufToS4OpPattern<pto::LoadCbufToCaS4Op>, LowerLoadCbufToS4OpPattern<pto::LoadCbufToCbS4Op>,
        LowerLoadCbufToCaMxOpPattern, LowerLoadCbufToCbMxOpPattern, LowerCopyMatrixCcToGmOpPattern,
        ...
        LowerMadRawPattern<pto::MadMxBiasRawOp>, LowerCopyUbufToUbufOpPattern, LowerCopyCbufToUbufOpPattern,
        LowerCopyUbufToCbufOpPattern>(typeConverter, patterns.getContext(), state);

    patterns.add<LowerCopyOpPattern<pto::CopyGmToUbufOp>>(typeConverter, patterns.getContext(), state, march);
    patterns.add<LowerCopyOpPattern<pto::CopyUbufToGmOp>>(typeConverter, patterns.getContext(), state, march);

    if (march == "dav-c220-vec") {
        patterns.add<LowerUBufBinaryOpPattern<pto::UBVaddOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufBinaryOpPattern<pto::UBVsubOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufBinaryOpPattern<pto::UBVmulOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufBinaryOpPattern<pto::UBVdivOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufBinaryOpPattern<pto::UBVmaxOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufBinaryOpPattern<pto::UBVminOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufBinaryOpPattern<pto::UBVandOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufBinaryOpPattern<pto::UBVorOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufBinaryOpPattern<pto::UBVaddReluOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufUnaryOpPattern<pto::UBVnotOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufUnaryOpPattern<pto::UBVabsOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufUnaryOpPattern<pto::UBVreluOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufUnaryOpPattern<pto::UBVexpOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufUnaryOpPattern<pto::UBVlnOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufUnaryOpPattern<pto::UBVsqrtOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufUnaryOpPattern<pto::UBVrsqrtOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufShiftOpPattern<pto::UBVshlOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufShiftOpPattern<pto::UBVshrOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufScalarBinaryPattern<pto::UBVmulSOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufScalarBinaryPattern<pto::UBVaddSOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufScalarBinaryPattern<pto::UBVmaxSOp>>(typeConverter, patterns.getContext(), state);
        patterns.add<LowerUBufScalarBinaryPattern<pto::UBVminSOp>>(typeConverter, patterns.getContext(), state);
        ...
        patterns.add<LowerUBSetMaskCountOpPattern>(typeConverter, patterns.getContext());
        patterns.add<LowerUBSetMaskNormOpPattern>(typeConverter, patterns.getContext());
    }
}

static void configureVPTOOpLoweringTarget(
    ConversionTarget& target, VPTOTypeConverter& typeConverter, const std::string& march)
{
    (void)typeConverter;
    ...
    target.addLegalDialect<
        arith::ArithDialect, cf::ControlFlowDialect, LLVM::LLVMDialect, func::FuncDialect, scf::SCFDialect>();
    ...
    target.addIllegalOp<
        pto::SetFlagOp, pto::WaitFlagOp, pto::SetFlagDynOp, pto::WaitFlagDynOp, pto::SyncSetOp, pto::SyncWaitOp,
        pto::BarrierOp, pto::MemBarOp, pto::CmoCacheInvalidOp, pto::FenceBarrierAllOp, pto::DsbOp, pto::DcciOp,
        pto::GetBufOp, pto::RlsBufOp, pto::GetBufDynOp, pto::RlsBufDynOp>();
    target.addIllegalOp<
    ...
    target.addIllegalOp<
        pto::SetLoop2StrideOutToUbOp, pto::SetLoop1StrideOutToUbOp, pto::SetLoopSizeOutToUbOp,
        pto::SetLoop2StrideUbToOutOp, pto::SetLoop1StrideUbToOutOp, pto::SetLoopSizeUbToOutOp, pto::SetLoop3ParaOp,
        pto::SetChannelParaOp, pto::SetLoop2StrideOutToL1Op, pto::SetLoop1StrideOutToL1Op, pto::SetLoopSizeOutToL1Op,
        pto::SetMte2NzParaOp, pto::SetPadValOutToL1Op, pto::SetReluAlphaOp, pto::SetFixClipReluOp, pto::SetFpcOp,
        pto::SetStoreAtomicCfgOp, pto::SetAtomicS32Op, pto::SetAtomicS8Op, pto::SetCtrlOp, pto::StoreVfSimtInfoOp,
        pto::SetMovPadValOp, pto::SetQuantPreOp>();
    target.addIllegalOp<pto::Sbitset0Op, pto::Sbitset1Op>();
    target.addIllegalOp<
        pto::VldsOp, pto::Vldsx2Op, pto::VsldbOp, pto::VldasOp, pto::InitAlignOp, pto::VldusOp, pto::SprclrOp,
        pto::SprstiOp, pto::SprstsOp, pto::VstsOp, pto::VsstbOp, pto::Vstsx2Op, pto::VstarOp, pto::VstasOp,
        pto::Vgather2Op, pto::Vgather2BcOp, pto::VgatherbOp, pto::VscatterOp, pto::PldiOp, pto::PldsOp, pto::PstiOp,
        pto::PstsOp, pto::PstuOp, pto::VstusOp, pto::VsturOp>();
    target.addIllegalOp<
        pto::PltB8Op, pto::PltB16Op, pto::PltB32Op, pto::PltmB8Op, pto::PltmB16Op, pto::PltmB32Op, pto::PsetB8Op,
        pto::PsetB16Op, pto::PsetB32Op, pto::PgeB8Op, pto::PgeB16Op, pto::PgeB32Op>();
    target.addIllegalOp<
        ...
        pto::VshlsOp, pto::VshrsOp, pto::VcaddOp, pto::VcmaxOp, pto::VcminOp, pto::VcgaddOp, pto::VcgmaxOp,
        pto::VcgminOp, pto::VcpaddOp, pto::Chistv2Op, pto::Dhistv2Op, pto::VcbmaxOp, pto::VcbminOp, pto::VdupOp,
        pto::VbrOp, pto::PpackOp, pto::PunpackOp, pto::PbitcastOp, pto::VselOp, pto::VselrOp, pto::PnotOp, pto::PselOp,
        pto::PandOp, pto::PorOp, pto::PxorOp, pto::PdintlvB8Op, pto::PdintlvB16Op, pto::PdintlvB32Op, pto::PintlvB8Op,
        pto::PintlvB16Op, pto::PintlvB32Op, pto::VsunpackOp, pto::VzunpackOp, pto::VpackOp, pto::VintlvOp,
        pto::VdintlvOp, pto::VpreluOp, pto::VaxpyOp, pto::VmulscvtOp, pto::VciOp, pto::VexpdifOp, pto::VbitsortOp,
        pto::Vmrgsort4Op, pto::VtrcOp, pto::VcvtOp, pto::VbitcastOp, pto::VcmpOp, pto::VcmpsOp, pto::CopyGmToUbufOp,
        pto::CopyUbufToGmOp, pto::CopyUbufToUbufOp, pto::CopyCbufToUbufOp, pto::CopyUbufToCbufOp, pto::CopyGmToCbufOp,
...
static void populateVPTOStructuralTypePatterns(
    VPTOTypeConverter& typeConverter, RewritePatternSet& patterns, ConversionTarget& target)
{
...
static LogicalResult lowerVPTOOps(ModuleOp module, const std::string& march, llvm::raw_ostream& diagOS)
{
...
static LogicalResult lowerVPTOTypes(ModuleOp module, llvm::raw_ostream& diagOS)
{
    ...
    target.addIllegalOp<
        pto::AddPtrOp, pto::CastPtrOp, pto::LoadScalarOp, pto::StoreScalarOp, pto::PTOLoadOp, pto::PTOStoreOp,
        pto::PTOLdgOp, pto::PTOStgOp, pto::DeclareStructOp, pto::StructGetOp, pto::StructSetOp>();
    target.addDynamicallyLegalOp<UnrealizedConversionCastOp>([&](UnrealizedConversionCastOp op) {
        return !hasVPTOConvertibleType(op->getOperandTypes()) && !hasVPTOConvertibleType(op->getResultTypes());
    });
    ...
    patterns.add<
        ConvertPtoTileBufAddrOp, ConvertPtoAddPtrOp, ConvertPtoCastPtrOp, ConvertPtoLoadScalarOp,
        ConvertPtoDeclareStructOp, ConvertPtoStructGetOp, ConvertPtoStructSetOp, ConvertPtoStoreScalarOp>(
        typeConverter, context);
    patterns.add<ConvertPtoLoadOp, ConvertPtoStoreOp, ConvertPtoLdgOp, ConvertPtoStgOp>(typeConverter, context, state);
...
static VPTOEmissionOptions makeDeviceEmissionOptions(const VPTOEmissionOptions& baseOptions, FunctionKernelKind kind)
{
    VPTOEmissionOptions options = baseOptions;
    constexpr llvm::StringLiteral kC220VecTargetFeatures = "+ASAN,+ATOMIC,+AtomicForB64,+AtomicForB8 ,+FFTSBlk,"
                                                           "+MOVX8,+MSTX,+MathOp,+SPR7bits,+dav-c220-vec";
    constexpr llvm::StringLiteral kC220CubeTargetFeatures = "+ASAN,+ATOMIC,+AtomicForB64,+AtomicForB8 ,+FFTSBlk,"
...
static FailureOr<ModuleOp> getUniqueDeviceModuleByKernelKind(
    ModuleOp module, FunctionKernelKind kind, llvm::raw_ostream& diagOS)
{
    ModuleOp matched;
...
static LogicalResult renameKernelFunctionsForKernelKind(ModuleOp module, llvm::raw_ostream& diagOS)
{
...
static llvm::StringSet<llvm::BumpPtrAllocator> collectSimtEntryFunctionNames(ModuleOp module)
{
...
static void applyArtifactVisibilityLinkage(ModuleOp sourceModule, llvm::Module& llvmModule)
{
...
static void applySimtEntryCallingConvention(
    llvm::Module& llvmModule, const llvm::StringSet<llvm::BumpPtrAllocator>& simtEntryNames)
{
    for (llvm::Function& function : llvmModule) {
...
static FailureOr<EmittedLLVMModule> emitDeviceLLVMModule(
    ModuleOp deviceModule, StringRef kernelKind, const VPTOEmissionOptions& options,
    const llvm::StringSet<llvm::BumpPtrAllocator>& simtEntryNames, llvm::raw_ostream& diagOS)
{
    if (!deviceModule)
...
static LogicalResult runPipeline(ModuleOp module, const std::string& march, llvm::raw_ostream& diagOS, EmitFn&& emit)
{
