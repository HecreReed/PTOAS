...
static std::string formatVMIVRegType(int64_t elementCount, Type elementType, Attribute layout)
{
    std::string result;
...
static std::string formatVMIMaskType(int64_t elementCount, StringRef granularity, Attribute layout)
{
    std::string result;
    llvm::raw_string_ostream os(result);
...
{
    return isa<IntegerType, FloatType, IndexType>(type) || pto::isPTOLowPrecisionType(type);
}

static bool isVMIFloatLikeType(Type type) { return isa<FloatType>(type) || pto::isPTOLowPrecisionType(type); }

static bool isVMIIntegerLikeType(Type type) { return isa<IntegerType, IndexType>(type); }

static bool isVMIF16OrF32Type(Type type) { return type.isF16() || type.isF32(); }

static bool isVMIF16BF16OrF32Type(Type type) { return type.isF16() || type.isBF16() || type.isF32(); }
...
static bool isVMIAnyI8I16I32Type(Type type)
{
    ...
    if (!integerType)
        return false;
    return integerType.getWidth() == 8 || integerType.getWidth() == 16 || integerType.getWidth() == 32;
}
...
static bool matchesVMIIntSemantics(IntegerType intType, VMIIntSignSemantics semantics)
{
    switch (semantics) {
...
static bool isVMIIotaElementType(Type type)
{
    if (auto intType = dyn_cast<IntegerType>(type))
        return intType.getWidth() == 8 || intType.getWidth() == 16 || intType.getWidth() == 32;
...
static LogicalResult parseOptionalVMILayout(AsmParser& parser, Attribute& layout)
{
    ...
    if (!mlir::isa<VMILayoutAttr>(layout))
        return parser.emitError(parser.getCurrentLocation(), "expected #pto.vmi.layout attribute");
    return success();
}
...
static FailureOr<int64_t> getLayoutBlockElems(Type type) { return getVMILayoutBlockElems(type); }
...
static FailureOr<StringRef> getVMIMaskPhysicalGranularity(VMIMaskType type)
{
    ...
    StringRef physicalGranularity = getMaskGranularityForBitWidth(bits * laneStride);
...
static bool isLayoutAssigned(VMIVRegType type) { return static_cast<bool>(type.getLayoutAttr()); }

static bool isLayoutAssigned(VMIMaskType type) { return static_cast<bool>(type.getLayoutAttr()); }

static LogicalResult verifyAllSameVRegShapeAndLayout(
    Operation* op, ArrayRef<VMIVRegType> types, bool requireSameElement)
{
    ...
    bool anyLayout = llvm::any_of(types, [](VMIVRegType type) { return isLayoutAssigned(type); });
...
static LogicalResult verifyElementwiseVRegOp(Operation* op, VMIVRegType lhs, VMIVRegType rhs, VMIVRegType result)
{
...
static LogicalResult verifyFloatUnaryVRegOp(Operation* op, VMIVRegType source, VMIVRegType result)
{
...
static LogicalResult verifyFloatTernaryVRegOp(
    Operation* op, VMIVRegType lhs, VMIVRegType rhs, VMIVRegType acc, VMIVRegType result)
{
    if (!isVMIFloatLikeType(lhs.getElementType()))
        return op->emitOpError("requires floating-point-like VMI element type");
    return verifyAllSameVRegShapeAndLayout(
        op, {lhs, rhs, acc, result},
        /*requireSameElement=*/true);
}

static LogicalResult verifyAllSameMaskShapeLayoutAndGranularity(Operation* op, ArrayRef<VMIMaskType> types)
{
    if (types.empty())
        return success();

    VMIMaskType first = types.front();
    bool anyLayout = llvm::any_of(types, [](VMIMaskType type) { return isLayoutAssigned(type); });

    for (VMIMaskType type : types) {
        if (type.getElementCount() != first.getElementCount())
            return op->emitOpError("requires all VMI mask values to have the same logical lane count");
        if (type.getGranularity() != first.getGranularity())
            return op->emitOpError("requires all VMI mask values to have the same granularity");
        if (anyLayout && !isLayoutAssigned(type))
            return op->emitOpError("requires either all or no VMI mask values to carry layout");
        if (anyLayout && type.getLayout() != first.getLayout())
            return op->emitOpError("requires all layout-assigned VMI mask values to have the same "
                                   "layout");
    }
    return success();
}

static LogicalResult verifyMaskMatchesData(Operation* op, VMIMaskType maskType, VMIVRegType dataType)
{
    ...
    if (elementBitWidth != 0 && maskBitWidth != 0 && elementBitWidth != static_cast<unsigned>(maskBitWidth))
...
static LogicalResult verifyUBBackedMemory(Operation* op, Type memoryType, StringRef role)
{
    if (isUBBackedMemoryType(memoryType))
        return success();
    return op->emitOpError() << "requires memory " << role << " to be UB-backed";
}

static LogicalResult verifyMemoryElementMatches(Operation* op, Type memoryType, VMIVRegType dataType, StringRef role)
{
    Type memoryElementType = getMemoryElementType(memoryType);
    if (!memoryElementType)
        return success();
    if (memoryElementType != dataType.getElementType())
        return op->emitOpError() << "requires memory " << role << " element type to match VMI data element type";
    return success();
}

static LogicalResult verifyContiguousIfLayoutAssigned(Operation* op, VMIVRegType type, StringRef role)
{
    VMILayoutAttr layout = type.getLayoutAttr();
    if (layout && !layout.isContiguous())
        return op->emitOpError() << "requires layout-assigned " << role << " to use #pto.vmi.layout<contiguous>";
    return success();
}
...
static bool isPackedByteGroupStore(Type memoryType, VMIVRegType dataType)
{
    ...
    return memoryIntegerType && dataIntegerType && memoryIntegerType.getWidth() == 8 &&
           dataIntegerType.getWidth() == 32;
}

static LogicalResult verifyNumGroups(Operation* op, VMIVRegType type, int64_t numGroups)
{
    if (numGroups <= 0)
        return op->emitOpError("requires num_groups to be positive");
    if (type.getElementCount() % numGroups != 0)
        return op->emitOpError() << "requires num_groups to evenly divide VMI logical lane count "
                                 << type.getElementCount();
    return success();
}

static LogicalResult verifyPhysicalParts(Operation* op, Type vmiType, TypeRange physicalTypes)
{
    ...
    if (static_cast<int64_t>(physicalTypes.size()) != *expectedArity)
        return op->emitOpError() << "requires " << *expectedArity << " physical parts, got " << physicalTypes.size();
    ...
    if (auto vregType = dyn_cast<VMIVRegType>(vmiType)) {
        ...
        for (Type physicalType : physicalTypes) {
            ...
            if (partType.getElementCount() != *lanesPerPart || partType.getElementType() != physicalElementType)
    ...
    FailureOr<StringRef> physicalGranularity = getVMIMaskPhysicalGranularity(maskType);
    if (failed(physicalGranularity))
        return op->emitOpError("requires mask type with supported physical carrier granularity");
    ...
    for (Type physicalType : physicalTypes) {
        ...
        if (partType.getGranularity() != *physicalGranularity)
            return op->emitOpError("requires physical mask part granularity to match VMI mask carrier");
    }
    return success();
}

static std::optional<int64_t> mapDenseLogicalLaneToPartIndex(
    int64_t elementCount, int64_t factor, int64_t blockElems, int64_t logicalLane, int64_t& part)
{
    if (logicalLane < 0 || logicalLane >= elementCount || factor <= 0 || blockElems <= 0)
        return std::nullopt;
...
static std::optional<int64_t> mapDensePartIndexToLogicalLane(
    int64_t elementCount, int64_t factor, int64_t blockElems, int64_t part, int64_t indexInPart)
{
    if (part < 0 || part >= factor || indexInPart < 0 || factor <= 0 || blockElems <= 0)
        return std::nullopt;
...
static int64_t getDenseLogicalLanesInPart(int64_t elementCount, int64_t factor, int64_t blockElems, int64_t part)
{
    int64_t maxIndex = -1;
    for (int64_t lane = 0; lane < elementCount; ++lane) {
        int64_t lanePart = 0;
        std::optional<int64_t> index = mapDenseLogicalLaneToPartIndex(elementCount, factor, blockElems, lane, lanePart);
...
VMILayoutAttr VMILayoutAttr::getContiguous(MLIRContext* context, int64_t laneStride)
{
    return VMILayoutAttr::get(context, "contiguous", 1, 1, 0, laneStride);
}

VMILayoutAttr VMILayoutAttr::getDeinterleaved(MLIRContext* context, int64_t factor, int64_t laneStride)
{
    return VMILayoutAttr::get(context, "deinterleaved", factor, 1, 0, laneStride);
}

VMILayoutAttr VMILayoutAttr::getBlockDeinterleaved(MLIRContext* context, int64_t factor)
{
    return VMILayoutAttr::get(context, "block_deinterleaved", factor, 1, 0, 1);
}

VMILayoutAttr VMILayoutAttr::getGroupSlots(MLIRContext* context, int64_t numGroups, int64_t slots, int64_t laneStride)
{
    return VMILayoutAttr::get(context, "num_groups", numGroups, 1, slots, laneStride);
}
...
Attribute VMILayoutAttr::parse(AsmParser& parser, Type)
{
    ...
    if (kind == "contiguous") {
        ...
        while (succeeded(parser.parseOptionalComma())) {
            StringRef field;
            if (failed(parser.parseKeyword(&field)) || failed(parser.parseEqual()) || field != "lane_stride" ||
                failed(parser.parseInteger(laneStride))) {
                parser.emitError(parser.getCurrentLocation(), "expected 'lane_stride = <integer>'");
                return {};
            }
        }
        ...
        while (succeeded(parser.parseOptionalComma())) {
            ...
            if (field == "slots") {
                ...
                parser.emitError(
                    parser.getCurrentLocation(), "expected 'slots = <integer>' or "
                                                 "'lane_stride = <integer>'");
                return {};
            }
        }
        ...
        parser.emitError(
            parser.getCurrentLocation(), "expected VMI layout kind 'contiguous' or "
    ...
    return parser.getChecked<VMILayoutAttr>(loc, parser.getContext(), kind, factor, blockElems, slots, laneStride);
}
...
LogicalResult VMILayoutAttr::verify(
    function_ref<InFlightDiagnostic()> emitError, StringRef kind, int64_t factor, int64_t blockElems, int64_t slots,
    int64_t laneStride)
{
    if (laneStride <= 0)
        return emitError() << "#pto.vmi.layout<" << kind << "> requires lane_stride to be positive";

    if (kind == "contiguous") {
        if (factor != 1 || blockElems != 1 || slots != 0)
            return emitError() << "#pto.vmi.layout<contiguous> requires factor, block_elems, "
    ...
    if (kind == "deinterleaved") {
        if (factor != 2 && factor != 4)
            return emitError() << "#pto.vmi.layout<deinterleaved = " << factor << "> expected factor to be 2 or 4";
        if (blockElems != 1)
            return emitError() << "#pto.vmi.layout<deinterleaved = " << factor << ", block_elems = " << blockElems
                               << "> requires block_elems to be omitted";
        if (slots != 0)
            return emitError() << "#pto.vmi.layout<deinterleaved = " << factor << "> requires slots to be omitted";
        return success();
    }
    ...
    if (kind == "num_groups") {
        if (factor <= 0)
            return emitError() << "#pto.vmi.layout<num_groups = " << factor << "> requires num_groups to be positive";
        if (blockElems != 1)
            return emitError() << "#pto.vmi.layout<num_groups = " << factor << "> requires block_elems to be omitted";
        if (slots < 0)
            return emitError() << "#pto.vmi.layout<num_groups = " << factor << ", slots = " << slots
...
Type VMIVRegType::parse(AsmParser& parser)
{
    ...
    if (failed(parser.parseLess()) ||
        failed(parser.parseDimensionList(
            ...
            /*withTrailingX=*/true)) ||
        shape.size() != 1 || failed(parser.parseType(elementType)) || failed(parseOptionalVMILayout(parser, layout)) ||
        failed(parser.parseGreater()))
        return {};

    return parser.getChecked<VMIVRegType>(loc, parser.getContext(), shape.front(), elementType, layout);
}
...
LogicalResult VMIVRegType::verify(
    function_ref<InFlightDiagnostic()> emitError, int64_t elementCount, Type elementType, Attribute layout)
{
    if (elementCount <= 0)
        return emitError() << "'" << formatVMIVRegType(elementCount, elementType, layout)
    ...
    if (auto layoutAttr = llvm::dyn_cast_or_null<VMILayoutAttr>(layout)) {
        if (layoutAttr.isGroupSlots() && elementCount != layoutAttr.getNumGroups())
            return emitError() << "'" << formatVMIVRegType(elementCount, elementType, layout)
...
Type VMIMaskType::parse(AsmParser& parser)
{
    ...
    if (failed(parser.parseLess()) ||
        ...
        failed(parseOptionalVMILayout(parser, layout)) || failed(parser.parseGreater()))
        return {};

    return parser.getChecked<VMIMaskType>(loc, parser.getContext(), shape.front(), granularity, layout);
}
...
LogicalResult VMIMaskType::verify(
    function_ref<InFlightDiagnostic()> emitError, int64_t elementCount, StringRef granularity, Attribute layout)
{
    if (elementCount <= 0)
        return emitError() << "'" << formatVMIMaskType(elementCount, granularity, layout)
                           << "' expected a positive element count";
...
LogicalResult VMIGroupIotaOp::verify()
{
    ...
    if (succeeded(lanesPerPart) && groupSize % *lanesPerPart != 0 && *lanesPerPart % groupSize != 0)
        return emitOpError("requires group_size to divide or be a multiple of "
                           "physical lanes per part (")
               << *lanesPerPart << ")";
    if (VMILayoutAttr layout = resultType.getLayoutAttr(); layout && !layout.isContiguous())
        return emitOpError("requires contiguous result layout");
    return success();
}

LogicalResult VMICreateMaskOp::verify() { return success(); }
...
LogicalResult VMIMaskAndOp::verify()
{
    ...
    auto resultType = cast<VMIMaskType>(getResult().getType());
    return verifyAllSameMaskShapeLayoutAndGranularity(getOperation(), {lhsType, rhsType, resultType});
}
...
LogicalResult VMIMaskNotOp::verify()
{
    ...
    auto resultType = cast<VMIMaskType>(getResult().getType());
    return verifyAllSameMaskShapeLayoutAndGranularity(getOperation(), {sourceType, resultType});
}
...
LogicalResult VMIFmaOp::verify()
{
    ...
    if (!isVMIF16BF16OrF32Type(lhsType.getElementType()))
        return emitOpError("requires f16, bf16, or f32 VMI element type");
    return verifyFloatTernaryVRegOp(getOperation(), lhsType, rhsType, accType, resultType);
}
...
LogicalResult VMIMinIOp::verify()
{
    ...
    auto rhsType = cast<VMIVRegType>(getRhs().getType());
    auto resultType = cast<VMIVRegType>(getResult().getType());
    if (!isVMIIntegerLikeType(lhsType.getElementType()) || !isVMIAnyI8I16I32Type(lhsType.getElementType()))
...
LogicalResult VMIShRUIOp::verify()
{
    ...
    if (!integerType || integerType.isSigned())
        return emitOpError("requires signless or unsigned integer VMI element type");
    if (!isVMIAnyI8I16I32Type(lhsType.getElementType()))
        return emitOpError("requires i8, i16, or i32 VMI element type");
    return verifyElementwiseVRegOp(getOperation(), lhsType, rhsType, resultType);
}
...
LogicalResult VMIShRSIOp::verify()
{
    ...
    if (!isVMISignedOrSignlessI8I16I32Type(lhsType.getElementType()))
        return emitOpError("requires signless or signed i8, i16, or i32 VMI element type");
    return verifyElementwiseVRegOp(getOperation(), lhsType, rhsType, resultType);
}
...
LogicalResult VMICompressStoreOp::verify()
{
    ...
    auto maskType = cast<VMIMaskType>(getMask().getType());
    if (failed(verifyMemoryElementMatches(getOperation(), getDestination().getType(), valueType, "destination")))
        return failure();
    if (failed(verifyUBBackedMemory(getOperation(), getDestination().getType(), "destination")))
        return failure();
    return verifyMaskMatchesData(getOperation(), maskType, valueType);
}

void VMICompressStoreOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
LogicalResult VMIReduceAddIOp::verify()
{
    ...
    if (sourceType.getElementType() != initType.getElementType() ||
        ...
        return emitOpError("requires source, init, and result element types to match");
    ...
    if (failed(verifyAllSameVRegShapeAndLayout(
            getOperation(), {initType, resultType},
            /*requireSameElement=*/true)))
        return failure();
    return verifyMaskMatchesData(getOperation(), maskType, sourceType);
}
...
LogicalResult VMIReduceAddFOp::verify()
{
    ...
    if (!getOperation()->hasAttr("reassoc"))
        return emitOpError("requires reassoc attr because VPTO vcadd performs pair-wise "
...
LogicalResult verifyReduceMinMaxFOp(OpTy op)
{
    ...
    if (sourceType.getElementType() != initType.getElementType() ||
        sourceType.getElementType() != resultType.getElementType())
        return op.emitOpError("requires source, init, and result element types to match");
    if (initType.getElementCount() != 1 || resultType.getElementCount() != 1)
        return op.emitOpError("requires init and result to be 1-lane VMI vectors");
    if (failed(verifyAllSameVRegShapeAndLayout(
            op.getOperation(), {initType, resultType},
            /*requireSameElement=*/true)))
        return failure();
...
template <typename OpTy>
LogicalResult verifyReduceMinMaxIOp(OpTy op)
{
    auto sourceType = cast<VMIVRegType>(op.getSource().getType());
    auto initType = cast<VMIVRegType>(op.getInit().getType());
    auto maskType = cast<VMIMaskType>(op.getMask().getType());
    auto resultType = cast<VMIVRegType>(op.getResult().getType());
    auto sourceIntegerType = dyn_cast<IntegerType>(sourceType.getElementType());
    if (!sourceIntegerType || !isVMIAnyI8I16I32Type(sourceType.getElementType()))
        return op.emitOpError("requires 8-bit, 16-bit, or 32-bit integer source element type");
    if (sourceType.getElementType() != initType.getElementType() ||
        sourceType.getElementType() != resultType.getElementType())
        return op.emitOpError("requires source, init, and result element types to match");
    if (initType.getElementCount() != 1 || resultType.getElementCount() != 1)
...
static LogicalResult verifyGroupReduceFloatOp(OpTy op, bool requiresReassoc)
{
    ...
    if (requiresReassoc && !op->hasAttr("reassoc"))
        return op.emitOpError("requires reassoc attr because grouped lowering uses pair-wise "
    ...
    if (resultType.getElementCount() != op.getNumGroupsAttr().getInt())
        return op.emitOpError("requires result logical lane count to match num_groups");
    ...
    if (auto sourceLayout = sourceType.getLayoutAttr()) {
        bool supportedSourceLayout =
            ...
            (sourceLayout.isDenseSplit() && (sourceLayout.getFactor() == 2 || sourceLayout.getFactor() == 4));
        if (!supportedSourceLayout)
            return op.emitOpError("requires layout-assigned source to use contiguous layout or "
                                  "deinterleaved=2/4 or block_deinterleaved=2/4 layout");
    }
    if (auto resultLayout = resultType.getLayoutAttr()) {
        if (!resultLayout.isGroupSlots() || resultLayout.getNumGroups() != op.getNumGroupsAttr().getInt())
    ...
    if (failed(verifyMaskMatchesData(op.getOperation(), maskType, sourceType)))
        return failure();
    return verifyNumGroups(op.getOperation(), sourceType, op.getNumGroupsAttr().getInt());
}

LogicalResult VMIGroupReduceAddFOp::verify() { return verifyGroupReduceFloatOp(*this, /*requiresReassoc=*/true); }

LogicalResult VMIGroupReduceMaxFOp::verify() { return verifyGroupReduceFloatOp(*this, /*requiresReassoc=*/false); }

LogicalResult VMIGroupReduceMinFOp::verify() { return verifyGroupReduceFloatOp(*this, /*requiresReassoc=*/false); }
...
LogicalResult VMIGroupReduceAddIOp::verify() { return verifyGroupReduceIntegerOp(*this); }

LogicalResult VMIGroupReduceMaxIOp::verify() { return verifyGroupReduceIntegerOp(*this); }

LogicalResult VMIGroupReduceMinIOp::verify() { return verifyGroupReduceIntegerOp(*this); }
...
LogicalResult VMIGroupBroadcastOp::verify()
{
    ...
    if (sourceType.getElementCount() != numGroups)
        return emitOpError("requires source logical lane count to match num_groups");
    if (resultType.getElementCount() % numGroups != 0)
        return emitOpError("requires num_groups to evenly divide result logical lane count");
    if (sourceType.getElementType() != resultType.getElementType())
    ...
    if (auto resultLayout = resultType.getLayoutAttr()) {
        if (resultLayout.isGroupSlots())
            return emitOpError("requires layout-assigned result to use a dense VMI layout");
    }
...
static LogicalResult verifyVMIHistogramOp(OpTy op)
{
    ...
    if (!accElemType || accElemType.getWidth() != 16 ||
        !matchesVMIIntSemantics(accElemType, VMIIntSignSemantics::Unsigned) || (bins != 128 && bins != 256))
...
LogicalResult VMIExtFOp::verify()
{
    ...
    if (!isVMIFloatLikeType(sourceType.getElementType()) || !isVMIFloatLikeType(resultType.getElementType()))
        return emitOpError("requires floating-point-like source and result element types");
    if (getVMIElementBitWidth(sourceType.getElementType()) >= getVMIElementBitWidth(resultType.getElementType()))
...
LogicalResult VMITruncFOp::verify()
{
    ...
    if (srcBits < dstBits)
        return emitOpError("requires result element type to be narrower than or same-width "
                           "as source element type");
    if (srcBits == dstBits) {
        // Same-width fp→fp (e.g. bf16→f16): only allowed for supported VMI
        // fp-to-fp contract pairs.
        if (!lookupVMIFpToFpContract(sourceType.getElementType(), resultType.getElementType()))
    ...
    if (auto roundingAttr = (*this)->getAttrOfType<StringAttr>("rounding")) {
        ...
        if (rounding != "R" && rounding != "A" && rounding != "H" && rounding != "Z")
...
LogicalResult VMIFPToSIOp::verify()
{
    ...
    if (!isVMISignedOrSignlessIntegerType(resultType.getElementType()))
        return emitOpError("requires signed or signless integer result element "
                           "type");
    auto contract = lookupVMIFpToSiContract(sourceType.getElementType(), resultType.getElementType());
    if (!contract)
...
LogicalResult VMIFPToUIOp::verify()
{
    ...
    auto contract = lookupVMIFpToUIContract(sourceType.getElementType(), resultType.getElementType());
    if (!contract)
...
LogicalResult VMISIToFPOp::verify()
{
    ...
    if (!isVMISignedOrSignlessIntegerType(sourceType.getElementType()))
        return emitOpError("requires signed or signless integer source element type");
...
LogicalResult VMIExtSIOp::verify()
{
    ...
    if (!isVMISignedOrSignlessIntegerType(sourceType.getElementType()) ||
        ...
        return emitOpError("requires signed or signless integer source and result element types");
    if (getVMIElementBitWidth(sourceType.getElementType()) >= getVMIElementBitWidth(resultType.getElementType()))
        return emitOpError("requires result element type to be wider than source element type");
    return success();
}
...
LogicalResult VMIExtUIOp::verify()
{
    ...
    if (!isVMIUnsignedIntegerType(sourceType.getElementType()) ||
        ...
        return emitOpError("requires unsigned integer source and result element types");
    if (getVMIElementBitWidth(sourceType.getElementType()) >= getVMIElementBitWidth(resultType.getElementType()))
        return emitOpError("requires result element type to be wider than source element type");
    return success();
}
...
LogicalResult VMITruncIOp::verify()
{
    ...
    if (sourceType.getElementCount() != resultType.getElementCount())
        return emitOpError("requires source and result logical lane counts to match");
    if (!isVMIIntegerLikeType(sourceType.getElementType()) || !isVMIIntegerLikeType(resultType.getElementType()))
...
LogicalResult VMIBitcastOp::verify()
{
    auto sourceType = cast<VMIVRegType>(getSource().getType());
    auto resultType = cast<VMIVRegType>(getResult().getType());
    std::optional<unsigned> sourceBits = getVMIIntegerOrFloatBitWidth(sourceType.getElementType());
    std::optional<unsigned> resultBits = getVMIIntegerOrFloatBitWidth(resultType.getElementType());
    if (!sourceBits || !resultBits)
        return emitOpError("requires integer or floating-point source and result element types");
    if (sourceType.getElementCount() * static_cast<int64_t>(*sourceBits) !=
        resultType.getElementCount() * static_cast<int64_t>(*resultBits))
        return emitOpError("requires source and result to carry the same total number of bits");

    if (isLayoutAssigned(sourceType) || isLayoutAssigned(resultType)) {
        if (!isLayoutAssigned(sourceType) || !isLayoutAssigned(resultType))
            return emitOpError("requires either both source and result to carry layout or neither "
                               "to carry layout");
...
LogicalResult VMILoadOp::verify()
{
    if (failed(verifyMemoryElementMatches(
            getOperation(), getSource().getType(), cast<VMIVRegType>(getResult().getType()), "source")))
        return failure();
    return verifyUBBackedMemory(getOperation(), getSource().getType(), "source");
}

void VMILoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
LogicalResult VMIDeinterleaveLoadOp::verify()
{
    ...
    if (failed(verifyAllSameVRegShapeAndLayout(
            getOperation(), {lowType, highType},
            /*requireSameElement=*/true)))
        return failure();
    if (failed(verifyMemoryElementMatches(getOperation(), getSource().getType(), lowType, "source")))
        return failure();
    if (failed(verifyUBBackedMemory(getOperation(), getSource().getType(), "source")))
        return failure();
    if (failed(verifyContiguousIfLayoutAssigned(getOperation(), lowType, "low result")) ||
        failed(verifyContiguousIfLayoutAssigned(getOperation(), highType, "high result")))
        return failure();
    return success();
}

void VMIDeinterleaveLoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
{
    auto resultType = cast<VMIVRegType>(getResult().getType());
    if (failed(verifyMemoryElementMatches(getOperation(), getSource().getType(), resultType, "source")))
        return failure();
    if (failed(verifyUBBackedMemory(getOperation(), getSource().getType(), "source")))
        return failure();
    return verifyNumGroups(getOperation(), resultType, getNumGroupsAttr().getInt());
}

void VMIGroupLoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
LogicalResult VMIGroupSlotLoadOp::verify()
{
    ...
    if (resultType.getElementCount() != numGroups)
        return emitOpError("requires result logical lane count to match num_groups");
    if (failed(verifyMemoryElementMatches(getOperation(), getSource().getType(), resultType, "source")))
        return failure();
    if (failed(verifyUBBackedMemory(getOperation(), getSource().getType(), "source")))
        return failure();
...
void VMIGroupSlotLoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
void VMIGroupBroadcastLoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
void VMIMaskedLoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
LogicalResult VMIGatherOp::verify()
{
    ...
    if (!indexElementType || indexElementType.isSigned() ||
        ...
        return emitOpError("requires signless or unsigned 16-bit or 32-bit integer indices");
    ...
    if (indexElementType.getWidth() == 16 &&
        (!resultIntegerType || !resultIntegerType.isUnsigned() || resultIntegerType.getWidth() != 16))
        return emitOpError("requires ui16 result and passthru element type when using ui16 "
                           "indices");
    return verifyMaskMatchesData(getOperation(), maskType, resultType);
}

void VMIGatherOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
void VMIExpandLoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
{
    if (failed(verifyMemoryElementMatches(
            getOperation(), getDestination().getType(), cast<VMIVRegType>(getValue().getType()), "destination")))
        return failure();
    return verifyUBBackedMemory(getOperation(), getDestination().getType(), "destination");
}

void VMIStoreOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
LogicalResult VMIInterleaveStoreOp::verify()
{
    ...
    if (failed(verifyAllSameVRegShapeAndLayout(
            ...
            /*requireSameElement=*/true)))
        return failure();
    if (failed(verifyMemoryElementMatches(getOperation(), getDestination().getType(), lowType, "destination")))
        return failure();
    if (failed(verifyUBBackedMemory(getOperation(), getDestination().getType(), "destination")))
        return failure();
    if (failed(verifyContiguousIfLayoutAssigned(getOperation(), lowType, "low input")) ||
        failed(verifyContiguousIfLayoutAssigned(getOperation(), highType, "high input")))
        return failure();
    return success();
}

void VMIInterleaveStoreOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
LogicalResult VMIGroupStoreOp::verify()
{
    ...
    if (!isPackedByteGroupStore(getDestination().getType(), valueType) &&
        failed(verifyMemoryElementMatches(getOperation(), getDestination().getType(), valueType, "destination")))
        return failure();
    if (failed(verifyUBBackedMemory(getOperation(), getDestination().getType(), "destination")))
        return failure();
    return verifyNumGroups(getOperation(), valueType, getNumGroupsAttr().getInt());
}

void VMIGroupStoreOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void VMIStrideLoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
void VMIMaskedStoreOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void VMIStrideStoreOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void VMIScatterOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
LogicalResult VMIChannelSplitOp::verify()
{
    ...
    if (sourceType.getElementCount() != static_cast<int64_t>(getResults().size()) * firstResultType.getElementCount())
    ...
    if (anyLayout) {
        ...
        if (channels == 2 || channels == 4) {
            ...
            auto expectedLayout = VMILayoutAttr::getDeinterleaved(getContext(), channels);
...
LogicalResult VMIChannelMergeOp::verify()
{
    ...
    if (resultType.getElementCount() != static_cast<int64_t>(getInputs().size()) * firstInputType.getElementCount() ||
...
{
    return verifyPhysicalParts(getOperation(), getSource().getType(), getParts().getTypes());
}

LogicalResult VMIPackOp::verify()
{
    return verifyPhysicalParts(getOperation(), getResult().getType(), getParts().getTypes());
}
...
static LogicalResult verifyVMIPmodeMask(
    Operation* op, VMIMaskType maskType, VMIVRegType dataType, std::optional<StringRef> pmode)
{
    ...
    if (pmode.has_value()) {
        ...
        if (mode != "merge" && mode != "zero")
            return op->emitOpError("pmode must be \"merge\" or \"zero\", got \"") << mode << "\"";
    }
    return success();
}
...
static LogicalResult verifyVMIVariadicPmodeMask(
    Operation* op, ValueRange maskParts, VMIVRegType dataType, std::optional<StringRef> pmode)
{
    if (pmode.has_value()) {
        StringRef mode = pmode.value();
        if (mode != "merge" && mode != "zero")
    ...
    return verifyMaskMatchesData(op, cast<VMIMaskType>(maskParts.front().getType()), dataType);
}
...
static LogicalResult verifyVMIVectorScalarOp(
    Operation* op, VMIVRegType srcType, Type scalarType, VMIVRegType resultType, VMIMaskType maskType,
    std::optional<StringRef> pmode)
{
    Type eltTy = srcType.getElementType();
    if (!isVMIFloatLikeType(eltTy) && !isVMIIntegerLikeType(eltTy))
        return op->emitOpError("requires floating-point-like or integer-like VMI element type");

    if (scalarType != eltTy)
        return op->emitOpError("requires scalar type to match vector element type, got scalar ")
               << scalarType << " vs vector element " << eltTy;

    if (failed(verifyAllSameVRegShapeAndLayout(
            op, {srcType, resultType},
            /*requireSameElement=*/true)))
        return failure();
    ...
    if (pmode.has_value()) {
        StringRef mode = pmode.value();
        if (mode != "merge" && mode != "zero")
            return op->emitOpError("unsupported pmode '") << mode << "'; expected \"merge\" or \"zero\"";
    }
...
static LogicalResult verifyVMIVectorScalarShiftOp(
    Operation* op, VMIVRegType srcType, Type scalarType, VMIVRegType resultType, VMIMaskType maskType,
    std::optional<StringRef> pmode)
{
    Type eltTy = srcType.getElementType();
    if (!isVMIIntegerLikeType(eltTy))
        return op->emitOpError("requires integer-like VMI element type for shift");
...
LogicalResult VMIAddSOp::verify()
{
    return verifyVMIVectorScalarOp(
        getOperation(), cast<VMIVRegType>(getSrc().getType()), getScalar().getType(),
        cast<VMIVRegType>(getResult().getType()), cast<VMIMaskType>(getMask().getType()), getPmode());
}
...
{
    return cmpMode == "eq" || cmpMode == "ne" || cmpMode == "lt" || cmpMode == "le" || cmpMode == "gt" ||
           cmpMode == "ge" || cmpMode == "oeq" || cmpMode == "one" || cmpMode == "olt" || cmpMode == "ole" ||
           cmpMode == "ogt" || cmpMode == "oge";
}
...
{
    static const std::set<StringRef> modes = {"continuous", "unpack", "dintlv", "brc"};
    return modes;
}
...
LogicalResult VMIVbrcOp::verify()
{
    ...
    if (auto groupAttr = getGroupAttr()) {
        ...
        if (vregType.getElementCount() != numGroupsVal)
            return emitOpError() << "requires source logical lane count " << vregType.getElementCount()
                                 << " to match num_groups " << numGroupsVal;
        if (vregType.getElementType() != resultType.getElementType())
            return emitOpError("requires source and result element types to match");
        if (auto sourceLayout = vregType.getLayoutAttr()) {
            if (!sourceLayout.isGroupSlots() || sourceLayout.getNumGroups() != numGroupsVal)
                return emitOpError() << "requires layout-assigned source to use "
                                        "#pto.vmi.layout<num_groups = "
                                     << numGroupsVal << ">";
        }
...
LogicalResult VMIPgeOp::verify()
{
    ...
    if (activeLanes > resultType.getElementCount())
        return emitOpError("PAT_VL active lanes ")
               << activeLanes << " exceeds mask element count " << resultType.getElementCount();
    if (!resultType.isPred() && !isLayoutAssigned(resultType))
        return emitOpError("requires concrete mask result to carry layout");
    return success();
}
...
LogicalResult VMIVaddOp::verify()
{
    ...
    if (failed(verifyElementwiseVRegOp(getOperation(), lhsType, rhsType, resultType)))
        return failure();
    if (failed(verifyVMIVariadicPmodeMask(getOperation(), getMask(), resultType, getPmode())))
        return failure();
    return success();
}
...
LogicalResult VMIVabsOp::verify()
{
    ...
    if (!isVMIFloatLikeType(eltTy) && !isVMIIntegerLikeType(eltTy))
        return emitOpError("requires floating-point-like or integer-like VMI element type");
...
LogicalResult VMIvSelOp::verify()
{
    ...
    if (auto pmode = getPmode(); pmode.has_value()) {
        StringRef mode = pmode.value();
        if (mode != "merge" && mode != "zero")
            return emitOpError("pmode must be \"merge\" or \"zero\", got \"") << mode << "\"";
    }
    return success();
}
    ...
    if (auto groupAttr = getGroupAttr()) {
        ...
        if (sourceType.getElementCount() % C != 0)
            return emitOpError("group count ")
                   << C << " must divide source lane count " << sourceType.getElementCount();
...
LogicalResult VMIVgatherOp::verify()
{
    ...
    if (!indexElementType || indexElementType.isSigned() ||
        (indexElementType.getWidth() != 32 && indexElementType.getWidth() != 16))
        return emitOpError("requires signless or unsigned 16-bit or 32-bit integer offsets");
    ...
    if (indexElementType.getWidth() == 16 &&
        (!resultIntegerType || !resultIntegerType.isUnsigned() || resultIntegerType.getWidth() != 16))
        return emitOpError("requires ui16 result element type when using ui16 offsets");
...
void VMIVgatherOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
void VMIVgatherbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
LogicalResult VMIVscatterOp::verify()
{
    ...
    if (failed(verifyAllSameVRegShapeAndLayout(
            getOperation(), {valueType, offsetsType},
            /*requireSameElement=*/false)))
        return failure();
...
void VMIVscatterOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
LogicalResult VMIVexpdifOp::verify()
{
    ...
    if (xType.getElementCount() != maxType.getElementCount() || xType.getElementCount() != resultType.getElementCount())
        return emitOpError("requires x, max, and result logical lane counts to match");
...
LogicalResult VMIVaxpyOp::verify()
{
    ...
    if (xType != accType || accType != resultType)
        return emitOpError("requires x, acc, and result to have identical VMI vreg types");
...
LogicalResult VMIVlreluOp::verify()
{
    ...
    if (slopeType != xType.getElementType())
        return emitOpError("requires slope scalar type to match vector element type");
...
LogicalResult VMIVpreluOp::verify()
{
    ...
    if (xType != alphaType || alphaType != resultType)
        return emitOpError("requires x, alpha, and result to have identical VMI vreg types");
...
LogicalResult VMIVmullOp::verify()
{
    ...
    auto isLegalElementType = [](Type type) {
        auto integerType = dyn_cast<IntegerType>(type);
        return integerType && integerType.getWidth() == 32 && (integerType.isSignless() || integerType.isUnsigned());
    };
    if (!isLegalElementType(aType.getElementType()) || !isLegalElementType(bType.getElementType()) ||
        !isLegalElementType(lowType.getElementType()) || !isLegalElementType(highType.getElementType()))
        return emitOpError("requires a, b, low, and high element types to be exactly i32 or ui32");

    if (aType != bType || aType != lowType || aType != highType)
        return emitOpError("requires a, b, low, and high to have identical VMI vreg types");
...
LogicalResult VMIVmulaOp::verify()
{
    ...
    if (accType != lhsType || lhsType != rhsType || rhsType != resultType)
        return emitOpError("requires acc, lhs, rhs, and result to have identical VMI vreg types");
...
LogicalResult VMICvtOp::verify()
{
    ...
    bool srcSignlessInt = srcInt && isa<IntegerType>(srcElem) && !cast<IntegerType>(srcElem).isUnsigned() &&
    ...
    if (srcFp && dstFp) {
        if (dstBits > srcBits)
            ...
            if (!lookupVMIFpToFpContract(srcElem, dstElem))
                return emitOpError("same-width fp-to-fp conversion is not supported for this type "
        ...
        if (!isVMISignedOrSignlessIntegerType(srcElem))
            return emitOpError("int-to-fp conversion requires signed or signless integer source "
                               "element type");
        ...
        if (dstBits > srcBits)
            ...
            return emitOpError("int-to-int conversion must change element bit-width");
    } else {
        return emitOpError("unsupported element type combination for vcvt");
    }
    ...
    if (dir == CvtDirection::FpToSi) {
        ...
        bool needSat = (dir == CvtDirection::FpNarrow || dir == CvtDirection::IntNarrow);
        if (needSat) {
            if (!satAttr)
            ...
            if (dir == CvtDirection::IntNarrow && satVal == "SAT" && srcBits == 32 && dstBits == 8 &&
                isa<IntegerType>(srcElem) && cast<IntegerType>(srcElem).isSigned() && isa<IntegerType>(dstElem) &&
...
ParseResult VMIvStoreOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (preBracketOperands.empty())
        return parser.emitError(parser.getCurrentLocation(), "expected at least one value and one destination");
    ...
    if (nTypes != expectedTypes)
        return parser.emitError(parser.getCurrentLocation())
               << "expected " << expectedTypes << " types (" << nValues << " value(s), 1 destination"
               << (hasMask ? ", 1 mask" : "") << "), got " << nTypes;
    ...
    if (parser.resolveOperand(preBracketOperands[nValues], destType, result.operands))
        return failure();

    if (parser.resolveOperand(offsetOperand, parser.getBuilder().getIndexType(), result.operands))
        return failure();

    if (hasStride &&
        parser.resolveOperand(postBracketOps[strideIdx], parser.getBuilder().getIndexType(), result.operands))
        return failure();

    if (hasBlock &&
        parser.resolveOperand(postBracketOps[blockIdx], parser.getBuilder().getIntegerType(16), result.operands))
        return failure();
    if (hasRepeat &&
        parser.resolveOperand(postBracketOps[repeatIdx], parser.getBuilder().getIntegerType(16), result.operands))
        return failure();

    if (hasMask) {
        Type maskType = types.back();
        if (parser.resolveOperand(postBracketOps[maskIdx], maskType, result.operands))
            return failure();
    }

    result.addAttribute(
        "operandSegmentSizes", parser.getBuilder().getDenseI32ArrayAttr(
                                   {static_cast<int32_t>(nValues), 1, 1, hasStride ? 1 : 0, hasBlock ? 1 : 0,
...
LogicalResult VMIvStoreOp::verify()
{
    ...
    if (hasBlock != hasRepeat)
        return emitOpError("block_stride and repeat_stride must both be present or absent");
    if (hasBlock) {
        if (getDistMode())
            return emitOpError("block_stride and dist_mode are mutually exclusive");
        if (getValues().size() != 1)
    ...
    if (distMode && (*distMode == "unpack" || *distMode == "brc"))
        return emitOpError("dist-mode \"") << *distMode << "\" is not valid for vstore";
    ...
    auto valueType = cast<VMIVRegType>(getValues()[0].getType());
    bool isPackedGroupStore = getGroup() && isPackedByteGroupStore(getDestination().getType(), valueType);
    ...
    if (nValues == 2) {
        ...
        if (failed(verifyAllSameVRegShapeAndLayout(
                getOperation(), {loType, hiType},
                /*requireSameElement=*/true)))
            return failure();
        if (failed(verifyContiguousIfLayoutAssigned(getOperation(), loType, "low input")) ||
            failed(verifyContiguousIfLayoutAssigned(getOperation(), hiType, "high input")))
            return failure();
    }
...
void VMIvStoreOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
LogicalResult VMIVsstbOp::verify()
{
    ...
    auto maskType = cast<VMIMaskType>(getMask().getType());
    if (failed(verifyMemoryElementMatches(getOperation(), getDestination().getType(), valueType, "destination")) ||
        failed(verifyUBBackedMemory(getOperation(), getDestination().getType(), "destination")))
        return failure();
...
void VMIVsstbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
LogicalResult VMIVselrOp::verify()
{
    ...
    if (indexType.getElementCount() != resultType.getElementCount())
        return emitOpError("requires index lane count to match result lane count");
    ...
    if (sourceBits != 8 && sourceBits != 16 && sourceBits != 32)
        return emitOpError("requires source/result element storage width to be 8, 16, or 32 bits");
    if (indexBits != sourceBits)
        return emitOpError("requires index element storage width to match source element storage width");
    ...
    if (indexHasLayout && !sourceHasLayout)
        return emitOpError("requires index to carry layout only when source does");
...
LogicalResult VMIVintlvOp::verify()
{
    ...
    if (failed(verifyAllSameVRegShapeAndLayoutPresence(
            ...
            /*requireSameElement=*/true)))
        return failure();
    if (failed(verifyVMIPmodeMask(getOperation(), cast<VMIMaskType>(getMask().getType()), lhsType, getPmode())))
        return failure();
    return success();
}
...
LogicalResult VMIVcmpOp::verify()
{
    ...
    if (!isSupportedVCmpPredicate(getCmp()))
        return emitOpError("unsupported compare predicate '") << getCmp() << "'; expected eq/ne/lt/le/gt/ge, "
                                                              << "or oeq/one/olt/ole/ogt/oge";

    // Validate pmode.
    if (auto pmode = getPmode()) {
        if (pmode.value() != "zeroing" && pmode.value() != "merge")
            return emitOpError("unsupported pmode '") << pmode.value() << "'; expected \"zeroing\" or \"merge\"";
    }
    ...
    if (seedType.getElementCount() != resultType.getElementCount())
        return emitOpError("requires result mask lane count to match seed mask lane count");
...
ParseResult VMIvLoadOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    // Parse: %source[%offset]
    if (parser.parseOperand(sourceOperand) || parser.parseLSquare() || parser.parseOperand(offsetOperand) ||
        parser.parseRSquare())
        return failure();
    ...
    if (parser.resolveOperand(offsetOperand, parser.getBuilder().getIndexType(), result.operands))
        return failure();
    if (hasStride && parser.resolveOperand(strideOperand, parser.getBuilder().getIndexType(), result.operands))
        return failure();
    if (hasBlock && parser.resolveOperand(blockStrideOperand, parser.getBuilder().getIntegerType(16), result.operands))
        return failure();
    if (hasRepeat &&
        parser.resolveOperand(repeatStrideOperand, parser.getBuilder().getIntegerType(16), result.operands))
        return failure();

    result.addAttribute(
        "operandSegmentSizes",
        parser.getBuilder().getDenseI32ArrayAttr({1, 1, hasStride ? 1 : 0, hasBlock ? 1 : 0, hasRepeat ? 1 : 0}));
...
LogicalResult VMIvLoadOp::verify()
{
    ...
    for (auto res : getResults()) {
        ...
        if (!isUnpack && failed(verifyMemoryElementMatches(getOperation(), getSource().getType(), resType, "source")))
            return failure();
        if (isDintlv && failed(verifyContiguousIfLayoutAssigned(getOperation(), resType, "result")))
            return failure();
    }
...
void VMIvLoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
FailureOr<int64_t> mlir::pto::getVMILayoutBlockElems(Type type)
{
    ...
    if (failed(lanesPerPart) || *lanesPerPart <= 0 || *lanesPerPart % kVCGBlocksPerPart != 0)
        return failure();
...
FailureOr<int64_t> mlir::pto::getVMIPhysicalArity(Type type)
{
    ...
    if ((*layout).isGroupSlots() && (*layout).getSlots() > 0)
        return divideCeilNonNegative((*layout).getNumGroups(), (*layout).getSlots());
    ...
    if (failed(blockElems))
        return failure();
    int64_t laneStride = isa<VMIMaskType>(type) ? 1 : ((*layout).isDense() ? (*layout).getLaneStride() : 1);
    int64_t arity = 0;
    for (int64_t part = 0; part < factor; ++part) {
        int64_t lanesInPart = getDenseLogicalLanesInPart(*elementCount, factor, *blockElems, part);
        int64_t requiredPhysicalLanes = lanesInPart == 0 ? 0 : (lanesInPart - 1) * laneStride + 1;
        arity += divideCeilNonNegative(requiredPhysicalLanes, *lanesPerPart);
    }
    return arity;
}

FailureOr<VMIPhysicalLane> mlir::pto::mapLogicalLaneToPhysical(Type type, int64_t logicalLane)
{
    FailureOr<int64_t> elementCount = getVMIElementCount(type);
    ...
    FailureOr<int64_t> lanesPerPart = getPhysicalLanesPerPart(type);
    if (failed(elementCount) || failed(factor) || failed(blockElems) || failed(laneStride) || failed(lanesPerPart))
        return failure();
    if (logicalLane < 0 || logicalLane >= *elementCount)
        return failure();

    FailureOr<VMILayoutAttr> layout = getAssignedVMILayout(type);
    if (succeeded(layout) && (*layout).isGroupSlots() && (*layout).getSlots() > 0) {
    ...
    int64_t part = 0;
    std::optional<int64_t> indexInPart =
        mapDenseLogicalLaneToPartIndex(*elementCount, *factor, *blockElems, logicalLane, part);
    if (!indexInPart)
        return failure();
    int64_t physicalIndex = *indexInPart * *laneStride;
    return VMIPhysicalLane{part, physicalIndex / *lanesPerPart, physicalIndex % *lanesPerPart};
}

FailureOr<int64_t> mlir::pto::mapPhysicalLaneToLogical(Type type, int64_t part, int64_t chunk, int64_t lane)
{
    FailureOr<int64_t> elementCount = getVMIElementCount(type);
    FailureOr<int64_t> factor = getLayoutFactor(type);
    FailureOr<int64_t> blockElems = getLayoutBlockElems(type);
    ...
    if (failed(elementCount) || failed(factor) || failed(blockElems) || failed(laneStride) || failed(lanesPerPart))
        return failure();
    if (part < 0 || part >= *factor || chunk < 0 || lane < 0 || lane >= *lanesPerPart)
        return failure();
    ...
    std::optional<int64_t> logicalLane =
        mapDensePartIndexToLogicalLane(*elementCount, *factor, *blockElems, part, indexInPart);
    if (!logicalLane)
        return failure();
    return *logicalLane;
}

FailureOr<bool> mlir::pto::isPaddingLane(Type type, int64_t part, int64_t chunk, int64_t lane)
{
    FailureOr<int64_t> elementCount = getVMIElementCount(type);
    FailureOr<int64_t> factor = getLayoutFactor(type);
    FailureOr<int64_t> blockElems = getLayoutBlockElems(type);
