...
// ✅ numeric getters（可选）
int32_t TileBufType::getSFractalSizeI32() const { return (int32_t)getConfigAttr().getSFractalSize().getInt(); }
...
static LogicalResult parseCompactTileBufFields(
    AsmParser& parser, StringRef firstToken, ParsedTileBufFields& fields, uint32_t* outMultiCount = nullptr)
{
    ...
    while (succeeded(parser.parseOptionalComma())) {
        ...
        if (key == "blayout") {
            if (seenBLayout) {
                parser.emitError(parser.getCurrentLocation(), "duplicate blayout in tile_buf compact syntax");
                return failure();
            }
            seenBLayout = true;
        ...
        if (key == "slayout") {
            if (seenSLayout) {
                parser.emitError(parser.getCurrentLocation(), "duplicate slayout in tile_buf compact syntax");
                return failure();
            }
            seenSLayout = true;
        ...
        if (key == "fractal") {
            if (seenFractal) {
                parser.emitError(parser.getCurrentLocation(), "duplicate fractal in tile_buf compact syntax");
                return failure();
            }
            seenFractal = true;
        ...
        if (key == "pad") {
            if (seenPad) {
                parser.emitError(parser.getCurrentLocation(), "duplicate pad in tile_buf compact syntax");
                return failure();
            }
            seenPad = true;
        ...
        if (key == "compact") {
            if (seenCompact) {
                parser.emitError(parser.getCurrentLocation(), "duplicate compact in tile_buf compact syntax");
                return failure();
            }
            seenCompact = true;
...
static Type buildTileBufType(AsmParser& parser, const ParsedTileBufFields& fields)
{
    ...
    auto emitError = [&]() -> InFlightDiagnostic { return parser.emitError(parser.getNameLoc()); };
    ...
    if (fields.fractal != kFractalMxSize && fields.fractal != kFractalABSize && fields.fractal != kFractalCSize) {
        emitError() << "unsupported s_fractal_size: " << fields.fractal << ", must be one of {" << kFractalMxSize
                    << ", " << kFractalABSize << ", " << kFractalCSize << "}";
        return Type();
    }

    BLayout effectiveBLayout = resolveTileBufBLayout(parser.getContext(), memorySpace.value(), bl.value());
    ...
    auto fractalAttr = IntegerAttr::get(IntegerType::get(ctx, kI32BitWidth), fields.fractal);
    ...
    auto cfg = TileBufConfigAttr::get(ctx, blAttr, slAttr, fractalAttr, padAttr, compactAttr);
    ...
    return TileBufType::get(
        ctx, shape, fields.dtype, memorySpaceAttr, llvm::ArrayRef<int64_t>(canonicalValidShape), cfg);
}
...
void mlir::pto::TileBufType::print(mlir::AsmPrinter& printer) const
{
    ...
    auto defaultCompact = llvm::dyn_cast<CompactModeAttr>(defaultCfg.getCompactMode());
    ...
    const bool printBLayout = blayout && defaultBLayout && blayout.getValue() != defaultBLayout.getValue();
    const bool printSLayout = slayout && defaultSLayout && slayout.getValue() != defaultSLayout.getValue();
    const bool printFractal = cfg.getSFractalSize().getInt() != defaultCfg.getSFractalSize().getInt();
    const bool printPad = pad && defaultPad && pad.getValue() != defaultPad.getValue();
    const bool printCompact = compact && defaultCompact && compact.getValue() != defaultCompact.getValue();
...
LogicalResult MultiTileBufType::verify(
    function_ref<InFlightDiagnostic()> emitError, mlir::pto::TileBufType slotType, uint32_t count)
{
    if (!slotType) {
        return emitError() << "multi_tile_buf slot type must be non-null";
    }
    if (count < kPtoMultiBufferMinNum) {
        return emitError() << "multi_tile_buf count must be >= " << kPtoMultiBufferMinNum << " (got " << count << ")";
    }
    if (count > kPtoMultiBufferMaxNum) {
        return emitError() << "multi_tile_buf count must be <= " << kPtoMultiBufferMaxNum << " (got " << count << ")";
    }
    return success();
}
...
// closing `>`.
static LogicalResult parseMultiTileBufCount(AsmParser& parser, uint32_t& count)
{
    if (failed(parser.parseComma()))
        return failure();
...
Type MultiTileBufType::parse(AsmParser& parser)
{
    ...
    if (typeRes.has_value()) {
        ...
        if (!slotType) {
            parser.emitError(parser.getCurrentLocation(), "multi_tile_buf slot type must be `!pto.tile_buf<...>`");
            return Type();
        }
    } else {
    ...
    return getChecked([&]() { return parser.emitError(parser.getNameLoc()); }, ctx, slotType, count);
}
