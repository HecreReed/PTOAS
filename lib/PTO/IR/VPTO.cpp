...
static llvm::cl::opt<bool> disableVPTOAlignChainVerification(
    "vpto-disable-align-chain-verification", llvm::cl::desc("Disable !pto.align linear-chain verifier checks"),
...
static LogicalResult verifyVRegTypeLike(Operation* op, Type type, StringRef roleDescription)
{
    ...
    return VRegType::verify(
        [&]() { return op->emitOpError() << roleDescription << " "; }, vecType.getElementCount(),
        vecType.getElementType());
}

static LogicalResult verifyMaskTypeLike(Operation* op, Type type, StringRef roleDescription)
{
...
static LogicalResult verifyNonLowPrecisionVRegElementTypeLike(Operation* op, Type type, StringRef roleDescription)
{
    auto vecType = dyn_cast<VRegType>(type);
    if (!vecType)
        return success();
...
static LogicalResult verifyMaskTypeWithGranularityLike(
    Operation* op, Type type, StringRef roleDescription, StringRef granularity)
{
    auto maskType = dyn_cast<MaskType>(type);
    if (!maskType)
        return op->emitOpError() << roleDescription << " must be !pto.mask<...>";
    if (maskType.getGranularity() != granularity) {
        return op->emitOpError() << roleDescription << " must be " << formatMaskType(granularity);
    }
    return success();
}

static LogicalResult verifyVPTOScalarAccessTypes(Operation* op, Type ptrTy, Type valueTy, StringRef opNameForDiag)
{
    Type elemTy;
    if (auto pty = dyn_cast<PtrType>(ptrTy)) {
        ...
        return op->emitOpError() << "expects " << opNameForDiag << " pointer operand to be !pto.ptr or memref";
    }

    if (valueTy != elemTy) {
        return op->emitOpError() << "expects " << opNameForDiag << " value type to match pointer element type";
    }
    return success();
}

static bool isMaskGranularityAdjacentWidening(StringRef inputGranularity, StringRef resultGranularity)
{
    return (inputGranularity == "b8" && resultGranularity == "b16") ||
           (inputGranularity == "b16" && resultGranularity == "b32");
}

static bool isMaskGranularityAdjacentNarrowing(StringRef inputGranularity, StringRef resultGranularity)
{
...
static bool isSupportedShuffleValueType(Type type)
{
    ...
    if (auto vecType = dyn_cast<VectorType>(type))
        return vecType.getRank() == 1 && vecType.getDimSize(0) == 2 && vecType.getElementType().isF16();
...
LogicalResult SimtLaunchOp::verify()
{
    ...
    if (!callee)
        return emitOpError() << "'" << getCalleeAttr().getValue() << "' does not reference a valid function";

    if (!callee->hasAttr(pto::kPTOSimtEntryAttrName)) {
        return emitOpError() << "callee '" << getCalleeAttr().getValue() << "' must be marked with '"
    ...
    for (auto [index, argType, operand] : llvm::enumerate(calleeType.getInputs(), getArgs())) {
        if (argType != operand.getType()) {
            return emitOpError("operand type mismatch: expected operand type ")
                   << argType << ", but provided " << operand.getType() << " for operand number " << index;
        }
    }
    return success();
}

static LogicalResult verifyShuffleSemanticControl(
    Operation* op, Type controlType, IntegerAttr widthAttr, StringRef ctrlName)
{
    if (!isSupportedShuffleValueType(op->getResultTypes().front()))
        return op->emitOpError() << "requires i32, i64, f16, f32 or vector<2xf16> value/result type";
    if (!controlType.isInteger(32))
        return op->emitOpError() << "requires " << ctrlName << " operand to be i32";
...
static LogicalResult verifyReduxSemanticType(
    Operation* op, Type valueType, Attribute signednessAttr, bool requireSignedness)
{
    if (!isSupportedReduxValueType(valueType))
        return op->emitOpError() << "requires i32, f16 or f32 value/result type";

    auto intType = dyn_cast<IntegerType>(valueType);
    if (!intType) {
        if (signednessAttr)
            return op->emitOpError() << "does not accept signedness for floating-point redux";
        return success();
    }

    if (!signednessAttr && requireSignedness)
        return op->emitOpError() << "requires explicit signedness for integer redux";
...
static bool isIntegerLikeConvertType(Type type) { return isa<IntegerType>(type); }

static bool isVector2Of(Type type, llvm::function_ref<bool(Type)> elementPred)
{
    auto vecType = dyn_cast<VectorType>(type);
    return vecType && vecType.getRank() == 1 && vecType.getDimSize(0) == 2 && elementPred(vecType.getElementType());
}
...
static bool isSupportedPackedConvertType(Type type)
{
    ...
    return isVector2Of(type, [](Type elem) {
        return elem.isF16() || elem.isBF16() || elem.isF32() || pto::isPTOFloat8Type(elem) ||
               pto::isPTOHiFloat8Type(elem);
    });
}

static bool isSupportedLowPrecisionConvertType(Type type) { return pto::isPTOFloat4PackedType(type); }

static bool isVector2F16OrBF16Type(Type type)
{
    return isVector2Of(type, [](Type elem) { return elem.isF16() || elem.isBF16(); });
}

static bool isInsideSimtExecutionScope(Operation* op)
{
    auto funcOp = op->getParentOfType<func::FuncOp>();
    return (funcOp && funcOp->hasAttr(pto::kPTOSimtEntryAttrName)) || op->getParentOfType<pto::SectionSimtOp>();
}
...
static LogicalResult verifyPackedConvertControls(Operation* op, Type srcType, Type dstType, pto::Rounding rounding)
{
    auto isV2F16 = [](Type type) { return isVector2Of(type, [](Type elem) { return elem.isF16(); }); };
    auto isV2BF16 = [](Type type) { return isVector2Of(type, [](Type elem) { return elem.isBF16(); }); };
    auto isV2F32 = [](Type type) { return isVector2Of(type, [](Type elem) { return elem.isF32(); }); };
    auto isV2F8 = [](Type type) { return isVector2Of(type, [](Type elem) { return pto::isPTOFloat8Type(elem); }); };
    auto isV2HiF8 = [](Type type) { return pto::isPTOHiFloat8x2Type(type); };
    auto isF4 = [](Type type) { return pto::isPTOFloat4PackedType(type); };
    auto isRoundRAFZC = [](pto::Rounding rounding) {
        return rounding == pto::Rounding::R || rounding == pto::Rounding::A || rounding == pto::Rounding::F ||
               rounding == pto::Rounding::C || rounding == pto::Rounding::Z;
    };

    if (isV2F32(srcType) && isV2F16(dstType)) {
        if (rounding == pto::Rounding::H)
            return op->emitOpError() << "f32x2-to-f16x2 conversion supports rounding r/a/f/c/z/o";
        return success();
    }
    if (isV2F32(srcType) && isV2BF16(dstType)) {
        if (rounding == pto::Rounding::O || rounding == pto::Rounding::H)
            return op->emitOpError() << "f32x2-to-bf16x2 conversion supports rounding r/a/f/c/z";
        return success();
    }
    if ((isV2F16(srcType) || isV2BF16(srcType)) && isV2F32(dstType)) {
        if (rounding == pto::Rounding::O || rounding == pto::Rounding::H)
            return op->emitOpError() << "packed-to-f32x2 conversion supports rounding r/a/f/c/z";
        return success();
    }
    if (isV2F32(srcType) && isV2F8(dstType)) {
        if (rounding != pto::Rounding::R)
            return op->emitOpError() << "f32x2-to-f8x2 conversion supports rounding r";
        return success();
    }
    if ((isV2F32(srcType) || isV2F16(srcType)) && isV2HiF8(dstType)) {
        if (rounding != pto::Rounding::A && rounding != pto::Rounding::H)
            return op->emitOpError() << "f32x2/f16x2-to-hif8x2 conversion supports rounding a/h";
        return success();
    }
    if ((isV2F8(srcType) || isV2HiF8(srcType)) && (isV2F32(dstType) || isV2F16(dstType))) {
        if (!isRoundRAFZC(rounding))
            return op->emitOpError() << "f8x2/hif8x2-to-f32x2/f16x2 conversion supports rounding r/a/f/c/z";
        return success();
    }
    if ((isV2BF16(srcType) && isF4(dstType)) || (isF4(srcType) && isV2BF16(dstType))) {
        if (!isRoundRAFZC(rounding))
            return op->emitOpError() << "bf16x2-to-f4 and f4-to-bf16x2 conversion supports rounding r/a/f/c/z";
        return success();
    }

    return op->emitOpError() << "unsupported packed conversion type pair; supported packed pairs are "
...
static LogicalResult verifyConvertControls(
    Operation* op, Type srcType, Type dstType, pto::Rounding rounding, pto::Saturation saturation,
    Attribute signednessAttr)
{
    if (!isSupportedConvertType(srcType) || !isSupportedConvertType(dstType))
        return op->emitOpError() << "requires i32, i64, f16, bf16, f32 or supported vector<2xT> "
                                    "conversion types";
    ...
    if (srcPacked || dstPacked || srcLowPrecision || dstLowPrecision) {
        if (srcInt || dstInt)
            return op->emitOpError() << "does not support mixed integer and packed conversion";
        if (signednessAttr)
            return op->emitOpError() << "does not accept signedness for packed floating conversion";
        if (!((srcPacked || srcLowPrecision) && (dstPacked || dstLowPrecision)))
            return op->emitOpError() << "does not support mixed scalar and packed conversion";
        return verifyPackedConvertControls(op, srcType, dstType, rounding);
    }

    if (srcInt && dstInt)
        return op->emitOpError() << "does not support integer-to-integer conversion";

    if ((srcInt || dstInt) && !signednessAttr)
        return op->emitOpError() << "requires signedness when converting to or from integer type";
    if (!srcInt && !dstInt && signednessAttr)
        return op->emitOpError() << "does not accept signedness for floating-to-floating conversion";

    if (srcInt) {
        if (srcType.isInteger(64) && !dstType.isF32())
            return op->emitOpError() << "supports i64 conversion only to f32 in the confirmed slice";
        if (srcType.isInteger(32) && !(dstType.isF32() || dstType.isF16() || dstType.isBF16()))
            return op->emitOpError() << "unsupported integer-to-floating conversion type pair";
        if (rounding == pto::Rounding::O || rounding == pto::Rounding::H)
            return op->emitOpError() << "integer-to-floating conversion supports rounding r/a/f/c/z";
        (void)saturation;
        return success();
    }

    if (dstType.isInteger(64) && !srcType.isF32())
        return op->emitOpError() << "supports conversion to i64 only from f32 in the confirmed slice";
    if (srcType.isF32()) {
        if (dstType.isInteger(32) || dstType.isInteger(64)) {
            if (saturation != pto::Saturation::Enable)
                return op->emitOpError() << "fp32-to-integer conversion requires saturation enable";
            if (rounding == pto::Rounding::O || rounding == pto::Rounding::H)
                return op->emitOpError() << "fp32-to-integer conversion supports rounding r/a/f/c/z";
            return success();
        }
        if (dstType.isF16() || dstType.isBF16() || dstType.isF32()) {
            if (dstType.isF16()) {
                if (rounding == pto::Rounding::H)
                    return op->emitOpError() << "fp32-to-fp16 conversion supports rounding r/a/f/c/z/o";
            } else if (rounding == pto::Rounding::O || rounding == pto::Rounding::H) {
                return op->emitOpError() << "fp32-to-floating conversion supports rounding r/a/f/c/z";
            }
            return success();
        }
    }
    ...
    if (srcType.isF16() || srcType.isBF16()) {
        if (dstType.isInteger(32)) {
            if (saturation != pto::Saturation::Enable)
                return op->emitOpError() << "fp16/bf16-to-integer conversion requires saturation enable";
            if (rounding == pto::Rounding::O || rounding == pto::Rounding::H)
                return op->emitOpError() << "fp16/bf16-to-integer conversion supports rounding r/a/f/c/z";
            return success();
        }
        if (dstType.isF32() || dstType.isF16() || dstType.isBF16()) {
            if (rounding == pto::Rounding::O || rounding == pto::Rounding::H)
                return op->emitOpError() << "fp16/bf16-to-floating conversion supports rounding r/a/f/c/z";
            return success();
        }
    }
...
static bool isSupportedAtomicScalarType(Type type)
{
    if (auto intType = dyn_cast<IntegerType>(type))
        return intType.getWidth() == 32 || intType.getWidth() == 64;
    return type.isF16() || type.isBF16() || type.isF32() || isVector2F16OrBF16Type(type);
}

static LogicalResult verifyAtomicCommon(
    Operation* op, Value ptr, Type valueType, Type resultType, bool bitwise, Attribute signednessAttr)
{
    if (!isSupportedAtomicScalarType(valueType))
        return op->emitOpError() << "requires i32, i64, f16, bf16, f32, vector<2xf16> or "
                                    "vector<2xbf16> atomic value type";
    if (resultType != valueType)
        return op->emitOpError() << "requires atomic result type to match value type";
    ...
    if (ptrTy.getElementType() != valueType)
        return op->emitOpError() << "requires atomic value type to match pointer element type";
    ...
    if (signednessAttr && !intType)
        return op->emitOpError() << "does not accept signedness for floating-point atomics";
    if (isVector2F16OrBF16Type(valueType)) {
        if (!isInsideSimtExecutionScope(op))
            return op->emitOpError() << "requires packed atomics to be inside a pto.simt_entry "
                                        "function or pto.section.simt on beta.1";
        if (!op->getResult(0).use_empty())
            return op->emitOpError() << "does not support using the old value result for packed "
                                        "atomics on beta.1; leave the result unused";
    }
    return success();
}

static LogicalResult verifyLdgStgAccess(Operation* op, Type ptrType, Type valueType)
{
    ...
    if (valueType.isF16() || valueType.isBF16() || valueType.isF32() || valueType.isF64())
        return success();
    ...
    return op->emitOpError() << "currently supports 8/16/32/64-bit integer, "
                                "f16/bf16/f32/f64/fp8/hif8, "
                                "packed vector<2xT> (T = f16/bf16/f32/i8/i16/i32), "
                                "packed vector<2/4/8xfp8>, and !pto.hif8x2 value type";
}

static LogicalResult verifyLdStDevAccess(Operation* op, Type ptrType, Type valueType)
{
    if (op->hasAttr("l1cache") || op->hasAttr("l2cache"))
        return op->emitOpError() << "does not accept l1cache or l2cache policy attributes";
    ...
    auto intType = dyn_cast<IntegerType>(valueType);
    if (!intType ||
        (intType.getWidth() != 8 && intType.getWidth() != 16 && intType.getWidth() != 32 && intType.getWidth() != 64))
        return op->emitOpError() << "supports only i8, i16, i32 or i64 values";

    if (isInsideSimtExecutionScope(op))
        return op->emitOpError() << "must be outside pto.simt_entry functions and pto.section.simt";
    auto funcOp = op->getParentOfType<func::FuncOp>();
    if (!funcOp || !pto::isPTOEntryFunction(funcOp))
        return op->emitOpError() << "requires an enclosing ordinary AICore entry function";
    return success();
}

LogicalResult PTOLoadOp::verify()
{
    if (failed(verifyVPTOScalarAccessTypes(getOperation(), getPtr().getType(), getValue().getType(), "load")))
        return failure();
    return success();
}
...
{
    if (failed(verifyVPTOScalarAccessTypes(getOperation(), getPtr().getType(), getValue().getType(), "store")))
        return failure();
    return success();
}
...
{
    if (failed(verifyVPTOScalarAccessTypes(getOperation(), getPtr().getType(), getValue().getType(), "ldg")))
        return failure();
    if (failed(verifyLdgStgAccess(getOperation(), getPtr().getType(), getValue().getType())))
        return failure();
    if (!isInsideSimtExecutionScope(getOperation()))
        return emitOpError() << "must be inside a pto.simt_entry function or pto.section.simt";
    return success();
}

LogicalResult PTOStgOp::verify()
{
    if (failed(verifyVPTOScalarAccessTypes(getOperation(), getPtr().getType(), getValue().getType(), "stg")))
        return failure();
    if (failed(verifyLdgStgAccess(getOperation(), getPtr().getType(), getValue().getType())))
        return failure();
    if (!isInsideSimtExecutionScope(getOperation()))
...
{
    if (failed(verifyVPTOScalarAccessTypes(getOperation(), getPtr().getType(), getValue().getType(), "ld_dev")))
        return failure();
    return verifyLdStDevAccess(getOperation(), getPtr().getType(), getValue().getType());
}

LogicalResult PTOStDevOp::verify()
{
    if (failed(verifyVPTOScalarAccessTypes(getOperation(), getPtr().getType(), getValue().getType(), "st_dev")))
        return failure();
    return verifyLdStDevAccess(getOperation(), getPtr().getType(), getValue().getType());
}
...
{
    return verifyShuffleSemanticControl(getOperation(), getIndex().getType(), getWidthAttr(), "index");
}

LogicalResult ShuffleUpOp::verify()
{
    return verifyShuffleSemanticControl(getOperation(), getOffset().getType(), getWidthAttr(), "offset");
}
...
{
    return verifyShuffleSemanticControl(getOperation(), getMask().getType(), getWidthAttr(), "mask");
}

LogicalResult ReduxAddOp::verify()
{
    return verifyReduxSemanticType(
        getOperation(), getValue().getType(), getSignednessAttr(), /*requireSignedness=*/false);
}

LogicalResult ReduxMaxOp::verify()
{
    return verifyReduxSemanticType(
        getOperation(), getValue().getType(), getSignednessAttr(), /*requireSignedness=*/true);
}
...
{
    if (!getResult().getType().isInteger(32) && !getResult().getType().isInteger(64))
...
LogicalResult AtomicCasOp::verify()
{
    ...
    return verifyAtomicCommon(
        getOperation(), getPtr(), getValue().getType(), getOld().getType(), /*bitwise=*/false, getSignednessAttr());
}
...
{
    return verifyAtomicCommon(
        getOperation(), getPtr(), getValue().getType(), getOld().getType(), /*bitwise=*/true, getSignednessAttr());
}
...
{
    return verifyConvertControls(
        getOperation(), getSrc().getType(), getDst().getType(), getRounding(), getSaturation(), getSignednessAttr());
}

void PTOLoadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getPtrMutable());
}

void PTOStoreOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getPtrMutable());
}

void PTOLdgOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getPtrMutable());
}

void PTOStgOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getPtrMutable());
}

void PTOLdDevOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getPtrMutable());
}

void PTOStDevOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getPtrMutable());
}

template <typename OpTy>
static void getAtomicEffects(OpTy op, SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
void AtomicCasOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    getAtomicEffects(*this, effects);
}
void AtomicExchOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    getAtomicEffects(*this, effects);
}
void AtomicAddOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    getAtomicEffects(*this, effects);
}
void AtomicSubOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    getAtomicEffects(*this, effects);
}
void AtomicMinOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    getAtomicEffects(*this, effects);
}
void AtomicMaxOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    getAtomicEffects(*this, effects);
}
void AtomicAndOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    getAtomicEffects(*this, effects);
}
void AtomicOrOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    getAtomicEffects(*this, effects);
}
void AtomicXorOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    getAtomicEffects(*this, effects);
}

static LogicalResult verifyNotNestedInVecScope(Operation* op, StringRef opNameForDiag)
{
    if (op->getParentOfType<VecScopeOp>() || op->getParentOfType<StrictVecScopeOp>()) {
        return op->emitOpError() << "must not be nested under pto.vecscope/pto.strict_vecscope; " << opNameForDiag
                                 << " is a UB helper op rather than a vecscope op";
    }
    return success();
}

static LogicalResult verifyNestedInVecScope(Operation* op, StringRef opNameForDiag)
{
    if (op->getParentOfType<VecScopeOp>() || op->getParentOfType<StrictVecScopeOp>())
        return success();
    return op->emitOpError() << "must be nested under pto.vecscope/pto.strict_vecscope; " << opNameForDiag
                             << " is part of the vecscope control sequence";
}

static LogicalResult verifyAlignTypeLike(Operation* op, Type type, StringRef roleDescription)
{
...
static bool isSupportedMovPadScalarType(Type type)
{
    if (auto intType = dyn_cast<IntegerType>(type))
        return intType.isSignless() &&
               (intType.getWidth() == 8 || intType.getWidth() == 16 || intType.getWidth() == 32);
...
{
    return isa<Float8E4M3FNType, Float8E5M2Type>(type) || isa<pto::F4E1M2x2Type, pto::F4E2M1x2Type>(type);
}
...
{
    return mode == "R" || mode == "A" || mode == "F" || mode == "C" || mode == "Z";
}

static bool isStoreAlignProducer(Operation* op) { return isa<InitAlignOp, PstuOp, VstusOp, VsturOp>(op); }

static bool isStoreAlignSink(Operation* op) { return isa<VstasOp, VstarOp>(op); }

static bool isLoadAlignProducer(Operation* op) { return isa<VldasOp, VldusOp>(op); }
...
static FailureOr<Value> resolveStoreAlignRootImpl(Value current, llvm::SmallPtrSet<void*, 8> visited)
{
    while (true) {
        ...
        if (Operation* def = current.getDefiningOp()) {
            ...
            if (auto ifOp = dyn_cast<scf::IfOp>(def)) {
                ...
                FailureOr<Value> thenRoot = resolveStoreAlignRootImpl(thenYield.getOperand(resultIdx), visited);
                FailureOr<Value> elseRoot = resolveStoreAlignRootImpl(elseYield.getOperand(resultIdx), visited);
...
static LogicalResult verifyStoreAlignLoopThreading(Value align, Operation* user, StringRef roleDescription)
{
    ...
    while (auto forOp = cursor->getParentOfType<scf::ForOp>()) {
        ...
        if (isValueOwnedByRegion(align, body))
            return success();
        if (!isValueOwnedByRegion(align, body)) {
            return user->emitOpError() << roleDescription
...
static LogicalResult verifyStoreAlignLinearUses(Value value, Operation* user)
{
    ...
    while (visited.insert(current.getAsOpaquePointer()).second) {
        ...
        for (OpOperand& use : current.getUses()) {
            ...
            if (auto forOp = dyn_cast<scf::ForOp>(owner)) {
                ...
                if (use.getOperandNumber() < firstInitArg)
                    return user->emitOpError() << "found unexpected scf.for control operand use for !pto.align";
                unsigned iterIdx = use.getOperandNumber() - firstInitArg;
                if (iterIdx >= forOp.getRegionIterArgs().size())
                    return user->emitOpError() << "found invalid scf.for iter_args use for !pto.align";
            ...
            if (auto yieldOp = dyn_cast<scf::YieldOp>(owner)) {
                ...
                if (!forOp)
                    return user->emitOpError() << "found !pto.align yielded from non-scf.for loop";
                unsigned resultIdx = use.getOperandNumber();
                if (resultIdx >= forOp.getNumResults())
                    return user->emitOpError() << "found invalid scf.yield result mapping for !pto.align";
                nextValues.push_back(forOp.getResult(resultIdx));
                continue;
            }
            return user->emitOpError() << "found unsupported !pto.align consumer " << owner->getName();
        }
        ...
        if (nextValues.size() + terminalUsers.size() > 1) {
            ...
            return user->emitOpError() << "!pto.align value must form a single linear store-state chain";
        }
        if (nextValues.empty())
            return success();
...
static LogicalResult verifyStoreAlignChain(Value align, Operation* user, StringRef roleDescription)
{
        ...
        if (Operation* def = align.getDefiningOp()) {
            if (!isa<scf::ForOp>(def)) {
                return user->emitOpError()
                       << roleDescription << " must be produced by pto.init_align or a prior store-state op, got "
                       << def->getName();
            }
        }
        return user->emitOpError() << roleDescription
...
static FailureOr<Value> resolveLoadAlignRootImpl(Value current, llvm::SmallPtrSet<void*, 8> visited)
{
    while (true) {
        if (!visited.insert(current.getAsOpaquePointer()).second)
            return failure();
        ...
        if (Operation* def = current.getDefiningOp()) {
            ...
            if (auto ifOp = dyn_cast<scf::IfOp>(def)) {
                ...
                FailureOr<Value> thenRoot = resolveLoadAlignRootImpl(thenYield.getOperand(resultIdx), visited);
                FailureOr<Value> elseRoot = resolveLoadAlignRootImpl(elseYield.getOperand(resultIdx), visited);
                if (failed(thenRoot) || failed(elseRoot) || *thenRoot != *elseRoot)
                    return failure();
                return *thenRoot;
            }
...
static LogicalResult verifyLoadAlignLoopThreading(Value align, Operation* user, StringRef roleDescription)
{
    Operation* cursor = user;
    while (auto forOp = cursor->getParentOfType<scf::ForOp>()) {
        Region* body = &forOp.getRegion();
...
static LogicalResult verifyLoadAlignLinearUses(Value value, Operation* user)
{
    ...
    while (visited.insert(current.getAsOpaquePointer()).second) {
        ...
        if (nextValues.size() > 1) {
            ...
            return user->emitOpError() << "!pto.align value must form a single linear load-state chain";
        }
        if (nextValues.empty())
            return success();
        current = nextValues.front();
...
static LogicalResult verifyLoadAlignChain(Value align, Operation* user, StringRef roleDescription)
{
    if (disableVPTOAlignChainVerification)
        return success();
    ...
    if (failed(root)) {
        if (Operation* def = align.getDefiningOp()) {
            if (!isa<scf::ForOp>(def)) {
                return user->emitOpError()
                       << roleDescription << " must be produced by pto.vldas or a prior load-state op, got "
                       << def->getName();
            }
        }
        return user->emitOpError() << roleDescription << " must be produced by pto.vldas or a prior load-state op";
    }

    Operation* def = (*root).getDefiningOp();
    if (!isLoadAlignProducer(def)) {
        return user->emitOpError() << roleDescription << " must be produced by pto.vldas or a prior load-state op, got "
                                   << def->getName();
    }
...
{
    return pattern == "PAT_ALL" || pattern == "PAT_VL1" || pattern == "PAT_VL2" || pattern == "PAT_VL3" ||
           pattern == "PAT_VL4" || pattern == "PAT_VL8" || pattern == "PAT_VL16" || pattern == "PAT_VL32" ||
           pattern == "PAT_VL64" || pattern == "PAT_VL128" || pattern == "PAT_M3" || pattern == "PAT_M4" ||
           pattern == "PAT_H" || pattern == "PAT_Q" || pattern == "PAT_ALLF";
}

static bool isSupportedPredicateLoadDist(StringRef dist) { return dist == "NORM" || dist == "US" || dist == "DS"; }

static bool isSupportedPredicateStoreDist(StringRef dist) { return dist == "NORM" || dist == "PK"; }

static bool isSupportedPartToken(StringRef part) { return part == "LOWER" || part == "HIGHER"; }
...
static std::optional<VcvtPartFamily> classifyVcvtPartFamily(unsigned srcBits, unsigned dstBits)
{
...
static bool isValidVcvtRoundModeForContract(StringRef roundMode, const VcvtContract& contract)
{
...
static std::optional<VcvtContract> lookupVcvtContract(VcvtElemKind src, VcvtElemKind dst)
{
    switch (src) {
        case VcvtElemKind::F32:
            switch (dst) {
                ...
                case VcvtElemKind::F8E5M2:
                    return VcvtContract{
                        /*requiresRnd=*/true, /*requiresSat=*/true,
                        /*requiresPart=*/true, VcvtPartFamily::Packed4, "RAHZ"};
                case VcvtElemKind::HiF8:
                    return VcvtContract{
                        /*requiresRnd=*/true, /*requiresSat=*/true,
                        /*requiresPart=*/true, VcvtPartFamily::Packed4, "AH"};
                case VcvtElemKind::F16:
                case VcvtElemKind::BF16:
                case VcvtElemKind::S16:
                case VcvtElemKind::S64:
        ...
        case VcvtElemKind::F16:
            switch (dst) {
                ...
                case VcvtElemKind::F32:
                    return VcvtContract{
                        /*requiresRnd=*/false, /*requiresSat=*/false,
                        /*requiresPart=*/true};
                case VcvtElemKind::S32:
                    return VcvtContract{
                        /*requiresRnd=*/true, /*requiresSat=*/false,
                        /*requiresPart=*/true};
                case VcvtElemKind::S16:
                    return VcvtContract{
                        /*requiresRnd=*/true, /*requiresSat=*/true,
        ...
        case VcvtElemKind::BF16:
            switch (dst) {
                ...
                case VcvtElemKind::F4E2M1x2:
                    return VcvtContract{
                        /*requiresRnd=*/true, /*requiresSat=*/false,
                        /*requiresPart=*/true, VcvtPartFamily::Packed4, "RAFZC"};
                case VcvtElemKind::F16:
                    return VcvtContract{
                        /*requiresRnd=*/true, /*requiresSat=*/true,
                        /*requiresPart=*/false};
        ...
        case VcvtElemKind::U16:
            switch (dst) {
                case VcvtElemKind::U8:
                    return VcvtContract{
                        /*requiresRnd=*/false, /*requiresSat=*/true,
                        /*requiresPart=*/true};
                case VcvtElemKind::U32:
                    return VcvtContract{
                        /*requiresRnd=*/false, /*requiresSat=*/false,
...
{
    return dist == "BDINTLV" || dist == "DINTLV_B8" || dist == "DINTLV_B16" || dist == "DINTLV_B32";
}

static bool isSupportedVldsDistToken(StringRef dist)
{
    return dist == "NORM" || dist == "BRC_B8" || dist == "BRC_B16" || dist == "BRC_B32" || dist == "US_B8" ||
           dist == "US_B16" || dist == "DS_B8" || dist == "DS_B16" || dist == "UNPK_B8" || dist == "UNPK_B16" ||
           dist == "UNPK_B32" || dist == "BRC_BLK" || dist == "E2B_B16" || dist == "E2B_B32" || dist == "UNPK4" ||
           dist == "SPLT4CHN" || dist == "SPLT2CHN_B8" || dist == "SPLT2CHN_B16";
}

static bool isSupportedVstsDistToken(StringRef dist)
{
    return dist == "NORM_B8" || dist == "NORM_B16" || dist == "NORM_B32" || dist == "1PT_B8" || dist == "1PT_B16" ||
           dist == "1PT_B32" || dist == "PK_B16" || dist == "PK_B32" || dist == "PK_B64" || dist == "PK4_B32" ||
           dist == "MRG4CHN_B8" || dist == "MRG2CHN_B8" || dist == "MRG2CHN_B16";
}
...
static std::optional<StringRef> getVstsMaskGranularityOverride(StringRef dist, Type elementType)
{
...
static bool isSupportedPostMode(StringRef mode) { return mode == "NO_POST_UPDATE" || mode == "POST_UPDATE"; }
...
static unsigned getIntOrFloatBitWidth(Type type)
{
    ...
    if (pto::isPTOFloat8Type(type) || pto::isPTOHiFloat8Type(type) || pto::isPTOFloat4PackedType(type))
        return 8;
    if (pto::isPTOHiFloat8x2Type(type))
        return 16;
    return 0;
...
static bool isIntegerOrFloatLike(Type type) { return isa<IntegerType>(type) || isa<FloatType>(type); }
...
static LogicalResult verifyIntegerVRegTypeLike(Operation* op, Type type, StringRef roleDescription)
{
    ...
    if (!isa<IntegerType>(vecType.getElementType()))
        return op->emitOpError() << roleDescription << " must use integer vector element type";
    return success();
}
...
static bool isBufferLike(Type type) { return isa<BaseMemRefType, pto::PtrType>(type); }
...
static std::optional<AddressSpace> getBufferAddressSpace(Type type)
{
    ...
    if (auto memrefType = dyn_cast<BaseMemRefType>(type)) {
        if (auto space = dyn_cast_or_null<pto::AddressSpaceAttr>(memrefType.getMemorySpace()))
...
static LogicalResult verifyCubeBridgeLoadLikeOp(BridgeLoadOp op, AddressSpace expectedDstSpace, StringRef dstName)
{
    if (!isBufferLike(op.getSource().getType()) || !isBufferLike(op.getDestination().getType()))
    ...
    if (getBufferAddressSpace(op.getDestination().getType()) != expectedDstSpace) {
        return op.emitOpError() << "requires " << dstName << " destination";
    }

    int64_t sourceElemBytes = getBufferElementByteSize(op.getSource().getType());
    int64_t destinationElemBytes = getBufferElementByteSize(op.getDestination().getType());
    if (sourceElemBytes <= 0 || destinationElemBytes <= 0) {
        return op.emitOpError("requires source and destination element types with known byte width");
    }
...
static ParseResult parseRequiredOperandWithComma(OpAsmParser& parser, OpAsmParser::UnresolvedOperand& operand)
{
...
static ParseResult parseDmaTripleGroup(
    OpAsmParser& parser, StringRef keyword, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& operands)
{
...
static ParseResult parseOptionalDmaTripleGroupAlias(
    OpAsmParser& parser, ArrayRef<StringRef> keywords, StringRef& parsedKeyword,
...
static ParseResult parseDmaTripleTypes(OpAsmParser& parser, SmallVectorImpl<Type>& types)
{
    for (int i = 0; i < 3; ++i) {
        Type type;
...
static ParseResult parseDmaPadTypes(OpAsmParser& parser, SmallVectorImpl<Type>& types)
{
    Type valueType;
    ...
    if (succeeded(parser.parseOptionalComma())) {
        Type leftType;
        Type rightType;
        if (parser.parseType(leftType) || parser.parseComma() || parser.parseType(rightType))
            return failure();
...
static void printDmaTripleGroup(OpAsmPrinter& printer, StringRef keyword, Value first, Value second, Value third)
{
    printer << " " << keyword << "(" << first << ", " << second << ", " << third << ")";
}

static void printDmaTripleTypes(OpAsmPrinter& printer, StringRef keyword, Type first, Type second, Type third)
{
    printer << ", " << keyword << " " << first << ", " << second << ", " << third;
}

static void printDmaPadGroup(OpAsmPrinter& printer, Value value, Value left, Value right)
{
    printer << " pad(" << value;
    if (left || right)
        printer << ", " << left << ", " << right;
...
static void printDmaPadTypes(OpAsmPrinter& printer, Type valueType, Type leftType, Type rightType)
{
...
static FailureOr<CubeLoadFracMode> parseCubeLoadFracModeKeyword(StringRef keyword)
{
...
static ParseResult parseFixedKeywordOperandGroup(
    OpAsmParser& parser, StringRef keyword, int operandCount, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& operands)
{
    if (parser.parseKeyword(keyword) || parser.parseLParen())
        return failure();
...
static ParseResult parseFixedKeywordTypes(
    OpAsmParser& parser, StringRef keyword, int typeCount, SmallVectorImpl<Type>& types)
{
...
static ParseResult parseCubeLoadFracSrcLayoutTypes(OpAsmParser& parser, SmallVectorImpl<Type>& types)
{
    if (parser.parseKeyword("src_layout") || parser.parseLParen())
        return failure();
...
static void printCubeLoadFracSrcLayoutGroup(OpAsmPrinter& printer, Value srcInnerStride, Value srcOuterStride)
{
...
static void printCubeLoadFracSrcLayoutTypes(OpAsmPrinter& printer, Type srcInnerStrideType, Type srcOuterStrideType)
{
...
[[maybe_unused]] static ParseResult parseAccStoreModeGroup(
    OpAsmParser& parser, StringRef& modeKeyword, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& modeOperands)
{
    if (parser.parseKeyword(&modeKeyword))
        return failure();
    if (failed(parseAccStoreModeKeyword(modeKeyword)))
        return parser.emitError(parser.getCurrentLocation(), "expected one of 'nz2nd', 'nz2dn', or 'nz2nz'");
...
[[maybe_unused]] static ParseResult parseAccStoreModeTypes(
    OpAsmParser& parser, StringRef modeKeyword, SmallVectorImpl<Type>& modeTypes)
{
    if (parser.parseKeyword(modeKeyword))
        return failure();
...
[[maybe_unused]] static void printAccStoreModeGroup(
    OpAsmPrinter& printer, AccStoreMode mode, Value split, Value loop0SrcStride)
{
...
[[maybe_unused]] static void printAccStoreModeTypes(
    OpAsmPrinter& printer, AccStoreMode mode, Type splitType, Type loop0SrcStrideType)
{
    printer << ", " << pto::stringifyAccStoreMode(mode);
    switch (mode) {
        case AccStoreMode::Nz2nd:
...
[[maybe_unused]] static ParseResult parseMteL0cL1OptionalLoop3(
    OpAsmParser& parser, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& loop3CountOperands,
    ...
    SmallVectorImpl<OpAsmParser::UnresolvedOperand>& loop3DstStrideOperands)
{
    ...
    if (parseOptionalDmaTripleGroupAlias(parser, {"loop3"}, parsedKeyword, loop3Operands))
        return failure();
...
[[maybe_unused]] static ParseResult parseMteL0cL1OptionalFpc(
    OpAsmParser& parser, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& fpcOperands)
{
...
[[maybe_unused]] static void printMteL0cL1OptionalFpc(OpAsmPrinter& printer, Value fpc)
{
    if (fpc)
        printer << ", fpc(" << fpc << ")";
}

[[maybe_unused]] static void printMteL0cL1OptionalFpcType(OpAsmPrinter& printer, Type fpcType)
{
    if (fpcType)
        printer << ", fpc(" << fpcType << ")";
}
...
[[maybe_unused]] static ParseResult parseMteL0cL1OptionalLoop3Types(
    OpAsmParser& parser, SmallVectorImpl<Type>& loop3CountTypes, SmallVectorImpl<Type>& loop3SrcStrideTypes,
    ...
    if (succeeded(parser.parseOptionalComma())) {
        ...
        if (succeeded(parser.parseOptionalComma()))
            return parser.emitError(
                parser.getCurrentLocation(), (Twine(opName) + " accepts at most one loop3 group").str());
    }
    return success();
}

[[maybe_unused]] static LogicalResult verifyAccStoreLikeModeOperands(
    Operation* op, AccStoreMode mode, Value split, Value loop0SrcStride, Value loop3Count, Value loop3SrcStride,
    Value loop3DstStride, StringRef nz2ndSplitError, StringRef nz2ndLoop0Error, StringRef nz2dnSplitError,
    StringRef nz2nzLoop0Error, StringRef nz2nzLoop3Error)
{
    ...
    if ((hasLoop3Count != hasLoop3SrcStride) || (hasLoop3Count != hasLoop3DstStride)) {
        return op->emitOpError("requires loop3 count, src stride, and dst stride to appear together");
    }
...
static bool isStructuredAccStoreScalingPayload(Value value)
{
    ...
    return ptrType && ptrType.getMemorySpace().getAddressSpace() == AddressSpace::SCALING;
}
...
{
    auto ptrType = dyn_cast_or_null<PtrType>(value.getType());
    if (!ptrType || ptrType.getMemorySpace().getAddressSpace() != AddressSpace::SCALING)
        return {};
    return ptrType.getElementType();
}
...
[[maybe_unused]] static bool isStructuredAccStoreIntegerPayloadType(Type type) { return type.isSignlessInteger(); }
...
static bool isStructuredAccStoreClipSupportedElementType(Type type)
{
    ...
    if (intType.isUnsignedInteger(8))
        return true;
    if (intType.isSignlessInteger(4) || intType.isSignlessInteger(8) || intType.isSignlessInteger(16))
        return true;
    if (intType.isSignedInteger(4) || intType.isSignedInteger(8) || intType.isSignedInteger(16))
        return true;
    return false;
}

static LogicalResult verifyStructuredAccStoreClipPayload(Operation* op, Type destinationElementType, Value clipValue)
{
    if (!clipValue)
        return success();
    ...
    if (intType.isSignlessInteger(4) || intType.isSignlessInteger(8) || intType.isSignlessInteger(16) ||
        intType.isSignedInteger(4) || intType.isSignedInteger(8) || intType.isSignedInteger(16)) {
...
enum class StructuredAccStoreDestinationFamily { Any, F32, F16, BF16, I32, I16, I8, I4, FP8 };

static StructuredAccStoreDestinationFamily getStructuredAccStorePreQuantDestinationFamily(AccStoreQuantPreMode mode)
{
    switch (mode) {
        case AccStoreQuantPreMode::F32F16:
        case AccStoreQuantPreMode::QF322F16PreVec:
...
static bool isStructuredAccStoreDestinationFamily(Type type, StructuredAccStoreDestinationFamily family)
{
    switch (family) {
        ...
        case StructuredAccStoreDestinationFamily::FP8:
            return pto::isPTOFloat8Type(type) || pto::isPTOHiFloat8Type(type) || pto::isPTOHiFloat8x2Type(type);
    }
    llvm_unreachable("unknown acc-store destination family");
}

static ParseResult parseStructuredAccStoreUnitFlag(OpAsmParser& parser, StructuredAccStoreAsmState& state)
{
    if (state.unitFlag)
    ...
    if (keyword == "check_only")
        ...
        return parser.emitError(parser.getCurrentLocation(), "expected 'check_only' or 'check_and_clear'");
    return success();
}

static ParseResult parseStructuredAccStorePreQuant(OpAsmParser& parser, StructuredAccStoreAsmState& state)
{
    ...
    if (parser.parseLParen() || parser.parseOperand(payload) || parser.parseComma() || parser.parseKeyword("mode") ||
        parser.parseEqual() || parser.parseKeyword(&modeKeyword) || parser.parseRParen())
        return failure();
...
static ParseResult parseStructuredAccStorePreRelu(OpAsmParser& parser, StructuredAccStoreAsmState& state)
{
    ...
    if (failed(parser.parseOptionalKeyword("mode"))) {
        hasPayload = true;
        if (parser.parseOperand(payload) || parser.parseComma() || parser.parseKeyword("mode"))
            return failure();
    }
    ...
    if (succeeded(parser.parseOptionalComma())) {
        ...
        if (!state.clipValueOperands.empty())
            return parser.emitError(parser.getCurrentLocation(), "duplicate clip payload in pre_relu clause");
...
static ParseResult parseStructuredAccStoreLoop3(OpAsmParser& parser, StructuredAccStoreAsmState& state)
{
    ...
    if (parser.parseLParen() || parser.parseOperand(count) || parser.parseComma() || parser.parseOperand(srcStride) ||
        parser.parseComma() || parser.parseOperand(dstStride) || parser.parseRParen())
        return failure();
...
static ParseResult parseStructuredAccStoreAtomic(OpAsmParser& parser, StructuredAccStoreAsmState& state)
{
    ...
    if (parser.parseLParen() || parser.parseKeyword("type") || parser.parseEqual() ||
        parser.parseKeyword(&typeKeyword) || parser.parseComma() || parser.parseKeyword("op") || parser.parseEqual() ||
...
static ParseResult parseStructuredAccStoreClauses(OpAsmParser& parser, StructuredAccStoreAsmState& state)
{
    int lastClause = -1;
    ...
    while (true) {
        ...
        if (static_cast<int>(kind) < lastClause) {
            return parser.emitError(parser.getCurrentLocation(), "mte_l0c clauses must follow canonical order");
        }
            ...
            case StructuredAccStoreClauseKind::Sat:
                ...
                if (succeeded(parser.parseOptionalLParen())) {
                    ...
                    if (parser.parseKeyword(&satOption) || satOption != "preserve_nan")
                        return parser.emitError(parser.getCurrentLocation(), "expected preserve_nan");
                    if (parser.parseRParen())
                        return failure();
...
static ParseResult parseStructuredOptionalType(OpAsmParser& parser, SmallVectorImpl<Type>& types)
{
    Type type;
    if (parser.parseType(type))
        return failure();
...
static LogicalResult verifyStructuredAccStoreLike(
    Operation* op, Type srcType, Type dstType, Value preQuant, Value preRelu, Value clipValue, Value split,
    Value loop0SrcStride, Value loop3Count, Value loop3SrcStride, Value loop3DstStride,
    ...
    std::optional<AccStoreAtomicType> atomicType, std::optional<AccStoreAtomicOp> atomicOp, bool allowAtomic)
{
    ...
    if (preQuantMode) {
        if (*preQuantMode == AccStoreQuantPreMode::NoConvert) {
            ...
            if (!isStructuredAccStoreFloatScalarPayloadType(getStructuredAccStoreScalingElementType(preQuant)))
                return op->emitOpError(
            ...
            return op->emitOpError("scalar pre_quant mode requires f16/bf16/f32 payload");
        }

        auto emitIncompatibleQuantModeError = [&]() -> LogicalResult {
            return op->emitOpError() << "pre_quant mode " << stringifyAccStoreQuantPreMode(*preQuantMode)
        ...
        if (*preQuantMode != AccStoreQuantPreMode::NoConvert) {
            if (isa<Float32Type>(sourceElementType)) {
                ...
                return op->emitOpError() << "pre_quant requires source element type to be f32 or i32, got "
                                         << sourceElementType;
            }

            StructuredAccStoreDestinationFamily destinationFamily =
                getStructuredAccStorePreQuantDestinationFamily(*preQuantMode);
            if (!isStructuredAccStoreDestinationFamily(destinationElementType, destinationFamily))
                return emitIncompatibleQuantModeError();
        }
    }
    ...
    if (clipValue && !isStructuredAccStoreClipSupportedElementType(destinationElementType))
        return op->emitOpError()
               << "clip requires destination element type to be f16, ui8, or signed 4/8/16-bit integer, got "
               << destinationElementType;
    if (failed(verifyStructuredAccStoreClipPayload(op, destinationElementType, clipValue)))
        return failure();
    ...
    if (!preReluMode) {
        ...
        switch (*preReluMode) {
            ...
            case ReluPreMode::VectorRelu:
                ...
                if (!isStructuredAccStoreFloatScalarPayloadType(getStructuredAccStoreScalingElementType(preRelu)))
                    return op->emitOpError("vector_relu requires scaling pointer element type to be f16, bf16, or f32");
                break;
    ...
    bool hasLoop3 =
        static_cast<bool>(loop3Count) || static_cast<bool>(loop3SrcStride) || static_cast<bool>(loop3DstStride);
        ...
        switch (*mode) {
            ...
            case AccStoreMode::Nz2dn: {
                ...
                if (unitFlag && *unitFlag != AccStoreUnitFlagCtrl::Off &&
                    (!matchPattern(loop0SrcStride, m_ConstantInt(&loop0Value)) || !loop0Value.isOne())) {
                    return op->emitOpError("unit_flag must be off when nz2dn loop0_src_stride is not 1");
                }
                break;
            }
            case AccStoreMode::Nz2nz:
                ...
                if (loop3Count)
                    return op->emitOpError("loop3 requires nz2nd or nz2dn");
                if (!isa<FloatType>(destinationElementType) || !cast<FloatType>(destinationElementType).isF32())
...
static void printStructuredAccStoreClauses(
    OpAsmPrinter& printer, std::optional<AccStoreUnitFlagCtrl> unitFlag, Value preQuant,
    std::optional<AccStoreQuantPreMode> preQuantMode, Value preRelu, std::optional<ReluPreMode> preReluMode,
    Value clipValue, std::optional<AccStoreMode> mode, Value split, Value loop0SrcStride, Value loop3Count,
    Value loop3SrcStride, Value loop3DstStride, std::optional<AccStoreSatMode> satMode,
    std::optional<AccStoreAtomicType> atomicType, std::optional<AccStoreAtomicOp> atomicOp)
{
    if (unitFlag && *unitFlag != AccStoreUnitFlagCtrl::Off) {
        printer << ", unit_flag(" << (*unitFlag == AccStoreUnitFlagCtrl::CheckOnly ? "check_only" : "check_and_clear")
                << ")";
    }
    if (preQuantMode) {
        printer << ", pre_quant(" << preQuant << ", mode = " << stringifyAccStoreQuantPreMode(*preQuantMode) << ")";
    }
    if (preReluMode) {
        printer << ", pre_relu(";
        if (preRelu)
    ...
    if (loop3Count) {
        printer << ", loop3(" << loop3Count << ", " << loop3SrcStride << ", " << loop3DstStride << ")";
    }
    if (satMode) {
        switch (*satMode) {
...
static void printStructuredAccStoreOptionalTypes(
    OpAsmPrinter& printer, Value preQuant, Value preRelu, Value clipValue, Value split, Value loop0SrcStride,
    Value loop3Count, Value loop3SrcStride, Value loop3DstStride)
{
    ...
    if (loop3Count)
        printer << ", " << loop3Count.getType() << ", " << loop3SrcStride.getType() << ", " << loop3DstStride.getType();
}

static ParseResult parseStructuredAccStoreTailTypes(OpAsmParser& parser, StructuredAccStoreAsmState& state)
{
    if (!state.preQuantOperands.empty() &&
        (parser.parseComma() || parseStructuredOptionalType(parser, state.preQuantTypes)))
        return failure();
    if (!state.preReluOperands.empty() &&
        (parser.parseComma() || parseStructuredOptionalType(parser, state.preReluTypes)))
        return failure();
    if (!state.clipValueOperands.empty() &&
        (parser.parseComma() || parseStructuredOptionalType(parser, state.clipValueTypes)))
        return failure();
    if (!state.splitOperands.empty() && (parser.parseComma() || parseStructuredOptionalType(parser, state.splitTypes)))
        return failure();
    if (!state.loop0SrcStrideOperands.empty() &&
        (parser.parseComma() || parseStructuredOptionalType(parser, state.loop0SrcStrideTypes)))
        return failure();
    if (!state.loop3CountOperands.empty() &&
        (parser.parseComma() || parseStructuredOptionalType(parser, state.loop3CountTypes) || parser.parseComma() ||
         parseStructuredOptionalType(parser, state.loop3SrcStrideTypes) || parser.parseComma() ||
...
template <typename OpTy>
static void setStructuredAccStoreSegmentSizes(OperationState& result, ArrayRef<int32_t> segmentSizes)
{
...
template <typename OpTy>
static void addStructuredAccStoreAttrs(
    OperationState& result, Builder& builder, const StructuredAccStoreAsmState& state)
{
    if (state.mode)
        result.addAttribute("mode", AccStoreModeAttr::get(builder.getContext(), *state.mode));
    if (state.unitFlag)
        result.addAttribute("unit_flag", AccStoreUnitFlagCtrlAttr::get(builder.getContext(), *state.unitFlag));
    if (state.preQuantMode)
        result.addAttribute("pre_quant_mode", AccStoreQuantPreModeAttr::get(builder.getContext(), *state.preQuantMode));
    if (state.preReluMode)
        result.addAttribute("pre_relu_mode", ReluPreModeAttr::get(builder.getContext(), *state.preReluMode));
    if (state.atomicType)
        result.addAttribute("atomic_type", AccStoreAtomicTypeAttr::get(builder.getContext(), *state.atomicType));
    if (state.atomicOp)
        result.addAttribute("atomic_op", AccStoreAtomicOpAttr::get(builder.getContext(), *state.atomicOp));
    if (state.satMode)
        result.addAttribute("sat_mode", AccStoreSatModeAttr::get(builder.getContext(), *state.satMode));
}

[[maybe_unused]] static ParseResult resolveStructuredMteL0cL1OptionalOperands(
    OpAsmParser& parser, StructuredAccStoreAsmState& state, SmallVectorImpl<Value>& resolvedOperands,
    OperationState& result)
{
    auto location = parser.getCurrentLocation();
    if (parser.resolveOperands(state.preQuantOperands, state.preQuantTypes, location, result.operands) ||
        parser.resolveOperands(state.preReluOperands, state.preReluTypes, location, result.operands) ||
        parser.resolveOperands(state.clipValueOperands, state.clipValueTypes, location, result.operands) ||
        parser.resolveOperands(state.splitOperands, state.splitTypes, location, result.operands) ||
        parser.resolveOperands(state.loop0SrcStrideOperands, state.loop0SrcStrideTypes, location, result.operands) ||
        parser.resolveOperands(state.loop3CountOperands, state.loop3CountTypes, location, result.operands) ||
        parser.resolveOperands(state.loop3SrcStrideOperands, state.loop3SrcStrideTypes, location, result.operands) ||
        parser.resolveOperands(state.loop3DstStrideOperands, state.loop3DstStrideTypes, location, result.operands))
        return failure();
    ...
    resolvedOperands.push_back(extractResolved(state.preQuantOperands, state.preQuantTypes));
    resolvedOperands.push_back(extractResolved(state.preReluOperands, state.preReluTypes));
    resolvedOperands.push_back(extractResolved(state.clipValueOperands, state.clipValueTypes));
    resolvedOperands.push_back(extractResolved(state.splitOperands, state.splitTypes));
    resolvedOperands.push_back(extractResolved(state.loop0SrcStrideOperands, state.loop0SrcStrideTypes));
    resolvedOperands.push_back(extractResolved(state.loop3CountOperands, state.loop3CountTypes));
    resolvedOperands.push_back(extractResolved(state.loop3SrcStrideOperands, state.loop3SrcStrideTypes));
    resolvedOperands.push_back(extractResolved(state.loop3DstStrideOperands, state.loop3DstStrideTypes));
    return success();
}
...
{
    if (!isBufferLike(op.getSource().getType()) || !isBufferLike(op.getDestination().getType()))
        return op.emitOpError("requires typed !pto.ptr or memref source and destination");
    ...
    if (!directionMatches) {
        return op.emitOpError() << "requires "
                                << (expectSourceGM ? "GM source and UB destination" : "UB source and GM destination");
    }
...
static LogicalResult verifyOptionalDmaLoopGroup(DmaOp op, Value count, Value srcStride, Value dstStride, StringRef name)
{
    bool hasAny = static_cast<bool>(count) || static_cast<bool>(srcStride) || static_cast<bool>(dstStride);
    bool hasAll = static_cast<bool>(count) && static_cast<bool>(srcStride) && static_cast<bool>(dstStride);
    if (hasAny && !hasAll)
...
static LogicalResult verifyDmaLoadStoreLoopGroups(
    Operation* op, ValueRange loopCounts, ValueRange loopSrcStrides, ValueRange loopDstStrides)
{
    if (loopCounts.size() != loopSrcStrides.size() || loopCounts.size() != loopDstStrides.size())
        return op->emitOpError() << "requires each loop group to provide count, src stride, and dst stride together";
    return success();
}
...
Type VRegType::parse(AsmParser& parser)
{
    ...
    if (failed(parser.parseLess()) ||
        failed(parser.parseDimensionList(
            shape, /*allowDynamic=*/false,
            /*withTrailingX=*/true)) ||
        shape.size() != 1 || failed(parser.parseType(elementType)) || failed(parser.parseGreater()))
        return {};

    return parser.getChecked<VRegType>(loc, parser.getContext(), shape.front(), elementType);
}
...
LogicalResult VRegType::verify(function_ref<InFlightDiagnostic()> emitError, int64_t elementCount, Type elementType)
{
    if (elementCount <= 0)
        return emitError() << "'" << formatVRegType(elementCount, elementType) << "' expected a positive element count";
    ...
    if (elementCount * static_cast<int64_t>(elementBitWidth) != 2048)
        return emitError() << "'" << formatVRegType(elementCount, elementType) << "' expected exactly 256 bytes";
...
LogicalResult StrictVecScopeOp::verify()
{
    ...
    if (body.getNumArguments() != getCaptures().size())
        return emitOpError() << "expects body block to have " << getCaptures().size()
                             << " arguments to match explicit captures, got " << body.getNumArguments();

    for (auto [idx, pair] : llvm::enumerate(llvm::zip(body.getArguments(), getCaptures()))) {
        BlockArgument blockArg = std::get<0>(pair);
        Value capture = std::get<1>(pair);
        if (blockArg.getType() != capture.getType())
            return emitOpError() << "expects body block argument #" << idx << " to have type " << capture.getType()
...
Type MaskType::parse(AsmParser& parser)
{
    ...
    if (failed(parser.parseLess()) || failed(parser.parseKeyword(&granularity)) || failed(parser.parseGreater()))
        return {};

    return parser.getChecked<MaskType>(loc, parser.getContext(), granularity);
}

void MaskType::print(AsmPrinter& printer) const { printer << "<" << getGranularity() << ">"; }

LogicalResult MaskType::verify(function_ref<InFlightDiagnostic()> emitError, StringRef granularity)
{
    if (!isSupportedGranularity(granularity))
        return emitError() << "'" << formatMaskType(granularity) << "' expected granularity to be one of b8, b16, b32";
    return success();
}

void CopyGmToUbufOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

LogicalResult CopyGmToUbufOp::verify() { return verifyCopyGmToUbufOp(*this, true); }

void MteGmUbOp::build(
    OpBuilder& builder, OperationState& state, Value source, Value destination, Value l2CacheCtl, Value lenBurst,
    pto::DmaLoopConfig nburst, llvm::ArrayRef<pto::DmaLoopConfig> loops, std::optional<pto::DmaPadConfig> pad)
{
    state.addOperands({source, destination, l2CacheCtl, lenBurst, nburst.count, nburst.srcStride, nburst.dstStride});
    ...
    assert(
        (!pad || static_cast<bool>(pad->leftCount) == static_cast<bool>(pad->rightCount)) &&
    ...
    state.addAttribute(
        ...
        builder.getDenseI32ArrayAttr(
            {1, 1, 1, 1, 1, 1, 1, static_cast<int32_t>(loops.size()), static_cast<int32_t>(loops.size()),
             static_cast<int32_t>(loops.size()), pad ? 1 : 0, hasPadCounts ? 1 : 0, hasPadCounts ? 1 : 0}));
}

void MteGmUbOp::build(
    OpBuilder& builder, OperationState& state, Value source, Value destination, Value l2CacheCtl, Value lenBurst,
    pto::DmaLoopConfig nburst, std::optional<pto::DmaLoopConfig> loop1, std::optional<pto::DmaLoopConfig> loop2,
    std::optional<pto::DmaPadConfig> pad)
{
    ...
    build(builder, state, source, destination, l2CacheCtl, lenBurst, nburst, loops, pad);
}
...
ParseResult MteGmUbOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parseRequiredOperandWithComma(parser, source) || parseRequiredOperandWithComma(parser, destination) ||
        parseRequiredOperandWithComma(parser, l2CacheCtl) || parser.parseOperand(lenBurst) ||
    ...
    while (true) {
        if (succeeded(parser.parseOptionalKeyword("pad"))) {
            ...
            if (succeeded(parser.parseOptionalComma())) {
                ...
                if (parser.parseOperand(left) || parser.parseComma() || parser.parseOperand(right))
                    return failure();
        ...
        if (parseOptionalDmaTripleGroupAlias(parser, {"loop", "loop1", "loop2"}, parsedKeyword, loopGroupOperands))
            return failure();
        if (parsedKeyword.empty())
            break;
    ...
    SmallVector<Type> nburstTypes, loopCountTypes, loopSrcStrideTypes, loopDstStrideTypes, padTypes;
    if (parser.parseType(sourceType) || parser.parseComma() || parser.parseType(destinationType) ||
        parser.parseComma() || parser.parseType(l2CacheCtlType) || parser.parseComma() ||
        parser.parseType(lenBurstType) || parser.parseComma() || parseDmaTripleTypes(parser, nburstTypes))
        return failure();
    while (succeeded(parser.parseOptionalComma())) {
        ...
        return parser.emitError(parser.getCurrentLocation(), "expected one of 'loop' or 'pad'");
    }
    ...
    if (loopCountOperands.size() != loopSrcStrideOperands.size() ||
        ...
        loopCountTypes.size() != loopSrcStrideTypes.size() || loopCountTypes.size() != loopDstStrideTypes.size())
        return parser.emitError(
            parser.getCurrentLocation(), "requires each loop group to provide count, src stride, and dst stride");
    if (loopCountOperands.size() != loopCountTypes.size())
        return parser.emitError(parser.getCurrentLocation(), "requires loop operand and type groups to match");

    auto& segments = result.getOrAddProperties<MteGmUbOp::Properties>().operandSegmentSizes;
    llvm::copy(
        ArrayRef<int32_t>{
            1, 1, 1, 1, 1, 1, 1, loopGroupCount, loopGroupCount, loopGroupCount,
            static_cast<int32_t>(padOperands.size() ? 1 : 0), static_cast<int32_t>(padOperands.size() == 3 ? 1 : 0),
    ...
    if (parser.resolveOperand(source, sourceType, result.operands) ||
        ...
        parser.resolveOperands(nburstOperands, nburstTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(loopCountOperands, loopCountTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            loopSrcStrideOperands, loopSrcStrideTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            loopDstStrideOperands, loopDstStrideTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(padOperands, padTypes, parser.getCurrentLocation(), result.operands))
        return failure();
    return success();
}
...
{
    printer << " " << getSource() << ", " << getDestination() << ", " << getL2CacheCtl() << ", " << getLenBurst();
    printDmaTripleGroup(printer, "nburst", getNBurst(), getNburstSrcStride(), getNburstDstStride());
    for (auto [count, srcStride, dstStride] : llvm::zip(getLoopCounts(), getLoopSrcStrides(), getLoopDstStrides()))
        printDmaTripleGroup(printer, "loop", count, srcStride, dstStride);
    if (getPadValue())
        printDmaPadGroup(printer, getPadValue(), getLeftPaddingCount(), getRightPaddingCount());
    printer.printOptionalAttrDict((*this)->getAttrs());
    printer << " : " << getSource().getType() << ", " << getDestination().getType() << ", " << getL2CacheCtl().getType()
            << ", " << getLenBurst().getType() << ", " << getNBurst().getType() << ", "
            << getNburstSrcStride().getType() << ", " << getNburstDstStride().getType();
    for (auto [count, srcStride, dstStride] : llvm::zip(getLoopCounts(), getLoopSrcStrides(), getLoopDstStrides()))
        printDmaTripleTypes(printer, "loop", count.getType(), srcStride.getType(), dstStride.getType());
    if (getPadValue())
        printDmaPadTypes(
            printer, getPadValue().getType(), getLeftPaddingCount() ? getLeftPaddingCount().getType() : Type{},
            getRightPaddingCount() ? getRightPaddingCount().getType() : Type{});
}

void MteGmUbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
LogicalResult MteGmUbOp::verify()
{
    if (failed(verifyCopyGmToUbufOp(*this, true)))
        return failure();
    if (failed(verifyDmaLoadStoreLoopGroups(getOperation(), getLoopCounts(), getLoopSrcStrides(), getLoopDstStrides())))
        return failure();
    if (!getPadValue() && (getLeftPaddingCount() || getRightPaddingCount()))
        return emitOpError() << "requires pad group to provide a pad value";
    if (getPadValue() && static_cast<bool>(getLeftPaddingCount()) != static_cast<bool>(getRightPaddingCount()))
        return emitOpError() << "requires pad group to provide both left and right counts, or omit both";
    if (Value padValue = getPadValue()) {
        Type valueType = padValue.getType();
        if (!isSupportedMovPadScalarType(valueType))
            return emitOpError() << "expects pad value to be i8/i16/i32 or f16/bf16/f32 scalar, but got " << valueType;
    }
    return success();
}
...
LogicalResult SetMovPadValOp::verify()
{
    ...
    if (isSupportedMovPadScalarType(valueType))
        return success();
    return emitOpError() << "expects i8/i16/i32 or f16/bf16/f32 scalar operand, but got " << valueType;
}
void MadOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
static LogicalResult verifyMadPointerKinds(
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, std::optional<Type> biasTy = std::nullopt)
{
    ...
    const bool isStrongCube =
        lhsAS == pto::AddressSpace::LEFT && rhsAS == pto::AddressSpace::RIGHT && dstAS == pto::AddressSpace::ACC;
    if (!isStrongCube)
...
void MadAccOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getLhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getRhsMutable());
...
void MadBiasOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getLhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getRhsMutable());
...
static LogicalResult verifyMadMxCommon(
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, std::optional<Type> biasTy = std::nullopt)
{
    ...
    if (!isMxElementType(lhsType.getElementType()) || !isMxElementType(rhsType.getElementType())) {
        return op->emitOpError("requires MX lhs/rhs element types (f8E4M3FN, f8E5M2, f4E1M2x2, or "
                               "f4E2M1x2)");
    }
    return success();
}

void MadMxOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getLhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getRhsMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
void MadMxAccOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getLhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getRhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getDstMutable());
...
void MadMxBiasOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getLhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getRhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getBiasMutable());
...
static std::optional<pto::MadUnitFlagMode> parseMadUnitFlagModeToken(StringRef token)
{
    if (token == "check_only")
...
static LogicalResult verifyMadSemanticClauses(
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, std::optional<Type> biasTy,
    std::optional<pto::Tf32Mode> tf32Mode, std::optional<pto::MadSatMode> satMode, bool hasNDir)
{
    if (failed(verifyMadPointerKinds(op, lhsTy, rhsTy, dstTy, biasTy)))
        return failure();
    ...
    if (tf32Mode) {
        if (!(lhsType.getElementType().isF32() && rhsType.getElementType().isF32() &&
            ...
            return op->emitOpError("requires tf32_mode only for f32 lhs/rhs/dst element types");
        }
    }
    if (pto::isPTOHiFloat8Type(lhsType.getElementType()) != pto::isPTOHiFloat8Type(rhsType.getElementType())) {
        return op->emitOpError("requires lhs/rhs to both use hif8 or both use non-hif8 element types");
    }
    if (satMode) {
        ...
        if (!(isFloatLike(lhsType.getElementType()) && isFloatLike(rhsType.getElementType()) &&
              isFloatLike(dstType.getElementType()))) {
            return op->emitOpError("requires sat/nosat only for floating lhs/rhs/dst element types");
        }
    }
    (void)hasNDir;
    return success();
...
static ParseResult parseMadSemanticOpCommon(
    OpAsmParser& parser, OperationState& result, bool hasBias, bool parseTf32ModeClause)
{
    ...
    if (parseRequiredOperandWithComma(parser, lhs) || parseRequiredOperandWithComma(parser, rhs) ||
        parseRequiredOperandWithComma(parser, dst) || (hasBias && parseRequiredOperandWithComma(parser, bias)) ||
        parseRequiredOperandWithComma(parser, m) || parseRequiredOperandWithComma(parser, n) || parser.parseOperand(k))
        return failure();

    auto parseUnitFlagClause = [&]() -> ParseResult {
        if (failed(parser.parseOptionalKeyword("unit_flag")))
            return success();
        if (parser.parseLParen() || parser.parseKeyword(&unitFlagKeyword) || parser.parseRParen())
            return failure();
        auto mode = parseMadUnitFlagModeToken(unitFlagKeyword);
        if (!mode)
            return parser.emitError(parser.getCurrentLocation()) << "expected unit_flag(check_only|check_and_set)";
        attrs.set("unit_flag_mode", pto::MadUnitFlagModeAttr::get(parser.getContext(), *mode));
        return success();
    };
    ...
    auto parseSatClause = [&]() -> ParseResult {
        if (succeeded(parser.parseOptionalKeyword("sat"))) {
            attrs.set("sat_mode", pto::MadSatModeAttr::get(parser.getContext(), pto::MadSatMode::Sat));
            return success();
        }
        if (succeeded(parser.parseOptionalKeyword("nosat"))) {
            attrs.set("sat_mode", pto::MadSatModeAttr::get(parser.getContext(), pto::MadSatMode::NoSat));
        }
        return success();
    };
    auto parseTf32Clause = [&]() -> ParseResult {
        ...
        if (failed(parser.parseOptionalKeyword("tf32_mode")))
            return success();
        if (parser.parseLParen() || parser.parseKeyword(&tf32Keyword) || parser.parseRParen())
            return failure();
        auto mode = parseTf32ModeToken(tf32Keyword);
        if (!mode)
            return parser.emitError(parser.getCurrentLocation()) << "expected tf32_mode(round_even|round_away)";
    ...
    if (failed(parseUnitFlagClause()) || failed(parseDisableGemvClause()) || failed(parseSatClause()) ||
        failed(parseTf32Clause()) || failed(parseNDirClause()))
        return failure();
    ...
    if (parser.parseType(lhsType) || parser.parseComma() || parser.parseType(rhsType) || parser.parseComma() ||
    ...
    if (parser.parseType(mType) || parser.parseComma() || parser.parseType(nType) || parser.parseComma() ||
    ...
    if (hasBias) {
        if (parser.resolveOperand(lhs, lhsType, result.operands) ||
            ...
            parser.resolveOperand(m, mType, result.operands) || parser.resolveOperand(n, nType, result.operands) ||
            parser.resolveOperand(k, kType, result.operands))
            return failure();
    } else {
        if (parser.resolveOperand(lhs, lhsType, result.operands) ||
            parser.resolveOperand(rhs, rhsType, result.operands) ||
            parser.resolveOperand(dst, dstType, result.operands) || parser.resolveOperand(m, mType, result.operands) ||
            parser.resolveOperand(n, nType, result.operands) || parser.resolveOperand(k, kType, result.operands))
            return failure();
    }
    return success();
}

static void printMadSemanticClauses(OpAsmPrinter& printer, Operation* op, bool allowTf32Mode)
{
    if (auto unitFlagMode = op->getAttrOfType<pto::MadUnitFlagModeAttr>("unit_flag_mode")) {
        printer << " unit_flag(" << stringifyMadUnitFlagModeToken(unitFlagMode.getValue()) << ")";
    }
...
{
    static constexpr StringRef kWithTf32[] = {"unit_flag_mode", "disable_gemv", "sat_mode", "tf32_mode", "n_dir"};
    static constexpr StringRef kWithoutTf32[] = {"unit_flag_mode", "disable_gemv", "sat_mode", "n_dir"};
    return allowTf32Mode ? ArrayRef<StringRef>(kWithTf32) : ArrayRef<StringRef>(kWithoutTf32);
}

template <typename OpT>
static void printMadSemanticOpNoBias(OpAsmPrinter& printer, OpT op, bool allowTf32Mode)
{
    printer << ' ' << op.getLhs() << ", " << op.getRhs() << ", " << op.getDst() << ", " << op.getM() << ", "
            << op.getN() << ", " << op.getK();
    printMadSemanticClauses(printer, op, allowTf32Mode);
    printer.printOptionalAttrDict(op->getAttrs(), getMadSemanticElidedAttrs(allowTf32Mode));
    printer << " : " << op.getLhs().getType() << ", " << op.getRhs().getType() << ", " << op.getDst().getType() << ", "
            << op.getM().getType() << ", " << op.getN().getType() << ", " << op.getK().getType();
}

template <typename OpT>
static void printMadSemanticOpWithBias(OpAsmPrinter& printer, OpT op, bool allowTf32Mode)
{
    printer << ' ' << op.getLhs() << ", " << op.getRhs() << ", " << op.getDst() << ", " << op.getBias() << ", "
            << op.getM() << ", " << op.getN() << ", " << op.getK();
    printMadSemanticClauses(printer, op, allowTf32Mode);
    printer.printOptionalAttrDict(op->getAttrs(), getMadSemanticElidedAttrs(allowTf32Mode));
    printer << " : " << op.getLhs().getType() << ", " << op.getRhs().getType() << ", " << op.getDst().getType() << ", "
            << op.getBias().getType() << ", " << op.getM().getType() << ", " << op.getN().getType() << ", "
            << op.getK().getType();
}
...
LogicalResult MadOp::verify()
{
    ...
    if (auto tf32ModeAttr = (*this)->getAttrOfType<pto::Tf32ModeAttr>("tf32_mode"))
        tf32Mode = tf32ModeAttr.getValue();
    return verifyMadSemanticClauses(
        *this, getLhs().getType(), getRhs().getType(), getDst().getType(), std::nullopt, tf32Mode, getSatMode(),
        (*this)->hasAttr("n_dir"));
}

ParseResult MadOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseMadSemanticOpCommon<MadOp>(
        parser, result, /*hasBias=*/false,
        /*parseTf32ModeClause=*/true);
}

void MadOp::print(OpAsmPrinter& printer) { printMadSemanticOpNoBias(printer, *this, /*allowTf32Mode=*/true); }
...
{
    return parseMadSemanticOpCommon<MadAccOp>(
        parser, result, /*hasBias=*/false,
        /*parseTf32ModeClause=*/true);
}

void MadAccOp::print(OpAsmPrinter& printer) { printMadSemanticOpNoBias(printer, *this, /*allowTf32Mode=*/true); }
...
LogicalResult MadBiasOp::verify()
{
    ...
    if (auto tf32ModeAttr = (*this)->getAttrOfType<pto::Tf32ModeAttr>("tf32_mode"))
        tf32Mode = tf32ModeAttr.getValue();
    return verifyMadSemanticClauses(
        *this, getLhs().getType(), getRhs().getType(), getDst().getType(), getBias().getType(), tf32Mode, getSatMode(),
        (*this)->hasAttr("n_dir"));
}

ParseResult MadBiasOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseMadSemanticOpCommon<MadBiasOp>(
        parser, result, /*hasBias=*/true,
        /*parseTf32ModeClause=*/true);
}

void MadBiasOp::print(OpAsmPrinter& printer) { printMadSemanticOpWithBias(printer, *this, /*allowTf32Mode=*/true); }
...
{
    if (failed(verifyMadMxCommon(*this, getLhs().getType(), getRhs().getType(), getDst().getType())))
        return failure();
    return verifyMadSemanticClauses(
        *this, getLhs().getType(), getRhs().getType(), getDst().getType(), std::nullopt, std::nullopt, getSatMode(),
        (*this)->hasAttr("n_dir"));
}

ParseResult MadMxOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseMadSemanticOpCommon<MadMxOp>(
        parser, result, /*hasBias=*/false,
        /*parseTf32ModeClause=*/false);
}

void MadMxOp::print(OpAsmPrinter& printer) { printMadSemanticOpNoBias(printer, *this, /*allowTf32Mode=*/false); }
...
{
    return parseMadSemanticOpCommon<MadMxAccOp>(
        parser, result, /*hasBias=*/false,
        /*parseTf32ModeClause=*/false);
}

void MadMxAccOp::print(OpAsmPrinter& printer) { printMadSemanticOpNoBias(printer, *this, /*allowTf32Mode=*/false); }
...
{
    if (failed(
            verifyMadMxCommon(*this, getLhs().getType(), getRhs().getType(), getDst().getType(), getBias().getType())))
        return failure();
    return verifyMadSemanticClauses(
        *this, getLhs().getType(), getRhs().getType(), getDst().getType(), getBias().getType(), std::nullopt,
        getSatMode(), (*this)->hasAttr("n_dir"));
}

ParseResult MadMxBiasOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseMadSemanticOpCommon<MadMxBiasOp>(
        parser, result, /*hasBias=*/true,
        /*parseTf32ModeClause=*/false);
}

void MadMxBiasOp::print(OpAsmPrinter& printer) { printMadSemanticOpWithBias(printer, *this, /*allowTf32Mode=*/false); }
...
void MadRawOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getLhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getRhsMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
{
    return verifyMadPointerKinds(*this, getLhs().getType(), getRhs().getType(), getDst().getType());
}
...
void MadBiasRawOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getLhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getRhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getBiasMutable());
...
{
    return verifyMadPointerKinds(
        *this, getLhs().getType(), getRhs().getType(), getDst().getType(), getBias().getType());
}
...
void MadMxRawOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getLhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getRhsMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
{
    return verifyMadMxCommon(*this, getLhs().getType(), getRhs().getType(), getDst().getType());
}
...
void MadMxBiasRawOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getLhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getRhsMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getBiasMutable());
...
{
    return verifyMadMxCommon(*this, getLhs().getType(), getRhs().getType(), getDst().getType(), getBias().getType());
}
...
static LogicalResult verifyWideningReductionVecOp(ReductionOp op, StringRef opName)
{
    ...
    if (auto inputInt = dyn_cast<IntegerType>(inputElemType)) {
        if (inputInt.getWidth() < 8 || inputInt.getWidth() > 32)
            return op.emitOpError("requires 8-bit, 16-bit, or 32-bit integer vector element type");
        if (inputInt.getWidth() == 8) {
            expectedResultElemType = IntegerType::get(op.getContext(), 16, inputInt.getSignedness());
            expectedResultLanes = inputType.getElementCount() / 2;
        }
        if (inputInt.getWidth() == 16) {
            expectedResultElemType = IntegerType::get(op.getContext(), 32, inputInt.getSignedness());
            expectedResultLanes = inputType.getElementCount() / 2;
        }
    ...
    if (resultType.getElementCount() == expectedResultLanes && resultType.getElementType() == expectedResultElemType)
        return success();

    return op.emitOpError() << opName << " expects result type !pto.vreg<" << expectedResultLanes << "x"
                            << expectedResultElemType << " for input element type " << inputElemType;
}

LogicalResult VcaddOp::verify() { return verifyWideningReductionVecOp(*this, "vcadd"); }
...
LogicalResult VciOp::verify()
{
    ...
    if (auto intType = dyn_cast<IntegerType>(resultElemType))
        supportedInteger = intType.getWidth() == 8 || intType.getWidth() == 16 || intType.getWidth() == 32;
...
void Vgather2Op::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
static bool isSameVgather2IntegerSemantics(IntegerType sourceType, IntegerType resultType)
{
...
static bool isVgather2B8ResultType(IntegerType sourceType, IntegerType resultType)
{
    if (!sourceType || !resultType || sourceType.getWidth() != 8 || resultType.getWidth() != 16)
        return false;
    if (sourceType.isUnsigned())
        return resultType.isUnsigned();
    return !resultType.isUnsigned();
...
LogicalResult Vgather2Op::verify()
{
    ...
    if (sourceElemWidth == 8 && isa<IntegerType>(sourceElemType)) {
        if (!isVgather2B8ResultType(cast<IntegerType>(sourceElemType), dyn_cast<IntegerType>(resultElemType)))
            return emitOpError("8-bit gather requires i8/ui8 source and matching i16/ui16 result");
        ...
        if (auto sourceInt = dyn_cast<IntegerType>(sourceElemType)) {
            if (!isSameVgather2IntegerSemantics(sourceInt, dyn_cast<IntegerType>(resultElemType)))
                return emitOpError("16-bit integer gather requires matching i16/ui16 result");
        } else if (!(sourceElemType.isF16() || sourceElemType.isBF16()) || sourceElemType != resultElemType) {
            return emitOpError("16-bit gather requires i16/ui16/f16/bf16 source and matching result");
        }
        expectedOffsetWidth = 16;
        expectedMaskGranularity = "b16";
        expectedLanes = 128;
        ...
        if (auto sourceInt = dyn_cast<IntegerType>(sourceElemType)) {
            if (!isSameVgather2IntegerSemantics(sourceInt, dyn_cast<IntegerType>(resultElemType)))
                return emitOpError("32-bit integer gather requires matching i32/ui32 result");
        } else if (!sourceElemType.isF32() || sourceElemType != resultElemType) {
            return emitOpError("32-bit gather requires i32/ui32/f32 source and matching result");
        }
        expectedOffsetWidth = 32;
        expectedMaskGranularity = "b32";
        expectedLanes = 64;
    } else {
        return emitOpError("requires source element type i8/ui8/i16/ui16/i32/ui32/f16/bf16/f32");
    }
    ...
    if (resultType.getElementCount() != expectedLanes)
        return emitOpError() << "expects result type " << formatVRegType(expectedLanes, resultElemType);
    if (!isUnsignedOrSignlessIntegerOfWidth(offsetsElemType, expectedOffsetWidth))
        return emitOpError() << "requires ui" << expectedOffsetWidth << "/i" << expectedOffsetWidth
                             << " offset vector elements";
    if (failed(verifyMaskTypeWithGranularityLike(
            getOperation(), getMask().getType(), "mask type", expectedMaskGranularity)))
        return failure();
    return success();
}
...
void CopyCbufToUbufOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

LogicalResult CopyCbufToUbufOp::verify() { return verifyCopyCbufToUbufLikeOp(*this); }

void CopyUbufToCbufOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void MteUbUbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void MteUbL1Op::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void VgatherbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
LogicalResult VgatherbOp::verify()
{
    ...
    if (failed(verifyMaskTypeWithGranularityLike(getOperation(), getMask().getType(), "mask type", "b32")))
        return failure();
...
void Vgather2BcOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
void VbitsortOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
...
LogicalResult Vmrgsort4Op::verify()
{
    ...
    if (!dstPtrType || !src0PtrType || !src1PtrType || !src2PtrType || !src3PtrType)
        return emitOpError("requires ptr-backed destination and sources");

    Type elemType = dstPtrType.getElementType();
    if (src0PtrType.getElementType() != elemType || src1PtrType.getElementType() != elemType ||
        src2PtrType.getElementType() != elemType || src3PtrType.getElementType() != elemType)
        return emitOpError("requires destination and all sources to have the same element type");
...
LogicalResult VmaxOp::verify()
{
    if (failed(verifyVRegTypeLike(*this, getLhs().getType(), "lhs")) ||
        ...
        failed(verifyVRegTypeLike(*this, getResult().getType(), "result")))
        return failure();
    if (getLhs().getType() != getRhs().getType() || getLhs().getType() != getResult().getType())
...
void VldsOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
static LogicalResult verifyVldsCommon(LoadOp op)
{
    ...
    if (op.getDistAttr()) {
        ...
        if (!isSupportedVldsDistToken(dist))
            return op.emitOpError("supports only NORM, BRC_B8/B16/B32, US_B8/B16, DS_B8/B16, "
...
void VldasOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
LogicalResult InitAlignOp::verify() { return verifyAlignTypeLike(*this, getResult().getType(), "result type"); }
...
static LogicalResult verifySprStoreCommon(
    Operation* op, StringRef opName, StringRef spr, Value destination, Value offset, bool requireImmediateOffset)
{
...
void SprstiOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
{
    if (failed(verifySprStoreCommon(
            getOperation(), "pto.sprsti", getSpr(), getDestination(), getOffset(),
            /*requireImmediateOffset=*/true)))
        return failure();
    if (getUpdatedBase() && getUpdatedBase().getType() != getDestination().getType())
        return emitOpError("requires updated base result to match base type");
    return success();
}

void SprstsOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
{
    if (failed(verifySprStoreCommon(
            getOperation(), "pto.sprsts", getSpr(), getDestination(), getOffset(),
...
void VldusOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
LogicalResult VldusOp::verify()
{
    if (failed(verifyLoadAlignChain(getAlign(), *this, "align type")) ||
        ...
        failed(verifyAlignTypeLike(*this, getUpdatedAlign().getType(), "updated align type")))
        return failure();
    if (!isBufferLike(getSource().getType()))
        return emitOpError("requires a pointer-like source");
    if (classifyMemoryRole(getSource().getType()) == MemoryRole::GM)
    ...
    if (static_cast<bool>(getIncrement()) != static_cast<bool>(getUpdatedBase()))
        return emitOpError("requires increment and updated base result to appear together");
...
void UvldOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
LogicalResult UvldOp::verify()
{
    ...
    if (sourceElementType != vectorElementType)
        return emitOpError("requires source element type to match vector element type");
    return success();
}
...
LogicalResult VdupOp::verify()
{
    ...
    std::optional<StringRef> granularity = getVdupMaskGranularity(resultType.getElementType());
    if (!granularity)
        return emitOpError("result element type must use b8, b16, or b32 mask granularity");
    if (failed(verifyMaskTypeWithGranularityLike(getOperation(), getMask().getType(), "mask type", *granularity)))
        return failure();
...
LogicalResult TensorViewAddrOp::verify()
{
    ...
    if (auto dstMemRefType = dyn_cast<BaseMemRefType>(dstType)) {
        ...
        auto dstSpace = dyn_cast_or_null<pto::AddressSpaceAttr>(dstMemRefType.getMemorySpace());
...
{
    if (failed(verifyMaskTypeWithGranularityLike(*this, getResult().getType(), "result type", "b8")))
        return failure();
...
{
    if (failed(verifyMaskTypeWithGranularityLike(*this, getResult().getType(), "result type", "b16")))
        return failure();
...
{
    if (failed(verifyMaskTypeWithGranularityLike(*this, getResult().getType(), "result type", "b32")))
        return failure();
    if (!isSupportedPredicatePattern(getPattern()))
        return emitOpError("requires a supported PAT_* predicate pattern");
    return success();
...
static LogicalResult verifyPredicateLaneCountOp(PltOp op, StringRef granularity)
{
    if (failed(verifyMaskTypeWithGranularityLike(op, op.getMask().getType(), "mask type", granularity)))
        return failure();
...
LogicalResult PltB16Op::verify() { return verifyPredicateLaneCountOp(*this, "b16"); }
LogicalResult PltB32Op::verify() { return verifyPredicateLaneCountOp(*this, "b32"); }

template <typename PltmOp>
static LogicalResult verifyPredicateLoopBoundOp(PltmOp op, StringRef granularity)
{
    if (failed(verifyMaskTypeWithGranularityLike(op, op.getMask().getType(), "mask type", granularity)))
        return failure();
...
LogicalResult PltmB8Op::verify() { return verifyPredicateLoopBoundOp(*this, "b8"); }
LogicalResult PltmB16Op::verify() { return verifyPredicateLoopBoundOp(*this, "b16"); }
LogicalResult PltmB32Op::verify() { return verifyPredicateLoopBoundOp(*this, "b32"); }
...
LogicalResult PpackOp::verify()
{
    ...
    if (inputGranularity != resultGranularity &&
        ...
        return emitOpError("requires result mask granularity to match the input or narrow by one step");
    }
    return success();
}
...
LogicalResult PunpackOp::verify()
{
    ...
    if (inputGranularity != resultGranularity &&
        ...
        return emitOpError("requires result mask granularity to match the input or widen by one step");
    }
    return success();
}
...
void PldsOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
void PldiOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
static LogicalResult verifyElementwiseVecScalarOpLike(OpTy op)
{
    ...
    if (elemInt.isSigned() && (scalarInt.isSigned() || scalarInt.isSignless()))
        return success();
    if (elemInt.isUnsigned() && (scalarInt.isUnsigned() || scalarInt.isSignless()))
        return success();
...
static LogicalResult verifyVecScalarMaskedOpLike(OpTy op)
{
    ...
    if (failed(verifyMaskTypeLike(op, op.getMask().getType(), "mask type")))
        return failure();
    if (failed(verifyNonLowPrecisionVRegElementTypeLike(op.getOperation(), op.getInput().getType(), "input type")))
        return failure();
    return success();
}
...
static LogicalResult verifyCarryVecOp(CarryOp op)
{
    if (failed(verifyIntegerVRegTypeLike(op, op.getLhs().getType(), "lhs type")) ||
        ...
        failed(verifyMaskTypeLike(op, op.getMask().getType(), "mask type")) ||
        failed(verifyIntegerVRegTypeLike(op, op.getResult().getType(), "result type")) ||
...
{
    if (failed(verifyCarryVecOp(op)) || failed(verifyMaskTypeLike(op, op.getCarryIn().getType(), "carry_in type")))
        return failure();
    return success();
}
...
static LogicalResult verifyUnaryVecOp(UnaryOp op)
{
    ...
    if (failed(verifyVRegTypeLike(op, op.getResult().getType(), "result type")))
        return failure();
    if (failed(verifyNonLowPrecisionVRegElementTypeLike(op.getOperation(), op.getInput().getType(), "operand type")))
        return failure();
...
static LogicalResult verifyBinaryVecOp(BinaryOp op, bool allowLowPrecision = false)
{
    ...
    if (!allowLowPrecision &&
        failed(verifyNonLowPrecisionVRegElementTypeLike(op.getOperation(), op.getLhs().getType(), "lhs type")))
        return failure();
    if (op.getLhs().getType() != op.getRhs().getType() || op.getLhs().getType() != op.getResult().getType())
...
LogicalResult VandOp::verify() { return verifyBinaryVecOp(*this, /*allowLowPrecision=*/true); }
LogicalResult VorOp::verify() { return verifyBinaryVecOp(*this, /*allowLowPrecision=*/true); }
LogicalResult VxorOp::verify() { return verifyBinaryVecOp(*this, /*allowLowPrecision=*/true); }
...
static LogicalResult verifyTernaryVecOp(TernaryOp op)
{
    if (failed(verifyVRegTypeLike(op, op.getAcc().getType(), "acc type")) ||
        ...
        return failure();
    if (op.getAcc().getType() != op.getLhs().getType() || op.getAcc().getType() != op.getRhs().getType() ||
        op.getAcc().getType() != op.getResult().getType()) {
        return op.emitOpError("requires acc, lhs, rhs, and result to share one vector type");
    }
    return success();
}
...
static LogicalResult verifyGroupReductionVecOp(ReductionOp op)
{
    ...
    Type elemType = inputType.getElementType();
    if (auto intType = dyn_cast<IntegerType>(elemType)) {
        if (intType.getWidth() != 8 && intType.getWidth() != 16 && intType.getWidth() != 32)
            return op.emitOpError("requires 8-bit, 16-bit, or 32-bit integer vector element type");
        return success();
    }
    if (!elemType.isF16() && !elemType.isF32())
...
static LogicalResult verifyHistogramOp(HistOp op)
{
    if (failed(verifyVRegTypeLike(op, op.getAcc().getType(), "acc type")) ||
        ...
        failed(verifyMaskTypeWithGranularityLike(op, op.getMask().getType(), "mask type", "b8")) ||
        failed(verifyVRegTypeLike(op, op.getResult().getType(), "result type")))
        return failure();
    ...
    if (!accElemType || accElemType.getWidth() != 16 || accType.getElementCount() != 128)
        return op.emitOpError("requires acc type to be !pto.vreg<128xi16>");
    if (!sourceElemType || sourceElemType.getWidth() != 8 || sourceType.getElementCount() != 256)
...
static LogicalResult verifyExtremaPredicateOp(ExtremaOp op)
{
    if (failed(verifyVRegTypeLike(op, op.getInput().getType(), "input type")) ||
        ...
        failed(verifyMaskTypeLike(op, op.getPredicate().getType(), "predicate type")))
        return failure();
    if (op.getInput().getType() != op.getValue().getType())
        return op.emitOpError("requires input and value result to share one vector type");
    if (op.getMask().getType() != op.getPredicate().getType())
        return op.emitOpError("requires mask and predicate result to share one mask type");
...
static LogicalResult verifyPairVecResults(PairOp op)
{
    if (failed(verifyVRegTypeLike(op, op.getLhs().getType(), "lhs type")) ||
        ...
        return failure();
    if (op.getLhs().getType() != op.getRhs().getType() || op.getLhs().getType() != op.getLow().getType() ||
...
LogicalResult VselOp::verify()
{
    if (failed(verifyVRegTypeLike(*this, getSrc0().getType(), "src0 type")) ||
        ...
        failed(verifyVRegTypeLike(*this, getResult().getType(), "result type")))
        return failure();
    if (failed(verifyNonLowPrecisionVRegElementTypeLike(getOperation(), getSrc0().getType(), "src0 type")))
        return failure();
    if (getSrc0().getType() != getSrc1().getType() || getSrc0().getType() != getResult().getType())
...
LogicalResult VpackOp::verify()
{
    ...
    if (resultType.getElementCount() != srcType.getElementCount() * 2)
        return emitOpError("requires result element count to be twice the source element count");
    ...
    if (!srcWidth || resultWidth * 2 != srcWidth)
        return emitOpError("requires result element width to be half the source element width");
    ...
    if (!((srcIntType.getWidth() == 32 && resultIntType.getWidth() == 16) ||
        ...
        return emitOpError("currently supports only s32/u32 -> u16 and s16/u16 -> u8");
    return success();
}
...
static LogicalResult verifyUnpackVecOp(UnpackOp op)
{
    ...
    Type resultElemType = resultType.getElementType();
    if (!isa<IntegerType>(srcElemType) || !isa<IntegerType>(resultElemType))
        return op.emitOpError("currently requires integer source and result element types");
    if (srcType.getElementCount() != resultType.getElementCount() * 2)
        return op.emitOpError("requires source element count to be twice the result element count");
    unsigned srcWidth = getIntOrFloatBitWidth(srcElemType);
    unsigned resultWidth = getIntOrFloatBitWidth(resultElemType);
    if (!srcWidth || srcWidth * 2 != resultWidth)
        return op.emitOpError("requires result element width to be twice the source element width");
    return success();
}
...
{
    return mode == "eq" || mode == "ne" || mode == "lt" || mode == "le" || mode == "gt" || mode == "ge";
}
...
ParseResult VtrcOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parser.parseOperand(input) || parser.parseComma() || parser.parseOperand(mask) || parser.parseComma() ||
        parser.parseKeywordOrString(&roundModeToken) || parser.parseOptionalAttrDict(attrs) ||
        parser.parseColonType(inputType) || parser.parseComma() || parser.parseType(maskType) || parser.parseArrow() ||
        parser.parseType(resultType))
        return failure();

    auto normalized = normalizeRoundModeToken(roundModeToken);
    if (!normalized || !isSupportedVtrcRoundMode(*normalized))
        return parser.emitError(parser.getCurrentLocation()) << "round mode must be one of R/A/F/C/Z or "
...
void VtrcOp::print(OpAsmPrinter& printer)
{
    ...
    printer.printAttributeWithoutType(builder.getStringAttr(normalized.value_or(getRoundMode())));
    printer.printOptionalAttrDict((*this)->getAttrs(), {"round_mode"});
    printer << " : " << getInput().getType() << ", " << getMask().getType() << " -> " << getResult().getType();
}
...
LogicalResult VtrcOp::verify()
{
    ...
    if (failed(verifyMaskTypeWithGranularityLike(*this, getMask().getType(), "mask type", *expectedGranularity)))
        return failure();
    auto normalized = normalizeRoundModeToken(getRoundMode());
    if (!normalized || !isSupportedVtrcRoundMode(*normalized))
...
LogicalResult VmulscvtOp::verify()
{
    ...
    if (failed(verifyMaskTypeWithGranularityLike(*this, getMask().getType(), "mask type", "b32")))
        return failure();
    ...
    if (!inputBits || !resultBits || *inputBits != *resultBits)
        return emitOpError("requires source and result to preserve total vector storage width");
...
ParseResult VcvtOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parser.parseOperand(input) || parser.parseComma() || parser.parseOperand(mask) ||
        parser.parseOptionalAttrDict(attrs) || parser.parseColonType(inputType) || parser.parseComma() ||
        parser.parseType(maskType) || parser.parseArrow() || parser.parseType(resultType))
        return failure();

    Attribute legacyRndAttr = attrs.get("round_mode");
    Attribute rndAttr = attrs.get("rnd");
    if (legacyRndAttr && rndAttr)
        return parser.emitError(parser.getCurrentLocation()) << "rnd and round_mode cannot be specified together";

    auto normalizeNamedStringAttr = [&](StringRef sourceName, StringRef canonicalName,
        ...
        if (!strAttr)
            return parser.emitError(parser.getCurrentLocation()) << sourceName << " must be a string literal";
    ...
    if (failed(normalizeNamedStringAttr("round_mode", "rnd", normalizeRoundModeToken)) ||
...
LogicalResult VcvtOp::verify()
{
    ...
    if (failed(verifyMaskTypeWithGranularityLike(*this, getMask().getType(), "mask type", expectedMaskGranularity)))
        return failure();
    ...
    if (getPartAttr()) {
        ...
        if (!isValidVcvtPartForFamily(*normalizedPart, *partFamily)) {
            switch (*partFamily) {
                ...
                case VcvtPartFamily::Packed4:
                    return emitOpError("part must be P0, P1, P2, or P3 for packed vcvt forms");
            }
        }
    }
...
LogicalResult VbitcastOp::verify()
{
    ...
    auto getStorageBits = [](VRegType type) -> std::optional<int64_t> {
        ...
        if (auto floatType = dyn_cast<FloatType>(elementType))
            return type.getElementCount() * static_cast<int64_t>(floatType.getWidth());
        return std::nullopt;
    };
...
{
    if (failed(verifyMaskTypeWithGranularityLike(*this, getLhs().getType(), "lhs type", "b8")) ||
        failed(verifyMaskTypeWithGranularityLike(*this, getRhs().getType(), "rhs type", "b8")) ||
        failed(verifyMaskTypeWithGranularityLike(*this, getLow().getType(), "low type", "b8")) ||
        failed(verifyMaskTypeWithGranularityLike(*this, getHigh().getType(), "high type", "b8")))
        return failure();
    return success();
}
...
{
    if (failed(verifyMaskTypeWithGranularityLike(*this, getLhs().getType(), "lhs type", "b16")) ||
        failed(verifyMaskTypeWithGranularityLike(*this, getRhs().getType(), "rhs type", "b16")) ||
        failed(verifyMaskTypeWithGranularityLike(*this, getLow().getType(), "low type", "b16")) ||
        failed(verifyMaskTypeWithGranularityLike(*this, getHigh().getType(), "high type", "b16")))
        return failure();
    return success();
}
...
{
    if (failed(verifyMaskTypeWithGranularityLike(*this, getLhs().getType(), "lhs type", "b32")) ||
        failed(verifyMaskTypeWithGranularityLike(*this, getRhs().getType(), "rhs type", "b32")) ||
        failed(verifyMaskTypeWithGranularityLike(*this, getLow().getType(), "low type", "b32")) ||
        failed(verifyMaskTypeWithGranularityLike(*this, getHigh().getType(), "high type", "b32")))
        return failure();
    return success();
}
...
{
    if (failed(verifyPairVecResults(*this)) || failed(verifyMaskTypeLike(*this, getMask().getType(), "mask type")))
        return failure();
    auto lhsType = cast<VRegType>(getLhs().getType());
...
LogicalResult VmulaOp::verify()
{
    if (failed(verifyVRegTypeLike(*this, getAcc().getType(), "acc type")) ||
        ...
        failed(verifyVRegTypeLike(*this, getResult().getType(), "result type")))
        return failure();
    if (getAcc().getType() != getLhs().getType() || getAcc().getType() != getRhs().getType() ||
...
static LogicalResult verifyFusedConvVecOp(ConvOp op)
{
    ...
    if (!isIntegerOrFloatLike(lhsType.getElementType()) || !isIntegerOrFloatLike(resultType.getElementType()))
        return op.emitOpError("requires integer or floating-point vector element types");
    auto lhsBits = getVRegStorageBitWidth(lhsType);
    auto resultBits = getVRegStorageBitWidth(resultType);
    if (!lhsBits || !resultBits || *lhsBits != *resultBits)
        return op.emitOpError("requires source and result to preserve total vector storage width");
    return success();
}

LogicalResult VaddreluconvOp::verify() { return verifyFusedConvVecOp(*this); }
LogicalResult VmulconvOp::verify() { return verifyFusedConvVecOp(*this); }

void Vldsx2Op::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
void VstsOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
static LogicalResult verifyVstsCommon(StoreOp op)
{
    ...
    if (std::optional<StringRef> dist = op.getDist(); dist && !isSupportedVstsDistToken(*dist)) {
    ...
    if (std::optional<StringRef> dist = op.getDist()) {
        if (std::optional<StringRef> granularity =
                getVstsMaskGranularityOverride(*dist, cast<VRegType>(op.getValue().getType()).getElementType())) {
            if (failed(verifyMaskTypeWithGranularityLike(op, op.getMask().getType(), "mask type", *granularity)))
                return failure();
        } else if (failed(verifyMaskTypeLike(op, op.getMask().getType(), "mask type"))) {
            return failure();
        }
    } else if (failed(verifyMaskTypeLike(op, op.getMask().getType(), "mask type"))) {
        return failure();
...
void Vstsx2Op::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
void VscatterOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getValueMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
LogicalResult VscatterOp::verify()
{
    ...
    if (offsetsElemType.getWidth() != expectedOffsetWidth)
        return emitOpError() << "requires " << expectedOffsetWidth << "-bit offset vector elements for "
                             << valueElemWidth << "-bit values";

    int64_t expectedOffsetCount = valueElemWidth == 8 ? valueType.getElementCount() / 2 : valueType.getElementCount();
    if (offsetsType.getElementCount() != expectedOffsetCount)
        return emitOpError() << "requires " << expectedOffsetCount << " offsets for " << valueType.getElementCount()
                             << "x" << valueElemWidth << "-bit values";

    if (failed(verifyMaskTypeWithGranularityLike(
            *this, getMask().getType(), "mask type", valueElemWidth == 32 ? "b32" : "b16")))
        return failure();
    auto destinationType = cast<PtrType>(getDestination().getType());
    if (destinationType.getElementType() != valueType.getElementType())
        return emitOpError("requires destination element type to match value element type");
...
void VsldbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
}
...
void PstsOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getValueMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

void PstiOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getValueMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void VsstbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getValueMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void VstasOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getValueMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void VstarOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getValueMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void PstuOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
void VstusOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getAlignInMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getValueMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getBaseMutable());
...
void VsturOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getAlignInMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getValueMutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getBaseMutable());
...
void CopyUbufToGmOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

LogicalResult CopyUbufToGmOp::verify() { return verifyCopyUbufToGmOp(*this, false); }

void MteUbGmOp::build(
    OpBuilder& builder, OperationState& state, Value source, Value destination, Value lenBurst,
    pto::DmaLoopConfig nburst, Value l2CacheCtl, llvm::ArrayRef<pto::DmaLoopConfig> loops)
{
    state.addOperands({source, destination, lenBurst, nburst.count, nburst.srcStride, nburst.dstStride});
    if (l2CacheCtl)
    ...
    state.addAttribute(
        getOperandSegmentSizeAttr(), builder.getDenseI32ArrayAttr(
                                         {1, 1, 1, 1, 1, 1, l2CacheCtl ? 1 : 0, static_cast<int32_t>(loops.size()),
                                          static_cast<int32_t>(loops.size()), static_cast<int32_t>(loops.size())}));
}

void MteUbGmOp::build(
    OpBuilder& builder, OperationState& state, Value source, Value destination, Value lenBurst,
    pto::DmaLoopConfig nburst, Value l2CacheCtl, std::optional<pto::DmaLoopConfig> loop1,
    std::optional<pto::DmaLoopConfig> loop2)
{
    ...
    if (loop2)
        loops.push_back(*loop2);
    build(builder, state, source, destination, lenBurst, nburst, l2CacheCtl, loops);
}
...
ParseResult MteUbGmOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    SmallVector<OpAsmParser::UnresolvedOperand> loopDstStrideOperands;
    if (parseRequiredOperandWithComma(parser, source) || parseRequiredOperandWithComma(parser, destination) ||
        parser.parseOperand(lenBurst) || parseDmaTripleGroup(parser, "nburst", nburstOperands))
        return failure();
    if (succeeded(parser.parseOptionalKeyword("l2_cache_ctl"))) {
        hasL2CacheCtl = true;
        if (parser.parseLParen() || parser.parseOperand(l2CacheCtl) || parser.parseRParen())
            return failure();
    }
    while (true) {
        StringRef parsedKeyword;
    ...
    SmallVector<Type> nburstTypes, loopCountTypes, loopSrcStrideTypes, loopDstStrideTypes;
    if (parser.parseType(sourceType) || parser.parseComma() || parser.parseType(destinationType) ||
        parser.parseComma() || parser.parseType(lenBurstType) || parser.parseComma() ||
    ...
    auto& segments = result.getOrAddProperties<MteUbGmOp::Properties>().operandSegmentSizes;
    llvm::copy(
        ArrayRef<int32_t>{1, 1, 1, 1, 1, 1, hasL2CacheCtl ? 1 : 0, loopGroupCount, loopGroupCount, loopGroupCount},
        segments.begin());
    ...
    if (parser.resolveOperand(source, sourceType, result.operands) ||
        ...
        parser.resolveOperands(nburstOperands, nburstTypes, parser.getCurrentLocation(), result.operands))
        return failure();
    if (hasL2CacheCtl && parser.resolveOperand(l2CacheCtl, l2CacheCtlType, result.operands))
        return failure();
    if (parser.resolveOperands(loopCountOperands, loopCountTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            loopSrcStrideOperands, loopSrcStrideTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(loopDstStrideOperands, loopDstStrideTypes, parser.getCurrentLocation(), result.operands))
        return failure();
    return success();
}
...
{
    printer << " " << getSource() << ", " << getDestination() << ", " << getLenBurst();
    printDmaTripleGroup(printer, "nburst", getNBurst(), getNburstSrcStride(), getNburstDstStride());
    ...
    for (auto [count, srcStride, dstStride] : llvm::zip(getLoopCounts(), getLoopSrcStrides(), getLoopDstStrides()))
        printDmaTripleGroup(printer, "loop", count, srcStride, dstStride);
    printer.printOptionalAttrDict((*this)->getAttrs());
    printer << " : " << getSource().getType() << ", " << getDestination().getType() << ", " << getLenBurst().getType()
...
void MteUbGmOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
{
    if (!isBufferLike(getSource().getType()) || !isBufferLike(getDestination().getType()))
        return emitOpError("requires typed !pto.ptr or memref source and destination");
    if (classifyMemoryRole(getSource().getType()) != MemoryRole::UB ||
    ...
    int64_t destinationElemBytes = getBufferElementByteSize(getDestination().getType());
    if (sourceElemBytes <= 0 || destinationElemBytes <= 0)
        return emitOpError("requires copy source and destination element types with known byte width");
    if (sourceElemBytes != destinationElemBytes)
        return emitOpError("requires source and destination element byte widths to match");
    ...
    return verifyDmaLoadStoreLoopGroups(getOperation(), getLoopCounts(), getLoopSrcStrides(), getLoopDstStrides());
}

void MteGmL1Op::build(
    OpBuilder& builder, OperationState& state, Value source, Value destination, Value lenBurst,
    pto::DmaLoopConfig nburst, llvm::ArrayRef<pto::DmaLoopConfig> loops)
{
    state.addOperands({source, destination, lenBurst, nburst.count, nburst.srcStride, nburst.dstStride});
    for (const pto::DmaLoopConfig& loop : loops)
        state.addOperands(loop.count);
...
void MteGmL1FracOp::build(
    OpBuilder& builder, OperationState& state, Value source, Value destination, pto::CubeLoadFracMode mode,
...
{
    state.addOperands({source, destination, shape.nValue, shape.dValue, srcLayout.srcInnerStride});
    state.addOperands(
    ...
    state.addAttribute(getModeAttrName(state.name), CubeLoadFracModeAttr::get(builder.getContext(), mode));
}
...
ParseResult MteGmL1Op::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    llvm::copy(ArrayRef<int32_t>{1, 1, 1, 1, 1, 1, loopGroupCount, loopGroupCount, loopGroupCount}, segments.begin());
...
ParseResult MteGmL1FracOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parseRequiredOperandWithComma(parser, source) || parseRequiredOperandWithComma(parser, destination) ||
        parser.parseKeyword(&modeKeyword) || failed(parseCubeLoadFracModeKeyword(modeKeyword)) || parser.parseComma() ||
        parseFixedKeywordOperandGroup(parser, "shape", 2, shapeOperands) || parser.parseComma() ||
        parseCubeLoadFracSrcLayoutGroup(parser, srcLayoutOperands) || parser.parseComma() ||
        parseFixedKeywordOperandGroup(parser, "dst_group", 4, dstGroupOperands) || parser.parseComma() ||
    ...
    if (parser.parseType(sourceType) || parser.parseComma() || parser.parseType(destinationType) ||
        parser.parseComma() || parser.parseKeyword(modeKeyword) || parser.parseComma() ||
        parseFixedKeywordTypes(parser, "shape", 2, shapeTypes) || parser.parseComma() ||
        parseCubeLoadFracSrcLayoutTypes(parser, srcLayoutTypes) || parser.parseComma() ||
        parseFixedKeywordTypes(parser, "dst_group", 4, dstGroupTypes) || parser.parseComma() ||
        parseFixedKeywordTypes(parser, "ctrl", 2, ctrlTypes))
        return failure();

    auto modeOr = parseCubeLoadFracModeKeyword(modeKeyword);
    if (failed(modeOr))
        return parser.emitError(parser.getCurrentLocation(), "expected one of 'nd2nz' or 'dn2nz'");
    if (shapeOperands.size() != 2 || shapeTypes.size() != 2)
        return parser.emitError(parser.getCurrentLocation(), "shape requires exactly two operands and types");
    if (srcLayoutOperands.empty() || srcLayoutOperands.size() > 2 || srcLayoutTypes.empty() ||
        srcLayoutTypes.size() > 2)
        return parser.emitError(parser.getCurrentLocation(), "src_layout requires one or two operands and types");
    if (dstGroupOperands.size() != 4 || dstGroupTypes.size() != 4)
        return parser.emitError(parser.getCurrentLocation(), "dst_group requires exactly four operands and types");
    if (ctrlOperands.size() != 2 || ctrlTypes.size() != 2)
        return parser.emitError(parser.getCurrentLocation(), "ctrl requires exactly two operands and types");
    if (srcLayoutOperands.size() != srcLayoutTypes.size())
        return parser.emitError(parser.getCurrentLocation(), "src_layout operand and type groups must match");

    bool hasSrcOuterStride = srcLayoutOperands.size() == 2;
    result.addAttribute(getModeAttrName(result.name), CubeLoadFracModeAttr::get(parser.getContext(), *modeOr));
    ...
    if (parser.resolveOperand(source, sourceType, result.operands) ||
        parser.resolveOperand(destination, destinationType, result.operands) ||
        parser.resolveOperands(flatOperands, flatTypes, parser.getCurrentLocation(), result.operands))
        return failure();
    return success();
}
...
{
    printer << " " << getSource() << ", " << getDestination() << ", " << getLenBurst();
    printDmaTripleGroup(printer, "nburst", getNBurst(), getNburstSrcGap(), getNburstDstGap());
    printer.printOptionalAttrDict((*this)->getAttrs());
    printer << " : " << getSource().getType() << ", " << getDestination().getType() << ", " << getLenBurst().getType()
            << ", " << getNBurst().getType() << ", " << getNburstSrcGap().getType() << ", "
...
{
    printer << " " << getSource() << ", " << getDestination() << ", " << pto::stringifyCubeLoadFracMode(getMode());
    printer << ", shape(" << getNValue() << ", " << getDValue() << ")";
    printCubeLoadFracSrcLayoutGroup(printer, getSrcInnerStride(), getSrcOuterStride());
    printer << ", dst_group(" << getGroupCount() << ", " << getDstLoop2Stride() << ", " << getDstLoop3Stride() << ", "
            << getDstLoop4Stride() << ")";
    ...
    printer << " : " << getSource().getType() << ", " << getDestination().getType() << ", "
            << pto::stringifyCubeLoadFracMode(getMode()) << ", shape " << getNValue().getType() << ", "
            << getDValue().getType();
    printCubeLoadFracSrcLayoutTypes(
        printer, getSrcInnerStride().getType(), getSrcOuterStride() ? getSrcOuterStride().getType() : Type());
    printer << ", dst_group " << getGroupCount().getType() << ", " << getDstLoop2Stride().getType() << ", "
            << getDstLoop3Stride().getType() << ", " << getDstLoop4Stride().getType() << ", ctrl "
...
LogicalResult MteL1BtOp::verify()
{
    ...
    const bool isI32 = isa<IntegerType>(srcElem) && isa<IntegerType>(dstElem) &&
                       cast<IntegerType>(srcElem).getWidth() == 32 && cast<IntegerType>(dstElem).getWidth() == 32;
    ...
    if (!isF32 && !isI32 && !isF16ToF32 && !isBF16ToF32) {
        return emitOpError("expects one of f32->f32, i32->i32, f16->f32, or bf16->f32");
    }
    return success();
}
...
LogicalResult MteL1FbOp::verify()
{
    ...
    std::optional<pto::AddressSpace> destinationAS = getAddressSpace(getDestination().getType());
...
LogicalResult MteGmL1FracOp::verify()
{
    ...
    if (failed(checkNonNegativeConst(getGroupCount(), "group_count")) ||
        ...
        (getSrcOuterStride() && failed(checkNonNegativeConst(getSrcOuterStride(), "src_outer_stride"))))
        return failure();

    APInt groupCount;
    if (matchPattern(getGroupCount(), m_ConstantInt(&groupCount)) && groupCount.isZero())
        return emitOpError("group_count must be greater than zero");

    APInt smallc0En;
    APInt dValue;
    if (matchPattern(getSmallc0En(), m_ConstantInt(&smallc0En)) && smallc0En.getBoolValue() &&
        matchPattern(getDValue(), m_ConstantInt(&dValue)) && dValue.ugt(4))
        return emitOpError("smallc0_en requires d_value <= 4");

    return success();
}

void MteGmL1Op::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

void MteL1UbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

void MteL1BtOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

void MteL1FbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

void MteGmL1FracOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
ParseResult MteL0cL1Op::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    OpAsmParser::UnresolvedOperand source, destination, m, n, srcStride, dstStride;
    if (parseRequiredOperandWithComma(parser, source) || parseRequiredOperandWithComma(parser, destination) ||
        parseRequiredOperandWithComma(parser, m) || parseRequiredOperandWithComma(parser, n) ||
        parseRequiredOperandWithComma(parser, srcStride) || parseRequiredOperandWithComma(parser, dstStride) ||
        parseStructuredAccStoreClauses(parser, state) || parser.parseOptionalAttrDict(result.attributes) ||
        parser.parseColon())
        return failure();

    Type sourceType, destinationType, mType, nType, srcStrideType, dstStrideType;
    if (parser.parseType(sourceType) || parser.parseComma() || parser.parseType(destinationType) ||
        parser.parseComma() || parser.parseType(mType) || parser.parseComma() || parser.parseType(nType) ||
        parser.parseComma() || parser.parseType(srcStrideType) || parser.parseComma() ||
        parser.parseType(dstStrideType) || parseStructuredAccStoreTailTypes(parser, state))
        return failure();

    setStructuredAccStoreSegmentSizes<MteL0cL1Op>(
        result, {1, 1, 1, 1, 1, 1, !state.preQuantOperands.empty() ? 1 : 0, !state.preReluOperands.empty() ? 1 : 0,
                 !state.clipValueOperands.empty() ? 1 : 0, !state.splitOperands.empty() ? 1 : 0,
                 !state.loop0SrcStrideOperands.empty() ? 1 : 0, !state.loop3CountOperands.empty() ? 1 : 0,
                 !state.loop3SrcStrideOperands.empty() ? 1 : 0, !state.loop3DstStrideOperands.empty() ? 1 : 0});
    if (state.atomicType || state.atomicOp) {
        return parser.emitError(parser.getCurrentLocation(), "atomic is only supported for mte_l0c_gm");
    }
    ...
    if (parser.resolveOperand(source, sourceType, result.operands) ||
        ...
        parser.resolveOperands(
            state.preQuantOperands, state.preQuantTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            state.preReluOperands, state.preReluTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            state.clipValueOperands, state.clipValueTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(state.splitOperands, state.splitTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            state.loop0SrcStrideOperands, state.loop0SrcStrideTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            state.loop3CountOperands, state.loop3CountTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            state.loop3SrcStrideOperands, state.loop3SrcStrideTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            state.loop3DstStrideOperands, state.loop3DstStrideTypes, parser.getCurrentLocation(), result.operands))
        return failure();
    return success();
}
...
{
    printer << " " << getSource() << ", " << getDestination() << ", " << getM() << ", " << getN() << ", "
            << getSrcStride() << ", " << getDstStride();
    printStructuredAccStoreClauses(
        printer, getUnitFlag(), getPreQuant(), getPreQuantMode(), getPreRelu(), getPreReluMode(), getClipValue(),
        getMode(), getSplit(), getLoop0SrcStride(), getLoop3Count(), getLoop3SrcStride(), getLoop3DstStride(),
        getSatMode(), getAtomicType(), getAtomicOp());
    printer.printOptionalAttrDict(
        (*this)->getAttrs(),
        /*elidedAttrs=*/{
            "operandSegmentSizes", "mode", "unit_flag", "pre_quant_mode", "pre_relu_mode", "atomic_type", "atomic_op",
            "sat_mode"});
    printer << " : " << getSource().getType() << ", " << getDestination().getType() << ", " << getM().getType() << ", "
            << getN().getType() << ", " << getSrcStride().getType() << ", " << getDstStride().getType();
    printStructuredAccStoreOptionalTypes(
        printer, getPreQuant(), getPreRelu(), getClipValue(), getSplit(), getLoop0SrcStride(), getLoop3Count(),
        getLoop3SrcStride(), getLoop3DstStride());
}

static LogicalResult verifyCubeBridgeLoadStart(
    Operation* op, Value firstStart, StringRef firstName, Value secondStart, StringRef secondName)
{
    auto checkNonNegativeConst = [&](Value value, StringRef name) -> LogicalResult {
        APInt intValue;
        if (matchPattern(value, m_ConstantInt(&intValue)) && intValue.isNegative())
    ...
    if (failed(checkNonNegativeConst(firstStart, firstName)) || failed(checkNonNegativeConst(secondStart, secondName)))
        return failure();
    return success();
}
...
{
    return verifyCubeBridgeLoadStart(op.getOperation(), op.getStartRow(), "start_row", op.getStartCol(), "start_col");
}

static LogicalResult verifyMxLoadControls(Operation* op, OperandRange controls)
{
    auto checkNonNegativeConst = [&](Value value, StringRef name) -> LogicalResult {
        APInt intValue;
        if (matchPattern(value, m_ConstantInt(&intValue)) && intValue.isNegative())
    ...
    if (controls.size() == 4)
        return verifyCubeBridgeLoadStart(op, controls[2], "start_row", controls[3], "start_col");
    if (controls.size() == 6) {
        if (failed(verifyCubeBridgeLoadStart(op, controls[0], "x_start", controls[1], "y_start")))
            return failure();
    ...
    return op->emitOpError() << "requires either four shape-derived controls or six explicit "
                                "MX controls";
}

static LogicalResult verifyMxDestinationAlignment(Operation* op, Value destination)
{
    ...
    std::optional<int64_t> address = mlir::getConstantIntValue(pointerCast.getInput());
    if (!address || (*address % kMxDestinationAddressUnitBytes) == 0)
        return success();

    return op->emitOpError() << "statically known LOAD.MX destination address must be aligned to "
...
LogicalResult MteL0cL1Op::verify()
{
    if (!isBufferLike(getSource().getType()) || !isBufferLike(getDestination().getType()))
        return emitOpError("requires buffer-like source and destination");
    std::optional<AddressSpace> sourceSpace = getBufferAddressSpace(getSource().getType());
    std::optional<AddressSpace> destinationSpace = getBufferAddressSpace(getDestination().getType());
    ...
    return verifyStructuredAccStoreLike(
        *this, getSource().getType(), getDestination().getType(), getPreQuant(), getPreRelu(), getClipValue(),
        getSplit(), getLoop0SrcStride(), getLoop3Count(), getLoop3SrcStride(), getLoop3DstStride(), getUnitFlag(),
        getPreQuantMode(), getPreReluMode(), getMode(), std::nullopt, std::nullopt, /*allowAtomic=*/false);
}
...
LogicalResult LoadCbufToCaMxOp::verify()
{
    ...
    return verifyCubeBridgeLoadStart(
        getOperation(), getXStartPosition(), "x_start_position", getYStartPosition(), "y_start_position");
}
...
void MteL1L0aOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

void MteL1L0bOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

void MteL1L0aMxOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

void MteL1L0bMxOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}

void MteL0cL1Op::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
ParseResult MteL0cGmOp::parse(OpAsmParser& parser, OperationState& result)
{
    Builder builder(parser.getContext());
    StructuredAccStoreAsmState state;
    OpAsmParser::UnresolvedOperand source, destination, m, n, srcStride, dstStride, sid, l2CacheCtrl;
    if (parseRequiredOperandWithComma(parser, source) || parseRequiredOperandWithComma(parser, destination) ||
        parseRequiredOperandWithComma(parser, m) || parseRequiredOperandWithComma(parser, n) ||
        parseRequiredOperandWithComma(parser, srcStride) || parseRequiredOperandWithComma(parser, dstStride) ||
        parseRequiredOperandWithComma(parser, sid) || parseRequiredOperandWithComma(parser, l2CacheCtrl) ||
        parseStructuredAccStoreClauses(parser, state) || parser.parseOptionalAttrDict(result.attributes) ||
        parser.parseColon())
        return failure();

    Type sourceType, destinationType, mType, nType, srcStrideType, dstStrideType, sidType, l2CacheCtrlType;
    if (parser.parseType(sourceType) || parser.parseComma() || parser.parseType(destinationType) ||
        parser.parseComma() || parser.parseType(mType) || parser.parseComma() || parser.parseType(nType) ||
        parser.parseComma() || parser.parseType(srcStrideType) || parser.parseComma() ||
        parser.parseType(dstStrideType) || parser.parseComma() || parser.parseType(sidType) || parser.parseComma() ||
        parser.parseType(l2CacheCtrlType) || parseStructuredAccStoreTailTypes(parser, state))
        return failure();

    setStructuredAccStoreSegmentSizes<MteL0cGmOp>(
        result, {1, 1, 1, 1, 1, 1, !state.preQuantOperands.empty() ? 1 : 0, !state.preReluOperands.empty() ? 1 : 0,
                 !state.clipValueOperands.empty() ? 1 : 0, 1, 1, !state.splitOperands.empty() ? 1 : 0,
                 !state.loop0SrcStrideOperands.empty() ? 1 : 0, !state.loop3CountOperands.empty() ? 1 : 0,
                 !state.loop3SrcStrideOperands.empty() ? 1 : 0, !state.loop3DstStrideOperands.empty() ? 1 : 0});
...
{
    printer << " " << getSource() << ", " << getDestination() << ", " << getM() << ", " << getN() << ", "
            << getSrcStride() << ", " << getDstStride() << ", " << getSid() << ", " << getL2CacheCtrl();
    printStructuredAccStoreClauses(
        printer, getUnitFlag(), getPreQuant(), getPreQuantMode(), getPreRelu(), getPreReluMode(), getClipValue(),
        getMode(), getSplit(), getLoop0SrcStride(), getLoop3Count(), getLoop3SrcStride(), getLoop3DstStride(),
        getSatMode(), getAtomicType(), getAtomicOp());
...
LogicalResult MteL0cGmOp::verify()
{
    ...
    return verifyStructuredAccStoreLike(
        *this, getSource().getType(), getDestination().getType(), getPreQuant(), getPreRelu(), getClipValue(),
        getSplit(), getLoop0SrcStride(), getLoop3Count(), getLoop3SrcStride(), getLoop3DstStride(), getUnitFlag(),
        getPreQuantMode(), getPreReluMode(), getMode(), getAtomicType(), getAtomicOp(), /*allowAtomic=*/true);
}

void MteL0cGmOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
ParseResult MteL0cUbOp::parse(OpAsmParser& parser, OperationState& result)
{
    Builder builder(parser.getContext());
    StructuredAccStoreAsmState state;
    OpAsmParser::UnresolvedOperand source, destination, m, n, srcStride, dstStride, subBlockId;
    ...
    if (subBlockIdParse.has_value()) {
        ...
        if (dstModeKeyword == "split_m") {
            ...
            return parser.emitError(
                parser.getCurrentLocation(), "expected dst_mode(%sub_blockid), dst_mode(split_m), or "
                                             "dst_mode(split_n)");
        }
    }
    if (parser.parseRParen())
        return failure();
    if (succeeded(parser.parseOptionalComma()) && parseStructuredAccStoreClauses(parser, state))
        return failure();
    if (parser.parseOptionalAttrDict(result.attributes) || parser.parseColon())
        return failure();

    Type sourceType, destinationType, mType, nType, srcStrideType, dstStrideType, subBlockIdType;
    if (parser.parseType(sourceType) || parser.parseComma() || parser.parseType(destinationType) ||
        parser.parseComma() || parser.parseType(mType) || parser.parseComma() || parser.parseType(nType) ||
        parser.parseComma() || parser.parseType(srcStrideType) || parser.parseComma() ||
        parser.parseType(dstStrideType))
        return failure();
    if (hasSubBlockId && (parser.parseComma() || parser.parseType(subBlockIdType)))
        return failure();
    ...
    setStructuredAccStoreSegmentSizes<MteL0cUbOp>(
        result, {1, 1, 1, 1, 1, 1, !state.preQuantOperands.empty() ? 1 : 0, !state.preReluOperands.empty() ? 1 : 0,
                 !state.clipValueOperands.empty() ? 1 : 0, hasSubBlockId ? 1 : 0, !state.splitOperands.empty() ? 1 : 0,
                 !state.loop0SrcStrideOperands.empty() ? 1 : 0, !state.loop3CountOperands.empty() ? 1 : 0,
                 !state.loop3SrcStrideOperands.empty() ? 1 : 0, !state.loop3DstStrideOperands.empty() ? 1 : 0});
    if (state.atomicType || state.atomicOp) {
        return parser.emitError(parser.getCurrentLocation(), "atomic is only supported for mte_l0c_gm");
    ...
    if (parser.resolveOperand(source, sourceType, result.operands) ||
        ...
        parser.resolveOperands(
            state.clipValueOperands, state.clipValueTypes, parser.getCurrentLocation(), result.operands) ||
        (hasSubBlockId && parser.resolveOperand(subBlockId, subBlockIdType, result.operands)) ||
        parser.resolveOperands(state.splitOperands, state.splitTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
            state.loop0SrcStrideOperands, state.loop0SrcStrideTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperands(
...
{
    printer << " " << getSource() << ", " << getDestination() << ", " << getM() << ", " << getN() << ", "
            << getSrcStride() << ", " << getDstStride() << ", dst_mode(";
    switch (getDstMode()) {
    ...
    printStructuredAccStoreClauses(
        printer, getUnitFlag(), getPreQuant(), getPreQuantMode(), getPreRelu(), getPreReluMode(), getClipValue(),
        getMode(), getSplit(), getLoop0SrcStride(), getLoop3Count(), getLoop3SrcStride(), getLoop3DstStride(),
        getSatMode(), std::nullopt, std::nullopt);
    printer.printOptionalAttrDict(
        (*this)->getAttrs(),
        /*elidedAttrs=*/{
            "operandSegmentSizes", "mode", "unit_flag", "pre_quant_mode", "pre_relu_mode", "dst_mode", "sat_mode"});
    printer << " : " << getSource().getType() << ", " << getDestination().getType() << ", " << getM().getType() << ", "
            << getN().getType() << ", " << getSrcStride().getType() << ", " << getDstStride().getType();
    if (getSubBlockid())
...
LogicalResult MteL0cUbOp::verify()
{
    ...
    if (getDstMode() == AccStoreUbDstMode::Single) {
        ...
        APInt subBlockId;
        if (matchPattern(getSubBlockid(), m_ConstantInt(&subBlockId)) && subBlockId.ugt(1))
    ...
    if (getPreQuant() || getPreRelu() || getClipValue() || getPreQuantMode() || getPreReluMode() || getSplit() ||
        getLoop0SrcStride() || getLoop3Count() || getLoop3SrcStride() || getLoop3DstStride()) {
    ...
    APInt mValue;
    APInt nValue;
    if (getDstMode() == AccStoreUbDstMode::SplitM && matchPattern(getM(), m_ConstantInt(&mValue)) &&
        mValue.getZExtValue() % 2 != 0)
        return emitOpError("split-M dual destination requires m to be even");
    if (getDstMode() == AccStoreUbDstMode::SplitN && matchPattern(getN(), m_ConstantInt(&nValue)) &&
        nValue.getZExtValue() % 32 != 0)
        return emitOpError("split-N dual destination requires n to be a multiple of 32");
    return success();
}

void MteL0cUbOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSourceMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDestinationMutable());
}
...
void UBVaddOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
{
    if (!isBufferLike(getDst().getType()) || !isBufferLike(getSrc0().getType()) || !isBufferLike(getSrc1().getType()))
...
void UBVsubOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc0Mutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc1Mutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
void UBVmulOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc0Mutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc1Mutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
void UBVdivOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc0Mutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc1Mutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
void UBVmaxOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc0Mutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc1Mutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
void UBVminOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc0Mutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc1Mutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
void UBVandOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc0Mutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc1Mutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
void UBVorOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc0Mutable());
    effects.emplace_back(MemoryEffects::Read::get(), &getSrc1Mutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
...
#define PTO_DEFINE_UB_BINARY_VERIFY_AND_EFFECTS(OpName)                                                   \
    void OpName::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects) \
    {                                                                                                     \
        effects.emplace_back(MemoryEffects::Read::get(), &getSrc0Mutable());                              \
        effects.emplace_back(MemoryEffects::Read::get(), &getSrc1Mutable());                              \
        effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());                              \
...
void UBVnotOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
void UBVabsOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrcMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
}
...
void UBVreluOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrcMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
}
...
void UBVdupOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
}
...
void UBVshlOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrcMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
}
...
void UBVshrOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrcMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
}
...
void UBVmulSOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), &getSrcMutable());
    effects.emplace_back(MemoryEffects::Write::get(), &getDstMutable());
}
