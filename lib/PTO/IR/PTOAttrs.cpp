    ...
    Attribute pad, Attribute compactMode)
{
    ...
    if (!compactMode || (!mlir::isa<CompactModeAttr>(compactMode) && !mlir::isa<IntegerAttr>(compactMode)))
    ...
    if (s != kFractalMxSize && s != kFractalABSize && s != kFractalCSize)
        return emitError() << "unsupported s_fractal_size: " << s << ", must be one of {" << kFractalMxSize << ", "
