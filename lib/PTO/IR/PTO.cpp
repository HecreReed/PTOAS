...
static LogicalResult verifyNDStyleVecTile(Operation* op, Type ty, StringRef name, bool allowLowPrecision = false);
...
static LogicalResult verifyMatTileOperands(
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, bool allowLowPrecision = false);
static LogicalResult verifyMatTileOperandsA2A3(
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, bool allowLowPrecision = false);
static LogicalResult verifyMatTileOperandsA5(
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, bool allowLowPrecision = false);
...
func::FuncOp mlir::pto::lookupPeerFuncAcrossContainer(Operation* op, FlatSymbolRefAttr peerAttr)
{
    if (!op || !peerAttr)
        return {};
    ...
    if (auto localPeer = dyn_cast_or_null<func::FuncOp>(SymbolTable::lookupSymbolIn(currentChildModule, target))) {
        return localPeer;
    }
    ...
    outerModule.walk([&](func::FuncOp funcOp) {
        ...
        if (symbolName == target ||
            (funcOp->hasAttr(kPTODSLLogicalNameAttrName) && getPTODSLLogicalNameOrSymbolName(funcOp) == target))
...
static bool isA5DeviceSpec(StringRef spec) { return spec.starts_with("Ascend950") || spec.starts_with("Ascend910_95"); }
...
static std::optional<PredicateLoadDist> parsePredicateLoadDist(StringRef dist)
{
    if (dist == "NORM")
...
static std::optional<PredicateStoreDist> parsePredicateStoreDist(StringRef dist)
{
    if (dist == "NORM")
...
static std::optional<int64_t> findAlignmentSize(const Rule (&rules)[N], Dist dist)
{
    auto rule = llvm::find_if(rules, [&](const Rule& entry) { return entry.dist == dist; });
    if (rule == std::end(rules))
        return std::nullopt;
    return rule->alignmentBytes;
}

std::optional<int64_t> mlir::pto::getLoadStoreVecAlignmentSize(Operation* op)
{
    ...
    if (auto pldi = dyn_cast<PldiOp>(op)) {
        ...
        return dist ? findAlignmentSize(kA5PredicateLoadAlignmentRules, *dist) : std::nullopt;
    }
    if (auto psti = dyn_cast<PstiOp>(op)) {
        auto dist = parsePredicateStoreDist(psti.getDist());
        return dist ? findAlignmentSize(kA5PredicateStoreAlignmentRules, *dist) : std::nullopt;
    }
...
static llvm::TypeSize getOneByteTypeSize() { return llvm::TypeSize::getFixed(8); }

llvm::TypeSize mlir::pto::HiF8Type::getTypeSizeInBits(const DataLayout&, DataLayoutEntryListRef) const
{
    return getOneByteTypeSize();
}

llvm::TypeSize mlir::pto::F8E8M0Type::getTypeSizeInBits(const DataLayout&, DataLayoutEntryListRef) const
{
    return getOneByteTypeSize();
}

uint64_t mlir::pto::HiF8Type::getABIAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 1; }

uint64_t mlir::pto::F8E8M0Type::getABIAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 1; }

uint64_t mlir::pto::HiF8Type::getPreferredAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 1; }

uint64_t mlir::pto::F8E8M0Type::getPreferredAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 1; }

static llvm::TypeSize getTwoByteTypeSize() { return llvm::TypeSize::getFixed(16); }

llvm::TypeSize mlir::pto::HiF8x2Type::getTypeSizeInBits(const DataLayout&, DataLayoutEntryListRef) const
{
    return getTwoByteTypeSize();
}

uint64_t mlir::pto::HiF8x2Type::getABIAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 2; }

uint64_t mlir::pto::HiF8x2Type::getPreferredAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 2; }

llvm::TypeSize mlir::pto::F4E1M2x2Type::getTypeSizeInBits(const DataLayout&, DataLayoutEntryListRef) const
{
    return getOneByteTypeSize();
}

uint64_t mlir::pto::F4E1M2x2Type::getABIAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 1; }

uint64_t mlir::pto::F4E1M2x2Type::getPreferredAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 1; }

llvm::TypeSize mlir::pto::F4E2M1x2Type::getTypeSizeInBits(const DataLayout&, DataLayoutEntryListRef) const
{
    return getOneByteTypeSize();
}

uint64_t mlir::pto::F4E2M1x2Type::getABIAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 1; }

uint64_t mlir::pto::F4E2M1x2Type::getPreferredAlignment(const DataLayout&, DataLayoutEntryListRef) const { return 1; }
...
[[maybe_unused]] static mlir::Type parsePTOTypeAllowNoBang(mlir::OpAsmParser& parser)
{
    ...
    auto parseShapeElemForOpParser = [&](llvm::SmallVectorImpl<int64_t>& shape,
...
ParseResult mlir::pto::TGatherOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (hasMask) {
        if (!insOps.empty())
            return parser.emitError(
                parser.getCurrentLocation(), "mask-pattern tgather does not take extra ins operands");
        if (hasCdst)
            return parser.emitError(parser.getCurrentLocation(), "mask-pattern tgather expects a single outs operand");
    } else if (hasCdst) {
        if (insOps.empty() || !(mlir::isa<IntegerType>(insTypes.front()) || mlir::isa<FloatType>(insTypes.front())))
            return parser.emitError(
                parser.getCurrentLocation(), "compare-form tgather expects a scalar kValue operand");
        hasKValue = true;
        if (insOps.size() >= 2) {
    ...
    result.addAttribute(
        "operandSegmentSizes", parser.getBuilder().getDenseI32ArrayAttr(
                                   {1, 1, hasCdst ? 1 : 0, hasIndices ? 1 : 0, hasTmp ? 1 : 0, hasKValue ? 1 : 0}));
    return success();
}
...
ParseResult mlir::pto::TScatterOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (succeeded(parser.parseOptionalLBrace())) {
        ...
        if (parser.parseOperand(indexes))
            return failure();
        hasIndexes = true;
        if (parser.parseColon() || parser.parseType(srcTy) || parser.parseComma() || parser.parseType(idxTy) ||
            parser.parseRParen())
            return failure();
    }
...
ParseResult mlir::pto::MakeTensorViewOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    auto segAttr = parser.getBuilder().getDenseI32ArrayAttr({1, (int32_t)shapeOps.size(), (int32_t)strideOps.size()});
...
ParseResult mlir::pto::PartitionViewOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    llvm::copy(
        ArrayRef<int32_t>({1, static_cast<int32_t>(offsets.size()), static_cast<int32_t>(sizes.size())}),
...
static LogicalResult verifyTileBufLayoutConstraints(Operation* op, pto::TileBufType tb, StringRef name)
{
    ...
    int32_t blayout = 0;
    int32_t slayout = 0;
    if (!readBLayout(cfg.getBLayout(), blayout) || !readSLayout(cfg.getSLayout(), slayout))
        return op->emitOpError() << "expects " << name << " to have concrete tile layout attributes";
    constexpr int64_t kAlignedBytes = 32;

    auto checkByteAlignment = [&](int64_t dim, StringRef layoutName, StringRef byteExpr) -> LogicalResult {
...
static bool isStaticLayoutInt(int64_t value) { return value != ShapedType::kDynamic && value >= 0; }
    ...
    ArrayRef<int64_t> shape, ArrayRef<int64_t> strides, unsigned elemBytes)
{
    ...
    if (llvm::any_of(shape, [](int64_t dim) { return !isStaticLayoutInt(dim); }) ||
        llvm::any_of(strides, [](int64_t stride) { return !isStaticLayoutInt(stride); }))
        return std::nullopt;
    ...
    if (shape.size() >= 5) {
        ...
        auto fractalBytes =
            sh3TimesSh4 ? multiplyLayoutInts(*sh3TimesSh4, static_cast<int64_t>(elemBytes)) : std::nullopt;
...
static LogicalResult verifyTColArgTmpA2A3(Operation* op, Type srcTy, Type tmpTy)
{
    ...
    if (srcValid[1] != ShapedType::kDynamic) {
        ...
        if (tmpValid[1] != ShapedType::kDynamic && tmpValid[1] < *minStride)
            return op->emitOpError() << "expects A2/A3 tmp valid_shape[1] to be at least " << *minStride
...
static LogicalResult verifyTColArgReductionOpA2A3(Operation* op, Type srcTy, Type tmpTy, Type dstTy)
{
    ...
    if (!(mlir::isa<IntegerType, FloatType>(srcElemTy) && (srcElemBits == 8 || srcElemBits == 16 || srcElemBits == 32)))
...
static LogicalResult verifyTColArgReductionOpA5(Operation* op, Type srcTy, Type tmpTy, Type dstTy)
{
    ...
    unsigned srcElemBits = srcElemTy ? getPTOStorageElemBitWidth(srcElemTy) : 0;
    if (!(mlir::isa<IntegerType, FloatType>(srcElemTy) && (srcElemBits == 8 || srcElemBits == 16 || srcElemBits == 32)))
        return op->emitOpError("expects src element type to be 1, 2, or 4 bytes wide");
    auto dstInt = dyn_cast<IntegerType>(getElemTy(dstTy));
    if (!dstInt || dstInt.getWidth() != 32)
        return op->emitOpError("expects dst element type to be i32 or ui32");
    return success();
...
LogicalResult mlir::pto::FusionRegionOp::verify()
{
    ...
    for (auto [idx, pair] : llvm::enumerate(llvm::zip(yield.getValues(), getOutputs()))) {
        ...
        if (yielded.getType() != output.getType())
            return emitOpError() << "expects yielded value #" << idx << " to have "
                                 << "type " << output.getType() << ", got " << yielded.getType();
    }
...
LogicalResult mlir::pto::YieldOp::verify()
{
    ...
    for (auto [idx, pair] : llvm::enumerate(llvm::zip(getValues(), parent.getOutputs()))) {
        Value yielded = std::get<0>(pair);
        Value output = std::get<1>(pair);
        if (yielded.getType() != output.getType())
            return emitOpError() << "expects yielded value #" << idx << " to have "
...
LogicalResult mlir::pto::PartitionViewOp::verify()
{
    ...
    for (int64_t i = 0; i < srcRank; ++i) {
        ...
        if (offVal && *offVal < 0)
            return emitOpError() << "offset at dim " << i << " must be non-negative, got " << *offVal;

        if (sizeVal && *sizeVal <= 0)
            return emitOpError() << "size at dim " << i << " must be positive, got " << *sizeVal;

        if (sameRank && sizeVal) {
            int64_t resDim = resShape[i];
            if (resDim != ShapedType::kDynamic && *sizeVal != resDim)
                return emitOpError() << "size/result mismatch at dim " << i << ": size operand=" << *sizeVal
        ...
        if (sizeVal && *sizeVal > srcDim)
            return emitOpError() << "size at dim " << i << " (" << *sizeVal << ") exceeds static source dim (" << srcDim
                                 << ")";

        if (offVal && sizeVal && (*offVal + *sizeVal > srcDim))
            return emitOpError() << "offset+size at dim " << i << " (" << (*offVal + *sizeVal)
...
{
    Type fieldTy;
    if (failed(walkStructPath(getOperation(), getS().getType(), getPath(), fieldTy)))
        return failure();
    if (failed(verifyStructLeafIsScalar(getOperation(), fieldTy)))
        return failure();
    if (getValue().getType() != fieldTy)
        return emitOpError() << "result type " << getValue().getType() << " does not match field type " << fieldTy
                             << " at the given path";
    return success();
}
...
LogicalResult mlir::pto::StructSetOp::verify()
{
    ...
    if (failed(verifyStructLeafIsScalar(getOperation(), fieldTy)))
        return failure();
    if (getValue().getType() != fieldTy)
        return emitOpError() << "value type " << getValue().getType() << " does not match field type " << fieldTy
                             << " at the given path";
    return success();
}
...
static std::optional<uint64_t> getLocalAddressAlignmentBytes(Attribute memorySpace)
{
...
static LogicalResult verifyConstantLocalAddress(Operation* op, Value addr, Attribute memorySpace, int addrIndex = -1)
{
    std::optional<uint64_t> alignment = getLocalAddressAlignmentBytes(memorySpace);
    ...
    if ((unsignedAddr % *alignment) != 0)
        return emitAddrError() << " must be aligned to " << *alignment << " bytes for local tile memory space, got "
                               << unsignedAddr;
...
LogicalResult AllocTileOp::verify()
{
    ...
    if (failed(verifyConstantLocalAddress(getOperation(), getAddr(), ty.getMemorySpace())))
        return failure();
...
LogicalResult AllocMultiTileOp::verify()
{
    ...
    if (isPTOLowPrecisionType(elemTy))
        return emitOpError() << "slot dtype " << elemTy << " is not supported by pto.alloc_multi_tile yet";

    if (failed(verifyTileBufLayoutConstraints(*this, slotTy, "slot")))
        return failure();

    if (failed(verifyConstantLocalAddress(getOperation(), getAddr(), slotTy.getMemorySpace())))
        return failure();
    ...
    if (slotTy.getCompactModeI32() == static_cast<int32_t>(mlir::pto::CompactMode::RowPlusOne))
        return emitOpError() << "multi_tile_buf slot uses row_plus_one compaction, whose padded "
    ...
    if (hasVR != needVR)
        return emitOpError() << "valid_row operand " << (needVR ? "is required" : "must be absent")
                             << " because slot v_row is " << (needVR ? "?" : std::to_string(vs[0]));
    if (hasVC != needVC)
        return emitOpError() << "valid_col operand " << (needVC ? "is required" : "must be absent")
                             << " because slot v_col is " << (needVC ? "?" : std::to_string(vs[1]));
    ...
    if (count < kPtoMultiBufferMinNum || count > kPtoMultiBufferMaxNum) {
        return emitOpError() << "multi_tile_buf count must be in [" << kPtoMultiBufferMinNum << ", "
    ...
    if (Attribute rawAddrs = (*this)->getAttr(kPtoMultiBufferAddrsAttrName)) {
        ...
        if (!addrs)
            return emitOpError() << "expects internal '" << kPtoMultiBufferAddrsAttrName << "' to be a dense i64 array";
        if (getAddr())
            return emitOpError() << "cannot carry both base 'addr' and internal '" << kPtoMultiBufferAddrsAttrName
                                 << "'";
        if (addrs.size() != count)
            return emitOpError() << "expects " << count << " planned slot addresses, got " << addrs.size();
        ...
        for (int64_t dim : slotTy.getShape()) {
            if (dim == ShapedType::kDynamic)
                return emitOpError("planned multi-buffer addresses require a static slot shape");
        ...
        for (auto [lhsIdx, lhs] : llvm::enumerate(addrs.asArrayRef())) {
            ...
            for (size_t rhsIdx = lhsIdx + 1; rhsIdx < static_cast<size_t>(addrs.size()); ++rhsIdx) {
                ...
                if (std::max(lhsBegin, rhsBegin) < std::min(lhsEnd, rhsEnd))
                    return emitOpError() << "planned slots " << lhsIdx << " and " << rhsIdx << " overlap";
            }
        }
    }
...
LogicalResult MultiTileGetOp::verify()
{
    ...
    if (srcTy.getSlotType() != resultTy) {
        return emitOpError() << "result tile_buf must match the multi_tile_buf slot type: "
    ...
    if (auto slotDef = getSlot().getDefiningOp<arith::ConstantOp>()) {
        if (auto attr = llvm::dyn_cast<IntegerAttr>(slotDef.getValue())) {
            ...
            if (slotVal < 0 || slotVal >= count) {
                return emitOpError() << "constant slot " << slotVal
...
LogicalResult TAssignOp::verify()
{
    ...
    if (failed(verifyConstantLocalAddress(getOperation(), getAddr(), tileTy.getMemorySpace())))
        return failure();
...
{
    auto verifyCommon =
        [&](bool allowLowPrecision) -> FailureOr<std::pair<pto::PartitionTensorViewType, pto::TileBufType>> {
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (!(dstElem.isInteger(8) || dstElem.isInteger(16) || dstElem.isInteger(32) || dstElem.isInteger(64) ||
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (dstSpace && *dstSpace == pto::AddressSpace::VEC) {
            ...
            bool isDN =
                (bl == static_cast<int32_t>(pto::BLayout::ColMajor) &&
                 sl == static_cast<int32_t>(pto::SLayout::NoneBox));
            bool isNZ =
                (bl == static_cast<int32_t>(pto::BLayout::ColMajor) &&
...
LogicalResult TPrefetchOp::verify()
{
    auto verifyImpl = [&](bool allowLowPrecision) -> LogicalResult {
        ...
        for (unsigned i = 0; i < dstValid.size(); ++i) {
            if (dstValid[i] != ShapedType::kDynamic && dstValid[i] < 0)
                return emitOpError() << "expects dst valid_shape[" << i << "] to be non-negative";
        }
        auto dstSpace = getPTOMemorySpaceEnum(dstTile);
        if (!dstSpace || (*dstSpace != pto::AddressSpace::VEC && *dstSpace != pto::AddressSpace::MAT))
    ...
    auto verifyA2A3 = [&]() -> LogicalResult { return verifyImpl(/*allowLowPrecision=*/false); };
    auto verifyA5 = [&]() -> LogicalResult { return verifyImpl(/*allowLowPrecision=*/true); };
...
LogicalResult mlir::pto::SetFFTsOp::verify()
{
    ...
    if (!ptrTy.getElementType().isInteger(64) && !ptrTy.getElementType().isInteger(8))
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        if (IntegerAttr eventIdAttr = getEventIdAttr()) {
            ...
            if (eventId < 0 || eventId > 31) {
                return emitOpError() << "A5 sync.set expects static event_id in [0, 31], but got " << eventId;
            }
        }
...
ParseResult mlir::pto::SyncAllOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    auto addSegmentSizes = [&](int32_t gm, int32_t used) {
        result.addAttribute("operandSegmentSizes", parser.getBuilder().getDenseI32ArrayAttr({gm, used}));
    };
    ...
    if (parser.resolveOperand(operands[0], operandTypes[0], result.operands))
        return failure();
    if (operands.size() == 2 && parser.resolveOperand(operands[1], operandTypes[1], result.operands))
        return failure();
...
void mlir::pto::SyncAllOp::print(OpAsmPrinter& p)
{
    ...
    if (!operands.empty()) {
        ...
        p << " : ";
        llvm::interleaveComma(operands, p, [&](Value operand) { p.printType(operand.getType()); });
    }
...
LogicalResult mlir::pto::SyncWaitOp::verify()
{
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        if (IntegerAttr eventIdAttr = getEventIdAttr()) {
            int64_t eventId = eventIdAttr.getInt();
            if (eventId < 0 || eventId > 31)
                return emitOpError() << "A5 sync.wait expects static physical event_id in [0, 31], but got " << eventId;
        }
        switch (getPipe().getPipe()) {
            case PIPE::PIPE_FIX:
            case PIPE::PIPE_MTE1:
...
{
    auto verifyCommon =
        [&](bool allowLowPrecision) -> FailureOr<std::pair<pto::TileBufType, pto::PartitionTensorViewType>> {
    ...
    auto isLoadStoreElemType = [&](Type ty) -> bool {
        return ty.isInteger(8) || ty.isInteger(16) || ty.isInteger(32) || ty.isInteger(64) || ty.isF16() ||
               ty.isBF16() || ty.isF32();
    };
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (hasPreQuant) {
            ...
            if (!(dstElem.isInteger(32) || dstElem.isF32() || dstElem.isF16() || dstElem.isBF16()))
                return emitOpError("expects A2/A3 acc tstore dst element type to be i32/f32/f16/bf16");
        }

        auto srcShape = srcTile.getShape();
        if (srcShape[1] != ShapedType::kDynamic && (srcShape[1] < 1 || srcShape[1] > 4095))
            return emitOpError("expects A2/A3 acc tstore src cols to be in [1, 4095]");
        auto srcValid = srcTile.getValidShape();
        if (srcValid[1] != ShapedType::kDynamic && (srcValid[1] < 0 || srcValid[1] > 4095))
...
{
    return mlir::isa<RankedTensorType, pto::TensorViewType, pto::TileBufType, pto::PartitionTensorViewType>(ty);
}

static bool isTileLikeType(Type ty) { return isa<pto::TileBufType>(ty); }
...
static SmallVector<int64_t, 4> getLogicalTileExtentVec(Type ty, bool useValidShape)
{
    SmallVector<int64_t, 4> dims = useValidShape ? getValidShapeVec(ty) : getShapeVec(ty);
...
static LogicalResult verifyAsyncFlatContiguous1DGMViewLike(Operation* op, Value value, StringRef name)
{
    ...
    if (!isa<pto::TensorViewType, pto::PartitionTensorViewType>(ty))
        return op->emitOpError() << "expects " << name << " to be a tensor_view or partition_view";
...
static bool isCommGlobalLikeType(Type ty) { return isa<pto::TensorViewType, pto::PartitionTensorViewType>(ty); }
...
static LogicalResult verifyCommGlobalLike(
    Operation* op, Value value, StringRef name, CommGlobalShapePolicy policy = CommGlobalShapePolicy::StaticOnly)
{
    Type ty = value.getType();
    ...
    bool opAllowsDynamic = policy == CommGlobalShapePolicy::AllowDynamicPartitionView;
    bool isAllowedDynamicType = isa<pto::PartitionTensorViewType>(ty);
    for (int64_t dim : shape) {
        if (dim == ShapedType::kDynamic) {
            if (!opAllowsDynamic)
                return op->emitOpError() << "does not support dynamic dimensions on " << name;
        ...
        if (dim <= 0)
            return op->emitOpError() << "expects every static dimension of " << name << " to be positive";
    }
    return success();
}
...
static bool isSupportedMScatterAtomicPayloadElemType(Type ty, pto::ScatterAtomicOp atomic)
{
    ...
    switch (atomic) {
        ...
        case pto::ScatterAtomicOp::Add:
            return ty.isF16() || ty.isF32() || (intTy && intTy.getWidth() == 32);
        case pto::ScatterAtomicOp::Max:
        case pto::ScatterAtomicOp::Min:
            return ty.isF32() || (intTy && intTy.getWidth() == 32);
    }
...
static LogicalResult verifyMGatherMScatterTileShape(
    Operation* op, Type dataTy, Type idxTy, StringRef dataName, std::optional<pto::Coalesce> coalesce)
{
    ...
    const bool rowCoalesce1xR =
        idxRowMajor && isKnownZeroOrUnitExtent(idxValid[0]) && hasCompatibleKnownExtent(idxValid[1], dataValid[0]);
    const bool rowCoalesceRx1 =
        idxColMajor && hasCompatibleKnownExtent(idxValid[0], dataValid[0]) && isKnownZeroOrUnitExtent(idxValid[1]);
    const bool baseRowCoalesce =
        idxRowMajor && hasCompatibleKnownExtent(idxValid[0], dataValid[0]) && isKnownZeroOrUnitExtent(idxValid[1]);
    ...
    if (!coalesce) {
        if (baseRowCoalesce || elemCoalesce)
            return success();
        return op->emitOpError() << "expects idx valid_shape to be [" << dataName << ".valid_row, 0|1] or match "
                                 << dataName << " valid_shape when coalesce is omitted";
    }
    ...
    if (*coalesce == pto::Coalesce::Row)
        return op->emitOpError() << "expects row-coalesce idx valid_shape to be [0|1, " << dataName
                                 << ".valid_row] or [" << dataName << ".valid_row, 0|1]";

    return op->emitOpError() << "expects elem-coalesce idx valid_shape to match " << dataName << " valid_shape";
}
...
static ParseResult parsePTOInherentAttrs(
    OpAsmParser& parser, OperationState& result, NamedAttrList& parsedAttrs, ArrayRef<StringRef> inherentAttrNames)
{
    ...
    if (failed(OpTy::setPropertiesFromAttr(properties, parsedAttrs.getDictionary(parser.getContext()), [&] {
...
static NamedAttrList getNonInherentAttrs(Operation* op, ArrayRef<StringRef> inherentAttrNames)
{
    NamedAttrList attrs;
...
static pto::ScatterAtomicOpAttr getMScatterScatterAtomicOpAttrIfPresent(pto::MScatterOp op)
{
    return dyn_cast_or_null<pto::ScatterAtomicOpAttr>(op.getProperties().scatterAtomicOp);
}

static pto::ScatterOOBAttr getMScatterScatterOobAttrIfPresent(pto::MScatterOp op)
{
    return dyn_cast_or_null<pto::ScatterOOBAttr>(op.getProperties().scatterOob);
}

static pto::ScatterConflictAttr getMScatterScatterConflictAttrIfPresent(pto::MScatterOp op)
{
    return dyn_cast_or_null<pto::ScatterConflictAttr>(op.getProperties().scatterConflict);
}
...
static pto::ScatterConflictAttr getScatterConflictAttrIfPresent(pto::MScatterOp op)
{
    return getMScatterScatterConflictAttrIfPresent(op);
}

static Value getTPrintTmpIfPresent(pto::TPrintOp op) { return op->getNumOperands() > 1 ? op->getOperand(1) : Value(); }
...
static bool isA5TLoadStoreTransferElemType(Type ty)
{
    return ty.isInteger(8) || ty.isInteger(16) || ty.isInteger(32) || ty.isInteger(64) || ty.isF16() || ty.isBF16() ||
           ty.isF32() || isPTOLowPrecisionType(ty);
}
...
static bool isA5AccStorePreQuantDstType(Type srcElem, Type dstElem)
{
    ...
    if (!srcElem.isF32())
        return false;
    return dstElem.isInteger(8) || dstElem.isF16() || dstElem.isBF16() || dstElem.isF32() ||
...
static LogicalResult verifyTileBufSameElemType(Operation* op, Type lhs, Type rhs, StringRef lhsName, StringRef rhsName)
{
    if (!isTileLikeType(lhs) || !isTileLikeType(rhs))
        return op->emitOpError() << "expects " << lhsName << " and " << rhsName << " to be !pto.tile_buf";
...
static LogicalResult verifyPartialValidPatternLoose(Operation* op, Type src0Ty, Type src1Ty, Type dstTy)
{
    auto src0Valid = getValidShapeVec(src0Ty);
    auto src1Valid = getValidShapeVec(src1Ty);
    auto dstValid = getValidShapeVec(dstTy);
...
static LogicalResult verifyArithmeticElemTypeForArch(
    ...
    StringRef a5Error)
{
    bool supported = elemTy.isInteger(32) || elemTy.isInteger(16) || elemTy.isF16() || elemTy.isF32();
    if (targetArch == PTOArch::A5)
        supported = supported || (allowInt8OnA5 && elemTy.isInteger(8)) || (allowBf16OnA5 && elemTy.isBF16());
    if (supported)
        return success();
...
{
    auto verifyByArch = [&](PTOArch targetArch, bool requireValidRowsEqual) -> LogicalResult {
    ...
    auto verifyA2A3 = [&]() -> LogicalResult { return verifyByArch(PTOArch::A3, requireValidRowsEqualOnA2A3); };
    auto verifyA5 = [&]() -> LogicalResult { return verifyByArch(PTOArch::A5, requireValidRowsEqualOnA5); };
    return dispatchVerifierByArch(op, verifyA2A3, verifyA5);
}
...
static LogicalResult verifyTColReductionElemTypeForArch(
    ...
    StringRef a5Error)
{
    bool ok = elemTy.isF16() || elemTy.isF32() || elemTy.isInteger(16) || elemTy.isInteger(32);
    if (targetArch == PTOArch::A5)
        ok = ok || (allowInt8OnA5 && elemTy.isInteger(8)) || (allowBf16OnA5 && elemTy.isBF16());
    if (ok)
        return success();
    return op->emitOpError(targetArch == PTOArch::A5 ? a5Error : a2a3Error);
}
...
{
    auto verifyByArch = [&](PTOArch targetArch, bool requireNonZeroSrc) -> LogicalResult {
    ...
    auto verifyA2A3 = [&]() -> LogicalResult { return verifyByArch(PTOArch::A3, requireNonZeroSrcOnA2A3); };
    auto verifyA5 = [&]() -> LogicalResult { return verifyByArch(PTOArch::A5, requireNonZeroSrcOnA5); };
    return dispatchVerifierByArch(op, verifyA2A3, verifyA5);
}
...
static bool isKnownZeroOrUnitExtent(int64_t value) { return value == ShapedType::kDynamic || value == 0 || value == 1; }

static bool hasCompatibleKnownExtentOrZero(int64_t lhs, int64_t rhs)
{
    return lhs == ShapedType::kDynamic || rhs == ShapedType::kDynamic || lhs == 0 || lhs == rhs;
}
...
static LogicalResult verifyMatTileOperandsA2A3(
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, bool allowLowPrecision)
{
    ...
    if (lhsValid.size() == 2 && rhsValid.size() == 2) {
        ...
        if ((m != ShapedType::kDynamic && (m < 0 || m > 4095)) || (k != ShapedType::kDynamic && (k < 0 || k > 4095)) ||
...
static LogicalResult verifyMatTileOperandsA5(Operation* op, Type lhsTy, Type rhsTy, Type dstTy, bool allowLowPrecision)
{
    if (failed(verifyMatTileOperandsA2A3(op, lhsTy, rhsTy, dstTy, allowLowPrecision)))
        return failure();
...
static LogicalResult verifyMatTileOperands(Operation* op, Type lhsTy, Type rhsTy, Type dstTy, bool allowLowPrecision)
{
    switch (getVerifierTargetArch(op)) {
        case VerifierTargetArch::A2A3:
            return verifyMatTileOperandsA2A3(op, lhsTy, rhsTy, dstTy, allowLowPrecision);
        case VerifierTargetArch::A5:
            return verifyMatTileOperandsA5(op, lhsTy, rhsTy, dstTy, allowLowPrecision);
    }
    return failure();
}
...
static LogicalResult verifyA5MxMatTileOperands(Operation* op, Type lhsTy, Type rhsTy, Type dstTy)
{
    if (failed(verifyMatTileOperandsA5(
            op, lhsTy, rhsTy, dstTy,
    ...
    if (lhsValid.size() == 2 && rhsValid.size() == 2) {
        ...
        int64_t k = lhsValid[1];
        int64_t n = rhsValid[1];
        if ((m != ShapedType::kDynamic && (m < 1 || m > 4095)) || (k != ShapedType::kDynamic && (k < 1 || k > 4095)) ||
...
static LogicalResult verifyA5MxMatScaleTile(
    Operation* op, Type scaleTy, Type lhsTy, Type rhsTy, StringRef scaleName, bool isLeftScale)
{
    if (failed(verifyTileBufCommon(
            op, scaleTy, scaleName,
            /*allowLowPrecision=*/true)))
        return failure();
    ...
    auto checkDims = [&](ArrayRef<int64_t> scaleDims, ArrayRef<int64_t> lhsDims, ArrayRef<int64_t> rhsDims,
                         StringRef dimsName) -> LogicalResult {
        ...
        if (!hasCompatibleKnownExtent(scaleDims[0], expectedRows) ||
            ...
            return op->emitOpError() << "expects " << scaleName << " " << dimsName << " to be "
                                     << (isLeftScale ? "[M, ceil(K/32)]" : "[ceil(K/32), N]");
        }
        return success();
    };

    if (failed(checkDims(getShapeVec(scaleTy), getShapeVec(lhsTy), getShapeVec(rhsTy), "shape")))
        return failure();
    if (failed(checkDims(getValidShapeVec(scaleTy), getValidShapeVec(lhsTy), getValidShapeVec(rhsTy), "valid_shape")))
        return failure();
    ...
    if (scaleTb.getBLayoutValueI32() !=
        static_cast<int32_t>(isLeftScale ? pto::BLayout::RowMajor : pto::BLayout::ColMajor)) {
        return op->emitOpError() << "expects " << scaleName << " to use the "
                                 << (isLeftScale ? "row_major" : "col_major") << " blayout on A5";
    }
    if (scaleTb.getSLayoutValueI32() !=
        static_cast<int32_t>(isLeftScale ? pto::SLayout::RowMajor : pto::SLayout::ColMajor)) {
        return op->emitOpError() << "expects " << scaleName << " to use the "
                                 << (isLeftScale ? "row_major" : "col_major") << " slayout on A5";
    }
...
static LogicalResult verifyA5MxMatScaleTiles(Operation* op, Type lhsScaleTy, Type rhsScaleTy, Type lhsTy, Type rhsTy)
{
    if (failed(verifyA5MxMatScaleTile(
            op, lhsScaleTy, lhsTy, rhsTy, "a_scale",
            /*isLeftScale=*/true)))
        return failure();
    return verifyA5MxMatScaleTile(
        op, rhsScaleTy, lhsTy, rhsTy, "b_scale",
        /*isLeftScale=*/false);
}

static LogicalResult verifyA5MxGemvTileOperands(Operation* op, Type lhsTy, Type rhsTy, Type dstTy)
{
...
static LogicalResult verifyA5MxGemvScaleTile(
    Operation* op, Type scaleTy, Type lhsTy, Type rhsTy, StringRef scaleName, bool isLeftScale)
{
    if (failed(verifyTileBufCommon(
            op, scaleTy, scaleName,
    ...
    auto lhsValid = getValidShapeVec(lhsTy);
    auto rhsValid = getValidShapeVec(rhsTy);
    if (scaleShape.size() != 2 || scaleValid.size() != 2 || lhsShape.size() != 2 || rhsShape.size() != 2 ||
        lhsValid.size() != 2 || rhsValid.size() != 2)
    ...
    if (!hasCompatibleKnownExtent(scaleShape[0], expectedShapeRows) ||
        ...
        if (isLeftScale)
            return op->emitOpError() << "expects " << scaleName << " shape/valid_shape to be [M, ceil(K/32)]";
...
static LogicalResult verifyMatmulTypeTriple(Operation* op, Type lhsElemTy, Type rhsElemTy, Type dstElemTy)
{
    ...
    auto isA5TMatmulFp8Type = [](Type ty) { return isPTOFloat8Type(ty); };
...
LogicalResult pto::TAddReluOp::verify()
{
    ...
    auto verifyA5 = [&]() -> LogicalResult { return emitOpError("taddrelu is only supported on A2/A3 targets"); };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}
...
LogicalResult pto::TAxpyOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (!(sameType || widenF16ToF32))
            return emitOpError("expects dst/src element types to match, or dst=f32 and src=f16");
...
LogicalResult pto::TAndOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
            return emitOpError("expects A2/A3 tand src0, src1, and dst element type to be i8/i16/i32");
        return success();
    };
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        auto it = mlir::dyn_cast<IntegerType>(*elemOr);
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
            return emitOpError("expects A5 tand src0, src1, and dst element type to be i8/i16/i32");
        return success();
    };
...
mlir::LogicalResult mlir::pto::TConcatOp::verify()
{
    auto verifyCommon = [&]() -> FailureOr<Type> {
        ...
        Type t1 = getSrc1().getType();
        Type td = getDst().getType();
        if (failed(verifyTileBufCommon(*this, t0, "src0")) || failed(verifyTileBufCommon(*this, t1, "src1")) ||
        ...
        auto sd = getShapeVec(td);
        if (sd.size() == 2 && sd[1] != ShapedType::kDynamic && v0[1] != ShapedType::kDynamic &&
            v1[1] != ShapedType::kDynamic) {
            if (v0[1] + v1[1] > sd[1])
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (failed(elemOr))
            return failure();
        if (failed(verifyLocVec(getSrc0().getType(), "src0")) || failed(verifyLocVec(getSrc1().getType(), "src1")) ||
...
LogicalResult pto::TConcatidxOp::verify()
{
    auto verifyCommon = [&]() -> FailureOr<std::pair<Type, Type>> {
        ...
        Type td = getDst().getType();
        if (failed(verifyTileBufCommon(*this, t0, "src0")) || failed(verifyTileBufCommon(*this, t1, "src1")) ||
            failed(verifyTileBufCommon(*this, ti0, "src0Idx")) || failed(verifyTileBufCommon(*this, ti1, "src1Idx")) ||
            failed(verifyTileBufCommon(*this, td, "dst")))
            return failure();
        ...
        auto vd = getValidShapeVec(getDst());
        if (v0.size() != 2 || v1.size() != 2 || vi0.size() != 2 || vi1.size() != 2 || vd.size() != 2)
        ...
        auto checkValidRow = [&](const auto& v, StringRef name) -> LogicalResult {
            if (v[0] != ShapedType::kDynamic && vd[0] != ShapedType::kDynamic && v[0] != vd[0])
                return emitOpError("expects ") << name << " valid row to match dst valid row";
            return success();
        };
        if (failed(checkValidRow(v0, "src0")) || failed(checkValidRow(v1, "src1")) ||
            failed(checkValidRow(vi0, "src0Idx")) || failed(checkValidRow(vi1, "src1Idx")))
            return failure();
    ...
    auto verifyElementTypes = [&](Type dataElem, Type idxElem) -> LogicalResult {
        ...
        if (!dataElem.isF16() && !dataElem.isF32() && !dataElem.isBF16()) {
            ...
            if (!it || !it.isSignless() || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
                return emitOpError() << "expects data element type to be i8, i16, i32, f16, f32, or bf16";
        }

        // Index element type: i8, i16, i32 (signless).
        auto it = mlir::dyn_cast<IntegerType>(idxElem);
        if (!it || !it.isSignless() || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
            return emitOpError() << "expects index element type to be i8, i16, or i32";
        return success();
    };
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (failed(verifyLocVec(getSrc0().getType(), "src0")) || failed(verifyLocVec(getSrc1().getType(), "src1")) ||
            ...
            failed(verifyLocVec(getSrc1Idx().getType(), "src1Idx")) || failed(verifyLocVec(getDst().getType(), "dst")))
            return failure();
...
LogicalResult pto::TAndSOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16))
            return emitOpError("expects A2/A3 tands src, scalar, and dst element type to be i8/i16");
        return success();
    };
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        auto it = mlir::dyn_cast<IntegerType>(*elemOr);
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
            return emitOpError("expects A5 tands src, scalar, and dst element type to be i8/i16/i32");
        return success();
    };
...
static ParseResult parseTCILikeOp(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parser.parseRParen() || parser.parseKeyword("outs") || parser.parseLParen() || parser.parseOperand(dst) ||
        parser.parseColonType(dstTy) || parser.parseRParen() || parser.parseOptionalAttrDict(result.attributes))
        return failure();
    ...
    result.addAttribute("operandSegmentSizes", parser.getBuilder().getDenseI32ArrayAttr({1, hasTmp ? 1 : 0, 1}));
    return success();
}

static void printTCILikeOp(OpAsmPrinter& p, Operation* op, Value s, Value tmp, Value dst)
{
    p << " ins(" << s;
    if (tmp)
        p << ", " << tmp;
...
void mlir::pto::TCIOp::print(OpAsmPrinter& p) { printTCILikeOp(p, getOperation(), getS(), getTmp(), getDst()); }
...
LogicalResult pto::TCmpOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        Type t1 = getSrc1().getType();
        Type td = getDst().getType();
        if (failed(verifyVecTileStorage(*this, t0, "src0")) || failed(verifyVecTileStorage(*this, t1, "src1")) ||
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (e0 != e1)
            return emitOpError("expects src0 and src1 to have the same element type");
        bool inputOk =
            e0.isF16() || e0.isF32() || e0.isBF16() || e0.isInteger(8) || e0.isInteger(16) || e0.isInteger(32);
        if (!inputOk)
...
LogicalResult pto::TCmpSOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (!(elemTy.isInteger(16) || elemTy.isInteger(32) || elemTy.isF16() || elemTy.isF32()))
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        Type elemTy = getElemTy(srcTy);
        if (!(elemTy.isInteger(8) || elemTy.isInteger(16) || elemTy.isInteger(32) || elemTy.isF16() || elemTy.isF32()))
    ...
    Operation* op, Type t0, Type t1, Type td, PTOArch targetArch, StringRef opName, bool allowIntegerTypes)
{
    ...
    if (src1Valid.size() == 2 && dstValid.size() == 2 && src1Valid[1] != ShapedType::kDynamic &&
...
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        return verifyTColArgReductionOpA2A3(*this, getSrc().getType(), getTmp().getType(), getDst().getType());
    };
    auto verifyA5 = [&]() -> LogicalResult {
        return verifyTColArgReductionOpA5(*this, getSrc().getType(), getTmp().getType(), getDst().getType());
    };
...
LogicalResult pto::TColSumOp::verify()
{
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (failed(verifyColReductionValidRegion(
            ...
            return failure();
        Type elem = getElemTy(srcTy);
        if (!(elem.isF16() || elem.isF32() || elem.isBF16() || elem.isInteger(8) || elem.isInteger(16) ||
              elem.isInteger(32)))
...
llvm::LogicalResult mlir::pto::TCvtOp::verify()
{
    ...
    auto isResolvedSubview = [](Value value) {
        ...
        auto semantics = alloc ? alloc->getAttrOfType<StringAttr>("pto.view_semantics") : StringAttr();
    ...
    if (!isResolvedSubview(getSrc()) && !isResolvedSubview(getDst()) &&
        failed(verifyTileBufSameLogicalExtent(
...
{
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("trandom is only supported for A5 targets"); };
    auto verifyA5 = [&]() -> LogicalResult {
        Type dstTy = getDst().getType();
        if (failed(verifyTileBufCommon(*this, dstTy, "dst")))
            return failure();
...
mlir::LogicalResult mlir::pto::TDivSOp::verify()
{
    auto isTileLike = [](Type ty) -> bool {
        return isa<mlir::pto::TileBufType, RankedTensorType, mlir::pto::PartitionTensorViewType>(ty);
    };
    ...
    auto verifyByArch = [&](PTOArch targetArch) -> LogicalResult {
        ...
        if (targetArch == PTOArch::A3 && !(elem.isInteger(32) || elem.isInteger(16) || elem.isF16() || elem.isF32()))
            return emitOpError("expects A2/A3 tdivs element type to be i32/i16/f16/f32");
        if (targetArch == PTOArch::A5 &&
            !(elem.isInteger(32) || elem.isInteger(16) || elem.isInteger(8) || elem.isF16() || elem.isF32()))
...
mlir::LogicalResult mlir::pto::TExtractOp::verify()
{
    ...
    auto isA5AccQuantExtractTypePair = [&](Type srcElem, Type dstElem) -> bool {
        if (srcElem.isF32())
            return dstElem.isInteger(8) || dstElem.isF16() || dstElem.isBF16() || dstElem.isF32() ||
                   (llvm::isa<FloatType>(dstElem) && llvm::cast<FloatType>(dstElem).getWidth() == 8);
        if (srcElem.isInteger(32))
            return dstElem.isInteger(8) || dstElem.isF16() || dstElem.isBF16();
        return false;
    };
    ...
    auto hasMatExtractSourceLayoutA5 = [&](pto::TileBufType srcTy, pto::AddressSpace dstSpace) -> bool {
        int32_t bl = srcTy.getBLayoutValueI32();
        int32_t sl = srcTy.getSLayoutValueI32();
    ...
    auto verifyCommon =
        [&](bool allowLowPrecision) -> FailureOr<std::tuple<
                                        Type, Type, pto::TileBufType, pto::TileBufType, Type, Type,
                                        std::optional<pto::AddressSpace>, std::optional<pto::AddressSpace>>> {
        Type srcTy = getSrc().getType();
        Type dstTy = getDst().getType();
        ...
        if ((!srcSpace || *srcSpace != pto::AddressSpace::ACC) && srcElem != dstElem)
            return emitOpError("expects src and dst to have the same element type");
        return std::make_tuple(srcTy, dstTy, srcTb, dstTb, srcElem, dstElem, srcSpace, dstSpace);
    };
    auto verifyA2A3 = [&]() -> LogicalResult {
        auto common = verifyCommon(/*allowLowPrecision=*/false);
        if (failed(common))
            return failure();
        auto [srcTy, dstTy, srcTb, dstTb, srcElem, dstElem, srcSpace, dstSpace] = *common;
        ...
        if (srcSpace && dstSpace && *srcSpace == pto::AddressSpace::VEC && *dstSpace == pto::AddressSpace::VEC) {
            if (hasPreQuantScalar || hasRelu)
                return emitOpError(
                    "expects vec->vec textract to use the base form without preQuantScalar or reluPreMode");
            return mlir::success();
        }
        ...
        if (*srcSpace == pto::AddressSpace::ACC) {
            ...
            if (srcTb.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::ColMajor) ||
                ...
                return emitOpError(
                    "expects A2/A3 acc-source textract src to use blayout=col_major and slayout=row_major");
            if (dstTb.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::ColMajor) ||
                dstTb.getSLayoutValueI32() != static_cast<int32_t>(pto::SLayout::RowMajor))
                return emitOpError(
                    "expects A2/A3 acc-source textract dst to use blayout=col_major and slayout=row_major");
            ...
            if (hasPreQuantScalar) {
                if (!isA2A3AccQuantExtractTypePair(srcElem, dstElem))
                    return emitOpError("expects A2/A3 acc preQuantScalar textract element types to be "
                                       "(src=f32,dst=i8) or (src=i32,dst=i8/f16/i16)");
            } else if (!isA2A3AccCastExtractTypePair(srcElem, dstElem)) {
                return emitOpError("expects A2/A3 acc textract element types to be src=f32, dst=f16/bf16");
            }
            return mlir::success();
        }
        if (*srcSpace != pto::AddressSpace::MAT)
            return emitOpError("expects A2/A3 textract src to use loc=mat, loc=acc, or loc=vec");
        if (*dstSpace != pto::AddressSpace::LEFT && *dstSpace != pto::AddressSpace::RIGHT)
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (hasRelu && (!srcSpace || *srcSpace != pto::AddressSpace::ACC))
            return emitOpError("expects reluPreMode form to use loc=acc src");
        if (hasAccToVecMode &&
            (!srcSpace || !dstSpace || *srcSpace != pto::AddressSpace::ACC || *dstSpace != pto::AddressSpace::VEC))
            return emitOpError("expects accToVecMode only on A5 acc->vec textract forms");
        if (!srcSpace || !dstSpace)
            return emitOpError("expects src and dst to have explicit loc");
        ...
        if (*srcSpace == pto::AddressSpace::MAT) {
            if (hasPreQuantScalar || hasRelu)
                return emitOpError(
                    "expects mat-source textract to use the base form without preQuantScalar or reluPreMode");
        ...
        } else if (*srcSpace == pto::AddressSpace::VEC && *dstSpace == pto::AddressSpace::VEC) {
            if (hasPreQuantScalar || hasRelu)
                return emitOpError(
                    "expects vec-source textract to use the base form without preQuantScalar or reluPreMode");
            if (!isRowMajorNoneBoxND(srcTb) || !isRowMajorNoneBoxND(dstTb))
                return emitOpError("expects A5 vec->vec textract src/dst to use ND layout "
            ...
            if (*dstSpace == pto::AddressSpace::MAT) {
                if (dstTb.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::ColMajor) ||
                    dstTb.getSLayoutValueI32() != static_cast<int32_t>(pto::SLayout::RowMajor))
                    return emitOpError(
                        "expects A5 acc-source textract dst to use blayout=col_major and slayout=row_major");
            } else {
                if (!isRowMajorNoneBoxND(dstTb))
                    return emitOpError(
                        "expects A5 acc->vec textract dst to use ND layout (blayout=row_major, slayout=none_box)");
            }
            if (hasPreQuantScalar) {
                if (!isA5AccQuantExtractTypePair(srcElem, dstElem))
                    return emitOpError("expects A5 acc preQuantScalar textract element types to be "
                                       "(src=f32,dst=i8/fp8/f16/bf16/f32) or (src=i32,dst=i8/f16/bf16)");
            } else if (!isA5AccCastExtractTypePair(srcElem, dstElem)) {
                return emitOpError("expects A5 acc textract element types to be "
...
mlir::LogicalResult mlir::pto::TInsertOp::verify()
{
    ...
    auto getSpace = [](Type ty) -> std::optional<pto::AddressSpace> { return getPTOMemorySpaceEnum(ty); };
    ...
    auto verifyOptionalArgs = [&](const std::optional<pto::AddressSpace>& srcSpace,
                                  const std::optional<pto::AddressSpace>& dstSpace, bool isA5) -> LogicalResult {
        const bool hasFp = static_cast<bool>(getFp());
        const bool hasPreQuantScalar = static_cast<bool>(getPreQuantScalar());
        const bool hasAccToVecMode = static_cast<bool>(getAccToVecModeAttr());
        ...
        if (hasAccToVecMode) {
            ...
            if (!srcSpace || !dstSpace || *srcSpace != pto::AddressSpace::ACC || *dstSpace != pto::AddressSpace::VEC)
        ...
        if (hasInsertMode) {
            ...
            if (!srcSpace || !dstSpace || *srcSpace != pto::AddressSpace::VEC || *dstSpace != pto::AddressSpace::MAT)
                return emitOpError("tinsertMode (SPLIT2/SPLIT4) is only valid with src=vec, dst=mat");
            auto srcTb = dyn_cast<pto::TileBufType>(getSrc().getType());
            if (!srcTb || !isColMajorRowMajorNZ(srcTb))
                return emitOpError("tinsertMode (SPLIT2/SPLIT4) requires src NZ layout "
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (srcSpace && dstSpace && *srcSpace == pto::AddressSpace::VEC && *dstSpace == pto::AddressSpace::VEC) {
            if (hasPreQuantScalar || hasRelu)
                return emitOpError("expects vec->vec tinsert to use the base form without "
                                   "preQuantScalar or reluPreMode");
            if (srcElem != dstElem || !isA2A3VecInsertElemType(srcElem))
                return emitOpError("expects A2/A3 vec->vec tinsert src/dst to have same supported dtype "
                                   "(i8/f16/bf16/f32)");
            return success();
        }
        if (!srcSpace || !dstSpace || *srcSpace != pto::AddressSpace::ACC || *dstSpace != pto::AddressSpace::MAT)
        ...
        if (hasPreQuantScalar) {
            if (!isA2A3AccQuantInsertTypePair(srcElem, dstElem))
                return emitOpError("expects A2/A3 acc preQuantScalar tinsert element types to be "
                                   "(src=f32,dst=i8) or (src=i32,dst=i8/f16/i16)");
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (hasTInsertMode &&
            (!srcSpace || !dstSpace || *srcSpace != pto::AddressSpace::VEC || *dstSpace != pto::AddressSpace::MAT))
        ...
        if (*srcSpace == pto::AddressSpace::ACC &&
            ...
            if (*dstSpace == pto::AddressSpace::MAT) {
                ...
                if (!dstIsND && !dstIsNZ)
                    return emitOpError("expects A5 acc->vec tinsert dst to use ND(row_major/none_box) or "
                                       "NZ(col_major/row_major) layout");
            }
            ...
            if (hasQuant) {
                ...
                okTypes = (srcElem.isF32() && (dstElem.isF16() || dstElem.isBF16() || dstElem.isF32())) ||
            ...
            if (!okTypes)
                return emitOpError(
                    ...
                    (hasQuant ?
                         std::string(
                             "; with fp/scalar: (src=f32,dst=i8/fp8/f16/bf16/f32) or (src=i32,dst=i8/f16/bf16)") :
                         std::string()));
            return success();
        }
        ...
        if (*srcSpace == pto::AddressSpace::VEC && *dstSpace == pto::AddressSpace::MAT) {
            if (hasPreQuantScalar || hasRelu)
                return emitOpError("expects vec->mat tinsert to use the base form without "
                                   "preQuantScalar or reluPreMode");
            if (!isColMajorRowMajorNZ(dstTb))
            ...
            if (srcElem != dstElem || !isA5SupportedVecElemType(srcElem))
                return emitOpError("expects A5 vec->mat tinsert src/dst to have same supported dtype "
        ...
        if (*srcSpace == pto::AddressSpace::VEC && *dstSpace == pto::AddressSpace::VEC) {
            ...
            if (srcIsND && dstIsND) {
                ...
                return emitOpError("expects A5 vec->vec tinsert src/dst layouts to match: "
                                   "both ND(row_major/none_box) or both NZ(col_major/row_major)");
            }
            if (srcElem != dstElem || !isA5SupportedVecElemType(srcElem))
                return emitOpError("expects A5 vec->vec tinsert src/dst to have same supported dtype "
                                   "(fp8/f16/bf16/f32/i8/i32)");
            return success();
        }

        return emitOpError("expects A5 tinsert to use a supported src/dst loc pair: "
...
static bool isA5MxFp8InputType(Type ty) { return ty && isa<Float8E4M3FNType, Float8E5M2Type>(ty); }
    ...
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, StringRef lhsName, StringRef rhsName, StringRef dstName)
{
    ...
    if (!isA5MxInputTypePair(lhsElem, rhsElem))
        return op->emitOpError() << "expects A5 mx " << lhsName << "/" << rhsName
...
{
    if (srcElem.isF32())
        return dstElem.isInteger(8) || isA5Fp8LikeType(dstElem) || isPTOHiFloat8Type(dstElem) || dstElem.isF16() ||
...
mlir::LogicalResult mlir::pto::TExtractFPOp::verify()
{
    auto verifyCommon =
        [&](bool allowLowPrecision) -> FailureOr<std::tuple<
                                        Type, Type, Type, pto::TileBufType, pto::TileBufType, pto::TileBufType,
                                        pto::AddressSpace, pto::AddressSpace, pto::AddressSpace>> {
        Type srcTy = getSrc().getType();
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (!isA2A3VectorPreQuantTypePair(srcElem, dstElem))
            return emitOpError("expects A2/A3 textract_fp element types to be (src=f32,dst=i8) "
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (!isA5VectorPreQuantTypePair(srcElem, dstElem))
            return emitOpError("expects A5 textract_fp element types to be (src=f32,dst=i8/fp8/f16/bf16/f32) "
...
mlir::LogicalResult mlir::pto::TInsertFPOp::verify()
{
    auto verifyCommon = [&](bool allowLowPrecision, bool isA5)
        -> FailureOr<std::tuple<
            Type, Type, Type, pto::TileBufType, pto::TileBufType, pto::TileBufType, pto::AddressSpace,
        ...
        if (*dstSpace != pto::AddressSpace::MAT && !(isA5 && *dstSpace == pto::AddressSpace::VEC))
            return emitOpError("expects dst to use loc=mat" + (isA5 ? StringRef(" or loc=vec (A5)") : StringRef("")));
        if (!isColMajorRowMajorNZTileBuf(srcTb))
            return emitOpError("expects src to use blayout=col_major and slayout=row_major");
        ...
        if (*dstSpace == pto::AddressSpace::VEC && !isRowMajorNoneBoxNDTileBuf(dstTb) &&
            !isColMajorRowMajorNZTileBuf(dstTb))
        ...
        if (static_cast<bool>(getAccToVecModeAttr()) && *dstSpace != pto::AddressSpace::VEC)
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        Type dstElem = getElemTy(dstTy);
        if (!isA2A3VectorPreQuantTypePair(srcElem, dstElem))
            return emitOpError("expects A2/A3 tinsert_fp element types to be (src=f32,dst=i8) "
                               "or (src=i32,dst=i8/f16/i16)");
        return success();
    };
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        Type dstElem = getElemTy(dstTy);
        if (!isA5VectorPreQuantTypePair(srcElem, dstElem))
            return emitOpError("expects A5 tinsert_fp element types to be (src=f32,dst=i8/fp8/f16/bf16/f32) "
                               "or (src=i32,dst=i8/f16/bf16)");
        return success();
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
...
llvm::LogicalResult mlir::pto::TGatherOp::verify()
{
    ...
    auto verifyMaskForm = [&](bool allowA5MaskTypes) -> LogicalResult {
        ...
        auto dstValid = getValidShapeVec(dstTy);
        auto dstShape = getShapeVec(dstTy);
        if (dstValid.size() == 2 && dstShape.size() == 2 && dstValid[1] != ShapedType::kDynamic &&
        ...
        if (axisVal == "row") {
            ...
            if (srcValid[1] != ShapedType::kDynamic && dstValid[1] != ShapedType::kDynamic &&
                ...
                return emitOpError(
                    "expects src valid cols to equal dst valid cols times the mask expansion factor for row direction");
        } else if (axisVal == "col") {
            if (srcValid[1] != ShapedType::kDynamic && dstValid[1] != ShapedType::kDynamic &&
            ...
            if (srcValid[0] != ShapedType::kDynamic && dstValid[0] != ShapedType::kDynamic &&
                ...
                return emitOpError(
                    "expects src valid rows to equal dst valid rows times the mask expansion factor for col direction");
        } else {
    ...
    auto verifyIndexForm = [&](bool allow16BitIndices, bool allowA5ElemTypes) -> LogicalResult {
        ...
        if (srcElem != dstElem)
            return emitOpError("expects src and dst to have the same element type");
        if (allowA5ElemTypes) {
            if (!isSupportedGatherElemTypeA5Index(srcElem) || !isSupportedGatherElemTypeA5Index(dstElem))
                return emitOpError("expects A5 gather src/dst element type to be i8/i16/i32/f16/f32");
        } else if (!isSupportedGatherElemTypeA2A3(srcElem) || !isSupportedGatherElemTypeA2A3(dstElem)) {
        ...
        if (!(width == 32 || (allow16BitIndices && width == 16))) {
            return emitOpError() << "expects indices element type to be i32" << (allow16BitIndices ? " or i16" : "");
        }
        ...
        if (idxValid.size() == 2 && idxShape.size() == 2 && idxValid[1] != ShapedType::kDynamic &&
            idxShape[1] != ShapedType::kDynamic && idxValid[1] != idxShape[1]) {
    ...
    auto verifyCompareForm = [&](bool allowA5SrcTypes) -> LogicalResult {
        ...
        if (failed(verifyTileBufCommon(*this, srcTy, "src")) || failed(verifyTileBufCommon(*this, dstTy, "dst")) ||
            failed(verifyTileBufCommon(*this, cdstTy, "cdst")) || failed(verifyTileBufCommon(*this, tmpTy, "tmp")))
            return failure();
        ...
        if (allowA5SrcTypes) {
            if (!(srcElem.isF16() || srcElem.isF32() || srcElem.isInteger(16) || srcElem.isInteger(32))) {
                return emitOpError("expects A5 compare-form tgather src element type to be i16/i32/f16/f32");
            }
        } else {
            if (!(srcElem.isF16() || srcElem.isF32() || (srcElem.isInteger(32) && cmpMode == pto::CmpMode::EQ))) {
                return emitOpError(
...
mlir::LogicalResult mlir::pto::TMovOp::verify()
{
    auto verifyImpl = [&](bool isA5) -> LogicalResult {
        ...
        if (failed(verifyTileBufCommon(*this, srcTy, "src", /*allowLowPrecision=*/isA5)) ||
            ...
            return failure();
        if (hasFp && failed(verifyTileBufCommon(
                         *this, fp.getType(), "fp",
        ...
        const bool isMatToTile = *srcSpace == pto::AddressSpace::MAT &&
                                 (*dstSpace == pto::AddressSpace::LEFT || *dstSpace == pto::AddressSpace::RIGHT ||
                                  *dstSpace == pto::AddressSpace::BIAS || *dstSpace == pto::AddressSpace::SCALING);
        const bool isVecToVec = *srcSpace == pto::AddressSpace::VEC && *dstSpace == pto::AddressSpace::VEC;
        const bool isVecToMat = *srcSpace == pto::AddressSpace::VEC && *dstSpace == pto::AddressSpace::MAT;
        const bool isAccToMat = *srcSpace == pto::AddressSpace::ACC && *dstSpace == pto::AddressSpace::MAT;
        const bool isAccToVec = *srcSpace == pto::AddressSpace::ACC && *dstSpace == pto::AddressSpace::VEC;

        bool okPair = isMatToTile || isVecToVec || isAccToMat || isAccToVec;
        if (isA5)
            okPair = okPair || isVecToMat;
        if (!okPair)
            return emitOpError() << "expects a supported tmov address-space pair for this target";

        if (accToVecModeAttr && !isAccToVec)
            return emitOpError() << "expects accToVecMode to be used only for acc-to-vec tmov";
        ...
        if (hasFp) {
            ...
            if (!(srcElemTy.isF32() || (srcIntTy && srcIntTy.getWidth() == 32)))
                return emitOpError() << "expects fp form src to have element type f32, i32";
        ...
        auto srcTb = dyn_cast<pto::TileBufType>(srcTy);
        auto dstTb = dyn_cast<pto::TileBufType>(dstTy);
        if (srcTb && *srcSpace == pto::AddressSpace::ACC && (hasFp || reluMode != pto::ReluPreMode::NoRelu)) {
            if (srcTb.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::ColMajor) ||
                srcTb.getSLayoutValueI32() != static_cast<int32_t>(pto::SLayout::RowMajor))
                return emitOpError()
                       << "expects acc-source fp/relu tmov src to use blayout=col_major and slayout=row_major";
        }
        if (srcTb && dstTb && isAccToMat && !isA5 && dstTb.getSFractalSizeI32() != 512)
...
mlir::LogicalResult mlir::pto::TMovFPOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        Type fpTy = getFp().getType();
        Type dstTy = getDst().getType();
        if (failed(verifyTileBufCommon(*this, srcTy, "src")) || failed(verifyTileBufCommon(*this, fpTy, "fp")) ||
            failed(verifyTileBufCommon(*this, dstTy, "dst")))
            return failure();
        auto srcElemTy = getElemTy(srcTy);
        auto srcIntTy = dyn_cast<IntegerType>(srcElemTy);
        if (!(srcElemTy.isF32() || (srcIntTy && srcIntTy.getWidth() == 32)))
            return emitOpError() << "expects src to have element type f32, i32";
        auto fpSpace = getPTOMemorySpaceEnum(fpTy);
        ...
        auto srcTb = dyn_cast<pto::TileBufType>(srcTy);
        auto dstTb = dyn_cast<pto::TileBufType>(dstTy);
        if (srcTb && (srcTb.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::ColMajor) ||
                      srcTb.getSLayoutValueI32() != static_cast<int32_t>(pto::SLayout::RowMajor)))
            return emitOpError() << "expects src to use blayout=col_major and slayout=row_major";
        if (dstTb && (dstTb.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::ColMajor) ||
                      dstTb.getSLayoutValueI32() != static_cast<int32_t>(pto::SLayout::RowMajor)))
...
ParseResult CmoCacheInvalidOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    Type addrTy;
    if (parser.parseOperand(addr) || parser.parseKeyword("single_cache_line") || parser.parseColonType(addrTy) ||
        parser.parseOptionalAttrDict(result.attributes))
        return failure();
    ...
    if (!result.attributes.get("space")) {
        result.addAttribute("space", AddressSpaceAttr::get(parser.getContext(), AddressSpace::GM));
    }
    return success();
}
...
static LogicalResult verifyBufSyncOp(Operation* op, Attribute opTypeAttr, IntegerAttr bufIdAttr, IntegerAttr modeAttr)
{
    ...
    if (auto pipeAttr = dyn_cast<PipeAttr>(opTypeAttr)) {
        ...
        if (failed(opTypeOr)) {
            auto diag = op->emitOpError("expects 'op_type' to be pipe_event_type/sync_op_type/pipe, got ");
            diag << opTypeAttr;
            return failure();
        }
...
static LogicalResult verifyBufDynSyncOp(Operation* op, Attribute opTypeAttr, Value bufId, IntegerAttr modeAttr)
{
    if (!opTypeAttr)
        return op->emitOpError("expects 'op_type' attribute");
...
{
    return verifyBufDynSyncOp(getOperation(), getOpTypeAttr(), getBufId(), getModeAttr());
}
...
static ParseResult parseLegacyOrAttrMemBar(OpAsmParser& parser, MemBarAttr& attr)
{
...
static void printLegacyOrAttrMemBar(OpAsmPrinter& p, MemBarAttr kind, ArrayRef<NamedAttribute> attrs)
{
    p << ' ' << '"' << stringifyMemBarKind(kind.getKind()) << '"';
    p.printOptionalAttrDict(attrs, {"kind"});
}

static ParseResult parseLegacyOrAttrDsbMem(OpAsmParser& parser, DsbMemAttr& attr)
{
    auto loc = parser.getCurrentLocation();
    std::string token;
    if (succeeded(parser.parseOptionalString(&token))) {
...
static void printLegacyOrAttrDsbMem(OpAsmPrinter& printer, Operation* op, DsbMemAttr mem)
{
    (void)op;
    printer << ' ' << '"' << stringifyDsbMem(mem.getKind()) << '"';
}

static ParseResult parseLegacyOrAttrDcciCacheLine(OpAsmParser& parser, DcciCacheLineAttr& attr)
{
    auto loc = parser.getCurrentLocation();
    std::string token;
    if (succeeded(parser.parseOptionalString(&token))) {
...
static void printLegacyOrAttrDcciCacheLine(OpAsmPrinter& printer, Operation* op, DcciCacheLineAttr cache)
{
    (void)op;
    printer << ' ' << '"' << stringifyDcciCacheLine(cache.getKind()) << '"';
}

static ParseResult parseOptionalDcciDst(OpAsmParser& parser, DcciDstAttr& attr)
{
...
static void printOptionalDcciDst(OpAsmPrinter& printer, Operation* op, DcciDstAttr dst)
{
    (void)op;
    if (!dst)
        return;
...
static void printLegacySyncTriplet(
    OpAsmPrinter& p, PipeAttr srcPipe, PipeAttr dstPipe, EventAttr eventId, ArrayRef<NamedAttribute> attrs)
{
    p << "[<" << stringifyPIPE(srcPipe.getPipe()) << ">, <" << stringifyPIPE(dstPipe.getPipe()) << ">, <"
...
ParseResult SetFlagOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    PipeAttr dstPipe;
    EventAttr eventId;
    if (parser.parseLSquare() || parseLegacyOrAttrPipe(parser, srcPipe) || parser.parseComma() ||
        parseLegacyOrAttrPipe(parser, dstPipe) || parser.parseComma() || parseLegacyOrAttrEvent(parser, eventId) ||
        parser.parseRSquare())
        return failure();
    if (parser.parseOptionalAttrDict(result.attributes))
        return failure();
...
{
    printLegacySyncTriplet(p, getSrcPipe(), getDstPipe(), getEventId(), (*this)->getAttrs());
}
...
void MemBarOp::print(OpAsmPrinter& p) { printLegacyOrAttrMemBar(p, getKind(), (*this)->getAttrs()); }
...
static ParseResult parseBufSyncOp(OpAsmParser& parser, OperationState& result)
{
    ...
    if (succeeded(parser.parseOptionalString(&token))) {
        ...
        if (parser.parseAttribute(opTypeAttr) || parser.parseComma() || parseI32LiteralAttr(parser, bufIdAttr))
            return failure();
        if (succeeded(parser.parseOptionalComma())) {
            if (parseI32LiteralAttr(parser, modeAttr))
                return failure();
...
static void printBufSyncOp(
    OpAsmPrinter& p, Attribute opTypeAttr, IntegerAttr bufIdAttr, IntegerAttr modeAttr, ArrayRef<NamedAttribute> attrs)
{
    if (auto pipeAttr = dyn_cast<PipeAttr>(opTypeAttr)) {
        p << " \"" << stringifyPIPE(pipeAttr.getPipe()) << "\", " << bufIdAttr.getInt() << ", " << modeAttr.getInt();
    } else if (auto pipeEventType = dyn_cast<PipeEventTypeAttr>(opTypeAttr)) {
        p << "[" << opTypeAttr << ", " << bufIdAttr.getInt() << ", " << modeAttr.getInt() << "]";
...
ParseResult GetBufOp::parse(OpAsmParser& parser, OperationState& result) { return parseBufSyncOp(parser, result); }

void GetBufOp::print(OpAsmPrinter& p)
{
    printBufSyncOp(p, getOpTypeAttr(), getBufIdAttr(), getModeAttr(), (*this)->getAttrs());
}

ParseResult RlsBufOp::parse(OpAsmParser& parser, OperationState& result) { return parseBufSyncOp(parser, result); }
...
static ParseResult parseBufDynSyncOp(OpAsmParser& parser, OperationState& result)
{
    Attribute opTypeAttr;
    IntegerAttr modeAttr;
    ...
    if (succeeded(parser.parseOptionalString(&token))) {
        if (auto pipe = symbolizePIPE(token))
            ...
            return parser.emitError(loc) << "invalid get_buf_dyn/rls_buf_dyn token: " << token;
        ...
        if (parser.parseOperand(bufOperand))
            return failure();
        if (parser.resolveOperand(bufOperand, parser.getBuilder().getIndexType(), result.operands))
            return failure();
...
static void printBufDynSyncOp(
    OpAsmPrinter& p, Attribute opTypeAttr, Value bufId, IntegerAttr modeAttr, ArrayRef<NamedAttribute> attrs)
{
    if (auto pipeAttr = dyn_cast<PipeAttr>(opTypeAttr)) {
        p << " \"" << stringifyPIPE(pipeAttr.getPipe()) << "\", " << bufId << ", " << modeAttr.getInt();
    } else {
...
{
    printBufDynSyncOp(p, getOpTypeAttr(), getBufId(), getModeAttr(), (*this)->getAttrs());
}
...
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        if (failed(verifyGemvTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())) ||
            failed(verifyMatBiasTile(*this, getBias().getType(), getDst().getType())))
            return failure();
        if (failed(verifyMatmulTypeTriple(
                *this, getElemTy(getA().getType()), getElemTy(getB().getType()), getElemTy(getDst().getType()))))
            return failure();
...
{
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("tgemv.mx is only supported on A5 targets"); };
    auto verifyA5 = [&]() -> LogicalResult {
        if (failed(verifyA5MxGemvTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())) ||
            failed(verifyA5MxGemvScaleTile(
                *this, getAScale().getType(), getA().getType(), getB().getType(), "a_scale", /*isLeftScale=*/true)) ||
            failed(verifyA5MxGemvScaleTile(
                *this, getBScale().getType(), getA().getType(), getB().getType(), "b_scale", /*isLeftScale=*/false)))
            return failure();
        if (failed(verifyA5MxTypeTriple(
                *this, getA().getType(), getB().getType(), getDst().getType(), "lhs", "rhs", "dst")))
            return failure();
        return verifyMatmulLike(*this, getA().getType(), getB().getType(), getDst().getType());
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
...
{
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("tgemv.mx.acc is only supported on A5 targets"); };
    auto verifyA5 = [&]() -> LogicalResult {
        if (failed(verifyAccTileCommon(*this, getCIn().getType(), "c_in")) ||
            failed(verifyA5MxGemvTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())) ||
            failed(verifyA5MxGemvScaleTile(
                *this, getAScale().getType(), getA().getType(), getB().getType(), "a_scale", /*isLeftScale=*/true)) ||
            failed(verifyA5MxGemvScaleTile(
                *this, getBScale().getType(), getA().getType(), getB().getType(), "b_scale", /*isLeftScale=*/false)))
...
{
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("tgemv.mx.bias is only supported on A5 targets"); };
    auto verifyA5 = [&]() -> LogicalResult {
        if (failed(verifyA5MxGemvTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())) ||
            failed(verifyA5MxGemvScaleTile(
                *this, getAScale().getType(), getA().getType(), getB().getType(), "a_scale", /*isLeftScale=*/true)) ||
...
LogicalResult TMatmulBiasOp::verify()
{
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        if (failed(verifyMatmulTypeTriple(
                *this, getElemTy(getA().getType()), getElemTy(getB().getType()), getElemTy(getDst().getType()))))
            return failure();
        if (failed(verifyMatTileOperands(
                *this, getA().getType(), getB().getType(), getDst().getType(),
                /*allowLowPrecision=*/true)) ||
            failed(verifyMatBiasTile(*this, getBias().getType(), getDst().getType())))
            return failure();
        return verifyMatmulLike(*this, getA().getType(), getB().getType(), getDst().getType());
...
{
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("tmatmul.mx is only supported on A5 targets"); };
    auto verifyA5 = [&]() -> LogicalResult {
        if (failed(verifyA5MxMatTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())) ||
            failed(verifyA5MxMatScaleTiles(
                *this, getAScale().getType(), getBScale().getType(), getA().getType(), getB().getType())))
            return failure();
        if (failed(verifyA5MxTypeTriple(
                *this, getA().getType(), getB().getType(), getDst().getType(), "lhs", "rhs", "dst")))
            return failure();
...
{
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("tmatmul.mx.acc is only supported on A5 targets"); };
    auto verifyA5 = [&]() -> LogicalResult {
        if (failed(verifyAccTileCommon(*this, getCIn().getType(), "c_in")) ||
            failed(verifyA5MxMatTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())) ||
            failed(verifyA5MxMatScaleTiles(
                *this, getAScale().getType(), getBScale().getType(), getA().getType(), getB().getType())))
            return failure();
        if (failed(verifyA5MxTypeTriple(
...
{
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("tmatmul.mx.bias is only supported on A5 targets"); };
    auto verifyA5 = [&]() -> LogicalResult {
        if (failed(verifyA5MxMatTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())) ||
            failed(verifyA5MxMatScaleTiles(
                *this, getAScale().getType(), getBScale().getType(), getA().getType(), getB().getType())) ||
...
LogicalResult THistogramOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("thistogram is only supported on A5"); };
    auto verifyA5 = [&]() -> LogicalResult {
        Type srcTy = getSrc().getType();
        Type idxTy = getIdx().getType();
        Type dstTy = getDst().getType();
        if (failed(verifyTileBufCommon(*this, srcTy, "src")) || failed(verifyTileBufCommon(*this, idxTy, "idx")) ||
            failed(verifyTileBufCommon(*this, dstTy, "dst")))
            return failure();
        ...
        auto idxValid = getValidShapeVec(idxTy);
        auto dstValid = getValidShapeVec(dstTy);
        if (srcShape.size() != 2 || idxShape.size() != 2 || dstShape.size() != 2 || srcValid.size() != 2 ||
            idxValid.size() != 2 || dstValid.size() != 2)
            return emitOpError("expects src, idx, and dst to have rank-2 shape and valid_shape");
        ...
        if (srcIsUi16) {
            ...
            if (idxTB.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::ColMajor) ||
                ...
                return emitOpError("expects idx to use DN layout (col_major + none_box) when src element type is ui16");
            ...
            if (!isKnownUnitExtent(idxShape[1]) || !isKnownZeroOrUnitExtent(idxValid[1]))
                return emitOpError("expects idx to have exactly one physical column and 0 or 1 valid column when src "
                                   "element type is ui16");
        } else {
            if (byte != 3) {
                if (idxTB.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::RowMajor) ||
                    idxTB.getSLayoutValueI32() != static_cast<int32_t>(pto::SLayout::NoneBox))
                    return emitOpError("expects idx to use row_major + none_box layout when src element type is ui32 "
                                       "and byte is 0, 1, or 2");
                if (!hasCompatibleKnownExtent(srcShape[1], idxShape[1]) ||
                    !hasCompatibleKnownExtent(srcValid[1], idxValid[1]))
                    return emitOpError("expects idx cols and valid cols to match src when src element type is ui32 and "
                                       "byte is 0, 1, or 2");
                ...
                if (!hasCompatibleKnownExtent(idxShape[0], expectedIdxRows) ||
                    ...
                    return emitOpError("expects idx rows to match the byte-selected filter depth and idx valid rows to "
                                       "be 0 or match it when src element type is ui32 and byte is 0, 1, or 2");
            }
        }
        if (dstShape[1] != ShapedType::kDynamic && dstShape[1] < 256)
            return emitOpError("expects dst shape[1] to be at least 256");
        if (dstValid[1] != ShapedType::kDynamic && dstValid[1] != 0 && dstValid[1] < 256)
...
{
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("tget_scale_addr is only supported on A5"); };
    auto verifyA5 = [&]() -> LogicalResult {
        Type srcTy = getSrc().getType();
        Type dstTy = getDst().getType();
        if (failed(verifyTileBufCommon(*this, srcTy, "src", /*allowLowPrecision=*/true)))
        ...
        if (failed(verifyTileBufCommon(*this, dstTy, "dst", /*allowLowPrecision=*/true)))
            return failure();
        auto srcSpace = getPTOMemorySpaceEnum(srcTy);
        if (!srcSpace || (*srcSpace != pto::AddressSpace::LEFT && *srcSpace != pto::AddressSpace::RIGHT))
        ...
        auto dstValid = getValidShapeVec(dstTy);
        auto srcValid = getValidShapeVec(srcTy);
        if (dstShape.size() != 2 || srcShape.size() != 2 || dstValid.size() != 2 || srcValid.size() != 2)
            return emitOpError("expects src/dst to have rank-2 shape and valid_shape");
...
// ---- MScatterOp ----
ParseResult mlir::pto::MScatterOp::parse(OpAsmParser& parser, OperationState& result)
{
    OpAsmParser::UnresolvedOperand src;
    ...
    if (parser.parseKeyword("ins") || parser.parseLParen() || parser.parseOperand(src) || parser.parseComma() ||
        parser.parseOperand(idx) || parser.parseColonType(srcTy) || parser.parseComma() || parser.parseType(idxTy) ||
        parser.parseRParen() || parser.parseKeyword("outs") || parser.parseLParen() || parser.parseOperand(mem) ||
        parser.parseColonType(memTy) || parser.parseRParen() ||
        parsePTOInherentAttrs<MScatterOp>(
            parser, result, parsedAttrs, {"coalesce", "scatterAtomicOp", "scatterOob", "scatterConflict"}))
        return failure();
...
{
    p << " ins(" << getSrc() << ", " << getIdx() << " : " << getSrc().getType() << ", ";
    ...
    NamedAttrList attrs =
        getNonInherentAttrs(getOperation(), {"coalesce", "scatterAtomicOp", "scatterOob", "scatterConflict"});
    ...
    if (auto scatterAtomicAttr = getMScatterScatterAtomicOpAttrIfPresent(*this);
        scatterAtomicAttr && scatterAtomicAttr.getValue() != pto::ScatterAtomicOp::None)
        attrs.append("scatterAtomicOp", scatterAtomicAttr);
    if (auto scatterOobAttr = getMScatterScatterOobAttrIfPresent(*this);
        scatterOobAttr && scatterOobAttr.getValue() != pto::ScatterOOB::Undefined)
        attrs.append("scatterOob", scatterOobAttr);
    if (auto scatterConflictAttr = getMScatterScatterConflictAttrIfPresent(*this))
...
LogicalResult MScatterOp::verify()
{
    ...
    if (failed(verifyMGatherMScatterTileShape(getOperation(), srcTy, idxTy, "src", coalesce)))
        return failure();

    if (!coalesce && (scatterAtomicOp != pto::ScatterAtomicOp::None || scatterOob != pto::ScatterOOB::Undefined ||
                      getScatterConflictAttrIfPresent(*this)))
        return emitOpError("expects coalesce when scatterAtomicOp/scatterOob/scatterConflict is specified");
...
static LogicalResult verifyMGatherGm2L1(
    Operation* op, Value mem, Value idx, Value dst, Value scratch, std::optional<pto::Coalesce> coalesce)
{
    Type dstTy = dst.getType();
    auto dstTb = dyn_cast<pto::TileBufType>(dstTy);
    if (!dstTb)
    ...
    if (!isSupportedMGatherMScatterPayloadElemType(op, dstElem))
        return op->emitOpError("expects GM->L1 mgather dst element type to be "
    ...
    unsigned elemBytes = std::max<unsigned>(1u, dstElem.getIntOrFloatBitWidth() / 8u);
    int64_t kC0 = 32 / static_cast<int64_t>(elemBytes);
    auto dstShape = getShapeVec(dstTy);
    if (dstShape.size() == 2) {
        if (kC0 > 0 && dstShape[1] != ShapedType::kDynamic && dstShape[1] % kC0 != 0)
            return op->emitOpError() << "expects GM->L1 mgather dst padded cols to be a multiple of " << kC0
                                     << " (C0 = 32 / sizeof(elem))";
    ...
    if (!isa<pto::PartitionTensorViewType>(idxTy))
        return op->emitOpError("expects GM->L1 mgather idx to be a partition_tensor_view");
    Type idxElem = getElemTy(idxTy);
    ...
    if (*coalesce == pto::Coalesce::Elem) {
        ...
        if (!isa<pto::PartitionTensorViewType>(scTy))
            return op->emitOpError("expects GM->L1 mgather scratch to be a partition_tensor_view");
...
ParseResult mlir::pto::MGatherOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (insOperands.size() < 2 || insOperands.size() > 3)
        return parser.emitError(parser.getCurrentLocation(), "expects mgather ins(mem, idx[, scratch])");
    ...
    if (insOperands.size() != insTypes.size())
        return parser.emitError(
            parser.getCurrentLocation(), "expects the number of ins operands to match the number of ins types");

    if (parser.parseRParen() || parser.parseKeyword("outs") || parser.parseLParen() || parser.parseOperand(dst) ||
        parser.parseColonType(dstTy) || parser.parseRParen() ||
        parsePTOInherentAttrs<MGatherOp>(parser, result, parsedAttrs, {"coalesce", "gatherOob"}))
        return failure();
    ...
    if (parser.resolveOperand(insOperands[0], insTypes[0], result.operands) ||
        ...
        return failure();
    if (insOperands.size() == 3 && parser.resolveOperand(insOperands[2], insTypes[2], result.operands))
        return failure();
    return success();
}
...
void mlir::pto::MGatherOp::print(OpAsmPrinter& p)
{
    ...
    NamedAttrList attrs = getNonInherentAttrs(getOperation(), {"coalesce", "gatherOob"});
...
LogicalResult MGatherOp::verify()
{
    ...
    if (isa<pto::TileBufType>(dstTy)) {
        if (auto as = getPTOMemorySpaceEnum(dstTy); as && *as == pto::AddressSpace::MAT) {
            std::optional<pto::Coalesce> coalesce;
            if (auto coalesceAttr = getCoalesceAttr())
                coalesce = coalesceAttr.getValue();
            return verifyMGatherGm2L1(getOperation(), getMem(), getIdx(), getDst(), getScratch(), coalesce);
        }
    }
    ...
    if (failed(verifyMGatherMScatterTileShape(getOperation(), dstTy, idxTy, "dst", coalesce)))
        return failure();
...
mlir::LogicalResult mlir::pto::TNegOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        Type elemTy = getElemTy(srcTy);
        if (!(elemTy.isInteger(16) || elemTy.isInteger(32) || elemTy.isF16() || elemTy.isF32()))
            return emitOpError() << "expects A2/A3 tneg element type to be i16/i32/f16/f32";
        return success();
    };
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (!(elemTy.isInteger(8) || elemTy.isInteger(16) || elemTy.isInteger(32) || elemTy.isF16() || elemTy.isF32() ||
              elemTy.isBF16()))
            return emitOpError() << "expects A5 tneg element type to be i8/i16/i32/f16/f32/bf16";
        return success();
    };
...
mlir::LogicalResult mlir::pto::TOrOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        auto it = mlir::dyn_cast<IntegerType>(*elemOr);
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
            return emitOpError("expects A2/A3 tor src0, src1, and dst element type to be i8/i16/i32");
        return success();
    };
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        auto it = mlir::dyn_cast<IntegerType>(*elemOr);
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
            return emitOpError("expects A5 tor src0, src1, and dst element type to be i8/i16/i32");
        return success();
    };
...
mlir::LogicalResult mlir::pto::TOrSOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        auto it = mlir::dyn_cast<IntegerType>(*elemOr);
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16))
            return emitOpError("expects A2/A3 tors src and dst element type to be i8/i16");
        return success();
    };
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        auto it = mlir::dyn_cast<IntegerType>(*elemOr);
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
            return emitOpError("expects A5 tors src and dst element type to be i8/i16/i32");
        return success();
    };
...
static FailureOr<Type> verifyPTOShapedBinarySameElemAndShape(Operation* op, Type src0Ty, Type src1Ty, Type dstTy)
{
    if (!isPTOShapedLike(src0Ty) || !isPTOShapedLike(src1Ty) || !isPTOShapedLike(dstTy))
        return op->emitOpError("expects src0/src1/dst to be tensor/tile_buf/tile_view types"), failure();
...
mlir::LogicalResult mlir::pto::TPartAddOp::verify()
{
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (getElemTy(src0Ty) != getElemTy(src1Ty) || getElemTy(src0Ty) != getElemTy(dstTy))
            return emitOpError() << "expects src0/src1/dst to have the same element type";
        Type elem = getElemTy(src0Ty);
        if (!(elem.isInteger(32) || elem.isInteger(16) || elem.isInteger(8) || elem.isF16() || elem.isBF16() ||
              elem.isF32()))
...
mlir::LogicalResult mlir::pto::TPartMaxOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        Type t1 = getSrc1().getType();
        Type td = getDst().getType();
        FailureOr<Type> elemOr = verifyPTOShapedBinarySameElemAndShape(getOperation(), t0, t1, td);
        if (failed(elemOr))
            return failure();
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (failed(elemOr))
            return failure();
        Type e0 = *elemOr;
        if (!(e0.isInteger(32) || e0.isInteger(16) || e0.isInteger(8) || e0.isF16() || e0.isBF16() || e0.isF32()))
...
mlir::LogicalResult mlir::pto::TPartMulOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        Type elem = getElemTy(src0Ty);
        if (!(elem.isInteger(32) || elem.isInteger(16) || elem.isF16() || elem.isF32()))
            return emitOpError("expects A2/A3 tpartmul element type to be i32/i16/f16/f32");
        return mlir::success();
    };
    auto verifyA5 = [&]() -> LogicalResult {
        Type src0Ty = getSrc0().getType();
        ...
        if (!(elem.isInteger(32) || elem.isInteger(16) || elem.isInteger(8) || elem.isF16() || elem.isBF16() ||
              elem.isF32()))
            return emitOpError("expects A5 tpartmul element type to be i32/i16/i8/f16/bf16/f32");
        auto s0 = getShapeVec(src0Ty);
        auto s1 = getShapeVec(src1Ty);
        auto d = getShapeVec(dstTy);
        if (s0.size() != 2 || s1.size() != 2 || d.size() != 2)
...
mlir::LogicalResult mlir::pto::TPReluOp::verify()
{
    auto verifyCommon = [&]() -> FailureOr<std::tuple<Type, Type, Type, Type>> {
        ...
        Type td = getDst().getType();
        if (failed(verifyTileBufCommon(*this, t0, "src0")) || failed(verifyTileBufCommon(*this, t1, "src1")) ||
            failed(verifyTileBufCommon(*this, tt, "tmp")) || failed(verifyTileBufCommon(*this, td, "dst")))
            return failure();
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (dstValid[0] != ShapedType::kDynamic && tmpShape[0] != ShapedType::kDynamic && tmpShape[0] < dstValid[0] + 1)
            return emitOpError() << "expects A2/A3 tmp shape[0] to be at least dst valid_shape[0] + 1 ("
                                 << (dstValid[0] + 1) << ")";
        if (dstValid[1] != ShapedType::kDynamic && tmpValid[1] != ShapedType::kDynamic) {
            ...
            if (tmpValid[1] < packedMaskCols)
                return emitOpError() << "expects A2/A3 tmp valid_shape[1] to be at least ceil(dst valid_shape[1] / 8) ("
                                     << packedMaskCols << ")";
        }
        if (auto arch = getVerifierArchName(getOperation()); arch && arch->equals_insensitive("a3")) {
            if (getSrc0() == getSrc1() || getSrc0() == getTmp() || getSrc0() == getDst() || getSrc1() == getTmp() ||
                getSrc1() == getDst() || getTmp() == getDst())
                return emitOpError("expects A3 src0, src1, tmp, and dst to use different storage");
        }
        return success();
    };
...
ParseResult mlir::pto::TQuantOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parser.parseRParen())
        return failure();
    if (failed(parsePTOInherentAttrs<TQuantOp>(parser, result, parsedAttrs, {"quant_type", "operandSegmentSizes"})))
        return failure();

    if (parser.resolveOperand(src, srcTy, result.operands) || parser.resolveOperand(fp, fpTy, result.operands))
        return failure();
    if (hasOffset) {
    ...
    llvm::copy(ArrayRef<int32_t>({1, 1, hasOffset ? 1 : 0, hasTmp ? 1 : 0, 1}), properties.operandSegmentSizes.begin());
    return success();
}
...
void mlir::pto::TQuantOp::print(OpAsmPrinter& p)
{
    ...
    if (auto offset = getOffset()) {
        p << ", " << offset << " : " << getSrc().getType() << ", " << getFp().getType() << ", " << offset.getType()
          << ")";
    } else {
    ...
    p << " outs(" << getDst();
    if (auto tmp = getTmp()) {
        p << ", " << tmp << " : " << getDst().getType() << ", " << tmp.getType() << ")";
    } else {
        p << " : " << getDst().getType() << ")";
    }
    NamedAttrList attrs = getNonInherentAttrs(getOperation(), {"quant_type", "operandSegmentSizes"});
    attrs.append("quant_type", getQuantTypeAttr());
    p.printOptionalAttrDict(attrs.getAttrs());
}

ParseResult mlir::pto::TQuantMxOp::parse(OpAsmParser& parser, OperationState& result)
{
    OpAsmParser::UnresolvedOperand src;
    Type srcTy;
    ...
    if (parser.parseKeyword("ins") || parser.parseLParen() || parser.parseOperand(src) ||
        parser.parseColonType(srcTy) || parser.parseRParen())
        return failure();
    ...
    if (outOperands.size() != outTypes.size())
        return parser.emitError(
            parser.getCurrentLocation(), "expects the number of outs operands to match the number of outs types");
    if (outOperands.size() != 4 && outOperands.size() != 5)
        return parser.emitError(parser.getCurrentLocation(), "expects 4 or 5 operands in outs(...)");

    if (parser.parseOptionalAttrDict(result.attributes))
        return failure();

    if (!llvm::isa_and_nonnull<pto::QuantTypeAttr>(result.attributes.get("quant_type")))
        return parser.emitError(parser.getCurrentLocation(), "expects quant_type attribute");
...
{
    p << " ins(" << getSrc() << " : " << getSrc().getType() << ")";
    p << " outs(" << getDst() << ", " << getExp() << ", " << getMax() << ", " << getScaling();
    if (auto expZz = getExpZz())
        p << ", " << expZz;
    p << " : " << getDst().getType() << ", " << getExp().getType() << ", " << getMax().getType() << ", "
      << getScaling().getType();
    if (auto expZz = getExpZz())
        p << ", " << expZz.getType();
    p << ")";
...
mlir::LogicalResult mlir::pto::TQuantOp::verify()
{
    auto verifyStructural = [&]() -> LogicalResult {
        ...
        if (getQuantType() == mlir::pto::QuantType::INT8_SYM) {
            if (!getFp())
                return emitOpError() << "INT8_SYM quantization requires an fp operand";
            if (getOffset())
                return emitOpError() << "INT8_SYM quantization must not have an offset operand";
            if (!dstIntTy || dstIntTy.getWidth() != 8)
                return emitOpError() << "expects dst element type i8/ui8 for INT8_SYM quantization";
        } else if (getQuantType() == mlir::pto::QuantType::INT8_ASYM) {
            if (!getFp())
                return emitOpError() << "INT8_ASYM quantization requires an fp operand";
            if (!getOffset())
                return emitOpError() << "INT8_ASYM quantization requires an offset operand";
            if (!dstIntTy || dstIntTy.getWidth() != 8)
                return emitOpError() << "expects dst element type i8/ui8 for INT8_ASYM quantization";
        } else {
            return emitOpError(
                "expects plain tquant quant_type to be INT8_SYM or INT8_ASYM; use tquant.mx for MX quantization");
        }
        return success();
    };
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        auto verifyA2A3Param = [&](Type paramTy, StringRef paramName) -> LogicalResult {
            if (isRowMajorTileBuf(paramTy))
                return emitOpError() << "expects A2/A3 " << paramName << " to use non-row-major layout";
            auto paramValid = getValidShapeVec(paramTy);
            auto dstValid = getValidShapeVec(dstTy);
            if (paramValid.size() != 2 || dstValid.size() != 2)
                return emitOpError() << "expects A2/A3 " << paramName << " and dst to have rank-2 valid_shape";
            if (paramValid[0] != ShapedType::kDynamic && dstValid[0] != ShapedType::kDynamic &&
                paramValid[0] != dstValid[0])
                return emitOpError() << "expects A2/A3 " << paramName << " valid_shape[0] to equal dst valid_shape[0]";
            if (paramValid[1] != ShapedType::kDynamic && paramValid[1] != 1)
                return emitOpError() << "expects A2/A3 " << paramName << " valid_shape[1] to be 1";
            return success();
        };
    ...
    auto verifyA5 = [&]() -> LogicalResult { return verifyInt8Common(); };
...
{
    auto verifyA2A3 = [&]() -> LogicalResult { return emitOpError("tquant.mx is only supported on A5"); };
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (failed(verifyNDStyleVecTile(*this, srcTy, "src")) ||
            ...
            failed(verifyNDStyleVecTile(*this, expTy, "exp")) || failed(verifyNDStyleVecTile(*this, maxTy, "max")) ||
            failed(verifyNDStyleVecTile(*this, scalingTy, "scaling")))
            return failure();
        if (getExpZz() && failed(verifyNDStyleVecTile(*this, getExpZz().getType(), "exp_zz")))
            return failure();

        auto quantType = getQuantType();
        if (quantType != mlir::pto::QuantType::MXFP8 && quantType != mlir::pto::QuantType::MXFP4_E2M1)
        ...
        if (getStoreMode() && getQuantScaleAlg() != mlir::pto::QuantScaleAlg::OCP)
        ...
        if (srcValid.size() != 2 || dstValid.size() != 2 || expValid.size() != 2 || maxValid.size() != 2 ||
            scalingValid.size() != 2)
        ...
        if (srcRows != ShapedType::kDynamic && srcCols != ShapedType::kDynamic) {
            ...
            int64_t scalingElems = scalingValid[0] == ShapedType::kDynamic || scalingValid[1] == ShapedType::kDynamic ?
                                       ShapedType::kDynamic :
                                       scalingValid[0] * scalingValid[1];
            int64_t expZzElems = ShapedType::kDynamic;
            if (auto expZz = getExpZz()) {
                auto expZzValid = getValidShapeVec(expZz.getType());
                expZzElems = expZzValid[0] == ShapedType::kDynamic || expZzValid[1] == ShapedType::kDynamic ?
                                 ShapedType::kDynamic :
            ...
            if (scalingElems != ShapedType::kDynamic && scalingElems != groups)
                return emitOpError("expects scaling valid element count to equal src valid elements / 32");
            if (expZzElems != ShapedType::kDynamic && expZzElems != groups)
                return emitOpError("expects exp_zz valid element count to equal src valid elements / 32");
        }
        return success();
    };
...
mlir::LogicalResult mlir::pto::TReluOp::verify()
{
    ...
    auto verifyA5 = [&]() -> LogicalResult { return verifyByArch("expects A5 trelu element type to be i32/f16/f32"); };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}
...
mlir::LogicalResult mlir::pto::TRemOp::verify()
{
    ...
    Type tmpTy = getTmp().getType();
    Type dstTy = getDst().getType();
    if (failed(verifyTileBufCommon(*this, src0Ty, "src0")) || failed(verifyTileBufCommon(*this, src1Ty, "src1")) ||
        failed(verifyTileBufCommon(*this, tmpTy, "tmp")) || failed(verifyTileBufCommon(*this, dstTy, "dst")))
        return failure();
...
mlir::LogicalResult mlir::pto::TRemSOp::verify()
{
    ...
    Type td = getDst().getType();
    Type scalarTy = getScalar().getType();
    if (failed(verifyTileBufCommon(*this, ts, "src")) || failed(verifyTileBufCommon(*this, tt, "tmp")) ||
        failed(verifyTileBufCommon(*this, td, "dst")))
        return failure();
...
mlir::LogicalResult mlir::pto::TPowOp::verify()
{
    ...
    Type expTy = getExp().getType();
    Type dstTy = getDst().getType();
    if (failed(verifyTileBufCommon(*this, baseTy, "base")) || failed(verifyTileBufCommon(*this, expTy, "exp")) ||
        failed(verifyTileBufCommon(*this, dstTy, "dst")))
        return failure();
    if (failed(verifyTileBufSameElemType(*this, baseTy, expTy, "base", "exp")) ||
        ...
        return failure();
    if (!isRowMajorTileBuf(baseTy) || !isRowMajorTileBuf(expTy) || !isRowMajorTileBuf(dstTy))
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        if (getPrecisionType() == pto::PowPrecision::HighPrecision)
            return emitOpError("A2/A3 does not support precisionType=high_precision");
        if (!(isIntElem || elem.isF32()))
            return emitOpError("expects A2/A3 tpow element type to be i8/i16/i32 or f32");
        return success();
    };
    auto verifyA5 = [&]() -> LogicalResult {
        if (getPrecisionType() == pto::PowPrecision::HighPrecision) {
            ...
            if (!(isIntElem || elem.isF16() || elem.isF32()))
                return emitOpError("expects A5 tpow element type to be i8/i16/i32/f16/f32 "
    ...
    if (isFpElem && !getTmp())
        return emitOpError("expects tmp when element type is floating-point (required by the "
                           "floating-point pow lowering)");
    if (isIntElem && getTmp())
        return emitOpError("does not accept tmp when element type is integer (the integer pow "
...
mlir::LogicalResult mlir::pto::TPowSOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        if (getPrecisionType() == pto::PowPrecision::HighPrecision)
            return emitOpError("A2/A3 does not support precisionType=high_precision");
        if (!(isIntElem || elem.isF32()))
            return emitOpError("expects A2/A3 tpows element type to be i8/i16/i32 or f32");
        return success();
    };
    auto verifyA5 = [&]() -> LogicalResult {
        if (getPrecisionType() == pto::PowPrecision::HighPrecision) {
            ...
            if (!(isIntElem || elem.isF16() || elem.isF32()))
                return emitOpError("expects A5 tpows element type to be i8/i16/i32/f16/f32 "
                                   "when precisionType=default");
        }
        return success();
    };
    ...
    if (isFpElem && !getTmp())
        return emitOpError("expects tmp when element type is floating-point (required by the "
                           "floating-point pow lowering)");
    if (isIntElem && getTmp())
        return emitOpError("does not accept tmp when element type is integer (the integer pows "
...
mlir::LogicalResult mlir::pto::SetValidShapeOp::verify()
{
    ...
    auto checkDim = [&](Value operand, unsigned dimIdx, StringRef dimName) -> LogicalResult {
...
mlir::LogicalResult mlir::pto::TRowExpandOp::verify()
{
    auto verifyCommon = [&]() -> LogicalResult {
        Type srcTy = getSrc().getType();
        Type dstTy = getDst().getType();
        if (failed(verifyTileBufCommon(*this, srcTy, "src")) || failed(verifyNDStyleVecTile(*this, dstTy, "dst")))
            return failure();
        auto srcSpace = getPTOMemorySpaceEnum(srcTy);
        if (!srcSpace || *srcSpace != pto::AddressSpace::VEC)
            return emitOpError("expects src to be in the vec address space");
...
ParseResult mlir::pto::TSort32Op::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    result.addAttribute("operandSegmentSizes", parser.getBuilder().getDenseI32ArrayAttr({1, 1, hasTmp ? 1 : 0, 1}));
    return success();
}
...
ParseResult mlir::pto::TPowOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parser.parseKeyword("ins") || parser.parseLParen() || parser.parseOperand(base) || parser.parseComma() ||
    ...
    if (parser.resolveOperand(base, baseTy, result.operands) || parser.resolveOperand(exp, expTy, result.operands) ||
        parser.resolveOperand(dst, dstTy, result.operands))
        return failure();
    if (hasTmp && parser.resolveOperand(tmp, tmpTy, result.operands))
        return failure();
...
ParseResult mlir::pto::TPowSOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parser.resolveOperand(src, srcTy, result.operands) ||
        parser.resolveOperand(scalar, scalarTy, result.operands) || parser.resolveOperand(dst, dstTy, result.operands))
        return failure();
    if (hasTmp && parser.resolveOperand(tmp, tmpTy, result.operands))
        return failure();
...
static ParseResult parseTRowExpandBinaryLikeOp(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parser.parseKeyword("ins") || parser.parseLParen() || parser.parseOperand(src0) || parser.parseComma() ||
...
ParseResult mlir::pto::TRowExpandAddOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseTRowExpandBinaryLikeOp(parser, result);
}
...
mlir::LogicalResult mlir::pto::TRowExpandDivOp::verify()
{
    auto verifyByArch = [&](PTOArch targetArch) -> LogicalResult {
        ...
        FailureOr<Type> elemOr = verifyTRowExpandBinaryCore(
            *this, src0Ty, src1Ty, dstTy, getTmp() ? getTmp().getType() : Type{}, static_cast<bool>(getTmp()));
        if (failed(elemOr))
            return failure();
        Type elem = *elemOr;
        bool supported = elem.isF16() || elem.isF32() ||
                         (targetArch == PTOArch::A5 && (elem.isInteger(8) || elem.isInteger(16) || elem.isInteger(32)));
        if (!supported) {
            if (targetArch == PTOArch::A5)
                return emitOpError("expects A5 trowexpanddiv element type to be i8/i16/i32/f16/f32");
            return emitOpError("expects element type to be f16 or f32");
        }
...
mlir::LogicalResult mlir::pto::TRowExpandMulOp::verify()
{
    auto verifyByArch = [&](PTOArch targetArch) -> LogicalResult {
        ...
        if (failed(elemOr))
            return failure();
        Type elem = *elemOr;
        bool supported = elem.isF16() || elem.isF32() || elem.isInteger(16) || elem.isInteger(32) ||
                         (targetArch == PTOArch::A5 && elem.isInteger(8));
        if (!supported) {
            if (targetArch == PTOArch::A5)
                return emitOpError("expects A5 trowexpandmul element type to be i8/i16/i32/f16/f32");
            return emitOpError("expects A2/A3 trowexpandmul element type to be i16/i32/f16/f32");
        }
        return mlir::success();
    };
    auto verifyA2A3 = [&]() -> LogicalResult { return verifyByArch(PTOArch::A3); };
...
mlir::LogicalResult mlir::pto::TRowExpandSubOp::verify()
{
    auto verifyByArch = [&](PTOArch targetArch) -> LogicalResult {
        ...
        if (!supported) {
            if (targetArch == PTOArch::A5)
                return emitOpError("expects A5 trowexpandsub element type to be i8/i16/i32/f16/f32");
            return emitOpError("expects A2/A3 trowexpandsub element type to be i16/i32/f16/f32");
        }
        return mlir::success();
    };
    auto verifyA2A3 = [&]() -> LogicalResult { return verifyByArch(PTOArch::A3); };
...
mlir::LogicalResult mlir::pto::TRowExpandAddOp::verify()
{
    auto verifyByArch = [&](PTOArch targetArch) -> LogicalResult {
        ...
        if (!supported) {
            if (targetArch == PTOArch::A5)
                return emitOpError("expects A5 trowexpandadd element type to be i8/i16/i32/f16/f32");
            return emitOpError("expects A2/A3 trowexpandadd element type to be i16/i32/f16/f32");
        }
        auto src1Valid = getValidShapeVec(src1Ty);
        auto dstValid = getValidShapeVec(dstTy);
    ...
    bool allowIntegerTypes)
{
    ...
    bool supported = elem.isF16() || elem.isF32() ||
                     (allowIntegerTypes &&
                      (elem.isInteger(16) || elem.isInteger(32) || (targetArch == PTOArch::A5 && elem.isInteger(8))));
    if (!supported) {
        if (!allowIntegerTypes)
            return op->emitOpError() << "expects " << opName << " element type to be f16 or f32";
        if (targetArch == PTOArch::A5)
    ...
    auto validShapeMatches = [](ArrayRef<int64_t> lhs, ArrayRef<int64_t> rhs) -> bool {
    ...
    auto checkBroadcastOperand = [&](Type operandTy, ArrayRef<int64_t> operandValid, StringRef operandName,
        ...
        if (operandIsRowMajor) {
            if (operandCol != ShapedType::kDynamic && operandCol != expectedCol) {
                return op->emitOpError() << "expects row-major " << operandName
    ...
    auto checkFullAndBroadcast = [&](Type fullTy, ArrayRef<int64_t> fullValid, StringRef fullName, Type broadcastTy,
                                     ArrayRef<int64_t> broadcastValid, StringRef broadcastName) -> LogicalResult {
        ...
        return checkBroadcastOperand(
            broadcastTy, broadcastValid, broadcastName,
            /*requireNonRowMajor=*/hasTmp && targetArch == PTOArch::A3);
    };

    // (A5 tmp-form invariant is checked earlier, before the empty-marker accept.)

    if (src0MatchesDst) {
        if (succeeded(checkFullAndBroadcast(src0Ty, src0Valid, "src0", src1Ty, src1Valid, "src1")))
            return success();
    }
    if (src1MatchesDst) {
        if (succeeded(checkFullAndBroadcast(src1Ty, src1Valid, "src1", src0Ty, src0Valid, "src0")))
            return success();
    }
...
mlir::LogicalResult mlir::pto::TRowExpandExpdifOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        return verifyTRowExpandReduceLikeOp(
            getOperation(), getSrc0().getType(), getSrc1().getType(), getDst().getType(),
            getTmp() ? getTmp().getType() : Type{}, (bool)getTmp(), PTOArch::A3, "trowexpandexpdif",
            /*allowIntegerTypes=*/false);
    };
    auto verifyA5 = [&]() -> LogicalResult {
        return verifyTRowExpandReduceLikeOp(
            getOperation(), getSrc0().getType(), getSrc1().getType(), getDst().getType(),
            getTmp() ? getTmp().getType() : Type{}, (bool)getTmp(), PTOArch::A5, "trowexpandexpdif",
            /*allowIntegerTypes=*/false);
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}
...
mlir::LogicalResult mlir::pto::TRowExpandMaxOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        return verifyTRowExpandReduceLikeOp(
            getOperation(), getSrc0().getType(), getSrc1().getType(), getDst().getType(),
            getTmp() ? getTmp().getType() : Type{}, (bool)getTmp(), PTOArch::A3, "trowexpandmax",
            /*allowIntegerTypes=*/true);
    };
    auto verifyA5 = [&]() -> LogicalResult {
        return verifyTRowExpandReduceLikeOp(
            getOperation(), getSrc0().getType(), getSrc1().getType(), getDst().getType(),
            getTmp() ? getTmp().getType() : Type{}, (bool)getTmp(), PTOArch::A5, "trowexpandmax",
            /*allowIntegerTypes=*/true);
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}
...
mlir::LogicalResult mlir::pto::TRowExpandMinOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        return verifyTRowExpandReduceLikeOp(
            getOperation(), getSrc0().getType(), getSrc1().getType(), getDst().getType(),
            getTmp() ? getTmp().getType() : Type{}, (bool)getTmp(), PTOArch::A3, "trowexpandmin",
            /*allowIntegerTypes=*/true);
    };
    auto verifyA5 = [&]() -> LogicalResult {
        return verifyTRowExpandReduceLikeOp(
            getOperation(), getSrc0().getType(), getSrc1().getType(), getDst().getType(),
            getTmp() ? getTmp().getType() : Type{}, (bool)getTmp(), PTOArch::A5, "trowexpandmin",
            /*allowIntegerTypes=*/true);
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}
...
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        return verifyTRowArgReductionOpA2A3(*this, getSrc().getType(), getTmp().getType(), getDst().getType());
    };
    auto verifyA5 = [&]() -> LogicalResult {
        return verifyTRowArgReductionOpA5(*this, getSrc().getType(), getTmp().getType(), getDst().getType());
    };
...
mlir::LogicalResult mlir::pto::TScatterOp::verify()
{
    ...
    auto verifyIndexedForm = [&]() -> LogicalResult {
        ...
        Type td = getDst().getType();
        if (failed(verifyVecTileCommon(*this, ts, "src")) || failed(verifyVecTileCommon(*this, ti, "indexes")) ||
    ...
    auto verifyMaskForm = [&]() -> LogicalResult {
        Type ts = getSrc().getType();
        Type td = getDst().getType();
        if (failed(verifyVecTileCommon(*this, ts, "src")) || failed(verifyVecTileCommon(*this, td, "dst")))
            return failure();
        ...
        if (axisVal == "row") {
            ...
            if (srcValid[1] != ShapedType::kDynamic && dstValid[1] != ShapedType::kDynamic &&
                ...
                return emitOpError(
                    "expects dst valid cols to equal src valid cols times the mask expansion factor for row direction");
        } else if (axisVal == "col") {
            if (srcValid[1] != ShapedType::kDynamic && dstValid[1] != ShapedType::kDynamic &&
                dstValid[1] != srcValid[1])
                return emitOpError("expects dst valid cols to equal src valid cols for col direction");
            if (srcValid[0] != ShapedType::kDynamic && dstValid[0] != ShapedType::kDynamic &&
                ...
                return emitOpError(
                    "expects dst valid rows to equal src valid rows times the mask expansion factor for col direction");
        } else {
            return emitOpError("Invalid axis value, expected \"row\" or \"col\"");
        }
...
mlir::LogicalResult mlir::pto::TSelOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (!ok)
            return emitOpError("expects A2/A3 tsel src0, src1, and dst element type to be i16/i32/f16/bf16/f32");
        return success();
    };
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (!ok)
            return emitOpError("expects A5 tsel src0, src1, and dst element type to be i8/i16/i32/f16/bf16/f32");
        return success();
    };
...
mlir::LogicalResult mlir::pto::TSelSOp::verify()
{
    ...
    auto verifyCommon = [&]() -> FailureOr<Type> {
        ...
        if (failed(verifyTileBufCommon(*this, tMask, "mask")) || failed(verifyTileBufCommon(*this, tSrc, "src")) ||
            failed(verifyTileBufCommon(*this, tTmp, "tmp")) || failed(verifyTileBufCommon(*this, tDst, "dst")))
            return failure();
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (auto it = mlir::dyn_cast<mlir::IntegerType>(elem))
            ok = (it.getWidth() == 16 || it.getWidth() == 32);
        if (!ok)
            return emitOpError("expects A2/A3 tsels src and dst element type to be i16, i32, f16, or f32");
        return success();
    };
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        if (auto it = mlir::dyn_cast<mlir::IntegerType>(elem))
            ok = (it.getWidth() == 8 || it.getWidth() == 16 || it.getWidth() == 32);
        if (!ok)
            return emitOpError("expects A5 tsels src and dst element type to be i8, i16, i32, f16, or f32");
        return success();
    };
...
mlir::LogicalResult mlir::pto::TStoreFPOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        if (srcValid[1] != ShapedType::kDynamic && (srcValid[1] < 0 || srcValid[1] > 4095))
            return emitOpError() << "expects src.valid_shape[1] to be in the range [0, 4095]";
        return mlir::success();
    };
    auto verifyA5 = [&]() -> LogicalResult {
        Type srcTy = getSrc().getType();
...
mlir::LogicalResult mlir::pto::TTransOp::verify()
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        Type tmpTy = getTmp().getType();
        Type dstTy = getDst().getType();
        if (failed(verifyTileBufCommon(*this, srcTy, "src")) || failed(verifyTileBufCommon(*this, tmpTy, "tmp")) ||
            failed(verifyTileBufCommon(*this, dstTy, "dst")))
            return failure();
        Type srcElem = getElemTy(srcTy);
...
mlir::LogicalResult mlir::pto::TXorOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        auto it = mlir::dyn_cast<IntegerType>(elem);
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
            return emitOpError("expects A2/A3 txor src0, src1, tmp, and dst element type to be i8/i16/i32");
        return success();
    };
...
mlir::LogicalResult mlir::pto::TXorSOp::verify()
{
    ...
    auto verifyA2A3 = [&]() -> LogicalResult {
        ...
        auto it = mlir::dyn_cast<IntegerType>(elem);
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16))
            return emitOpError("expects A2/A3 txors src and dst element type to be i8/i16");
        return success();
    };
    ...
    auto verifyA5 = [&]() -> LogicalResult {
        ...
        auto it = mlir::dyn_cast<IntegerType>(*elemOr);
        if (!it || (it.getWidth() != 8 && it.getWidth() != 16 && it.getWidth() != 32))
            return emitOpError("expects A5 txors src and dst element type to be i8/i16/i32");
        return success();
    };
...
ParseResult mlir::pto::TPrintOp::parse(OpAsmParser& parser, OperationState& result)
{
    OpAsmParser::UnresolvedOperand src;
    OpAsmParser::UnresolvedOperand tmp;
    Type srcTy, tmpTy;
    ...
    if (parser.parseRParen())
        return failure();
    if (failed(parsePTOInherentAttrs<TPrintOp>(parser, result, parsedAttrs, {"printFormat"})))
        return failure();
...
void mlir::pto::TPrintOp::print(OpAsmPrinter& p)
{
    ...
    if (Value tmp = getTPrintTmpIfPresent(*this)) {
        p << ", " << tmp << " : " << getSrc().getType() << ", " << tmp.getType();
    } else {
        p << " : " << getSrc().getType();
    }
    p << ")";
    NamedAttrList attrs = getNonInherentAttrs(getOperation(), {"printFormat"});
    if (auto printFormatAttr = dyn_cast_or_null<pto::PrintFormatAttr>(getProperties().printFormat))
...
mlir::LogicalResult mlir::pto::TPrintOp::verify()
{
    ...
    auto printFormatAttr = dyn_cast_or_null<pto::PrintFormatAttr>(getProperties().printFormat);
    ...
    if (auto tb = mlir::dyn_cast<mlir::pto::TileBufType>(srcType)) {
        ...
        if (!(elem.isF16() || elem.isF32() || elem.isInteger(8) || elem.isInteger(16) || elem.isInteger(32)))
        ...
        if (*space != pto::AddressSpace::VEC && *space != pto::AddressSpace::MAT && *space != pto::AddressSpace::ACC)
...
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        if (failed(verifyMatTileOperands(*this, getLhs().getType(), getRhs().getType(), getDst().getType())))
            return failure();
        if (failed(verifyMatmulTypeTriple(
                *this, getElemTy(getLhs().getType()), getElemTy(getRhs().getType()), getElemTy(getDst().getType()))))
            return failure();
        return verifyMatmulLike(*this, getLhs().getType(), getRhs().getType(), getDst().getType());
    };
    auto verifyA5 = [&]() -> LogicalResult {
        if (failed(verifyMatmulTypeTriple(
                *this, getElemTy(getLhs().getType()), getElemTy(getRhs().getType()), getElemTy(getDst().getType()))))
...
{
    auto verifyA2A3 = [&]() -> LogicalResult {
        if (failed(verifyGemvTileOperands(*this, getLhs().getType(), getRhs().getType(), getDst().getType())))
            return failure();
        if (failed(verifyMatmulTypeTriple(
                *this, getElemTy(getLhs().getType()), getElemTy(getRhs().getType()), getElemTy(getDst().getType()))))
            return failure();
...
void TileType::print(AsmPrinter& printer) const { printShapeAndElem(printer, getShape(), getElementType()); }
...
Type LocalArrayType::parse(AsmParser& parser)
{
    ...
    return LocalArrayType::getChecked(
        [&]() { return parser.emitError(parser.getNameLoc()); }, parser.getContext(), shape, elemTy);
}

void LocalArrayType::print(AsmPrinter& printer) const { printShapeAndElem(printer, getShape(), getElementType()); }
...
static bool isStructStorable(Type t) { return isStructScalar(t) || llvm::isa<StructType>(t); }

Type StructType::parse(AsmParser& parser)
{
    SmallVector<Type> fields;
    if (parser.parseCommaSeparatedList(AsmParser::Delimiter::LessGreater, [&]() -> ParseResult {
            Type t;
        ...
        return Type();
    return StructType::getChecked([&]() { return parser.emitError(parser.getNameLoc()); }, parser.getContext(), fields);
}
...
LogicalResult StructType::verify(llvm::function_ref<InFlightDiagnostic()> emitError, llvm::ArrayRef<Type> fieldTypes)
{
    if (fieldTypes.empty())
        return emitError() << "'!pto.struct' requires at least one field";
    for (auto [i, f] : llvm::enumerate(fieldTypes)) {
        if (!isStructStorable(f))
            return emitError() << "'!pto.struct' field " << i << " type " << f
...
ParseResult mlir::pto::SubViewOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (succeeded(parser.parseOptionalKeyword("valid"))) {
        ...
        if (parser.parseLSquare() || parser.parseOperand(vrow) || parser.parseComma() || parser.parseOperand(vcol) ||
            parser.parseRSquare())
            return failure();
    ...
    if (parser.resolveOperands(offsets, indexTy, result.operands))
        return failure();
    if (!valids.empty() && parser.resolveOperands(valids, indexTy, result.operands))
        return failure();

    int32_t hasValid = valids.empty() ? 0 : 1;
    result.addAttribute(
        "operandSegmentSizes",
        parser.getBuilder().getDenseI32ArrayAttr({1, static_cast<int32_t>(offsets.size()), hasValid, hasValid}));
    ...
    OpaqueProperties properties, RegionRange regions, SmallVectorImpl<Type>& inferredReturnTypes)
{
    ...
    if (attributes) {
        if (auto segAttr = attributes.getAs<DenseI32ArrayAttr>("operandSegmentSizes")) {
...
mlir::LogicalResult mlir::pto::SubViewOp::verify()
{
    ...
    if (hasValidRow != hasValidCol)
        return emitOpError("subview expects valid_row and valid_col to be both present or both absent");
    ...
    bool rowInferred =
        !getValidRow() && dstValid[0] != ShapedType::kDynamic && dstValid[0] >= 0 && dstValid[0] <= sizeR;
    bool colInferred =
        !getValidCol() && dstValid[1] != ShapedType::kDynamic && dstValid[1] >= 0 && dstValid[1] <= sizeC;
    ...
    bool boxed = false;
    int32_t bl = 0, sl = 0;
    if (failed(computeInnerShape(cfg, srcTy.getElementType(), innerRows, innerCols, boxed, bl, sl)))
    ...
    (void)bl;
    if (srcShape.size() != 2 || srcShape[0] == ShapedType::kDynamic || srcShape[1] == ShapedType::kDynamic) {
...
void TQuantMxOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    PTO_ADD_READ(getSrcMutable());
    PTO_ADD_WRITE(getDstMutable());
...
void TPowOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
void TPowSOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    PTO_ADD_READ(getSrcMutable());
    auto tmp = getTmpMutable();
    if (!tmp.empty())
...
void TRowExpandAddOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    PTO_ADD_READ(getSrc0Mutable());
    PTO_ADD_READ(getSrc1Mutable());
    auto tmp = getTmpMutable();
...
{
    return isInsideSectionCube(op) || isInsideSectionVector(op) || isInsideTileOpHelper(op) ||
...
static bool isOddSplit(int64_t split) { return split == 3 || split == 4; }
...
static LogicalResult verifyFrontendKernelKind(Operation* op, FunctionKernelKind expected, StringRef kernelName)
{
    ...
    if (isInsideSectionCube(op)) {
        if (expected == FunctionKernelKind::Cube)
            return success();
        return op->emitOpError("must be inside a ") << kernelName << " kernel function or section";
    }
    if (isInsideSectionVector(op)) {
    ...
    std::optional<FunctionKernelKind> kernelKind = getEnclosingFunctionKernelKind(op);
...
static ParseResult parseFrontendInitializePipeOp(OpAsmParser& parser, OperationState& result)
{
    NamedAttrList attrs;
    bool sawId = false;
    ...
    while (failed(parser.parseOptionalRBrace())) {
        ...
        if (keyword == "id") {
            if (sawId)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'id' clause");
            IntegerAttr idAttr;
            if (parser.parseAttribute(idAttr, parser.getBuilder().getI32Type(), "id", attrs))
                return failure();
            sawId = true;
        } else if (keyword == "dir_mask") {
            if (sawDirMask)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'dir_mask' clause");
            IntegerAttr dirMaskAttr;
            if (parser.parseAttribute(dirMaskAttr, parser.getBuilder().getI8Type(), "dir_mask", attrs))
                return failure();
            sawDirMask = true;
        } else if (keyword == "slot_size") {
            if (sawSlotSize)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'slot_size' clause");
            IntegerAttr slotSizeAttr;
            if (parser.parseAttribute(slotSizeAttr, parser.getBuilder().getI32Type(), "slot_size", attrs))
                return failure();
            sawSlotSize = true;
        } else if (keyword == "slot_num") {
            if (sawSlotNum)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'slot_num' clause");
            IntegerAttr slotNumAttr;
            if (parser.parseAttribute(slotNumAttr, parser.getBuilder().getI32Type(), "slot_num", attrs))
                return failure();
            sawSlotNum = true;
        } else if (keyword == "local_slot_num") {
            if (sawLocalSlotNum)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'local_slot_num' clause");
            IntegerAttr localSlotNumAttr;
            if (parser.parseAttribute(localSlotNumAttr, parser.getBuilder().getI32Type(), "local_slot_num", attrs))
                return failure();
            sawLocalSlotNum = true;
        } else if (keyword == "nosplit") {
            if (sawNoSplit)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'nosplit' clause");
            ...
            if (sawAccPushEpilogue)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'acc_push_epilogue' clause");
            AccPushEpilogueAttr accPushEpilogueAttr;
            if (parser.parseAttribute(accPushEpilogueAttr, "acc_push_epilogue", attrs))
                return failure();
    ...
    while (failed(parser.parseOptionalRParen())) {
        ...
        if (keyword == "gm_slot_buffer") {
            if (hasGmSlotBuffer)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'gm_slot_buffer' operand");
            if (parser.parseOperand(gmSlotBuffer) || parser.parseColonType(gmSlotBufferTy))
                return failure();
            hasGmSlotBuffer = true;
        } else if (keyword == "gm_slot_tensor") {
            if (hasGmSlotTensor)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'gm_slot_tensor' operand");
            if (parser.parseOperand(gmSlotTensor) || parser.parseColonType(gmSlotTensorTy))
                return failure();
            hasGmSlotTensor = true;
        } else if (keyword == "c2v_consumer_buf") {
            if (hasC2vConsumerBuf)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'c2v_consumer_buf' operand");
            if (parser.parseOperand(c2vConsumerBuf) || parser.parseColonType(c2vConsumerBufTy))
                return failure();
            hasC2vConsumerBuf = true;
        } else if (keyword == "v2c_consumer_buf") {
            if (hasV2cConsumerBuf)
                return parser.emitError(parser.getCurrentLocation(), "duplicate 'v2c_consumer_buf' operand");
            if (parser.parseOperand(v2cConsumerBuf) || parser.parseColonType(v2cConsumerBufTy))
                return failure();
    ...
    result.addAttribute(
        ...
        parser.getBuilder().getDenseI32ArrayAttr(
            {hasGmSlotBuffer ? 1 : 0, hasGmSlotTensor ? 1 : 0, hasC2vConsumerBuf ? 1 : 0, hasV2cConsumerBuf ? 1 : 0}));
    if (hasGmSlotBuffer && parser.resolveOperand(gmSlotBuffer, gmSlotBufferTy, result.operands))
        return failure();
    if (hasGmSlotTensor && parser.resolveOperand(gmSlotTensor, gmSlotTensorTy, result.operands))
        return failure();
    if (hasC2vConsumerBuf && parser.resolveOperand(c2vConsumerBuf, c2vConsumerBufTy, result.operands))
        return failure();
    if (hasV2cConsumerBuf && parser.resolveOperand(v2cConsumerBuf, v2cConsumerBufTy, result.operands))
        return failure();
    return success();
}
...
static void printFrontendInitializePipeOp(InitOpT op, OpAsmPrinter& p)
{
    ...
    p.printOptionalAttrDict(
        op->getAttrs(),
        /*elidedAttrs=*/{
            "id", "dir_mask", "slot_size", "slot_num", "local_slot_num", "acc_push_epilogue", "nosplit",
            "operandSegmentSizes"});
}
...
static LogicalResult verifyFrontendInitCommon(InitOpT op, FunctionKernelKind expected, StringRef kernelName)
{
    ...
    if (hasGlobalSlotTensor) {
        if (hasGmSlotBuffer) {
            return op.emitOpError("'gm_slot_tensor' cannot be combined with 'gm_slot_buffer'");
        }
        if ((hasC2vConsumerBuf || hasV2cConsumerBuf) && (dirMask != 1 || !hasC2vConsumerBuf || hasV2cConsumerBuf)) {
            return op.emitOpError("GM-backed tile pipe init currently supports only dir_mask = 1 "
        ...
        if (failed(verifyFrontendGlobalSlotTensor(op.getOperation(), op.getGmSlotTensor(), dirMask, op.getSlotSize())))
            return failure();
    }

    if (!hasC2vConsumerBuf && !hasV2cConsumerBuf) {
        return op.emitOpError("expects local pipe init to provide at least one consumer buffer "
                              "operand; use 'gm_slot_tensor' for globaltensor pipe entries");
    }
    if (dirMask == 1 && !hasC2vConsumerBuf) {
        return op.emitOpError("expects 'c2v_consumer_buf' when dir_mask is 1");
    }
    ...
    if (auto localSlotNumAttr = op.getLocalSlotNumAttr()) {
        if (arch == PTOArch::A5)
            return op.emitOpError("'local_slot_num' is only supported for a2/a3 frontend pipe lowering");
        ...
        if (localSlotNum > slotNum) {
            return op.emitOpError() << "expects 'local_slot_num' to be less than or equal to slot_num (" << slotNum
    ...
    if (auto accPushEpilogue = op.getAccPushEpilogueAttr()) {
        ...
        if (dirMask != 1) {
            return op.emitOpError("expects fixpipe pipe (with 'acc_push_epilogue') to have dir_mask = 1 (C2V only)");
        }

        // Rule 2: fixpipe must have nosplit = true
        if (!op.getNosplit()) {
            return op.emitOpError("expects fixpipe pipe (with 'acc_push_epilogue') to have nosplit = true");
        }
        ...
        if (hasC2vConsumerBuf) {
            ...
            if (!foundReserve) {
                return op.emitOpError("expects fixpipe pipe 'c2v_consumer_buf' to trace to reserve_buffer or "
        ...
        if (relu != pto::FixpipeRelu::NoRelu && relu != pto::FixpipeRelu::NormalRelu) {
            return op.emitOpError("expects 'acc_push_epilogue.relu' to be 'no_relu' or 'normal_relu' in v1");
        }
        ...
        if (!validQuant) {
            return op.emitOpError("expects 'acc_push_epilogue.quant' to be one of the v1 allowed quantization modes");
        }

        // Rule 10: Check A5-only modes
        if (quant == pto::FixpipeQuant::QS322BF16PreScalar || quant == pto::FixpipeQuant::QS322BF16PreVec) {
            if (arch != PTOArch::A5) {
                return op.emitOpError("expects 'qs322bf16_pre_*' quantization modes to be used only on A5 target");
            }
        }
        if (quant == pto::FixpipeQuant::QF322HIF8PreScalar || quant == pto::FixpipeQuant::QF322FP8PreScalar) {
            if (arch != PTOArch::A5) {
                return op.emitOpError(
...
ParseResult AicInitializePipeOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseFrontendInitializePipeOp(parser, result);
}

void AicInitializePipeOp::print(OpAsmPrinter& p) { printFrontendInitializePipeOp(*this, p); }

ParseResult AivInitializePipeOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseFrontendInitializePipeOp(parser, result);
}

void AivInitializePipeOp::print(OpAsmPrinter& p) { printFrontendInitializePipeOp(*this, p); }

ReserveBufferOp mlir::pto::findReserveBufferByName(func::FuncOp funcOp, StringRef name)
{
...
constexpr llvm::StringLiteral kPipePeerOwnerFuncAttrName = "__pto.peer_owner_func";
constexpr llvm::StringLiteral kPipePeerReserveNameAttrName = "__pto.peer_reserve_name";
constexpr llvm::StringLiteral kPipePeerDirMaskAttrName = "__pto.peer_dir_mask";

struct FixpipeQuantStateResource : public SideEffects::Resource::Base<FixpipeQuantStateResource> {
...
static bool matchesLoweredFixpipePeerContract(
    Operation* initOp, func::FuncOp expectedOwnerFunc, StringRef expectedReserveName)
{
    if (!initOp || !isa<InitializeL2LPipeOp, InitializeL2G2LPipeOp>(initOp))
        return false;

    auto ownerAttr = initOp->getAttrOfType<FlatSymbolRefAttr>(kPipePeerOwnerFuncAttrName);
    auto reserveAttr = initOp->getAttrOfType<StringAttr>(kPipePeerReserveNameAttrName);
    auto dirMaskAttr = initOp->getAttrOfType<IntegerAttr>(kPipePeerDirMaskAttrName);
    if (!ownerAttr || !reserveAttr || !dirMaskAttr)
        return false;

    if (ownerAttr.getValue() != expectedOwnerFunc.getSymName() || reserveAttr.getValue() != expectedReserveName ||
...
static FailureOr<Operation*> lookupFrontendOrLoweredInitOpById(Operation* op, func::FuncOp funcOp, int32_t id);

static FailureOr<Operation*> lookupFixpipePeerConsumerInit(AicInitializePipeOp producerInit)
{
    if (!producerInit.getC2vConsumerBuf())
        return failure();
    auto importOp = dyn_cast_or_null<ImportReservedBufferOp>(producerInit.getC2vConsumerBuf().getDefiningOp());
    if (!importOp)
        return failure();

    auto peerConsumerFunc = lookupPeerFuncAcrossContainer(importOp.getOperation(), importOp.getPeerFuncAttr());
    if (!peerConsumerFunc)
        return failure();
    ...
    peerConsumerFunc.walk([&](Operation* candidate) {
        if (auto aivInit = dyn_cast<AivInitializePipeOp>(candidate)) {
            if (!aivInit.getC2vConsumerBuf())
                return WalkResult::advance();
            auto reserveOp = dyn_cast_or_null<ReserveBufferOp>(aivInit.getC2vConsumerBuf().getDefiningOp());
        ...
        if (!matchesLoweredFixpipePeerContract(candidate, peerConsumerFunc, bufferName))
            return WalkResult::advance();
...
static FailureOr<Operation*> lookupFixpipePeerProducerInit(
    AivInitializePipeOp consumerInit, func::FuncOp peerProducerFunc, StringRef bufferName,
    func::FuncOp currentConsumerFunc)
{
    ...
    peerProducerFunc.walk([&](Operation* candidate) {
        if (auto aicInit = dyn_cast<AicInitializePipeOp>(candidate)) {
            ...
            auto importOp = dyn_cast_or_null<ImportReservedBufferOp>(aicInit.getC2vConsumerBuf().getDefiningOp());
            if (!importOp)
                return WalkResult::advance();
        ...
        if (!matchesLoweredFixpipePeerContract(candidate, currentConsumerFunc, bufferName))
            return WalkResult::advance();
    ...
    if (matchedInitCount != 1) {
        consumerInit.emitOpError() << "expects peer producer function to contain a matching "
...
static FailureOr<Operation*> lookupFrontendOrLoweredInitOpById(Operation* op, func::FuncOp funcOp, int32_t id)
{
    ...
    funcOp.walk([&](Operation* candidate) {
        if (!isa<InitializeL2LPipeOp, InitializeL2G2LPipeOp>(candidate))
            return WalkResult::advance();
        auto frontendIdAttr = candidate->getAttrOfType<IntegerAttr>(kFrontendPipeIdAttrName);
...
static LogicalResult verifyFrontendSplitOp(
    Operation* op, FunctionKernelKind expected, StringRef kernelName, int32_t id, int64_t split, bool expectC2V)
{
    ...
    if (!isOddSplit(split))
        return success();
    if (!expectC2V) {
        return op->emitOpError("supports odd split modes (split = 3 or 4) only for C2V "
    ...
    auto supportsOddC2V = [&](auto init) {
        PTOArch arch = getTargetArch(op);
        bool hasSupportedGmBacking = static_cast<bool>(init.getGmSlotTensor()) ||
                                     (arch != PTOArch::A5 && static_cast<bool>(init.getGmSlotBuffer()));
        return init.getDirMask() == 1 && hasSupportedGmBacking && static_cast<bool>(init.getC2vConsumerBuf());
    };
    ...
    if (!supported) {
        return op->emitOpError("supports odd split modes (split = 3 or 4) only for a "
...
static LogicalResult verifyOddSplitTileEntry(Operation* op, int64_t split, Type entryTy)
{
    if (isOddSplit(split) && isa<TensorViewType>(entryTy)) {
        return op->emitOpError("supports odd split modes (split = 3 or 4) only for tile entries; "
                               "the pinned pto-isa does not implement odd GlobalTensor offsets");
    }
    return success();
}

static LogicalResult verifyC2VProducerSplitParity(Operation* op, int64_t split, Type entryTy)
{
    if (split == 0)
        return success();
...
static LogicalResult verifyAivSubblockIdOperand(Operation* op, Value aivSubblockId, int64_t split, Type pipeEntryType)
{
    if (!aivSubblockId)
        return success();

    if (split == 0) {
        return op->emitOpError("expects 'aiv_subblockid' only when 'split' is 1, 2, 3, or 4");
    }

    if (isa<TensorViewType>(pipeEntryType)) {
        return op->emitOpError("does not support 'aiv_subblockid' for !pto.tensor_view pipe entries");
    }

    auto addrSpace = getPTOMemorySpaceEnum(pipeEntryType);
    if (!addrSpace || *addrSpace != AddressSpace::VEC) {
        return op->emitOpError("expects 'aiv_subblockid' only on AIV-side vector tile pipe entries");
    }
...
{
    if (failed(verifyFrontendSplitOp(op.getOperation(), expected, kernelName, op.getId(), op.getSplit(), expectC2V)))
        return failure();
    if (failed(verifyFrontendDataOpDirection(op.getOperation(), op.getId(), expectC2V)))
        return failure();
    if (failed(verifyOddSplitTileEntry(op.getOperation(), op.getSplit(), op.getTile().getType())))
        return failure();
    ...
    if (isOddSplit(op.getSplit()) && !hasValidRow && !isa<TensorViewType>(op.getTile().getType())) {
        return op.emitOpError("expects odd C2V split tpop to provide per-sub-core valid_row and "
                              "valid_col operands");
    }
    if (!hasValidRow)
        return success();
...
static bool matchesFixpipeConsumerLayout(FixpipeLayout layout, TileBufType tileTy)
{
...
static bool matchesFixpipeConsumerElementType(FixpipeQuant quant, Type resultElemType)
{
    switch (quant) {
...
static bool isFixpipeQuantPayloadElemType(Type elemTy, PTOArch arch)
{
    if (!elemTy)
        return false;
    if (arch == PTOArch::A3)
        return elemTy.isUnsignedInteger(64) || elemTy.isSignlessInteger(64) || elemTy.isSignedInteger(64);
    return elemTy.isF16() || elemTy.isBF16() || elemTy.isF32();
}

static bool matchesFixpipeProducerAndConsumerTypes(FixpipeQuant quant, Type srcElemType, Type dstElemType)
{
    switch (quant) {
        case FixpipeQuant::NoConvert:
            return (srcElemType.isF32() || srcElemType.isInteger(32)) && srcElemType == dstElemType;
        case FixpipeQuant::F32F16:
...
static LogicalResult verifyNoUnpublishedFixpipeFrontendAttrs(Operation* op)
{
    for (NamedAttribute attr : op->getAttrs()) {
        ...
        if (!isUnpublishedFixpipeFrontendAttrName(name))
            continue;
        return op->emitOpError() << "does not allow unpublished fixpipe attr '" << name
...
static LogicalResult verifyFixpipeConsumerType(Operation* tpopOp, int32_t id, Type resultTileType)
{
    ...
    if (!tileTy) {
        return tpopOp->emitOpError("expects fixpipe TPOP result to be a tile type");
    }
    ...
    funcOp.walk([&](TPopFromAicOp otherPop) {
        if (otherPop.getOperation() == tpopOp || otherPop.getId() != static_cast<uint32_t>(id))
            return WalkResult::advance();
        if (otherPop.getTile().getType() != resultTileType) {
            mismatchedPeerType = true;
            tpopOp->emitOpError() << "expects all tpop_from_aic results for fixpipe pipe id = " << id
    ...
    if (!matchesFixpipeConsumerElementType(quant, resultElemType)) {
        return tpopOp->emitOpError() << "expects consumer element type to match acc_push_epilogue.quant "
                                     << stringifyFixpipeQuant(quant);
    }

    if (!matchesFixpipeConsumerLayout(accPushEpilogue.getLayout(), tileTy)) {
        return tpopOp->emitOpError() << "expects consumer tile layout to match acc_push_epilogue.layout "
...
static LogicalResult verifyInternalOddSplitSupport(Operation* op, Value pipeHandle, int64_t split, bool producerSide)
{
    if (!isOddSplit(split))
        return success();

    bool isC2VSide = producerSide ? isInsideCubeKernelOrSection(op) : isInsideVectorKernelOrSection(op);
    auto initOp = pipeHandle.getDefiningOp<InitializeL2G2LPipeOp>();
    if (!isC2VSide || !initOp || initOp.getDirMask() != 1 || !initOp.getLocalAddr()) {
        return op->emitOpError("supports odd split modes (split = 3 or 4) only for a "
...
static LogicalResult verifyTensorEntryMatchesInternalPipeInit(Operation* op, Value pipeHandle, Type entryTy)
{
    ...
    auto initOp = pipeHandle.getDefiningOp<InitializeL2G2LPipeOp>();
    if (!initOp) {
        return op->emitOpError() << "expects !pto.tensor_view pipe entry to use a pipe produced by "
                                    "pto.initialize_l2g2l_pipe";
    }
    if (initOp.getLocalAddr()) {
        return op->emitOpError() << "expects !pto.tensor_view pipe entry to use global-only "
...
{
    if (failed(verifyCommGlobalLike(*this, getDst(), "dst", CommGlobalShapePolicy::AllowDynamicPartitionView)) ||
        failed(verifyCommGlobalLike(*this, getSrc(), "src", CommGlobalShapePolicy::AllowDynamicPartitionView)) ||
    ...
    if (getShapeVec(getDst().getType()) != getShapeVec(getSrc().getType()))
        return emitOpError("expects src and dst to have the same static/dynamic shape signature");
...
static LogicalResult verifySyncAllGmWorkspace(Operation* op, Value workspace, StringRef name)
{
    ...
    if (auto ptrTy = dyn_cast<pto::PtrType>(ty)) {
        ...
        return op->emitOpError() << "expects " << name << " to be a GM ptr/tensor_view/partition_view";
    }
    ...
    if (!llvm::is_contained(shape, ShapedType::kDynamic)) {
        ...
        if (staticCapacity < kMinWorkspaceElements)
            return op->emitOpError() << "expects " << name << " to contain at least " << kMinWorkspaceElements
                                     << " i32 elements (64 bytes), but static capacity is " << staticCapacity;
    }
...
LogicalResult SyncAllOp::verify()
{
    ...
    if (mode == pto::SyncAllMode::Hard) {
        if (hasGm || getUsedCores())
            return emitOpError("expects hard syncall to have no gm_workspace or used_cores");
        return success();
    }
...
LogicalResult AicInitializePipeOp::verify()
{
    ...
    if (failed(peerConsumerInitOr)) {
        return emitOpError() << "expects peer consumer function to contain a matching "
    ...
    if (!peerAccPushEpilogue) {
        return emitOpError() << "expects peer consumer pipe init to also have "
                                "'acc_push_epilogue' for fixpipe contract consistency";
    }

    if (peerAccPushEpilogue.getLayout() != accPushEpilogue.getLayout()) {
        return emitOpError() << "expects acc_push_epilogue.layout to match peer consumer "
    ...
    if (peerAccPushEpilogue.getQuant() != accPushEpilogue.getQuant()) {
        return emitOpError() << "expects acc_push_epilogue.quant to match peer consumer "
    ...
    if (peerAccPushEpilogue.getRelu() != accPushEpilogue.getRelu()) {
        return emitOpError() << "expects acc_push_epilogue.relu to match peer consumer "
                             << "(producer has " << stringifyFixpipeRelu(accPushEpilogue.getRelu()) << ", consumer has "
                             << stringifyFixpipeRelu(peerAccPushEpilogue.getRelu()) << ")";
    }
...
LogicalResult AivInitializePipeOp::verify()
{
    ...
    if (accPushEpilogue) {
        ...
        if (!getC2vConsumerBuf()) {
            return emitOpError("expects fixpipe consumer pipe to have 'c2v_consumer_buf'");
        }
        ...
        if (auto importOp = dyn_cast_or_null<ImportReservedBufferOp>(bufDefOp)) {
            return emitOpError("expects consumer-side fixpipe pipe to use reserve_buffer, not import_reserved_buffer");
            ...
            moduleOp.walk([&](ImportReservedBufferOp candidateImport) {
                ...
                auto peerConsumerFunc =
                    lookupPeerFuncAcrossContainer(candidateImport.getOperation(), candidateImport.getPeerFuncAttr());
            ...
            if (matchedImportCount == 0) {
                return emitOpError() << "cannot find peer import_reserved_buffer for consumer buffer '" << bufferName
                                     << "'";
            }
            if (matchedImportCount > 1) {
                return emitOpError() << "finds multiple peer import_reserved_buffer ops for consumer buffer '"
                                     << bufferName << "'";
            }

            peerProducerFunc = matchedImport->getParentOfType<func::FuncOp>();
        } else {
            return emitOpError("expects fixpipe pipe 'c2v_consumer_buf' to trace to reserve_buffer or "
                               "import_reserved_buffer for peer contract verification");
        }
        ...
        auto peerProducerInitOr =
            lookupFixpipePeerProducerInit(*this, peerProducerFunc, bufferName, currentConsumerFunc);
        if (failed(peerProducerInitOr))
            return failure();

        Operation* peerProducerInitOp = *peerProducerInitOr;
        auto peerProducerFrontendInit = dyn_cast<AicInitializePipeOp>(peerProducerInitOp);
        std::optional<uint32_t> peerProducerId;
        if (peerProducerFrontendInit) {
            if (!peerProducerFrontendInit.getC2vConsumerBuf()) {
                return emitOpError() << "expects peer producer aic_initialize_pipe to have 'c2v_consumer_buf'";
            }

            Operation* peerBufDefOp = peerProducerFrontendInit.getC2vConsumerBuf().getDefiningOp();
            auto peerImportOp = dyn_cast_or_null<ImportReservedBufferOp>(peerBufDefOp);
            if (!peerImportOp) {
                return emitOpError() << "expects peer producer aic_initialize_pipe to use import_reserved_buffer "
                                     << "for c2v_consumer_buf";
            }

            auto peerConsumerFunc =
                lookupPeerFuncAcrossContainer(peerImportOp.getOperation(), peerImportOp.getPeerFuncAttr());
            if (peerImportOp.getName() != bufferName || peerConsumerFunc != currentConsumerFunc) {
                return emitOpError() << "cannot find matching producer aic_initialize_pipe for buffer '" << bufferName
                                     << "' in peer function";
            }
            peerProducerId = peerProducerFrontendInit.getId();
        } else if (isa<InitializeL2LPipeOp, InitializeL2G2LPipeOp>(peerProducerInitOp)) {
            auto frontendIdAttr = peerProducerInitOp->getAttrOfType<IntegerAttr>(kFrontendPipeIdAttrName);
            if (!frontendIdAttr) {
                return emitOpError() << "expects lowered peer producer fixpipe pipe to retain "
                                     << kFrontendPipeIdAttrName;
            }
            peerProducerId = static_cast<uint32_t>(frontendIdAttr.getInt());
        }
        if (!peerProducerId) {
            return emitOpError() << "expects peer producer fixpipe contract to resolve to "
        ...
        if (!peerAccPushEpilogue) {
            return emitOpError() << "expects peer producer pipe init to also have "
                                 << "'acc_push_epilogue' for fixpipe contract consistency";
        }

        // Verify layout/quant/relu match
        if (peerAccPushEpilogue.getLayout() != accPushEpilogue.getLayout()) {
            return emitOpError() << "expects acc_push_epilogue.layout to match peer producer "
        ...
        if (peerAccPushEpilogue.getQuant() != accPushEpilogue.getQuant()) {
            return emitOpError() << "expects acc_push_epilogue.quant to match peer producer "
        ...
        if (peerAccPushEpilogue.getRelu() != accPushEpilogue.getRelu()) {
            return emitOpError() << "expects acc_push_epilogue.relu to match peer producer "
        ...
        if (matchingPops.empty()) {
            return emitOpError() << "expects at least one tpop_from_aic for fixpipe pipe id = " << getId()
                                 << " to resolve the consumer entry type";
        }
        ...
        for (TPopFromAicOp pop : llvm::drop_begin(matchingPops)) {
            if (pop.getTile().getType() != resolvedConsumerTileType) {
                return emitOpError() << "expects all tpop_from_aic results for fixpipe pipe id = " << getId()
                                     << " to use the same tile type";
            }
        }

        auto resolvedConsumerTileTy = dyn_cast<pto::TileBufType>(resolvedConsumerTileType);
        if (!resolvedConsumerTileTy) {
            return emitOpError() << "expects fixpipe consumer tpop result to be !pto.tile_buf";
        }
        ...
        if (!matchesFixpipeConsumerElementType(quant, resolvedConsumerElemTy)) {
            return emitOpError() << "expects consumer element type to match acc_push_epilogue.quant "
                                 << stringifyFixpipeQuant(quant);
        }
        if (!matchesFixpipeConsumerLayout(accPushEpilogue.getLayout(), resolvedConsumerTileTy)) {
            return emitOpError() << "expects consumer tile layout to match acc_push_epilogue.layout "
                                 << stringifyFixpipeLayout(accPushEpilogue.getLayout());
        }
        ...
        if (auto requiredSlotBytes = getStaticTileByteSize(resolvedConsumerTileTy)) {
            if (static_cast<uint64_t>(getSlotSize()) < *requiredSlotBytes) {
                return emitOpError() << "expects consumer-side fixpipe slot_size to be at least " << *requiredSlotBytes
                                     << " bytes for the resolved post-fixpipe consumer entry";
            }
            auto peerProducerSlotSize = getInitSlotSize(peerProducerInitOp);
            if (peerProducerSlotSize && static_cast<uint64_t>(*peerProducerSlotSize) < *requiredSlotBytes) {
                return emitOpError() << "expects peer producer fixpipe slot_size to be at least " << *requiredSlotBytes
                                     << " bytes for the resolved post-fixpipe consumer entry";
            }
        }
        ...
        peerProducerFunc.walk([&](TPushToAivOp push) {
            ...
            if (!srcTileTy)
                return WalkResult::advance();
            if (!matchesFixpipeProducerAndConsumerTypes(quant, srcTileTy.getElementType(), resolvedConsumerElemTy)) {
                producerConsumerTypeMismatch = true;
                emitOpError() << "expects producer source element type and consumer tpop result "
...
LogicalResult TAllocToAivOp::verify()
{
    if (failed(verifyNoUnpublishedFixpipeFrontendAttrs(getOperation())))
        return failure();
    if (failed(verifyFrontendSplitOp(
            getOperation(), FunctionKernelKind::Cube, "cube", getId(), getSplit(),
            /*expectC2V=*/true)))
        return failure();
    ...
    if (failed(verifyOddSplitTileEntry(getOperation(), getSplit(), getEntry().getType())))
        return failure();
...
LogicalResult TAllocToAicOp::verify()
{
    if (failed(verifyNoUnpublishedFixpipeFrontendAttrs(getOperation())))
        return failure();
    if (failed(verifyFrontendSplitOp(
            getOperation(), FunctionKernelKind::Vector, "vector", getId(), getSplit(),
            /*expectC2V=*/false)))
        return failure();
    if (failed(verifyFrontendDataOpDirection(
            getOperation(), getId(),
...
LogicalResult TPushToAivOp::verify()
{
    ...
    if (failed(verifyFrontendDataOpDirection(
            ...
            /*expectC2V=*/true)))
        return failure();
    if (failed(verifyFrontendTensorEntryMatchesInit(getOperation(), getId(), getTile().getType())))
        return failure();
    if (failed(verifyOddSplitTileEntry(getOperation(), getSplit(), getTile().getType())))
        return failure();
    if (failed(verifyC2VProducerSplitParity(getOperation(), getSplit(), getTile().getType())))
        return failure();
    ...
    if (!tileTy) {
        return emitOpError("expects fixpipe TPUSH source tile to be a tile type");
    }
    ...
    if (getSplit() != 0) {
        return emitOpError("expects fixpipe TPUSH to have split = 0");
    }
    ...
    if (quant == pto::FixpipeQuant::NoConvert) {
        ...
        quant == pto::FixpipeQuant::F32F16 || quant == pto::FixpipeQuant::F32BF16 ||
        quant == pto::FixpipeQuant::QF322B8PreScalar || quant == pto::FixpipeQuant::QF322B8PreVec ||
        quant == pto::FixpipeQuant::QF322F16PreScalar || quant == pto::FixpipeQuant::QF322BF16PreScalar ||
        quant == pto::FixpipeQuant::QF322HIF8PreScalar || quant == pto::FixpipeQuant::QF322FP8PreScalar) {
        // f32-based quant modes
        if (!elemTy.isF32()) {
            srcTypeValid = false;
        }
    } else if (
        quant == pto::FixpipeQuant::REQ8Scalar || quant == pto::FixpipeQuant::REQ8Vec ||
        quant == pto::FixpipeQuant::DEQF16Scalar || quant == pto::FixpipeQuant::DEQF16Vec ||
        quant == pto::FixpipeQuant::QS322BF16PreScalar || quant == pto::FixpipeQuant::QS322BF16PreVec) {
    ...
    if (!srcTypeValid) {
        return emitOpError() << "expects fixpipe TPUSH source element type to match "
    ...
    if (isScalarQuant || isVectorQuant) {
        ...
        if (!foundQuantConfig) {
            if (isScalarQuant) {
                return emitOpError() << "expects a preceding pto.set_quant_scalar with id = " << getId()
                                     << " in the same block";
            }
            return emitOpError() << "expects a preceding pto.set_quant_vector with id = " << getId()
                                 << " in the same block";
        }
    }
...
LogicalResult TPushToAicOp::verify()
{
    ...
    if (failed(verifyFrontendTensorEntryMatchesInit(getOperation(), getId(), getTile().getType())))
        return failure();
    return verifyAivSubblockIdOperand(getOperation(), getAivSubblockid(), getSplit(), getTile().getType());
}
...
LogicalResult TPopFromAicOp::verify()
{
    if (failed(verifyNoUnpublishedFixpipeFrontendAttrs(getOperation())))
        return failure();
    if (failed(verifyFrontendPopOp(
            *this, FunctionKernelKind::Vector, "vector",
            /*expectC2V=*/true)))
        return failure();
    if (failed(verifyAivSubblockIdOperand(getOperation(), getAivSubblockid(), getSplit(), getTile().getType())))
        return failure();
...
LogicalResult TPopFromAivOp::verify()
{
    ...
    return verifyFrontendPopOp(
        *this, FunctionKernelKind::Cube, "cube",
        /*expectC2V=*/false);
}
...
LogicalResult TFreeFromAicOp::verify()
{
    ...
    if (getEntry() && failed(verifyOddSplitTileEntry(getOperation(), getSplit(), getEntry().getType())))
        return failure();
    if (getEntry())
        return verifyFrontendTensorEntryMatchesInit(getOperation(), getId(), getEntry().getType());
    return success();
...
LogicalResult SetQuantScalarOp::verify()
{
    ...
    auto initOr = lookupFrontendOrLoweredInitOpById(getOperation(), funcOp, getId());
    if (failed(initOr))
        return failure();
    ...
    if (auto aicInit = dyn_cast<AicInitializePipeOp>(initOp)) {
        ...
        return emitOpError() << "expects 'id' = " << getId()
                             << " to reference an aic_initialize_pipe or lowered producer pipe";
    }

    if (!accPushEpilogue) {
        return emitOpError() << "expects 'id' = " << getId() << " to reference a fixpipe pipe (with acc_push_epilogue)";
    }
...
LogicalResult SetQuantVectorOp::verify()
{
    ...
    if (!isFixpipeQuantPayloadElemType(scalingElemTy, arch)) {
        if (arch == PTOArch::A3)
            return emitOpError("expects 'scaling_tile' element type to be packed i64/ui64 on A3");
        return emitOpError("expects 'scaling_tile' element type to be f16, bf16, or f32 on A5");
    }
...
LogicalResult TPushOp::verify()
{
    ...
    if (failed(verifyAivSubblockIdOperand(getOperation(), getAivSubblockid(), getSplit(), getTile().getType())))
        return failure();
    if (failed(verifyTensorEntryMatchesInternalPipeInit(getOperation(), getPipeHandle(), getTile().getType())))
        return failure();
...
static func::FuncOp getParentFunc(Operation* op) { return op ? op->getParentOfType<func::FuncOp>() : func::FuncOp(); }
...
{
    std::optional<unsigned> registerCount = getSimtKeepResumeRegisterCount(getSimtKeepResumeValueType(op));
    if (!registerCount)
        return success();
    int64_t slot = op.getSlot();
    if (slot < 0 || slot >= kSimtKeepResumeSlotLimit)
        return op.emitOpError() << "requires slot in range [0, " << (kSimtKeepResumeSlotLimit - 1) << "]";
    if (*registerCount == 2) {
        if ((slot % 2) != 0)
            return op.emitOpError() << "requires an even slot for 64-bit keep/resume values";
        if (slot + 1 >= kSimtKeepResumeSlotLimit)
            return op.emitOpError() << "requires slot in range [0, " << (kSimtKeepResumeSlotLimit - 2)
...
template <typename OpT>
static bool overlapsEarlierSimtKeepResumeSlotUse(OpT op, SmallVectorImpl<int64_t>& used)
{
    std::optional<unsigned> registerCount = getSimtKeepResumeRegisterCount(getSimtKeepResumeValueType(op));
    if (!registerCount)
        return false;
...
static LogicalResult verifyUniqueResumeGroupSlots(ResumeOp current, Operation* first)
{
...
static LogicalResult verifyUniqueKeepGroupSlots(KeepOp current, Operation* first, Operation* last)
{
    SmallVector<int64_t, 4> slots;
    for (Operation* cur = first; cur; cur = cur->getNextNode()) {
...
static bool isInsideSimtExecutionScope(Operation* op)
{
    ...
    return (func && func->hasAttr(pto::kPTOSimtEntryAttrName)) || op->getParentOfType<pto::SectionSimtOp>();
}
...
LogicalResult SyncthreadsOp::verify() { return verifyInsideSimtExecutionScope(getOperation()); }

LogicalResult ThreadfenceOp::verify() { return verifyInsideSimtExecutionScope(getOperation()); }

LogicalResult ThreadfenceBlockOp::verify() { return verifyInsideSimtExecutionScope(getOperation()); }

void SyncthreadsOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
void ThreadfenceOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), SideEffects::DefaultResource::get());
    effects.emplace_back(MemoryEffects::Write::get(), SideEffects::DefaultResource::get());
}

void ThreadfenceBlockOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    effects.emplace_back(MemoryEffects::Read::get(), SideEffects::DefaultResource::get());
    effects.emplace_back(MemoryEffects::Write::get(), SideEffects::DefaultResource::get());
}
...
LogicalResult KeepOp::verify()
{
    ...
    if (!isSupportedSimtKeepResumeType(getPayload().getType()))
        return emitOpError() << "supports integer scalar payloads up to 64 bits and "
                                "f16/bf16/f32 payloads";
    if (failed(verifySimtKeepResumeSlotRange(*this)))
        return failure();

    Block* block = getOperation()->getBlock();
    bool insideSection = getOperation()->getParentOfType<SectionSimtOp>() != nullptr;
    ...
    if (!lastPayloadOp)
        return emitOpError("must be placed in the SIMT epilogue before func.return or the end "
                           "of pto.section.simt");
    ...
    if (!lastKeep || !isa<KeepOp>(lastKeep))
        return emitOpError() << "must be placed in the SIMT epilogue before func.return; only "
    ...
    if (!isOpInRange(getOperation(), firstKeep, lastKeep))
        return emitOpError() << "must be in the contiguous SIMT keep epilogue group immediately "
...
LogicalResult ResumeOp::verify()
{
    ...
    if (!isSupportedSimtKeepResumeType(getResult().getType()))
        return emitOpError() << "supports integer scalar results up to 64 bits and "
    ...
    if (!first || !isa<ResumeOp>(first))
        return emitOpError() << "must be in the contiguous SIMT resume prologue group after "
...
void TPushOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    ...
    if (auto pipeId = getFrontendPipeIdFromHandle(getPipeHandle())) {
        auto accPushEpilogue = getAccPushEpilogueFromInitOp(getPipeHandle().getDefiningOp());
        if (accPushEpilogue &&
            (isScalarFixpipeQuant(accPushEpilogue.getQuant()) || isVectorFixpipeQuant(accPushEpilogue.getQuant()))) {
            effects.emplace_back(
                MemoryEffects::Read::get(), getFixpipeQuantStateIdAttr(getOperation(), *pipeId),
                FixpipeQuantStateResource::get());
        }
    }
}

void TPushToAivOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    addEffect(effects, &getTileMutable(), MemoryEffects::Read::get());
    ...
    if (isScalarFixpipeQuant(quant) || isVectorFixpipeQuant(quant)) {
        effects.emplace_back(
            MemoryEffects::Read::get(), getFixpipeQuantStateIdAttr(getOperation(), getId()),
            FixpipeQuantStateResource::get());
    }
}
...
void SetQuantScalarOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
    addEffect(effects, &getScaleMutable(), MemoryEffects::Read::get());
    effects.emplace_back(
        MemoryEffects::Write::get(), getFixpipeQuantStateIdAttr(getOperation(), getId()),
        FixpipeQuantStateResource::get());
}

void SetQuantVectorOp::getEffects(SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects)
{
...
static ParseResult parseConvertRounding(OpAsmParser& parser, RoundingAttr& roundingAttr)
{
    StringRef roundingKeyword;
    if (parser.parseKeyword("round") || parser.parseLParen() || parser.parseKeyword(&roundingKeyword) ||
        parser.parseRParen())
        return failure();
    std::optional<Rounding> rounding = symbolizeRounding(roundingKeyword);
    if (!rounding)
        return parser.emitError(parser.getCurrentLocation())
               << "expected convert rounding to be one of " << kConvertRoundingKeywords;
    roundingAttr = RoundingAttr::get(parser.getContext(), *rounding);
    return success();
}

static void printConvertRounding(OpAsmPrinter& printer, Operation* op, RoundingAttr rounding)
{
    printer << "round(" << stringifyRounding(rounding.getValue()) << ")";
}

static ParseResult parseConvertSaturation(OpAsmParser& parser, SaturationAttr& saturationAttr)
{
    StringRef saturationKeyword;
    if (parser.parseKeyword(&saturationKeyword))
        return failure();
    std::optional<Saturation> saturation = symbolizeSaturation(saturationKeyword);
    if (!saturation)
        return parser.emitError(parser.getCurrentLocation()) << "expected convert saturation to be sat or nosat";
    saturationAttr = SaturationAttr::get(parser.getContext(), *saturation);
    return success();
}

static void printConvertSaturation(OpAsmPrinter& printer, Operation* op, SaturationAttr saturation)
{
    printer << stringifySaturation(saturation.getValue());
}

static ParseResult parseSignedness(OpAsmParser& parser, SignednessAttr& signedness)
{
    ...
    if (!parsed)
        return parser.emitError(parser.getCurrentLocation()) << "expected signedness to be signed or unsigned";
    signedness = SignednessAttr::get(parser.getContext(), *parsed);
    return success();
}

static void printSignedness(OpAsmPrinter& printer, Operation* op, SignednessAttr signedness)
{
    printer << stringifySignedness(signedness.getValue());
}

static OptionalParseResult parseOptionalSignedness(OpAsmParser& parser, SignednessAttr& signedness)
{
    ...
    if (succeeded(parser.parseOptionalKeyword("unsigned"))) {
        signedness = SignednessAttr::get(parser.getContext(), Signedness::Unsigned);
        return success();
    }
    return std::nullopt;
}

static void printOptionalSignedness(OpAsmPrinter& printer, Operation* op, SignednessAttr signedness)
{
    printer << stringifySignedness(signedness.getValue());
}

static constexpr const char kLdL2CacheKeywords[] = "nmfv/nmlv/nmprs/nmpref/nakeep/naclean/nadrop/idsfv/idslv/idsprs/"
                                                   "idspref/exfv/exlv/exprs/expref";

static constexpr const char kStL2CacheKeywords[] = "nmfv/nmlv/nmprs/nmred/naci/napw/napi/nared/wbhfv/wbhlv/wbhprs/"
...
static ParseResult parseL1Cache(OpAsmParser& parser, L1CacheAttr& l1cache)
{
    ...
    if (!parsed)
        return parser.emitError(parser.getCurrentLocation()) << "expected memory l1cache to be cache or uncache";
    l1cache = L1CacheAttr::get(parser.getContext(), *parsed);
    return success();
}

static void printL1Cache(OpAsmPrinter& printer, Operation* op, L1CacheAttr l1cache)
{
    if (!l1cache)
        return;
...
static ParseResult parseLdL2Cache(OpAsmParser& parser, LdL2CacheAttr& l2cache)
{
    ...
    if (!parsed)
        return parser.emitError(parser.getCurrentLocation())
               << "expected load L2 cache control to be one of " << kLdL2CacheKeywords;
    l2cache = LdL2CacheAttr::get(parser.getContext(), *parsed);
    return success();
}

static void printLdL2Cache(OpAsmPrinter& printer, Operation* op, LdL2CacheAttr l2cache)
{
    if (!l2cache)
        return;
...
static ParseResult parseStL2Cache(OpAsmParser& parser, StL2CacheAttr& l2cache)
{
    if (failed(parser.parseOptionalKeyword("l2cache"))) {
    ...
    if (!parsed)
        return parser.emitError(parser.getCurrentLocation())
               << "expected store L2 cache control to be one of " << kStL2CacheKeywords;
    l2cache = StL2CacheAttr::get(parser.getContext(), *parsed);
    return success();
}

static void printStL2Cache(OpAsmPrinter& printer, Operation* op, StL2CacheAttr l2cache)
{
    if (!l2cache)
        return;
...
static ParseResult parseStructPath(OpAsmParser& parser, DenseI64ArrayAttr& path)
{
...
static void printStructPath(OpAsmPrinter& printer, Operation* op, DenseI64ArrayAttr path)
{
    printer << "[";
