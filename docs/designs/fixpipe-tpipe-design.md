...
- 当前公共手册 `docs/PTO_IR_manual.md` 里仍有“`ui8` 按 signless `i8` 处理”的旧表述；
  若按本文落地 fixpipe 8-bit
  contract，则必须在后续同步更新该手册及相关 type-system 说明，明确这里依赖
  `si8`，以及在 scalar 8-bit 场景下依赖 `ui8`，作为 peer consumer `tpop`
  结果类型上的公开语义
...
- v1 必须公开
  ...
  这一前端 IR 形状
- 但 `vector quant payload` 的精确 shape / element type / layout 约束，不建议在
  当前阶段一次性写死成所有 target 共享的唯一公共 contract
- 更稳妥的做法是：公共 verifier 先只要求 `loc=scaling`，其余 payload shape/type
...
当前 PTOAS 代码库里还没有与 `SET_QUANT_SCALAR` / `SET_QUANT_VECTOR` 一一对应的
前端 pipe IR op 或 lowering 处理。因此如果第一版要真正支持这些 scalar/vector
quant family，除了 pipe-level `acc_push_epilogue` 外，还需要补一套显式的
producer-side quant-config 表达与 lowering。
...
def PTO_AccPushEpilogueAttr : AttrDef<PTO_Dialect, "AccPushEpilogue"> {
  ...
  let parameters = (ins
    EnumParameter<PTO_AccStoreModeEnum>:$layout,
    EnumParameter<PTO_AccStoreQuantPreModeEnum>:$quant,
    EnumParameter<PTO_ReluPreModeEnum>:$relu
  );
  let assemblyFormat =
...
这里可以直接复用现有 `PTO_AccStoreQuantPreModeEnum` / `PTO_ReluPreModeEnum`
作为字段底层类型；但 v1 verifier 必须主动收窄合法值域：
...
其余现存 enum 值在 v1 中都应 verifier reject，而不是默认为“语法合法但语义待定”。
...
为覆盖 `SET_QUANT_SCALAR` / `SET_QUANT_VECTOR`，建议在 PTOIR 中新增两类显式 op。
...
`pto.set_quant_vector` 同理不需要额外类型 attr，因为 `pto-isa`
`SET_QUANT_VECTOR(fpTile)` 当前只要求输入是 Scaling tile；公共前端 contract 不必在
这一层提前把 payload element type 固化成单一跨平台约束。
...
7. `acc_push_epilogue.relu` 在 v1 中只允许 `no_relu` / `normal_relu`；若使用
   `scalar_relu` / `vector_relu` / `pwl`，verifier 必须报错。
8. `acc_push_epilogue.quant` 在 v1 中只允许：
   ...
   以及未纳入 v1 的其它 `*_vec` / `*_scalar` 变体）在 v1 中都必须 verifier reject。
...
24. `pto.set_quant_vector` 的输入必须是 `loc=scaling` 的 tile；其大小、element
    type 与 layout 应满足目标平台 fixpipe vector quant payload 的参数布局要求。
    某些 target 当前可能以 `uint64_t` scaling tile 访问 packed scaling buffer，
    但这只应视为 target-specific lowering / payload 访问形式，不应上升为统一的
    前端公共语义类型约束。
25. 对 `pto.set_quant_vector`，第一版公共 verifier 至少检查 `loc=scaling`；其余
    payload shape / element type / layout 约束，可由 arch-specific verifier
    按目标平台现有 fixpipe vector quant 规则继续收紧。
...
- `DEQF16` 缺少对应 `pto.set_quant_scalar` 的 verifier 失败 case
- `REQ8` / `QF322B8_PRE` 缺少对应 `pto.set_quant_scalar` 的 verifier 失败 case
- `QF322F16_PRE` / `QF322BF16_PRE` / `QS322BF16_PRE` 缺少对应
  `pto.set_quant_scalar` 的 verifier 失败 case
- `QF322HIF8_PRE` / `QF322FP8_PRE` 缺少对应 `pto.set_quant_scalar` 的 verifier
  失败 case
- `VDEQF16` 缺少对应 `pto.set_quant_vector` 的 verifier 失败 case
- `VREQ8` / `VQF322B8_PRE` / `VQS322BF16_PRE` 缺少对应
  `pto.set_quant_vector` 的 verifier 失败 case
...
- 使用 `scalar_relu` / `vector_relu` / `pwl` 的 verifier 失败 case
...
- `qs322bf16_pre_scalar` / `qs322bf16_pre_vec` 在非 A5 target/profile 上的 verifier
  失败 case
- A5 UB fixpipe 命中 `SubBlockId != 0` / `SingleModeVec1` 语义的 reject case
...
- `pto.set_quant_vector` 由 arch-specific verifier 拒绝非法 payload shape/type 的
  case
