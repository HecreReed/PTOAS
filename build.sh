...
usage() {
  ...
  echo "    --tests Run samples and remote NPU validation after build"
  echo ""
}
...
checkopts() {
  ...
  parsed_args=$(getopt -a -o j:hvuO: -l help,pkg,build,tests,cann_3rd_lib_path: -- "$@") || {
  usage
  exit 1
  }
