...
int main()
{
    ...
    inputSize = bufSize;
    FILE_CHECK(ReadFile("./v1.bin", inputSize, lhsHost, bufSize) && inputSize == bufSize, "./v1.bin");
    inputSize = bufSize;
    FILE_CHECK(ReadFile("./v2.bin", inputSize, rhsHost, bufSize) && inputSize == bufSize, "./v2.bin");
    inputSize = bufSize;
    FILE_CHECK(ReadFile("./v3.bin", inputSize, outHost, bufSize) && inputSize == bufSize, "./v3.bin");

    ACL_CHECK(aclrtMemcpy(lhsDevice, bufSize, lhsHost, bufSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(rhsDevice, bufSize, rhsHost, bufSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outDevice, bufSize, outHost, bufSize, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchMixed_external_vadd_kernel(lhsDevice, rhsDevice, outDevice, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));

    ACL_CHECK(aclrtMemcpy(outHost, bufSize, outDevice, bufSize, ACL_MEMCPY_DEVICE_TO_HOST));
