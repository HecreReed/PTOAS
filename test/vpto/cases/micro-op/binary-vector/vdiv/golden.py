...
def gen_f32(out, rng):
    ...
    g = (np.divide(v1, v2, where=v2 != 0, out=np.full_like(v1, np.nan))).astype(
...
def gen_f16(out, rng):
    ...
    g = (
        (
            ...
            / np.maximum(np.abs(v2.astype(np.float32)), 1e-8)
            * np.sign(v2.astype(np.float32))
        ).astype(np.float16)
    ).astype(np.float16)
...
def gen_tail(out, rng):
    ...
    g.reshape(-1)[:LOGICAL_ELEMS] = (
        np.divide(
            ...
            v2.reshape(-1)[:LOGICAL_ELEMS],
            where=v2.reshape(-1)[:LOGICAL_ELEMS] != 0,
            out=np.full(LOGICAL_ELEMS, np.nan),
        )
