...
#define ACL_CHECK(expr)                                                                \
    do {                                                                               \
        const aclError _r = (expr);                                                    \
        if (_r != ACL_SUCCESS) {                                                       \
            std::fprintf(stderr, "[ERROR] %s:%d acle=%d\n", #expr, __LINE__, (int)_r); \
            rc = 1;                                                                    \
            goto cleanup;                                                              \
        }                                                                              \
    } while (0)
...
void LaunchVshlDeepMerged(
    uint16_t* p0, uint16_t* p1, uint16_t* p2, uint32_t* p3, uint32_t* p4, uint32_t* p5, uint16_t* p6, uint16_t* p7,
    uint16_t* p8, void* stream);
int main()
{
    ...
    fsz = SZ_i32_unsigned;
    FCK(ReadFile("v1_i32_unsigned.bin", fsz, h_i32_unsigned_v1, SZ_i32_unsigned) && fsz == SZ_i32_unsigned,
        "v1_i32_unsigned.bin");
    fsz = SZ_i32_unsigned;
    FCK(ReadFile("v2_i32_unsigned.bin", fsz, h_i32_unsigned_v2, SZ_i32_unsigned) && fsz == SZ_i32_unsigned,
        "v2_i32_unsigned.bin");
    fsz = SZ_i32_unsigned;
    FCK(ReadFile("v3_i32_unsigned.bin", fsz, h_i32_unsigned_v3, SZ_i32_unsigned) && fsz == SZ_i32_unsigned,
        "v3_i32_unsigned.bin");
    fsz = SZ_shift_boundary;
    FCK(ReadFile("v1_shift_boundary.bin", fsz, h_shift_boundary_v1, SZ_shift_boundary) && fsz == SZ_shift_boundary,
        "v1_shift_boundary.bin");
    fsz = SZ_shift_boundary;
    FCK(ReadFile("v2_shift_boundary.bin", fsz, h_shift_boundary_v2, SZ_shift_boundary) && fsz == SZ_shift_boundary,
        "v2_shift_boundary.bin");
    fsz = SZ_shift_boundary;
    FCK(ReadFile("v3_shift_boundary.bin", fsz, h_shift_boundary_v3, SZ_shift_boundary) && fsz == SZ_shift_boundary,
        "v3_shift_boundary.bin");
    ...
    ACL_CHECK(
        aclrtMemcpy(d_i32_unsigned_v1, SZ_i32_unsigned, h_i32_unsigned_v1, SZ_i32_unsigned, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(
        aclrtMemcpy(d_i32_unsigned_v2, SZ_i32_unsigned, h_i32_unsigned_v2, SZ_i32_unsigned, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(
        aclrtMemcpy(d_i32_unsigned_v3, SZ_i32_unsigned, h_i32_unsigned_v3, SZ_i32_unsigned, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(
        d_shift_boundary_v1, SZ_shift_boundary, h_shift_boundary_v1, SZ_shift_boundary, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(
        d_shift_boundary_v2, SZ_shift_boundary, h_shift_boundary_v2, SZ_shift_boundary, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(
        d_shift_boundary_v3, SZ_shift_boundary, h_shift_boundary_v3, SZ_shift_boundary, ACL_MEMCPY_HOST_TO_DEVICE));
    LaunchVshlDeepMerged(
        d_f32_v1, d_f32_v2, d_f32_v3, d_i32_unsigned_v1, d_i32_unsigned_v2, d_i32_unsigned_v3, d_shift_boundary_v1,
        d_shift_boundary_v2, d_shift_boundary_v3, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));
    ACL_CHECK(aclrtMemcpy(h_f32_v3, SZ_f32, d_f32_v3, SZ_f32, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(
        aclrtMemcpy(h_i32_unsigned_v3, SZ_i32_unsigned, d_i32_unsigned_v3, SZ_i32_unsigned, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(
        h_shift_boundary_v3, SZ_shift_boundary, d_shift_boundary_v3, SZ_shift_boundary, ACL_MEMCPY_DEVICE_TO_HOST));
