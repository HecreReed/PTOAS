...
void LaunchMad_mx_acc_kernel(uint8_t* a, uint8_t* b, uint8_t* a_scale, uint8_t* b_scale, float* c, void* stream);
...
int main()
{
    ...
    ACL_CHECK(aclrtMalloc((void**)&aScaleDevice, scaleSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc((void**)&bScaleDevice, scaleSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    inputSize = scaleSize;
    FILE_CHECK(ReadFile("./v4.bin", inputSize, aScaleHost, scaleSize) && inputSize == scaleSize, "./v4.bin");
    inputSize = scaleSize;
    FILE_CHECK(ReadFile("./v5.bin", inputSize, bScaleHost, scaleSize) && inputSize == scaleSize, "./v5.bin");
    inputSize = cSize;
    FILE_CHECK(ReadFile("./v3.bin", inputSize, cHost, cSize) && inputSize == cSize, "./v3.bin");

    ACL_CHECK(aclrtMemcpy(aDevice, aSize, aHost, aSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(bDevice, bSize, bHost, bSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(aScaleDevice, scaleSize, aScaleHost, scaleSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(bScaleDevice, scaleSize, bScaleHost, scaleSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(cDevice, cSize, cHost, cSize, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchMad_mx_acc_kernel(aDevice, bDevice, aScaleDevice, bScaleDevice, cDevice, stream);
