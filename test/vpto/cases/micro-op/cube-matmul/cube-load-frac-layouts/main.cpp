...
static bool writeExact(const char* path, void* src, size_t size) { return WriteFile(path, src, size); }
...
int main()
{
    ...
    ACL_CHECK(aclrtMalloc((void**)&nd2nzCase1LhsDevice, kNd2NzCase1LhsSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc((void**)&nd2nzCase1RhsDevice, kNd2NzCase1RhsSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc((void**)&outNd2nzCase1Device, kNd2NzCase1OutSize, ACL_MEM_MALLOC_HUGE_FIRST));

    FILE_CHECK(readExact("./lhs_nd2nz_case1.bin", nd2nzCase1LhsHost, kNd2NzCase1LhsSize), "./lhs_nd2nz_case1.bin");
    FILE_CHECK(readExact("./src_nd2nz_case1.bin", nd2nzCase1RhsHost, kNd2NzCase1RhsSize), "./src_nd2nz_case1.bin");
    FILE_CHECK(readExact("./out_nd2nz_case1.bin", outNd2nzCase1Host, kNd2NzCase1OutSize), "./out_nd2nz_case1.bin");

    ACL_CHECK(aclrtMemcpy(
        nd2nzCase1LhsDevice, kNd2NzCase1LhsSize, nd2nzCase1LhsHost, kNd2NzCase1LhsSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(
        nd2nzCase1RhsDevice, kNd2NzCase1RhsSize, nd2nzCase1RhsHost, kNd2NzCase1RhsSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(
        outNd2nzCase1Device, kNd2NzCase1OutSize, outNd2nzCase1Host, kNd2NzCase1OutSize, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchCubeLoadFracLayoutsDeepMerged(nd2nzCase1RhsDevice, nd2nzCase1LhsDevice, outNd2nzCase1Device, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));

    ACL_CHECK(aclrtMemcpy(
        outNd2nzCase1Host, kNd2NzCase1OutSize, outNd2nzCase1Device, kNd2NzCase1OutSize, ACL_MEMCPY_DEVICE_TO_HOST));

    FILE_CHECK(writeExact("./out_nd2nz_case1.bin", outNd2nzCase1Host, kNd2NzCase1OutSize), "./out_nd2nz_case1.bin");
