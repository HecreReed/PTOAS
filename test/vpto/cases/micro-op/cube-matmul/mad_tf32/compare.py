...
def compare_bin(golden_path: str, output_path: str) -> bool:
    ...
    if golden.shape != output.shape:
        ...
        return False
    if np.allclose(golden, output, atol=2.5e-3, rtol=0.0):
        return True
    diff = np.where(np.abs(golden - output) > 2.5e-3)[0]
    idx = int(diff[0]) if diff.size else 0
    print(
        f"[ERROR] first mismatch at idx={idx}: golden={float(golden[idx])}, out={float(output[idx])}"
    )
    return False
