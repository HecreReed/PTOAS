...
void LaunchMad_hif8_kernel(
    uint8_t* a, uint8_t* b, float* cHif8, uint8_t* aFp8, uint8_t* bFp8, float* cFp8, void* stream);
...
int main()
{
    ...
    inputSize = cSize;
    FILE_CHECK(ReadFile("./v3.bin", inputSize, cHif8Host, cSize) && inputSize == cSize, "./v3.bin");
    inputSize = cSize;
    FILE_CHECK(ReadFile("./v4.bin", inputSize, cFp8Host, cSize) && inputSize == cSize, "./v4.bin");
    ...
    LaunchMad_hif8_kernel(aDevice, bDevice, cHif8Device, aFp8Device, bFp8Device, cFp8Device, stream);
