...
int main()
{
    ...
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&inputHost), kInputBytes));
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&outputIHost), kOutputBytes));
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&outputSHost), kOutputBytes));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&inputDevice), kInputBytes, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&outputIDevice), kOutputBytes, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&outputSDevice), kOutputBytes, ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    ACL_CHECK(aclrtMemcpy(inputDevice, kInputBytes, inputHost, kInputBytes, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outputIDevice, kOutputBytes, outputIHost, kOutputBytes, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outputSDevice, kOutputBytes, outputSHost, kOutputBytes, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchSprStorePostUpdate(inputDevice, outputIDevice, outputSDevice, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));
    ACL_CHECK(aclrtMemcpy(outputIHost, kOutputBytes, outputIDevice, kOutputBytes, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outputSHost, kOutputBytes, outputSDevice, kOutputBytes, ACL_MEMCPY_DEVICE_TO_HOST));
