...
int main()
{
    ...
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&initialHost), kBufferSize));
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&explicitHost), kBufferSize));
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&rewrittenHost), kBufferSize));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&inputDevice), kBufferSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&initialDevice), kBufferSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&explicitDevice), kBufferSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&rewrittenDevice), kBufferSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    ReadFile("./rewritten_output.bin", rewrittenSize, rewrittenHost, kBufferSize);
    ACL_CHECK(aclrtMemcpy(inputDevice, kBufferSize, inputHost, kBufferSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(initialDevice, kBufferSize, initialHost, kBufferSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(explicitDevice, kBufferSize, explicitHost, kBufferSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(rewrittenDevice, kBufferSize, rewrittenHost, kBufferSize, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchSoftPostUpdateVldusVstus(inputDevice, initialDevice, explicitDevice, rewrittenDevice, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));
    ACL_CHECK(aclrtMemcpy(explicitHost, kBufferSize, explicitDevice, kBufferSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(rewrittenHost, kBufferSize, rewrittenDevice, kBufferSize, ACL_MEMCPY_DEVICE_TO_HOST));
