...
int main()
{
    ...
    FILE_CHECK(ReadFile("./v1.bin", inputFileSize, v1Host, fileSize) && inputFileSize == fileSize, "./v1.bin");
    inputFileSize = fileSize;
    FILE_CHECK(ReadFile("./v2.bin", inputFileSize, v2Host, fileSize) && inputFileSize == fileSize, "./v2.bin");
    inputFileSize = fileSize;
    FILE_CHECK(ReadFile("./v3.bin", inputFileSize, v3Host, fileSize) && inputFileSize == fileSize, "./v3.bin");
    inputFileSize = fileSize;
    FILE_CHECK(ReadFile("./v4.bin", inputFileSize, v4Host, fileSize) && inputFileSize == fileSize, "./v4.bin");

    ACL_CHECK(aclrtMemcpy(v1Device, fileSize, v1Host, fileSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(v2Device, fileSize, v2Host, fileSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(v3Device, fileSize, v3Host, fileSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(v4Device, fileSize, v4Host, fileSize, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchIssue173VstsSignedSignlessDeepMerged(v1Device, v2Device, v3Device, v4Device, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));

    ACL_CHECK(aclrtMemcpy(v2Host, fileSize, v2Device, fileSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(v4Host, fileSize, v4Device, fileSize, ACL_MEMCPY_DEVICE_TO_HOST));
