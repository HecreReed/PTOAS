...
void LaunchSoftPostUpdateAddressUnits(
    float* input, float* output, uint32_t* sprInput, uint32_t* sprOutput, void* stream);
...
int main()
{
    ...
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&sprInputHost), kBufferSize));
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&sprOutputHost), kBufferSize));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&inputDevice), kBufferSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&outputDevice), kBufferSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&sprInputDevice), kBufferSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&sprOutputDevice), kBufferSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    ACL_CHECK(aclrtMemcpy(outputDevice, kBufferSize, outputHost, kBufferSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(sprInputDevice, kBufferSize, sprInputHost, kBufferSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(sprOutputDevice, kBufferSize, sprOutputHost, kBufferSize, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchSoftPostUpdateAddressUnits(inputDevice, outputDevice, sprInputDevice, sprOutputDevice, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));
    ACL_CHECK(aclrtMemcpy(outputHost, kBufferSize, outputDevice, kBufferSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(sprOutputHost, kBufferSize, sprOutputDevice, kBufferSize, ACL_MEMCPY_DEVICE_TO_HOST));
