...
void LaunchCbuf_ubuf_roundtrip_mixed_kernel(int16_t* src, int16_t* dst, void* stream);
...
int main()
{
    ...
    inputSize = bufSize;
    FILE_CHECK(ReadFile("./v1.bin", inputSize, srcHost, bufSize) && inputSize == bufSize, "./v1.bin");
    inputSize = bufSize;
    FILE_CHECK(ReadFile("./v2.bin", inputSize, dstHost, bufSize) && inputSize == bufSize, "./v2.bin");

    ACL_CHECK(aclrtMemcpy(srcDevice, bufSize, srcHost, bufSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(dstDevice, bufSize, dstHost, bufSize, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchCbuf_ubuf_roundtrip_mixed_kernel(srcDevice, dstDevice, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));

    ACL_CHECK(aclrtMemcpy(dstHost, bufSize, dstDevice, bufSize, ACL_MEMCPY_DEVICE_TO_HOST));
