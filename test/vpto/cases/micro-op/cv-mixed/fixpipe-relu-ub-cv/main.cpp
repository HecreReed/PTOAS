...
void LaunchFixpipe_relu_ub_cv_kernel(
    __fp16* src, __fp16* id, __fp16* outUbRelu, __fp16* outUbClip, __fp16* outGmRelu, __fp16* outGmClip,
...
int main()
{
    ...
    for (int index = 3; index <= 8; ++index) {
        ...
        switch (index) {
            case 3:
                hostBuf = outUbReluHost;
                break;
            case 4:
                hostBuf = outUbClipHost;
                break;
            case 5:
                hostBuf = outGmReluHost;
                break;
            case 6:
                hostBuf = outGmClipHost;
                break;
            case 7:
                hostBuf = outL1ReluHost;
                break;
            case 8:
    ...
    ACL_CHECK(aclrtMemcpy(outUbReluDevice, kOutSize, outUbReluHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outUbClipDevice, kOutSize, outUbClipHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outGmReluDevice, kOutSize, outGmReluHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outGmClipDevice, kOutSize, outGmClipHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outL1ReluDevice, kOutSize, outL1ReluHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outL1ClipDevice, kOutSize, outL1ClipHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchFixpipe_relu_ub_cv_kernel(
        srcDevice, idDevice, outUbReluDevice, outUbClipDevice, outGmReluDevice, outGmClipDevice, outL1ReluDevice,
        outL1ClipDevice, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));

    ACL_CHECK(aclrtMemcpy(outUbReluHost, kOutSize, outUbReluDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outUbClipHost, kOutSize, outUbClipDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outGmReluHost, kOutSize, outGmReluDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outGmClipHost, kOutSize, outGmClipDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outL1ReluHost, kOutSize, outL1ReluDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outL1ClipHost, kOutSize, outL1ClipDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
