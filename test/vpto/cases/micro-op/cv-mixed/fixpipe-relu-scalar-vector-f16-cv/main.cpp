...
int main()
{
    ...
    inputSize = kLhsSize;
    FILE_CHECK(ReadFile("./v1.bin", inputSize, lhsHost, kLhsSize) && inputSize == kLhsSize, "./v1.bin");
    inputSize = kRhsSize;
    FILE_CHECK(ReadFile("./v2.bin", inputSize, rhsHost, kRhsSize) && inputSize == kRhsSize, "./v2.bin");
    inputSize = kReluFpSize;
    FILE_CHECK(ReadFile("./v3.bin", inputSize, reluFpHost, kReluFpSize) && inputSize == kReluFpSize, "./v3.bin");

    for (int index = 4; index <= 9; ++index) {
        __fp16* hostBuf = nullptr;
        switch (index) {
            case 4:
                hostBuf = outUbScalarHost;
                break;
            case 5:
                hostBuf = outUbVectorHost;
                break;
            case 6:
                hostBuf = outGmScalarHost;
                break;
            case 7:
                hostBuf = outGmVectorHost;
                break;
            case 8:
                hostBuf = outL1ScalarHost;
                break;
            case 9:
                hostBuf = outL1VectorHost;
                break;
        }
        inputSize = kOutSize;
        char path[16];
        std::snprintf(path, sizeof(path), "./v%d.bin", index);
        FILE_CHECK(ReadFile(path, inputSize, hostBuf, kOutSize) && inputSize == kOutSize, path);
    }
    ...
    ACL_CHECK(aclrtMemcpy(reluFpDevice, kReluFpSize, reluFpHost, kReluFpSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outUbScalarDevice, kOutSize, outUbScalarHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outUbVectorDevice, kOutSize, outUbVectorHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outGmScalarDevice, kOutSize, outGmScalarHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outGmVectorDevice, kOutSize, outGmVectorHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outL1ScalarDevice, kOutSize, outL1ScalarHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outL1VectorDevice, kOutSize, outL1VectorHost, kOutSize, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchFixpipe_relu_scalar_vector_f16_cv_kernel(
        lhsDevice, rhsDevice, reluFpDevice, outUbScalarDevice, outUbVectorDevice, outGmScalarDevice, outGmVectorDevice,
        outL1ScalarDevice, outL1VectorDevice, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));

    ACL_CHECK(aclrtMemcpy(outUbScalarHost, kOutSize, outUbScalarDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outUbVectorHost, kOutSize, outUbVectorDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outGmScalarHost, kOutSize, outGmScalarDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outGmVectorHost, kOutSize, outGmVectorDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outL1ScalarHost, kOutSize, outL1ScalarDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outL1VectorHost, kOutSize, outL1VectorDevice, kOutSize, ACL_MEMCPY_DEVICE_TO_HOST));
