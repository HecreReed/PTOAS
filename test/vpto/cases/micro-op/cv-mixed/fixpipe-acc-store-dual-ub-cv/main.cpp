...
void LaunchFixpipe_acc_store_dual_ub_cv_kernel(
    __fp16* src, __fp16* id, float* outSplitM0, float* outSplitM1, float* outSplitN0, float* outSplitN1, void* stream);
...
int main()
{
    ...
    FILE_CHECK(
        ReadFile("./v3.bin", inputSize, outSplitM0Host, kOutSplitMSize) && inputSize == kOutSplitMSize, "./v3.bin");
    inputSize = kOutSplitMSize;
    FILE_CHECK(
        ReadFile("./v4.bin", inputSize, outSplitM1Host, kOutSplitMSize) && inputSize == kOutSplitMSize, "./v4.bin");
    inputSize = kOutSplitNSize;
    FILE_CHECK(
        ReadFile("./v5.bin", inputSize, outSplitN0Host, kOutSplitNSize) && inputSize == kOutSplitNSize, "./v5.bin");
    inputSize = kOutSplitNSize;
    FILE_CHECK(
        ReadFile("./v6.bin", inputSize, outSplitN1Host, kOutSplitNSize) && inputSize == kOutSplitNSize, "./v6.bin");
    ...
    LaunchFixpipe_acc_store_dual_ub_cv_kernel(
        srcDevice, idDevice, outSplitM0Device, outSplitM1Device, outSplitN0Device, outSplitN1Device, stream);
