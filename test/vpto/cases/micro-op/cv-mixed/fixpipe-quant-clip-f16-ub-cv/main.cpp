...
void LaunchFixpipe_quant_clip_f16_ub_cv_kernel(
    __fp16* src, __fp16* id, uint32_t* fp, __fp16* outUbQuant, __fp16* outUbClip, __fp16* outGmQuant, __fp16* outGmClip,
...
int main()
{
    ...
    inputSize = kSizeId;
    FILE_CHECK(ReadFile("./v1.bin", inputSize, idHost, kSizeId) && inputSize == kSizeId, "./v1.bin");
    inputSize = kSizeSrc;
    FILE_CHECK(ReadFile("./v2.bin", inputSize, srcHost, kSizeSrc) && inputSize == kSizeSrc, "./v2.bin");
    inputSize = kSizeFp;
    FILE_CHECK(ReadFile("./v3.bin", inputSize, fpHost, kSizeFp) && inputSize == kSizeFp, "./v3.bin");
    for (int index = 4; index <= 9; ++index) {
        __fp16* hostBuf = nullptr;
        switch (index) {
            case 4:
                hostBuf = outUbQuantHost;
                break;
            case 5:
                hostBuf = outUbClipHost;
                break;
            case 6:
                hostBuf = outGmQuantHost;
                break;
            case 7:
                hostBuf = outGmClipHost;
                break;
            case 8:
                hostBuf = outL1QuantHost;
                break;
            case 9:
                hostBuf = outL1ClipHost;
                break;
        }
        inputSize = kSizeOut;
        char path[16];
        std::snprintf(path, sizeof(path), "./v%d.bin", index);
        FILE_CHECK(ReadFile(path, inputSize, hostBuf, kSizeOut) && inputSize == kSizeOut, path);
    }
    ...
    ACL_CHECK(aclrtMemcpy(outUbQuantDevice, kSizeOut, outUbQuantHost, kSizeOut, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outUbClipDevice, kSizeOut, outUbClipHost, kSizeOut, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outGmQuantDevice, kSizeOut, outGmQuantHost, kSizeOut, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outGmClipDevice, kSizeOut, outGmClipHost, kSizeOut, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outL1QuantDevice, kSizeOut, outL1QuantHost, kSizeOut, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(outL1ClipDevice, kSizeOut, outL1ClipHost, kSizeOut, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchFixpipe_quant_clip_f16_ub_cv_kernel(
        srcDevice, idDevice, fpDevice, outUbQuantDevice, outUbClipDevice, outGmQuantDevice, outGmClipDevice,
        outL1QuantDevice, outL1ClipDevice, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));

    ACL_CHECK(aclrtMemcpy(outUbQuantHost, kSizeOut, outUbQuantDevice, kSizeOut, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outUbClipHost, kSizeOut, outUbClipDevice, kSizeOut, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outGmQuantHost, kSizeOut, outGmQuantDevice, kSizeOut, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outGmClipHost, kSizeOut, outGmClipDevice, kSizeOut, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outL1QuantHost, kSizeOut, outL1QuantDevice, kSizeOut, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(outL1ClipHost, kSizeOut, outL1ClipDevice, kSizeOut, ACL_MEMCPY_DEVICE_TO_HOST));
