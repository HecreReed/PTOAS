...
void LaunchVmi_vdhist_tail_mask_store_kernel(uint8_t* src, uint16_t* acc, uint16_t* dst, void* stream);
...
int main()
{
    ...
    ACL_CHECK(aclrtMemcpy(accDevice, accBytes, accHost, accBytes, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(dstDevice, dstBytes, dstHost, dstBytes, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchVmi_vdhist_tail_mask_store_kernel(srcDevice, accDevice, dstDevice, stream);
