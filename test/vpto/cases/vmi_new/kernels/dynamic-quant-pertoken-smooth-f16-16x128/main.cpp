...
void LaunchVmi_dynamic_quant_pertoken_smooth_f16_16x128_kernel(
    uint16_t* src, uint16_t* smooth, float* scale, uint8_t* out, void* stream);
...
int main()
{
    ...
    ACL_CHECK(aclrtMemcpy(smoothDevice, smoothBytes, smoothHost, smoothBytes, ACL_MEMCPY_HOST_TO_DEVICE));
    ...
    LaunchVmi_dynamic_quant_pertoken_smooth_f16_16x128_kernel(srcDevice, smoothDevice, scaleDevice, outDevice, stream);
