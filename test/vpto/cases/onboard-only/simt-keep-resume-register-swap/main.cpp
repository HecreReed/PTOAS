...
#define ACL_CHECK(expr)                                                                                         \
    do {                                                                                                        \
        ...
        if (error != ACL_SUCCESS) {                                                                             \
            std::fprintf(                                                                                       \
                stderr, "[ERROR] %s failed: %d (%s:%d)\n", #expr, static_cast<int>(error), __FILE__, __LINE__); \
            rc = 1;                                                                                             \
            goto cleanup;                                                                                       \
        }                                                                                                       \
    } while (0)
...
int main()
{
    ...
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&hostOutput32), output32Bytes));
    ACL_CHECK(aclrtMallocHost(reinterpret_cast<void**>(&hostOutput64), output64Bytes));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&deviceOutput32), output32Bytes, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&deviceOutput64), output64Bytes, ACL_MEM_MALLOC_HUGE_FIRST));

    for (size_t i = 0; i < kOutputElements; ++i) {
        hostOutput32[i] = -1;
        hostOutput64[i] = -1;
    }
    ACL_CHECK(aclrtMemcpy(deviceOutput32, output32Bytes, hostOutput32, output32Bytes, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(deviceOutput64, output64Bytes, hostOutput64, output64Bytes, ACL_MEMCPY_HOST_TO_DEVICE));

    LaunchSimtKeepResumeRegisterSwapKernel(deviceOutput32, deviceOutput64, stream);
    ACL_CHECK(aclrtSynchronizeStream(stream));
    ACL_CHECK(aclrtMemcpy(hostOutput32, output32Bytes, deviceOutput32, output32Bytes, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(hostOutput64, output64Bytes, deviceOutput64, output64Bytes, ACL_MEMCPY_DEVICE_TO_HOST));

    output32File = std::fopen("./output_i32.bin", "wb");
    if (!output32File || std::fwrite(hostOutput32, 1, output32Bytes, output32File) != output32Bytes) {
    ...
    if (!output64File || std::fwrite(hostOutput64, 1, output64Bytes, output64File) != output64Bytes) {
    ...
    for (int32_t tid = 0; tid < static_cast<int32_t>(kThreads); ++tid) {
        ...
        if (actual32_0 != expected32_0 || actual32_1 != expected32_1 || actual64_0 != expected64_0 ||
            actual64_1 != expected64_1) {
            std::fprintf(
                stderr, "[ERROR] lane %d: got i32=[%d, %d], i64=[%lld, %lld]\n", tid, actual32_0, actual32_1,
                static_cast<long long>(actual64_0), static_cast<long long>(actual64_1));
            rc = 2;
            goto cleanup;
        }
    }
