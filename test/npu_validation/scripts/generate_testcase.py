...
def _detect_prefetch_workspace_pointer_params(text: str, pointer_param_names):
    ...
    for m in re.finditer(
        r"\bPrefetchAsyncContext\s+\w+\s*=\s*[^;]*\(([^)]*)\)\s*;", text, re.S
    ):
        raw_arg = m.group(1).strip()
        arg_name = _extract_identifier(raw_arg)
        if not arg_name:
...
def _infer_gm_pointer_elem_counts(
    ...
    def record_global_tensor_footprint(
        gt_object: str, base_ptr_expr: str, shape_dims, stride_dims
    ):
...
def generate_testcase(
    ...
    uses_prefetch_async_runtime = (
        bool(prefetch_workspace_param_names)
        and "TPREFETCH_ASYNC(" in raw_kernel_for_analysis
    )
    ...
    is_a5_soc = (
        "950" in (soc_version or "").lower() or "a5" in (soc_version or "").lower()
    )

    if (
        uses_prefetch_async_runtime
        and (not is_a5_soc)
        and aicore_arch.startswith("dav-c310")
    ):
    ...
    for writer_text in kernel_info["writer_texts"]:
        output_param_names.extend(
            _detect_output_pointer_params(writer_text, non_runtime_pointer_param_names)
        )
                    ...
                    "role": (
                        ...
                        if name in ffts_param_names
                            ...
                            if name in prefetch_workspace_param_names
                                ...
                                "output" if name in output_param_name_set else "input"
                            )
                        )
                    ),
                }
    ...
    data_ptrs = [
        p
        for p in params
        if p["kind"] == "ptr" and p["role"] not in {"ffts", "prefetch_workspace"}
    ]
    ...
    if prefetch_workspace_ptrs:
        ...
        param_decls_lines.append(
            '    const char *sdmaSocVersion = std::getenv("SOC_VERSION");'
        )
        param_decls_lines.append(
            '    const char *ptoasBoardIsA3 = std::getenv("PTOAS_BOARD_IS_A3");'
        )
        param_decls_lines.append(
        ...
        init_runtime_ptrs.append("    }")
        init_runtime_ptrs.append(
            "    if (!skipSdmaWorkspaceInit && !sdmaWorkspaceOk) {"
        )
        init_runtime_ptrs.append(
        ...
        for p in prefetch_workspace_ptrs:
            ...
            init_runtime_ptrs.append(
                f"    if (sdmaWorkspaceOk && {p['name']}Device == nullptr) {{"
            )
            init_runtime_ptrs.append(
