...
def main():
    ...
    a_scale = np.random.randint(127, 130, [M, ceil_div(K_ALIGNED, 32)]).astype(np.uint8)
    b_scale = np.random.randint(127, 130, [ceil_div(K_ALIGNED, 32), N]).astype(np.uint8)

    a_full = np.zeros([M, K_ALIGNED], dtype=np.float64)
    b_full = np.zeros([K_ALIGNED, N], dtype=np.float64)
    ...
    golden[:M, :N] = np.matmul(a_full[:, :K], b_full[:K, :]).astype(np.float32)
    golden.tofile("golden.bin")

    convert_scale_a_format(a_scale).astype(np.uint8).tofile("scale1.bin")
