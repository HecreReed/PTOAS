        ...
        with Location.unknown(ctx):
            ...
            with InsertionPoint(entry):
                ...
                tv_src = pto.MakeTensorViewOp(
                    pto.TensorViewType.get(2, f32, ctx), gm_src, [c32, c32], [c32, c1]
                ).result
                tv_dst = pto.MakeTensorViewOp(
                    pto.TensorViewType.get(2, f32, ctx), gm_dst, [c32, c32], [c32, c1]
                ).result
                sv_src = pto.PartitionViewOp(
                    pto.PartitionTensorViewType.get([32, 32], f32, ctx),
                    tv_src,
                    offsets=[c0, c0],
                    sizes=[c32, c32],
                ).result
