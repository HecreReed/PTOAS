...
for _tile, _valid, _np_dtype, _upper, _diag in _TRIL_CASES:
    ...
    def _make(
        ...
        def _kernel(out_ptr: pto.ptr(pdsl, "gm")):
            ...
            out_tile = pto.alloc_tile(shape=[tr, ac], dtype=pdsl, valid_shape=[vr, vc])
    ...
    def _expected(
        np_dtype=_np_dtype, vs=_valid, up=_upper, dg=_diag, out_dtype=_out_dtype
    ):
        vr, vc = vs
        data = np.zeros((vr, vc), dtype=out_dtype)
        mask = (np.triu if up == "upper" else np.tril)(
            np.ones((vr, vc), dtype=out_dtype), k=dg
        )
        data[:vr, :vc] = mask
        return data
