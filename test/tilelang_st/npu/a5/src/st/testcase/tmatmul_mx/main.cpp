...
#define ACL_CHECK(expr)                                                                                        \
    do {                                                                                                       \
        ...
        if (_ret != ACL_SUCCESS) {                                                                             \
            std::fprintf(                                                                                      \
                stderr, "[ERROR] %s failed: %d (%s:%d)\n", #expr, static_cast<int>(_ret), __FILE__, __LINE__); \
...
void LaunchTMATMUL_MX_fp8_e5m2_128x64x64(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
void LaunchTMATMUL_MX_fp8_e4m3_127x72x64(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
void LaunchTMATMUL_MX_fp8_e4m3_e5m2_128x110x63(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
void LaunchTMATMUL_MX_fp4_e2m1_128x64x64(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
void LaunchTMATMUL_MX_fp4_e1m2_e2m1_117x64x60(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
void LaunchTMATMUL_MX_fp4_e2m1_e1m2_128x118x64(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
void LaunchTMATMUL_MX_fp4_e2m1_e1m2_115x64x30(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
void LaunchTMATMUL_MX_fp8_e4m3_16x32x16(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
void LaunchTMATMUL_MX_fp8_e4m3_e5m2_10x50x54(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
void LaunchTMATMUL_MX_fp4_e2m1_4x30x8(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* c, void* stream);
...
static constexpr size_t ceil_align(size_t num, size_t align) { return (num + align - 1) / align * align; }

static constexpr size_t ceil_div(size_t num, size_t div) { return (num + div - 1) / div; }
...
static const TestCase kCases[] = {
    ...
    {"fp8_e4m3_e5m2_128x110x63", false, false, 128, 110, 63, 128, 64, 128, LaunchTMATMUL_MX_fp8_e4m3_e5m2_128x110x63,
     nullptr},
    {"fp4_e2m1_128x64x64", false, true, 128, 64, 64, 128, 64, 64, LaunchTMATMUL_MX_fp4_e2m1_128x64x64, nullptr},
    {"fp4_e1m2_e2m1_117x64x60", false, true, 117, 64, 60, 128, 64, 64, LaunchTMATMUL_MX_fp4_e1m2_e2m1_117x64x60,
     nullptr},
    {"fp4_e2m1_e1m2_128x118x64", false, true, 128, 118, 64, 128, 64, 128, LaunchTMATMUL_MX_fp4_e2m1_e1m2_128x118x64,
     nullptr},
    {"fp4_e2m1_e1m2_115x64x30", false, true, 115, 64, 30, 128, 64, 64, LaunchTMATMUL_MX_fp4_e2m1_e1m2_115x64x30,
     nullptr},
...
static int RunCase(const TestCase& tc, int deviceId, aclrtStream stream)
{
    ...
    const size_t scaleBBytes =
        tc.is_fp4 ? packedScaleBFp4Bytes(tc.n_padded, tc.k_aligned) : packedScaleBBytes(tc.n, tc.k_aligned);
    ...
    std::printf(
        "[INFO] === case: %s (m=%zu, k=%zu, n=%zu, is_bias=%d, is_fp4=%d) ===\n", tc.name, tc.m, tc.k, tc.n, tc.is_bias,
        tc.is_fp4);

    std::string caseDir = getCaseRoot() + "/" + tc.name;

    void *aHost = nullptr, *bHost = nullptr, *scaleAHost = nullptr, *scaleBHost = nullptr, *biasHost = nullptr,
         *outHost = nullptr;
    void *aDevice = nullptr, *bDevice = nullptr, *scaleADevice = nullptr, *scaleBDevice = nullptr,
         *biasDevice = nullptr, *outDevice = nullptr;
    ...
    if (!ReadFile((caseDir + "/input1.bin").c_str(), aFileSize, aHost, aBytes)) {
        std::fprintf(stderr, "[ERROR] read input1 failed\n");
        rc = 1;
    }
    if (rc == 0 && !ReadFile((caseDir + "/input2.bin").c_str(), bFileSize, bHost, bBytes)) {
        std::fprintf(stderr, "[ERROR] read input2 failed\n");
        rc = 1;
    }
    if (rc == 0 && !ReadFile((caseDir + "/scale1.bin").c_str(), scaleAFileSize, scaleAHost, scaleABytes)) {
        std::fprintf(stderr, "[ERROR] read scale1 failed\n");
        rc = 1;
    }
    if (rc == 0 && !ReadFile((caseDir + "/scale2.bin").c_str(), scaleBFileSize, scaleBHost, scaleBBytes)) {
        std::fprintf(stderr, "[ERROR] read scale2 failed\n");
        rc = 1;
    }
    if (rc == 0 && tc.is_bias && !ReadFile((caseDir + "/bias.bin").c_str(), biasFileSize, biasHost, biasBytes)) {
        std::fprintf(stderr, "[ERROR] read bias failed\n");
        rc = 1;
    }
    ...
    if (rc == 0) {
        ...
        if (tc.is_bias) {
            tc.launch_bias(
                static_cast<uint8_t*>(aDevice), static_cast<uint8_t*>(bDevice), static_cast<uint8_t*>(scaleADevice),
                static_cast<uint8_t*>(scaleBDevice), static_cast<float*>(biasDevice), static_cast<float*>(outDevice),
                stream);
        } else {
            tc.launch_no_bias(
                static_cast<uint8_t*>(aDevice), static_cast<uint8_t*>(bDevice), static_cast<uint8_t*>(scaleADevice),
                static_cast<uint8_t*>(scaleBDevice), static_cast<float*>(outDevice), stream);
        }
