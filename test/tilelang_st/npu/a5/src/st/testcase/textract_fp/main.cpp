...
static int RunCase(const TestCase& tc, int deviceId, aclrtStream stream)
{
    ...
    std::printf(
        ...
        tc.aCols, tc.bRows, tc.bCols, tc.fbRows, tc.fbCols, tc.idRows, tc.idCols, tc.outRows, tc.outCols);
    ...
    if (rc == 0) {
        ...
        tc.launch(
            static_cast<uint16_t*>(aDevice), static_cast<uint16_t*>(bDevice), static_cast<float*>(fbDevice),
