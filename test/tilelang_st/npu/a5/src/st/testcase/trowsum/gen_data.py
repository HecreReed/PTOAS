...
for case in CASES:
    ...
    if np.issubdtype(dtype, np.integer):
        if dtype == np.int32:
            input_arr = np.random.randint(low=-100, high=100, size=(row, col)).astype(
                dtype
            )
        elif dtype == np.int16:
            if case.get("overflow"):
                ...
                input_arr = np.random.randint(low=-50, high=50, size=(row, col)).astype(
                    dtype
                )
        else:
            input_arr = np.random.randint(low=-10, high=10, size=(row, col)).astype(
                dtype
            )
    else:
    ...
    output_arr = np.zeros(
        (row,), dtype=np.int64 if np.issubdtype(dtype, np.integer) else np.float64
    )
    ...
    print(
        f"[INFO] gen_data: {case['name']} shape=({row},{col}) valid=({valid_row},{valid_col}) dtype={dtype.__name__}"
    )
