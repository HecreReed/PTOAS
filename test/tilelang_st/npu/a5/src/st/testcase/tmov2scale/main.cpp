...
static int RunCase(const TestCase& tc, int deviceId, aclrtStream stream)
{
    ...
    std::printf(
        "[INFO] === case: %s (lhs=%zux%zu, rhs=%zux%zu, scale=%zux%zu, out=%zux%zu) ===\n", tc.name, tc.lhsRows,
        tc.lhsCols, tc.rhsRows, tc.rhsCols, tc.scaleRows, tc.scaleCols, tc.outRows, tc.outCols);
    ...
    if (rc == 0) {
        ...
        tc.launch(
            static_cast<uint16_t*>(lhsDevice), static_cast<uint16_t*>(rhsDevice), static_cast<float*>(scaleDevice),
...
int main(int argc, char* argv[])
{
    ...
    for (size_t i = 0; i < kNumCases; ++i) {
        if (caseFilter && std::strcmp(kCases[i].name, caseFilter) != 0)
            continue;
        if (RunCase(kCases[i], deviceId, stream) != 0) {
            rc = 1;
            break;
        }
    }
