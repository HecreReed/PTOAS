...
void LaunchTMATMUL_MX_bias_fp8_e5m2_e4m3_115x64x30(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* bias, float* c, void* stream);
void LaunchTMATMUL_MX_bias_fp8_e4m3_200x192x95(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* bias, float* c, void* stream);
void LaunchTMATMUL_MX_bias_fp4_e2m1_e1m2_35x128x56(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* bias, float* c, void* stream);
void LaunchTMATMUL_MX_bias_fp4_e1m2_47x128x62(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* bias, float* c, void* stream);
void LaunchTMATMUL_MX_bias_fp8_e4m3_e5m2_64x192x64(
    uint8_t* a, uint8_t* b, uint8_t* scale_a, uint8_t* scale_b, float* bias, float* c, void* stream);
...
static constexpr size_t ubBiasCols(size_t n_padded) { return ceil_align(n_padded, 64); }
...
static int RunCase(const TestCase& tc, int deviceId, aclrtStream stream)
{
    ...
    const size_t scaleBBytes =
        tc.is_fp4 ? packedScaleBFp4Bytes(tc.n_padded, tc.k_aligned) : packedScaleBBytes(tc.n_padded, tc.k_aligned);
    ...
    if (rc == 0 && !ReadFile((caseDir + "/bias.bin").c_str(), biasFileSize, biasHost, biasBytes)) {
