...
"""Generate golden data for tfillpad test cases.

TFILLPAD semantics:
  1. Copy src.valid_shape data to dst
  2. Fill cols from src.valid_cols to dst.cols with FillPadVal
  3. Fill rows from src.rows to dst.rows with FillPadVal

Note: LoadPadVal is used by TLOAD only, TFILLPAD uses FillPadVal for expansion.
"""
...
for case in CASES:
    ...
    dst_valid = case["valid_shape"]
    src_shape = case.get("src_shape", dst_shape)
    src_valid = case.get("src_valid_shape", dst_valid)
    ...
    # Step 2: TFILLPAD fills cols from src_valid_cols to dst_cols with FillPadVal
    # (NOT LoadPadVal! TFILLPAD uses FillPadVal for expansion)
    if dst_vc > src_vc:
        fill_val = get_pad_value(dtype, fill_padval)
        golden[:dst_vr, src_vc:dst_vc] = fill_val

    # Step 3: TFILLPAD fills rows from src_rows to dst_rows with FillPadVal
