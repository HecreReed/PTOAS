...
static int RunCase(const TestCase& tc, int deviceId, aclrtStream stream)
{
    ...
    std::printf(
        "[INFO] === case: %s (lhs=%zux%zu, rhs=%zux%zu, bias=%zux%zu, out=%zux%zu) ===\n", tc.name, tc.lhsRows,
        tc.lhsCols, tc.rhsRows, tc.rhsCols, tc.biasRows, tc.biasCols, tc.outRows, tc.outCols);
    ...
    if (rc == 0) {
        ...
        tc.launch(
            static_cast<uint16_t*>(lhsDevice), static_cast<uint16_t*>(rhsDevice), static_cast<float*>(biasDevice),
