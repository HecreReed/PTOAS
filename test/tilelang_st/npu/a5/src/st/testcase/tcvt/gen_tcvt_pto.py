...
def gen_kernel(case, idx=0):
    ...
    tile_valid = (
        "" if v_rows == rows and v_cols == cols else f", valid={v_rows}x{v_cols}"
    )
    dst_tile_valid = (
        ""
        if dst_v_rows == dst_rows and dst_v_cols == dst_cols
        else f", valid={dst_v_rows}x{dst_v_cols}"
    )
    ...
    const_defs = [
        f"    %c{i:<{longest_const}} = arith.constant {i:<{longest_const}} : index"
        for i in const_vals
    ]
    ...
    lines.extend(
        [
            ...
            f"              outs(%src : {tile_src})",
            "",
            f"    pto.tcvt ins(%src {{rmode = {gen_rmode_attr(rmode)}}} : {tile_src})"
            if rmode != "RINT"
            else f"    pto.tcvt ins(%src : {tile_src})",
            f"             outs(%dst : {tile_dst})",
            "",
