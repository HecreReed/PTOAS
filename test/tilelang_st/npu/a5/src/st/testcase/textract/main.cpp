...
static int RunCase(const TestCase& tc, int deviceId, aclrtStream stream)
{
    ...
    std::printf(
        "[INFO] === case: %s (i1=%zux%zu, i2=%zux%zu, out=%zux%zu) ===\n", tc.name, tc.input1Rows, tc.input1Cols,
        tc.input2Rows, tc.input2Cols, tc.outRows, tc.outCols);
    ...
    if (rc == 0) {
        ...
        tc.launch(
            static_cast<uint16_t*>(i1Device), static_cast<uint16_t*>(i2Device), static_cast<float*>(outDevice), stream);
    ...
    if (outDevice != nullptr)
        aclrtFree(outDevice);
    if (i1Host != nullptr)
        aclrtFreeHost(i1Host);
    if (i2Host != nullptr)
        aclrtFreeHost(i2Host);
    if (outHost != nullptr)
