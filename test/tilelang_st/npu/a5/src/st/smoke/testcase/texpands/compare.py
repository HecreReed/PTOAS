...
"""Compare output against golden for texpands test cases."""
...
def main():
    ...
    for case in CASES:
        ...
        if eps == 0:
            ...
            if not np.array_equal(golden[:vr, :vc], output[:vr, :vc]):
                diff = golden[:vr, :vc] - output[:vr, :vc]
